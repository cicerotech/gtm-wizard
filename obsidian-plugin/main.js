...
`}var X={serverUrl:"https://gtm-wizard.onrender.com",accountsFolder:"Accounts",recordingsFolder:"Recordings",syncOnStartup:!0,autoSyncAfterTranscription:!0,saveAudioFiles:!0,appendTranscript:!0,lastSyncTime:null,cachedAccounts:[],enableSmartTags:!0,showCalendarView:!0,userEmail:"",setupCompleted:!1,calendarConfigured:!1,salesforceConnected:!1,accountsImported:!1,importedAccountCount:0,openaiApiKey:""},w="eudia-calendar-view",S="eudia-setup-view",P=class extends c.EditorSuggest{constructor(n,e){super(n),this.plugin=e}onTrigger(n,e,t){let i=e.getLine(n.line),a=e.getValue(),s=e.posToOffset(n),r=a.indexOf("---"),o=a.indexOf("---",r+3);if(r===-1||s<r||s>o)return null;let u=i.match(/^account:\s*(.*)$/);if(!u)return null;let m=u[1].trim(),l=i.indexOf(":")+1,p=i.substring(l).match(/^\s*/)?.[0].length||0;return{start:{line:n.line,ch:l+p},end:n,query:m}}getSuggestions(n){let e=n.query.toLowerCase(),t=this.plugin.settings.cachedAccounts;return e?t.filter(i=>i.name.toLowerCase().includes(e)).sort((i,a)=>{let s=i.name.toLowerCase().startsWith(e),r=a.name.toLowerCase().startsWith(e);return s&&!r?-1:r&&!s?1:i.name.localeCompare(a.name)}).slice(0,10):t.slice(0,10)}renderSuggestion(n,e){e.createEl("div",{text:n.name,cls:"suggestion-title"})}selectSuggestion(n,e){this.context&&this.context.editor.replaceRange(n.name,this.context.start,this.context.end)}},N=class{constructor(n,e,t,i){this.containerEl=null;this.waveformBars=[];this.durationEl=null;this.waveformData=new Array(16).fill(0);this.onPause=n,this.onResume=e,this.onStop=t,this.onCancel=i}show(){if(this.containerEl)return;this.containerEl=document.createElement("div"),this.containerEl.className="eudia-transcription-bar active";let n=document.createElement("div");n.className="eudia-recording-dot",this.containerEl.appendChild(n);let e=document.createElement("div");e.className="eudia-waveform",this.waveformBars=[];for(let s=0;s<16;s++){let r=document.createElement("div");r.className="eudia-waveform-bar",r.style.height="2px",e.appendChild(r),this.waveformBars.push(r)}this.containerEl.appendChild(e),this.durationEl=document.createElement("div"),this.durationEl.className="eudia-duration",this.durationEl.textContent="0:00",this.containerEl.appendChild(this.durationEl);let t=document.createElement("div");t.className="eudia-controls-minimal";let i=document.createElement("button");i.className="eudia-control-btn stop",i.innerHTML='<span class="eudia-stop-icon"></span>',i.title="Stop and summarize",i.onclick=()=>this.onStop(),t.appendChild(i);let a=document.createElement("button");a.className="eudia-control-btn cancel",a.textContent="Cancel",a.onclick=()=>this.onCancel(),t.appendChild(a),this.containerEl.appendChild(t),document.body.appendChild(this.containerEl)}hide(){this.containerEl&&(this.containerEl.remove(),this.containerEl=null,this.waveformBars=[],this.durationEl=null)}updateState(n){if(this.containerEl){if(this.waveformData.shift(),this.waveformData.push(n.audioLevel),this.waveformBars.forEach((e,t)=>{let i=this.waveformData[t]||0,a=Math.max(2,Math.min(24,i*.24));e.style.height=`${a}px`}),this.durationEl){let e=Math.floor(n.duration/60),t=Math.floor(n.duration%60);this.durationEl.textContent=`${e}:${t.toString().padStart(2,"0")}`}this.containerEl.className=n.isPaused?"eudia-transcription-bar paused":"eudia-transcription-bar active"}}showProcessing(){if(!this.containerEl)return;this.containerEl.innerHTML="",this.containerEl.className="eudia-transcription-bar processing";let n=document.createElement("div");n.className="eudia-processing-spinner",this.containerEl.appendChild(n);let e=document.createElement("div");e.className="eudia-processing-text",e.textContent="Processing...",this.containerEl.appendChild(e)}showComplete(n){if(!this.containerEl)return;this.containerEl.innerHTML="",this.containerEl.className="eudia-transcription-bar complete";let e=document.createElement("div");e.className="eudia-complete-checkmark",this.containerEl.appendChild(e);let t=document.createElement("div");if(t.className="eudia-complete-content",n.summaryPreview){let o=document.createElement("div");o.className="eudia-summary-preview",o.textContent=n.summaryPreview.length>80?n.summaryPreview.substring(0,80)+"...":n.summaryPreview,t.appendChild(o)}let i=document.createElement("div");i.className="eudia-complete-stats-row";let a=Math.floor(n.duration/60),s=Math.floor(n.duration%60);i.textContent=`${a}:${s.toString().padStart(2,"0")} recorded`,n.nextStepsCount>0&&(i.textContent+=` | ${n.nextStepsCount} action${n.nextStepsCount>1?"s":""}`),n.meddiccCount>0&&(i.textContent+=` | ${n.meddiccCount} signals`),t.appendChild(i),this.containerEl.appendChild(t);let r=document.createElement("button");r.className="eudia-control-btn close",r.textContent="Dismiss",r.onclick=()=>this.hide(),this.containerEl.appendChild(r),setTimeout(()=>this.hide(),8e3)}};var $=class extends c.Modal{constructor(n,e,t){super(n),this.plugin=e,this.onSelect=t}onOpen(){let{contentEl:n}=this;n.empty(),n.addClass("eudia-account-selector"),n.createEl("h3",{text:"Select Account for Meeting Note"}),this.searchInput=n.createEl("input",{type:"text",placeholder:"Search accounts..."}),this.searchInput.style.cssText="width: 100%; padding: 10px; margin-bottom: 10px; border-radius: 6px; border: 1px solid var(--background-modifier-border);",this.resultsContainer=n.createDiv({cls:"eudia-account-results"}),this.resultsContainer.style.cssText="max-height: 300px; overflow-y: auto;",this.updateResults(""),this.searchInput.addEventListener("input",()=>this.updateResults(this.searchInput.value)),this.searchInput.focus()}updateResults(n){this.resultsContainer.empty();let e=this.plugin.settings.cachedAccounts,t=n?e.filter(i=>i.name.toLowerCase().includes(n.toLowerCase())).slice(0,15):e.slice(0,15);if(t.length===0){this.resultsContainer.createDiv({cls:"eudia-no-results",text:"No accounts found"});return}t.forEach(i=>{let a=this.resultsContainer.createDiv({cls:"eudia-account-item",text:i.name});a.onclick=()=>{this.onSelect(i),this.close()}})}onClose(){this.contentEl.empty()}};var W=class extends c.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new k(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return S}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,c.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}this.plugin.settings.accountsImported&&(this.steps[2].status="complete")}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),i=t.createDiv({cls:"eudia-setup-title-section"});i.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),i.createEl("p",{text:"Complete these steps to unlock your sales superpowers",cls:"eudia-setup-subtitle"});let a=t.createDiv({cls:"eudia-setup-progress-section"}),s=this.getCompletionPercentage(),r=a.createDiv({cls:"eudia-setup-progress-label"});r.createSpan({text:"Setup Progress"}),r.createSpan({text:`${s}%`,cls:"eudia-setup-progress-value"});let u=a.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});u.style.width=`${s}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],i=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=i.createDiv({cls:"eudia-setup-step-header"});a.createDiv({cls:"eudia-setup-step-number"}).setText(t.status==="complete"?"\u2713":"1");let r=a.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=i.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let u=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=u.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let m=u.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});m.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async l=>{l.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let i=await(0,c.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(i.status===200&&i.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new c.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");try{let a=await this.accountOwnershipService.getAccountsForUser(e);a.length>0&&(await this.plugin.createTailoredAccountFolders(a),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,await this.plugin.saveSettings(),new c.Notice(`Imported ${a.length} account folders!`))}catch(a){console.error("[Eudia] Account import failed:",a)}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],i=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=i.createDiv({cls:"eudia-setup-step-header"});a.createDiv({cls:"eudia-setup-step-number"}).setText(t.status==="complete"?"\u2713":"2");let r=a.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=i.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let m=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),l=o.createDiv({cls:"eudia-setup-sf-status"});m.onclick=async()=>{let p=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(p,"_blank"),l.textContent="Complete the login in the popup window...",l.className="eudia-setup-sf-status loading",new c.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(l)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,i=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,c.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new c.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=i&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=await this.accountOwnershipService.getAccountsForUser(this.plugin.settings.userEmail);if(t.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}await this.plugin.createTailoredAccountFolders(t),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=t.length,await this.plugin.saveSettings(),this.steps[2].status="complete",e.textContent=`${t.length} accounts imported successfully!`,e.className="eudia-setup-sf-status success"}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error"}}renderTranscribeStep(e){let t=this.steps[2],i=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=i.createDiv({cls:"eudia-setup-step-header"});a.createDiv({cls:"eudia-setup-step-number"}).setText(t.status==="complete"?"\u2713":"3");let r=a.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=i.createDiv({cls:"eudia-setup-step-content"}),u=o.createDiv({cls:"eudia-setup-instructions"}),m=u.createDiv({cls:"eudia-setup-instruction"});m.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),m.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let l=u.createDiv({cls:"eudia-setup-instruction"});l.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),l.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let p=u.createDiv({cls:"eudia-setup-instruction"});p.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),p.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(s=>s.status==="complete")){let s=t.createDiv({cls:"eudia-setup-completion"});s.createEl("h2",{text:"\u{1F389} You're all set!"}),s.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let r=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});r.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(S),await this.plugin.activateCalendarView()}}else{let s=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});s.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(S),new c.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let a=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});a.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},L=class extends c.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return w}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),i=t.createDiv({cls:"eudia-calendar-title-row"});i.createEl("h4",{text:"Your Meetings"});let a=i.createDiv({cls:"eudia-calendar-actions"}),s=a.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});s.title="Refresh",s.onclick=async()=>{s.addClass("spinning"),await this.render(),s.removeClass("spinning")};let r=a.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});r.title="Settings",r.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},i=this.plugin.settings.serverUrl,a=this.plugin.settings.userEmail;try{(await(0,c.requestUrl)({url:`${i}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(a&&t.server==="connected")try{let l=await(0,c.requestUrl)({url:`${i}/api/calendar/validate/${encodeURIComponent(a)}`,method:"GET",throw:!1});l.status===200&&l.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(a&&t.server==="connected")try{let l=await(0,c.requestUrl)({url:`${i}/api/sf/auth/status?email=${encodeURIComponent(a)}`,method:"GET",throw:!1});l.status===200&&l.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let s=e.createDiv({cls:"eudia-status-indicators"}),r=s.createSpan({cls:`eudia-status-dot ${t.server}`});r.title=t.serverMessage||"Server";let o=s.createSpan({cls:`eudia-status-dot ${t.calendar}`});o.title=t.calendarMessage||"Calendar";let u=s.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(u.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let l=e.createDiv({cls:"eudia-status-warning"});l.innerHTML=`<strong>${a}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),i=t.createDiv({cls:"eudia-calendar-loading"});i.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let a=new A(this.plugin.settings.serverUrl,this.plugin.settings.userEmail),s=await a.getWeekMeetings();if(i.remove(),!s.success){this.renderError(t,s.error||"Failed to load calendar");return}let r=Object.keys(s.byDay||{}).sort();if(r.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,a);for(let o of r){let u=s.byDay[o];!u||u.length===0||this.renderDaySection(t,o,u)}}catch(a){i.remove(),this.renderError(t,a.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let i=await t.getCurrentMeeting();if(i.meeting){let a=e.createDiv({cls:"eudia-now-card"});i.isNow?a.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):a.createDiv({cls:"eudia-now-badge soon",text:`In ${i.minutesUntilStart}m`});let s=a.createDiv({cls:"eudia-now-content"});s.createEl("div",{cls:"eudia-now-subject",text:i.meeting.subject}),i.meeting.accountName&&s.createEl("div",{cls:"eudia-now-account",text:i.meeting.accountName});let r=a.createEl("button",{cls:"eudia-now-action",text:"Create Note"});r.onclick=()=>this.createNoteForMeeting(i.meeting)}}catch{}}renderDaySection(e,t,i){let a=e.createDiv({cls:"eudia-calendar-day"});a.createEl("div",{cls:"eudia-calendar-day-header",text:A.getDayName(t)});for(let s of i){let r=a.createDiv({cls:`eudia-calendar-meeting ${s.isCustomerMeeting?"customer":"internal"}`});r.createEl("div",{cls:"eudia-calendar-time",text:A.formatTime(s.start)});let o=r.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:s.subject}),s.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:s.accountName});else if(s.attendees&&s.attendees.length>0){let u=s.attendees.slice(0,2).map(m=>m.name||m.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:u})}r.onclick=()=>this.createNoteForMeeting(s),r.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `;let i=t.createDiv({cls:"eudia-setup-input-group"}),a=i.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});a.addClass("eudia-setup-email");let s=i.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),r=t.createDiv({cls:"eudia-setup-status"});s.onclick=async()=>{let o=a.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){r.textContent="Please use your @eudia.com email",r.className="eudia-setup-status error";return}s.disabled=!0,s.textContent="Connecting...",r.textContent="";try{if(!(await(0,c.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){r.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,r.className="eudia-setup-status error",s.disabled=!1,s.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),r.textContent="Connected",r.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(u){let m=u.message||"Connection failed";m.includes("403")?r.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:r.textContent=m,r.className="eudia-setup-status error",s.disabled=!1,s.textContent="Connect"}},a.onkeydown=o=>{o.key==="Enter"&&s.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractAccountFromAttendees(e){if(!e||e.length===0)return null;let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com"],i=[];for(let o of e){if(!o.email)continue;let m=o.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(m){let l=m[1];!l.includes("eudia.com")&&!t.includes(l)&&i.push(l)}}if(i.length===0)return null;let a=i[0],s=a.split(".")[0],r=s.charAt(0).toUpperCase()+s.slice(1);return console.log(`[Eudia Calendar] Extracted company "${r}" from attendee domain ${a}`),r}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let a=(t[1]||t[2]||"").trim();if(a.toLowerCase()!=="eudia")return a}let i=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(i){let s=(i[1]||i[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(s.toLowerCase()!=="eudia"&&s.length>0)return s}if(!e.toLowerCase().includes("eudia")){let a=e.match(/^([^-–|]+)/);if(a){let s=a[1].trim();if(s.length>2&&s.length<50)return s}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",i=this.app.vault.getAbstractFileByPath(t);if(!(i instanceof c.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let a=e.toLowerCase().trim(),s=[];for(let p of i.children)p instanceof c.TFolder&&s.push(p.name);console.log(`[Eudia Calendar] Searching for "${a}" in ${s.length} folders`);let r=s.find(p=>p.toLowerCase()===a);if(r)return console.log(`[Eudia Calendar] Exact match found: ${r}`),`${t}/${r}`;let o=s.find(p=>p.toLowerCase().startsWith(a));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${t}/${o}`;let u=s.find(p=>a.startsWith(p.toLowerCase()));if(u)return console.log(`[Eudia Calendar] Search starts with folder match: ${u}`),`${t}/${u}`;let m=s.find(p=>{let d=p.toLowerCase();return d.length>=3&&a.includes(d)});if(m)return console.log(`[Eudia Calendar] Search contains folder match: ${m}`),`${t}/${m}`;let l=s.find(p=>{let d=p.toLowerCase();return a.length>=3&&d.includes(a)});return l?(console.log(`[Eudia Calendar] Folder contains search match: ${l}`),`${t}/${l}`):(console.log(`[Eudia Calendar] No folder match found for "${a}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],i=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),a=`${t} - ${i}.md`,s=null,r=e.accountName||null,o=null;if(console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(d=>d.email)||[])}`),!s&&e.attendees&&e.attendees.length>0){let d=this.extractAccountFromAttendees(e.attendees);console.log(`[Eudia Calendar] Extracted domain company name: "${d||"none"}"`),d&&(s=this.findAccountFolder(d),console.log(`[Eudia Calendar] Domain-based "${d}" -> folder: ${s||"not found"}`),s&&!r&&(r=s.split("/").pop()||d))}if(!s&&e.accountName&&(s=this.findAccountFolder(e.accountName),console.log(`[Eudia Calendar] Server accountName "${e.accountName}" -> folder: ${s||"not found"}`)),!s){let d=this.extractAccountFromSubject(e.subject);d&&(s=this.findAccountFolder(d),console.log(`[Eudia Calendar] Subject-based "${d}" -> folder: ${s||"not found"}`),s&&!r&&(r=s.split("/").pop()||d))}if(!s){let d=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(d)instanceof c.TFolder&&(s=d,console.log(`[Eudia Calendar] No match found, using Accounts root: ${s}`))}if(r){let d=this.plugin.settings.cachedAccounts.find(h=>h.name.toLowerCase()===r?.toLowerCase());d&&(o=d.id,r=d.name,console.log(`[Eudia Calendar] Matched to cached account: ${d.name} (${d.id})`))}let u=s?`${s}/${a}`:a,m=this.app.vault.getAbstractFileByPath(u);if(m instanceof c.TFile){await this.app.workspace.getLeaf().openFile(m),new c.Notice(`Opened existing note: ${a}`);return}let l=(e.attendees||[]).map(d=>d.name||d.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),p=`---
title: "${e.subject}"
date: ${t}
attendees: [${l}]
account: "${r||""}"
...
`;try{let d=await this.app.vault.create(u,p);await this.app.workspace.getLeaf().openFile(d),new c.Notice(`Created: ${u}`)}catch(d){console.error("[Eudia Calendar] Failed to create note:",d),new c.Notice(`Could not create note: ${d.message||"Unknown error"}`)}}},D=class extends c.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null}async onload(){await this.loadSettings(),this.transcriptionService=new b(this.settings.serverUrl,this.settings.openaiApiKey),this.calendarService=new A(this.settings.serverUrl,this.settings.userEmail),this.smartTagService=new x,this.registerView(w,e=>new L(e,this)),this.registerView(S,e=>new W(e,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addCommand({id:"transcribe-meeting",name:"Transcribe Meeting",callback:async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}}),this.addCommand({id:"open-calendar",name:"Open Calendar",callback:()=>this.activateCalendarView()}),this.addCommand({id:"sync-accounts",name:"Sync Salesforce Accounts",callback:()=>this.syncAccounts()}),this.addCommand({id:"sync-note",name:"Sync Note to Salesforce",callback:()=>this.syncNoteToSalesforce()}),this.addCommand({id:"new-meeting-note",name:"New Meeting Note",callback:()=>this.createMeetingNote()}),this.addSettingTab(new R(this.app,this)),this.registerEditorSuggest(new P(this.app,this)),this.app.workspace.onLayoutReady(async()=>{!this.settings.setupCompleted&&!this.settings.userEmail?await this.activateSetupView():this.settings.syncOnStartup&&(await this.scanLocalAccountFolders(),this.settings.showCalendarView&&this.settings.userEmail&&await this.activateCalendarView())})}async onunload(){this.app.workspace.detachLeavesOfType(w)}async loadSettings(){this.settings=Object.assign({},X,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async activateCalendarView(){let e=this.app.workspace,t=e.getLeavesOfType(w);if(t.length>0)e.revealLeaf(t[0]);else{let i=e.getRightLeaf(!1);i&&(await i.setViewState({type:w,active:!0}),e.revealLeaf(i))}}async activateSetupView(){let e=this.app.workspace,t=e.getLeavesOfType(S);if(t.length>0)e.revealLeaf(t[0]);else{let i=e.getLeaf(!0);i&&(await i.setViewState({type:S,active:!0}),e.revealLeaf(i))}}async createTailoredAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t);let a=0;for(let s of e){let r=s.name.replace(/[<>:"/\\|?*]/g,"_").trim(),o=`${t}/${r}`;if(this.app.vault.getAbstractFileByPath(o)instanceof c.TFolder){console.log(`[Eudia] Account folder already exists: ${r}`);continue}try{await this.app.vault.createFolder(o);let m=[{name:"Overview.md",content:O(s)},{name:"Contacts.md",content:`---
account: "${s.name}"
account_id: "${s.id}"
type: contacts
sync_to_salesforce: false
...
`}];for(let l of m){let p=`${o}/${l.name}`;await this.app.vault.create(p,l.content)}a++,console.log(`[Eudia] Created account folder with subnotes: ${r}`)}catch(m){console.error(`[Eudia] Failed to create folder for ${r}:`,m)}}this.settings.cachedAccounts=e.map(s=>({id:s.id,name:s.name})),await this.saveSettings(),a>0&&new c.Notice(`Created ${a} account folders`)}async startRecording(){if(!I.isSupported()){new c.Notice("Audio transcription is not supported in this browser");return}let e=this.app.workspace.getActiveFile();if(e||(await this.createMeetingNote(),e=this.app.workspace.getActiveFile()),!e){new c.Notice("Please open or create a note first");return}this.audioRecorder=new I,this.recordingStatusBar=new N(()=>this.audioRecorder?.pause(),()=>this.audioRecorder?.resume(),()=>this.stopRecording(),()=>this.cancelRecording());try{await this.audioRecorder.start(),this.recordingStatusBar.show();let t=setInterval(()=>{if(this.audioRecorder?.isRecording()){let i=this.audioRecorder.getState();this.recordingStatusBar?.updateState(i)}else clearInterval(t)},100);new c.Notice("Transcription started. Click stop when finished.")}catch(t){new c.Notice(`Failed to start transcription: ${t.message}`),this.recordingStatusBar?.hide(),this.recordingStatusBar=null}}async stopRecording(){if(!this.audioRecorder?.isRecording())return;let e=this.app.workspace.getActiveFile();if(!e){new c.Notice("No active file to save transcription"),this.cancelRecording();return}this.recordingStatusBar?.showProcessing();try{let t=await this.audioRecorder.stop();await this.processRecording(t,e)}catch(t){new c.Notice(`Transcription failed: ${t.message}`)}finally{this.recordingStatusBar?.hide(),this.recordingStatusBar=null,this.audioRecorder=null}}async cancelRecording(){this.audioRecorder?.isRecording()&&this.audioRecorder.cancel(),this.recordingStatusBar?.hide(),this.recordingStatusBar=null,this.audioRecorder=null,new c.Notice("Transcription cancelled")}async processRecording(e,t){let i=e.audioBlob?.size||0;if(console.log(`[Eudia] Audio blob size: ${i} bytes, duration: ${e.duration}s`),i<1e3){new c.Notice("Recording too short or no audio captured. Please try again.");return}let a=Math.ceil(e.duration/60),s=Math.max(1,Math.ceil(a/5));new c.Notice(`Transcription started. Estimated ${s}-${s+1} minutes.`);let r=await this.app.vault.read(t),o=`
...
---
`;await this.app.vault.modify(t,r+o),this.processTranscriptionAsync(e,t).catch(u=>{console.error("Background transcription failed:",u),new c.Notice(`Transcription failed: ${u.message}`)})}async processTranscriptionAsync(e,t){try{let i,a=t.path.split("/");if(a.length>=2&&a[0]===this.settings.accountsFolder){let p=a[1],d=this.settings.cachedAccounts.find(h=>h.name.toLowerCase()===p.toLowerCase());d&&(i={accountName:d.name,accountId:d.id})}let s=[];try{let p=await this.calendarService.getCurrentMeeting();p.meeting?.attendees&&(s=p.meeting.attendees.map(d=>d.name||d.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let r=await this.transcriptionService.transcribeAudio(e.audioBlob,i?{...i,speakerHints:s}:{speakerHints:s}),o=p=>p?!!(p.summary?.trim()||p.nextSteps?.trim()):!1,u=r.sections;if(o(u)||r.text?.trim()&&(u=await this.transcriptionService.processTranscription(r.text,i)),!o(u)&&!r.text?.trim()){let d=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,d+`

**Transcription failed:** No audio detected.
`),new c.Notice("Transcription failed: No audio detected.");return}let m=this.buildNoteContent(u,r);await this.app.vault.modify(t,m);let l=Math.floor(e.duration/60);new c.Notice(`Transcription complete (${l} min recording)`),this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(i){try{let s=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,s+`
...
`):typeof y=="object"?JSON.stringify(y):String(y),a=i(e.title)||"Meeting Notes",s=i(e.summary),r=i(e.painPoints),o=i(e.productInterest),u=i(e.meddiccSignals),m=i(e.nextSteps),l=i(e.actionItems),p=i(e.keyDates),d=i(e.risksObjections),h=i(e.attendees),f=`---
title: "${a}"
...
`),p&&!p.includes("No specific dates")&&(f+=`## Key Dates
...
`),this.settings.appendTranscript&&t.text&&(f+=`---

## Full Transcript

${t.text}
`),f}async syncAccounts(e=!1){e||new c.Notice("Syncing Salesforce accounts...");try{let i=(await(0,c.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!i.success||!i.accounts){e||new c.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=i.accounts.map(a=>({id:a.id,name:a.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),e||new c.Notice(`Synced ${i.accounts.length} accounts for matching`)}catch(t){e||new c.Notice(`Failed to sync accounts: ${t.message}`)}}async scanLocalAccountFolders(){try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!e||!(e instanceof c.TFolder))return;let t=[];for(let i of e.children)i instanceof c.TFolder&&t.push({id:`local-${i.name.replace(/\s+/g,"-").toLowerCase()}`,name:i.name});this.settings.cachedAccounts=t,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(e){console.error("Failed to scan local account folders:",e)}}async syncNoteToSalesforce(){let e=this.app.workspace.getActiveFile();if(!e){new c.Notice("No active file to sync");return}let t=await this.app.vault.read(e),i=this.app.metadataCache.getFileCache(e)?.frontmatter;if(!i?.sync_to_salesforce){new c.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}let a=i.account_id,s=i.account;if(!a&&s){let r=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===s.toLowerCase());r&&(a=r.id)}if(!a){let r=e.path.split("/");if(r.length>=2&&r[0]===this.settings.accountsFolder){let o=r[1],u=this.settings.cachedAccounts.find(m=>m.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);u&&(a=u.id,s=u.name)}}if(!a){new c.Notice("Could not determine account for this note");return}try{new c.Notice("Syncing to Salesforce...");let r=await(0,c.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:a,accountName:s,noteTitle:e.basename,notePath:e.path,content:t,frontmatter:i,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});r.json?.success?new c.Notice("Synced to Salesforce"):new c.Notice("Failed to sync: "+(r.json?.error||"Unknown error"))}catch(r){new c.Notice(`Sync failed: ${r.message}`)}}async createMeetingNote(){return new Promise(e=>{new $(this.app,this,async i=>{if(!i){e();return}let a=new Date().toISOString().split("T")[0],s=i.name.replace(/[<>:"/\\|?*]/g,"_").trim(),r=`${this.settings.accountsFolder}/${s}`,o=`${a} Meeting.md`,u=`${r}/${o}`;this.app.vault.getAbstractFileByPath(r)||await this.app.vault.createFolder(r);let m=`---
...
`,l=await this.app.vault.create(u,m);await this.app.workspace.getLeaf().openFile(l),new c.Notice(`Created meeting note for ${i.name}`),e()}).open()})}async fetchAndInsertContext(){new c.Notice("Fetching pre-call context...")}},R=class extends c.PluginSettingTab{constructor(n,e){super(n,e),this.plugin=e}display(){let{containerEl:n}=this;n.empty(),n.createEl("h2",{text:"Eudia Sync & Scribe"}),n.createEl("h3",{text:"Your Profile"});let e=n.createDiv();e.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let t=e.createDiv();t.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let i=t.createSpan(),a=t.createSpan(),s=e.createDiv();s.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",s.setText("Connect with Salesforce to sync notes with your user attribution.");let r=e.createEl("button");r.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,u=async()=>{if(!this.plugin.settings.userEmail)return i.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",a.setText("Enter email above first"),r.setText("Setup Required"),r.disabled=!0,r.style.opacity="0.5",r.style.cursor="not-allowed",!1;r.disabled=!1,r.style.opacity="1",r.style.cursor="pointer";try{return i.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",a.setText("Checking..."),(await(0,c.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(i.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",a.setText("Connected to Salesforce"),r.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(i.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",a.setText("Not connected"),r.setText("Connect to Salesforce"),!1)}catch{return i.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",a.setText("Status unavailable"),r.setText("Connect to Salesforce"),!1}};new c.Setting(n).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(l=>l.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async p=>{let d=p.trim().toLowerCase();this.plugin.settings.userEmail=d,await this.plugin.saveSettings(),await u()})),n.createEl("h3",{text:"Salesforce Connection"}),n.appendChild(e);let m=()=>{o&&window.clearInterval(o);let l=0,p=30;o=window.setInterval(async()=>{l++,await u()?(o&&(window.clearInterval(o),o=null),new c.Notice("Salesforce connected successfully!")):l>=p&&o&&(window.clearInterval(o),o=null)},5e3)};r.onclick=async()=>{if(!this.plugin.settings.userEmail){new c.Notice("Please enter your email first");return}let l=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(l,"_blank"),new c.Notice("Complete the Salesforce login in the popup window",5e3),m()},u(),n.createEl("h3",{text:"Server"}),new c.Setting(n).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(l=>l.setValue(this.plugin.settings.serverUrl).onChange(async p=>{this.plugin.settings.serverUrl=p,await this.plugin.saveSettings()})),new c.Setting(n).setName("OpenAI API Key").setDesc("For transcription (optional if server provides)").addText(l=>{l.setPlaceholder("sk-...").setValue(this.plugin.settings.openaiApiKey).onChange(async p=>{this.plugin.settings.openaiApiKey=p,await this.plugin.saveSettings()}),l.inputEl.type="password"}),n.createEl("h3",{text:"Transcription"}),new c.Setting(n).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(l=>l.setValue(this.plugin.settings.saveAudioFiles).onChange(async p=>{this.plugin.settings.saveAudioFiles=p,await this.plugin.saveSettings()})),new c.Setting(n).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(l=>l.setValue(this.plugin.settings.appendTranscript).onChange(async p=>{this.plugin.settings.appendTranscript=p,await this.plugin.saveSettings()})),n.createEl("h3",{text:"Sync"}),new c.Setting(n).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(l=>l.setValue(this.plugin.settings.syncOnStartup).onChange(async p=>{this.plugin.settings.syncOnStartup=p,await this.plugin.saveSettings()})),new c.Setting(n).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(l=>l.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async p=>{this.plugin.settings.autoSyncAfterTranscription=p,await this.plugin.saveSettings()})),n.createEl("h3",{text:"Folders"}),new c.Setting(n).setName("Accounts Folder").setDesc("Where account folders are stored").addText(l=>l.setValue(this.plugin.settings.accountsFolder).onChange(async p=>{this.plugin.settings.accountsFolder=p||"Accounts",await this.plugin.saveSettings()})),new c.Setting(n).setName("Recordings Folder").setDesc("Where audio files are saved").addText(l=>l.setValue(this.plugin.settings.recordingsFolder).onChange(async p=>{this.plugin.settings.recordingsFolder=p||"Recordings",await this.plugin.saveSettings()})),n.createEl("h3",{text:"Actions"}),new c.Setting(n).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching (folders are pre-loaded)`).addButton(l=>l.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),this.plugin.settings.lastSyncTime&&n.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),n.createEl("p",{text:`Audio transcription: ${I.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
