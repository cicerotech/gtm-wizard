...
- Action items have clear owners`}function Te(S,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
...
`}function We(S,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${r.account.name}
${r.account.owner?`- Owner: ${r.account.owner}`:""}
...
`}function Ie(S,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${r.account.name}
${r.account.owner?`- Owner: ${r.account.owner}`:""}
...
`}if(d.objections&&d.objections.length>0){let u=d.objections.filter(m=>m.handled).length;n+=`**Objections:** ${d.objections.length} raised, ${u} handled
...
`;try{let f=await this.app.vault.create(v,C);await this.app.workspace.getLeaf().openFile(f);try{let g=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;g?.revealInFolder&&g.revealInFolder(f)}catch{}new p.Notice(`Created: ${v}`)}catch(f){console.error("[Eudia Calendar] Failed to create note:",f),new p.Notice(`Could not create note: ${f.message||"Unknown error"}`)}}async _createPipelineNote(t,e){let n=new Date(e+"T00:00:00"),i=String(n.getMonth()+1).padStart(2,"0"),a=String(n.getDate()).padStart(2,"0"),s=String(n.getFullYear()).slice(-2),o=`${i}.${a}.${s}`,l=`Team Pipeline Meeting - ${o}.md`,c="Pipeline Meetings";this.app.vault.getAbstractFileByPath(c)||await this.app.vault.createFolder(c);let u=`${c}/${l}`,m=this.app.vault.getAbstractFileByPath(u);if(m instanceof p.TFile){await this.app.workspace.getLeaf().openFile(m);try{let v=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;v?.revealInFolder&&v.revealInFolder(m)}catch{}new p.Notice(`Opened existing: ${l}`);return}let h=(t.attendees||[]).map(v=>v.name||v.email?.split("@")[0]||"Unknown"),y=`---
...
`},{name:"Meeting Notes.md",content:O},{name:"Contacts.md",content:v},{name:"Intelligence.md",content:b},{name:"Next Steps.md",content:C}];for(let w of f){let x=`${u}/${w.name}`;await this.app.vault.create(x,w.content)}return console.log(`[Eudia] Created account folder with subnotes${y?" (enriched)":""}: ${d}`),!0}catch(h){return console.error(`[Eudia] Failed to create folder for ${d}:`,h),!1}},l=5;for(let c=0;c<t.length;c+=l){let d=t.slice(c,c+l),u=await Promise.allSettled(d.map(m=>o(m)));a+=u.filter(m=>m.status==="fulfilled"&&m.value===!0).length}a>0?(this.settings.cachedAccounts=t.map(c=>({id:c.id,name:c.name})),await this.saveSettings(),new p.Notice(`Created ${a} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let i=e?`
...
*Previous next steps will be archived here.*
`}];for(let h of m){let y=`${o}/${h.name}`;await this.app.vault.create(y,h.content)}i++}catch(u){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,u)}}if(i>0){console.log(`[Eudia] Created ${i} prospect account folders in Accounts/`);let a=new Set((this.settings.cachedAccounts||[]).map(s=>s.id));for(let s of t)s.id&&!a.has(s.id)&&this.settings.cachedAccounts.push({id:s.id,name:s.name});await this.saveSettings()}return i}async migrateAccountStructure(){let t=this.settings.accountsFolder||"Accounts",e=`${t}/_Prospects`,n=this.app.vault.getAbstractFileByPath(e);if(n instanceof p.TFolder){let a=[...n.children],s=0;for(let l of a){if(!(l instanceof p.TFolder))continue;let c=`${t}/${l.name}`;if(this.app.vault.getAbstractFileByPath(c)instanceof p.TFolder){let u=[...l.children];for(let m of u){let h=`${c}/${m.name}`;if(!this.app.vault.getAbstractFileByPath(h))try{await this.app.fileManager.renameFile(m,h)}catch{}}try{await this.app.vault.delete(l,!0)}catch{}}else try{await this.app.fileManager.renameFile(l,c),s++}catch(u){console.warn(`[Eudia Migration] Failed to move ${l.name}:`,u)}}let o=this.app.vault.getAbstractFileByPath(e);if(o instanceof p.TFolder&&o.children.length===0)try{await this.app.vault.delete(o,!0)}catch{}s>0&&console.log(`[Eudia Migration] Moved ${s} prospect folders to Accounts/`)}let i=this.settings.userEmail;if(i&&!F(i)&&!L(i))try{let s=await new R(this.settings.serverUrl).getAccountsWithProspects(i),o=new Set([...s.accounts,...s.prospects].map(c=>c.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase())),l=this.app.vault.getAbstractFileByPath(t);if(l instanceof p.TFolder){let c=`${t}/_Other_Accounts`,d=0;for(let u of[...l.children]){if(!(u instanceof p.TFolder)||u.name.startsWith("_")||u.name.startsWith(".")||o.has(u.name.toLowerCase()))continue;if(!this.app.vault.getAbstractFileByPath(c))try{await this.app.vault.createFolder(c)}catch{}let m=`${c}/${u.name}`;if(!this.app.vault.getAbstractFileByPath(m))try{await this.app.fileManager.renameFile(u,m),d++}catch{}}d>0&&console.log(`[Eudia Migration] Archived ${d} non-owned accounts to _Other_Accounts/`)}this.settings.cachedAccounts=[...s.accounts,...s.prospects].map(c=>({id:c.id,name:c.name}))}catch(a){console.warn("[Eudia Migration] Could not fetch ownership \u2014 skipping archive step:",a)}this.settings.prospectsMigrated=!0,await this.saveSettings(),console.log("[Eudia Migration] Account structure migration complete")}async createCSManagerDashboard(t,e){let n="CS Manager",i=new Date().toISOString().split("T")[0],a=ye(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s={};for(let u of e){let m=u.ownerName||"Unassigned";s[m]||(s[m]=[]),s[m].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
`,l=Object.keys(s).sort();for(let u of l){let m=s[u];o+=`### ${u} (${m.length} accounts)
`;for(let h of m.slice(0,10))o+=`- **${h.name}** \u2014 ${h.type||"Account"}
`;m.length>10&&(o+=`- _...and ${m.length-10} more_
`),o+=`
`}o+=`---
...
`;let c=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(c);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(c,o);for(let u of a){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,g=>g.toUpperCase()),h=u.split("@")[0].replace("."," ").toLowerCase(),y=h.split(" ")[0],v=h.split(" ").pop()||"",b=e.filter(g=>{let w=(g.csmName||"").toLowerCase();if(w&&(w.includes(y)||w.includes(v)))return!0;let x=(g.ownerName||"").toLowerCase();return x.includes(y)||x.includes(v)}),O=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
...
`;if(b.length>0){O+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let g of b){let w=g.name.replace(/[<>:"/\\|?*]/g,"_").trim();O+=`| ${g.name} | ${g.type||""} | ${g.ownerName||""} | [[Accounts/${w}/Contacts\\|View]] |
`}}else O+=`*No accounts currently matched to this rep. Accounts will populate after connecting to Salesforce (Step 2).*
`;O+=`
---

## Recent Activity

Meeting notes and activity for ${m}'s accounts sync through Salesforce:
...
`;await this.app.vault.create(e,o)}}async updateAccountNextSteps(t,e,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${t}`),console.log(`[Eudia] Content length: ${e?.length||0} chars`);let i=t.replace(/[<>:"/\\|?*]/g,"_").trim(),a=`${this.settings.accountsFolder}/${i}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${a}`);let s=this.app.vault.getAbstractFileByPath(a);if(!s||!(s instanceof p.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${a}`);let O=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${i}`);O&&O instanceof p.TFolder?console.log(`[Eudia] Files in ${i} folder:`,O.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${i}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],l=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),c=n.split("/").pop()?.replace(".md","")||"Meeting",d=e;!e.includes("- [ ]")&&!e.includes("- [x]")&&(d=e.split(`
`).filter(O=>O.trim()).map(O=>{let C=O.replace(/^[-•*]\s*/,"").trim();return C?`- [ ] ${C}`:""}).filter(Boolean).join(`
`));let u=await this.app.vault.read(s),m="",h=u.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);h&&h[1]&&(m=h[1].trim());let y=`### ${o} - ${c}
${d||"*None*"}`,v=m?`${y}
...
${m}`:y,b=`---
account: "${t}"
account_id: "${this.settings.cachedAccounts.find(O=>O.name===t)?.id||""}"
type: next_steps
auto_updated: true
last_updated: ${o}
sync_to_salesforce: false
...
---
`;await this.app.vault.modify(e,C+f),this.processTranscriptionAsync(t,e).catch(g=>{console.error("Background transcription failed:",g),new p.Notice(`Transcription failed: ${g.message}`)})}async processTranscriptionAsync(t,e){try{let n={},i=e.path.split("/");console.log(`[Eudia] Processing transcription for: ${e.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(i)}, accountsFolder: ${this.settings.accountsFolder}`);let a=i[0]==="Pipeline Meetings",s=!1;try{let g=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);g&&(s=/meeting_type:\s*pipeline_review/.test(g[1]))}catch{}if(!s&&a&&(s=!0),s){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let f="";try{let g=await(0,p.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});g.json?.success&&g.json?.context&&(f=g.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${f.length} chars)`))}catch(g){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",g)}n={meetingType:"pipeline_review",pipelineContext:f}}else if(i.length>=2&&i[0]===this.settings.accountsFolder){let f=i[1];console.log(`[Eudia] Detected account folder: ${f}`);let g=this.settings.cachedAccounts.find(w=>w.name.toLowerCase()===f.toLowerCase());g?(n={accountName:g.name,accountId:g.id,userEmail:this.settings.userEmail},console.log(`[Eudia] Found cached account: ${g.name} (${g.id})`)):(n={accountName:f,accountId:"",userEmail:this.settings.userEmail},console.log(`[Eudia] Account not in cache, using folder name: ${f}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let f=await this.calendarService.getCurrentMeeting();f.meeting?.attendees&&(o=f.meeting.attendees.map(g=>g.name||g.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let l=Date.now(),c=await this.transcriptionService.transcribeAudio(t.audioBlob,{...n,speakerHints:o,captureMode:t.captureMode,hasVirtualDevice:t.hasVirtualDevice,meetingTemplate:this.settings.meetingTemplate||"meddic"}),d=Math.round(t.audioBlob.size/1024/1024*100)/100,u=d>15;this.telemetry.reportTranscriptionResult({success:!!c.text?.trim(),isChunked:u,totalSizeMB:d,transcriptLength:c.text?.length||0,processingTimeSec:Math.round((Date.now()-l)/1e3),error:c.error});let m=f=>f?!!(f.summary?.trim()||f.nextSteps?.trim()):!1,h=c.sections;if(m(h)||c.text?.trim()&&(h=await this.transcriptionService.processTranscription(c.text,n)),!m(h)&&!c.text?.trim()){let g=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Processing your recording\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,"").replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,""),w=c.error,E=!w||w.includes("audio")||w.includes("microphone")?"No audio detected. Check your microphone settings.":w,k=t._savedAudioPath,j=k?`
...
`),new p.Notice(`Transcription failed: ${E}`,1e4);return}let y=await this.app.vault.read(e),v="",b=Math.max(y.indexOf(`---
**Processing your recording`),y.indexOf(`---
**Transcription in progress`));if(b>0){let f=y.indexOf("---"),g=f>=0?y.indexOf("---",f+3):-1;g>0&&g+3<b&&(v=y.substring(g+3,b).trim())}else{let f=y.indexOf("---"),g=f>=0?y.indexOf("---",f+3):-1;if(g>0){let w=y.substring(g+3).trim();w.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(v=w)}}try{let f="_backups";this.app.vault.getAbstractFileByPath(f)||await this.app.vault.createFolder(f);let g=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),w=`${f}/${e.name}_${g}.md`;await this.app.vault.create(w,y),console.log(`[Eudia] Backed up note to ${w}`)}catch(f){console.warn("[Eudia] Backup failed (non-critical):",f.message)}let O;if(s?O=this.buildPipelineNoteContent(h,c,e.path):O=this.buildNoteContent(h,c),v&&v.length>5){let f=O.indexOf("---",O.indexOf("---")+3);if(f>0){let g=O.substring(0,f+3),w=O.substring(f+3);O=g+`
...
---
`+w}}await this.app.vault.modify(e,O);let C=Math.floor(t.duration/60);new p.Notice(`Transcription complete (${C} min recording)`);try{if(!s){let f=h.nextSteps||h.actionItems;f&&n?.accountName&&await this.updateAccountNextSteps(n.accountName,f,e.path)}}catch(f){console.warn("[Eudia] Next Steps extraction failed (non-critical):",f.message)}try{this.settings.autoSyncAfterTranscription&&e&&await this._executeSyncToSalesforce(e)}catch(f){console.warn("[Eudia] Auto-sync failed (non-critical):",f.message)}}catch(n){try{let i=await this.app.vault.read(e);if(i.includes("## Summary")||i.includes(`## Next Steps
...
`):typeof y=="object"?JSON.stringify(y,null,2):String(y),u=d(t.summary),m=e.transcript||e.text||"",h=`---
...
`;if(u)h+=u;else{let y=[t.painPoints,t.productInterest,t.nextSteps,t.actionItems].filter(Boolean).map(d).join(`

`);y?h+=y:h+="*Pipeline summary could not be generated. See transcript below.*"}return m&&(h+=`

---

<details>
<summary><strong>Full Transcript</strong> (${Math.ceil(m.length/1e3)}k chars)</summary>

${m}

</details>
`),h}buildNoteContent(t,e){let n=w=>w==null?"":Array.isArray(w)?w.map(x=>typeof x=="object"?x.category?`**${x.category}**: ${x.signal||x.insight||""}`:JSON.stringify(x):String(x)).join(`
`):typeof w=="object"?JSON.stringify(w):String(w),i=n(t.title)||"Meeting Notes",a=n(t.summary),s=n(t.discussionContext),o=n(t.keyQuotes),l=n(t.painPoints),c=n(t.productInterest),d=n(t.meddiccSignals),u=n(t.nextSteps),m=n(t.actionItems),h=n(t.keyDates),y=n(t.dealSignals),v=n(t.risksObjections),b=n(t.attendees||t.keyStakeholders),O=n(t.emailDraft),C=`---
title: "${i}"
...
`),h&&!h.includes("No specific dates")&&!h.includes("Not discussed")&&(C+=`## Key Dates
...
\u2022 `));let u="",m=e.match(/"([^"]{20,120})"/);m&&(u=m[1]);let h=`*${i} \u2014 ${s}*
`;o&&(h+=`${o}
`),c&&(h+=`
*Next Steps:*
\u2022 ${c}
