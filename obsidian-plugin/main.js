...
`,t}async blobToBase64(i){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let r=n.result.split(",")[1];t(r)},n.onerror=e,n.readAsDataURL(i)})}async transcribeAudio(i,t){let e=i.size/1024/1024,n=i.type||"audio/webm";if(e>8)return console.log(`[Eudia] Large recording (${e.toFixed(1)}MB) \u2014 using chunked transcription`),this.transcribeAudioChunked(i,n,t);try{let s=await this.blobToBase64(i),r=t?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:t.pipelineContext}:void 0,a=await this.transcribeAndSummarize(s,n,t?.accountName,t?.accountId,r,{captureMode:t?.captureMode,hasVirtualDevice:t?.hasVirtualDevice},t?.meetingTemplate);return{text:a.transcript,confidence:a.success?.95:0,duration:a.duration,sections:a.sections,diarizedTranscript:a.diarizedTranscript,error:a.error}}catch(s){return console.error("transcribeAudio error:",s),{text:"",confidence:0,duration:0,sections:this.getEmptySections(),error:s.message||"Transcription request failed"}}}static{this.CHUNK_MAX_RETRIES=3}static{this.CHUNK_RETRY_DELAYS=[1e4,3e4,6e4]}async transcribeChunkWithRetry(i,t,e,n){for(let s=0;s<=O.CHUNK_MAX_RETRIES;s++){if(s>0){let r=O.CHUNK_RETRY_DELAYS[s-1]||3e4;console.log(`[Eudia] Chunk ${e+1}/${n} retry ${s}/${O.CHUNK_MAX_RETRIES} in ${r/1e3}s...`),await new Promise(a=>setTimeout(a,r))}try{let a=(0,P.requestUrl)({url:`${this.serverUrl}/api/transcribe-chunk`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({audio:i,mimeType:t})}),o=new Promise((d,u)=>setTimeout(()=>u(new Error(`Chunk request timed out after ${9e4/1e3}s`)),9e4)),l=await Promise.race([a,o]),c=l.json?.text||l.json?.transcript||"";if(l.json?.success&&c)return s>0&&console.log(`[Eudia] Chunk ${e+1}/${n} succeeded on retry ${s}`),{text:c,duration:l.json.duration||0};console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${s+1} returned no text: ${l.json?.error||"unknown"}`)}catch(r){console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${s+1} failed: ${r.message}`)}}return null}async transcribeAudioChunked(i,t,e){let s=await i.arrayBuffer(),r=s.byteLength,a=Math.ceil(r/4194304);console.log(`[Eudia] Chunked transcription: ${(r/1024/1024).toFixed(1)}MB \u2192 ${a} chunks`);let o=16*1024,l=r/o,c=[],d=0,u=0;for(let g=0;g<a;g++){let v=g*4194304,b=Math.min(v+4194304,r),w=s.slice(v,b),C=new Blob([w],{type:t});console.log(`[Eudia] Transcribing chunk ${g+1}/${a} (${((b-v)/1024/1024).toFixed(1)}MB)`);let f=await this.blobToBase64(C),m=await this.transcribeChunkWithRetry(f,t,g,a);if(m)c.push(m.text),d+=m.duration,console.log(`[Eudia] Chunk ${g+1}/${a} OK: ${m.text.length} chars`);else{u++;let S=Math.round(v/r*l),x=Math.round(b/r*l),E=`${Math.floor(S/60)}:${(S%60).toString().padStart(2,"0")}`,k=`${Math.floor(x/60)}:${(x%60).toString().padStart(2,"0")}`,j=`
