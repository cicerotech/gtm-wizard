...
`;let c=Math.round(i/5),d=Math.round(a/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
...
`,c=Object.keys(a).sort();for(let p of c){let g=a[p];o+=`### ${p} (${g.length} accounts)
...
`;let f=`${n}/${g}.md`,S=this.app.vault.getAbstractFileByPath(f);S instanceof u.TFile?await this.app.vault.modify(S,m):await this.app.vault.create(f,m)}console.log(`[Eudia] Created CS Manager dashboard for ${e} with ${t.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let r=0,i=0,a=new Date().toISOString().split("T")[0],o=async d=>{let l=d.name.replace(/[<>:"/\\|?*]/g,"_").trim(),p=`${t}/${l}`;if(this.app.vault.getAbstractFileByPath(p)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(p),await this.createExecAccountSubnotes(p,d,a),d.isOwned?r++:i++,console.log(`[Eudia Admin] Created ${d.isOwned?"owned":"view-only"} folder: ${l}`),!0}catch(h){return console.error(`[Eudia Admin] Failed to create folder for ${l}:`,h),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c);await Promise.allSettled(l.map(p=>o(p)))}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),r+i>0&&new u.Notice(`Created ${r} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let r=t.ownerName||"Unknown",i=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(m.length>0||h!=="Unknown")&&r.push({account:c.name,lastUpdated:h,nextSteps:m})}}r.sort((c,d)=>d.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],a=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`):typeof f=="object"?JSON.stringify(f):String(f),r=n(e.title)||"Meeting Notes",i=n(e.summary),a=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),l=n(e.actionItems),p=n(e.keyDates),g=n(e.risksObjections),h=n(e.attendees),m=`---
title: "${r}"
...
*Add org chart, decision makers, champions, and blockers here.*`),t.intelligence&&await c("Intelligence.md",t.intelligence),t.nextSteps&&await c("Next Steps.md",t.nextSteps),t.opportunities||t.recentActivity){let d=`${i}/Meeting Notes.md`,l=this.app.vault.getAbstractFileByPath(d);if(l instanceof u.TFile){let p=await this.app.vault.read(l),g="",h=p;if(p.startsWith("---")){let O=p.indexOf("---",3);O!==-1&&(g=p.substring(0,O+3),h=p.substring(O+3),g.includes("enriched_at:")?g=g.replace(/enriched_at:.*/,`enriched_at: "${o}"`):g=g.substring(0,O)+`enriched_at: "${o}"
---`)}let m=h.match(/^(\s*#[^\n]+)/),S=[m?m[1]:`
