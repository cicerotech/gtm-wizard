...
`).replace(/\n/g,"<br>")}onClose(){this.contentEl.empty()}};var O=class extends l.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new T(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return b}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}this.plugin.settings.accountsImported&&(this.steps[2].status="complete")}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),s=t.createDiv({cls:"eudia-setup-title-section"});s.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),s.createEl("p",{text:"Complete these steps to unlock your sales superpowers",cls:"eudia-setup-subtitle"});let a=t.createDiv({cls:"eudia-setup-progress-section"}),i=this.getCompletionPercentage(),r=a.createDiv({cls:"eudia-setup-progress-label"});r.createSpan({text:"Setup Progress"}),r.createSpan({text:`${i}%`,cls:"eudia-setup-progress-value"});let c=a.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${i}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],s=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=s.createDiv({cls:"eudia-setup-step-header"}),i=a.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"1"),t.status==="complete"&&i.addClass("eudia-step-complete");let r=a.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=s.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let d=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});d.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async p=>{p.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let s=await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(s.status===200&&s.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new l.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");try{let a;I(e)?(console.log("[Eudia] Admin user detected - importing all accounts"),a=await this.accountOwnershipService.getAllAccountsForAdmin(e)):a=await this.accountOwnershipService.getAccountsForUser(e),a.length>0&&(I(e)?await this.plugin.createAdminAccountFolders(a):await this.plugin.createTailoredAccountFolders(a),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,await this.plugin.saveSettings(),new l.Notice(`Imported ${a.length} account folders!`))}catch(a){console.error("[Eudia] Account import failed:",a)}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],s=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=s.createDiv({cls:"eudia-setup-step-header"}),i=a.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"2"),t.status==="complete"&&i.addClass("eudia-step-complete");let r=a.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=s.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let d=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),p=o.createDiv({cls:"eudia-setup-sf-status"});d.onclick=async()=>{let g=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(g,"_blank"),p.textContent="Complete the login in the popup window...",p.className="eudia-setup-sf-status loading",new l.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(p)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,s=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new l.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=s&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=this.plugin.settings.userEmail,s;if(I(t)?(console.log("[Eudia] Admin user detected - importing all accounts"),e.textContent="Admin detected - importing all accounts...",s=await this.accountOwnershipService.getAllAccountsForAdmin(t)):s=await this.accountOwnershipService.getAccountsForUser(t),s.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}I(t)?await this.plugin.createAdminAccountFolders(s):await this.plugin.createTailoredAccountFolders(s),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length,await this.plugin.saveSettings(),this.steps[2].status="complete";let a=s.filter(r=>r.isOwned!==!1).length,i=s.filter(r=>r.isOwned===!1).length;I(t)&&i>0?e.textContent=`${a} owned + ${i} view-only accounts imported!`:e.textContent=`${s.length} accounts imported successfully!`,e.className="eudia-setup-sf-status success"}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error"}}renderTranscribeStep(e){let t=this.steps[2],s=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=s.createDiv({cls:"eudia-setup-step-header"}),i=a.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"3"),t.status==="complete"&&i.addClass("eudia-step-complete");let r=a.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=s.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),d.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let p=c.createDiv({cls:"eudia-setup-instruction"});p.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),p.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let g=c.createDiv({cls:"eudia-setup-instruction"});g.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),g.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(i=>i.status==="complete")){let i=t.createDiv({cls:"eudia-setup-completion"});i.createEl("h2",{text:"\u{1F389} You're all set!"}),i.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let r=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});r.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(b),await this.plugin.activateCalendarView()}}else{let i=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});i.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(b),new l.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let a=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});a.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},_=class extends l.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return x}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),s=t.createDiv({cls:"eudia-calendar-title-row"});s.createEl("h4",{text:"Your Meetings"});let a=s.createDiv({cls:"eudia-calendar-actions"}),i=a.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});i.title="Refresh",i.onclick=async()=>{i.addClass("spinning"),await this.render(),i.removeClass("spinning")};let r=a.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});r.title="Settings",r.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},s=this.plugin.settings.serverUrl,a=this.plugin.settings.userEmail;try{(await(0,l.requestUrl)({url:`${s}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(a&&t.server==="connected")try{let p=await(0,l.requestUrl)({url:`${s}/api/calendar/validate/${encodeURIComponent(a)}`,method:"GET",throw:!1});p.status===200&&p.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(a&&t.server==="connected")try{let p=await(0,l.requestUrl)({url:`${s}/api/sf/auth/status?email=${encodeURIComponent(a)}`,method:"GET",throw:!1});p.status===200&&p.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let i=e.createDiv({cls:"eudia-status-indicators"}),r=i.createSpan({cls:`eudia-status-dot ${t.server}`});r.title=t.serverMessage||"Server";let o=i.createSpan({cls:`eudia-status-dot ${t.calendar}`});o.title=t.calendarMessage||"Calendar";let c=i.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(c.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let p=e.createDiv({cls:"eudia-status-warning"});p.innerHTML=`<strong>${a}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),s=t.createDiv({cls:"eudia-calendar-loading"});s.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let a=new S(this.plugin.settings.serverUrl,this.plugin.settings.userEmail),i=await a.getWeekMeetings();if(s.remove(),!i.success){this.renderError(t,i.error||"Failed to load calendar");return}let r=Object.keys(i.byDay||{}).sort();if(r.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,a);for(let o of r){let c=i.byDay[o];!c||c.length===0||this.renderDaySection(t,o,c)}}catch(a){s.remove(),this.renderError(t,a.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let s=await t.getCurrentMeeting();if(s.meeting){let a=e.createDiv({cls:"eudia-now-card"});s.isNow?a.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):a.createDiv({cls:"eudia-now-badge soon",text:`In ${s.minutesUntilStart}m`});let i=a.createDiv({cls:"eudia-now-content"});i.createEl("div",{cls:"eudia-now-subject",text:s.meeting.subject}),s.meeting.accountName&&i.createEl("div",{cls:"eudia-now-account",text:s.meeting.accountName});let r=a.createEl("button",{cls:"eudia-now-action",text:"Create Note"});r.onclick=()=>this.createNoteForMeeting(s.meeting)}}catch{}}renderDaySection(e,t,s){let a=e.createDiv({cls:"eudia-calendar-day"});a.createEl("div",{cls:"eudia-calendar-day-header",text:S.getDayName(t)});for(let i of s){let r=a.createDiv({cls:`eudia-calendar-meeting ${i.isCustomerMeeting?"customer":"internal"}`});r.createEl("div",{cls:"eudia-calendar-time",text:S.formatTime(i.start,this.plugin.settings.timezone)});let o=r.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:i.subject}),i.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:i.accountName});else if(i.attendees&&i.attendees.length>0){let c=i.attendees.slice(0,2).map(d=>d.name||d.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}r.onclick=()=>this.createNoteForMeeting(i),r.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `;let s=t.createDiv({cls:"eudia-setup-input-group"}),a=s.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});a.addClass("eudia-setup-email");let i=s.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),r=t.createDiv({cls:"eudia-setup-status"});i.onclick=async()=>{let o=a.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){r.textContent="Please use your @eudia.com email",r.className="eudia-setup-status error";return}i.disabled=!0,i.textContent="Connecting...",r.textContent="";try{if(!(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){r.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,r.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),r.textContent="Connected",r.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let d=c.message||"Connection failed";d.includes("403")?r.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:r.textContent=d,r.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect"}},a.onkeydown=o=>{o.key==="Enter"&&i.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractAccountFromAttendees(e){if(!e||e.length===0)return null;let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com"],s=[];for(let o of e){if(!o.email)continue;let d=o.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(d){let p=d[1];!p.includes("eudia.com")&&!t.includes(p)&&s.push(p)}}if(s.length===0)return null;let a=s[0],i=a.split(".")[0],r=i.charAt(0).toUpperCase()+i.slice(1);return console.log(`[Eudia Calendar] Extracted company "${r}" from attendee domain ${a}`),r}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let a=(t[1]||t[2]||"").trim();if(a.toLowerCase()!=="eudia")return a}let s=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(s){let i=(s[1]||s[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(i.toLowerCase()!=="eudia"&&i.length>0)return i}if(!e.toLowerCase().includes("eudia")){let a=e.match(/^([^-–|]+)/);if(a){let i=a[1].trim();if(i.length>2&&i.length<50)return i}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",s=this.app.vault.getAbstractFileByPath(t);if(!(s instanceof l.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let a=e.toLowerCase().trim(),i=[];for(let g of s.children)g instanceof l.TFolder&&i.push(g.name);console.log(`[Eudia Calendar] Searching for "${a}" in ${i.length} folders`);let r=i.find(g=>g.toLowerCase()===a);if(r)return console.log(`[Eudia Calendar] Exact match found: ${r}`),`${t}/${r}`;let o=i.find(g=>g.toLowerCase().startsWith(a));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${t}/${o}`;let c=i.find(g=>a.startsWith(g.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${t}/${c}`;let d=i.find(g=>{let u=g.toLowerCase();return u.length>=3&&a.includes(u)});if(d)return console.log(`[Eudia Calendar] Search contains folder match: ${d}`),`${t}/${d}`;let p=i.find(g=>{let u=g.toLowerCase();return a.length>=3&&u.includes(a)});return p?(console.log(`[Eudia Calendar] Folder contains search match: ${p}`),`${t}/${p}`):(console.log(`[Eudia Calendar] No folder match found for "${a}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],s=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),a=`${t} - ${s}.md`,i=null,r=e.accountName||null,o=null;if(console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(u=>u.email)||[])}`),!i&&e.attendees&&e.attendees.length>0){let u=this.extractAccountFromAttendees(e.attendees);console.log(`[Eudia Calendar] Extracted domain company name: "${u||"none"}"`),u&&(i=this.findAccountFolder(u),console.log(`[Eudia Calendar] Domain-based "${u}" -> folder: ${i||"not found"}`),i&&!r&&(r=i.split("/").pop()||u))}if(!i&&e.accountName&&(i=this.findAccountFolder(e.accountName),console.log(`[Eudia Calendar] Server accountName "${e.accountName}" -> folder: ${i||"not found"}`)),!i){let u=this.extractAccountFromSubject(e.subject);u&&(i=this.findAccountFolder(u),console.log(`[Eudia Calendar] Subject-based "${u}" -> folder: ${i||"not found"}`),i&&!r&&(r=i.split("/").pop()||u))}if(!i){let u=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(u)instanceof l.TFolder&&(i=u,console.log(`[Eudia Calendar] No match found, using Accounts root: ${i}`))}if(r){let u=this.plugin.settings.cachedAccounts.find(m=>m.name.toLowerCase()===r?.toLowerCase());u&&(o=u.id,r=u.name,console.log(`[Eudia Calendar] Matched to cached account: ${u.name} (${u.id})`))}let c=i?`${i}/${a}`:a,d=this.app.vault.getAbstractFileByPath(c);if(d instanceof l.TFile){await this.app.workspace.getLeaf().openFile(d),new l.Notice(`Opened existing note: ${a}`);return}let p=(e.attendees||[]).map(u=>u.name||u.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),g=`---
title: "${e.subject}"
date: ${t}
attendees: [${p}]
account: "${r||""}"
...
`;try{let u=await this.app.vault.create(c,g);await this.app.workspace.getLeaf().openFile(u),new l.Notice(`Created: ${c}`)}catch(u){console.error("[Eudia Calendar] Failed to create note:",u),new l.Notice(`Could not create note: ${u.message||"Unknown error"}`)}}},M=class extends l.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null}async onload(){await this.loadSettings(),this.transcriptionService=new N(this.settings.serverUrl),this.calendarService=new S(this.settings.serverUrl,this.settings.userEmail),this.smartTagService=new D,this.registerView(x,e=>new _(e,this)),this.registerView(b,e=>new O(e,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addCommand({id:"transcribe-meeting",name:"Transcribe Meeting",callback:async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}}),this.addCommand({id:"open-calendar",name:"Open Calendar",callback:()=>this.activateCalendarView()}),this.addCommand({id:"sync-accounts",name:"Sync Salesforce Accounts",callback:()=>this.syncAccounts()}),this.addCommand({id:"sync-note",name:"Sync Note to Salesforce",callback:()=>this.syncNoteToSalesforce()}),this.addCommand({id:"new-meeting-note",name:"New Meeting Note",callback:()=>this.createMeetingNote()}),this.addCommand({id:"ask-gtm-brain",name:"Ask gtm-brain",callback:()=>this.openIntelligenceQueryForCurrentNote()}),this.addSettingTab(new H(this.app,this)),this.registerEditorSuggest(new j(this.app,this)),this.app.workspace.onLayoutReady(async()=>{if(this.settings.setupCompleted){if(this.settings.syncOnStartup){if(await this.scanLocalAccountFolders(),this.settings.userEmail&&this.settings.syncAccountsOnStartup){let e=new Date().toISOString().split("T")[0];this.settings.lastAccountRefreshDate!==e&&setTimeout(async()=>{try{console.log("[Eudia] Startup account sync - checking for changes...");let s=await this.syncAccountFolders();if(s.success){if(this.settings.lastAccountRefreshDate=e,await this.saveSettings(),s.added>0||s.archived>0){let a=[];s.added>0&&a.push(`${s.added} added`),s.archived>0&&a.push(`${s.archived} archived`),new l.Notice(`Account folders synced: ${a.join(", ")}`)}}else console.log("[Eudia] Sync failed:",s.error)}catch{console.log("[Eudia] Startup sync skipped (server unreachable), will retry tomorrow")}},2e3)}this.settings.showCalendarView&&this.settings.userEmail&&await this.activateCalendarView(),this.settings.userEmail&&this.telemetry&&setTimeout(async()=>{try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),t=0;e&&e instanceof l.TFolder&&(t=e.children.filter(i=>i instanceof l.TFolder&&!i.name.startsWith("_")).length);let s={salesforce:this.settings.salesforceConnected?"connected":"not_configured",calendar:this.settings.calendarConfigured?"connected":"not_configured"};await this.telemetry.sendHeartbeat(t,s);let a=await this.telemetry.checkForPushedConfig();if(a.length>0){let i=!1;for(let r of a)r.key&&this.settings.hasOwnProperty(r.key)&&(this.settings[r.key]=r.value,i=!0,console.log(`[Eudia] Applied pushed config: ${r.key} = ${r.value}`));i&&(await this.saveSettings(),new l.Notice("Settings updated by admin"))}}catch{console.log("[Eudia] Heartbeat/config check skipped")}},3e3)}}else{await new Promise(t=>setTimeout(t,100));let e=document.querySelector(".modal-container .modal");if(e){let t=e.querySelector(".modal-close-button");t&&t.click()}await this.activateSetupView()}})}async onunload(){this.app.workspace.detachLeavesOfType(x)}async loadSettings(){this.settings=Object.assign({},ae,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async activateCalendarView(){let e=this.app.workspace,t=e.getLeavesOfType(x);if(t.length>0)e.revealLeaf(t[0]);else{let s=e.getRightLeaf(!1);s&&(await s.setViewState({type:x,active:!0}),e.revealLeaf(s))}}async activateSetupView(){let e=this.app.workspace,t=e.getLeavesOfType(b);if(t.length>0)e.revealLeaf(t[0]);else{let s=e.getLeaf(!0);s&&(await s.setViewState({type:b,active:!0}),e.revealLeaf(s))}}async createTailoredAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t);let a=0;for(let i of e){let r=i.name.replace(/[<>:"/\\|?*]/g,"_").trim(),o=`${t}/${r}`;if(this.app.vault.getAbstractFileByPath(o)instanceof l.TFolder){console.log(`[Eudia] Account folder already exists: ${r}`);continue}try{await this.app.vault.createFolder(o);let d=new Date().toISOString().split("T")[0],p=[{name:"Note 1.md",content:`---
account: "${i.name}"
account_id: "${i.id}"
type: meeting_note
sync_to_salesforce: false
...
`}];for(let g of p){let u=`${o}/${g.name}`;await this.app.vault.create(u,g.content)}a++,console.log(`[Eudia] Created account folder with subnotes: ${r}`)}catch(d){console.error(`[Eudia] Failed to create folder for ${r}:`,d)}}this.settings.cachedAccounts=e.map(i=>({id:i.id,name:i.name})),await this.saveSettings(),a>0&&new l.Notice(`Created ${a} account folders`),await this.ensureNextStepsFolderExists()}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let a=0,i=0,r=new Date().toISOString().split("T")[0];for(let o of e){let c=o.name.replace(/[<>:"/\\|?*]/g,"_").trim(),d=`${t}/${c}`;if(!(this.app.vault.getAbstractFileByPath(d)instanceof l.TFolder))try{await this.app.vault.createFolder(d),o.isOwned?(await this.createFullAccountSubnotes(d,o,r),a++):(await this.createViewOnlyAccountNote(d,o,r),i++),console.log(`[Eudia Admin] Created ${o.isOwned?"owned":"view-only"} folder: ${c}`)}catch(g){console.error(`[Eudia Admin] Failed to create folder for ${c}:`,g)}}this.settings.cachedAccounts=e.map(o=>({id:o.id,name:o.name})),await this.saveSettings(),a+i>0&&new l.Notice(`Created ${a} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createViewOnlyAccountNote(e,t,s){let a=t.ownerName||"Unknown",i=`---
account: "${t.name}"
account_id: "${t.id}"
type: view_only
owner: "${a}"
...
`;await this.app.vault.create(t,o)}}async updateAccountNextSteps(e,t,s){try{console.log(`[Eudia] updateAccountNextSteps called for: ${e}`),console.log(`[Eudia] Content length: ${t?.length||0} chars`);let a=e.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${this.settings.accountsFolder}/${a}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${i}`);let r=this.app.vault.getAbstractFileByPath(i);if(!r||!(r instanceof l.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${i}`);let A=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${a}`);A&&A instanceof l.TFolder?console.log(`[Eudia] Files in ${a} folder:`,A.children.map(k=>k.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${a}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),d=s.split("/").pop()?.replace(".md","")||"Meeting",p=t;!t.includes("- [ ]")&&!t.includes("- [x]")&&(p=t.split(`
`).filter(A=>A.trim()).map(A=>{let k=A.replace(/^[-•*]\s*/,"").trim();return k?`- [ ] ${k}`:""}).filter(Boolean).join(`
`));let g=await this.app.vault.read(r),u="",m=g.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);m&&m[1]&&(u=m[1].trim());let h=`### ${o} - ${d}
${p||"*None*"}`,y=u?`${h}
...
${y}
`;await this.app.vault.modify(r,v),console.log(`[Eudia] Updated Next Steps for ${e} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(a){console.error(`[Eudia] Failed to update Next Steps for ${e}:`,a)}}async regenerateNextStepsDashboard(){try{let t=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!t||!(t instanceof l.TFile)){await this.ensureNextStepsFolderExists();return}let s=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!s||!(s instanceof l.TFolder))return;let a=[];for(let c of s.children)if(c instanceof l.TFolder){let d=`${c.path}/Next Steps.md`,p=this.app.vault.getAbstractFileByPath(d);if(p instanceof l.TFile){let g=await this.app.vault.read(p),u=g.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),m=u?u[1]:"Unknown",h=g.split(`
`).filter(y=>y.match(/^- \[[ x]\]/)).slice(0,5);(h.length>0||m!=="Unknown")&&a.push({account:c.name,lastUpdated:m,nextSteps:h})}}a.sort((c,d)=>d.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],r=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`,o+=`|---------|--------------|------------|
`;for(let c of a){let d=c.nextSteps.filter(p=>p.includes("- [ ]")).length;o+=`| ${c.account} | ${c.lastUpdated} | ${d} |
...
---
`;await this.app.vault.modify(t,r+o),this.processTranscriptionAsync(e,t).catch(c=>{console.error("Background transcription failed:",c),new l.Notice(`Transcription failed: ${c.message}`)})}async processTranscriptionAsync(e,t){try{let s,a=t.path.split("/");if(console.log(`[Eudia] Processing transcription for: ${t.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(a)}, accountsFolder: ${this.settings.accountsFolder}`),a.length>=2&&a[0]===this.settings.accountsFolder){let u=a[1];console.log(`[Eudia] Detected account folder: ${u}`);let m=this.settings.cachedAccounts.find(h=>h.name.toLowerCase()===u.toLowerCase());m?(s={accountName:m.name,accountId:m.id},console.log(`[Eudia] Found cached account: ${m.name} (${m.id})`)):(s={accountName:u,accountId:""},console.log(`[Eudia] Account not in cache, using folder name: ${u}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let i=[];try{let u=await this.calendarService.getCurrentMeeting();u.meeting?.attendees&&(i=u.meeting.attendees.map(m=>m.name||m.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let r=await this.transcriptionService.transcribeAudio(e.audioBlob,s?{...s,speakerHints:i}:{speakerHints:i}),o=u=>u?!!(u.summary?.trim()||u.nextSteps?.trim()):!1,c=r.sections;if(o(c)||r.text?.trim()&&(c=await this.transcriptionService.processTranscription(r.text,s)),!o(c)&&!r.text?.trim()){let m=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,m+`

**Transcription failed:** No audio detected.
`),new l.Notice("Transcription failed: No audio detected.");return}let d=this.buildNoteContent(c,r);await this.app.vault.modify(t,d);let p=Math.floor(e.duration/60);new l.Notice(`Transcription complete (${p} min recording)`);let g=c.nextSteps||c.actionItems;console.log(`[Eudia] Next Steps extraction - accountContext: ${s?.accountName||"undefined"}`),console.log(`[Eudia] Next Steps content found: ${g?"YES ("+g.length+" chars)":"NO"}`),console.log(`[Eudia] sections.nextSteps: ${c.nextSteps?"YES":"NO"}, sections.actionItems: ${c.actionItems?"YES":"NO"}`),g&&s?.accountName?(console.log(`[Eudia] Calling updateAccountNextSteps for ${s.accountName}`),await this.updateAccountNextSteps(s.accountName,g,t.path)):console.log("[Eudia] Skipping Next Steps update - missing content or account context"),this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(s){try{let i=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,i+`
...
`):typeof y=="object"?JSON.stringify(y):String(y),a=s(e.title)||"Meeting Notes",i=s(e.summary),r=s(e.painPoints),o=s(e.productInterest),c=s(e.meddiccSignals),d=s(e.nextSteps),p=s(e.actionItems),g=s(e.keyDates),u=s(e.risksObjections),m=s(e.attendees),h=`---
title: "${a}"
...
${t.text}
`),h}openIntelligenceQuery(){new F(this.app,this).open()}openIntelligenceQueryForCurrentNote(){let e=this.app.workspace.getActiveFile(),t;if(e){let s=this.app.metadataCache.getFileCache(e)?.frontmatter;if(s?.account_id&&s?.account)t={id:s.account_id,name:s.account};else if(s?.account){let a=this.settings.cachedAccounts.find(i=>i.name.toLowerCase()===s.account.toLowerCase());a?t={id:a.id,name:a.name}:t={id:"",name:s.account}}else{let a=e.path.split("/");if(a.length>=2&&a[0]===this.settings.accountsFolder){let i=a[1],r=this.settings.cachedAccounts.find(o=>o.name.replace(/[<>:"/\\|?*]/g,"_").trim()===i);r?t={id:r.id,name:r.name}:t={id:"",name:i}}}}new F(this.app,this,t).open()}async syncAccounts(e=!1){e||new l.Notice("Syncing Salesforce accounts...");try{let s=(await(0,l.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!s.success||!s.accounts){e||new l.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=s.accounts.map(a=>({id:a.id,name:a.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),e||new l.Notice(`Synced ${s.accounts.length} accounts for matching`)}catch(t){e||new l.Notice(`Failed to sync accounts: ${t.message}`)}}async scanLocalAccountFolders(){try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!e||!(e instanceof l.TFolder))return;let t=[];for(let s of e.children)s instanceof l.TFolder&&t.push({id:`local-${s.name.replace(/\s+/g,"-").toLowerCase()}`,name:s.name});this.settings.cachedAccounts=t,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(e){console.error("Failed to scan local account folders:",e)}}async refreshAccountFolders(){if(!this.settings.userEmail)throw new Error("Please configure your email first");let e=new T(this.settings.serverUrl);if((await e.getAccountsForUser(this.settings.userEmail)).length===0)return console.log("[Eudia] No accounts found for user"),0;let s=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),a=[];if(s&&s instanceof l.TFolder)for(let r of s.children)r instanceof l.TFolder&&a.push(r.name);let i=await e.getNewAccounts(this.settings.userEmail,a);return i.length===0?(console.log("[Eudia] All account folders exist"),0):(console.log(`[Eudia] Creating ${i.length} new account folders`),await this.createTailoredAccountFolders(i),i.length)}async syncAccountFolders(){if(!this.settings.userEmail)return{success:!1,added:0,archived:0,error:"No email configured"};let e=this.settings.userEmail.toLowerCase().trim();console.log(`[Eudia] Syncing account folders for: ${e}`);try{let t=await fetch(`${this.settings.serverUrl}/api/bl-accounts/${encodeURIComponent(e)}`);if(!t.ok){let u=await t.json().catch(()=>({}));throw new Error(u.error||`Server returned ${t.status}`)}let s=await t.json();if(!s.success||!s.accounts)throw new Error(s.error||"Invalid response from server");let a=s.accounts,i=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),r=new Map;if(i&&i instanceof l.TFolder)for(let u of i.children)u instanceof l.TFolder&&!u.name.startsWith("_")&&r.set(u.name.toLowerCase().trim(),u);let o=new Set(a.map(u=>u.name.toLowerCase().trim())),c=a.filter(u=>{let m=u.name.toLowerCase().trim();return!r.has(m)}),d=[];for(let[u,m]of r.entries())o.has(u)||d.push(m);let p=0,g=0;return c.length>0&&(console.log(`[Eudia] Creating ${c.length} new account folders`),await this.createTailoredAccountFolders(c.map(u=>({id:u.id,name:u.name,type:u.customerType}))),p=c.length,this.telemetry&&this.telemetry.reportInfo("Accounts synced - added",{count:p})),this.settings.archiveRemovedAccounts&&d.length>0&&(console.log(`[Eudia] Archiving ${d.length} removed account folders`),g=await this.archiveAccountFolders(d),this.telemetry&&this.telemetry.reportInfo("Accounts synced - archived",{count:g})),console.log(`[Eudia] Sync complete: ${p} added, ${g} archived`),{success:!0,added:p,archived:g}}catch(t){return console.error("[Eudia] Account sync error:",t),this.telemetry&&this.telemetry.reportError("Account sync failed",{error:t.message}),{success:!1,added:0,archived:0,error:t.message}}}async archiveAccountFolders(e){let t=0,s=`${this.settings.accountsFolder}/_Archived`;this.app.vault.getAbstractFileByPath(s)||await this.app.vault.createFolder(s);for(let i of e)try{let r=`${s}/${i.name}`;if(this.app.vault.getAbstractFileByPath(r)){let p=new Date().toISOString().split("T")[0];await this.app.fileManager.renameFile(i,`${s}/${i.name}_${p}`)}else await this.app.fileManager.renameFile(i,r);let c=`${s}/${i.name}/_archived.md`,d=`---
...
`,p=await this.app.vault.create(c,d);await this.app.workspace.getLeaf().openFile(p),new l.Notice(`Created meeting note for ${s.name}`),e()}).open()})}async fetchAndInsertContext(){new l.Notice("Fetching pre-call context...")}},H=class extends l.PluginSettingTab{constructor(n,e){super(n,e),this.plugin=e}display(){let{containerEl:n}=this;n.empty(),n.createEl("h2",{text:"Eudia Sync & Scribe"}),n.createEl("h3",{text:"Your Profile"});let e=n.createDiv();e.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let t=e.createDiv();t.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let s=t.createSpan(),a=t.createSpan(),i=e.createDiv();i.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",i.setText("Connect with Salesforce to sync notes with your user attribution.");let r=e.createEl("button");r.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,c=async()=>{if(!this.plugin.settings.userEmail)return s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",a.setText("Enter email above first"),r.setText("Setup Required"),r.disabled=!0,r.style.opacity="0.5",r.style.cursor="not-allowed",!1;r.disabled=!1,r.style.opacity="1",r.style.cursor="pointer";try{return s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",a.setText("Checking..."),(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",a.setText("Connected to Salesforce"),r.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",a.setText("Not connected"),r.setText("Connect to Salesforce"),!1)}catch{return s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",a.setText("Status unavailable"),r.setText("Connect to Salesforce"),!1}};new l.Setting(n).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(m=>m.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async h=>{let y=h.trim().toLowerCase();this.plugin.settings.userEmail=y,await this.plugin.saveSettings(),await c()})),new l.Setting(n).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(m=>{G.forEach(h=>{m.addOption(h.value,h.label)}),m.setValue(this.plugin.settings.timezone),m.onChange(async h=>{this.plugin.settings.timezone=h,await this.plugin.saveSettings(),new l.Notice(`Timezone set to ${G.find(y=>y.value===h)?.label||h}`)})}),n.createEl("h3",{text:"Salesforce Connection"}),n.appendChild(e);let d=()=>{o&&window.clearInterval(o);let m=0,h=30;o=window.setInterval(async()=>{m++,await c()?(o&&(window.clearInterval(o),o=null),new l.Notice("Salesforce connected successfully!")):m>=h&&o&&(window.clearInterval(o),o=null)},5e3)};r.onclick=async()=>{if(!this.plugin.settings.userEmail){new l.Notice("Please enter your email first");return}let m=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(m,"_blank"),new l.Notice("Complete the Salesforce login in the popup window",5e3),d()},c(),n.createEl("h3",{text:"Server"}),new l.Setting(n).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(m=>m.setValue(this.plugin.settings.serverUrl).onChange(async h=>{this.plugin.settings.serverUrl=h,await this.plugin.saveSettings()}));let p=n.createDiv({cls:"settings-advanced-collapsed"}),g=p.createDiv({cls:"eudia-transcription-status"});g.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",g.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?g.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':g.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{g.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let u=new l.Setting(n).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(m=>m.setValue(!1).onChange(h=>{p.style.display=h?"block":"none"}));p.style.display="none",n.createEl("h3",{text:"Transcription"}),new l.Setting(n).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(m=>m.setValue(this.plugin.settings.saveAudioFiles).onChange(async h=>{this.plugin.settings.saveAudioFiles=h,await this.plugin.saveSettings()})),new l.Setting(n).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(m=>m.setValue(this.plugin.settings.appendTranscript).onChange(async h=>{this.plugin.settings.appendTranscript=h,await this.plugin.saveSettings()})),n.createEl("h3",{text:"Sync"}),new l.Setting(n).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(m=>m.setValue(this.plugin.settings.syncOnStartup).onChange(async h=>{this.plugin.settings.syncOnStartup=h,await this.plugin.saveSettings()})),new l.Setting(n).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(m=>m.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async h=>{this.plugin.settings.autoSyncAfterTranscription=h,await this.plugin.saveSettings()})),n.createEl("h3",{text:"Folders"}),new l.Setting(n).setName("Accounts Folder").setDesc("Where account folders are stored").addText(m=>m.setValue(this.plugin.settings.accountsFolder).onChange(async h=>{this.plugin.settings.accountsFolder=h||"Accounts",await this.plugin.saveSettings()})),new l.Setting(n).setName("Recordings Folder").setDesc("Where audio files are saved").addText(m=>m.setValue(this.plugin.settings.recordingsFolder).onChange(async h=>{this.plugin.settings.recordingsFolder=h||"Recordings",await this.plugin.saveSettings()})),n.createEl("h3",{text:"Actions"}),new l.Setting(n).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(m=>m.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new l.Setting(n).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(m=>m.setButtonText("Refresh Folders").onClick(async()=>{m.setButtonText("Checking..."),m.setDisabled(!0);try{let h=await this.plugin.refreshAccountFolders();h>0?new l.Notice(`Created ${h} new account folder${h>1?"s":""}`):new l.Notice("All account folders are up to date")}catch(h){new l.Notice("Failed to refresh folders: "+h.message)}m.setButtonText("Refresh Folders"),m.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&n.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),n.createEl("p",{text:`Audio transcription: ${w.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
