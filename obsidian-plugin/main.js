...
- Action items have clear owners`}function We(S,a){let t="";return a?.account&&(t=`
ACCOUNT CONTEXT:
...
`}function Te(S,a){let t="";return a?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${a.account.name}
${a.account.owner?`- Owner: ${a.account.owner}`:""}
...
`}function Ie(S,a){let t="";return a?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${a.account.name}
${a.account.owner?`- Owner: ${a.account.owner}`:""}
...
`);let i=e?.enabled&&e?.formattedTranscript?e.formattedTranscript:t;if(i){let r=e?.enabled?"Full Transcript (Speaker-Attributed)":"Full Transcript";n+=`---
...
`}return n}static formatSectionsWithAudio(a,t,e,n){let i=this.formatSectionsForNote(a,t,n);return e&&(i+=`
---

## Recording

![[${e}]]
`),i}static formatContextForNote(a){if(!a.success)return"";let t=`## Pre-Call Context
...
`,t}async blobToBase64(a){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let r=n.result.split(",")[1];t(r)},n.onerror=e,n.readAsDataURL(a)})}async transcribeAudio(a,t){let e=a.size/1024/1024,n=a.type||"audio/webm";if(e>15)return console.log(`[Eudia] Large recording (${e.toFixed(1)}MB) \u2014 using chunked transcription`),this.transcribeAudioChunked(a,n,t);try{let i=await this.blobToBase64(a),r=t?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:t.pipelineContext}:void 0,s=await this.transcribeAndSummarize(i,n,t?.accountName,t?.accountId,r,{captureMode:t?.captureMode,hasVirtualDevice:t?.hasVirtualDevice},t?.meetingTemplate);return{text:s.transcript,confidence:s.success?.95:0,duration:s.duration,sections:s.sections,diarizedTranscript:s.diarizedTranscript,error:s.error}}catch(i){return console.error("transcribeAudio error:",i),{text:"",confidence:0,duration:0,sections:this.getEmptySections(),error:i.message||"Transcription request failed"}}}static{this.CHUNK_MAX_RETRIES=3}static{this.CHUNK_RETRY_DELAYS=[5e3,15e3,3e4]}async transcribeChunkWithRetry(a,t,e,n){for(let i=0;i<=S.CHUNK_MAX_RETRIES;i++){if(i>0){let r=S.CHUNK_RETRY_DELAYS[i-1]||3e4;console.log(`[Eudia] Chunk ${e+1}/${n} retry ${i}/${S.CHUNK_MAX_RETRIES} in ${r/1e3}s...`),await new Promise(s=>setTimeout(s,r))}try{let r=await(0,P.requestUrl)({url:`${this.serverUrl}/api/transcribe-chunk`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({audio:a,mimeType:t})}),s=r.json?.text||r.json?.transcript||"";if(r.json?.success&&s)return i>0&&console.log(`[Eudia] Chunk ${e+1}/${n} succeeded on retry ${i}`),{text:s,duration:r.json.duration||0};console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${i+1} returned no text: ${r.json?.error||"unknown"}`)}catch(r){console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${i+1} failed: ${r.message}`)}}return null}async transcribeAudioChunked(a,t,e){let i=await a.arrayBuffer(),r=i.byteLength,s=Math.ceil(r/8388608);console.log(`[Eudia] Chunked transcription: ${(r/1024/1024).toFixed(1)}MB \u2192 ${s} chunks`);let o=16*1024,l=r/o,c=[],d=0,u=0;for(let g=0;g<s;g++){let O=g*8388608,b=Math.min(O+8388608,r),v=i.slice(O,b),C=new Blob([v],{type:t});console.log(`[Eudia] Transcribing chunk ${g+1}/${s} (${((b-O)/1024/1024).toFixed(1)}MB)`);let f=await this.blobToBase64(C),h=await this.transcribeChunkWithRetry(f,t,g,s);if(h)c.push(h.text),d+=h.duration,console.log(`[Eudia] Chunk ${g+1}/${s} OK: ${h.text.length} chars`);else{u++;let w=Math.round(O/r*l),x=Math.round(b/r*l),E=`${Math.floor(w/60)}:${(w%60).toString().padStart(2,"0")}`,k=`${Math.floor(x/60)}:${(x%60).toString().padStart(2,"0")}`,j=`
...
`),e=e.replace(/^#{1,3}\s+.+\n+(?=#{1,3}\s|\s*$)/gm,""),e=e.replace(/^#{2,3}\s+(.+)$/gm,'</p><h3 class="eudia-intel-header">$1</h3><p>'),e=e.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^-\s+\[\s*\]\s+(.+)$/gm,'<li class="eudia-intel-todo">$1</li>'),e=e.replace(/^-\s+\[x\]\s+(.+)$/gm,'<li class="eudia-intel-done">$1</li>'),e=e.replace(/^[•\-]\s+(.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li[^>]*>.*?<\/li>\s*)+/g,'<ul class="eudia-intel-list">$&</ul>'),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=e.replace(/<p>\s*(<ul)/g,"$1"),e=e.replace(/<\/ul>\s*<\/p>/g,"</ul>"),e=e.replace(/<p>\s*(<h3)/g,"$1"),e=e.replace(/<\/h3>\s*<\/p>/g,"</h3>"),e=e.replace(/<\/li>\s*<br>\s*<li/g,"</li><li"),e=e.replace(/<p>\s*<\/p>/g,""),e=e.replace(/<p>\s*<br>\s*<\/p>/g,""),e=e.replace(/(<br>\s*){2,}/g,""),e=e.replace(/<\/h3>\s*<br>/g,"</h3>"),e=e.replace(/<br>\s*<h3/g,"<h3"),e=e.replace(/<br>\s*<ul/g,"<ul"),e=e.replace(/<\/ul>\s*<br>/g,"</ul>"),e=e.replace(/^(<br>)+|(<br>)+$/g,""),e="<p>"+e+"</p>",e=e.replace(/<p><\/p>/g,""),e}onClose(){this.contentEl.empty()}};var se=class extends p.ItemView{constructor(t,e){super(t);this.emailInput=null;this.pollInterval=null;this.plugin=e,this.accountOwnershipService=new H(e.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return _}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}if(this.plugin.settings.accountsImported){let e=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(s=>s.children!==void 0)||[];e.length>0?(this.steps[2].status="complete",console.log(`[Eudia] Vault reopen: ${e.length} account folders verified`)):(console.warn("[Eudia] accountsImported=true but 0 account folders found \u2014 resetting for re-import"),this.plugin.settings.accountsImported=!1,this.plugin.settings.importedAccountCount=0,await this.plugin.saveSettings());try{let s=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");s&&await this.plugin.app.vault.delete(s)}catch{}let n=this.plugin.settings.userEmail,r=(this.plugin.settings.cachedAccounts||[]).filter(s=>s.id&&String(s.id).startsWith("001"));if(n&&r.length>0){let s=this.plugin.settings.accountsFolder||"Accounts",o=!1;for(let l of r.slice(0,5)){let c=(l.name||"").replace(/[<>:"/\\|?*]/g,"_").trim(),d=`${s}/${c}/Contacts.md`,u=this.plugin.app.vault.getAbstractFileByPath(d);if(u instanceof p.TFile&&!this.plugin.app.metadataCache.getFileCache(u)?.frontmatter?.enriched_at){o=!0;break}}o&&(console.log("[Eudia Setup] Accounts need enrichment \u2014 triggering on vault reopen..."),setTimeout(async()=>{try{let l=r.map(c=>({id:c.id,name:c.name,type:"",isOwned:!1,hadOpportunity:!0,website:null,industry:null}));await this.plugin.enrichAccountFolders(l),console.log(`[Eudia] Vault-reopen enrichment complete: ${l.length} accounts enriched`)}catch(l){console.log("[Eudia] Vault-reopen enrichment failed (will retry next open):",l)}},3e3))}}else{console.log("[Eudia Setup] Email set but accounts not imported \u2014 auto-retrying import...");let t=this.plugin.app.workspace.leftSplit,e=t?.collapsed;try{let n=this.plugin.settings.userEmail,i=M(n)?"admin":B(n)?"cs":"bl",r=[],s=[];if(console.log(`[Eudia Setup] Auto-retry for ${n} (group: ${i})`),i==="cs")r=[...D],console.log(`[Eudia Setup] Auto-retry CS: using ${r.length} static accounts`);else if(i==="admin")r=await this.accountOwnershipService.getAllAccountsForAdmin(n);else{let o=await this.accountOwnershipService.getAccountsWithProspects(n);r=o.accounts,s=o.prospects}if(r.length>0||s.length>0){if(t&&!e&&t.collapse(),i==="admin"?await this.plugin.createAdminAccountFolders(r):(await this.plugin.createTailoredAccountFolders(r,{}),s.length>0&&await this.plugin.createProspectAccountFiles(s)),U(n))try{await this.plugin.createCSManagerDashboard(n,r)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=r.length+s.length,await this.plugin.saveSettings(),this.steps[2].status="complete";try{let o=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");o&&await this.plugin.app.vault.delete(o)}catch{}t&&!e&&t.expand(),console.log(`[Eudia Setup] Auto-retry imported ${r.length} accounts for ${n}`),new p.Notice(`Enriching ${r.length} accounts with Salesforce contacts...`);try{let o=i==="cs"?r:[...r,...s];await this.plugin.enrichAccountFolders(o),new p.Notice(`${r.length} accounts loaded and enriched!`),console.log("[Eudia Setup] Auto-retry enrichment complete")}catch(o){console.log("[Eudia Setup] Auto-retry enrichment failed:",o),new p.Notice(`${r.length} accounts imported! Contacts will populate on next open.`)}}else console.warn(`[Eudia Setup] Auto-retry returned 0 accounts for ${n}. Server may still be starting.`),t&&!e&&t.expand()}catch(n){console.error("[Eudia Setup] Auto-retry account import failed:",n),t&&!e&&t.expand()}}}}getCompletionPercentage(){let t=this.steps.filter(e=>e.status==="complete").length;return Math.round(t/this.steps.length*100)}async render(){let t=this.containerEl.children[1];t.empty(),t.addClass("eudia-setup-view"),this.renderHeader(t),this.renderSteps(t),this.renderFooter(t)}renderHeader(t){let e=t.createDiv({cls:"eudia-setup-header"}),n=e.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to transcribe and summarize meetings -- capturing objections, next steps, and pain points to drive better client outcomes and smarter selling.",cls:"eudia-setup-subtitle"});let i=e.createDiv({cls:"eudia-setup-progress-section"}),r=this.getCompletionPercentage(),s=i.createDiv({cls:"eudia-setup-progress-label"});s.createSpan({text:"Setup Progress"}),s.createSpan({text:`${r}%`,cls:"eudia-setup-progress-value"});let l=i.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});l.style.width=`${r}%`}renderSteps(t){let e=t.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(e),this.renderSalesforceStep(e),this.renderTranscribeStep(e)}renderCalendarStep(t){let e=this.steps[0],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),r=i.createDiv({cls:"eudia-setup-step-number"});r.setText(e.status==="complete"?"":"1"),e.status==="complete"&&r.addClass("eudia-step-complete");let s=i.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:e.title}),s.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(e.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let l=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=l.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let c=l.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});c.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async d=>{d.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let t=this.emailInput.value.trim().toLowerCase(),e=this.containerEl.querySelector(".eudia-setup-validation-message");if(!t){e&&(e.textContent="Please enter your email",e.className="eudia-setup-validation-message error");return}if(!t.endsWith("@eudia.com")){e&&(e.textContent="Please use your @eudia.com email address",e.className="eudia-setup-validation-message error");return}e&&(e.textContent="Validating...",e.className="eudia-setup-validation-message loading");try{let n=await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(t)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=t,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new p.Notice("Calendar connected successfully!"),e&&(e.textContent="Importing your accounts...",e.className="eudia-setup-validation-message loading");let i=this.plugin.app.workspace.leftSplit,r=i?.collapsed;i&&!r&&i.collapse();try{let s,o=[],l=M(t)?"admin":B(t)?"cs":"bl";if(console.log(`[Eudia] User group detected: ${l} for ${t}`),l==="cs"){if(console.log(`[Eudia] CS user detected \u2014 loading ${D.length} accounts from static data (instant, no server needed)`),s=[...D],o=[],e&&(e.textContent=`Loading ${s.length} Customer Success accounts...`),await this.plugin.createTailoredAccountFolders(s,{}),U(t))try{await this.plugin.createCSManagerDashboard(t,s)}catch(u){console.error("[Eudia] CS Manager dashboard creation failed (non-blocking):",u)}let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(u=>u.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length,console.log(`[Eudia] CS accounts verified: ${d.length} folders created`)):(console.warn(`[Eudia] CS folder creation may have failed \u2014 ${d.length} folders found. Keeping accountsImported=false for retry.`),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let u=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");u&&await this.plugin.app.vault.delete(u)}catch{}console.log(`[Eudia] CS accounts created: ${s.length} folders from static data`),e&&(e.textContent=`Enriching ${s.length} accounts with Salesforce contacts...`),new p.Notice(`Enriching ${s.length} accounts with contacts from Salesforce...`),console.log(`[Eudia] Starting synchronous enrichment for ${s.length} CS accounts...`);try{await this.plugin.enrichAccountFolders(s),console.log("[Eudia] Synchronous enrichment complete"),new p.Notice(`${s.length} accounts loaded with contacts from Salesforce!`),e&&(e.textContent=`${s.length} accounts loaded and enriched with Salesforce contacts!`)}catch(u){console.log("[Eudia] Synchronous enrichment failed, will retry in background:",u),new p.Notice(`${s.length} accounts loaded! Contacts will populate shortly...`);let y=t,m=[5e3,2e4,6e4],g=async O=>{let b=m[O];if(b!==void 0){await new Promise(v=>setTimeout(v,b));try{await this.plugin.enrichAccountFolders(s),console.log(`[Eudia] Background enrichment retry ${O+1} succeeded`)}catch{return g(O+1)}}};g(0)}}else if(l==="admin"){if(console.log("[Eudia] Admin user detected - importing all accounts"),s=await this.accountOwnershipService.getAllAccountsForAdmin(t),s.length>0){e&&(e.textContent=`Creating ${s.length} account folders...`),await this.plugin.createAdminAccountFolders(s);let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(y=>y.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length,console.log(`[Eudia] Admin accounts verified: ${d.length} folders created`)):(console.warn("[Eudia] Admin folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let y=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");y&&await this.plugin.app.vault.delete(y)}catch{}new p.Notice(`Imported ${s.length} accounts! Enriching with Salesforce data...`);let u=s.filter(y=>y.id&&y.id.startsWith("001"));if(u.length>0){e&&(e.textContent=`Enriching ${u.length} accounts with Salesforce contacts...`);try{await this.plugin.enrichAccountFolders(u),new p.Notice(`${s.length} accounts loaded and enriched with Salesforce data!`),console.log(`[Eudia] Admin/exec synchronous enrichment complete: ${u.length} accounts`),e&&(e.textContent=`${s.length} accounts loaded and enriched!`)}catch(y){console.log("[Eudia] Admin/exec synchronous enrichment failed, will retry on next open:",y),new p.Notice(`${s.length} accounts imported! Contacts will populate on next vault open.`);let m=[5e3,2e4,6e4],g=async O=>{let b=m[O];if(b!==void 0){await new Promise(v=>setTimeout(v,b));try{await this.plugin.enrichAccountFolders(u),console.log(`[Eudia] Admin/exec background enrichment retry ${O+1} succeeded`)}catch{return g(O+1)}}};g(0)}}}}else{let c=await this.accountOwnershipService.getAccountsWithProspects(t);if(s=c.accounts,o=c.prospects,s.length>0||o.length>0){e&&(e.textContent=`Creating ${s.length} account folders...`),await this.plugin.createTailoredAccountFolders(s,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o);let u=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(m=>m.children!==void 0)||[];u.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+o.length,console.log(`[Eudia] BL accounts verified: ${u.length} folders created`)):(console.warn("[Eudia] BL folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let m=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");m&&await this.plugin.app.vault.delete(m)}catch{}new p.Notice(`Imported ${s.length} active accounts + ${o.length} prospects!`);let y=[...s,...o];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(y)}catch(m){console.log("[Eudia] Background enrichment skipped:",m)}},500)}else{console.warn(`[Eudia] No accounts returned for ${t} \u2014 auto-retrying...`);let d=!1;for(let u=1;u<=3;u++){e&&(e.textContent=`Server warming up... retrying in 10s (attempt ${u}/3)`,e.className="eudia-setup-validation-message warning"),await new Promise(y=>setTimeout(y,1e4));try{let y=await this.plugin.accountOwnershipService.getAccountsWithProspects(t);if(y.accounts.length>0||y.prospects.length>0){s=y.accounts,o=y.prospects,e&&(e.textContent=`Creating ${s.length} account folders...`),await this.plugin.createTailoredAccountFolders(s,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o),(this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(O=>O.children!==void 0)||[]).length>0&&(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+o.length),await this.plugin.saveSettings(),new p.Notice(`Imported ${s.length} accounts + ${o.length} prospects!`),d=!0;break}}catch(y){console.warn(`[Eudia] Retry ${u} failed:`,y)}}d||(e&&(e.textContent="Could not load accounts after 3 attempts. Close this window, wait 1 minute, then re-open Obsidian and try again.",e.className="eudia-setup-validation-message error"),new p.Notice("Account import failed after retries. Wait 1 minute and try again."))}}}catch(s){console.error("[Eudia] Account import failed:",s),e&&(e.textContent="Account import failed. Please try again.",e.className="eudia-setup-validation-message error"),new p.Notice("Account import failed \u2014 please try again.")}finally{i&&!r&&i.expand()}await this.render()}else e&&(e.innerHTML=`<strong>${t}</strong> is not authorized for calendar access. Contact your admin.`,e.className="eudia-setup-validation-message error")}catch{e&&(e.textContent="Connection failed. Please try again.",e.className="eudia-setup-validation-message error")}}renderSalesforceStep(t){let e=this.steps[1],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),r=i.createDiv({cls:"eudia-setup-step-number"});r.setText(e.status==="complete"?"":"2"),e.status==="complete"&&r.addClass("eudia-step-complete");let s=i.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:e.title}),s.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(e.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let c=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),d=o.createDiv({cls:"eudia-setup-sf-status"});c.onclick=async()=>{let u=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(u,"_blank"),d.textContent="Complete the login in the popup window...",d.className="eudia-setup-sf-status loading",new p.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(d)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(t){this.pollInterval&&window.clearInterval(this.pollInterval);let e=0,n=60;this.pollInterval=window.setInterval(async()=>{e++;try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new p.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(t),await this.render()):e>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),t.textContent="Connection timed out. Please try again.",t.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(t){t.textContent="Importing your accounts...",t.className="eudia-setup-sf-status loading";try{let e=this.plugin.settings.userEmail,n=M(e)?"admin":B(e)?"cs":"bl";console.log(`[Eudia SF Import] Importing for ${e} (group: ${n})`);let i,r=[];if(n==="cs"){console.log("[Eudia SF Import] CS user SF Connect \u2014 fetching live data from Salesforce..."),t.textContent="Syncing with Salesforce for latest account data...";try{let m=await this.accountOwnershipService.getCSAccounts(e);i=m.accounts,r=m.prospects,console.log(`[Eudia SF Import] CS server sync: ${i.length} accounts (with real SF IDs + CSM data)`)}catch{if(this.plugin.settings.accountsImported){t.textContent="Salesforce connected! Account folders already loaded. Enrichment will retry later.",t.className="eudia-setup-sf-status success",this.steps[1].status="complete";return}i=[...D],console.log(`[Eudia SF Import] CS server unavailable \u2014 using ${i.length} static accounts`)}}else if(n==="admin")console.log("[Eudia] Admin user detected - importing all accounts"),t.textContent="Admin detected - importing all accounts...",i=await this.accountOwnershipService.getAllAccountsForAdmin(e);else{let m=await this.accountOwnershipService.getAccountsWithProspects(e);i=m.accounts,r=m.prospects}if(i.length===0&&r.length===0){t.textContent="No accounts found for your email. Contact your admin.",t.className="eudia-setup-sf-status warning";return}t.textContent=`Creating ${i.length} account folders...`;let s=this.plugin.app.workspace.leftSplit,o=s?.collapsed;if(s&&!o&&s.collapse(),M(e)?await this.plugin.createAdminAccountFolders(i):(await this.plugin.createTailoredAccountFolders(i,{}),r.length>0&&await this.plugin.createProspectAccountFiles(r)),U(e))try{await this.plugin.createCSManagerDashboard(e,i)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=i.length+r.length,await this.plugin.saveSettings();try{let m=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");m&&await this.plugin.app.vault.delete(m)}catch{}s&&!o&&s.expand(),this.steps[2].status="complete";let l=i.filter(m=>m.isOwned!==!1).length,c=i.filter(m=>m.isOwned===!1).length;n==="admin"&&c>0?t.textContent=`${l} owned + ${c} view-only accounts imported! Enriching...`:t.textContent=`${i.length} active + ${r.length} prospect accounts imported! Enriching...`,t.className="eudia-setup-sf-status success";let d=[...i,...r],u=e,y=n;setTimeout(async()=>{try{let m=d.filter(g=>g.id&&g.id.startsWith("001"));if(m.length>0?(t.textContent=`Enriching ${m.length} accounts with Salesforce data...`,await this.plugin.enrichAccountFolders(m),t.textContent=`${i.length} accounts imported, ${m.length} enriched with Salesforce data`):t.textContent=`${i.length} accounts imported (enrichment requires Salesforce IDs)`,y==="cs"&&U(u))try{console.log("[Eudia SF Import] Regenerating CS Manager dashboard with live CSM data..."),await this.plugin.createCSManagerDashboard(u,i),console.log("[Eudia SF Import] CS Manager dashboard updated with CSM assignments")}catch(g){console.error("[Eudia SF Import] Dashboard regeneration failed (non-blocking):",g)}}catch(m){console.log("[Eudia] Background enrichment skipped:",m),t.textContent=`${i.length+r.length} accounts imported (enrichment will retry on next launch)`}},500)}catch{t.textContent="Failed to import accounts. Please try again.",t.className="eudia-setup-sf-status error";let n=this.plugin.app.workspace.leftSplit;if(n?.collapsed===!1)try{n.expand()}catch{}}}renderTranscribeStep(t){let e=this.steps[2],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),r=i.createDiv({cls:"eudia-setup-step-number"});r.setText(e.status==="complete"?"":"3"),e.status==="complete"&&r.addClass("eudia-step-complete");let s=i.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:e.title}),s.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),l=o.createDiv({cls:"eudia-setup-instructions"}),c=l.createDiv({cls:"eudia-setup-instruction"});c.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),c.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let d=l.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),d.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let u=l.createDiv({cls:"eudia-setup-instruction"});u.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),u.createSpan({text:"AI will summarize and extract key insights automatically"}),e.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(t){let e=t.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(r=>r.status==="complete")){let r=e.createDiv({cls:"eudia-setup-completion"});r.createEl("h2",{text:"\u{1F389} You're all set!"}),r.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let s=e.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});s.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(_),await this.plugin.activateCalendarView()}}else{let r=e.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});r.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(_),new p.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let i=e.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});i.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},re=class extends p.ItemView{constructor(t,e){super(t);this.updateInterval=null;this.chatHistory=[];this.plugin=e}getViewType(){return L}getDisplayText(){return"Live Query"}getIcon(){return"message-circle"}async onOpen(){await this.render(),this.updateInterval=window.setInterval(()=>this.updateStatus(),5e3)}async onClose(){this.updateInterval&&window.clearInterval(this.updateInterval)}updateStatus(){let t=this.containerEl.querySelector(".eudia-lq-status");if(t)if(this.plugin.audioRecorder?.isRecording()){let e=Math.round((this.plugin.liveTranscript?.length||0)/5),n=this.plugin.audioRecorder.getState().duration,i=Math.floor(n/60),r=n%60;t.setText(`Recording ${i}:${r.toString().padStart(2,"0")} \u2014 ${e} words captured`),t.style.color="var(--text-success)"}else t.setText("Not recording. Start a recording to use Live Query."),t.style.color="var(--text-muted)"}async render(){let t=this.containerEl.children[1];t.empty(),t.addClass("eudia-live-query-view"),t.style.cssText="display:flex;flex-direction:column;height:100%;padding:12px;";let e=t.createDiv({cls:"eudia-lq-status"});e.style.cssText="font-size:12px;padding:8px 0;border-bottom:1px solid var(--background-modifier-border);margin-bottom:8px;",this.updateStatus();let n=t.createDiv({cls:"eudia-lq-quick-actions"});n.style.cssText="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;";let i=[{label:"Summarize so far",query:"Give me a concise summary of everything discussed so far."},{label:"Action items",query:"What action items or next steps have been discussed so far?"},{label:"Key concerns",query:"What concerns, objections, or risks have been raised?"}];for(let c of i){let d=n.createEl("button",{text:c.label});d.style.cssText="font-size:11px;padding:4px 10px;border-radius:12px;border:1px solid var(--background-modifier-border);cursor:pointer;background:var(--background-secondary);",d.onclick=()=>this.submitQuery(c.query,r,o)}let r=t.createDiv({cls:"eudia-lq-chat"});r.style.cssText="flex:1;overflow-y:auto;margin-bottom:12px;display:flex;flex-direction:column;gap:8px;";for(let c of this.chatHistory)this.renderMessage(r,c.role,c.text);if(this.chatHistory.length===0){let c=r.createDiv();c.style.cssText="color:var(--text-muted);font-size:12px;text-align:center;padding:20px 0;",c.setText("Ask a question about the conversation while recording.")}let s=t.createDiv({cls:"eudia-lq-input-area"});s.style.cssText="display:flex;gap:8px;border-top:1px solid var(--background-modifier-border);padding-top:8px;";let o=s.createEl("textarea",{attr:{placeholder:"Ask about the conversation...",rows:"2"}});o.style.cssText="flex:1;resize:none;border-radius:8px;padding:8px;font-size:13px;border:1px solid var(--background-modifier-border);background:var(--background-primary);";let l=s.createEl("button",{text:"Ask"});l.style.cssText="padding:8px 16px;border-radius:8px;cursor:pointer;align-self:flex-end;font-weight:600;",l.addClass("mod-cta"),l.onclick=()=>this.submitQuery(o.value.trim(),r,o),o.addEventListener("keydown",c=>{c.key==="Enter"&&!c.shiftKey&&(c.preventDefault(),l.click())})}renderMessage(t,e,n){let i=t.querySelector(".eudia-lq-chat > div:only-child");i&&i.textContent?.includes("Ask a question")&&i.remove();let r=t.createDiv(),s=e==="user";r.style.cssText=`padding:8px 12px;border-radius:10px;font-size:13px;line-height:1.5;max-width:90%;${s?"align-self:flex-end;background:var(--interactive-accent);color:var(--text-on-accent);":"align-self:flex-start;background:var(--background-secondary);"}`,r.setText(n)}async submitQuery(t,e,n){if(!t)return;if(!this.plugin.audioRecorder?.isRecording()){new p.Notice("Start a recording first to use Live Query.");return}let i=this.plugin.liveTranscript||"";if(i.length<50){new p.Notice("Not enough transcript captured yet. Keep recording for a few more minutes.");return}this.chatHistory.push({role:"user",text:t}),this.renderMessage(e,"user",t),n.value="";let r=e.createDiv();r.style.cssText="align-self:flex-start;padding:8px 12px;border-radius:10px;font-size:13px;background:var(--background-secondary);color:var(--text-muted);",r.setText("Thinking..."),e.scrollTop=e.scrollHeight;try{let s=await this.plugin.transcriptionService.liveQueryTranscript(t,i,this.plugin.getAccountNameFromActiveFile());r.remove();let o=s.success?s.answer:s.error||"Query failed";this.chatHistory.push({role:"assistant",text:o}),this.renderMessage(e,"assistant",o)}catch(s){r.remove();let o=`Error: ${s.message}`;this.chatHistory.push({role:"assistant",text:o}),this.renderMessage(e,"assistant",o)}e.scrollTop=e.scrollHeight}},oe=class S extends p.ItemView{constructor(t,e){super(t);this.refreshInterval=null;this.lastError=null;this.showExternalOnly=!0;this.weeksBack=2;this.plugin=e}getViewType(){return G}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let t=this.containerEl.children[1];if(t.empty(),t.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(t);return}this.renderHeader(t),await this.renderCalendarContent(t)}static{this.INTERNAL_SUBJECT_PATTERNS=[/^block\b/i,/\bblock\s+for\b/i,/\bcommute\b/i,/\bpersonal\b/i,/\blunch\b/i,/\bOOO\b/i,/\bout of office\b/i,/\bfocus time\b/i,/\bno meetings?\b/i,/\bmeeting free\b/i,/\btravel\b/i,/\beye appt\b/i,/\bdoctor\b/i,/\bdentist\b/i,/\bgym\b/i,/\bworkout\b/i]}isExternalMeeting(t){if(t.isCustomerMeeting)return!0;if(!t.attendees||t.attendees.length===0)return!1;let e=this.plugin.settings.userEmail?.split("@")[1]||"eudia.com";if(t.attendees.some(i=>{if(i.isExternal===!0)return!0;if(i.isExternal===!1||!i.email)return!1;let r=i.email.split("@")[1]?.toLowerCase();return r&&r!==e.toLowerCase()}))return!0;for(let i of S.INTERNAL_SUBJECT_PATTERNS)if(i.test(t.subject))return!1;return!1}renderHeader(t){let e=t.createDiv({cls:"eudia-calendar-header"}),n=e.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let i=n.createDiv({cls:"eudia-calendar-actions"}),r=i.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});r.title="Refresh",r.onclick=async()=>{r.addClass("spinning"),this._forceRefresh=!0,await this.render(),r.removeClass("spinning")};let s=i.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});s.title="Settings",s.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=e.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o);let l=e.createDiv({cls:"eudia-calendar-filter-row"});l.style.cssText="display:flex;align-items:center;gap:8px;margin-top:6px;padding:4px 0;";let c=l.createEl("button",{text:this.showExternalOnly?"External Only":"All Meetings",cls:"eudia-filter-toggle"});c.style.cssText=`font-size:11px;padding:3px 10px;border-radius:12px;cursor:pointer;border:1px solid var(--background-modifier-border);background:${this.showExternalOnly?"var(--interactive-accent)":"var(--background-secondary)"};color:${this.showExternalOnly?"var(--text-on-accent)":"var(--text-muted)"};`,c.title=this.showExternalOnly?"Showing customer/external meetings only \u2014 click to show all":"Showing all meetings \u2014 click to filter to external only",c.onclick=async()=>{this.showExternalOnly=!this.showExternalOnly,await this.render()}}async renderConnectionStatus(t){let e={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,i=this.plugin.settings.userEmail;try{(await(0,p.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(e.server="connected",e.serverMessage="Server online"):(e.server="error",e.serverMessage="Server unavailable")}catch{e.server="error",e.serverMessage="Cannot reach server"}if(i&&e.server==="connected")try{let d=await(0,p.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(i)}`,method:"GET",throw:!1});d.status===200&&d.json?.authorized?(e.calendar="connected",e.calendarMessage="Calendar synced"):(e.calendar="not_authorized",e.calendarMessage="Not authorized")}catch{e.calendar="error",e.calendarMessage="Error checking access"}if(i&&e.server==="connected")try{let d=await(0,p.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(i)}`,method:"GET",throw:!1});d.status===200&&d.json?.connected?(e.salesforce="connected",e.salesforceMessage="Salesforce connected"):(e.salesforce="not_configured",e.salesforceMessage="Not connected")}catch{e.salesforce="not_configured"}let r=t.createDiv({cls:"eudia-status-indicators"}),s=r.createSpan({cls:`eudia-status-dot ${e.server}`});s.title=e.serverMessage||"Server";let o=r.createSpan({cls:`eudia-status-dot ${e.calendar}`});o.title=e.calendarMessage||"Calendar";let l=r.createSpan({cls:`eudia-status-dot ${e.salesforce}`});if(l.title=e.salesforceMessage||"Salesforce",t.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),e.calendar==="not_authorized"){let d=t.createDiv({cls:"eudia-status-warning"});d.innerHTML=`<strong>${i}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(t){let e=t.createDiv({cls:"eudia-calendar-content"}),n=e.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let i=new F(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),r=this._forceRefresh||!1;this._forceRefresh=!1;let s=await i.getWeekMeetings(r);if(!s.success){n.remove(),this.renderError(e,s.error||"Failed to load calendar");return}let o=new Date,l=new Date(o);l.setDate(l.getDate()-this.weeksBack*7);let c=new Date(o);c.setDate(c.getDate()-1);let d=[];try{d=await i.getMeetingsInRange(l,c)}catch{console.log("[Calendar] Could not fetch past meetings")}n.remove();let u={};for(let v of d){let C=v.start.split("T")[0];u[C]||(u[C]=[]),u[C].push(v)}for(let[v,C]of Object.entries(s.byDay||{})){u[v]||(u[v]=[]);let f=new Set(u[v].map(h=>h.id));for(let h of C)f.has(h.id)||u[v].push(h)}let y={};for(let[v,C]of Object.entries(u)){let f=this.showExternalOnly?C.filter(h=>this.isExternalMeeting(h)):C;f.length>0&&(y[v]=f)}let m=Object.keys(y).sort();if(m.length===0){this.renderEmptyState(e);return}await this.renderCurrentMeeting(e,i);let g=e.createEl("button",{text:"\u2190 Load earlier meetings",cls:"eudia-load-earlier"});g.style.cssText="width:100%;padding:8px;margin-bottom:8px;font-size:12px;cursor:pointer;border-radius:6px;border:1px solid var(--background-modifier-border);background:var(--background-secondary);color:var(--text-muted);",g.onclick=async()=>{this.weeksBack+=2,await this.render()};let O=o.toISOString().split("T")[0],b=null;for(let v of m){let C=y[v];if(!C||C.length===0)continue;let f=this.renderDaySection(e,v,C);v===O&&(b=f)}b&&setTimeout(()=>b.scrollIntoView({block:"start",behavior:"auto"}),100)}catch(i){n.remove(),this.renderError(e,i.message||"Failed to load calendar")}}async renderCurrentMeeting(t,e){try{let n=await e.getCurrentMeeting();if(n.meeting){let i=t.createDiv({cls:"eudia-now-card"});n.isNow?i.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):i.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let r=i.createDiv({cls:"eudia-now-content"});r.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&r.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let s=i.createEl("button",{cls:"eudia-now-action",text:"Create Note"});s.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(t,e,n){let i=t.createDiv({cls:"eudia-calendar-day"}),r=new Date().toISOString().split("T")[0],s=e===r,o=e<r,l=s?"TODAY":F.getDayName(e),c=i.createEl("div",{cls:`eudia-calendar-day-header ${s?"today":""} ${o?"past":""}`,text:l});s?c.style.cssText="font-weight:700;color:var(--interactive-accent);":o&&(c.style.cssText="opacity:0.7;");for(let d of n){let u=i.createDiv({cls:`eudia-calendar-meeting ${d.isCustomerMeeting?"customer":"internal"} ${o?"past":""}`});o&&(u.style.cssText="opacity:0.85;"),u.createEl("div",{cls:"eudia-calendar-time",text:F.formatTime(d.start,this.plugin.settings.timezone)});let y=u.createDiv({cls:"eudia-calendar-details"});if(y.createEl("div",{cls:"eudia-calendar-subject",text:d.subject}),d.accountName)y.createEl("div",{cls:"eudia-calendar-account",text:d.accountName});else if(d.attendees&&d.attendees.length>0){let m=d.attendees.filter(g=>g.isExternal!==!1).slice(0,2).map(g=>g.name||g.email?.split("@")[0]||"Unknown").join(", ");m&&y.createEl("div",{cls:"eudia-calendar-attendees",text:m})}u.onclick=()=>this.createNoteForMeeting(d),u.title="Click to create meeting note"}return i}renderEmptyState(t){let e=t.createDiv({cls:"eudia-calendar-empty"});e.innerHTML=`
    ...
    `}renderError(t,e){let n=t.createDiv({cls:"eudia-calendar-error"}),i="",r="Unable to load calendar",s="";e.includes("not authorized")||e.includes("403")?(i="\u{1F512}",r="Calendar Access Required",s="Contact your admin to be added to the authorized users list."):e.includes("network")||e.includes("fetch")?(i="\u{1F4E1}",r="Connection Issue",s="Check your internet connection and try again."):(e.includes("server")||e.includes("500"))&&(i="\u{1F527}",r="Server Unavailable",s="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=e.createDiv({cls:"eudia-setup-input-group"}),i=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});i.addClass("eudia-setup-email");let r=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),s=e.createDiv({cls:"eudia-setup-status"});r.onclick=async()=>{let o=i.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){s.textContent="Please use your @eudia.com email",s.className="eudia-setup-status error";return}r.disabled=!0,r.textContent="Connecting...",s.textContent="";try{if(!(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){s.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,s.className="eudia-setup-status error",r.disabled=!1,r.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),s.textContent="Connected",s.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(l){let c=l.message||"Connection failed";c.includes("403")?s.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:s.textContent=c,s.className="eudia-setup-status error",r.disabled=!1,r.textContent="Connect"}},i.onkeydown=o=>{o.key==="Enter"&&r.click()},e.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(t){let e=t.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],i=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],r=e.filter(o=>!i.includes(o)&&o.length>1);if(r.length===0)return e[0]||"";if(r.length>1&&n.includes(r[0]))return r[1].charAt(0).toUpperCase()+r[1].slice(1);let s=r[r.length-1];return s.charAt(0).toUpperCase()+s.slice(1)}getExternalDomainsFromAttendees(t){if(!t||t.length===0)return[];let e=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,i=[];for(let r of t){if(!r.email)continue;let o=r.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let l=o[1];if(l.includes("eudia.com")||e.includes(l)||n.has(l))continue;n.add(l);let c=this.extractCompanyFromDomain(l);c.length>=2&&i.push({domain:l,company:c})}}return i}findBestAccountMatch(t,e,n){let i=this.plugin.settings.accountsFolder||"Accounts",r=this.app.vault.getAbstractFileByPath(i);if(!(r instanceof p.TFolder))return null;let s=[];for(let l of r.children)l instanceof p.TFolder&&s.push(l.name);if(s.length===0)return null;let o=[];for(let{domain:l,company:c}of t){let d=this.findAccountFolder(c),u=d?1:0;o.push({domain:l,company:c,folder:d,score:u})}if(o.sort((l,c)=>c.score-l.score),o.length>0&&o[0].folder){let l=o[0],c=l.folder.split("/").pop()||l.company;return console.log(`[Eudia Calendar] Best domain match: "${l.company}" from ${l.domain} -> ${l.folder}`),{folder:l.folder,accountName:c,source:"domain"}}if(e){let l=this.findAccountFolder(e);if(l){let c=l.split("/").pop()||e;return console.log(`[Eudia Calendar] Server account match: "${e}" -> ${l}`),{folder:l,accountName:c,source:"server"}}}if(n){let l=this.findAccountFolder(n);if(l){let c=l.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${l}`),{folder:l,accountName:c,source:"subject"}}}for(let{company:l}of t){let c=s.find(d=>{let u=d.toLowerCase(),y=l.toLowerCase();return u.includes(y)||y.includes(u)});if(c){let d=`${i}/${c}`;return console.log(`[Eudia Calendar] Partial domain match: "${l}" -> ${d}`),{folder:d,accountName:c,source:"domain-partial"}}}return null}extractAccountFromAttendees(t){let e=this.getExternalDomainsFromAttendees(t);if(e.length===0)return null;let n=e[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(t){if(!t)return null;let e=t.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(e){let i=(e[1]||e[2]||"").trim();if(i.toLowerCase()!=="eudia")return i}let n=t.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let r=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(r.toLowerCase()!=="eudia"&&r.length>0)return r}if(!t.toLowerCase().includes("eudia")){let i=t.match(/^([^-–|]+)/);if(i){let r=i[1].trim();if(r.length>2&&r.length<50)return r}}return null}findAccountFolder(t){if(!t)return null;let e=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(e);if(!(n instanceof p.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${e}" not found`),null;let i=t.toLowerCase().trim(),r=[];for(let u of n.children)u instanceof p.TFolder&&r.push(u.name);console.log(`[Eudia Calendar] Searching for "${i}" in ${r.length} folders`);let s=r.find(u=>u.toLowerCase()===i);if(s)return console.log(`[Eudia Calendar] Exact match found: ${s}`),`${e}/${s}`;let o=r.find(u=>u.toLowerCase().startsWith(i));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${e}/${o}`;let l=r.find(u=>i.startsWith(u.toLowerCase()));if(l)return console.log(`[Eudia Calendar] Search starts with folder match: ${l}`),`${e}/${l}`;let c=r.find(u=>{let y=u.toLowerCase();return y.length<3||!i.includes(y)?!1:new RegExp(`\\b${y.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`).test(i)});if(c)return console.log(`[Eudia Calendar] Search contains folder match: ${c}`),`${e}/${c}`;let d=r.find(u=>{let y=u.toLowerCase();return i.length<3||!y.includes(i)?!1:new RegExp(`\\b${i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`).test(y)});return d?(console.log(`[Eudia Calendar] Folder contains search match: ${d}`),`${e}/${d}`):(console.log(`[Eudia Calendar] No folder match found for "${i}"`),null)}async createNoteForMeeting(t){let e=t.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",i=M(n),r=(t.attendees||[]).map(f=>f.email).filter(Boolean),s=de(t.subject,r);if(i&&s.isPipelineMeeting&&s.confidence>=60){await this._createPipelineNote(t,e);return}let o=t.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),l=`${e} - ${o}.md`,c=null,d=t.accountName||null,u=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${t.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(t.attendees?.map(f=>f.email)||[])}`);let y=this.getExternalDomainsFromAttendees(t.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(y)}`);let m=this.extractAccountFromSubject(t.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${m||"none"}"`);let g=this.findBestAccountMatch(y,t.accountName,m||void 0);if(g&&(c=g.folder,d=g.accountName,console.log(`[Eudia Calendar] Best match (${g.source}): "${d}" -> ${c}`)),!c){let f=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(f)instanceof p.TFolder&&(c=f,console.log(`[Eudia Calendar] No match found, using Accounts root: ${c}`))}if(d){let f=this.plugin.settings.cachedAccounts.find(h=>h.name.toLowerCase()===d?.toLowerCase());f&&(u=f.id,d=f.name,console.log(`[Eudia Calendar] Matched to cached account: ${f.name} (${f.id})`))}let O=c?`${c}/${l}`:l,b=this.app.vault.getAbstractFileByPath(O);if(b instanceof p.TFile){await this.app.workspace.getLeaf().openFile(b);try{let f=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;f?.revealInFolder&&f.revealInFolder(b)}catch{}new p.Notice(`Opened existing note: ${l}`);return}let v=(t.attendees||[]).map(f=>f.name||f.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${t.subject}"
date: ${e}
attendees: [${v}]
account: "${d||""}"
...
---`,r=[];return e?.opportunities&&r.push(e.opportunities),e?.recentActivity&&r.push(e.recentActivity),r.length>0?`${i}
...
3. **Next Steps** are auto-extracted after transcription
4. Set \`sync_to_salesforce: true\` to sync to Salesforce
`}buildNextStepsContent(t,e,n){let i=n||new Date().toISOString().split("T")[0],r=e?`
...
*Previous next steps will be archived here.*
`}];for(let b of O){let v=`${l}/${b.name}`;await this.app.vault.create(v,b.content)}r++}catch(g){console.log(`[Eudia] Failed to create prospect folder for ${s.name}:`,g)}}return r>0&&console.log(`[Eudia] Created ${r} prospect account folders in _Prospects/`),r}async createCSManagerDashboard(t,e){let n="CS Manager",i=new Date().toISOString().split("T")[0],r=ye(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s={};for(let u of e){let y=u.ownerName||"Unassigned";s[y]||(s[y]=[]),s[y].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
`,l=Object.keys(s).sort();for(let u of l){let y=s[u];o+=`### ${u} (${y.length} accounts)
...
`;let C=`${n}/${y}.md`,f=this.app.vault.getAbstractFileByPath(C);f instanceof p.TFile?await this.app.vault.modify(f,v):await this.app.vault.create(C,v)}console.log(`[Eudia] Created CS Manager dashboard for ${t} with ${e.length} accounts across ${l.length} reps`)}async createAdminAccountFolders(t){let e=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),await this.ensurePipelineFolderExists();let i=0,r=0,s=new Date().toISOString().split("T")[0],o=async c=>{let d=c.name.replace(/[<>:"/\\|?*]/g,"_").trim(),u=`${e}/${d}`;if(this.app.vault.getAbstractFileByPath(u)instanceof p.TFolder)return!1;try{return await this.app.vault.createFolder(u),await this.createExecAccountSubnotes(u,c,s),c.isOwned?i++:r++,console.log(`[Eudia Admin] Created ${c.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,m),!1}},l=5;for(let c=0;c<t.length;c+=l){let d=t.slice(c,c+l);await Promise.allSettled(d.map(u=>o(u)))}this.settings.cachedAccounts=t.map(c=>({id:c.id,name:c.name})),await this.saveSettings(),i+r>0&&new p.Notice(`Created ${i} owned + ${r} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(t,e,n){let i=e.ownerName||"Unknown",r=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let s of r){let o=`${t}/${s.name}`;await this.app.vault.create(o,s.content)}}async createFullAccountSubnotes(t,e,n){let i=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let r of i){let s=`${t}/${r.name}`;await this.app.vault.create(s,r.content)}}async ensurePipelineFolderExists(){let t="Pipeline",e=`${t}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),!this.app.vault.getAbstractFileByPath(e)){let s=`---
...
`;await this.app.vault.create(e,o)}}async updateAccountNextSteps(t,e,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${t}`),console.log(`[Eudia] Content length: ${e?.length||0} chars`);let i=t.replace(/[<>:"/\\|?*]/g,"_").trim(),r=`${this.settings.accountsFolder}/${i}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${r}`);let s=this.app.vault.getAbstractFileByPath(r);if(!s||!(s instanceof p.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${r}`);let v=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${i}`);v&&v instanceof p.TFolder?console.log(`[Eudia] Files in ${i} folder:`,v.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${i}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],l=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),c=n.split("/").pop()?.replace(".md","")||"Meeting",d=e;!e.includes("- [ ]")&&!e.includes("- [x]")&&(d=e.split(`
`).filter(v=>v.trim()).map(v=>{let C=v.replace(/^[-•*]\s*/,"").trim();return C?`- [ ] ${C}`:""}).filter(Boolean).join(`
`));let u=await this.app.vault.read(s),y="",m=u.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);m&&m[1]&&(y=m[1].trim());let g=`### ${o} - ${c}
${d||"*None*"}`,O=y?`${g}
...
`).filter(O=>O.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||m!=="Unknown")&&i.push({account:l.name,lastUpdated:m,nextSteps:g})}}i.sort((l,c)=>c.lastUpdated.localeCompare(l.lastUpdated));let r=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${r}
---
...
`)}catch{}throw n}}buildPipelineNoteContent(t,e,n){let i=new Date,r=String(i.getMonth()+1).padStart(2,"0"),s=String(i.getDate()).padStart(2,"0"),o=String(i.getFullYear()).slice(-2),l=i.toISOString().split("T")[0],c=`${r}.${s}.${o}`,d=g=>g==null?"":Array.isArray(g)?g.map(String).join(`
...
`;try{await this.app.vault.create(l,c)}catch{}e++,console.log(`[Eudia] Archived: ${r.name}`)}catch(s){console.error(`[Eudia] Failed to archive ${r.name}:`,s)}return e}async syncSpecificNoteToSalesforce(t){let e=await this.app.vault.read(t),n=this.app.metadataCache.getFileCache(t)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let i=n.account_id,r=n.account;if(!i&&r){let s=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===r.toLowerCase());s&&(i=s.id)}if(!i){let s=t.path.split("/");if(s.length>=2&&s[0]===this.settings.accountsFolder){let o=s[1]==="_Prospects"&&s.length>=3?s[2]:s[1],l=this.settings.cachedAccounts.find(c=>c.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);l&&(i=l.id,r=l.name)}}if(!i)return{success:!1,error:`Could not determine account for ${t.path}`};try{let s=await(0,p.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:i,accountName:r,noteTitle:t.basename,notePath:t.path,content:e,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return s.json?.success?{success:!0}:{success:!1,error:s.json?.error||"Unknown error",authRequired:s.json?.authRequired}}catch(s){return{success:!1,error:s.message}}}async copyForSlack(){let t=this.app.workspace.getActiveFile();if(!t){new p.Notice("No note open to copy");return}try{let e=await this.app.vault.read(t),i=e.match(/^account:\s*"?([^"\n]+)"?/m)?.[1]||t.parent?.name||"Meeting",s=e.match(/^last_updated:\s*(\S+)/m)?.[1]||new Date().toISOString().split("T")[0],o="",l=e.match(/## Summary\n([\s\S]*?)(?=\n## |\n---|\Z)/);l&&(o=l[1].trim().split(`
...
\u2022 `));let u="",y=e.match(/"([^"]{20,120})"/);y&&(u=y[1]);let m=`*${i} \u2014 ${s}*
`;o&&(m+=`${o}
`),c&&(m+=`
*Next Steps:*
\u2022 ${c}
...
`;await this.app.vault.modify(t,s)}async updateTeamPerformanceNote(t,e){if(!e)return;let n=new Date().toISOString().split("T")[0],i=s=>s>0?`\u2191 ${Math.abs(s).toFixed(1)}%`:s<0?`\u2193 ${Math.abs(s).toFixed(1)}%`:"--",r=`---
type: analytics_dashboard
auto_refresh: true
