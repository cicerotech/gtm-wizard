...
`}function We(O,a){let t="";return a?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${a.account.name}
${a.account.owner?`- Owner: ${a.account.owner}`:""}
...
`}function je(O){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${O?`
...
`},{name:"Meeting Notes.md",content:f},{name:"Contacts.md",content:v},{name:"Intelligence.md",content:y},{name:"Next Steps.md",content:S}];for(let C of b){let x=`${u}/${C.name}`;await this.app.vault.create(x,C.content)}return console.log(`[Eudia] Created account folder with subnotes${g?" (enriched)":""}: ${d}`),!0}catch(h){return console.error(`[Eudia] Failed to create folder for ${d}:`,h),!1}},c=5;for(let l=0;l<t.length;l+=c){let d=t.slice(l,l+c),u=await Promise.allSettled(d.map(m=>o(m)));r+=u.filter(m=>m.status==="fulfilled"&&m.value===!0).length}r>0?(this.settings.cachedAccounts=t.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),new p.Notice(`Created ${r} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let s=e?`
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let u of r){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,w=>w.toUpperCase()),h=u.split("@")[0].replace("."," ").toLowerCase(),g=h.split(" ")[0],v=h.split(" ").pop()||"",y=e.filter(w=>{let C=(w.csmName||"").toLowerCase();if(C&&(C.includes(g)||C.includes(v)))return!0;let x=(w.ownerName||"").toLowerCase();return x.includes(g)||x.includes(v)}),f=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
account_count: ${y.length}
created: ${s}
---
...
**Email:** ${u}
**CS Accounts:** ${y.length}

---

## Assigned Accounts

`;if(y.length>0){f+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let w of y){let C=w.name.replace(/[<>:"/\\|?*]/g,"_").trim();f+=`| ${w.name} | ${w.type||""} | ${w.ownerName||""} | [[Accounts/${C}/Contacts\\|View]] |
...
${m}`:g,y=`---
account: "${t}"
account_id: "${this.settings.cachedAccounts.find(f=>f.name===t)?.id||""}"
type: next_steps
auto_updated: true
last_updated: ${o}
sync_to_salesforce: false
...
---
`;await this.app.vault.modify(e,c+l),this.processTranscriptionAsync(t,e).catch(d=>{console.error("Background transcription failed:",d),new p.Notice(`Transcription failed: ${d.message}`)})}async processTranscriptionAsync(t,e){try{let n={},s=e.path.split("/");console.log(`[Eudia] Processing transcription for: ${e.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(s)}, accountsFolder: ${this.settings.accountsFolder}`);let r=s[0]==="Pipeline Meetings",i=!1;try{let f=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);f&&(i=/meeting_type:\s*pipeline_review/.test(f[1]))}catch{}if(!i&&r&&(i=!0),i){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let y="";try{let f=await(0,p.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});f.json?.success&&f.json?.context&&(y=f.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${y.length} chars)`))}catch(f){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",f)}n={meetingType:"pipeline_review",pipelineContext:y}}else if(s.length>=2&&s[0]===this.settings.accountsFolder){let y=s[1];console.log(`[Eudia] Detected account folder: ${y}`);let f=this.settings.cachedAccounts.find(S=>S.name.toLowerCase()===y.toLowerCase());f?(n={accountName:f.name,accountId:f.id,userEmail:this.settings.userEmail},console.log(`[Eudia] Found cached account: ${f.name} (${f.id})`)):(n={accountName:y,accountId:"",userEmail:this.settings.userEmail},console.log(`[Eudia] Account not in cache, using folder name: ${y}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let y=await this.calendarService.getCurrentMeeting();y.meeting?.attendees&&(o=y.meeting.attendees.map(f=>f.name||f.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let c=await this.transcriptionService.transcribeAudio(t.audioBlob,{...n,speakerHints:o,captureMode:t.captureMode,hasVirtualDevice:t.hasVirtualDevice,meetingTemplate:this.settings.meetingTemplate||"meddic"}),l=y=>y?!!(y.summary?.trim()||y.nextSteps?.trim()):!1,d=c.sections;if(l(d)||c.text?.trim()&&(d=await this.transcriptionService.processTranscription(c.text,n)),!l(d)&&!c.text?.trim()){let f=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(e,f+`

**Transcription failed:** No audio detected.
`),new p.Notice("Transcription failed: No audio detected.");return}let u=await this.app.vault.read(e),m="",h=Math.max(u.indexOf(`---
**Processing your recording`),u.indexOf(`---
**Transcription in progress`));if(h>0){let y=u.indexOf("---"),f=y>=0?u.indexOf("---",y+3):-1;f>0&&f+3<h&&(m=u.substring(f+3,h).trim())}else{let y=u.indexOf("---"),f=y>=0?u.indexOf("---",y+3):-1;if(f>0){let S=u.substring(f+3).trim();S.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(m=S)}}try{let y="_backups";this.app.vault.getAbstractFileByPath(y)||await this.app.vault.createFolder(y);let f=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),S=`${y}/${e.name}_${f}.md`;await this.app.vault.create(S,u),console.log(`[Eudia] Backed up note to ${S}`)}catch(y){console.warn("[Eudia] Backup failed (non-critical):",y.message)}let g;if(i?g=this.buildPipelineNoteContent(d,c,e.path):g=this.buildNoteContent(d,c),m&&m.length>5){let y=g.indexOf("---",g.indexOf("---")+3);if(y>0){let f=g.substring(0,y+3),S=g.substring(y+3);g=f+`
...
---
`+S}}await this.app.vault.modify(e,g);let v=Math.floor(t.duration/60);if(new p.Notice(`Transcription complete (${v} min recording)`),!i){let y=d.nextSteps||d.actionItems;console.log(`[Eudia] Next Steps extraction - accountContext: ${n?.accountName||"undefined"}`),console.log(`[Eudia] Next Steps content found: ${y?"YES ("+y.length+" chars)":"NO"}`),console.log(`[Eudia] sections.nextSteps: ${d.nextSteps?"YES":"NO"}, sections.actionItems: ${d.actionItems?"YES":"NO"}`),y&&n?.accountName?(console.log(`[Eudia] Calling updateAccountNextSteps for ${n.accountName}`),await this.updateAccountNextSteps(n.accountName,y,e.path)):console.log("[Eudia] Skipping Next Steps update - missing content or account context")}this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(n){try{let r=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(e,r+`

**Transcription failed:** ${n.message}
`)}catch{}throw n}}buildPipelineNoteContent(t,e,n){let s=new Date,r=String(s.getMonth()+1).padStart(2,"0"),i=String(s.getDate()).padStart(2,"0"),o=String(s.getFullYear()).slice(-2),c=s.toISOString().split("T")[0],l=`${r}.${i}.${o}`,d=g=>g==null?"":Array.isArray(g)?g.map(String).join(`
