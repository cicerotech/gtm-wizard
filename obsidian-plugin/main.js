...
`;await this.app.vault.create(t,o)}}async updateAccountNextSteps(e,t,i){try{let a=e.replace(/[<>:"/\\|?*]/g,"_").trim(),s=`${this.settings.accountsFolder}/${a}/Next Steps.md`,r=this.app.vault.getAbstractFileByPath(s);if(!r||!(r instanceof l.TFile)){console.log(`[Eudia] Next Steps file not found for ${e}`);return}let o=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),p=i.split("/").pop()?.replace(".md","")||"Meeting",d=t;!t.includes("- [ ]")&&!t.includes("- [x]")&&(d=t.split(`
`).filter(u=>u.trim()).map(u=>{let g=u.replace(/^[-•*]\s*/,"").trim();return g?`- [ ] ${g}`:""}).filter(Boolean).join(`
`));let m=`---
account: "${e}"
type: next_steps
auto_updated: true
...
`):typeof y=="object"?JSON.stringify(y):String(y),a=i(e.title)||"Meeting Notes",s=i(e.summary),r=i(e.painPoints),o=i(e.productInterest),c=i(e.meddiccSignals),p=i(e.nextSteps),d=i(e.actionItems),m=i(e.keyDates),u=i(e.risksObjections),g=i(e.attendees),f=`---
title: "${a}"
...
`),m&&!m.includes("No specific dates")&&(f+=`## Key Dates

${m}

`),u&&!u.includes("None raised")&&(f+=`## Risks and Objections
