...
- Action items have clear owners`}function Oe(y){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${y?`
...
`}if(u.objections&&u.objections.length>0){let h=u.objections.filter(g=>g.handled).length;n+=`**Objections:** ${u.objections.length} raised, ${h} handled
...
`):typeof p=="object"?JSON.stringify(p,null,2):String(p),h=u(e.summary),g=t.transcript||t.text||"",m=`---
...
`),m}buildNoteContent(e,t){let n=f=>f==null?"":Array.isArray(f)?f.map(S=>typeof S=="object"?S.category?`**${S.category}**: ${S.signal||S.insight||""}`:JSON.stringify(S):String(S)).join(`
`):typeof f=="object"?JSON.stringify(f):String(f),s=n(e.title)||"Meeting Notes",i=n(e.summary),r=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),u=n(e.actionItems),h=n(e.keyDates),g=n(e.risksObjections),m=n(e.attendees),p=`---
title: "${s}"
