...
*Previous next steps will be archived here.*
`}];for(let S of f){let v=`${c}/${S.name}`;await this.app.vault.create(v,S.content)}r++}catch(p){console.log(`[Eudia] Failed to create prospect folder for ${i.name}:`,p)}}return r>0&&console.log(`[Eudia] Created ${r} prospect account folders in _Prospects/`),r}async createCSManagerDashboard(e,t){let n="CS Manager",s=new Date().toISOString().split("T")[0],r=ue(e);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let i={};for(let h of t){let g=h.ownerName||"Unassigned";i[g]||(i[g]=[]),i[g].push(h)}let o=`---
role: cs_manager
manager: "${e}"
...
`,c=Object.keys(i).sort();for(let h of c){let g=i[h];o+=`### ${h} (${g.length} accounts)
...
`;let f=`${n}/${g}.md`,S=this.app.vault.getAbstractFileByPath(f);S instanceof u.TFile?await this.app.vault.modify(S,p):await this.app.vault.create(f,p)}console.log(`[Eudia] Created CS Manager dashboard for ${e} with ${t.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let s=0,r=0,i=new Date().toISOString().split("T")[0],o=async d=>{let l=d.name.replace(/[<>:"/\\|?*]/g,"_").trim(),h=`${t}/${l}`;if(this.app.vault.getAbstractFileByPath(h)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(h),await this.createExecAccountSubnotes(h,d,i),d.isOwned?s++:r++,console.log(`[Eudia Admin] Created ${d.isOwned?"owned":"view-only"} folder: ${l}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${l}:`,m),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c);await Promise.allSettled(l.map(h=>o(h)))}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),s+r>0&&new u.Notice(`Created ${s} owned + ${r} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let s=t.ownerName||"Unknown",r=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
`):typeof f=="object"?JSON.stringify(f):String(f),s=n(e.title)||"Meeting Notes",r=n(e.summary),i=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),l=n(e.actionItems),h=n(e.keyDates),g=n(e.risksObjections),m=n(e.attendees),p=`---
title: "${s}"
...
`+i;await this.app.vault.modify(e,o)}}}}catch(t){console.error(`[Eudia SF Sync] Failed to update sync timestamp for ${e.path}:`,t)}}updateSfSyncStatusBar(e){this.sfSyncStatusBarEl&&this.sfSyncStatusBarEl.setText(e)}async createMeetingNote(){return new Promise(e=>{new ee(this.app,this,async n=>{if(!n){e();return}let s=new Date().toISOString().split("T")[0],r=n.name.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${this.settings.accountsFolder}/${r}`,o=`${s} Meeting.md`,c=`${i}/${o}`;this.app.vault.getAbstractFileByPath(i)||await this.app.vault.createFolder(i);let d=`---
