...
`;if(O.length>0){b+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let y of O){let w=y.name.replace(/[<>:"/\\|?*]/g,"_").trim();b+=`| ${y.name} | ${y.type||""} | ${y.ownerName||""} | [[Accounts/${w}/Contacts\\|View]] |
`}}else b+=`*No accounts currently matched to this rep. Accounts will populate after connecting to Salesforce (Step 2).*
`;b+=`
---
...
`;await this.app.vault.create(e,o)}}async updateAccountNextSteps(t,e,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${t}`),console.log(`[Eudia] Content length: ${e?.length||0} chars`);let i=t.replace(/[<>:"/\\|?*]/g,"_").trim(),a=`${this.settings.accountsFolder}/${i}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${a}`);let s=this.app.vault.getAbstractFileByPath(a);if(!s||!(s instanceof p.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${a}`);let b=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${i}`);b&&b instanceof p.TFolder?console.log(`[Eudia] Files in ${i} folder:`,b.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${i}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],l=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),c=n.split("/").pop()?.replace(".md","")||"Meeting",d=e;!e.includes("- [ ]")&&!e.includes("- [x]")&&(d=e.split(`
`).filter(b=>b.trim()).map(b=>{let C=b.replace(/^[-•*]\s*/,"").trim();return C?`- [ ] ${C}`:""}).filter(Boolean).join(`
...
${h}`:g,O=`---
account: "${t}"
account_id: "${this.settings.cachedAccounts.find(b=>b.name===t)?.id||""}"
type: next_steps
auto_updated: true
last_updated: ${o}
sync_to_salesforce: false
...
`).filter(v=>v.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||m!=="Unknown")&&i.push({account:l.name,lastUpdated:m,nextSteps:g})}}i.sort((l,c)=>c.lastUpdated.localeCompare(l.lastUpdated));let a=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${a}
---
...
`),new p.Notice(`Transcription failed: ${E}`,1e4);return}let g=await this.app.vault.read(e),v="",O=Math.max(g.indexOf(`---
**Processing your recording`),g.indexOf(`---
**Transcription in progress`));if(O>0){let f=g.indexOf("---"),y=f>=0?g.indexOf("---",f+3):-1;y>0&&y+3<O&&(v=g.substring(y+3,O).trim())}else{let f=g.indexOf("---"),y=f>=0?g.indexOf("---",f+3):-1;if(y>0){let w=g.substring(y+3).trim();w.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(v=w)}}try{let f="_backups";this.app.vault.getAbstractFileByPath(f)||await this.app.vault.createFolder(f);let y=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),w=`${f}/${e.name}_${y}.md`;await this.app.vault.create(w,g),console.log(`[Eudia] Backed up note to ${w}`)}catch(f){console.warn("[Eudia] Backup failed (non-critical):",f.message)}let b;if(s?b=this.buildPipelineNoteContent(m,c,e.path):b=this.buildNoteContent(m,c),v&&v.length>5){let f=b.indexOf("---",b.indexOf("---")+3);if(f>0){let y=b.substring(0,f+3),w=b.substring(f+3);b=y+`
