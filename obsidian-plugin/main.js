var ye=Object.create;var G=Object.defineProperty;var ge=Object.getOwnPropertyDescriptor;var fe=Object.getOwnPropertyNames;var we=Object.getPrototypeOf,Oe=Object.prototype.hasOwnProperty;var ve=(f,r)=>{for(var e in r)G(f,e,{get:r[e],enumerable:!0})},se=(f,r,e,t)=>{if(r&&typeof r=="object"||typeof r=="function")for(let n of fe(r))!Oe.call(f,n)&&n!==e&&G(f,n,{get:()=>r[n],enumerable:!(t=ge(r,n))||t.enumerable});return f};var U=(f,r,e)=>(e=f!=null?ye(we(f)):{},se(r||!f||!f.__esModule?G(e,"default",{value:f,enumerable:!0}):e,f)),Se=f=>se(G({},"__esModule",{value:!0}),f);var Le={};ve(Le,{default:()=>q});module.exports=Se(Le);var u=require("obsidian");var T=class f{constructor(){this.mediaRecorder=null;this.audioChunks=[];this.stream=null;this.startTime=0;this.pausedDuration=0;this.pauseStartTime=0;this.durationInterval=null;this.audioContext=null;this.analyser=null;this.levelInterval=null;this.lastExtractedChunkIndex=0;this.mimeTypeCache="audio/webm";this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0};this.stateCallback=null;this.levelHistory=[];this.trackingLevels=!1}onStateChange(r){this.stateCallback=r}static isIOSOrSafari(){let r=navigator.userAgent,e=/iPad|iPhone|iPod/.test(r)&&!window.MSStream,t=/^((?!chrome|android).)*safari/i.test(r);return e||t}getSupportedMimeType(){let r=f.isIOSOrSafari(),e=r?["audio/mp4","audio/mp4;codecs=aac","audio/aac","audio/webm;codecs=opus","audio/webm"]:["audio/webm;codecs=opus","audio/webm","audio/mp4","audio/ogg;codecs=opus","audio/ogg"];for(let t of e)if(MediaRecorder.isTypeSupported(t))return console.log(`[AudioRecorder] Using MIME type: ${t} (iOS/Safari: ${r})`),t;return r?"audio/mp4":"audio/webm"}async startRecording(){if(this.state.isRecording)throw new Error("Already recording");try{let r=f.isIOSOrSafari(),e=r?{echoCancellation:!0,noiseSuppression:!0}:{echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0,sampleRate:48e3,channelCount:1};this.stream=await navigator.mediaDevices.getUserMedia({audio:e}),console.log(`[AudioRecorder] Microphone access granted (iOS/Safari: ${r})`),this.setupAudioAnalysis();let t=this.getSupportedMimeType();this.mimeTypeCache=t,this.mediaRecorder=new MediaRecorder(this.stream,{mimeType:t,audioBitsPerSecond:96e3}),this.audioChunks=[],this.lastExtractedChunkIndex=0,this.mediaRecorder.ondataavailable=n=>{n.data.size>0&&this.audioChunks.push(n.data)},this.mediaRecorder.start(1e3),this.startTime=Date.now(),this.pausedDuration=0,this.state={isRecording:!0,isPaused:!1,duration:0,audioLevel:0},this.startDurationTracking(),this.startLevelTracking(),this.notifyStateChange()}catch(r){throw this.cleanup(),new Error(`Failed to start recording: ${r.message}`)}}setupAudioAnalysis(){if(this.stream)try{this.audioContext=new AudioContext;let r=this.audioContext.createMediaStreamSource(this.stream);this.analyser=this.audioContext.createAnalyser(),this.analyser.fftSize=256,r.connect(this.analyser)}catch(r){console.warn("Failed to set up audio analysis:",r)}}startDurationTracking(){this.durationInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused){let r=Date.now()-this.startTime-this.pausedDuration;this.state.duration=Math.floor(r/1e3),this.notifyStateChange()}},100)}startLevelTracking(){if(!this.analyser)return;let r=new Uint8Array(this.analyser.frequencyBinCount);this.levelInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused&&this.analyser){this.analyser.getByteFrequencyData(r);let e=0;for(let n=0;n<r.length;n++)e+=r[n];let t=e/r.length;this.state.audioLevel=Math.min(100,Math.round(t/255*100*2)),this.notifyStateChange()}},50)}pauseRecording(){!this.state.isRecording||this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="recording"&&(this.mediaRecorder.pause(),this.pauseStartTime=Date.now(),this.state.isPaused=!0,this.notifyStateChange())}resumeRecording(){!this.state.isRecording||!this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="paused"&&(this.mediaRecorder.resume(),this.pausedDuration+=Date.now()-this.pauseStartTime,this.state.isPaused=!1,this.notifyStateChange())}async stopRecording(){return new Promise((r,e)=>{if(!this.mediaRecorder||!this.state.isRecording){e(new Error("Not currently recording"));return}let t=this.mediaRecorder.mimeType,n=this.state.duration,s=!1,i=setTimeout(()=>{if(!s){s=!0,console.warn("AudioRecorder: onstop timeout, forcing completion");try{let a=new Blob(this.audioChunks,{type:t}),o=new Date,c=o.toISOString().split("T")[0],l=o.toTimeString().split(" ")[0].replace(/:/g,"-"),d=t.includes("webm")?"webm":t.includes("mp4")?"m4a":t.includes("ogg")?"ogg":"webm",p=`recording-${c}-${l}.${d}`;this.cleanup(),r({audioBlob:a,duration:n,mimeType:t,filename:p})}catch{this.cleanup(),e(new Error("Failed to process recording after timeout"))}}},1e4);this.mediaRecorder.onstop=()=>{if(!s){s=!0,clearTimeout(i);try{console.log(`[AudioRecorder] Chunks collected: ${this.audioChunks.length}`);let a=new Blob(this.audioChunks,{type:t});console.log(`[AudioRecorder] Blob size: ${a.size} bytes`);let o=new Date,c=o.toISOString().split("T")[0],l=o.toTimeString().split(" ")[0].replace(/:/g,"-"),d=t.includes("webm")?"webm":t.includes("mp4")?"m4a":t.includes("ogg")?"ogg":"webm",p=`recording-${c}-${l}.${d}`;this.cleanup(),r({audioBlob:a,duration:n,mimeType:t,filename:p})}catch(a){this.cleanup(),e(a)}}},this.mediaRecorder.onerror=a=>{s||(s=!0,clearTimeout(i),this.cleanup(),e(new Error("Recording error occurred")))},this.mediaRecorder.state==="recording"&&this.mediaRecorder.requestData(),setTimeout(()=>{this.mediaRecorder&&this.mediaRecorder.state!=="inactive"&&this.mediaRecorder.stop()},100)})}cancelRecording(){this.cleanup()}cleanup(){this.durationInterval&&(clearInterval(this.durationInterval),this.durationInterval=null),this.levelInterval&&(clearInterval(this.levelInterval),this.levelInterval=null),this.audioContext&&(this.audioContext.close().catch(()=>{}),this.audioContext=null,this.analyser=null),this.stream&&(this.stream.getTracks().forEach(r=>r.stop()),this.stream=null),this.mediaRecorder=null,this.audioChunks=[],this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0},this.notifyStateChange()}getState(){return{...this.state}}static isSupported(){if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||!window.MediaRecorder)return!1;let e=["audio/webm","audio/mp4","audio/ogg","audio/webm;codecs=opus"].some(t=>MediaRecorder.isTypeSupported(t));return e||console.warn("[AudioRecorder] No supported audio formats found"),e}static getMobileInstructions(){return this.isIOSOrSafari()?"For best results on iOS, ensure you have granted microphone permissions in Settings > Privacy > Microphone.":null}notifyStateChange(){this.stateCallback&&this.stateCallback({...this.state})}static formatDuration(r){let e=Math.floor(r/60),t=r%60;return`${e.toString().padStart(2,"0")}:${t.toString().padStart(2,"0")}`}static async blobToBase64(r){return new Promise((e,t)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];e(i)},n.onerror=t,n.readAsDataURL(r)})}static async blobToArrayBuffer(r){return r.arrayBuffer()}async start(){return this.startRecording()}async stop(){return this.stopRecording()}pause(){return this.pauseRecording()}resume(){return this.resumeRecording()}cancel(){return this.cancelRecording()}isRecording(){return this.state.isRecording}extractNewChunks(){if(!this.state.isRecording||this.audioChunks.length===0)return null;let r=this.audioChunks.slice(this.lastExtractedChunkIndex);return r.length===0?null:(this.lastExtractedChunkIndex=this.audioChunks.length,new Blob(r,{type:this.mimeTypeCache}))}getAllChunksAsBlob(){return this.audioChunks.length===0?null:new Blob(this.audioChunks,{type:this.mimeTypeCache})}getDuration(){return this.state.duration}getMimeType(){return this.mimeTypeCache}startLevelHistoryTracking(){this.levelHistory=[],this.trackingLevels=!0}recordLevelSample(){this.trackingLevels&&this.levelHistory.push(this.state.audioLevel)}getAudioDiagnostic(){if(this.levelHistory.length===0)return{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:100,warning:"Unable to analyze audio levels - recording may be too short"};let r=this.levelHistory.reduce((i,a)=>i+a,0)/this.levelHistory.length,e=Math.max(...this.levelHistory),t=this.levelHistory.filter(i=>i<5).length,n=Math.round(t/this.levelHistory.length*100),s=null;return e<5?s="SILENT AUDIO: No audio was detected during recording. Check your microphone settings and ensure Obsidian has microphone permission.":r<10&&n>80?s="VERY LOW AUDIO: Audio levels were extremely low. The transcription may not be accurate. Check your microphone or move closer to it.":n>90&&(s="MOSTLY SILENT: Over 90% of the recording had no audio. Make sure you're capturing the meeting audio, not just silence."),{hasAudio:e>=5,averageLevel:Math.round(r),peakLevel:e,silentPercent:n,warning:s}}static async analyzeAudioBlob(r){try{let e=new AudioContext,t=await r.arrayBuffer(),n;try{n=await e.decodeAudioData(t)}catch{return await e.close(),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:"Could not analyze audio format. Proceeding with transcription."}}let s=n.getChannelData(0),i=0,a=0,o=0,c=.01,l=100,d=0;for(let O=0;O<s.length;O+=l){let S=Math.abs(s[O]);i+=S,S>a&&(a=S),S<c&&o++,d++}await e.close();let p=i/d,y=Math.round(o/d*100),m=Math.round(p*100*10),h=Math.round(a*100),g=null;return a<.01?g='SILENT AUDIO DETECTED: The recording appears to contain only silence. This typically causes Whisper to hallucinate random text like "Yes. Yes. Yes." Check your audio input source.':p<.005&&y>95?g="NEAR-SILENT AUDIO: The recording is almost entirely silent. The transcription will likely be inaccurate.":y>90&&(g="MOSTLY SILENT: Over 90% of the recording is silent. Consider checking your audio setup."),{hasAudio:a>=.01,averageLevel:m,peakLevel:h,silentPercent:y,warning:g}}catch(e){return console.error("Audio analysis failed:",e),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:null}}}};var N=require("obsidian");var X=class{constructor(){this.salesforceAccounts=[]}setAccounts(r){this.salesforceAccounts=r}detectAccount(r,e,t){if(r){let n=this.detectFromTitle(r);if(n.confidence>=70)return n}if(t){let n=this.detectFromFilePath(t);if(n.confidence>=70)return n}if(e&&e.length>0){let n=this.detectFromAttendees(e);if(n.confidence>=50)return n}return{account:null,accountId:null,confidence:0,source:"none",evidence:"No account detected from available context"}}detectFromTitle(r){if(!r)return{account:null,accountId:null,confidence:0,source:"title",evidence:"No title"};let e=[{regex:/^([A-Za-z0-9][^-–—]+?)\s*[-–—]\s*(?:[A-Z][a-z]+|[A-Za-z]{2,})/,confidence:85},{regex:/(?:call|meeting|sync|check-in|demo|discovery)\s+(?:with|re:?|@)\s+([^-–—]+?)(?:\s*[-–—]|$)/i,confidence:80},{regex:/^([A-Za-z][^-–—]+?)\s+(?:discovery|demo|review|kickoff|intro|onboarding|sync)\s*(?:call)?$/i,confidence:75},{regex:/^([^:]+?):\s+/i,confidence:70},{regex:/^\[([^\]]+)\]/,confidence:75}],t=["weekly","daily","monthly","internal","team","1:1","one on one","standup","sync","meeting","call","notes","monday","tuesday","wednesday","thursday","friday","untitled","new","test"];for(let n of e){let s=r.match(n.regex);if(s&&s[1]){let i=s[1].trim();if(t.some(o=>i.toLowerCase()===o)||i.length<2)continue;let a=this.fuzzyMatchSalesforce(i);return a?{account:a.name,accountId:a.id,confidence:Math.min(n.confidence+10,100),source:"salesforce_match",evidence:`Matched "${i}" from title to Salesforce account "${a.name}"`}:{account:i,accountId:null,confidence:n.confidence,source:"title",evidence:"Extracted from meeting title pattern"}}}return{account:null,accountId:null,confidence:0,source:"title",evidence:"No pattern matched"}}detectFromFilePath(r){let e=r.match(/Accounts\/([^\/]+)\//i);if(e&&e[1]){let t=e[1].trim(),n=this.fuzzyMatchSalesforce(t);return n?{account:n.name,accountId:n.id,confidence:95,source:"salesforce_match",evidence:`File in account folder "${t}" matched to "${n.name}"`}:{account:t,accountId:null,confidence:85,source:"title",evidence:`File located in Accounts/${t} folder`}}return{account:null,accountId:null,confidence:0,source:"none",evidence:"Not in Accounts folder"}}detectFromAttendees(r){let e=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com"],t=new Set;for(let a of r){let c=a.toLowerCase().match(/@([a-z0-9.-]+)/);if(c){let l=c[1];!l.includes("eudia.com")&&!e.includes(l)&&t.add(l)}}if(t.size===0)return{account:null,accountId:null,confidence:0,source:"attendee_domain",evidence:"No external domains"};for(let a of t){let o=a.split(".")[0],c=o.charAt(0).toUpperCase()+o.slice(1),l=this.fuzzyMatchSalesforce(c);if(l)return{account:l.name,accountId:l.id,confidence:75,source:"salesforce_match",evidence:`Matched attendee domain ${a} to "${l.name}"`}}let n=Array.from(t)[0],s=n.split(".")[0];return{account:s.charAt(0).toUpperCase()+s.slice(1),accountId:null,confidence:50,source:"attendee_domain",evidence:`Guessed from external attendee domain: ${n}`}}fuzzyMatchSalesforce(r){if(!r||this.salesforceAccounts.length===0)return null;let e=r.toLowerCase().trim();for(let t of this.salesforceAccounts)if(t.name?.toLowerCase()===e)return t;for(let t of this.salesforceAccounts)if(t.name?.toLowerCase().startsWith(e))return t;for(let t of this.salesforceAccounts)if(t.name?.toLowerCase().includes(e))return t;for(let t of this.salesforceAccounts)if(e.includes(t.name?.toLowerCase()))return t;return null}suggestAccounts(r,e=10){if(!r||r.length<2)return this.salesforceAccounts.slice(0,e).map(s=>({...s,score:0}));let t=r.toLowerCase(),n=[];for(let s of this.salesforceAccounts){let i=s.name?.toLowerCase()||"",a=0;i===t?a=100:i.startsWith(t)?a=90:i.includes(t)?a=70:t.includes(i)&&(a=50),a>0&&n.push({...s,score:a})}return n.sort((s,i)=>i.score-s.score).slice(0,e)}},Be=new X,Ce=["pipeline review","pipeline call","weekly pipeline","forecast call","forecast review","deal review","opportunity review","sales review","pipeline sync","forecast sync","deal sync","pipeline update","forecast meeting"];function re(f,r){if(f){let e=f.toLowerCase();for(let t of Ce)if(e.includes(t))return{isPipelineMeeting:!0,confidence:95,evidence:`Title contains "${t}"`}}if(r&&r.length>=2){let e=["eudia.com","johnsonhana.com"];if(r.every(n=>{let s=n.toLowerCase().split("@")[1]||"";return e.some(i=>s.includes(i))})&&r.length>=3){if(f){let n=f.toLowerCase();if(["sync","review","update","weekly","team","forecast"].some(a=>n.includes(a)))return{isPipelineMeeting:!0,confidence:70,evidence:`All internal attendees (${r.length}) with team meeting signal`}}return{isPipelineMeeting:!1,confidence:40,evidence:"All internal attendees but no clear pipeline signal"}}}return{isPipelineMeeting:!1,confidence:0,evidence:"No pipeline meeting indicators found"}}function be(f,r){let e="";return(r?.account||r?.opportunities?.length)&&(e=`
ACCOUNT CONTEXT (use to inform your analysis):
${r.account?`- Account: ${r.account.name}`:""}
${r.account?.owner?`- Account Owner: ${r.account.owner}`:""}
${r.opportunities?.length?`- Open Opportunities: ${r.opportunities.map(t=>`${t.name} (${t.stage}, $${(t.acv/1e3).toFixed(0)}k)`).join("; ")}`:""}
${r.contacts?.length?`- Known Contacts: ${r.contacts.slice(0,5).map(t=>`${t.name} - ${t.title}`).join("; ")}`:""}
...
Format your response as a brief, actionable answer suitable for quick reference during a call.`}static formatSectionsForNote(r,e,t){let n="";if(r.summary&&(n+=`## TL;DR
...
`);let s=t?.enabled&&t?.formattedTranscript?t.formattedTranscript:e;if(s){let i=t?.enabled?"Full Transcript (Speaker-Attributed)":"Full Transcript";n+=`---
...
`}return n}static formatSectionsWithAudio(r,e,t,n){let s=this.formatSectionsForNote(r,e,n);return t&&(s+=`
---

## Recording

![[${t}]]
`),s}static formatContextForNote(r){if(!r.success)return"";let e=`## Pre-Call Context

`;if(r.account&&(e+=`**Account:** ${r.account.name}
`,e+=`**Owner:** ${r.account.owner}

`),r.opportunities&&r.opportunities.length>0){e+=`### Open Opportunities

`;for(let t of r.opportunities){let n=t.acv?`$${(t.acv/1e3).toFixed(0)}k`:"TBD";e+=`- **${t.name}** - ${t.stage} - ${n}`,t.targetSignDate&&(e+=` - Target: ${new Date(t.targetSignDate).toLocaleDateString()}`),e+=`
`}e+=`
`}if(r.contacts&&r.contacts.length>0){e+=`### Key Contacts

`;for(let t of r.contacts.slice(0,5))e+=`- **${t.name}**`,t.title&&(e+=` - ${t.title}`),e+=`
`;e+=`
`}if(r.lastMeeting&&(e+=`### Last Meeting

`,e+=`${new Date(r.lastMeeting.date).toLocaleDateString()} - ${r.lastMeeting.subject}

`),r.account?.customerBrain){let t=r.account.customerBrain.substring(0,500);t&&(e+=`### Recent Notes

`,e+=`${t}${r.account.customerBrain.length>500?"...":""}

`)}return e+=`---

`,e}async blobToBase64(r){return new Promise((e,t)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];e(i)},n.onerror=t,n.readAsDataURL(r)})}async transcribeAudio(r,e){try{let t=await this.blobToBase64(r),n=r.type||"audio/webm",s=e?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:e.pipelineContext}:void 0,i=await this.transcribeAndSummarize(t,n,e?.accountName,e?.accountId,s);return{text:i.transcript,confidence:i.success?.95:0,duration:i.duration,sections:i.sections}}catch(t){return console.error("transcribeAudio error:",t),{text:"",confidence:0,duration:0,sections:this.getEmptySections()}}}async processTranscription(r,e){if(!r||r.trim().length===0)return this.getEmptySections();try{let t=await(0,N.requestUrl)({url:`${this.serverUrl}/api/process-sections`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:r,accountName:e?.accountName,context:e})});if(t.json?.success&&t.json?.sections){let n=t.json.sections;return{summary:n.summary||"",painPoints:n.painPoints||n.keyPoints||"",productInterest:n.productInterest||"",meddiccSignals:n.meddiccSignals||"",nextSteps:n.nextSteps||"",actionItems:n.actionItems||"",keyDates:n.keyDates||"",buyingTriggers:n.buyingTriggers||"",dealSignals:n.dealSignals||"",risksObjections:n.risksObjections||n.concerns||"",competitiveIntel:n.competitiveIntel||"",attendees:n.attendees||"",transcript:r}}return console.warn("Server process-sections returned no sections, using fallback"),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:r}}catch(t){return console.error("processTranscription server error:",t),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:r}}}};var Y=require("obsidian"),P=class f{constructor(r,e,t="America/New_York"){this.serverUrl=r,this.userEmail=e.toLowerCase(),this.timezone=t}setUserEmail(r){this.userEmail=r.toLowerCase()}setServerUrl(r){this.serverUrl=r}setTimezone(r){this.timezone=r}async getTodaysMeetings(r=!1){if(!this.userEmail)return{success:!1,date:new Date().toISOString().split("T")[0],email:"",meetingCount:0,meetings:[],error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=r?"&forceRefresh=true":"";return(await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/today?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch today's meetings:",e),{success:!1,date:new Date().toISOString().split("T")[0],email:this.userEmail,meetingCount:0,meetings:[],error:e.message||"Failed to fetch calendar"}}}async getWeekMeetings(r=!1){if(!this.userEmail)return{success:!1,startDate:"",endDate:"",email:"",totalMeetings:0,byDay:{},error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=r?"&forceRefresh=true":"";return(await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/week?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch week's meetings:",e),{success:!1,startDate:"",endDate:"",email:this.userEmail,totalMeetings:0,byDay:{},error:e.message||"Failed to fetch calendar"}}}async getMeetingsInRange(r,e){if(!this.userEmail)return[];try{let t=r.toISOString().split("T")[0],n=e.toISOString().split("T")[0],s=await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/range?start=${t}&end=${n}`,method:"GET",headers:{Accept:"application/json"}});return s.json.success?s.json.meetings||[]:[]}catch(t){return console.error("Failed to fetch calendar range:",t),[]}}async getCurrentMeeting(){let r=await this.getTodaysMeetings();if(!r.success||r.meetings.length===0)return{meeting:null,isNow:!1};let e=new Date;for(let t of r.meetings){let n=f.safeParseDate(t.start),s=f.safeParseDate(t.end);if(e>=n&&e<=s)return{meeting:t,isNow:!0};let i=(n.getTime()-e.getTime())/(1e3*60);if(i>0&&i<=15)return{meeting:t,isNow:!1,minutesUntilStart:Math.ceil(i)}}return{meeting:null,isNow:!1}}async getMeetingsForAccount(r){let e=await this.getWeekMeetings();if(!e.success)return[];let t=[];Object.values(e.byDay).forEach(s=>{t.push(...s)});let n=r.toLowerCase();return t.filter(s=>s.accountName?.toLowerCase().includes(n)||s.subject.toLowerCase().includes(n)||s.attendees.some(i=>i.email.toLowerCase().includes(n.split(" ")[0])))}static formatMeetingForNote(r){let e=r.attendees.filter(t=>t.isExternal!==!1).map(t=>t.name||t.email.split("@")[0]).slice(0,5).join(", ");return{title:r.subject,attendees:e,meetingStart:r.start,accountName:r.accountName}}static getDayName(r){let e;r.length===10&&r.includes("-")?e=new Date(r+"T00:00:00"):e=new Date(r);let t=new Date;t.setHours(0,0,0,0);let n=new Date(e);n.setHours(0,0,0,0);let s=Math.round((n.getTime()-t.getTime())/(1e3*60*60*24));return s===0?"Today":s===1?"Tomorrow":e.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}static formatTime(r,e){let t=r;t&&!t.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(t)&&(t=t+"Z");let n=new Date(t);if(isNaN(n.getTime()))return r;let s={hour:"numeric",minute:"2-digit",hour12:!0};return e&&(s.timeZone=e),n.toLocaleTimeString("en-US",s)}static safeParseDate(r){if(!r)return new Date(NaN);let e=r;return!e.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(e)&&(e=e+"Z"),new Date(e)}static getMeetingDuration(r,e){let t=f.safeParseDate(r),n=f.safeParseDate(e);return Math.round((n.getTime()-t.getTime())/(1e3*60))}};var We=["ai-contracting-tech","ai-contracting-services","ai-compliance-tech","ai-compliance-services","ai-ma-tech","ai-ma-services","sigma"],je=["metrics-identified","economic-buyer-identified","decision-criteria-discussed","decision-process-discussed","pain-confirmed","champion-identified","competition-mentioned"],Ie=["progressing","stalled","at-risk","champion-engaged","early-stage"],xe=["discovery","demo","negotiation","qbr","implementation","follow-up"],Ee=`You are a sales intelligence tagger for Eudia, an AI legal technology company. Extract structured tags from meeting analysis.
...
`).replace(/\n/g,"<br>")}onClose(){this.contentEl.empty()}};var ne=class extends u.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new D(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return M}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}if(this.plugin.settings.accountsImported){this.steps[2].status="complete";try{let s=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");s&&await this.plugin.app.vault.delete(s)}catch{}let e=this.plugin.settings.userEmail,n=(this.plugin.settings.cachedAccounts||[]).filter(s=>s.id&&String(s.id).startsWith("001"));if(e&&n.length>0){let s=this.plugin.settings.accountsFolder||"Accounts",i=!1;for(let a of n.slice(0,5)){let o=(a.name||"").replace(/[<>:"/\\|?*]/g,"_").trim(),c=`${s}/${o}/Contacts.md`,l=this.plugin.app.vault.getAbstractFileByPath(c);if(l instanceof u.TFile&&!this.plugin.app.metadataCache.getFileCache(l)?.frontmatter?.enriched_at){i=!0;break}}i&&(console.log("[Eudia Setup] Accounts need enrichment \u2014 triggering on vault reopen..."),setTimeout(async()=>{try{let a=n.map(o=>({id:o.id,name:o.name,type:"",isOwned:!1,hadOpportunity:!0,website:null,industry:null}));await this.plugin.enrichAccountFolders(a),console.log(`[Eudia] Vault-reopen enrichment complete: ${a.length} accounts enriched`)}catch(a){console.log("[Eudia] Vault-reopen enrichment failed (will retry next open):",a)}},3e3))}}else{console.log("[Eudia Setup] Email set but accounts not imported \u2014 auto-retrying import...");let e=this.plugin.app.workspace.leftSplit,t=e?.collapsed;try{let n=this.plugin.settings.userEmail,s=$(n)?"admin":L(n)?"cs":"bl",i=[],a=[];if(console.log(`[Eudia Setup] Auto-retry for ${n} (group: ${s})`),s==="cs")i=[...k],console.log(`[Eudia Setup] Auto-retry CS: using ${i.length} static accounts`);else if(s==="admin")i=await this.accountOwnershipService.getAllAccountsForAdmin(n);else{let o=await this.accountOwnershipService.getAccountsWithProspects(n);i=o.accounts,a=o.prospects}if(i.length>0||a.length>0){if(e&&!t&&e.collapse(),s==="admin"?await this.plugin.createAdminAccountFolders(i):(await this.plugin.createTailoredAccountFolders(i,{}),a.length>0&&await this.plugin.createProspectAccountFiles(a)),H(n))try{await this.plugin.createCSManagerDashboard(n,i)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=i.length+a.length,await this.plugin.saveSettings(),this.steps[2].status="complete";try{let o=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");o&&await this.plugin.app.vault.delete(o)}catch{}e&&!t&&e.expand(),console.log(`[Eudia Setup] Auto-retry imported ${i.length} accounts for ${n}`),new u.Notice(`Enriching ${i.length} accounts with Salesforce contacts...`);try{let o=s==="cs"?i:[...i,...a];await this.plugin.enrichAccountFolders(o),new u.Notice(`${i.length} accounts loaded and enriched!`),console.log("[Eudia Setup] Auto-retry enrichment complete")}catch(o){console.log("[Eudia Setup] Auto-retry enrichment failed:",o),new u.Notice(`${i.length} accounts imported! Contacts will populate on next open.`)}}else console.warn(`[Eudia Setup] Auto-retry returned 0 accounts for ${n}. Server may still be starting.`),e&&!t&&e.expand()}catch(n){console.error("[Eudia Setup] Auto-retry account import failed:",n),e&&!t&&e.expand()}}}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),n=t.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to transcribe and summarize meetings -- capturing objections, next steps, and pain points to drive better client outcomes and smarter selling.",cls:"eudia-setup-subtitle"});let s=t.createDiv({cls:"eudia-setup-progress-section"}),i=this.getCompletionPercentage(),a=s.createDiv({cls:"eudia-setup-progress-label"});a.createSpan({text:"Setup Progress"}),a.createSpan({text:`${i}%`,cls:"eudia-setup-progress-value"});let c=s.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${i}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),s=n.createDiv({cls:"eudia-setup-step-header"}),i=s.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"1"),t.status==="complete"&&i.addClass("eudia-step-complete");let a=s.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:t.title}),a.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let l=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});l.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async d=>{d.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let n=await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new u.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");let s=this.plugin.app.workspace.leftSplit,i=s?.collapsed;s&&!i&&s.collapse();try{let a,o=[],c=$(e)?"admin":L(e)?"cs":"bl";if(console.log(`[Eudia] User group detected: ${c} for ${e}`),c==="cs"){if(console.log(`[Eudia] CS user detected \u2014 loading ${k.length} accounts from static data (instant, no server needed)`),a=[...k],o=[],t&&(t.textContent=`Loading ${a.length} Customer Success accounts...`),await this.plugin.createTailoredAccountFolders(a,{}),H(e))try{await this.plugin.createCSManagerDashboard(e,a)}catch(l){console.error("[Eudia] CS Manager dashboard creation failed (non-blocking):",l)}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,await this.plugin.saveSettings();try{let l=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");l&&await this.plugin.app.vault.delete(l)}catch{}console.log(`[Eudia] CS accounts created: ${a.length} folders from static data`),t&&(t.textContent=`Enriching ${a.length} accounts with Salesforce contacts...`),new u.Notice(`Enriching ${a.length} accounts with contacts from Salesforce...`),console.log(`[Eudia] Starting synchronous enrichment for ${a.length} CS accounts...`);try{await this.plugin.enrichAccountFolders(a),console.log("[Eudia] Synchronous enrichment complete"),new u.Notice(`${a.length} accounts loaded with contacts from Salesforce!`),t&&(t.textContent=`${a.length} accounts loaded and enriched with Salesforce contacts!`)}catch(l){console.log("[Eudia] Synchronous enrichment failed, will retry in background:",l),new u.Notice(`${a.length} accounts loaded! Contacts will populate shortly...`);let d=e,p=[5e3,2e4,6e4],y=async m=>{let h=p[m];if(h!==void 0){await new Promise(g=>setTimeout(g,h));try{await this.plugin.enrichAccountFolders(a),console.log(`[Eudia] Background enrichment retry ${m+1} succeeded`)}catch{return y(m+1)}}};y(0)}}else if(c==="admin"){if(console.log("[Eudia] Admin user detected - importing all accounts"),a=await this.accountOwnershipService.getAllAccountsForAdmin(e),a.length>0){t&&(t.textContent=`Creating ${a.length} account folders...`),await this.plugin.createAdminAccountFolders(a),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,await this.plugin.saveSettings();try{let d=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");d&&await this.plugin.app.vault.delete(d)}catch{}new u.Notice(`Imported ${a.length} accounts! Enriching with Salesforce data...`);let l=a.filter(d=>d.id&&d.id.startsWith("001"));if(l.length>0){t&&(t.textContent=`Enriching ${l.length} accounts with Salesforce contacts...`);try{await this.plugin.enrichAccountFolders(l),new u.Notice(`${a.length} accounts loaded and enriched with Salesforce data!`),console.log(`[Eudia] Admin/exec synchronous enrichment complete: ${l.length} accounts`),t&&(t.textContent=`${a.length} accounts loaded and enriched!`)}catch(d){console.log("[Eudia] Admin/exec synchronous enrichment failed, will retry on next open:",d),new u.Notice(`${a.length} accounts imported! Contacts will populate on next vault open.`);let p=[5e3,2e4,6e4],y=async m=>{let h=p[m];if(h!==void 0){await new Promise(g=>setTimeout(g,h));try{await this.plugin.enrichAccountFolders(l),console.log(`[Eudia] Admin/exec background enrichment retry ${m+1} succeeded`)}catch{return y(m+1)}}};y(0)}}}}else{let l=await this.accountOwnershipService.getAccountsWithProspects(e);if(a=l.accounts,o=l.prospects,a.length>0||o.length>0){t&&(t.textContent=`Creating ${a.length} account folders...`),await this.plugin.createTailoredAccountFolders(a,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length+o.length,await this.plugin.saveSettings();try{let p=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");p&&await this.plugin.app.vault.delete(p)}catch{}new u.Notice(`Imported ${a.length} active accounts + ${o.length} prospects!`);let d=[...a,...o];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(d)}catch(p){console.log("[Eudia] Background enrichment skipped:",p)}},500)}else console.warn(`[Eudia] No accounts returned for ${e} (userGroup: ${c})`),t&&(t.textContent=`No accounts found for ${e}. The server may still be starting \u2014 try again in 30 seconds.`,t.className="eudia-setup-validation-message warning"),new u.Notice("No accounts found. Server may be cold-starting \u2014 please try again shortly.")}}catch(a){console.error("[Eudia] Account import failed:",a),t&&(t.textContent="Account import failed. Please try again.",t.className="eudia-setup-validation-message error"),new u.Notice("Account import failed \u2014 please try again.")}finally{s&&!i&&s.expand()}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),s=n.createDiv({cls:"eudia-setup-step-header"}),i=s.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"2"),t.status==="complete"&&i.addClass("eudia-step-complete");let a=s.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:t.title}),a.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let l=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),d=o.createDiv({cls:"eudia-setup-sf-status"});l.onclick=async()=>{let p=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(p,"_blank"),d.textContent="Complete the login in the popup window...",d.className="eudia-setup-sf-status loading",new u.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(d)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,n=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new u.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=this.plugin.settings.userEmail,n=$(t)?"admin":L(t)?"cs":"bl";console.log(`[Eudia SF Import] Importing for ${t} (group: ${n})`);let s,i=[];if(n==="cs"){console.log("[Eudia SF Import] CS user SF Connect \u2014 fetching live data from Salesforce..."),e.textContent="Syncing with Salesforce for latest account data...";try{let m=await this.accountOwnershipService.getCSAccounts(t);s=m.accounts,i=m.prospects,console.log(`[Eudia SF Import] CS server sync: ${s.length} accounts (with real SF IDs + CSM data)`)}catch{if(this.plugin.settings.accountsImported){e.textContent="Salesforce connected! Account folders already loaded. Enrichment will retry later.",e.className="eudia-setup-sf-status success",this.steps[1].status="complete";return}s=[...k],console.log(`[Eudia SF Import] CS server unavailable \u2014 using ${s.length} static accounts`)}}else if(n==="admin")console.log("[Eudia] Admin user detected - importing all accounts"),e.textContent="Admin detected - importing all accounts...",s=await this.accountOwnershipService.getAllAccountsForAdmin(t);else{let m=await this.accountOwnershipService.getAccountsWithProspects(t);s=m.accounts,i=m.prospects}if(s.length===0&&i.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}e.textContent=`Creating ${s.length} account folders...`;let a=this.plugin.app.workspace.leftSplit,o=a?.collapsed;if(a&&!o&&a.collapse(),$(t)?await this.plugin.createAdminAccountFolders(s):(await this.plugin.createTailoredAccountFolders(s,{}),i.length>0&&await this.plugin.createProspectAccountFiles(i)),H(t))try{await this.plugin.createCSManagerDashboard(t,s)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+i.length,await this.plugin.saveSettings();try{let m=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");m&&await this.plugin.app.vault.delete(m)}catch{}a&&!o&&a.expand(),this.steps[2].status="complete";let c=s.filter(m=>m.isOwned!==!1).length,l=s.filter(m=>m.isOwned===!1).length;n==="admin"&&l>0?e.textContent=`${c} owned + ${l} view-only accounts imported! Enriching...`:e.textContent=`${s.length} active + ${i.length} prospect accounts imported! Enriching...`,e.className="eudia-setup-sf-status success";let d=[...s,...i],p=t,y=n;setTimeout(async()=>{try{let m=d.filter(h=>h.id&&h.id.startsWith("001"));if(m.length>0?(e.textContent=`Enriching ${m.length} accounts with Salesforce data...`,await this.plugin.enrichAccountFolders(m),e.textContent=`${s.length} accounts imported, ${m.length} enriched with Salesforce data`):e.textContent=`${s.length} accounts imported (enrichment requires Salesforce IDs)`,y==="cs"&&H(p))try{console.log("[Eudia SF Import] Regenerating CS Manager dashboard with live CSM data..."),await this.plugin.createCSManagerDashboard(p,s),console.log("[Eudia SF Import] CS Manager dashboard updated with CSM assignments")}catch(h){console.error("[Eudia SF Import] Dashboard regeneration failed (non-blocking):",h)}}catch(m){console.log("[Eudia] Background enrichment skipped:",m),e.textContent=`${s.length+i.length} accounts imported (enrichment will retry on next launch)`}},500)}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error";let n=this.plugin.app.workspace.leftSplit;if(n?.collapsed===!1)try{n.expand()}catch{}}}renderTranscribeStep(e){let t=this.steps[2],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),s=n.createDiv({cls:"eudia-setup-step-header"}),i=s.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"3"),t.status==="complete"&&i.addClass("eudia-step-complete");let a=s.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:t.title}),a.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),l=c.createDiv({cls:"eudia-setup-instruction"});l.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),l.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),d.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let p=c.createDiv({cls:"eudia-setup-instruction"});p.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),p.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(i=>i.status==="complete")){let i=t.createDiv({cls:"eudia-setup-completion"});i.createEl("h2",{text:"\u{1F389} You're all set!"}),i.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let a=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});a.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(M),await this.plugin.activateCalendarView()}}else{let i=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});i.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(M),new u.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let s=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});s.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},ae=class extends u.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return R}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),n=t.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let s=n.createDiv({cls:"eudia-calendar-actions"}),i=s.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});i.title="Refresh (fetches latest from calendar)",i.onclick=async()=>{i.addClass("spinning"),this._forceRefresh=!0,await this.render(),i.removeClass("spinning")};let a=s.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});a.title="Settings",a.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,s=this.plugin.settings.userEmail;try{(await(0,u.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(s&&t.server==="connected")try{let d=await(0,u.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(s)}`,method:"GET",throw:!1});d.status===200&&d.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(s&&t.server==="connected")try{let d=await(0,u.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(s)}`,method:"GET",throw:!1});d.status===200&&d.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let i=e.createDiv({cls:"eudia-status-indicators"}),a=i.createSpan({cls:`eudia-status-dot ${t.server}`});a.title=t.serverMessage||"Server";let o=i.createSpan({cls:`eudia-status-dot ${t.calendar}`});o.title=t.calendarMessage||"Calendar";let c=i.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(c.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let d=e.createDiv({cls:"eudia-status-warning"});d.innerHTML=`<strong>${s}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),n=t.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let s=new P(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),i=this._forceRefresh||!1;this._forceRefresh=!1;let a=await s.getWeekMeetings(i);if(n.remove(),!a.success){this.renderError(t,a.error||"Failed to load calendar");return}let o=Object.keys(a.byDay||{}).sort();if(o.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,s);for(let c of o){let l=a.byDay[c];!l||l.length===0||this.renderDaySection(t,c,l)}}catch(s){n.remove(),this.renderError(t,s.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let n=await t.getCurrentMeeting();if(n.meeting){let s=e.createDiv({cls:"eudia-now-card"});n.isNow?s.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):s.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let i=s.createDiv({cls:"eudia-now-content"});i.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&i.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let a=s.createEl("button",{cls:"eudia-now-action",text:"Create Note"});a.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(e,t,n){let s=e.createDiv({cls:"eudia-calendar-day"});s.createEl("div",{cls:"eudia-calendar-day-header",text:P.getDayName(t)});for(let i of n){let a=s.createDiv({cls:`eudia-calendar-meeting ${i.isCustomerMeeting?"customer":"internal"}`});a.createEl("div",{cls:"eudia-calendar-time",text:P.formatTime(i.start,this.plugin.settings.timezone)});let o=a.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:i.subject}),i.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:i.accountName});else if(i.attendees&&i.attendees.length>0){let c=i.attendees.slice(0,2).map(l=>l.name||l.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}a.onclick=()=>this.createNoteForMeeting(i),a.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `}renderError(e,t){let n=e.createDiv({cls:"eudia-calendar-error"}),s="",i="Unable to load calendar",a="";t.includes("not authorized")||t.includes("403")?(s="\u{1F512}",i="Calendar Access Required",a="Contact your admin to be added to the authorized users list."):t.includes("network")||t.includes("fetch")?(s="\u{1F4E1}",i="Connection Issue",a="Check your internet connection and try again."):(t.includes("server")||t.includes("500"))&&(s="\u{1F527}",i="Server Unavailable",a="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=t.createDiv({cls:"eudia-setup-input-group"}),s=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});s.addClass("eudia-setup-email");let i=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),a=t.createDiv({cls:"eudia-setup-status"});i.onclick=async()=>{let o=s.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){a.textContent="Please use your @eudia.com email",a.className="eudia-setup-status error";return}i.disabled=!0,i.textContent="Connecting...",a.textContent="";try{if(!(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){a.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,a.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),a.textContent="Connected",a.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let l=c.message||"Connection failed";l.includes("403")?a.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:a.textContent=l,a.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect"}},s.onkeydown=o=>{o.key==="Enter"&&i.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(e){let t=e.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],s=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],i=t.filter(o=>!s.includes(o)&&o.length>1);if(i.length===0)return t[0]||"";if(i.length>1&&n.includes(i[0]))return i[1].charAt(0).toUpperCase()+i[1].slice(1);let a=i[i.length-1];return a.charAt(0).toUpperCase()+a.slice(1)}getExternalDomainsFromAttendees(e){if(!e||e.length===0)return[];let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,s=[];for(let i of e){if(!i.email)continue;let o=i.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let c=o[1];if(c.includes("eudia.com")||t.includes(c)||n.has(c))continue;n.add(c);let l=this.extractCompanyFromDomain(c);l.length>=2&&s.push({domain:c,company:l})}}return s}findBestAccountMatch(e,t,n){let s=this.plugin.settings.accountsFolder||"Accounts",i=this.app.vault.getAbstractFileByPath(s);if(!(i instanceof u.TFolder))return null;let a=[];for(let c of i.children)c instanceof u.TFolder&&a.push(c.name);if(a.length===0)return null;let o=[];for(let{domain:c,company:l}of e){let d=this.findAccountFolder(l),p=d?1:0;o.push({domain:c,company:l,folder:d,score:p})}if(o.sort((c,l)=>l.score-c.score),o.length>0&&o[0].folder){let c=o[0],l=c.folder.split("/").pop()||c.company;return console.log(`[Eudia Calendar] Best domain match: "${c.company}" from ${c.domain} -> ${c.folder}`),{folder:c.folder,accountName:l,source:"domain"}}if(t){let c=this.findAccountFolder(t);if(c){let l=c.split("/").pop()||t;return console.log(`[Eudia Calendar] Server account match: "${t}" -> ${c}`),{folder:c,accountName:l,source:"server"}}}if(n){let c=this.findAccountFolder(n);if(c){let l=c.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${c}`),{folder:c,accountName:l,source:"subject"}}}for(let{company:c}of e){let l=a.find(d=>{let p=d.toLowerCase(),y=c.toLowerCase();return p.includes(y)||y.includes(p)});if(l){let d=`${s}/${l}`;return console.log(`[Eudia Calendar] Partial domain match: "${c}" -> ${d}`),{folder:d,accountName:l,source:"domain-partial"}}}return null}extractAccountFromAttendees(e){let t=this.getExternalDomainsFromAttendees(e);if(t.length===0)return null;let n=t[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let s=(t[1]||t[2]||"").trim();if(s.toLowerCase()!=="eudia")return s}let n=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let i=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(i.toLowerCase()!=="eudia"&&i.length>0)return i}if(!e.toLowerCase().includes("eudia")){let s=e.match(/^([^-–|]+)/);if(s){let i=s[1].trim();if(i.length>2&&i.length<50)return i}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(t);if(!(n instanceof u.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let s=e.toLowerCase().trim(),i=[];for(let p of n.children)p instanceof u.TFolder&&i.push(p.name);console.log(`[Eudia Calendar] Searching for "${s}" in ${i.length} folders`);let a=i.find(p=>p.toLowerCase()===s);if(a)return console.log(`[Eudia Calendar] Exact match found: ${a}`),`${t}/${a}`;let o=i.find(p=>p.toLowerCase().startsWith(s));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${t}/${o}`;let c=i.find(p=>s.startsWith(p.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${t}/${c}`;let l=i.find(p=>{let y=p.toLowerCase();return y.length>=3&&s.includes(y)});if(l)return console.log(`[Eudia Calendar] Search contains folder match: ${l}`),`${t}/${l}`;let d=i.find(p=>{let y=p.toLowerCase();return s.length>=3&&y.includes(s)});return d?(console.log(`[Eudia Calendar] Folder contains search match: ${d}`),`${t}/${d}`):(console.log(`[Eudia Calendar] No folder match found for "${s}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",s=$(n),i=(e.attendees||[]).map(w=>w.email).filter(Boolean),a=re(e.subject,i);if(s&&a.isPipelineMeeting&&a.confidence>=60){await this._createPipelineNote(e,t);return}let o=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),c=`${t} - ${o}.md`,l=null,d=e.accountName||null,p=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(w=>w.email)||[])}`);let y=this.getExternalDomainsFromAttendees(e.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(y)}`);let m=this.extractAccountFromSubject(e.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${m||"none"}"`);let h=this.findBestAccountMatch(y,e.accountName,m||void 0);if(h&&(l=h.folder,d=h.accountName,console.log(`[Eudia Calendar] Best match (${h.source}): "${d}" -> ${l}`)),!l){let w=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(w)instanceof u.TFolder&&(l=w,console.log(`[Eudia Calendar] No match found, using Accounts root: ${l}`))}if(d){let w=this.plugin.settings.cachedAccounts.find(b=>b.name.toLowerCase()===d?.toLowerCase());w&&(p=w.id,d=w.name,console.log(`[Eudia Calendar] Matched to cached account: ${w.name} (${w.id})`))}let g=l?`${l}/${c}`:c,O=this.app.vault.getAbstractFileByPath(g);if(O instanceof u.TFile){await this.app.workspace.getLeaf().openFile(O);try{let w=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;w?.revealInFolder&&w.revealInFolder(O)}catch{}new u.Notice(`Opened existing note: ${c}`);return}let S=(e.attendees||[]).map(w=>w.name||w.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${e.subject}"
date: ${t}
attendees: [${S}]
account: "${d||""}"
...
## Attendees
${(e.attendees||[]).map(w=>`- ${w.name||w.email}`).join(`
`)}
...
`;try{let w=await this.app.vault.create(g,C);await this.app.workspace.getLeaf().openFile(w);try{let b=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;b?.revealInFolder&&b.revealInFolder(w)}catch{}new u.Notice(`Created: ${g}`)}catch(w){console.error("[Eudia Calendar] Failed to create note:",w),new u.Notice(`Could not create note: ${w.message||"Unknown error"}`)}}async _createPipelineNote(e,t){let n=new Date(t+"T00:00:00"),s=String(n.getMonth()+1).padStart(2,"0"),i=String(n.getDate()).padStart(2,"0"),a=String(n.getFullYear()).slice(-2),o=`${s}.${i}.${a}`,c=`Team Pipeline Meeting - ${o}.md`,l="Pipeline Meetings";this.app.vault.getAbstractFileByPath(l)||await this.app.vault.createFolder(l);let p=`${l}/${c}`,y=this.app.vault.getAbstractFileByPath(p);if(y instanceof u.TFile){await this.app.workspace.getLeaf().openFile(y);try{let g=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;g?.revealInFolder&&g.revealInFolder(y)}catch{}new u.Notice(`Opened existing: ${c}`);return}let m=(e.attendees||[]).map(g=>g.name||g.email?.split("@")[0]||"Unknown"),h=`---
...
`},{name:"Meeting Notes.md",content:S},{name:"Contacts.md",content:g},{name:"Intelligence.md",content:O},{name:"Next Steps.md",content:C}];for(let W of w){let I=`${p}/${W.name}`;await this.app.vault.create(I,W.content)}return console.log(`[Eudia] Created account folder with subnotes${h?" (enriched)":""}: ${d}`),!0}catch(m){return console.error(`[Eudia] Failed to create folder for ${d}:`,m),!1}},c=5;for(let l=0;l<e.length;l+=c){let d=e.slice(l,l+c),p=await Promise.allSettled(d.map(y=>o(y)));i+=p.filter(y=>y.status==="fulfilled"&&y.value===!0).length}this.settings.cachedAccounts=e.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),i>0&&new u.Notice(`Created ${i} account folders`),await this.ensureNextStepsFolderExists()}buildContactsContent(e,t,n){let s=t?`
...
---`,i=[];return t?.opportunities&&i.push(t.opportunities),t?.recentActivity&&i.push(t.recentActivity),i.length>0?`${s}
...
3. **Next Steps** are auto-extracted after transcription
4. Set \`sync_to_salesforce: true\` to sync to Salesforce
`}buildNextStepsContent(e,t,n){let s=n||new Date().toISOString().split("T")[0],i=t?`
...
*Previous next steps will be archived here.*
`}];for(let O of g){let S=`${c}/${O.name}`;await this.app.vault.create(S,O.content)}i++}catch(h){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,h)}}return i>0&&console.log(`[Eudia] Created ${i} prospect account folders in _Prospects/`),i}async createCSManagerDashboard(e,t){let n="CS Manager",s=new Date().toISOString().split("T")[0],i=pe(e);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let a={};for(let p of t){let y=p.ownerName||"Unassigned";a[y]||(a[y]=[]),a[y].push(p)}let o=`---
role: cs_manager
manager: "${e}"
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof u.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let p of i){let y=p.split("@")[0].replace("."," ").replace(/\b\w/g,b=>b.toUpperCase()),m=p.split("@")[0].replace("."," ").toLowerCase(),h=m.split(" ")[0],g=m.split(" ").pop()||"",O=t.filter(b=>{let W=(b.csmName||"").toLowerCase();if(W&&(W.includes(h)||W.includes(g)))return!0;let I=(b.ownerName||"").toLowerCase();return I.includes(h)||I.includes(g)}),S=`---
rep: "${p}"
rep_name: "${y}"
role: cs_rep_summary
...
|---------|------|-------|--------|
`;for(let b of O){let W=b.name.replace(/[<>:"/\\|?*]/g,"_").trim();S+=`| ${b.name} | ${b.type||""} | ${b.ownerName||""} | [[Accounts/${W}/Contacts\\|View]] |
...
`;let C=`${n}/${y}.md`,w=this.app.vault.getAbstractFileByPath(C);w instanceof u.TFile?await this.app.vault.modify(w,S):await this.app.vault.create(C,S)}console.log(`[Eudia] Created CS Manager dashboard for ${e} with ${t.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let s=0,i=0,a=new Date().toISOString().split("T")[0],o=async l=>{let d=l.name.replace(/[<>:"/\\|?*]/g,"_").trim(),p=`${t}/${d}`;if(this.app.vault.getAbstractFileByPath(p)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(p),await this.createExecAccountSubnotes(p,l,a),l.isOwned?s++:i++,console.log(`[Eudia Admin] Created ${l.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,m),!1}},c=5;for(let l=0;l<e.length;l+=c){let d=e.slice(l,l+c);await Promise.allSettled(d.map(p=>o(p)))}this.settings.cachedAccounts=e.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),s+i>0&&new u.Notice(`Created ${s} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let s=t.ownerName||"Unknown",i=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let a of i){let o=`${e}/${a.name}`;await this.app.vault.create(o,a.content)}}async createFullAccountSubnotes(e,t,n){let s=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let i of s){let a=`${e}/${i.name}`;await this.app.vault.create(a,i.content)}}async ensurePipelineFolderExists(){let e="Pipeline",t=`${e}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let a=`---
...
`;await this.app.vault.create(t,o)}}async updateAccountNextSteps(e,t,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${e}`),console.log(`[Eudia] Content length: ${t?.length||0} chars`);let s=e.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${this.settings.accountsFolder}/${s}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${i}`);let a=this.app.vault.getAbstractFileByPath(i);if(!a||!(a instanceof u.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${i}`);let S=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${s}`);S&&S instanceof u.TFolder?console.log(`[Eudia] Files in ${s} folder:`,S.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${s}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),l=n.split("/").pop()?.replace(".md","")||"Meeting",d=t;!t.includes("- [ ]")&&!t.includes("- [x]")&&(d=t.split(`
...
`;if(p)m+=p;else{let h=[e.painPoints,e.productInterest,e.nextSteps,e.actionItems].filter(Boolean).map(d).join(`
...
`),m}buildNoteContent(e,t){let n=g=>g==null?"":Array.isArray(g)?g.map(O=>typeof O=="object"?O.category?`**${O.category}**: ${O.signal||O.insight||""}`:JSON.stringify(O):String(O)).join(`
`):typeof g=="object"?JSON.stringify(g):String(g),s=n(e.title)||"Meeting Notes",i=n(e.summary),a=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),l=n(e.nextSteps),d=n(e.actionItems),p=n(e.keyDates),y=n(e.risksObjections),m=n(e.attendees),h=`---
title: "${s}"
...
`;try{await this.app.vault.create(c,l)}catch{}t++,console.log(`[Eudia] Archived: ${i.name}`)}catch(a){console.error(`[Eudia] Failed to archive ${i.name}:`,a)}return t}async syncSpecificNoteToSalesforce(e){let t=await this.app.vault.read(e),n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let s=n.account_id,i=n.account;if(!s&&i){let a=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===i.toLowerCase());a&&(s=a.id)}if(!s){let a=e.path.split("/");if(a.length>=2&&a[0]===this.settings.accountsFolder){let o=a[1]==="_Prospects"&&a.length>=3?a[2]:a[1],c=this.settings.cachedAccounts.find(l=>l.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&(s=c.id,i=c.name)}}if(!s)return{success:!1,error:`Could not determine account for ${e.path}`};try{let a=await(0,u.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:s,accountName:i,noteTitle:e.basename,notePath:e.path,content:t,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return a.json?.success?{success:!0}:{success:!1,error:a.json?.error||"Unknown error",authRequired:a.json?.authRequired}}catch(a){return{success:!1,error:a.message}}}async syncNoteToSalesforce(){let e=this.app.workspace.getActiveFile();if(!e){new u.Notice("No active file to sync");return}if(!this.app.metadataCache.getFileCache(e)?.frontmatter?.sync_to_salesforce){new u.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}new u.Notice("Syncing to Salesforce...");let n=await this.syncSpecificNoteToSalesforce(e);n.success?new u.Notice("Synced to Salesforce"):n.authRequired?new u.Notice("Salesforce authentication required. Please reconnect."):new u.Notice("Failed to sync: "+(n.error||"Unknown error"))}async checkAndAutoEnrich(){let e=this.settings.accountsFolder||"Accounts",t=this.app.vault.getAbstractFileByPath(e);if(!t||!(t instanceof u.TFolder))return;let n=[];for(let s of t.children){if(!(s instanceof u.TFolder)||s.name.startsWith("_"))continue;let i=`${s.path}/Contacts.md`,a=this.app.vault.getAbstractFileByPath(i);if(!(!a||!(a instanceof u.TFile))){if(this.app.metadataCache.getFileCache(a)?.frontmatter?.enriched_at)continue}let o=s.name,c=this.settings.cachedAccounts.find(l=>l.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&c.id&&n.push({id:c.id,name:c.name,owner:"",ownerEmail:""})}if(n.length===0){console.log("[Eudia] Auto-enrich: all account folders already enriched");return}console.log(`[Eudia] Auto-enrich: ${n.length} accounts need enrichment`);try{await this.enrichAccountFolders(n)}catch(s){console.error("[Eudia] Auto-enrich failed:",s)}}async enrichAccountFolders(e){if(!e||e.length===0)return;let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=this.settings.accountsFolder||"Accounts",s=e.filter(d=>d.id&&d.id.startsWith("001"));if(s.length===0)return;let i=s.length,a=0,o=0;console.log(`[Eudia Enrich] Starting enrichment for ${i} accounts`),new u.Notice(`Enriching account data: 0/${i}...`);let c=20;for(let d=0;d<s.length;d+=c){let p=s.slice(d,d+c),y=p.map(h=>h.id);try{let h=await(0,u.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:y,userEmail:this.settings.userEmail})});if(h.json?.success&&h.json?.enrichments){let g=h.json.enrichments;for(let O of p){let S=g[O.id];if(S)try{await this.writeEnrichmentToAccount(O,S,n),a++}catch(C){o++,console.error(`[Eudia Enrich] Write failed for ${O.name}:`,C)}}}}catch(h){o+=p.length,console.error("[Eudia Enrich] Batch fetch failed:",h)}let m=Math.min(d+c,i);new u.Notice(`Enriching account data: ${m}/${i}...`),d+c<s.length&&await new Promise(h=>setTimeout(h,100))}let l=o>0?`Enrichment complete: ${a} enriched, ${o} skipped`:`Enrichment complete: ${a} accounts enriched with Salesforce data`;console.log(`[Eudia Enrich] ${l}`),new u.Notice(l)}async writeEnrichmentToAccount(e,t,n){let s=e.name.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${n}/${s}`,a=this.app.vault.getAbstractFileByPath(i);if(a instanceof u.TFolder||(i=`${n}/_Prospects/${s}`,a=this.app.vault.getAbstractFileByPath(i)),!(a instanceof u.TFolder))return;let o=new Date().toISOString(),c=async(l,d)=>{let p=`${i}/${l}`,y=this.app.vault.getAbstractFileByPath(p);if(!(y instanceof u.TFile))return;let m=await this.app.vault.read(y),h="",g=m;if(m.startsWith("---")){let w=m.indexOf("---",3);w!==-1&&(h=m.substring(0,w+3),g=m.substring(w+3),h.includes("enriched_at:")?h=h.replace(/enriched_at:.*/,`enriched_at: "${o}"`):h=h.substring(0,w)+`enriched_at: "${o}"
---`)}let O=g.match(/^(\s*#[^\n]+)/),C=`${O?O[1]:""}
...
*Add org chart, decision makers, champions, and blockers here.*`),t.intelligence&&await c("Intelligence.md",t.intelligence),t.nextSteps&&await c("Next Steps.md",t.nextSteps),t.opportunities||t.recentActivity){let l=`${i}/Meeting Notes.md`,d=this.app.vault.getAbstractFileByPath(l);if(d instanceof u.TFile){let p=await this.app.vault.read(d),y="",m=p;if(p.startsWith("---")){let S=p.indexOf("---",3);S!==-1&&(y=p.substring(0,S+3),m=p.substring(S+3),y.includes("enriched_at:")?y=y.replace(/enriched_at:.*/,`enriched_at: "${o}"`):y=y.substring(0,S)+`enriched_at: "${o}"
---`)}let h=m.match(/^(\s*#[^\n]+)/),O=[h?h[1]:`
...
${O.join(`
`)}
`)}}}startSalesforceSyncScanner(){if(!this.settings.sfAutoSyncEnabled){console.log("[Eudia SF Sync] Auto-sync is disabled in settings"),this.updateSfSyncStatusBar("SF Sync: Off");return}let e=(this.settings.sfAutoSyncIntervalMinutes||15)*60*1e3;console.log(`[Eudia SF Sync] Starting scanner \u2014 interval: ${this.settings.sfAutoSyncIntervalMinutes}min`),this.updateSfSyncStatusBar("SF Sync: Idle");let t=window.setTimeout(()=>{this.runSalesforceSyncScan()},3e4);this.registerInterval(t),this.sfSyncIntervalId=window.setInterval(()=>{this.runSalesforceSyncScan()},e),this.registerInterval(this.sfSyncIntervalId)}async runSalesforceSyncScan(){if(!(!this.settings.sfAutoSyncEnabled||!this.settings.userEmail)){console.log("[Eudia SF Sync] Running scan...");try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!(e instanceof u.TFolder)){console.log("[Eudia SF Sync] Accounts folder not found");return}let t=[],n=d=>{for(let p of d.children)p instanceof u.TFile&&p.extension==="md"?t.push(p):p instanceof u.TFolder&&n(p)};n(e);let s=[];for(let d of t){let y=this.app.metadataCache.getFileCache(d)?.frontmatter;if(!y?.sync_to_salesforce)continue;let m=y.last_sf_sync?new Date(y.last_sf_sync).getTime():0;d.stat.mtime>m&&s.push(d)}if(s.length===0){console.log("[Eudia SF Sync] No flagged notes need syncing"),this.updateSfSyncStatusBar("SF Sync: Idle");return}console.log(`[Eudia SF Sync] ${s.length} note(s) queued for sync`),this.updateSfSyncStatusBar(`SF Sync: Syncing ${s.length}...`);let i=0,a=0;for(let d of s){let p=await this.syncSpecificNoteToSalesforce(d);if(p.success)i++,await this.updateNoteSyncTimestamp(d);else if(a++,console.log(`[Eudia SF Sync] Failed to sync ${d.path}: ${p.error}`),p.authRequired){new u.Notice("Salesforce authentication expired. Please reconnect to resume auto-sync."),this.updateSfSyncStatusBar("SF Sync: Auth required");return}}let c=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),l=a>0?`SF Sync: ${i} synced, ${a} failed at ${c}`:`SF Sync: ${i} note${i!==1?"s":""} synced at ${c}`;console.log(`[Eudia SF Sync] ${l}`),this.updateSfSyncStatusBar(l),i>0&&new u.Notice(l)}catch(e){console.error("[Eudia SF Sync] Scan error:",e),this.updateSfSyncStatusBar("SF Sync: Error")}}}async updateNoteSyncTimestamp(e){try{let t=await this.app.vault.read(e),n=new Date().toISOString();if(t.startsWith("---")){let s=t.indexOf("---",3);if(s!==-1){let i=t.substring(0,s),a=t.substring(s);if(i.includes("last_sf_sync:")){let o=i.replace(/last_sf_sync:.*/,`last_sf_sync: "${n}"`)+a;await this.app.vault.modify(e,o)}else{let o=i+`last_sf_sync: "${n}"
`+a;await this.app.vault.modify(e,o)}}}}catch(t){console.error(`[Eudia SF Sync] Failed to update sync timestamp for ${e.path}:`,t)}}updateSfSyncStatusBar(e){this.sfSyncStatusBarEl&&this.sfSyncStatusBarEl.setText(e)}async createMeetingNote(){return new Promise(e=>{new te(this.app,this,async n=>{if(!n){e();return}let s=new Date().toISOString().split("T")[0],i=n.name.replace(/[<>:"/\\|?*]/g,"_").trim(),a=`${this.settings.accountsFolder}/${i}`,o=`${s} Meeting.md`,c=`${a}/${o}`;this.app.vault.getAbstractFileByPath(a)||await this.app.vault.createFolder(a);let l=`---
...
`;await this.app.vault.modify(e,a)}async updateTeamPerformanceNote(e,t){if(!t)return;let n=new Date().toISOString().split("T")[0],s=a=>a>0?`\u2191 ${Math.abs(a).toFixed(1)}%`:a<0?`\u2193 ${Math.abs(a).toFixed(1)}%`:"--",i=`---
type: analytics_dashboard
auto_refresh: true
...
`;await this.app.vault.modify(e,i)}},ie=class extends u.PluginSettingTab{constructor(r,e){super(r,e),this.plugin=e}display(){let{containerEl:r}=this;r.empty(),r.createEl("h2",{text:"Eudia Sync & Scribe"}),r.createEl("h3",{text:"Your Profile"});let e=r.createDiv();e.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let t=e.createDiv();t.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let n=t.createSpan(),s=t.createSpan(),i=e.createDiv();i.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",i.setText("Connect with Salesforce to sync notes with your user attribution.");let a=e.createEl("button");a.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,c=async()=>{if(!this.plugin.settings.userEmail)return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",s.setText("Enter email above first"),a.setText("Setup Required"),a.disabled=!0,a.style.opacity="0.5",a.style.cursor="not-allowed",!1;a.disabled=!1,a.style.opacity="1",a.style.cursor="pointer";try{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",s.setText("Checking..."),(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",s.setText("Connected to Salesforce"),a.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",s.setText("Not connected"),a.setText("Connect to Salesforce"),!1)}catch{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",s.setText("Status unavailable"),a.setText("Connect to Salesforce"),!1}};new u.Setting(r).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(m=>m.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async h=>{let g=h.trim().toLowerCase();this.plugin.settings.userEmail=g,await this.plugin.saveSettings(),await c()})),new u.Setting(r).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(m=>{me.forEach(h=>{m.addOption(h.value,h.label)}),m.setValue(this.plugin.settings.timezone),m.onChange(async h=>{this.plugin.settings.timezone=h,await this.plugin.saveSettings(),this.plugin.calendarService?.setTimezone(h),new u.Notice(`Timezone set to ${me.find(g=>g.value===h)?.label||h}`)})}),r.createEl("h3",{text:"Salesforce Connection"}),r.appendChild(e);let l=()=>{o&&window.clearInterval(o);let m=0,h=30;o=window.setInterval(async()=>{m++,await c()?(o&&(window.clearInterval(o),o=null),new u.Notice("Salesforce connected successfully!")):m>=h&&o&&(window.clearInterval(o),o=null)},5e3)};a.onclick=async()=>{if(!this.plugin.settings.userEmail){new u.Notice("Please enter your email first");return}let m=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(m,"_blank"),new u.Notice("Complete the Salesforce login in the popup window",5e3),l()},c(),r.createEl("h3",{text:"Server"}),new u.Setting(r).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(m=>m.setValue(this.plugin.settings.serverUrl).onChange(async h=>{this.plugin.settings.serverUrl=h,await this.plugin.saveSettings()}));let d=r.createDiv({cls:"settings-advanced-collapsed"}),p=d.createDiv({cls:"eudia-transcription-status"});p.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",p.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?p.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':p.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{p.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let y=new u.Setting(r).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(m=>m.setValue(!1).onChange(h=>{d.style.display=h?"block":"none"}));d.style.display="none",r.createEl("h3",{text:"Transcription"}),new u.Setting(r).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(m=>m.setValue(this.plugin.settings.saveAudioFiles).onChange(async h=>{this.plugin.settings.saveAudioFiles=h,await this.plugin.saveSettings()})),new u.Setting(r).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(m=>m.setValue(this.plugin.settings.appendTranscript).onChange(async h=>{this.plugin.settings.appendTranscript=h,await this.plugin.saveSettings()})),r.createEl("h3",{text:"Sync"}),new u.Setting(r).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(m=>m.setValue(this.plugin.settings.syncOnStartup).onChange(async h=>{this.plugin.settings.syncOnStartup=h,await this.plugin.saveSettings()})),new u.Setting(r).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(m=>m.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async h=>{this.plugin.settings.autoSyncAfterTranscription=h,await this.plugin.saveSettings()})),new u.Setting(r).setName("Auto-Sync Flagged Notes").setDesc("Periodically push notes with sync_to_salesforce: true to Salesforce").addToggle(m=>m.setValue(this.plugin.settings.sfAutoSyncEnabled).onChange(async h=>{this.plugin.settings.sfAutoSyncEnabled=h,await this.plugin.saveSettings(),h?this.plugin.startSalesforceSyncScanner():this.plugin.updateSfSyncStatusBar("SF Sync: Off")})),new u.Setting(r).setName("Auto-Sync Interval").setDesc("How often to scan for flagged notes (in minutes)").addDropdown(m=>{m.addOption("5","Every 5 minutes"),m.addOption("15","Every 15 minutes"),m.addOption("30","Every 30 minutes"),m.setValue(String(this.plugin.settings.sfAutoSyncIntervalMinutes)),m.onChange(async h=>{this.plugin.settings.sfAutoSyncIntervalMinutes=parseInt(h),await this.plugin.saveSettings(),new u.Notice(`SF auto-sync interval set to ${h} minutes. Restart Obsidian for changes to take effect.`)})}),r.createEl("h3",{text:"Folders"}),new u.Setting(r).setName("Accounts Folder").setDesc("Where account folders are stored").addText(m=>m.setValue(this.plugin.settings.accountsFolder).onChange(async h=>{this.plugin.settings.accountsFolder=h||"Accounts",await this.plugin.saveSettings()})),new u.Setting(r).setName("Recordings Folder").setDesc("Where audio files are saved").addText(m=>m.setValue(this.plugin.settings.recordingsFolder).onChange(async h=>{this.plugin.settings.recordingsFolder=h||"Recordings",await this.plugin.saveSettings()})),r.createEl("h3",{text:"Actions"}),new u.Setting(r).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(m=>m.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new u.Setting(r).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(m=>m.setButtonText("Refresh Folders").onClick(async()=>{m.setButtonText("Checking..."),m.setDisabled(!0);try{let h=await this.plugin.refreshAccountFolders();h>0?new u.Notice(`Created ${h} new account folder${h>1?"s":""}`):new u.Notice("All account folders are up to date")}catch(h){new u.Notice("Failed to refresh folders: "+h.message)}m.setButtonText("Refresh Folders"),m.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&r.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),r.createEl("p",{text:`Audio transcription: ${T.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
