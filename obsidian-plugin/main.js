...
`;let l=Math.round(s/5),c=Math.round(a/5);if(n+=`\`${"\u2588".repeat(l)}${"\u2591".repeat(20-l)}\` Rep
...
`,t}async blobToBase64(i){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let s=n.result.split(",")[1];t(s)},n.onerror=e,n.readAsDataURL(i)})}async transcribeAudio(i,t){let e=i.size/1024/1024,n=i.type||"audio/webm";if(e>15)return console.log(`[Eudia] Large recording (${e.toFixed(1)}MB) \u2014 using chunked transcription`),this.transcribeAudioChunked(i,n,t);try{let r=await this.blobToBase64(i),s=t?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:t.pipelineContext}:void 0,a=await this.transcribeAndSummarize(r,n,t?.accountName,t?.accountId,s,{captureMode:t?.captureMode,hasVirtualDevice:t?.hasVirtualDevice},t?.meetingTemplate);return{text:a.transcript,confidence:a.success?.95:0,duration:a.duration,sections:a.sections,diarizedTranscript:a.diarizedTranscript,error:a.error}}catch(r){return console.error("transcribeAudio error:",r),{text:"",confidence:0,duration:0,sections:this.getEmptySections(),error:r.message||"Transcription request failed"}}}static{this.CHUNK_MAX_RETRIES=3}static{this.CHUNK_RETRY_DELAYS=[5e3,15e3,3e4]}async transcribeChunkWithRetry(i,t,e,n){for(let r=0;r<=O.CHUNK_MAX_RETRIES;r++){if(r>0){let s=O.CHUNK_RETRY_DELAYS[r-1]||3e4;console.log(`[Eudia] Chunk ${e+1}/${n} retry ${r}/${O.CHUNK_MAX_RETRIES} in ${s/1e3}s...`),await new Promise(a=>setTimeout(a,s))}try{let s=await(0,P.requestUrl)({url:`${this.serverUrl}/api/transcribe-chunk`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({audio:i,mimeType:t})}),a=s.json?.text||s.json?.transcript||"";if(s.json?.success&&a)return r>0&&console.log(`[Eudia] Chunk ${e+1}/${n} succeeded on retry ${r}`),{text:a,duration:s.json.duration||0};console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${r+1} returned no text: ${s.json?.error||"unknown"}`)}catch(s){console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${r+1} failed: ${s.message}`)}}return null}async transcribeAudioChunked(i,t,e){let r=await i.arrayBuffer(),s=r.byteLength,a=Math.ceil(s/8388608);console.log(`[Eudia] Chunked transcription: ${(s/1024/1024).toFixed(1)}MB \u2192 ${a} chunks`);let o=16*1024,l=s/o,c=[],d=0,u=0;for(let g=0;g<a;g++){let v=g*8388608,b=Math.min(v+8388608,s),w=r.slice(v,b),C=new Blob([w],{type:t});console.log(`[Eudia] Transcribing chunk ${g+1}/${a} (${((b-v)/1024/1024).toFixed(1)}MB)`);let f=await this.blobToBase64(C),m=await this.transcribeChunkWithRetry(f,t,g,a);if(m)c.push(m.text),d+=m.duration,console.log(`[Eudia] Chunk ${g+1}/${a} OK: ${m.text.length} chars`);else{u++;let S=Math.round(v/s*l),x=Math.round(b/s*l),E=`${Math.floor(S/60)}:${(S%60).toString().padStart(2,"0")}`,k=`${Math.floor(x/60)}:${(x%60).toString().padStart(2,"0")}`,j=`
...
*Previous next steps will be archived here.*
`}];for(let b of v){let w=`${l}/${b.name}`;await this.app.vault.create(w,b.content)}s++}catch(g){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,g)}}if(s>0){console.log(`[Eudia] Created ${s} prospect account folders in _Prospects/`);let a=new Set((this.settings.cachedAccounts||[]).map(o=>o.id));for(let o of t)o.id&&!a.has(o.id)&&this.settings.cachedAccounts.push({id:o.id,name:o.name});await this.saveSettings()}return s}async createCSManagerDashboard(t,e){let n="CS Manager",r=new Date().toISOString().split("T")[0],s=ye(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let a={};for(let u of e){let h=u.ownerName||"Unassigned";a[h]||(a[h]=[]),a[h].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
|---------|--------------|--------|
| *None yet* | - | Complete a meeting transcription |
`;else{for(let l of r)o+=`## ${l.account}
...
---
`+S}}await this.app.vault.modify(e,w);let C=Math.floor(t.duration/60);if(new p.Notice(`Transcription complete (${C} min recording)`),!a){let f=y.nextSteps||y.actionItems;console.log(`[Eudia] Next Steps extraction - accountContext: ${n?.accountName||"undefined"}`),console.log(`[Eudia] Next Steps content found: ${f?"YES ("+f.length+" chars)":"NO"}`),console.log(`[Eudia] sections.nextSteps: ${y.nextSteps?"YES":"NO"}, sections.actionItems: ${y.actionItems?"YES":"NO"}`),f&&n?.accountName?(console.log(`[Eudia] Calling updateAccountNextSteps for ${n.accountName}`),await this.updateAccountNextSteps(n.accountName,f,e.path)):console.log("[Eudia] Skipping Next Steps update - missing content or account context")}this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(n){try{let s=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Processing your recording\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,"").replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,""),a=t?._savedAudioPath,o=a?`
Your recording was saved to **${a}** \u2014 you can retry transcription from there.`:"";await this.app.vault.modify(e,s+`

**Transcription failed:** ${n.message}${o}
`)}catch{}throw n}}buildPipelineNoteContent(t,e,n){let r=new Date,s=String(r.getMonth()+1).padStart(2,"0"),a=String(r.getDate()).padStart(2,"0"),o=String(r.getFullYear()).slice(-2),l=r.toISOString().split("T")[0],c=`${s}.${a}.${o}`,d=g=>g==null?"":Array.isArray(g)?g.map(String).join(`
...
\u2022 `));let u="",h=e.match(/"([^"]{20,120})"/);h&&(u=h[1]);let y=`*${r} \u2014 ${a}*
`;o&&(y+=`${o}
`),c&&(y+=`
*Next Steps:*
\u2022 ${c}
...
`;await this.app.vault.modify(t,l)}async updateObjectionNote(t,e){if(!e.objections||e.objections.length===0)return;let n=new Date().toISOString().split("T")[0],r=e.objections.slice(0,10).map(o=>{let l=o.handleRatePercent>=75?"\u2705 Strong":o.handleRatePercent>=50?"\u26A0\uFE0F Moderate":"\u274C Needs Work";return`| ${o.objection?.substring(0,40)||"--"}... | ${o.count||0} | ${o.handleRatePercent||0}% | ${l} |`}).join(`
...
`,s+=`**Best Responses:**
`;for(let l of o.bestResponses.slice(0,2))s+=`1. *"${l.response}"* - ${l.rep||"Team member"}
`;s+=`
`}let a=`---
type: analytics_dashboard
auto_refresh: true
