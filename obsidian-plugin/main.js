...
- Action items have clear owners`}function We(O,a){let t="";return a?.account&&(t=`
ACCOUNT CONTEXT:
...
`}function xe(O,a){let t="";return a?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${a.account.name}
${a.account.owner?`- Owner: ${a.account.owner}`:""}
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let u of r){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,w=>w.toUpperCase()),h=u.split("@")[0].replace("."," ").toLowerCase(),y=h.split(" ")[0],v=h.split(" ").pop()||"",g=e.filter(w=>{let S=(w.csmName||"").toLowerCase();if(S&&(S.includes(y)||S.includes(v)))return!0;let W=(w.ownerName||"").toLowerCase();return W.includes(y)||W.includes(v)}),f=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
account_count: ${g.length}
created: ${s}
---
...
**Email:** ${u}
**CS Accounts:** ${g.length}

---

## Assigned Accounts

`;if(g.length>0){f+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let w of g){let S=w.name.replace(/[<>:"/\\|?*]/g,"_").trim();f+=`| ${w.name} | ${w.type||""} | ${w.ownerName||""} | [[Accounts/${S}/Contacts\\|View]] |
...
`}}await this.app.vault.modify(e,o),console.log("[Eudia] Regenerated All Next Steps dashboard")}catch(t){console.error("[Eudia] Failed to regenerate Next Steps dashboard:",t)}}async startRecording(){if(!E.isSupported()){new p.Notice("Audio transcription is not supported in this browser");return}if(!this.settings.audioSystemDeviceId)try{let n=await E.detectVirtualAudioDevice();n&&(this.settings.audioSystemDeviceId=n.deviceId,await this.saveSettings(),console.log(`[Eudia] Virtual audio device found: ${n.label}`))}catch{}let t=await this.showTemplatePicker();if(!t)return;this.settings.meetingTemplate=t;let e=this.app.workspace.getActiveFile();if(e||(await this.createMeetingNote(),e=this.app.workspace.getActiveFile()),!e){new p.Notice("Please open or create a note first");return}this.audioRecorder=new E,this.recordingStatusBar=new ee(()=>this.audioRecorder?.pause(),()=>this.audioRecorder?.resume(),()=>this.stopRecording(),()=>this.cancelRecording());try{let n=this.settings.audioCaptureMode||"full_call",s={captureMode:n,micDeviceId:this.settings.audioMicDeviceId||void 0,systemAudioDeviceId:this.settings.audioSystemDeviceId||void 0};if(await this.audioRecorder.start(s),n==="full_call"&&this.audioRecorder.getState().isRecording){let c=this.audioRecorder.getSystemAudioMethod();c==="electron"||c==="display_media"?new p.Notice("Recording \u2014 capturing both sides of the call.",5e3):c==="virtual_device"?new p.Notice("Recording (Full Call + Virtual Device) \u2014 both sides captured.",5e3):new p.Notice(`Recording (Mic only) \u2014 headphones block call audio capture.
...
---
`;await this.app.vault.modify(e,c+l),this.processTranscriptionAsync(t,e).catch(d=>{console.error("Background transcription failed:",d),new p.Notice(`Transcription failed: ${d.message}`)})}async processTranscriptionAsync(t,e){try{let n={},s=e.path.split("/");console.log(`[Eudia] Processing transcription for: ${e.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(s)}, accountsFolder: ${this.settings.accountsFolder}`);let r=s[0]==="Pipeline Meetings",i=!1;try{let f=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);f&&(i=/meeting_type:\s*pipeline_review/.test(f[1]))}catch{}if(!i&&r&&(i=!0),i){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let g="";try{let f=await(0,p.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});f.json?.success&&f.json?.context&&(g=f.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${g.length} chars)`))}catch(f){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",f)}n={meetingType:"pipeline_review",pipelineContext:g}}else if(s.length>=2&&s[0]===this.settings.accountsFolder){let g=s[1];console.log(`[Eudia] Detected account folder: ${g}`);let f=this.settings.cachedAccounts.find(C=>C.name.toLowerCase()===g.toLowerCase());f?(n={accountName:f.name,accountId:f.id,userEmail:this.settings.userEmail},console.log(`[Eudia] Found cached account: ${f.name} (${f.id})`)):(n={accountName:g,accountId:"",userEmail:this.settings.userEmail},console.log(`[Eudia] Account not in cache, using folder name: ${g}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let g=await this.calendarService.getCurrentMeeting();g.meeting?.attendees&&(o=g.meeting.attendees.map(f=>f.name||f.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let c=await this.transcriptionService.transcribeAudio(t.audioBlob,{...n,speakerHints:o,captureMode:t.captureMode,hasVirtualDevice:t.hasVirtualDevice,meetingTemplate:this.settings.meetingTemplate||"meddic"}),l=g=>g?!!(g.summary?.trim()||g.nextSteps?.trim()):!1,d=c.sections;if(l(d)||c.text?.trim()&&(d=await this.transcriptionService.processTranscription(c.text,n)),!l(d)&&!c.text?.trim()){let f=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(e,f+`
...
**Transcription in progress`));if(h>0){let g=u.indexOf("---"),f=g>=0?u.indexOf("---",g+3):-1;f>0&&f+3<h&&(m=u.substring(f+3,h).trim())}else{let g=u.indexOf("---"),f=g>=0?u.indexOf("---",g+3):-1;if(f>0){let C=u.substring(f+3).trim();C.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(m=C)}}try{let g="_backups";this.app.vault.getAbstractFileByPath(g)||await this.app.vault.createFolder(g);let f=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),C=`${g}/${e.name}_${f}.md`;await this.app.vault.create(C,u),console.log(`[Eudia] Backed up note to ${C}`)}catch(g){console.warn("[Eudia] Backup failed (non-critical):",g.message)}let y;if(i?y=this.buildPipelineNoteContent(d,c,e.path):y=this.buildNoteContent(d,c),m&&m.length>5){let g=y.indexOf("---",y.indexOf("---")+3);if(g>0){let f=y.substring(0,g+3),C=y.substring(g+3);y=f+`
...
---
`+C}}await this.app.vault.modify(e,y);let v=Math.floor(t.duration/60);if(new p.Notice(`Transcription complete (${v} min recording)`),!i){let g=d.nextSteps||d.actionItems;console.log(`[Eudia] Next Steps extraction - accountContext: ${n?.accountName||"undefined"}`),console.log(`[Eudia] Next Steps content found: ${g?"YES ("+g.length+" chars)":"NO"}`),console.log(`[Eudia] sections.nextSteps: ${d.nextSteps?"YES":"NO"}, sections.actionItems: ${d.actionItems?"YES":"NO"}`),g&&n?.accountName?(console.log(`[Eudia] Calling updateAccountNextSteps for ${n.accountName}`),await this.updateAccountNextSteps(n.accountName,g,e.path)):console.log("[Eudia] Skipping Next Steps update - missing content or account context")}this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(n){try{let r=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(e,r+`
...
`),h}buildNoteContent(t,e){let n=S=>S==null?"":Array.isArray(S)?S.map(W=>typeof W=="object"?W.category?`**${W.category}**: ${W.signal||W.insight||""}`:JSON.stringify(W):String(W)).join(`
`):typeof S=="object"?JSON.stringify(S):String(S),s=n(t.title)||"Meeting Notes",r=n(t.summary),i=n(t.discussionContext),o=n(t.keyQuotes),c=n(t.painPoints),l=n(t.productInterest),d=n(t.meddiccSignals),u=n(t.nextSteps),m=n(t.actionItems),h=n(t.keyDates),y=n(t.dealSignals),v=n(t.risksObjections),g=n(t.attendees||t.keyStakeholders),f=n(t.emailDraft),C=`---
title: "${s}"
...
`;i&&!i.includes("Not discussed")&&(C+=`## Discussion Context
