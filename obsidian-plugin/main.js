...
- Action items have clear owners`}function Ae(g){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${g?`
...
`},{name:"Meeting Notes.md",content:O},{name:"Contacts.md",content:f},{name:"Intelligence.md",content:S},{name:"Next Steps.md",content:C}];for(let E of v){let N=`${p}/${E.name}`;await this.app.vault.create(N,E.content)}return console.log(`[Eudia] Created account folder with subnotes${m?" (enriched)":""}: ${d}`),!0}catch(h){return console.error(`[Eudia] Failed to create folder for ${d}:`,h),!1}},c=5;for(let l=0;l<e.length;l+=c){let d=e.slice(l,l+c),p=await Promise.allSettled(d.map(y=>o(y)));i+=p.filter(y=>y.status==="fulfilled"&&y.value===!0).length}this.settings.cachedAccounts=e.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),i>0&&new u.Notice(`Created ${i} account folders`),await this.ensureNextStepsFolderExists()}buildContactsContent(e,t,n){let r=t?`
...
*Previous next steps will be archived here.*
`}];for(let S of f){let O=`${c}/${S.name}`;await this.app.vault.create(O,S.content)}i++}catch(m){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,m)}}return i>0&&console.log(`[Eudia] Created ${i} prospect account folders in _Prospects/`),i}async createCSManagerDashboard(e,t){let n="CS Manager",r=new Date().toISOString().split("T")[0],i=pe(e);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let a={};for(let p of t){let y=p.ownerName||"Unassigned";a[y]||(a[y]=[]),a[y].push(p)}let o=`---
role: cs_manager
manager: "${e}"
...
`;for(let h of y.slice(0,10))o+=`- **${h.name}** \u2014 ${h.type||"Account"}
...
|---------|------|
`;for(let O of h)m+=`| ${O.name} | ${O.type||""} |
...
`;let f=`${n}/${y}.md`,S=this.app.vault.getAbstractFileByPath(f);S instanceof u.TFile?await this.app.vault.modify(S,m):await this.app.vault.create(f,m)}console.log(`[Eudia] Created CS Manager dashboard for ${e} with ${t.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let r=0,i=0,a=new Date().toISOString().split("T")[0],o=async l=>{let d=l.name.replace(/[<>:"/\\|?*]/g,"_").trim(),p=`${t}/${d}`;if(this.app.vault.getAbstractFileByPath(p)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(p),await this.createExecAccountSubnotes(p,l,a),l.isOwned?r++:i++,console.log(`[Eudia Admin] Created ${l.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(h){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,h),!1}},c=5;for(let l=0;l<e.length;l+=c){let d=e.slice(l,l+c);await Promise.allSettled(d.map(p=>o(p)))}this.settings.cachedAccounts=e.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),r+i>0&&new u.Notice(`Created ${r} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let r=t.ownerName||"Unknown",i=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
`;if(p)h+=p;else{let m=[e.painPoints,e.productInterest,e.nextSteps,e.actionItems].filter(Boolean).map(d).join(`

`);m?h+=m:h+="*Pipeline summary could not be generated. See transcript below.*"}return y&&(h+=`
...
`):typeof f=="object"?JSON.stringify(f):String(f),r=n(e.title)||"Meeting Notes",i=n(e.summary),a=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),l=n(e.nextSteps),d=n(e.actionItems),p=n(e.keyDates),y=n(e.risksObjections),h=n(e.attendees),m=`---
title: "${r}"
...
`),y&&!y.includes("None raised")&&(m+=`## Risks and Objections
