...
- Action items have clear owners`}function je(S){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${S?`
...
`},{name:"Meeting Notes.md",content:f},{name:"Contacts.md",content:v},{name:"Intelligence.md",content:y},{name:"Next Steps.md",content:C}];for(let O of b){let W=`${u}/${O.name}`;await this.app.vault.create(W,O.content)}return console.log(`[Eudia] Created account folder with subnotes${g?" (enriched)":""}: ${d}`),!0}catch(h){return console.error(`[Eudia] Failed to create folder for ${d}:`,h),!1}},c=5;for(let l=0;l<t.length;l+=c){let d=t.slice(l,l+c),u=await Promise.allSettled(d.map(m=>o(m)));s+=u.filter(m=>m.status==="fulfilled"&&m.value===!0).length}s>0?(this.settings.cachedAccounts=t.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),new p.Notice(`Created ${s} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let r=e?`
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let u of s){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,w=>w.toUpperCase()),h=u.split("@")[0].replace("."," ").toLowerCase(),g=h.split(" ")[0],v=h.split(" ").pop()||"",y=e.filter(w=>{let O=(w.csmName||"").toLowerCase();if(O&&(O.includes(g)||O.includes(v)))return!0;let W=(w.ownerName||"").toLowerCase();return W.includes(g)||W.includes(v)}),f=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
...
|---------|------|-------|--------|
`;for(let w of y){let O=w.name.replace(/[<>:"/\\|?*]/g,"_").trim();f+=`| ${w.name} | ${w.type||""} | ${w.ownerName||""} | [[Accounts/${O}/Contacts\\|View]] |
...
**Transcription in progress`));if(h>0){let y=u.indexOf("---"),f=y>=0?u.indexOf("---",y+3):-1;f>0&&f+3<h&&(m=u.substring(f+3,h).trim())}else{let y=u.indexOf("---"),f=y>=0?u.indexOf("---",y+3):-1;if(f>0){let C=u.substring(f+3).trim();C.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(m=C)}}try{let y="_backups";this.app.vault.getAbstractFileByPath(y)||await this.app.vault.createFolder(y);let f=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),C=`${y}/${e.name}_${f}.md`;await this.app.vault.create(C,u),console.log(`[Eudia] Backed up note to ${C}`)}catch(y){console.warn("[Eudia] Backup failed (non-critical):",y.message)}let g;if(a?g=this.buildPipelineNoteContent(d,c,e.path):g=this.buildNoteContent(d,c),m&&m.length>5){let y=g.indexOf("---",g.indexOf("---")+3);if(y>0){let f=g.substring(0,y+3),C=g.substring(y+3);g=f+`
...
`),h}buildNoteContent(t,e){let n=O=>O==null?"":Array.isArray(O)?O.map(W=>typeof W=="object"?W.category?`**${W.category}**: ${W.signal||W.insight||""}`:JSON.stringify(W):String(W)).join(`
`):typeof O=="object"?JSON.stringify(O):String(O),r=n(t.title)||"Meeting Notes",s=n(t.summary),a=n(t.discussionContext),o=n(t.keyQuotes),c=n(t.painPoints),l=n(t.productInterest),d=n(t.meddiccSignals),u=n(t.nextSteps),m=n(t.actionItems),h=n(t.keyDates),g=n(t.dealSignals),v=n(t.risksObjections),y=n(t.attendees||t.keyStakeholders),f=n(t.emailDraft),C=`---
title: "${r}"
