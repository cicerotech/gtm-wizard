...
`;for(let y of m.slice(0,10))o+=`- **${y.name}** \u2014 ${y.type||"Account"}
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let u of s){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,w=>w.toUpperCase()),y=u.split("@")[0].replace("."," ").toLowerCase(),h=y.split(" ")[0],v=y.split(" ").pop()||"",g=e.filter(w=>{let O=(w.csmName||"").toLowerCase();if(O&&(O.includes(h)||O.includes(v)))return!0;let W=(w.ownerName||"").toLowerCase();return W.includes(h)||W.includes(v)}),f=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
...
|---------|------|-------|--------|
`;for(let w of g){let O=w.name.replace(/[<>:"/\\|?*]/g,"_").trim();f+=`| ${w.name} | ${w.type||""} | ${w.ownerName||""} | [[Accounts/${O}/Contacts\\|View]] |
...
`));let u=await this.app.vault.read(a),m="",y=u.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);y&&y[1]&&(m=y[1].trim());let h=`### ${o} - ${l}
${d||"*None*"}`,v=m?`${h}
...
`):typeof h=="object"?JSON.stringify(h,null,2):String(h),u=d(t.summary),m=e.transcript||e.text||"",y=`---
...
`;if(u)y+=u;else{let h=[t.painPoints,t.productInterest,t.nextSteps,t.actionItems].filter(Boolean).map(d).join(`

`);h?y+=h:y+="*Pipeline summary could not be generated. See transcript below.*"}return m&&(y+=`
...
`),y}buildNoteContent(t,e){let n=O=>O==null?"":Array.isArray(O)?O.map(W=>typeof W=="object"?W.category?`**${W.category}**: ${W.signal||W.insight||""}`:JSON.stringify(W):String(W)).join(`
...
`),h&&!h.includes("No significant deal signals")&&!h.includes("Not discussed")&&(C+=`## Deal Signals
