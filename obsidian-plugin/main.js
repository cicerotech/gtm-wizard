var z=Object.create;var $=Object.defineProperty;var K=Object.getOwnPropertyDescriptor;var V=Object.getOwnPropertyNames;var q=Object.getPrototypeOf,Y=Object.prototype.hasOwnProperty;var J=(y,s)=>{for(var e in s)$(y,e,{get:s[e],enumerable:!0})},U=(y,s,e,t)=>{if(s&&typeof s=="object"||typeof s=="function")for(let n of V(s))!Y.call(y,n)&&n!==e&&$(y,n,{get:()=>s[n],enumerable:!(t=K(s,n))||t.enumerable});return y};var B=(y,s,e)=>(e=y!=null?z(q(y)):{},U(s||!y||!y.__esModule?$(e,"default",{value:y,enumerable:!0}):e,y)),Q=y=>U($({},"__esModule",{value:!0}),y);var re={};J(re,{default:()=>F});module.exports=Q(re);var l=require("obsidian");var w=class{constructor(){this.mediaRecorder=null;this.audioChunks=[];this.stream=null;this.startTime=0;this.pausedDuration=0;this.pauseStartTime=0;this.durationInterval=null;this.audioContext=null;this.analyser=null;this.levelInterval=null;this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0};this.stateCallback=null;this.levelHistory=[];this.trackingLevels=!1}onStateChange(s){this.stateCallback=s}getSupportedMimeType(){let s=["audio/webm;codecs=opus","audio/webm","audio/mp4","audio/ogg;codecs=opus","audio/ogg"];for(let e of s)if(MediaRecorder.isTypeSupported(e))return e;return"audio/webm"}async startRecording(){if(this.state.isRecording)throw new Error("Already recording");try{this.stream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0,sampleRate:48e3,channelCount:1}}),this.setupAudioAnalysis();let s=this.getSupportedMimeType();this.mediaRecorder=new MediaRecorder(this.stream,{mimeType:s,audioBitsPerSecond:96e3}),this.audioChunks=[],this.mediaRecorder.ondataavailable=e=>{e.data.size>0&&this.audioChunks.push(e.data)},this.mediaRecorder.start(1e3),this.startTime=Date.now(),this.pausedDuration=0,this.state={isRecording:!0,isPaused:!1,duration:0,audioLevel:0},this.startDurationTracking(),this.startLevelTracking(),this.notifyStateChange()}catch(s){throw this.cleanup(),new Error(`Failed to start recording: ${s.message}`)}}setupAudioAnalysis(){if(this.stream)try{this.audioContext=new AudioContext;let s=this.audioContext.createMediaStreamSource(this.stream);this.analyser=this.audioContext.createAnalyser(),this.analyser.fftSize=256,s.connect(this.analyser)}catch(s){console.warn("Failed to set up audio analysis:",s)}}startDurationTracking(){this.durationInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused){let s=Date.now()-this.startTime-this.pausedDuration;this.state.duration=Math.floor(s/1e3),this.notifyStateChange()}},100)}startLevelTracking(){if(!this.analyser)return;let s=new Uint8Array(this.analyser.frequencyBinCount);this.levelInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused&&this.analyser){this.analyser.getByteFrequencyData(s);let e=0;for(let n=0;n<s.length;n++)e+=s[n];let t=e/s.length;this.state.audioLevel=Math.min(100,Math.round(t/255*100*2)),this.notifyStateChange()}},50)}pauseRecording(){!this.state.isRecording||this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="recording"&&(this.mediaRecorder.pause(),this.pauseStartTime=Date.now(),this.state.isPaused=!0,this.notifyStateChange())}resumeRecording(){!this.state.isRecording||!this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="paused"&&(this.mediaRecorder.resume(),this.pausedDuration+=Date.now()-this.pauseStartTime,this.state.isPaused=!1,this.notifyStateChange())}async stopRecording(){return new Promise((s,e)=>{if(!this.mediaRecorder||!this.state.isRecording){e(new Error("Not currently recording"));return}let t=this.mediaRecorder.mimeType,n=this.state.duration,a=!1,i=setTimeout(()=>{if(!a){a=!0,console.warn("AudioRecorder: onstop timeout, forcing completion");try{let o=new Blob(this.audioChunks,{type:t}),r=new Date,c=r.toISOString().split("T")[0],d=r.toTimeString().split(" ")[0].replace(/:/g,"-"),u=t.includes("webm")?"webm":t.includes("mp4")?"m4a":t.includes("ogg")?"ogg":"webm",g=`recording-${c}-${d}.${u}`;this.cleanup(),s({audioBlob:o,duration:n,mimeType:t,filename:g})}catch{this.cleanup(),e(new Error("Failed to process recording after timeout"))}}},1e4);this.mediaRecorder.onstop=()=>{if(!a){a=!0,clearTimeout(i);try{console.log(`[AudioRecorder] Chunks collected: ${this.audioChunks.length}`);let o=new Blob(this.audioChunks,{type:t});console.log(`[AudioRecorder] Blob size: ${o.size} bytes`);let r=new Date,c=r.toISOString().split("T")[0],d=r.toTimeString().split(" ")[0].replace(/:/g,"-"),u=t.includes("webm")?"webm":t.includes("mp4")?"m4a":t.includes("ogg")?"ogg":"webm",g=`recording-${c}-${d}.${u}`;this.cleanup(),s({audioBlob:o,duration:n,mimeType:t,filename:g})}catch(o){this.cleanup(),e(o)}}},this.mediaRecorder.onerror=o=>{a||(a=!0,clearTimeout(i),this.cleanup(),e(new Error("Recording error occurred")))},this.mediaRecorder.state==="recording"&&this.mediaRecorder.requestData(),setTimeout(()=>{this.mediaRecorder&&this.mediaRecorder.state!=="inactive"&&this.mediaRecorder.stop()},100)})}cancelRecording(){this.cleanup()}cleanup(){this.durationInterval&&(clearInterval(this.durationInterval),this.durationInterval=null),this.levelInterval&&(clearInterval(this.levelInterval),this.levelInterval=null),this.audioContext&&(this.audioContext.close().catch(()=>{}),this.audioContext=null,this.analyser=null),this.stream&&(this.stream.getTracks().forEach(s=>s.stop()),this.stream=null),this.mediaRecorder=null,this.audioChunks=[],this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0},this.notifyStateChange()}getState(){return{...this.state}}static isSupported(){return!!(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia&&window.MediaRecorder)}notifyStateChange(){this.stateCallback&&this.stateCallback({...this.state})}static formatDuration(s){let e=Math.floor(s/60),t=s%60;return`${e.toString().padStart(2,"0")}:${t.toString().padStart(2,"0")}`}static async blobToBase64(s){return new Promise((e,t)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];e(i)},n.onerror=t,n.readAsDataURL(s)})}static async blobToArrayBuffer(s){return s.arrayBuffer()}async start(){return this.startRecording()}async stop(){return this.stopRecording()}pause(){return this.pauseRecording()}resume(){return this.resumeRecording()}cancel(){return this.cancelRecording()}isRecording(){return this.state.isRecording}startLevelHistoryTracking(){this.levelHistory=[],this.trackingLevels=!0}recordLevelSample(){this.trackingLevels&&this.levelHistory.push(this.state.audioLevel)}getAudioDiagnostic(){if(this.levelHistory.length===0)return{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:100,warning:"Unable to analyze audio levels - recording may be too short"};let s=this.levelHistory.reduce((i,o)=>i+o,0)/this.levelHistory.length,e=Math.max(...this.levelHistory),t=this.levelHistory.filter(i=>i<5).length,n=Math.round(t/this.levelHistory.length*100),a=null;return e<5?a="SILENT AUDIO: No audio was detected during recording. Check your microphone settings and ensure Obsidian has microphone permission.":s<10&&n>80?a="VERY LOW AUDIO: Audio levels were extremely low. The transcription may not be accurate. Check your microphone or move closer to it.":n>90&&(a="MOSTLY SILENT: Over 90% of the recording had no audio. Make sure you're capturing the meeting audio, not just silence."),{hasAudio:e>=5,averageLevel:Math.round(s),peakLevel:e,silentPercent:n,warning:a}}static async analyzeAudioBlob(s){try{let e=new AudioContext,t=await s.arrayBuffer(),n;try{n=await e.decodeAudioData(t)}catch{return await e.close(),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:"Could not analyze audio format. Proceeding with transcription."}}let a=n.getChannelData(0),i=0,o=0,r=0,c=.01,d=100,u=0;for(let v=0;v<a.length;v+=d){let A=Math.abs(a[v]);i+=A,A>o&&(o=A),A<c&&r++,u++}await e.close();let g=i/u,p=Math.round(r/u*100),m=Math.round(g*100*10),h=Math.round(o*100),f=null;return o<.01?f='SILENT AUDIO DETECTED: The recording appears to contain only silence. This typically causes Whisper to hallucinate random text like "Yes. Yes. Yes." Check your audio input source.':g<.005&&p>95?f="NEAR-SILENT AUDIO: The recording is almost entirely silent. The transcription will likely be inaccurate.":p>90&&(f="MOSTLY SILENT: Over 90% of the recording is silent. Consider checking your audio setup."),{hasAudio:o>=.01,averageLevel:m,peakLevel:h,silentPercent:p,warning:f}}catch(e){return console.error("Audio analysis failed:",e),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:null}}}};var E=require("obsidian");var j=class{constructor(){this.salesforceAccounts=[]}setAccounts(s){this.salesforceAccounts=s}detectAccount(s,e,t){if(s){let n=this.detectFromTitle(s);if(n.confidence>=70)return n}if(t){let n=this.detectFromFilePath(t);if(n.confidence>=70)return n}if(e&&e.length>0){let n=this.detectFromAttendees(e);if(n.confidence>=50)return n}return{account:null,accountId:null,confidence:0,source:"none",evidence:"No account detected from available context"}}detectFromTitle(s){if(!s)return{account:null,accountId:null,confidence:0,source:"title",evidence:"No title"};let e=[{regex:/^([A-Za-z0-9][^-–—]+?)\s*[-–—]\s*(?:[A-Z][a-z]+|[A-Za-z]{2,})/,confidence:85},{regex:/(?:call|meeting|sync|check-in|demo|discovery)\s+(?:with|re:?|@)\s+([^-–—]+?)(?:\s*[-–—]|$)/i,confidence:80},{regex:/^([A-Za-z][^-–—]+?)\s+(?:discovery|demo|review|kickoff|intro|onboarding|sync)\s*(?:call)?$/i,confidence:75},{regex:/^([^:]+?):\s+/i,confidence:70},{regex:/^\[([^\]]+)\]/,confidence:75}],t=["weekly","daily","monthly","internal","team","1:1","one on one","standup","sync","meeting","call","notes","monday","tuesday","wednesday","thursday","friday","untitled","new","test"];for(let n of e){let a=s.match(n.regex);if(a&&a[1]){let i=a[1].trim();if(t.some(r=>i.toLowerCase()===r)||i.length<2)continue;let o=this.fuzzyMatchSalesforce(i);return o?{account:o.name,accountId:o.id,confidence:Math.min(n.confidence+10,100),source:"salesforce_match",evidence:`Matched "${i}" from title to Salesforce account "${o.name}"`}:{account:i,accountId:null,confidence:n.confidence,source:"title",evidence:"Extracted from meeting title pattern"}}}return{account:null,accountId:null,confidence:0,source:"title",evidence:"No pattern matched"}}detectFromFilePath(s){let e=s.match(/Accounts\/([^\/]+)\//i);if(e&&e[1]){let t=e[1].trim(),n=this.fuzzyMatchSalesforce(t);return n?{account:n.name,accountId:n.id,confidence:95,source:"salesforce_match",evidence:`File in account folder "${t}" matched to "${n.name}"`}:{account:t,accountId:null,confidence:85,source:"title",evidence:`File located in Accounts/${t} folder`}}return{account:null,accountId:null,confidence:0,source:"none",evidence:"Not in Accounts folder"}}detectFromAttendees(s){let e=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com"],t=new Set;for(let o of s){let c=o.toLowerCase().match(/@([a-z0-9.-]+)/);if(c){let d=c[1];!d.includes("eudia.com")&&!e.includes(d)&&t.add(d)}}if(t.size===0)return{account:null,accountId:null,confidence:0,source:"attendee_domain",evidence:"No external domains"};for(let o of t){let r=o.split(".")[0],c=r.charAt(0).toUpperCase()+r.slice(1),d=this.fuzzyMatchSalesforce(c);if(d)return{account:d.name,accountId:d.id,confidence:75,source:"salesforce_match",evidence:`Matched attendee domain ${o} to "${d.name}"`}}let n=Array.from(t)[0],a=n.split(".")[0];return{account:a.charAt(0).toUpperCase()+a.slice(1),accountId:null,confidence:50,source:"attendee_domain",evidence:`Guessed from external attendee domain: ${n}`}}fuzzyMatchSalesforce(s){if(!s||this.salesforceAccounts.length===0)return null;let e=s.toLowerCase().trim();for(let t of this.salesforceAccounts)if(t.name?.toLowerCase()===e)return t;for(let t of this.salesforceAccounts)if(t.name?.toLowerCase().startsWith(e))return t;for(let t of this.salesforceAccounts)if(t.name?.toLowerCase().includes(e))return t;for(let t of this.salesforceAccounts)if(e.includes(t.name?.toLowerCase()))return t;return null}suggestAccounts(s,e=10){if(!s||s.length<2)return this.salesforceAccounts.slice(0,e).map(a=>({...a,score:0}));let t=s.toLowerCase(),n=[];for(let a of this.salesforceAccounts){let i=a.name?.toLowerCase()||"",o=0;i===t?o=100:i.startsWith(t)?o=90:i.includes(t)?o=70:t.includes(i)&&(o=50),o>0&&n.push({...a,score:o})}return n.sort((a,i)=>i.score-a.score).slice(0,e)}},de=new j;function X(y,s){let e="";return(s?.account||s?.opportunities?.length)&&(e=`
...
- Action items have clear owners`}var N=class{constructor(s){this.serverUrl=s}setServerUrl(s){this.serverUrl=s}async transcribeAndSummarize(s,e,t,n,a){try{let i=await(0,E.requestUrl)({url:`${this.serverUrl}/api/transcribe-and-summarize`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({audio:s,mimeType:e,accountName:t,accountId:n,context:a?{customerBrain:a.account?.customerBrain,opportunities:a.opportunities,contacts:a.contacts}:void 0,systemPrompt:X(t,a)})});return i.json.success?{success:!0,transcript:i.json.transcript||"",sections:this.normalizeSections(i.json.sections),duration:i.json.duration||0}:{success:!1,transcript:"",sections:this.getEmptySections(),duration:0,error:i.json.error||"Transcription failed"}}catch(i){console.error("Server transcription error:",i),i.response&&console.error("Server response:",i.response);let o="";try{i.response?.json?.error?o=i.response.json.error:typeof i.response=="string"&&(o=JSON.parse(i.response).error||"")}catch{}let r=o||`Transcription failed: ${i.message}`;return i.message?.includes("413")?r="Audio file too large for server. Try a shorter recording.":i.message?.includes("500")?r=o||"Server error during transcription. Please try again.":(i.message?.includes("Failed to fetch")||i.message?.includes("NetworkError"))&&(r="Could not reach transcription server. Check your internet connection."),console.error("Final error message:",r),{success:!1,transcript:"",sections:this.getEmptySections(),duration:0,error:r}}}parseSections(s){let e=this.getEmptySections(),t={summary:"summary",attendees:"attendees","meddicc signals":"meddiccSignals","product interest":"productInterest","pain points":"painPoints","buying triggers":"buyingTriggers","key dates":"keyDates","next steps":"nextSteps","action items":"actionItems","action items (internal)":"actionItems","deal signals":"dealSignals","risks & objections":"risksObjections","risks and objections":"risksObjections","competitive intelligence":"competitiveIntel"},n=/## ([^\n]+)\n([\s\S]*?)(?=## |$)/g,a;for(;(a=n.exec(s))!==null;){let i=a[1].trim().toLowerCase(),o=a[2].trim(),r=t[i];r&&(e[r]=o)}return e}normalizeSections(s){let e=this.getEmptySections();return s?{...e,...s}:e}async getMeetingContext(s){try{let e=await(0,E.requestUrl)({url:`${this.serverUrl}/api/meeting-context/${s}`,method:"GET",headers:{Accept:"application/json"}});return e.json.success?{success:!0,account:e.json.account,opportunities:e.json.opportunities,contacts:e.json.contacts,lastMeeting:e.json.lastMeeting}:{success:!1,error:e.json.error||"Failed to fetch context"}}catch(e){return console.error("Meeting context error:",e),{success:!1,error:e.message||"Network error"}}}async syncToSalesforce(s,e,t,n,a,i){try{let o=await(0,E.requestUrl)({url:`${this.serverUrl}/api/transcription/sync-to-salesforce`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:s,accountName:e,noteTitle:t,sections:n,transcript:a,meetingDate:i||new Date().toISOString(),syncedAt:new Date().toISOString()})});return o.json.success?{success:!0,customerBrainUpdated:o.json.customerBrainUpdated,eventCreated:o.json.eventCreated,eventId:o.json.eventId,contactsCreated:o.json.contactsCreated,tasksCreated:o.json.tasksCreated}:{success:!1,error:o.json.error||"Sync failed"}}catch(o){return console.error("Salesforce sync error:",o),{success:!1,error:o.message||"Network error"}}}getEmptySections(){return{summary:"",attendees:"",meddiccSignals:"",productInterest:"",painPoints:"",buyingTriggers:"",keyDates:"",nextSteps:"",actionItems:"",dealSignals:"",risksObjections:"",competitiveIntel:""}}static formatSectionsForNote(s,e,t){let n="";if(s.summary&&(n+=`## TL;DR

${s.summary}

`),t?.enabled&&t?.talkTime){n+=`## Call Analytics

`;let i=t.talkTime.repPercent,o=t.talkTime.customerPercent,r=t.talkTime.isHealthyRatio?"\u2705":"\u26A0\uFE0F";n+=`**Talk Time:** Rep ${i}% / Customer ${o}% ${r}
`;let c=Math.round(i/5),d=Math.round(o/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
`,n+=`\`${"\u2588".repeat(d)}${"\u2591".repeat(20-d)}\` Customer

`,t.coaching){let u=t.coaching;if(u.totalQuestions>0){let g=Math.round(u.openQuestions/u.totalQuestions*100);n+=`**Questions:** ${u.totalQuestions} total (${u.openQuestions} open, ${u.closedQuestions} closed - ${g}% open)
`}if(u.objections&&u.objections.length>0){let g=u.objections.filter(p=>p.handled).length;n+=`**Objections:** ${u.objections.length} raised, ${g} handled
...
`).replace(/\n/g,"<br>")}onClose(){this.contentEl.empty()}};var O=class extends l.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new T(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return b}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}this.plugin.settings.accountsImported&&(this.steps[2].status="complete")}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),n=t.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to unlock your sales superpowers",cls:"eudia-setup-subtitle"});let a=t.createDiv({cls:"eudia-setup-progress-section"}),i=this.getCompletionPercentage(),o=a.createDiv({cls:"eudia-setup-progress-label"});o.createSpan({text:"Setup Progress"}),o.createSpan({text:`${i}%`,cls:"eudia-setup-progress-value"});let c=a.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${i}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=n.createDiv({cls:"eudia-setup-step-header"}),i=a.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"1"),t.status==="complete"&&i.addClass("eudia-step-complete");let o=a.createDiv({cls:"eudia-setup-step-info"});o.createEl("h3",{text:t.title}),o.createEl("p",{text:t.description});let r=n.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")r.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=r.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let d=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});d.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async u=>{u.key==="Enter"&&await this.handleCalendarConnect()},r.createDiv({cls:"eudia-setup-validation-message"}),r.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let n=await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new l.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");try{let a;I(e)?(console.log("[Eudia] Admin user detected - importing all accounts"),a=await this.accountOwnershipService.getAllAccountsForAdmin(e)):a=await this.accountOwnershipService.getAccountsForUser(e),a.length>0&&(I(e)?await this.plugin.createAdminAccountFolders(a):await this.plugin.createTailoredAccountFolders(a),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,await this.plugin.saveSettings(),new l.Notice(`Imported ${a.length} account folders!`))}catch(a){console.error("[Eudia] Account import failed:",a)}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=n.createDiv({cls:"eudia-setup-step-header"}),i=a.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"2"),t.status==="complete"&&i.addClass("eudia-step-complete");let o=a.createDiv({cls:"eudia-setup-step-info"});o.createEl("h3",{text:t.title}),o.createEl("p",{text:t.description});let r=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){r.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")r.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&r.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let d=r.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),u=r.createDiv({cls:"eudia-setup-sf-status"});d.onclick=async()=>{let g=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(g,"_blank"),u.textContent="Complete the login in the popup window...",u.className="eudia-setup-sf-status loading",new l.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(u)},r.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,n=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new l.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=this.plugin.settings.userEmail,n;if(I(t)?(console.log("[Eudia] Admin user detected - importing all accounts"),e.textContent="Admin detected - importing all accounts...",n=await this.accountOwnershipService.getAllAccountsForAdmin(t)):n=await this.accountOwnershipService.getAccountsForUser(t),n.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}I(t)?await this.plugin.createAdminAccountFolders(n):await this.plugin.createTailoredAccountFolders(n),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=n.length,await this.plugin.saveSettings(),this.steps[2].status="complete";let a=n.filter(o=>o.isOwned!==!1).length,i=n.filter(o=>o.isOwned===!1).length;I(t)&&i>0?e.textContent=`${a} owned + ${i} view-only accounts imported!`:e.textContent=`${n.length} accounts imported successfully!`,e.className="eudia-setup-sf-status success"}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error"}}renderTranscribeStep(e){let t=this.steps[2],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=n.createDiv({cls:"eudia-setup-step-header"}),i=a.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"3"),t.status==="complete"&&i.addClass("eudia-step-complete");let o=a.createDiv({cls:"eudia-setup-step-info"});o.createEl("h3",{text:t.title}),o.createEl("p",{text:t.description});let r=n.createDiv({cls:"eudia-setup-step-content"}),c=r.createDiv({cls:"eudia-setup-instructions"}),d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),d.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let u=c.createDiv({cls:"eudia-setup-instruction"});u.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),u.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let g=c.createDiv({cls:"eudia-setup-instruction"});g.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),g.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&r.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(i=>i.status==="complete")){let i=t.createDiv({cls:"eudia-setup-completion"});i.createEl("h2",{text:"\u{1F389} You're all set!"}),i.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let o=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});o.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(b),await this.plugin.activateCalendarView()}}else{let i=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});i.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(b),new l.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let a=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});a.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},_=class extends l.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return x}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),n=t.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let a=n.createDiv({cls:"eudia-calendar-actions"}),i=a.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});i.title="Refresh",i.onclick=async()=>{i.addClass("spinning"),await this.render(),i.removeClass("spinning")};let o=a.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});o.title="Settings",o.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let r=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(r)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,a=this.plugin.settings.userEmail;try{(await(0,l.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(a&&t.server==="connected")try{let u=await(0,l.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(a)}`,method:"GET",throw:!1});u.status===200&&u.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(a&&t.server==="connected")try{let u=await(0,l.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(a)}`,method:"GET",throw:!1});u.status===200&&u.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let i=e.createDiv({cls:"eudia-status-indicators"}),o=i.createSpan({cls:`eudia-status-dot ${t.server}`});o.title=t.serverMessage||"Server";let r=i.createSpan({cls:`eudia-status-dot ${t.calendar}`});r.title=t.calendarMessage||"Calendar";let c=i.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(c.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let u=e.createDiv({cls:"eudia-status-warning"});u.innerHTML=`<strong>${a}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),n=t.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let a=new S(this.plugin.settings.serverUrl,this.plugin.settings.userEmail),i=await a.getWeekMeetings();if(n.remove(),!i.success){this.renderError(t,i.error||"Failed to load calendar");return}let o=Object.keys(i.byDay||{}).sort();if(o.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,a);for(let r of o){let c=i.byDay[r];!c||c.length===0||this.renderDaySection(t,r,c)}}catch(a){n.remove(),this.renderError(t,a.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let n=await t.getCurrentMeeting();if(n.meeting){let a=e.createDiv({cls:"eudia-now-card"});n.isNow?a.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):a.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let i=a.createDiv({cls:"eudia-now-content"});i.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&i.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let o=a.createEl("button",{cls:"eudia-now-action",text:"Create Note"});o.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(e,t,n){let a=e.createDiv({cls:"eudia-calendar-day"});a.createEl("div",{cls:"eudia-calendar-day-header",text:S.getDayName(t)});for(let i of n){let o=a.createDiv({cls:`eudia-calendar-meeting ${i.isCustomerMeeting?"customer":"internal"}`});o.createEl("div",{cls:"eudia-calendar-time",text:S.formatTime(i.start,this.plugin.settings.timezone)});let r=o.createDiv({cls:"eudia-calendar-details"});if(r.createEl("div",{cls:"eudia-calendar-subject",text:i.subject}),i.accountName)r.createEl("div",{cls:"eudia-calendar-account",text:i.accountName});else if(i.attendees&&i.attendees.length>0){let c=i.attendees.slice(0,2).map(d=>d.name||d.email?.split("@")[0]||"Unknown").join(", ");r.createEl("div",{cls:"eudia-calendar-attendees",text:c})}o.onclick=()=>this.createNoteForMeeting(i),o.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `}renderError(e,t){let n=e.createDiv({cls:"eudia-calendar-error"}),a="",i="Unable to load calendar",o="";t.includes("not authorized")||t.includes("403")?(a="\u{1F512}",i="Calendar Access Required",o="Contact your admin to be added to the authorized users list."):t.includes("network")||t.includes("fetch")?(a="\u{1F4E1}",i="Connection Issue",o="Check your internet connection and try again."):(t.includes("server")||t.includes("500"))&&(a="\u{1F527}",i="Server Unavailable",o="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
      <div class="eudia-error-icon">${a}</div>
      <p class="eudia-error-title">${i}</p>
      <p class="eudia-error-message">${t}</p>
      ${o?`<p class="eudia-error-action">${o}</p>`:""}
    `;let r=n.createEl("button",{cls:"eudia-btn-retry",text:"Try Again"});r.onclick=()=>this.render()}renderSetupPanel(e){let t=e.createDiv({cls:"eudia-calendar-setup-panel"});t.innerHTML=`
    ...
    `;let n=t.createDiv({cls:"eudia-setup-input-group"}),a=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});a.addClass("eudia-setup-email");let i=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),o=t.createDiv({cls:"eudia-setup-status"});i.onclick=async()=>{let r=a.value.trim().toLowerCase();if(!r||!r.endsWith("@eudia.com")){o.textContent="Please use your @eudia.com email",o.className="eudia-setup-status error";return}i.disabled=!0,i.textContent="Connecting...",o.textContent="";try{if(!(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${r}`,method:"GET"})).json?.authorized){o.innerHTML=`<strong>${r}</strong> is not authorized. Contact your admin to be added.`,o.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect";return}this.plugin.settings.userEmail=r,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),o.textContent="Connected",o.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let d=c.message||"Connection failed";d.includes("403")?o.innerHTML=`<strong>${r}</strong> is not authorized for calendar access.`:o.textContent=d,o.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect"}},a.onkeydown=r=>{r.key==="Enter"&&i.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractAccountFromAttendees(e){if(!e||e.length===0)return null;let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com"],n=[];for(let r of e){if(!r.email)continue;let d=r.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(d){let u=d[1];!u.includes("eudia.com")&&!t.includes(u)&&n.push(u)}}if(n.length===0)return null;let a=n[0],i=a.split(".")[0],o=i.charAt(0).toUpperCase()+i.slice(1);return console.log(`[Eudia Calendar] Extracted company "${o}" from attendee domain ${a}`),o}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let a=(t[1]||t[2]||"").trim();if(a.toLowerCase()!=="eudia")return a}let n=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let i=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(i.toLowerCase()!=="eudia"&&i.length>0)return i}if(!e.toLowerCase().includes("eudia")){let a=e.match(/^([^-–|]+)/);if(a){let i=a[1].trim();if(i.length>2&&i.length<50)return i}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(t);if(!(n instanceof l.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let a=e.toLowerCase().trim(),i=[];for(let g of n.children)g instanceof l.TFolder&&i.push(g.name);console.log(`[Eudia Calendar] Searching for "${a}" in ${i.length} folders`);let o=i.find(g=>g.toLowerCase()===a);if(o)return console.log(`[Eudia Calendar] Exact match found: ${o}`),`${t}/${o}`;let r=i.find(g=>g.toLowerCase().startsWith(a));if(r)return console.log(`[Eudia Calendar] Folder starts with match: ${r}`),`${t}/${r}`;let c=i.find(g=>a.startsWith(g.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${t}/${c}`;let d=i.find(g=>{let p=g.toLowerCase();return p.length>=3&&a.includes(p)});if(d)return console.log(`[Eudia Calendar] Search contains folder match: ${d}`),`${t}/${d}`;let u=i.find(g=>{let p=g.toLowerCase();return a.length>=3&&p.includes(a)});return u?(console.log(`[Eudia Calendar] Folder contains search match: ${u}`),`${t}/${u}`):(console.log(`[Eudia Calendar] No folder match found for "${a}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],n=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),a=`${t} - ${n}.md`,i=null,o=e.accountName||null,r=null;if(console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(p=>p.email)||[])}`),!i&&e.attendees&&e.attendees.length>0){let p=this.extractAccountFromAttendees(e.attendees);console.log(`[Eudia Calendar] Extracted domain company name: "${p||"none"}"`),p&&(i=this.findAccountFolder(p),console.log(`[Eudia Calendar] Domain-based "${p}" -> folder: ${i||"not found"}`),i&&!o&&(o=i.split("/").pop()||p))}if(!i&&e.accountName&&(i=this.findAccountFolder(e.accountName),console.log(`[Eudia Calendar] Server accountName "${e.accountName}" -> folder: ${i||"not found"}`)),!i){let p=this.extractAccountFromSubject(e.subject);p&&(i=this.findAccountFolder(p),console.log(`[Eudia Calendar] Subject-based "${p}" -> folder: ${i||"not found"}`),i&&!o&&(o=i.split("/").pop()||p))}if(!i){let p=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(p)instanceof l.TFolder&&(i=p,console.log(`[Eudia Calendar] No match found, using Accounts root: ${i}`))}if(o){let p=this.plugin.settings.cachedAccounts.find(m=>m.name.toLowerCase()===o?.toLowerCase());p&&(r=p.id,o=p.name,console.log(`[Eudia Calendar] Matched to cached account: ${p.name} (${p.id})`))}let c=i?`${i}/${a}`:a,d=this.app.vault.getAbstractFileByPath(c);if(d instanceof l.TFile){await this.app.workspace.getLeaf().openFile(d),new l.Notice(`Opened existing note: ${a}`);return}let u=(e.attendees||[]).map(p=>p.name||p.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),g=`---
title: "${e.subject}"
date: ${t}
attendees: [${u}]
account: "${o||""}"
...
## Attendees
${(e.attendees||[]).map(p=>`- ${p.name||p.email}`).join(`
`)}
...
`;try{let p=await this.app.vault.create(c,g);await this.app.workspace.getLeaf().openFile(p),new l.Notice(`Created: ${c}`)}catch(p){console.error("[Eudia Calendar] Failed to create note:",p),new l.Notice(`Could not create note: ${p.message||"Unknown error"}`)}}},F=class extends l.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null}async onload(){await this.loadSettings(),this.transcriptionService=new N(this.settings.serverUrl),this.calendarService=new S(this.settings.serverUrl,this.settings.userEmail),this.smartTagService=new D,this.registerView(x,e=>new _(e,this)),this.registerView(b,e=>new O(e,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.addCommand({id:"transcribe-meeting",name:"Transcribe Meeting",callback:async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}}),this.addCommand({id:"open-calendar",name:"Open Calendar",callback:()=>this.activateCalendarView()}),this.addCommand({id:"sync-accounts",name:"Sync Salesforce Accounts",callback:()=>this.syncAccounts()}),this.addCommand({id:"sync-note",name:"Sync Note to Salesforce",callback:()=>this.syncNoteToSalesforce()}),this.addCommand({id:"new-meeting-note",name:"New Meeting Note",callback:()=>this.createMeetingNote()}),this.addCommand({id:"ask-gtm-brain",name:"Ask gtm-brain",callback:()=>this.openIntelligenceQueryForCurrentNote()}),this.addCommand({id:"refresh-analytics",name:"Refresh Analytics Dashboard",callback:async()=>{let e=this.app.workspace.getActiveFile();e?await this.refreshAnalyticsDashboard(e):new l.Notice("No active file")}}),this.addSettingTab(new H(this.app,this)),this.registerEditorSuggest(new L(this.app,this)),this.app.workspace.onLayoutReady(async()=>{if(this.settings.setupCompleted){if(this.settings.syncOnStartup){if(await this.scanLocalAccountFolders(),this.settings.userEmail&&this.settings.syncAccountsOnStartup){let e=new Date().toISOString().split("T")[0];this.settings.lastAccountRefreshDate!==e&&setTimeout(async()=>{try{console.log("[Eudia] Startup account sync - checking for changes...");let n=await this.syncAccountFolders();if(n.success){if(this.settings.lastAccountRefreshDate=e,await this.saveSettings(),n.added>0||n.archived>0){let a=[];n.added>0&&a.push(`${n.added} added`),n.archived>0&&a.push(`${n.archived} archived`),new l.Notice(`Account folders synced: ${a.join(", ")}`)}}else console.log("[Eudia] Sync failed:",n.error)}catch{console.log("[Eudia] Startup sync skipped (server unreachable), will retry tomorrow")}},2e3)}this.settings.showCalendarView&&this.settings.userEmail&&await this.activateCalendarView(),this.settings.userEmail&&this.telemetry&&setTimeout(async()=>{try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),t=0;e&&e instanceof l.TFolder&&(t=e.children.filter(i=>i instanceof l.TFolder&&!i.name.startsWith("_")).length);let n={salesforce:this.settings.salesforceConnected?"connected":"not_configured",calendar:this.settings.calendarConfigured?"connected":"not_configured"};await this.telemetry.sendHeartbeat(t,n);let a=await this.telemetry.checkForPushedConfig();if(a.length>0){let i=!1;for(let o of a)o.key&&this.settings.hasOwnProperty(o.key)&&(this.settings[o.key]=o.value,i=!0,console.log(`[Eudia] Applied pushed config: ${o.key} = ${o.value}`));i&&(await this.saveSettings(),new l.Notice("Settings updated by admin"))}}catch{console.log("[Eudia] Heartbeat/config check skipped")}},3e3)}}else{await new Promise(t=>setTimeout(t,100));let e=document.querySelector(".modal-container .modal");if(e){let t=e.querySelector(".modal-close-button");t&&t.click()}await this.activateSetupView()}this.app.workspace.on("file-open",async e=>{if(e&&(e.path.includes("_Analytics/")||e.path.includes("_Customer Health/")))try{let t=await this.app.vault.read(e);if(t.includes("type: analytics_dashboard")){let a=t.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/)?.[1],i=new Date().toISOString().split("T")[0];a!==i&&(console.log(`[Eudia] Auto-refreshing analytics: ${e.name}`),await this.refreshAnalyticsDashboard(e))}}catch{}})})}async onunload(){this.app.workspace.detachLeavesOfType(x)}async loadSettings(){this.settings=Object.assign({},oe,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async activateCalendarView(){let e=this.app.workspace,t=e.getLeavesOfType(x);if(t.length>0)e.revealLeaf(t[0]);else{let n=e.getRightLeaf(!1);n&&(await n.setViewState({type:x,active:!0}),e.revealLeaf(n))}}async activateSetupView(){let e=this.app.workspace,t=e.getLeavesOfType(b);if(t.length>0)e.revealLeaf(t[0]);else{let n=e.getLeaf(!0);n&&(await n.setViewState({type:b,active:!0}),e.revealLeaf(n))}}async createTailoredAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t);let a=0;for(let i of e){let o=i.name.replace(/[<>:"/\\|?*]/g,"_").trim(),r=`${t}/${o}`;if(this.app.vault.getAbstractFileByPath(r)instanceof l.TFolder){console.log(`[Eudia] Account folder already exists: ${o}`);continue}try{await this.app.vault.createFolder(r);let d=new Date().toISOString().split("T")[0],u=[{name:"Note 1.md",content:`---
account: "${i.name}"
account_id: "${i.id}"
type: meeting_note
sync_to_salesforce: false
...
`}];for(let g of u){let p=`${r}/${g.name}`;await this.app.vault.create(p,g.content)}a++,console.log(`[Eudia] Created account folder with subnotes: ${o}`)}catch(d){console.error(`[Eudia] Failed to create folder for ${o}:`,d)}}this.settings.cachedAccounts=e.map(i=>({id:i.id,name:i.name})),await this.saveSettings(),a>0&&new l.Notice(`Created ${a} account folders`),await this.ensureNextStepsFolderExists()}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let a=0,i=0,o=new Date().toISOString().split("T")[0];for(let r of e){let c=r.name.replace(/[<>:"/\\|?*]/g,"_").trim(),d=`${t}/${c}`;if(!(this.app.vault.getAbstractFileByPath(d)instanceof l.TFolder))try{await this.app.vault.createFolder(d),r.isOwned?(await this.createFullAccountSubnotes(d,r,o),a++):(await this.createViewOnlyAccountNote(d,r,o),i++),console.log(`[Eudia Admin] Created ${r.isOwned?"owned":"view-only"} folder: ${c}`)}catch(g){console.error(`[Eudia Admin] Failed to create folder for ${c}:`,g)}}this.settings.cachedAccounts=e.map(r=>({id:r.id,name:r.name})),await this.saveSettings(),a+i>0&&new l.Notice(`Created ${a} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createViewOnlyAccountNote(e,t,n){let a=t.ownerName||"Unknown",i=`---
account: "${t.name}"
account_id: "${t.id}"
type: view_only
owner: "${a}"
...
`,o=`${e}/_Account Info.md`;await this.app.vault.create(o,i)}async createFullAccountSubnotes(e,t,n){let a=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let i of a){let o=`${e}/${i.name}`;await this.app.vault.create(o,i.content)}}async ensurePipelineFolderExists(){let e="Pipeline",t=`${e}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let o=`---
...
### New Opportunities
*None yet*
`;await this.app.vault.create(t,o)}}async ensureNextStepsFolderExists(){let e="Next Steps",t=`${e}/All Next Steps.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let i=new Date().toISOString().split("T")[0],o=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),r=`---
...
`;await this.app.vault.create(t,r)}}async updateAccountNextSteps(e,t,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${e}`),console.log(`[Eudia] Content length: ${t?.length||0} chars`);let a=e.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${this.settings.accountsFolder}/${a}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${i}`);let o=this.app.vault.getAbstractFileByPath(i);if(!o||!(o instanceof l.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${i}`);let A=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${a}`);A&&A instanceof l.TFolder?console.log(`[Eudia] Files in ${a} folder:`,A.children.map(k=>k.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${a}`);return}console.log("[Eudia] Found Next Steps file, updating...");let r=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),d=n.split("/").pop()?.replace(".md","")||"Meeting",u=t;!t.includes("- [ ]")&&!t.includes("- [x]")&&(u=t.split(`
`).filter(A=>A.trim()).map(A=>{let k=A.replace(/^[-•*]\s*/,"").trim();return k?`- [ ] ${k}`:""}).filter(Boolean).join(`
`));let g=await this.app.vault.read(o),p="",m=g.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);m&&m[1]&&(p=m[1].trim());let h=`### ${r} - ${d}
${u||"*None*"}`,f=p?`${h}
...
${f}
`;await this.app.vault.modify(o,v),console.log(`[Eudia] Updated Next Steps for ${e} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(a){console.error(`[Eudia] Failed to update Next Steps for ${e}:`,a)}}async regenerateNextStepsDashboard(){try{let t=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!t||!(t instanceof l.TFile)){await this.ensureNextStepsFolderExists();return}let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!n||!(n instanceof l.TFolder))return;let a=[];for(let c of n.children)if(c instanceof l.TFolder){let d=`${c.path}/Next Steps.md`,u=this.app.vault.getAbstractFileByPath(d);if(u instanceof l.TFile){let g=await this.app.vault.read(u),p=g.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),m=p?p[1]:"Unknown",h=g.split(`
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(h.length>0||m!=="Unknown")&&a.push({account:c.name,lastUpdated:m,nextSteps:h})}}a.sort((c,d)=>d.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],o=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),r=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`;if(a.length===0)r+=`## Your Next Steps
...
|---------|--------------|--------|
| *None yet* | - | Complete a meeting transcription |
`;else{for(let c of a)r+=`## ${c.account}

`,c.nextSteps.length>0?r+=c.nextSteps.join(`
`)+`
`:r+=`*No current next steps*
`,r+=`
...
---
`;await this.app.vault.modify(t,o+r),this.processTranscriptionAsync(e,t).catch(c=>{console.error("Background transcription failed:",c),new l.Notice(`Transcription failed: ${c.message}`)})}async processTranscriptionAsync(e,t){try{let n,a=t.path.split("/");if(console.log(`[Eudia] Processing transcription for: ${t.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(a)}, accountsFolder: ${this.settings.accountsFolder}`),a.length>=2&&a[0]===this.settings.accountsFolder){let p=a[1];console.log(`[Eudia] Detected account folder: ${p}`);let m=this.settings.cachedAccounts.find(h=>h.name.toLowerCase()===p.toLowerCase());m?(n={accountName:m.name,accountId:m.id},console.log(`[Eudia] Found cached account: ${m.name} (${m.id})`)):(n={accountName:p,accountId:""},console.log(`[Eudia] Account not in cache, using folder name: ${p}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let i=[];try{let p=await this.calendarService.getCurrentMeeting();p.meeting?.attendees&&(i=p.meeting.attendees.map(m=>m.name||m.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let o=await this.transcriptionService.transcribeAudio(e.audioBlob,n?{...n,speakerHints:i}:{speakerHints:i}),r=p=>p?!!(p.summary?.trim()||p.nextSteps?.trim()):!1,c=o.sections;if(r(c)||o.text?.trim()&&(c=await this.transcriptionService.processTranscription(o.text,n)),!r(c)&&!o.text?.trim()){let m=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,m+`

**Transcription failed:** No audio detected.
`),new l.Notice("Transcription failed: No audio detected.");return}let d=this.buildNoteContent(c,o);await this.app.vault.modify(t,d);let u=Math.floor(e.duration/60);new l.Notice(`Transcription complete (${u} min recording)`);let g=c.nextSteps||c.actionItems;console.log(`[Eudia] Next Steps extraction - accountContext: ${n?.accountName||"undefined"}`),console.log(`[Eudia] Next Steps content found: ${g?"YES ("+g.length+" chars)":"NO"}`),console.log(`[Eudia] sections.nextSteps: ${c.nextSteps?"YES":"NO"}, sections.actionItems: ${c.actionItems?"YES":"NO"}`),g&&n?.accountName?(console.log(`[Eudia] Calling updateAccountNextSteps for ${n.accountName}`),await this.updateAccountNextSteps(n.accountName,g,t.path)):console.log("[Eudia] Skipping Next Steps update - missing content or account context"),this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(n){try{let i=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,i+`

**Transcription failed:** ${n.message}
`)}catch{}throw n}}buildNoteContent(e,t){let n=f=>f==null?"":Array.isArray(f)?f.map(v=>typeof v=="object"?v.category?`**${v.category}**: ${v.signal||v.insight||""}`:JSON.stringify(v):String(v)).join(`
`):typeof f=="object"?JSON.stringify(f):String(f),a=n(e.title)||"Meeting Notes",i=n(e.summary),o=n(e.painPoints),r=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),u=n(e.actionItems),g=n(e.keyDates),p=n(e.risksObjections),m=n(e.attendees),h=`---
title: "${a}"
...
`;return o&&!o.includes("None explicitly")&&(h+=`## Pain Points

${o}

`),r&&!r.includes("None identified")&&(h+=`## Product Interest
...
`),g&&!g.includes("No specific dates")&&(h+=`## Key Dates

${g}

`),p&&!p.includes("None raised")&&(h+=`## Risks and Objections
...
${t.text}
`),h}openIntelligenceQuery(){new M(this.app,this).open()}openIntelligenceQueryForCurrentNote(){let e=this.app.workspace.getActiveFile(),t;if(e){let n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(n?.account_id&&n?.account)t={id:n.account_id,name:n.account};else if(n?.account){let a=this.settings.cachedAccounts.find(i=>i.name.toLowerCase()===n.account.toLowerCase());a?t={id:a.id,name:a.name}:t={id:"",name:n.account}}else{let a=e.path.split("/");if(a.length>=2&&a[0]===this.settings.accountsFolder){let i=a[1],o=this.settings.cachedAccounts.find(r=>r.name.replace(/[<>:"/\\|?*]/g,"_").trim()===i);o?t={id:o.id,name:o.name}:t={id:"",name:i}}}}new M(this.app,this,t).open()}async syncAccounts(e=!1){e||new l.Notice("Syncing Salesforce accounts...");try{let n=(await(0,l.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!n.success||!n.accounts){e||new l.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=n.accounts.map(a=>({id:a.id,name:a.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),e||new l.Notice(`Synced ${n.accounts.length} accounts for matching`)}catch(t){e||new l.Notice(`Failed to sync accounts: ${t.message}`)}}async scanLocalAccountFolders(){try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!e||!(e instanceof l.TFolder))return;let t=[];for(let n of e.children)n instanceof l.TFolder&&t.push({id:`local-${n.name.replace(/\s+/g,"-").toLowerCase()}`,name:n.name});this.settings.cachedAccounts=t,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(e){console.error("Failed to scan local account folders:",e)}}async refreshAccountFolders(){if(!this.settings.userEmail)throw new Error("Please configure your email first");let e=new T(this.settings.serverUrl);if((await e.getAccountsForUser(this.settings.userEmail)).length===0)return console.log("[Eudia] No accounts found for user"),0;let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),a=[];if(n&&n instanceof l.TFolder)for(let o of n.children)o instanceof l.TFolder&&a.push(o.name);let i=await e.getNewAccounts(this.settings.userEmail,a);return i.length===0?(console.log("[Eudia] All account folders exist"),0):(console.log(`[Eudia] Creating ${i.length} new account folders`),await this.createTailoredAccountFolders(i),i.length)}async syncAccountFolders(){if(!this.settings.userEmail)return{success:!1,added:0,archived:0,error:"No email configured"};let e=this.settings.userEmail.toLowerCase().trim();console.log(`[Eudia] Syncing account folders for: ${e}`);try{let t=await fetch(`${this.settings.serverUrl}/api/bl-accounts/${encodeURIComponent(e)}`);if(!t.ok){let f=await t.json().catch(()=>({}));throw new Error(f.error||`Server returned ${t.status}`)}let n=await t.json();if(!n.success||!n.accounts)throw new Error(n.error||"Invalid response from server");let a=n.meta?.userGroup||"bl",i=n.meta?.queryDescription||"accounts",o=n.meta?.region||null;console.log(`[Eudia] User group: ${a}, accounts: ${n.accounts.length} (${i})`),o&&console.log(`[Eudia] Sales Leader region: ${o}`);let r=n.accounts,c=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),d=new Map;if(c&&c instanceof l.TFolder)for(let f of c.children)f instanceof l.TFolder&&!f.name.startsWith("_")&&d.set(f.name.toLowerCase().trim(),f);let u=new Set(r.map(f=>f.name.toLowerCase().trim())),g=r.filter(f=>{let v=f.name.toLowerCase().trim();return!d.has(v)}),p=[];if(a==="bl")for(let[f,v]of d.entries())u.has(f)||p.push(v);let m=0,h=0;if(g.length>0){console.log(`[Eudia] Creating ${g.length} new account folders for ${a}`);let f=g.map(v=>({id:v.id,name:v.name,type:v.customerType,isOwned:a==="bl",ownerName:v.ownerName}));a==="admin"||a==="exec"?await this.createAdminAccountFolders(f):await this.createTailoredAccountFolders(f),m=g.length,this.telemetry&&this.telemetry.reportInfo("Accounts synced - added",{count:m,userGroup:a,region:o||void 0})}return this.settings.archiveRemovedAccounts&&p.length>0&&(console.log(`[Eudia] Archiving ${p.length} removed account folders`),h=await this.archiveAccountFolders(p),this.telemetry&&this.telemetry.reportInfo("Accounts synced - archived",{count:h})),console.log(`[Eudia] Sync complete: ${m} added, ${h} archived (group: ${a})`),{success:!0,added:m,archived:h,userGroup:a}}catch(t){return console.error("[Eudia] Account sync error:",t),this.telemetry&&this.telemetry.reportError("Account sync failed",{error:t.message}),{success:!1,added:0,archived:0,error:t.message}}}async archiveAccountFolders(e){let t=0,n=`${this.settings.accountsFolder}/_Archived`;this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);for(let i of e)try{let o=`${n}/${i.name}`;if(this.app.vault.getAbstractFileByPath(o)){let u=new Date().toISOString().split("T")[0];await this.app.fileManager.renameFile(i,`${n}/${i.name}_${u}`)}else await this.app.fileManager.renameFile(i,o);let c=`${n}/${i.name}/_archived.md`,d=`---
...
`;try{await this.app.vault.create(c,d)}catch{}t++,console.log(`[Eudia] Archived: ${i.name}`)}catch(o){console.error(`[Eudia] Failed to archive ${i.name}:`,o)}return t}async syncNoteToSalesforce(){let e=this.app.workspace.getActiveFile();if(!e){new l.Notice("No active file to sync");return}let t=await this.app.vault.read(e),n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(!n?.sync_to_salesforce){new l.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}let a=n.account_id,i=n.account;if(!a&&i){let o=this.settings.cachedAccounts.find(r=>r.name.toLowerCase()===i.toLowerCase());o&&(a=o.id)}if(!a){let o=e.path.split("/");if(o.length>=2&&o[0]===this.settings.accountsFolder){let r=o[1],c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===r);c&&(a=c.id,i=c.name)}}if(!a){new l.Notice("Could not determine account for this note");return}try{new l.Notice("Syncing to Salesforce...");let o=await(0,l.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:a,accountName:i,noteTitle:e.basename,notePath:e.path,content:t,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});o.json?.success?new l.Notice("Synced to Salesforce"):new l.Notice("Failed to sync: "+(o.json?.error||"Unknown error"))}catch(o){new l.Notice(`Sync failed: ${o.message}`)}}async createMeetingNote(){return new Promise(e=>{new W(this.app,this,async n=>{if(!n){e();return}let a=new Date().toISOString().split("T")[0],i=n.name.replace(/[<>:"/\\|?*]/g,"_").trim(),o=`${this.settings.accountsFolder}/${i}`,r=`${a} Meeting.md`,c=`${o}/${r}`;this.app.vault.getAbstractFileByPath(o)||await this.app.vault.createFolder(o);let d=`---
...
`,u=await this.app.vault.create(c,d);await this.app.workspace.getLeaf().openFile(u),new l.Notice(`Created meeting note for ${n.name}`),e()}).open()})}async fetchAndInsertContext(){new l.Notice("Fetching pre-call context...")}async refreshAnalyticsDashboard(e){if(!this.settings.userEmail){console.log("[Eudia] Cannot refresh analytics - no email configured");return}let n=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);if(!n)return;let a=n[1];if(!a.includes("type: analytics_dashboard"))return;let i=a.match(/category:\s*(\w+)/)?.[1]||"team";console.log(`[Eudia] Refreshing analytics dashboard: ${e.name} (${i})`);try{let o=null,r=this.settings.serverUrl,c=encodeURIComponent(this.settings.userEmail);switch(i){case"pain_points":o=(await(0,l.requestUrl)({url:`${r}/api/analytics/pain-points?days=30`,method:"GET"})).json,o.success&&await this.updatePainPointNote(e,o.painPoints);break;case"objections":o=(await(0,l.requestUrl)({url:`${r}/api/analytics/objection-playbook?days=90`,method:"GET"})).json,o.success&&await this.updateObjectionNote(e,o);break;case"coaching":case"team":default:o=(await(0,l.requestUrl)({url:`${r}/api/analytics/team-trends?managerId=${c}`,method:"GET"})).json,o.success&&await this.updateTeamPerformanceNote(e,o.trends);break}o?.success&&new l.Notice(`Analytics refreshed: ${e.name}`)}catch(o){console.error("[Eudia] Analytics refresh error:",o)}}async updatePainPointNote(e,t){if(!t||t.length===0)return;let n=new Date().toISOString().split("T")[0],a=t.slice(0,10).map(d=>`| ${d.painPoint||"--"} | ${d.count||0} | ${d.category||"--"} | ${d.averageSeverity||"medium"} |`).join(`
`),i={};for(let d of t){let u=d.category||"other";i[u]||(i[u]=[]),i[u].push(d)}let o="";for(let[d,u]of Object.entries(i)){o+=`
### ${d.charAt(0).toUpperCase()+d.slice(1)}
`;for(let g of u.slice(0,3))o+=`- ${g.painPoint}
`}let r=t.filter(d=>d.exampleQuotes&&d.exampleQuotes.length>0).slice(0,5).map(d=>`> "${d.exampleQuotes[0]}" - on ${d.painPoint}`).join(`
...
${r||"*No quotes available*"}

---

> **Tip:** Use these pain points to prepare for customer calls.
`;await this.app.vault.modify(e,c)}async updateObjectionNote(e,t){if(!t.objections||t.objections.length===0)return;let n=new Date().toISOString().split("T")[0],a=t.objections.slice(0,10).map(r=>{let c=r.handleRatePercent>=75?"\u2705 Strong":r.handleRatePercent>=50?"\u26A0\uFE0F Moderate":"\u274C Needs Work";return`| ${r.objection?.substring(0,40)||"--"}... | ${r.count||0} | ${r.handleRatePercent||0}% | ${c} |`}).join(`
`),i="";for(let r of t.objections.slice(0,5))if(r.bestResponses&&r.bestResponses.length>0){i+=`
### Objection: "${r.objection?.substring(0,50)}..."

`,i+=`**Frequency:** ${r.count} times  
`,i+=`**Handle Rate:** ${r.handleRatePercent}%

`,i+=`**Best Responses:**
`;for(let c of r.bestResponses.slice(0,2))i+=`1. *"${c.response}"* - ${c.rep||"Team member"}
`;i+=`
`}let o=`---
type: analytics_dashboard
auto_refresh: true
...
`;await this.app.vault.modify(e,o)}async updateTeamPerformanceNote(e,t){if(!t)return;let n=new Date().toISOString().split("T")[0],a=o=>o>0?`\u2191 ${Math.abs(o).toFixed(1)}%`:o<0?`\u2193 ${Math.abs(o).toFixed(1)}%`:"--",i=`---
type: analytics_dashboard
auto_refresh: true
...
${t.topPainPoints?.slice(0,5).map(o=>`- **${o.painPoint}** (${o.count} mentions)`).join(`
...
${t.trendingTopics?.slice(0,8).map(o=>`- ${o.topic} (${o.count})`).join(`
...
${t.topObjections?.slice(0,5).map(o=>`- ${o.objection} - ${o.handleRatePercent}% handled`).join(`
...
`;await this.app.vault.modify(e,i)}},H=class extends l.PluginSettingTab{constructor(s,e){super(s,e),this.plugin=e}display(){let{containerEl:s}=this;s.empty(),s.createEl("h2",{text:"Eudia Sync & Scribe"}),s.createEl("h3",{text:"Your Profile"});let e=s.createDiv();e.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let t=e.createDiv();t.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let n=t.createSpan(),a=t.createSpan(),i=e.createDiv();i.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",i.setText("Connect with Salesforce to sync notes with your user attribution.");let o=e.createEl("button");o.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let r=null,c=async()=>{if(!this.plugin.settings.userEmail)return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",a.setText("Enter email above first"),o.setText("Setup Required"),o.disabled=!0,o.style.opacity="0.5",o.style.cursor="not-allowed",!1;o.disabled=!1,o.style.opacity="1",o.style.cursor="pointer";try{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",a.setText("Checking..."),(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",a.setText("Connected to Salesforce"),o.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",a.setText("Not connected"),o.setText("Connect to Salesforce"),!1)}catch{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",a.setText("Status unavailable"),o.setText("Connect to Salesforce"),!1}};new l.Setting(s).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(m=>m.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async h=>{let f=h.trim().toLowerCase();this.plugin.settings.userEmail=f,await this.plugin.saveSettings(),await c()})),new l.Setting(s).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(m=>{G.forEach(h=>{m.addOption(h.value,h.label)}),m.setValue(this.plugin.settings.timezone),m.onChange(async h=>{this.plugin.settings.timezone=h,await this.plugin.saveSettings(),new l.Notice(`Timezone set to ${G.find(f=>f.value===h)?.label||h}`)})}),s.createEl("h3",{text:"Salesforce Connection"}),s.appendChild(e);let d=()=>{r&&window.clearInterval(r);let m=0,h=30;r=window.setInterval(async()=>{m++,await c()?(r&&(window.clearInterval(r),r=null),new l.Notice("Salesforce connected successfully!")):m>=h&&r&&(window.clearInterval(r),r=null)},5e3)};o.onclick=async()=>{if(!this.plugin.settings.userEmail){new l.Notice("Please enter your email first");return}let m=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(m,"_blank"),new l.Notice("Complete the Salesforce login in the popup window",5e3),d()},c(),s.createEl("h3",{text:"Server"}),new l.Setting(s).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(m=>m.setValue(this.plugin.settings.serverUrl).onChange(async h=>{this.plugin.settings.serverUrl=h,await this.plugin.saveSettings()}));let u=s.createDiv({cls:"settings-advanced-collapsed"}),g=u.createDiv({cls:"eudia-transcription-status"});g.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",g.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?g.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':g.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{g.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let p=new l.Setting(s).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(m=>m.setValue(!1).onChange(h=>{u.style.display=h?"block":"none"}));u.style.display="none",s.createEl("h3",{text:"Transcription"}),new l.Setting(s).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(m=>m.setValue(this.plugin.settings.saveAudioFiles).onChange(async h=>{this.plugin.settings.saveAudioFiles=h,await this.plugin.saveSettings()})),new l.Setting(s).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(m=>m.setValue(this.plugin.settings.appendTranscript).onChange(async h=>{this.plugin.settings.appendTranscript=h,await this.plugin.saveSettings()})),s.createEl("h3",{text:"Sync"}),new l.Setting(s).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(m=>m.setValue(this.plugin.settings.syncOnStartup).onChange(async h=>{this.plugin.settings.syncOnStartup=h,await this.plugin.saveSettings()})),new l.Setting(s).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(m=>m.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async h=>{this.plugin.settings.autoSyncAfterTranscription=h,await this.plugin.saveSettings()})),s.createEl("h3",{text:"Folders"}),new l.Setting(s).setName("Accounts Folder").setDesc("Where account folders are stored").addText(m=>m.setValue(this.plugin.settings.accountsFolder).onChange(async h=>{this.plugin.settings.accountsFolder=h||"Accounts",await this.plugin.saveSettings()})),new l.Setting(s).setName("Recordings Folder").setDesc("Where audio files are saved").addText(m=>m.setValue(this.plugin.settings.recordingsFolder).onChange(async h=>{this.plugin.settings.recordingsFolder=h||"Recordings",await this.plugin.saveSettings()})),s.createEl("h3",{text:"Actions"}),new l.Setting(s).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(m=>m.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new l.Setting(s).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(m=>m.setButtonText("Refresh Folders").onClick(async()=>{m.setButtonText("Checking..."),m.setDisabled(!0);try{let h=await this.plugin.refreshAccountFolders();h>0?new l.Notice(`Created ${h} new account folder${h>1?"s":""}`):new l.Notice("All account folders are up to date")}catch(h){new l.Notice("Failed to refresh folders: "+h.message)}m.setButtonText("Refresh Folders"),m.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&s.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),s.createEl("p",{text:`Audio transcription: ${w.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
