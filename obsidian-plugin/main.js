...
`).filter(w=>w.match(/^- \[[ x]\]/)).slice(0,5);(y.length>0||m!=="Unknown")&&a.push({account:l.name,lastUpdated:m,nextSteps:y})}}a.sort((l,c)=>c.lastUpdated.localeCompare(l.lastUpdated));let i=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`),new p.Notice(`Transcription failed: ${n.message}`,1e4)}}catch{}}}buildPipelineNoteContent(t,e,n){let a=new Date,i=String(a.getMonth()+1).padStart(2,"0"),s=String(a.getDate()).padStart(2,"0"),o=String(a.getFullYear()).slice(-2),l=a.toISOString().split("T")[0],c=`${i}.${s}.${o}`,d=y=>y==null?"":Array.isArray(y)?y.map(String).join(`
