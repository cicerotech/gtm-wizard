...
- Action items have clear owners`}var E=class{constructor(t,e){this.openaiApiKey=null;this.serverUrl=t,this.openaiApiKey=e||null}setServerUrl(t){this.serverUrl=t}setOpenAIKey(t){this.openaiApiKey=t}async transcribeAndSummarize(t,e,n,s,i){try{let r=await(0,b.requestUrl)({url:`${this.serverUrl}/api/transcribe-and-summarize`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({audio:t,mimeType:e,accountName:n,accountId:s,openaiApiKey:this.openaiApiKey,context:i?{customerBrain:i.account?.customerBrain,opportunities:i.opportunities,contacts:i.contacts}:void 0,systemPrompt:L(n,i)})});return r.json.success?{success:!0,transcript:r.json.transcript||"",sections:this.normalizeSections(r.json.sections),duration:r.json.duration||0}:{success:!1,transcript:"",sections:this.getEmptySections(),duration:0,error:r.json.error||"Transcription failed"}}catch(r){console.error("Server transcription error:",r),r.response&&console.error("Server response:",r.response);let a="";try{r.response?.json?.error?a=r.response.json.error:typeof r.response=="string"&&(a=JSON.parse(r.response).error||"")}catch{}let o=a||`Transcription failed: ${r.message}`;return r.message?.includes("413")?o="Audio file too large for server. Try a shorter recording.":r.message?.includes("500")?o=a||"Server error during transcription. Please try again.":(r.message?.includes("Failed to fetch")||r.message?.includes("NetworkError"))&&(o="Could not reach transcription server. Check your internet connection."),console.error("Final error message:",o),{success:!1,transcript:"",sections:this.getEmptySections(),duration:0,error:o}}}async transcribeLocal(t,e,n,s){if(!this.openaiApiKey)return{success:!1,transcript:"",sections:this.getEmptySections(),duration:0,error:"No OpenAI API key configured. Add it in plugin settings."};try{let i=atob(t),r=new Uint8Array(i.length);for(let f=0;f<i.length;f++)r[f]=i.charCodeAt(f);let a=new Blob([r],{type:e}),o=new FormData,u=e.includes("webm")?"webm":e.includes("mp4")?"m4a":"ogg";o.append("file",a,`audio.${u}`),o.append("model","whisper-1"),o.append("response_format","verbose_json"),o.append("language","en");let g=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{Authorization:`Bearer ${this.openaiApiKey}`},body:o});if(!g.ok){let f=await g.text();throw new Error(`Whisper API error: ${g.status} - ${f}`)}let d=await g.json(),p=d.text||"",l=d.duration||0,h=await this.summarizeLocal(p,n,s);return{success:!0,transcript:p,sections:h,duration:l}}catch(i){return console.error("Local transcription error:",i),{success:!1,transcript:"",sections:this.getEmptySections(),duration:0,error:i.message||"Local transcription failed"}}}async summarizeLocal(t,e,n){if(!this.openaiApiKey)return this.getEmptySections();try{let s=L(e,n),i=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${this.openaiApiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-4o",messages:[{role:"system",content:s},{role:"user",content:`Analyze this meeting transcript:
...
## Attendees
${(e.attendees||[]).map(l=>`- ${l.name||l.email}`).join(`
`)}
...
---
`;await this.app.vault.modify(n,a+o),this.processTranscriptionAsync(e,n).catch(u=>{console.error("Background transcription failed:",u),new c.Notice(`Transcription failed: ${u.message}`)})}async processTranscriptionAsync(e,n){try{let s,i=n.path.split("/");if(i.length>=2&&i[0]===this.settings.accountsFolder){let p=i[1],l=this.settings.cachedAccounts.find(h=>h.name.toLowerCase()===p.toLowerCase());l&&(s={accountName:l.name,accountId:l.id})}let r=[];try{let p=await this.calendarService.getCurrentMeeting();p.meeting?.attendees&&(r=p.meeting.attendees.map(l=>l.name||l.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let a=await this.transcriptionService.transcribeAudio(e.audioBlob,s?{...s,speakerHints:r}:{speakerHints:r}),o=p=>p?!!(p.summary?.trim()||p.nextSteps?.trim()):!1,u=a.sections;if(o(u)||a.text?.trim()&&(u=await this.transcriptionService.processTranscription(a.text,s)),!o(u)&&!a.text?.trim()){let l=(await this.app.vault.read(n)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(n,l+`

**Transcription failed:** No audio detected.
`),new c.Notice("Transcription failed: No audio detected.");return}let g=this.buildNoteContent(u,a);await this.app.vault.modify(n,g);let d=Math.floor(e.duration/60);new c.Notice(`Transcription complete (${d} min recording)`),this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(s){try{let r=(await this.app.vault.read(n)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(n,r+`

**Transcription failed:** ${s.message}
`)}catch{}throw s}}buildNoteContent(e,n){let s=y=>y==null?"":Array.isArray(y)?y.map(v=>typeof v=="object"?v.category?`**${v.category}**: ${v.signal||v.insight||""}`:JSON.stringify(v):String(v)).join(`
`):typeof y=="object"?JSON.stringify(y):String(y),i=s(e.title)||"Meeting Notes",r=s(e.summary),a=s(e.painPoints),o=s(e.productInterest),u=s(e.meddiccSignals),g=s(e.nextSteps),d=s(e.actionItems),p=s(e.keyDates),l=s(e.risksObjections),h=s(e.attendees),f=`---
title: "${i}"
...
`),l&&!l.includes("None raised")&&(f+=`## Risks and Objections
...
`,d=await this.app.vault.create(u,g);await this.app.workspace.getLeaf().openFile(d),new c.Notice(`Created meeting note for ${s.name}`),e()}).open()})}async fetchAndInsertContext(){new c.Notice("Fetching pre-call context...")}},M=class extends c.PluginSettingTab{constructor(t,e){super(t,e),this.plugin=e}display(){let{containerEl:t}=this;t.empty(),t.createEl("h2",{text:"Eudia Sync & Scribe"}),t.createEl("h3",{text:"Your Profile"}),new c.Setting(t).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(d=>d.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async p=>{this.plugin.settings.userEmail=p.trim().toLowerCase(),await this.plugin.saveSettings()})),t.createEl("h3",{text:"Salesforce Connection"});let e=t.createDiv();e.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px;";let n=e.createDiv();n.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let s=n.createSpan(),i=n.createSpan(),r=e.createDiv();r.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",r.setText("Connect with Salesforce to sync notes with your user attribution.");let a=e.createEl("button");a.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,u=async()=>{if(!this.plugin.settings.userEmail)return s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",i.setText("Enter email first"),a.setText("Setup Required"),a.disabled=!0,!1;try{return(await(0,c.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",i.setText("Connected to Salesforce"),a.setText("Reconnect"),a.disabled=!1,!0):(s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",i.setText("Not connected"),a.setText("Connect to Salesforce"),a.disabled=!1,!1)}catch{return s.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",i.setText("Status unavailable"),a.setText("Connect to Salesforce"),a.disabled=!1,!1}},g=()=>{o&&window.clearInterval(o);let d=0,p=30;o=window.setInterval(async()=>{d++,await u()?(o&&(window.clearInterval(o),o=null),new c.Notice("Salesforce connected successfully!")):d>=p&&o&&(window.clearInterval(o),o=null)},5e3)};a.onclick=async()=>{if(!this.plugin.settings.userEmail){new c.Notice("Please enter your email first");return}let d=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(d,"_blank"),new c.Notice("Complete the Salesforce login in the popup window",5e3),g()},u(),t.createEl("h3",{text:"Server"}),new c.Setting(t).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(d=>d.setValue(this.plugin.settings.serverUrl).onChange(async p=>{this.plugin.settings.serverUrl=p,await this.plugin.saveSettings()})),new c.Setting(t).setName("OpenAI API Key").setDesc("For transcription (optional if server provides)").addText(d=>{d.setPlaceholder("sk-...").setValue(this.plugin.settings.openaiApiKey).onChange(async p=>{this.plugin.settings.openaiApiKey=p,await this.plugin.saveSettings()}),d.inputEl.type="password"}),t.createEl("h3",{text:"Transcription"}),new c.Setting(t).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(d=>d.setValue(this.plugin.settings.saveAudioFiles).onChange(async p=>{this.plugin.settings.saveAudioFiles=p,await this.plugin.saveSettings()})),new c.Setting(t).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(d=>d.setValue(this.plugin.settings.appendTranscript).onChange(async p=>{this.plugin.settings.appendTranscript=p,await this.plugin.saveSettings()})),t.createEl("h3",{text:"Sync"}),new c.Setting(t).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(d=>d.setValue(this.plugin.settings.syncOnStartup).onChange(async p=>{this.plugin.settings.syncOnStartup=p,await this.plugin.saveSettings()})),new c.Setting(t).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(d=>d.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async p=>{this.plugin.settings.autoSyncAfterTranscription=p,await this.plugin.saveSettings()})),t.createEl("h3",{text:"Folders"}),new c.Setting(t).setName("Accounts Folder").setDesc("Where account folders are stored").addText(d=>d.setValue(this.plugin.settings.accountsFolder).onChange(async p=>{this.plugin.settings.accountsFolder=p||"Accounts",await this.plugin.saveSettings()})),new c.Setting(t).setName("Recordings Folder").setDesc("Where audio files are saved").addText(d=>d.setValue(this.plugin.settings.recordingsFolder).onChange(async p=>{this.plugin.settings.recordingsFolder=p||"Recordings",await this.plugin.saveSettings()})),t.createEl("h3",{text:"Actions"}),new c.Setting(t).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching (folders are pre-loaded)`).addButton(d=>d.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),this.plugin.settings.lastSyncTime&&t.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),t.createEl("p",{text:`Audio transcription: ${w.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
