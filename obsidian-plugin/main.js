...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof u.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let p of i){let y=p.split("@")[0].replace("."," ").replace(/\b\w/g,b=>b.toUpperCase()),m=p.split("@")[0].replace("."," ").toLowerCase(),h=m.split(" ")[0],g=m.split(" ").pop()||"",w=t.filter(b=>{let A=(b.csmName||"").toLowerCase();if(A&&(A.includes(h)||A.includes(g)))return!0;let j=(b.ownerName||"").toLowerCase();return j.includes(h)||j.includes(g)}),O=`---
rep: "${p}"
rep_name: "${y}"
role: cs_rep_summary
...
|---------|------|-------|--------|
`;for(let b of w){let A=b.name.replace(/[<>:"/\\|?*]/g,"_").trim();O+=`| ${b.name} | ${b.type||""} | ${b.ownerName||""} | [[Accounts/${A}/Contacts\\|View]] |
...
`):typeof g=="object"?JSON.stringify(g):String(g),r=n(e.title)||"Meeting Notes",i=n(e.summary),a=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),l=n(e.nextSteps),d=n(e.actionItems),p=n(e.keyDates),y=n(e.risksObjections),m=n(e.attendees),h=`---
title: "${r}"
