...
`;let c=Math.round(r/5),d=Math.round(s/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
...
`);let a=t?.enabled&&t?.formattedTranscript?t.formattedTranscript:e;if(a){let r=t?.enabled?"Full Transcript (Speaker-Attributed)":"Full Transcript";n+=`---
...
`}return n}static formatSectionsWithAudio(i,e,t,n){let a=this.formatSectionsForNote(i,e,n);return t&&(a+=`
---

## Recording

![[${t}]]
`),a}static formatContextForNote(i){if(!i.success)return"";let e=`## Pre-Call Context
...
`,e}async blobToBase64(i){return new Promise((e,t)=>{let n=new FileReader;n.onload=()=>{let r=n.result.split(",")[1];e(r)},n.onerror=t,n.readAsDataURL(i)})}async transcribeAudio(i,e){try{let t=await this.blobToBase64(i),n=i.type||"audio/webm",a=e?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:e.pipelineContext}:void 0,r=await this.transcribeAndSummarize(t,n,e?.accountName,e?.accountId,a);return{text:r.transcript,confidence:r.success?.95:0,duration:r.duration,sections:r.sections}}catch(t){return console.error("transcribeAudio error:",t),{text:"",confidence:0,duration:0,sections:this.getEmptySections()}}}async processTranscription(i,e){if(!i||i.trim().length===0)return this.getEmptySections();try{let t=await(0,T.requestUrl)({url:`${this.serverUrl}/api/process-sections`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:i,accountName:e?.accountName,context:e})});if(t.json?.success&&t.json?.sections){let n=t.json.sections;return{summary:n.summary||"",painPoints:n.painPoints||n.keyPoints||"",productInterest:n.productInterest||"",meddiccSignals:n.meddiccSignals||"",nextSteps:n.nextSteps||"",actionItems:n.actionItems||"",keyDates:n.keyDates||"",buyingTriggers:n.buyingTriggers||"",dealSignals:n.dealSignals||"",risksObjections:n.risksObjections||n.concerns||"",competitiveIntel:n.competitiveIntel||"",attendees:n.attendees||"",transcript:i}}return console.warn("Server process-sections returned no sections, using fallback"),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:i}}catch(t){return console.error("processTranscription server error:",t),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:i}}}};var U=require("obsidian"),I=class y{constructor(i,e,t="America/New_York"){this.serverUrl=i,this.userEmail=e.toLowerCase(),this.timezone=t}setUserEmail(i){this.userEmail=i.toLowerCase()}setServerUrl(i){this.serverUrl=i}setTimezone(i){this.timezone=i}async getTodaysMeetings(i=!1){if(!this.userEmail)return{success:!1,date:new Date().toISOString().split("T")[0],email:"",meetingCount:0,meetings:[],error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=i?"&forceRefresh=true":"";return(await(0,U.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/today?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch today's meetings:",e),{success:!1,date:new Date().toISOString().split("T")[0],email:this.userEmail,meetingCount:0,meetings:[],error:e.message||"Failed to fetch calendar"}}}async getWeekMeetings(i=!1){if(!this.userEmail)return{success:!1,startDate:"",endDate:"",email:"",totalMeetings:0,byDay:{},error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=i?"&forceRefresh=true":"";return(await(0,U.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/week?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch week's meetings:",e),{success:!1,startDate:"",endDate:"",email:this.userEmail,totalMeetings:0,byDay:{},error:e.message||"Failed to fetch calendar"}}}async getMeetingsInRange(i,e){if(!this.userEmail)return[];try{let t=i.toISOString().split("T")[0],n=e.toISOString().split("T")[0],a=await(0,U.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/range?start=${t}&end=${n}`,method:"GET",headers:{Accept:"application/json"}});return a.json.success?a.json.meetings||[]:[]}catch(t){return console.error("Failed to fetch calendar range:",t),[]}}async getCurrentMeeting(){let i=await this.getTodaysMeetings();if(!i.success||i.meetings.length===0)return{meeting:null,isNow:!1};let e=new Date;for(let t of i.meetings){let n=y.safeParseDate(t.start),a=y.safeParseDate(t.end);if(e>=n&&e<=a)return{meeting:t,isNow:!0};let r=(n.getTime()-e.getTime())/(1e3*60);if(r>0&&r<=15)return{meeting:t,isNow:!1,minutesUntilStart:Math.ceil(r)}}return{meeting:null,isNow:!1}}async getMeetingsForAccount(i){let e=await this.getWeekMeetings();if(!e.success)return[];let t=[];Object.values(e.byDay).forEach(a=>{t.push(...a)});let n=i.toLowerCase();return t.filter(a=>a.accountName?.toLowerCase().includes(n)||a.subject.toLowerCase().includes(n)||a.attendees.some(r=>r.email.toLowerCase().includes(n.split(" ")[0])))}static formatMeetingForNote(i){let e=i.attendees.filter(t=>t.isExternal!==!1).map(t=>t.name||t.email.split("@")[0]).slice(0,5).join(", ");return{title:i.subject,attendees:e,meetingStart:i.start,accountName:i.accountName}}static getDayName(i){let e;i.length===10&&i.includes("-")?e=new Date(i+"T00:00:00"):e=new Date(i);let t=new Date;t.setHours(0,0,0,0);let n=new Date(e);n.setHours(0,0,0,0);let a=Math.round((n.getTime()-t.getTime())/(1e3*60*60*24));return a===0?"Today":a===1?"Tomorrow":e.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}static formatTime(i,e){let t=i;t&&!t.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(t)&&(t=t+"Z");let n=new Date(t);if(isNaN(n.getTime()))return i;let a={hour:"numeric",minute:"2-digit",hour12:!0};return e&&(a.timeZone=e),n.toLocaleTimeString("en-US",a)}static safeParseDate(i){if(!i)return new Date(NaN);let e=i;return!e.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(e)&&(e=e+"Z"),new Date(e)}static getMeetingDuration(i,e){let t=y.safeParseDate(i),n=y.safeParseDate(e);return Math.round((n.getTime()-t.getTime())/(1e3*60))}};var be=["ai-contracting-tech","ai-contracting-services","ai-compliance-tech","ai-compliance-services","ai-ma-tech","ai-ma-services","sigma"],je=["metrics-identified","economic-buyer-identified","decision-criteria-discussed","decision-process-discussed","pain-confirmed","champion-identified","competition-mentioned"],Ae=["progressing","stalled","at-risk","champion-engaged","early-stage"],xe=["discovery","demo","negotiation","qbr","implementation","follow-up"],Ee=`You are a sales intelligence tagger for Eudia, an AI legal technology company. Extract structured tags from meeting analysis.
...
`).replace(/\n/g,"<br>")}onClose(){this.contentEl.empty()}};var te=class extends u.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new F(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return N}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}if(this.plugin.settings.accountsImported)this.steps[2].status="complete";else{console.log("[Eudia Setup] Email set but accounts not imported \u2014 auto-retrying import...");try{let e=this.plugin.settings.userEmail,t=[],n=[];if(j(e))t=await this.accountOwnershipService.getAllAccountsForAdmin(e);else if(M(e)){let a=await this.accountOwnershipService.getCSAccounts(e);t=a.accounts,n=a.prospects}else{let a=await this.accountOwnershipService.getAccountsWithProspects(e);t=a.accounts,n=a.prospects}if(t.length>0||n.length>0){let a=this.plugin.app.workspace.leftSplit,r=a?.collapsed;a&&!r&&a.collapse(),j(e)?await this.plugin.createAdminAccountFolders(t):(await this.plugin.createTailoredAccountFolders(t,{}),n.length>0&&await this.plugin.createProspectAccountFiles(n)),z(e)&&await this.plugin.createCSManagerDashboard(e,t),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=t.length+n.length,await this.plugin.saveSettings(),this.steps[2].status="complete",a&&!r&&a.expand(),new u.Notice(`Imported ${t.length} accounts!`),console.log(`[Eudia Setup] Auto-retry imported ${t.length} accounts for ${e}`);let s=[...t,...n];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(s)}catch{}},500)}}catch(e){console.log("[Eudia Setup] Auto-retry account import failed:",e)}}}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),n=t.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to unlock your sales superpowers",cls:"eudia-setup-subtitle"});let a=t.createDiv({cls:"eudia-setup-progress-section"}),r=this.getCompletionPercentage(),s=a.createDiv({cls:"eudia-setup-progress-label"});s.createSpan({text:"Setup Progress"}),s.createSpan({text:`${r}%`,cls:"eudia-setup-progress-value"});let c=a.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${r}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=n.createDiv({cls:"eudia-setup-step-header"}),r=a.createDiv({cls:"eudia-setup-step-number"});r.setText(t.status==="complete"?"":"1"),t.status==="complete"&&r.addClass("eudia-step-complete");let s=a.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:t.title}),s.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let d=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});d.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async l=>{l.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let n=await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new u.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");try{let a,r=[];if(j(e))console.log("[Eudia] Admin user detected - importing all accounts"),a=await this.accountOwnershipService.getAllAccountsForAdmin(e);else if(M(e)){console.log("[Eudia] CS user detected - importing CS-relevant accounts"),t&&(t.textContent="Loading Customer Success accounts...");let s=await this.accountOwnershipService.getCSAccounts(e);a=s.accounts,r=s.prospects}else{let s=await this.accountOwnershipService.getAccountsWithProspects(e);a=s.accounts,r=s.prospects}if(a.length>0||r.length>0){t&&(t.textContent=`Creating ${a.length} account folders...`);let s=this.plugin.app.workspace.leftSplit,o=s?.collapsed;s&&!o&&s.collapse(),j(e)?await this.plugin.createAdminAccountFolders(a):(await this.plugin.createTailoredAccountFolders(a,{}),r.length>0&&await this.plugin.createProspectAccountFiles(r)),z(e)&&await this.plugin.createCSManagerDashboard(e,a),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length+r.length,await this.plugin.saveSettings(),s&&!o&&s.expand(),new u.Notice(`Imported ${a.length} active accounts + ${r.length} prospects!`);let c=[...a,...r];setTimeout(async()=>{try{t&&(t.textContent="Loading Salesforce intelligence...",t.className="eudia-setup-validation-message loading"),await this.plugin.enrichAccountFolders(c)}catch(d){console.log("[Eudia] Background enrichment skipped:",d)}},500)}}catch(a){console.error("[Eudia] Account import failed:",a)}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=n.createDiv({cls:"eudia-setup-step-header"}),r=a.createDiv({cls:"eudia-setup-step-number"});r.setText(t.status==="complete"?"":"2"),t.status==="complete"&&r.addClass("eudia-step-complete");let s=a.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:t.title}),s.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let d=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),l=o.createDiv({cls:"eudia-setup-sf-status"});d.onclick=async()=>{let h=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(h,"_blank"),l.textContent="Complete the login in the popup window...",l.className="eudia-setup-sf-status loading",new u.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(l)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,n=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new u.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=this.plugin.settings.userEmail,n,a=[];if(j(t))console.log("[Eudia] Admin user detected - importing all accounts"),e.textContent="Admin detected - importing all accounts...",n=await this.accountOwnershipService.getAllAccountsForAdmin(t);else if(M(t)){console.log("[Eudia] CS user detected - importing CS-relevant accounts"),e.textContent="Loading Customer Success accounts...";let l=await this.accountOwnershipService.getCSAccounts(t);n=l.accounts,a=l.prospects}else{let l=await this.accountOwnershipService.getAccountsWithProspects(t);n=l.accounts,a=l.prospects}if(n.length===0&&a.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}e.textContent=`Creating ${n.length} account folders...`;let r=this.plugin.app.workspace.leftSplit,s=r?.collapsed;r&&!s&&r.collapse(),j(t)?await this.plugin.createAdminAccountFolders(n):(await this.plugin.createTailoredAccountFolders(n,{}),a.length>0&&await this.plugin.createProspectAccountFiles(a)),z(t)&&await this.plugin.createCSManagerDashboard(t,n),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=n.length+a.length,await this.plugin.saveSettings(),r&&!s&&r.expand(),this.steps[2].status="complete";let o=n.filter(l=>l.isOwned!==!1).length,c=n.filter(l=>l.isOwned===!1).length;j(t)&&c>0?e.textContent=`${o} owned + ${c} view-only accounts imported! Enriching...`:e.textContent=`${n.length} active + ${a.length} prospect accounts imported! Enriching...`,e.className="eudia-setup-sf-status success";let d=[...n,...a];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(d);let l=d.filter(h=>h.id&&h.id.length>=15).length;e.textContent=`${n.length+a.length} accounts imported, ${l} enriched with Salesforce data`}catch(l){console.log("[Eudia] Background enrichment skipped:",l),e.textContent=`${n.length+a.length} accounts imported (enrichment will retry on next launch)`}},500)}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error"}}renderTranscribeStep(e){let t=this.steps[2],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),a=n.createDiv({cls:"eudia-setup-step-header"}),r=a.createDiv({cls:"eudia-setup-step-number"});r.setText(t.status==="complete"?"":"3"),t.status==="complete"&&r.addClass("eudia-step-complete");let s=a.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:t.title}),s.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),d.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let l=c.createDiv({cls:"eudia-setup-instruction"});l.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),l.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let h=c.createDiv({cls:"eudia-setup-instruction"});h.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),h.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(r=>r.status==="complete")){let r=t.createDiv({cls:"eudia-setup-completion"});r.createEl("h2",{text:"\u{1F389} You're all set!"}),r.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let s=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});s.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(N),await this.plugin.activateCalendarView()}}else{let r=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});r.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(N),new u.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let a=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});a.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},ne=class extends u.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return L}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),n=t.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let a=n.createDiv({cls:"eudia-calendar-actions"}),r=a.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});r.title="Refresh (fetches latest from calendar)",r.onclick=async()=>{r.addClass("spinning"),this._forceRefresh=!0,await this.render(),r.removeClass("spinning")};let s=a.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});s.title="Settings",s.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,a=this.plugin.settings.userEmail;try{(await(0,u.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(a&&t.server==="connected")try{let l=await(0,u.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(a)}`,method:"GET",throw:!1});l.status===200&&l.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(a&&t.server==="connected")try{let l=await(0,u.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(a)}`,method:"GET",throw:!1});l.status===200&&l.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let r=e.createDiv({cls:"eudia-status-indicators"}),s=r.createSpan({cls:`eudia-status-dot ${t.server}`});s.title=t.serverMessage||"Server";let o=r.createSpan({cls:`eudia-status-dot ${t.calendar}`});o.title=t.calendarMessage||"Calendar";let c=r.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(c.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let l=e.createDiv({cls:"eudia-status-warning"});l.innerHTML=`<strong>${a}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),n=t.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let a=new I(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),r=this._forceRefresh||!1;this._forceRefresh=!1;let s=await a.getWeekMeetings(r);if(n.remove(),!s.success){this.renderError(t,s.error||"Failed to load calendar");return}let o=Object.keys(s.byDay||{}).sort();if(o.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,a);for(let c of o){let d=s.byDay[c];!d||d.length===0||this.renderDaySection(t,c,d)}}catch(a){n.remove(),this.renderError(t,a.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let n=await t.getCurrentMeeting();if(n.meeting){let a=e.createDiv({cls:"eudia-now-card"});n.isNow?a.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):a.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let r=a.createDiv({cls:"eudia-now-content"});r.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&r.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let s=a.createEl("button",{cls:"eudia-now-action",text:"Create Note"});s.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(e,t,n){let a=e.createDiv({cls:"eudia-calendar-day"});a.createEl("div",{cls:"eudia-calendar-day-header",text:I.getDayName(t)});for(let r of n){let s=a.createDiv({cls:`eudia-calendar-meeting ${r.isCustomerMeeting?"customer":"internal"}`});s.createEl("div",{cls:"eudia-calendar-time",text:I.formatTime(r.start,this.plugin.settings.timezone)});let o=s.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:r.subject}),r.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:r.accountName});else if(r.attendees&&r.attendees.length>0){let c=r.attendees.slice(0,2).map(d=>d.name||d.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}s.onclick=()=>this.createNoteForMeeting(r),s.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `}renderError(e,t){let n=e.createDiv({cls:"eudia-calendar-error"}),a="",r="Unable to load calendar",s="";t.includes("not authorized")||t.includes("403")?(a="\u{1F512}",r="Calendar Access Required",s="Contact your admin to be added to the authorized users list."):t.includes("network")||t.includes("fetch")?(a="\u{1F4E1}",r="Connection Issue",s="Check your internet connection and try again."):(t.includes("server")||t.includes("500"))&&(a="\u{1F527}",r="Server Unavailable",s="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=t.createDiv({cls:"eudia-setup-input-group"}),a=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});a.addClass("eudia-setup-email");let r=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),s=t.createDiv({cls:"eudia-setup-status"});r.onclick=async()=>{let o=a.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){s.textContent="Please use your @eudia.com email",s.className="eudia-setup-status error";return}r.disabled=!0,r.textContent="Connecting...",s.textContent="";try{if(!(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){s.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,s.className="eudia-setup-status error",r.disabled=!1,r.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),s.textContent="Connected",s.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let d=c.message||"Connection failed";d.includes("403")?s.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:s.textContent=d,s.className="eudia-setup-status error",r.disabled=!1,r.textContent="Connect"}},a.onkeydown=o=>{o.key==="Enter"&&r.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(e){let t=e.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],a=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],r=t.filter(o=>!a.includes(o)&&o.length>1);if(r.length===0)return t[0]||"";if(r.length>1&&n.includes(r[0]))return r[1].charAt(0).toUpperCase()+r[1].slice(1);let s=r[r.length-1];return s.charAt(0).toUpperCase()+s.slice(1)}getExternalDomainsFromAttendees(e){if(!e||e.length===0)return[];let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,a=[];for(let r of e){if(!r.email)continue;let o=r.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let c=o[1];if(c.includes("eudia.com")||t.includes(c)||n.has(c))continue;n.add(c);let d=this.extractCompanyFromDomain(c);d.length>=2&&a.push({domain:c,company:d})}}return a}findBestAccountMatch(e,t,n){let a=this.plugin.settings.accountsFolder||"Accounts",r=this.app.vault.getAbstractFileByPath(a);if(!(r instanceof u.TFolder))return null;let s=[];for(let c of r.children)c instanceof u.TFolder&&s.push(c.name);if(s.length===0)return null;let o=[];for(let{domain:c,company:d}of e){let l=this.findAccountFolder(d),h=l?1:0;o.push({domain:c,company:d,folder:l,score:h})}if(o.sort((c,d)=>d.score-c.score),o.length>0&&o[0].folder){let c=o[0],d=c.folder.split("/").pop()||c.company;return console.log(`[Eudia Calendar] Best domain match: "${c.company}" from ${c.domain} -> ${c.folder}`),{folder:c.folder,accountName:d,source:"domain"}}if(t){let c=this.findAccountFolder(t);if(c){let d=c.split("/").pop()||t;return console.log(`[Eudia Calendar] Server account match: "${t}" -> ${c}`),{folder:c,accountName:d,source:"server"}}}if(n){let c=this.findAccountFolder(n);if(c){let d=c.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${c}`),{folder:c,accountName:d,source:"subject"}}}for(let{company:c}of e){let d=s.find(l=>{let h=l.toLowerCase(),g=c.toLowerCase();return h.includes(g)||g.includes(h)});if(d){let l=`${a}/${d}`;return console.log(`[Eudia Calendar] Partial domain match: "${c}" -> ${l}`),{folder:l,accountName:d,source:"domain-partial"}}}return null}extractAccountFromAttendees(e){let t=this.getExternalDomainsFromAttendees(e);if(t.length===0)return null;let n=t[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let a=(t[1]||t[2]||"").trim();if(a.toLowerCase()!=="eudia")return a}let n=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let r=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(r.toLowerCase()!=="eudia"&&r.length>0)return r}if(!e.toLowerCase().includes("eudia")){let a=e.match(/^([^-–|]+)/);if(a){let r=a[1].trim();if(r.length>2&&r.length<50)return r}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(t);if(!(n instanceof u.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let a=e.toLowerCase().trim(),r=[];for(let h of n.children)h instanceof u.TFolder&&r.push(h.name);console.log(`[Eudia Calendar] Searching for "${a}" in ${r.length} folders`);let s=r.find(h=>h.toLowerCase()===a);if(s)return console.log(`[Eudia Calendar] Exact match found: ${s}`),`${t}/${s}`;let o=r.find(h=>h.toLowerCase().startsWith(a));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${t}/${o}`;let c=r.find(h=>a.startsWith(h.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${t}/${c}`;let d=r.find(h=>{let g=h.toLowerCase();return g.length>=3&&a.includes(g)});if(d)return console.log(`[Eudia Calendar] Search contains folder match: ${d}`),`${t}/${d}`;let l=r.find(h=>{let g=h.toLowerCase();return a.length>=3&&g.includes(a)});return l?(console.log(`[Eudia Calendar] Folder contains search match: ${l}`),`${t}/${l}`):(console.log(`[Eudia Calendar] No folder match found for "${a}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",a=j(n),r=(e.attendees||[]).map(w=>w.email).filter(Boolean),s=se(e.subject,r);if(a&&s.isPipelineMeeting&&s.confidence>=60){await this._createPipelineNote(e,t);return}let o=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),c=`${t} - ${o}.md`,d=null,l=e.accountName||null,h=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(w=>w.email)||[])}`);let g=this.getExternalDomainsFromAttendees(e.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(g)}`);let m=this.extractAccountFromSubject(e.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${m||"none"}"`);let p=this.findBestAccountMatch(g,e.accountName,m||void 0);if(p&&(d=p.folder,l=p.accountName,console.log(`[Eudia Calendar] Best match (${p.source}): "${l}" -> ${d}`)),!d){let w=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(w)instanceof u.TFolder&&(d=w,console.log(`[Eudia Calendar] No match found, using Accounts root: ${d}`))}if(l){let w=this.plugin.settings.cachedAccounts.find(D=>D.name.toLowerCase()===l?.toLowerCase());w&&(h=w.id,l=w.name,console.log(`[Eudia Calendar] Matched to cached account: ${w.name} (${w.id})`))}let f=d?`${d}/${c}`:c,S=this.app.vault.getAbstractFileByPath(f);if(S instanceof u.TFile){await this.app.workspace.getLeaf().openFile(S),new u.Notice(`Opened existing note: ${c}`);return}let v=(e.attendees||[]).map(w=>w.name||w.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${e.subject}"
date: ${t}
attendees: [${v}]
account: "${l||""}"
...
`;try{let w=await this.app.vault.create(f,C);await this.app.workspace.getLeaf().openFile(w),new u.Notice(`Created: ${f}`)}catch(w){console.error("[Eudia Calendar] Failed to create note:",w),new u.Notice(`Could not create note: ${w.message||"Unknown error"}`)}}async _createPipelineNote(e,t){let n=new Date(t+"T00:00:00"),a=String(n.getMonth()+1).padStart(2,"0"),r=String(n.getDate()).padStart(2,"0"),s=String(n.getFullYear()).slice(-2),o=`${a}.${r}.${s}`,c=`Team Pipeline Meeting - ${o}.md`,d="Pipeline Meetings";this.app.vault.getAbstractFileByPath(d)||await this.app.vault.createFolder(d);let h=`${d}/${c}`,g=this.app.vault.getAbstractFileByPath(h);if(g instanceof u.TFile){await this.app.workspace.getLeaf().openFile(g),new u.Notice(`Opened existing: ${c}`);return}let m=(e.attendees||[]).map(f=>f.name||f.email?.split("@")[0]||"Unknown"),p=`---
...
`;try{let f=await this.app.vault.create(h,p);await this.app.workspace.getLeaf().openFile(f),new u.Notice(`Created pipeline note: ${c}`),console.log(`[Eudia Pipeline] Created pipeline meeting note: ${h}`)}catch(f){console.error("[Eudia Pipeline] Failed to create pipeline note:",f),new u.Notice(`Could not create pipeline note: ${f.message||"Unknown error"}`)}}},V=class extends u.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null;this.micRibbonIcon=null;this.liveTranscript="";this.liveTranscriptChunkInterval=null;this.isTranscribingChunk=!1;this.sfSyncStatusBarEl=null;this.sfSyncIntervalId=null}async onload(){await this.loadSettings(),this.transcriptionService=new _(this.settings.serverUrl),this.calendarService=new I(this.settings.serverUrl,this.settings.userEmail,this.settings.timezone||"America/New_York"),this.smartTagService=new G,this.checkForPluginUpdate(),this.registerView(L,e=>new ne(e,this)),this.registerView(N,e=>new te(e,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.micRibbonIcon=this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.addCommand({id:"transcribe-meeting",name:"Transcribe Meeting",callback:async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}}),this.addCommand({id:"open-calendar",name:"Open Calendar",callback:()=>this.activateCalendarView()}),this.addCommand({id:"sync-accounts",name:"Sync Salesforce Accounts",callback:()=>this.syncAccounts()}),this.addCommand({id:"sync-note",name:"Sync Note to Salesforce",callback:()=>this.syncNoteToSalesforce()}),this.addCommand({id:"new-meeting-note",name:"New Meeting Note",callback:()=>this.createMeetingNote()}),this.addCommand({id:"ask-gtm-brain",name:"Ask gtm-brain",callback:()=>this.openIntelligenceQueryForCurrentNote()}),this.addCommand({id:"enrich-accounts",name:"Enrich Account Folders with Salesforce Data",callback:async()=>{if(!this.settings.userEmail){new u.Notice("Please set up your email first.");return}let e=new F(this.settings.serverUrl),t;M(this.settings.userEmail)?t=await e.getCSAccounts(this.settings.userEmail):t=await e.getAccountsWithProspects(this.settings.userEmail);let n=[...t.accounts,...t.prospects];if(n.length===0){new u.Notice("No accounts found to enrich.");return}await this.enrichAccountFolders(n)}}),this.addCommand({id:"refresh-analytics",name:"Refresh Analytics Dashboard",callback:async()=>{let e=this.app.workspace.getActiveFile();e?await this.refreshAnalyticsDashboard(e):new u.Notice("No active file")}}),this.addCommand({id:"live-query-transcript",name:"Query Current Transcript (Live)",callback:async()=>{if(!this.audioRecorder?.isRecording()){new u.Notice("No active recording. Start recording first to use live query.");return}if(!this.liveTranscript||this.liveTranscript.length<50){new u.Notice("Not enough transcript captured yet. Keep recording for a few more minutes.");return}this.openLiveQueryModal()}}),this.sfSyncStatusBarEl=this.addStatusBarItem(),this.sfSyncStatusBarEl.setText("SF Sync: Idle"),this.sfSyncStatusBarEl.addClass("eudia-sf-sync-status"),this.addSettingTab(new ae(this.app,this)),this.registerEditorSuggest(new Z(this.app,this)),this.app.workspace.onLayoutReady(async()=>{if(this.settings.setupCompleted){if(this.settings.syncOnStartup){if(await this.scanLocalAccountFolders(),this.settings.userEmail&&this.settings.syncAccountsOnStartup){let e=new Date().toISOString().split("T")[0];this.settings.lastAccountRefreshDate!==e&&setTimeout(async()=>{try{console.log("[Eudia] Startup account sync - checking for changes...");let n=await this.syncAccountFolders();if(n.success){if(this.settings.lastAccountRefreshDate=e,await this.saveSettings(),n.added>0||n.archived>0){let a=[];n.added>0&&a.push(`${n.added} added`),n.archived>0&&a.push(`${n.archived} archived`),new u.Notice(`Account folders synced: ${a.join(", ")}`)}}else console.log("[Eudia] Sync failed:",n.error)}catch{console.log("[Eudia] Startup sync skipped (server unreachable), will retry tomorrow")}},2e3)}this.settings.showCalendarView&&this.settings.userEmail&&await this.activateCalendarView(),this.settings.userEmail&&this.settings.cachedAccounts.length>0&&setTimeout(async()=>{try{await this.checkAndAutoEnrich()}catch{console.log("[Eudia] Auto-enrich skipped (server unreachable)")}},5e3),this.settings.userEmail&&this.telemetry?setTimeout(async()=>{try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),t=0;e&&e instanceof u.TFolder&&(t=e.children.filter(r=>r instanceof u.TFolder&&!r.name.startsWith("_")).length);let n={salesforce:this.settings.salesforceConnected?"connected":"not_configured",calendar:this.settings.calendarConfigured?"connected":"not_configured"};await this.telemetry.sendHeartbeat(t,n);let a=await this.telemetry.checkForPushedConfig();if(a.length>0){let r=!1;for(let s of a)s.key&&this.settings.hasOwnProperty(s.key)&&(this.settings[s.key]=s.value,r=!0,console.log(`[Eudia] Applied pushed config: ${s.key} = ${s.value}`));r&&(await this.saveSettings(),new u.Notice("Settings updated by admin"))}await this.checkAndConsumeSyncFlags(),this.startSalesforceSyncScanner()}catch{console.log("[Eudia] Heartbeat/config check skipped"),this.startSalesforceSyncScanner()}},3e3):this.settings.sfAutoSyncEnabled&&this.settings.salesforceConnected&&setTimeout(()=>this.startSalesforceSyncScanner(),5e3)}}else{await new Promise(t=>setTimeout(t,100));let e=document.querySelector(".modal-container .modal");if(e){let t=e.querySelector(".modal-close-button");t&&t.click()}await this.activateSetupView()}this.app.workspace.on("file-open",async e=>{if(e&&(e.path.includes("_Analytics/")||e.path.includes("_Customer Health/")))try{let t=await this.app.vault.read(e);if(t.includes("type: analytics_dashboard")){let a=t.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/)?.[1],r=new Date().toISOString().split("T")[0];a!==r&&(console.log(`[Eudia] Auto-refreshing analytics: ${e.name}`),await this.refreshAnalyticsDashboard(e))}}catch{}})})}async onunload(){this.app.workspace.detachLeavesOfType(L)}async checkForPluginUpdate(){try{let e=this.settings.serverUrl||"https://gtm-wizard.onrender.com",t=await(0,u.requestUrl)({url:`${e}/api/plugin/version`,method:"GET",headers:{"Content-Type":"application/json"}});if(!t.json?.success)return;let n=t.json.currentVersion,a=this.manifest?.version||"0.0.0",r=n.split(".").map(Number),s=a.split(".").map(Number),o=!1;for(let c=0;c<3;c++){if((r[c]||0)>(s[c]||0)){o=!0;break}if((r[c]||0)<(s[c]||0))break}o?(console.log(`[Eudia Update] New version available: ${n} (current: ${a})`),await this.performAutoUpdate(e,n,a)):console.log(`[Eudia Update] Plugin is up to date (v${a})`)}catch(e){console.log("[Eudia Update] Could not check for updates:",e.message||e)}}async performAutoUpdate(e,t,n){try{if(this.audioRecorder?.isRecording()){console.log("[Eudia Update] Skipping auto-update \u2014 recording in progress"),new u.Notice(`Eudia update v${t} available.
Finish your recording, then restart Obsidian to update.`,1e4);return}let a=this.manifest.dir;if(!a){console.log("[Eudia Update] Cannot determine plugin directory \u2014 skipping");return}let r=this.app.vault.adapter;console.log("[Eudia Update] Downloading plugin files...");let[s,o,c]=await Promise.all([(0,u.requestUrl)({url:`${e}/api/plugin/main.js`}),(0,u.requestUrl)({url:`${e}/api/plugin/manifest.json`}),(0,u.requestUrl)({url:`${e}/api/plugin/styles.css`})]),d=s.text,l=o.text,h=c.text,g=1024,m=5*1024*1024;for(let[C,w]of[["main.js",d],["manifest.json",l],["styles.css",h]])if(!w||w.length<g||w.length>m){console.log(`[Eudia Update] Downloaded ${C} failed validation (${w?.length??0} bytes) \u2014 aborting`);return}try{let C=await r.read(`${a}/main.js`);await r.write(`${a}/main.js.bak`,C),console.log("[Eudia Update] Backed up current main.js")}catch{console.log("[Eudia Update] Could not back up main.js \u2014 continuing")}await r.write(`${a}/main.js`,d),await r.write(`${a}/manifest.json`,l),await r.write(`${a}/styles.css`,h),console.log(`[Eudia Update] Files written \u2014 v${n} \u2192 v${t}`);try{(0,u.requestUrl)({url:`${e}/api/plugin/telemetry`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:"info",message:`Auto-updated from v${n} to v${t}`,userEmail:this.settings.userEmail||"anonymous",pluginVersion:t,platform:"obsidian",context:{fromVersion:n,toVersion:t}})}).catch(()=>{})}catch{}let p=document.createDocumentFragment(),f=document.createElement("div");f.style.cssText="display:flex;flex-direction:column;gap:8px;";let S=document.createElement("span");S.textContent=`Eudia updated to v${t}. Reload to apply.`,f.appendChild(S);let v=document.createElement("button");v.textContent="Reload now",v.style.cssText="padding:4px 12px;border-radius:4px;border:1px solid var(--interactive-accent);background:var(--interactive-accent);color:var(--text-on-accent);cursor:pointer;font-size:12px;align-self:flex-start;",v.addEventListener("click",()=>{this.app.commands.executeCommandById("app:reload")}),f.appendChild(v),p.appendChild(f),new u.Notice(p,0)}catch(a){console.log("[Eudia Update] Auto-update failed:",a.message||a)}}async loadSettings(){this.settings=Object.assign({},De,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async activateCalendarView(){let e=this.app.workspace,t=e.getLeavesOfType(L);if(t.length>0)e.revealLeaf(t[0]);else{let n=e.getRightLeaf(!1);n&&(await n.setViewState({type:L,active:!0}),e.revealLeaf(n))}}async activateSetupView(){let e=this.app.workspace,t=e.getLeavesOfType(N);if(t.length>0)e.revealLeaf(t[0]);else{let n=e.getLeaf(!0);n&&(await n.setViewState({type:N,active:!0}),e.revealLeaf(n))}}async createTailoredAccountFolders(e,t){let n=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);let r=0,s=new Date().toISOString().split("T")[0],o=async d=>{let l=d.name.replace(/[<>:"/\\|?*]/g,"_").trim(),h=`${n}/${l}`;if(this.app.vault.getAbstractFileByPath(h)instanceof u.TFolder)return console.log(`[Eudia] Account folder already exists: ${l}`),!1;try{await this.app.vault.createFolder(h);let m=t?.[d.id],p=!!m,f=this.buildContactsContent(d,m,s),S=this.buildIntelligenceContent(d,m,s),v=this.buildMeetingNotesContent(d,m),C=this.buildNextStepsContent(d,m,s),w=[{name:"Note 1.md",content:`---
account: "${d.name}"
account_id: "${d.id}"
type: meeting_note
sync_to_salesforce: false
...
`},{name:"Meeting Notes.md",content:v},{name:"Contacts.md",content:f},{name:"Intelligence.md",content:S},{name:"Next Steps.md",content:C}];for(let E of w){let $=`${h}/${E.name}`;await this.app.vault.create($,E.content)}return console.log(`[Eudia] Created account folder with subnotes${p?" (enriched)":""}: ${l}`),!0}catch(m){return console.error(`[Eudia] Failed to create folder for ${l}:`,m),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c),h=await Promise.allSettled(l.map(g=>o(g)));r+=h.filter(g=>g.status==="fulfilled"&&g.value===!0).length}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),r>0&&new u.Notice(`Created ${r} account folders`),await this.ensureNextStepsFolderExists()}buildContactsContent(e,t,n){let a=t?`
...
*Log key interactions and relationship developments.*
`}buildIntelligenceContent(e,t,n){let a=t?`
enriched_at: "${new Date().toISOString()}"`:"",r=`---
account: "${e.name}"
account_id: "${e.id}"
type: intelligence
...
---`,r=[];return t?.opportunities&&r.push(t.opportunities),t?.recentActivity&&r.push(t.recentActivity),r.length>0?`${a}
...
3. **Next Steps** are auto-extracted after transcription
4. Set \`sync_to_salesforce: true\` to sync to Salesforce
`}buildNextStepsContent(e,t,n){let a=n||new Date().toISOString().split("T")[0],r=t?`
...
*Previous next steps will be archived here.*
`}async fetchEnrichmentData(e){let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=e.filter(s=>s.id&&s.id.length>=15);if(n.length===0)return{};let a={},r=20;console.log(`[Eudia Enrich] Fetching enrichment data for ${n.length} accounts`);for(let s=0;s<n.length;s+=r){let c=n.slice(s,s+r).map(d=>d.id);try{let d=await(0,u.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:c,userEmail:this.settings.userEmail})});d.json?.success&&d.json?.enrichments&&Object.assign(a,d.json.enrichments)}catch(d){console.error(`[Eudia Enrich] Batch fetch failed (batch ${s/r+1}):`,d)}s+r<n.length&&await new Promise(d=>setTimeout(d,100))}return console.log(`[Eudia Enrich] Got enrichment data for ${Object.keys(a).length}/${n.length} accounts`),a}async createProspectAccountFiles(e){if(!e||e.length===0)return 0;let t=this.settings.accountsFolder||"Accounts",n=`${t}/_Prospects`;if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let r=0;for(let s of e){let o=s.name.replace(/[<>:"/\\|?*]/g,"_").trim(),c=`${n}/${o}`;if(this.app.vault.getAbstractFileByPath(c)instanceof u.TFolder)continue;let l=`${t}/${o}`;if(this.app.vault.getAbstractFileByPath(l)instanceof u.TFolder)continue;let g=`${n}/${o}.md`,m=this.app.vault.getAbstractFileByPath(g);if(m instanceof u.TFile)try{await this.app.vault.delete(m)}catch{}try{await this.app.vault.createFolder(c);let p=new Date().toISOString().split("T")[0],f=[{name:"Note 1.md",content:`---
account: "${s.name}"
account_id: "${s.id}"
type: meeting_note
tier: prospect
...
*Previous next steps will be archived here.*
`}];for(let S of f){let v=`${c}/${S.name}`;await this.app.vault.create(v,S.content)}r++}catch(p){console.log(`[Eudia] Failed to create prospect folder for ${s.name}:`,p)}}return r>0&&console.log(`[Eudia] Created ${r} prospect account folders in _Prospects/`),r}async createCSManagerDashboard(e,t){let n="CS Manager",a=new Date().toISOString().split("T")[0],r=ue(e);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s={};for(let h of t){let g=h.ownerName||"Unassigned";s[g]||(s[g]=[]),s[g].push(h)}let o=`---
role: cs_manager
manager: "${e}"
...
`,c=Object.keys(s).sort();for(let h of c){let g=s[h];o+=`### ${h} (${g.length} accounts)
`;for(let m of g.slice(0,10))o+=`- **${m.name}** \u2014 ${m.type||"Account"}
...
| Account | Type | Owner |
|---------|------|-------|
`;for(let h of t.slice(0,50))o+=`| ${h.name} | ${h.type||""} | ${h.ownerName||""} |
`;o+=`
---

*This dashboard auto-updates when the vault syncs. New Stage 4/5 and Existing accounts will appear automatically.*
`;let d=`${n}/CS Manager Overview.md`,l=this.app.vault.getAbstractFileByPath(d);l instanceof u.TFile?await this.app.vault.modify(l,o):await this.app.vault.create(d,o);for(let h of r){let g=h.split("@")[0].replace("."," ").replace(/\b\w/g,v=>v.toUpperCase()),m=t.filter(v=>{let C=(v.ownerName||"").toLowerCase(),w=h.split("@")[0].replace("."," ").toLowerCase();return C.includes(w.split(" ")[0])||C.includes(w.split(" ").pop()||"")}),p=`---
rep: "${h}"
rep_name: "${g}"
role: cs_rep_summary
account_count: ${m.length}
created: ${a}
---
...
**Email:** ${h}
**CS Accounts:** ${m.length}

---

## Assigned Accounts

`;if(m.length>0){p+=`| Account | Type |
|---------|------|
`;for(let v of m)p+=`| ${v.name} | ${v.type||""} |
`}else p+=`*No accounts currently matched to this rep. Accounts are matched by opportunity owner.*
`;p+=`
---

*Updates automatically as new CS-relevant accounts sync.*
`;let f=`${n}/${g}.md`,S=this.app.vault.getAbstractFileByPath(f);S instanceof u.TFile?await this.app.vault.modify(S,p):await this.app.vault.create(f,p)}console.log(`[Eudia] Created CS Manager dashboard for ${e} with ${t.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let a=0,r=0,s=new Date().toISOString().split("T")[0],o=async d=>{let l=d.name.replace(/[<>:"/\\|?*]/g,"_").trim(),h=`${t}/${l}`;if(this.app.vault.getAbstractFileByPath(h)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(h),await this.createExecAccountSubnotes(h,d,s),d.isOwned?a++:r++,console.log(`[Eudia Admin] Created ${d.isOwned?"owned":"view-only"} folder: ${l}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${l}:`,m),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c);await Promise.allSettled(l.map(h=>o(h)))}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),a+r>0&&new u.Notice(`Created ${a} owned + ${r} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let a=t.ownerName||"Unknown",r=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let s of r){let o=`${e}/${s.name}`;await this.app.vault.create(o,s.content)}}async createFullAccountSubnotes(e,t,n){let a=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let r of a){let s=`${e}/${r.name}`;await this.app.vault.create(s,r.content)}}async ensurePipelineFolderExists(){let e="Pipeline",t=`${e}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let s=`---
...
`;await this.app.vault.create(t,o)}}async updateAccountNextSteps(e,t,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${e}`),console.log(`[Eudia] Content length: ${t?.length||0} chars`);let a=e.replace(/[<>:"/\\|?*]/g,"_").trim(),r=`${this.settings.accountsFolder}/${a}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${r}`);let s=this.app.vault.getAbstractFileByPath(r);if(!s||!(s instanceof u.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${r}`);let v=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${a}`);v&&v instanceof u.TFolder?console.log(`[Eudia] Files in ${a} folder:`,v.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${a}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),d=n.split("/").pop()?.replace(".md","")||"Meeting",l=t;!t.includes("- [ ]")&&!t.includes("- [x]")&&(l=t.split(`
`).filter(v=>v.trim()).map(v=>{let C=v.replace(/^[-•*]\s*/,"").trim();return C?`- [ ] ${C}`:""}).filter(Boolean).join(`
`));let h=await this.app.vault.read(s),g="",m=h.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);m&&m[1]&&(g=m[1].trim());let p=`### ${o} - ${d}
${l||"*None*"}`,f=g?`${p}
...
${g}`:p,S=`---
account: "${e}"
account_id: "${this.settings.cachedAccounts.find(v=>v.name===e)?.id||""}"
type: next_steps
auto_updated: true
last_updated: ${o}
sync_to_salesforce: false
...
${f}
`;await this.app.vault.modify(s,S),console.log(`[Eudia] Updated Next Steps for ${e} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(a){console.error(`[Eudia] Failed to update Next Steps for ${e}:`,a)}}async regenerateNextStepsDashboard(){try{let t=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!t||!(t instanceof u.TFile)){await this.ensureNextStepsFolderExists();return}let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!n||!(n instanceof u.TFolder))return;let a=[];for(let c of n.children)if(c instanceof u.TFolder){let d=`${c.path}/Next Steps.md`,l=this.app.vault.getAbstractFileByPath(d);if(l instanceof u.TFile){let h=await this.app.vault.read(l),g=h.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),m=g?g[1]:"Unknown",p=h.split(`
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(p.length>0||m!=="Unknown")&&a.push({account:c.name,lastUpdated:m,nextSteps:p})}}a.sort((c,d)=>d.lastUpdated.localeCompare(c.lastUpdated));let r=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${r}
---
...
`):typeof f=="object"?JSON.stringify(f):String(f),a=n(e.title)||"Meeting Notes",r=n(e.summary),s=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),l=n(e.actionItems),h=n(e.keyDates),g=n(e.risksObjections),m=n(e.attendees),p=`---
title: "${a}"
...
`;return s&&!s.includes("None explicitly")&&(p+=`## Pain Points
...
${t.text}
`),p}openIntelligenceQuery(){new Y(this.app,this).open()}openIntelligenceQueryForCurrentNote(){let e=this.app.workspace.getActiveFile(),t;if(e){let n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(n?.account_id&&n?.account)t={id:n.account_id,name:n.account};else if(n?.account){let a=this.settings.cachedAccounts.find(r=>r.name.toLowerCase()===n.account.toLowerCase());a?t={id:a.id,name:a.name}:t={id:"",name:n.account}}else{let a=e.path.split("/");if(a.length>=2&&a[0]===this.settings.accountsFolder){let r=a[1],s=this.settings.cachedAccounts.find(o=>o.name.replace(/[<>:"/\\|?*]/g,"_").trim()===r);s?t={id:s.id,name:s.name}:t={id:"",name:r}}}}new Y(this.app,this,t).open()}async syncAccounts(e=!1){e||new u.Notice("Syncing Salesforce accounts...");try{let n=(await(0,u.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!n.success||!n.accounts){e||new u.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=n.accounts.map(a=>({id:a.id,name:a.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),e||new u.Notice(`Synced ${n.accounts.length} accounts for matching`)}catch(t){e||new u.Notice(`Failed to sync accounts: ${t.message}`)}}async scanLocalAccountFolders(){try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!e||!(e instanceof u.TFolder))return;let t=[];for(let n of e.children)n instanceof u.TFolder&&t.push({id:`local-${n.name.replace(/\s+/g,"-").toLowerCase()}`,name:n.name});this.settings.cachedAccounts=t,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(e){console.error("Failed to scan local account folders:",e)}}async refreshAccountFolders(){if(!this.settings.userEmail)throw new Error("Please configure your email first");let e=new F(this.settings.serverUrl);if((await e.getAccountsForUser(this.settings.userEmail)).length===0)return console.log("[Eudia] No accounts found for user"),0;let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),a=[];if(n&&n instanceof u.TFolder)for(let o of n.children)o instanceof u.TFolder&&a.push(o.name);let r=await e.getNewAccounts(this.settings.userEmail,a);if(r.length===0)return console.log("[Eudia] All account folders exist"),0;console.log(`[Eudia] Creating ${r.length} new account folders`);let s=await this.fetchEnrichmentData(r);return await this.createTailoredAccountFolders(r,s),r.length}async checkAndConsumeSyncFlags(){if(!this.settings.userEmail)return;let e=encodeURIComponent(this.settings.userEmail.toLowerCase().trim()),t=this.settings.serverUrl||"https://gtm-wizard.onrender.com";try{let r=((await(0,u.requestUrl)({url:`${t}/api/admin/users/${e}/sync-flags`,method:"GET"})).json?.flags||[]).filter(o=>!o.consumed_at);if(r.length===0)return;console.log(`[Eudia] Found ${r.length} pending sync flag(s)`);let s=!1;for(let o of r)if(o.flag==="resync_accounts"){s=!0;let c=o.payload||{},d=c.added?.length||0,l=c.removed?.length||0;console.log(`[Eudia] Sync flag: resync_accounts (+${d} / -${l})`)}else o.flag==="update_plugin"?new u.Notice("A plugin update is available. Please download the latest vault."):o.flag==="reset_setup"&&(console.log("[Eudia] Sync flag: reset_setup received"),this.settings.setupCompleted=!1,await this.saveSettings(),new u.Notice("Setup has been reset by admin. Please re-run the setup wizard."));if(s){console.log("[Eudia] Triggering account folder resync from sync flag..."),new u.Notice("Syncing account updates...");let o=await this.syncAccountFolders();o.success?new u.Notice(`Account sync complete: ${o.added} new, ${o.archived} archived`):console.log(`[Eudia] Account resync error: ${o.error}`)}try{await(0,u.requestUrl)({url:`${t}/api/admin/users/${e}/sync-flags/consume`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagIds:r.map(o=>o.id)})}),console.log(`[Eudia] Consumed ${r.length} sync flag(s)`)}catch{console.log("[Eudia] Failed to consume sync flags (will retry next startup)")}}catch{console.log("[Eudia] Sync flag check skipped (endpoint not available)")}}async syncAccountFolders(){if(!this.settings.userEmail)return{success:!1,added:0,archived:0,error:"No email configured"};let e=this.settings.userEmail.toLowerCase().trim();console.log(`[Eudia] Syncing account folders for: ${e}`);try{let t=await fetch(`${this.settings.serverUrl}/api/bl-accounts/${encodeURIComponent(e)}`);if(!t.ok){let O=await t.json().catch(()=>({}));throw new Error(O.error||`Server returned ${t.status}`)}let n=await t.json();if(!n.success||!n.accounts)throw new Error(n.error||"Invalid response from server");let a=n.meta?.userGroup||"bl",r=n.meta?.queryDescription||"accounts",s=n.meta?.region||null;console.log(`[Eudia] User group: ${a}, accounts: ${n.accounts.length} (${r})`),s&&console.log(`[Eudia] Sales Leader region: ${s}`);let o=n.accounts||[],c=n.prospectAccounts||[],d=o.length+c.length;console.log(`[Eudia] Server returned: ${o.length} active + ${c.length} prospects = ${d} total`);let l=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),h=new Map,g=`${this.settings.accountsFolder}/_Prospects`,m=this.app.vault.getAbstractFileByPath(g),p=new Map,f=new Map;if(l&&l instanceof u.TFolder)for(let O of l.children)O instanceof u.TFolder&&!O.name.startsWith("_")&&h.set(O.name.toLowerCase().trim(),O);if(m&&m instanceof u.TFolder)for(let O of m.children)O instanceof u.TFolder?p.set(O.name.toLowerCase().trim(),O):O instanceof u.TFile&&O.extension==="md"&&f.set(O.basename.toLowerCase().trim(),O);let S=new Set(o.map(O=>O.name.toLowerCase().trim())),v=o.filter(O=>{let W=O.name.toLowerCase().trim();return!h.has(W)}),C=c.filter(O=>{let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();return!p.has(W)&&!f.has(W)&&!h.has(O.name.toLowerCase().trim())}),w=[];for(let O of o){let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();(p.has(W)||f.has(W))&&!h.has(O.name.toLowerCase().trim())&&w.push(O)}let D=new Set([...o.map(O=>O.name.toLowerCase().trim()),...c.map(O=>O.name.toLowerCase().trim())]),E=[];if(a==="bl")for(let[O,W]of h.entries())D.has(O)||E.push(W);let $=0,R=0,H=0,q=0;if(w.length>0){console.log(`[Eudia] Promoting ${w.length} accounts from prospect to active`);for(let O of w){let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim(),b=p.get(W.toLowerCase()),A=f.get(W.toLowerCase());try{if(b){let P=`${this.settings.accountsFolder}/${W}`;await this.app.vault.rename(b,P),H++,new u.Notice(`${O.name} promoted to active`)}else if(A){await this.app.vault.delete(A);let P=[{id:O.id,name:O.name,type:O.customerType,isOwned:!0,hadOpportunity:!0}],me=await this.fetchEnrichmentData(P);await this.createTailoredAccountFolders(P,me),H++,new u.Notice(`${O.name} promoted to active -- full account folder created`)}}catch(P){console.error(`[Eudia] Failed to promote ${O.name}:`,P)}}}if(v.length>0){console.log(`[Eudia] Creating ${v.length} new active account folders for ${a}`);let O=new Set(w.map(b=>b.name.toLowerCase().trim())),W=v.filter(b=>!O.has(b.name.toLowerCase().trim()));if(W.length>0){let b=W.map(A=>({id:A.id,name:A.name,type:A.customerType,isOwned:a==="bl",ownerName:A.ownerName,hadOpportunity:!0}));if(a==="admin"||a==="exec")await this.createAdminAccountFolders(b);else{let A=await this.fetchEnrichmentData(b);await this.createTailoredAccountFolders(b,A)}$=W.length}this.telemetry&&this.telemetry.reportInfo("Accounts synced - added",{count:$,userGroup:a,region:s||void 0})}return C.length>0&&a==="bl"&&(console.log(`[Eudia] Creating ${C.length} new prospect files`),q=await this.createProspectAccountFiles(C.map(O=>({id:O.id,name:O.name,type:"Prospect",hadOpportunity:!1,website:O.website,industry:O.industry})))),this.settings.archiveRemovedAccounts&&E.length>0&&(console.log(`[Eudia] Archiving ${E.length} removed account folders`),R=await this.archiveAccountFolders(E),this.telemetry&&this.telemetry.reportInfo("Accounts synced - archived",{count:R})),console.log(`[Eudia] Sync complete: ${$} active added, ${q} prospects added, ${H} promoted, ${R} archived (group: ${a})`),{success:!0,added:$+q+H,archived:R,userGroup:a}}catch(t){return console.error("[Eudia] Account sync error:",t),this.telemetry&&this.telemetry.reportError("Account sync failed",{error:t.message}),{success:!1,added:0,archived:0,error:t.message}}}async archiveAccountFolders(e){let t=0,n=`${this.settings.accountsFolder}/_Archived`;this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);for(let r of e)try{let s=`${n}/${r.name}`;if(this.app.vault.getAbstractFileByPath(s)){let l=new Date().toISOString().split("T")[0];await this.app.fileManager.renameFile(r,`${n}/${r.name}_${l}`)}else await this.app.fileManager.renameFile(r,s);let c=`${n}/${r.name}/_archived.md`,d=`---
...
`;try{await this.app.vault.create(c,d)}catch{}t++,console.log(`[Eudia] Archived: ${r.name}`)}catch(s){console.error(`[Eudia] Failed to archive ${r.name}:`,s)}return t}async syncSpecificNoteToSalesforce(e){let t=await this.app.vault.read(e),n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let a=n.account_id,r=n.account;if(!a&&r){let s=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===r.toLowerCase());s&&(a=s.id)}if(!a){let s=e.path.split("/");if(s.length>=2&&s[0]===this.settings.accountsFolder){let o=s[1]==="_Prospects"&&s.length>=3?s[2]:s[1],c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&(a=c.id,r=c.name)}}if(!a)return{success:!1,error:`Could not determine account for ${e.path}`};try{let s=await(0,u.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:a,accountName:r,noteTitle:e.basename,notePath:e.path,content:t,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return s.json?.success?{success:!0}:{success:!1,error:s.json?.error||"Unknown error",authRequired:s.json?.authRequired}}catch(s){return{success:!1,error:s.message}}}async syncNoteToSalesforce(){let e=this.app.workspace.getActiveFile();if(!e){new u.Notice("No active file to sync");return}if(!this.app.metadataCache.getFileCache(e)?.frontmatter?.sync_to_salesforce){new u.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}new u.Notice("Syncing to Salesforce...");let n=await this.syncSpecificNoteToSalesforce(e);n.success?new u.Notice("Synced to Salesforce"):n.authRequired?new u.Notice("Salesforce authentication required. Please reconnect."):new u.Notice("Failed to sync: "+(n.error||"Unknown error"))}async checkAndAutoEnrich(){let e=this.settings.accountsFolder||"Accounts",t=this.app.vault.getAbstractFileByPath(e);if(!t||!(t instanceof u.TFolder))return;let n=[];for(let a of t.children){if(!(a instanceof u.TFolder)||a.name.startsWith("_"))continue;let r=`${a.path}/Contacts.md`,s=this.app.vault.getAbstractFileByPath(r);if(!(!s||!(s instanceof u.TFile))){if(this.app.metadataCache.getFileCache(s)?.frontmatter?.enriched_at)continue}let o=a.name,c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&c.id&&n.push({id:c.id,name:c.name,owner:"",ownerEmail:""})}if(n.length===0){console.log("[Eudia] Auto-enrich: all account folders already enriched");return}console.log(`[Eudia] Auto-enrich: ${n.length} accounts need enrichment`);try{await this.enrichAccountFolders(n)}catch(a){console.error("[Eudia] Auto-enrich failed:",a)}}async enrichAccountFolders(e){if(!e||e.length===0)return;let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=this.settings.accountsFolder||"Accounts",a=e.filter(l=>l.id&&l.id.length>=15);if(a.length===0)return;let r=a.length,s=0,o=0;console.log(`[Eudia Enrich] Starting enrichment for ${r} accounts`),new u.Notice(`Enriching account data: 0/${r}...`);let c=20;for(let l=0;l<a.length;l+=c){let h=a.slice(l,l+c),g=h.map(p=>p.id);try{let p=await(0,u.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:g,userEmail:this.settings.userEmail})});if(p.json?.success&&p.json?.enrichments){let f=p.json.enrichments;for(let S of h){let v=f[S.id];if(v)try{await this.writeEnrichmentToAccount(S,v,n),s++}catch(C){o++,console.error(`[Eudia Enrich] Write failed for ${S.name}:`,C)}}}}catch(p){o+=h.length,console.error("[Eudia Enrich] Batch fetch failed:",p)}let m=Math.min(l+c,r);new u.Notice(`Enriching account data: ${m}/${r}...`),l+c<a.length&&await new Promise(p=>setTimeout(p,100))}let d=o>0?`Enrichment complete: ${s} enriched, ${o} skipped`:`Enrichment complete: ${s} accounts enriched with Salesforce data`;console.log(`[Eudia Enrich] ${d}`),new u.Notice(d)}async writeEnrichmentToAccount(e,t,n){let a=e.name.replace(/[<>:"/\\|?*]/g,"_").trim(),r=`${n}/${a}`,s=this.app.vault.getAbstractFileByPath(r);if(s instanceof u.TFolder||(r=`${n}/_Prospects/${a}`,s=this.app.vault.getAbstractFileByPath(r)),!(s instanceof u.TFolder))return;let o=new Date().toISOString(),c=async(d,l)=>{let h=`${r}/${d}`,g=this.app.vault.getAbstractFileByPath(h);if(!(g instanceof u.TFile))return;let m=await this.app.vault.read(g),p="",f=m;if(m.startsWith("---")){let w=m.indexOf("---",3);w!==-1&&(p=m.substring(0,w+3),f=m.substring(w+3),p.includes("enriched_at:")?p=p.replace(/enriched_at:.*/,`enriched_at: "${o}"`):p=p.substring(0,w)+`enriched_at: "${o}"
---`)}let S=f.match(/^(\s*#[^\n]+)/),C=`${S?S[1]:""}
...
${S.join(`
`)}
`)}}}startSalesforceSyncScanner(){if(!this.settings.sfAutoSyncEnabled){console.log("[Eudia SF Sync] Auto-sync is disabled in settings"),this.updateSfSyncStatusBar("SF Sync: Off");return}let e=(this.settings.sfAutoSyncIntervalMinutes||15)*60*1e3;console.log(`[Eudia SF Sync] Starting scanner \u2014 interval: ${this.settings.sfAutoSyncIntervalMinutes}min`),this.updateSfSyncStatusBar("SF Sync: Idle");let t=window.setTimeout(()=>{this.runSalesforceSyncScan()},3e4);this.registerInterval(t),this.sfSyncIntervalId=window.setInterval(()=>{this.runSalesforceSyncScan()},e),this.registerInterval(this.sfSyncIntervalId)}async runSalesforceSyncScan(){if(!(!this.settings.sfAutoSyncEnabled||!this.settings.userEmail)){console.log("[Eudia SF Sync] Running scan...");try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!(e instanceof u.TFolder)){console.log("[Eudia SF Sync] Accounts folder not found");return}let t=[],n=l=>{for(let h of l.children)h instanceof u.TFile&&h.extension==="md"?t.push(h):h instanceof u.TFolder&&n(h)};n(e);let a=[];for(let l of t){let g=this.app.metadataCache.getFileCache(l)?.frontmatter;if(!g?.sync_to_salesforce)continue;let m=g.last_sf_sync?new Date(g.last_sf_sync).getTime():0;l.stat.mtime>m&&a.push(l)}if(a.length===0){console.log("[Eudia SF Sync] No flagged notes need syncing"),this.updateSfSyncStatusBar("SF Sync: Idle");return}console.log(`[Eudia SF Sync] ${a.length} note(s) queued for sync`),this.updateSfSyncStatusBar(`SF Sync: Syncing ${a.length}...`);let r=0,s=0;for(let l of a){let h=await this.syncSpecificNoteToSalesforce(l);if(h.success)r++,await this.updateNoteSyncTimestamp(l);else if(s++,console.log(`[Eudia SF Sync] Failed to sync ${l.path}: ${h.error}`),h.authRequired){new u.Notice("Salesforce authentication expired. Please reconnect to resume auto-sync."),this.updateSfSyncStatusBar("SF Sync: Auth required");return}}let c=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),d=s>0?`SF Sync: ${r} synced, ${s} failed at ${c}`:`SF Sync: ${r} note${r!==1?"s":""} synced at ${c}`;console.log(`[Eudia SF Sync] ${d}`),this.updateSfSyncStatusBar(d),r>0&&new u.Notice(d)}catch(e){console.error("[Eudia SF Sync] Scan error:",e),this.updateSfSyncStatusBar("SF Sync: Error")}}}async updateNoteSyncTimestamp(e){try{let t=await this.app.vault.read(e),n=new Date().toISOString();if(t.startsWith("---")){let a=t.indexOf("---",3);if(a!==-1){let r=t.substring(0,a),s=t.substring(a);if(r.includes("last_sf_sync:")){let o=r.replace(/last_sf_sync:.*/,`last_sf_sync: "${n}"`)+s;await this.app.vault.modify(e,o)}else{let o=r+`last_sf_sync: "${n}"
`+s;await this.app.vault.modify(e,o)}}}}catch(t){console.error(`[Eudia SF Sync] Failed to update sync timestamp for ${e.path}:`,t)}}updateSfSyncStatusBar(e){this.sfSyncStatusBarEl&&this.sfSyncStatusBarEl.setText(e)}async createMeetingNote(){return new Promise(e=>{new ee(this.app,this,async n=>{if(!n){e();return}let a=new Date().toISOString().split("T")[0],r=n.name.replace(/[<>:"/\\|?*]/g,"_").trim(),s=`${this.settings.accountsFolder}/${r}`,o=`${a} Meeting.md`,c=`${s}/${o}`;this.app.vault.getAbstractFileByPath(s)||await this.app.vault.createFolder(s);let d=`---
...
`;await this.app.vault.modify(e,s)}async updateTeamPerformanceNote(e,t){if(!t)return;let n=new Date().toISOString().split("T")[0],a=s=>s>0?`\u2191 ${Math.abs(s).toFixed(1)}%`:s<0?`\u2193 ${Math.abs(s).toFixed(1)}%`:"--",r=`---
type: analytics_dashboard
auto_refresh: true
...
`;await this.app.vault.modify(e,r)}},ae=class extends u.PluginSettingTab{constructor(i,e){super(i,e),this.plugin=e}display(){let{containerEl:i}=this;i.empty(),i.createEl("h2",{text:"Eudia Sync & Scribe"}),i.createEl("h3",{text:"Your Profile"});let e=i.createDiv();e.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let t=e.createDiv();t.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let n=t.createSpan(),a=t.createSpan(),r=e.createDiv();r.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",r.setText("Connect with Salesforce to sync notes with your user attribution.");let s=e.createEl("button");s.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,c=async()=>{if(!this.plugin.settings.userEmail)return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",a.setText("Enter email above first"),s.setText("Setup Required"),s.disabled=!0,s.style.opacity="0.5",s.style.cursor="not-allowed",!1;s.disabled=!1,s.style.opacity="1",s.style.cursor="pointer";try{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",a.setText("Checking..."),(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",a.setText("Connected to Salesforce"),s.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",a.setText("Not connected"),s.setText("Connect to Salesforce"),!1)}catch{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",a.setText("Status unavailable"),s.setText("Connect to Salesforce"),!1}};new u.Setting(i).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(m=>m.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async p=>{let f=p.trim().toLowerCase();this.plugin.settings.userEmail=f,await this.plugin.saveSettings(),await c()})),new u.Setting(i).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(m=>{pe.forEach(p=>{m.addOption(p.value,p.label)}),m.setValue(this.plugin.settings.timezone),m.onChange(async p=>{this.plugin.settings.timezone=p,await this.plugin.saveSettings(),this.plugin.calendarService?.setTimezone(p),new u.Notice(`Timezone set to ${pe.find(f=>f.value===p)?.label||p}`)})}),i.createEl("h3",{text:"Salesforce Connection"}),i.appendChild(e);let d=()=>{o&&window.clearInterval(o);let m=0,p=30;o=window.setInterval(async()=>{m++,await c()?(o&&(window.clearInterval(o),o=null),new u.Notice("Salesforce connected successfully!")):m>=p&&o&&(window.clearInterval(o),o=null)},5e3)};s.onclick=async()=>{if(!this.plugin.settings.userEmail){new u.Notice("Please enter your email first");return}let m=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(m,"_blank"),new u.Notice("Complete the Salesforce login in the popup window",5e3),d()},c(),i.createEl("h3",{text:"Server"}),new u.Setting(i).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(m=>m.setValue(this.plugin.settings.serverUrl).onChange(async p=>{this.plugin.settings.serverUrl=p,await this.plugin.saveSettings()}));let l=i.createDiv({cls:"settings-advanced-collapsed"}),h=l.createDiv({cls:"eudia-transcription-status"});h.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",h.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?h.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':h.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{h.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let g=new u.Setting(i).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(m=>m.setValue(!1).onChange(p=>{l.style.display=p?"block":"none"}));l.style.display="none",i.createEl("h3",{text:"Transcription"}),new u.Setting(i).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(m=>m.setValue(this.plugin.settings.saveAudioFiles).onChange(async p=>{this.plugin.settings.saveAudioFiles=p,await this.plugin.saveSettings()})),new u.Setting(i).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(m=>m.setValue(this.plugin.settings.appendTranscript).onChange(async p=>{this.plugin.settings.appendTranscript=p,await this.plugin.saveSettings()})),i.createEl("h3",{text:"Sync"}),new u.Setting(i).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(m=>m.setValue(this.plugin.settings.syncOnStartup).onChange(async p=>{this.plugin.settings.syncOnStartup=p,await this.plugin.saveSettings()})),new u.Setting(i).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(m=>m.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async p=>{this.plugin.settings.autoSyncAfterTranscription=p,await this.plugin.saveSettings()})),new u.Setting(i).setName("Auto-Sync Flagged Notes").setDesc("Periodically push notes with sync_to_salesforce: true to Salesforce").addToggle(m=>m.setValue(this.plugin.settings.sfAutoSyncEnabled).onChange(async p=>{this.plugin.settings.sfAutoSyncEnabled=p,await this.plugin.saveSettings(),p?this.plugin.startSalesforceSyncScanner():this.plugin.updateSfSyncStatusBar("SF Sync: Off")})),new u.Setting(i).setName("Auto-Sync Interval").setDesc("How often to scan for flagged notes (in minutes)").addDropdown(m=>{m.addOption("5","Every 5 minutes"),m.addOption("15","Every 15 minutes"),m.addOption("30","Every 30 minutes"),m.setValue(String(this.plugin.settings.sfAutoSyncIntervalMinutes)),m.onChange(async p=>{this.plugin.settings.sfAutoSyncIntervalMinutes=parseInt(p),await this.plugin.saveSettings(),new u.Notice(`SF auto-sync interval set to ${p} minutes. Restart Obsidian for changes to take effect.`)})}),i.createEl("h3",{text:"Folders"}),new u.Setting(i).setName("Accounts Folder").setDesc("Where account folders are stored").addText(m=>m.setValue(this.plugin.settings.accountsFolder).onChange(async p=>{this.plugin.settings.accountsFolder=p||"Accounts",await this.plugin.saveSettings()})),new u.Setting(i).setName("Recordings Folder").setDesc("Where audio files are saved").addText(m=>m.setValue(this.plugin.settings.recordingsFolder).onChange(async p=>{this.plugin.settings.recordingsFolder=p||"Recordings",await this.plugin.saveSettings()})),i.createEl("h3",{text:"Actions"}),new u.Setting(i).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(m=>m.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new u.Setting(i).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(m=>m.setButtonText("Refresh Folders").onClick(async()=>{m.setButtonText("Checking..."),m.setDisabled(!0);try{let p=await this.plugin.refreshAccountFolders();p>0?new u.Notice(`Created ${p} new account folder${p>1?"s":""}`):new u.Notice("All account folders are up to date")}catch(p){new u.Notice("Failed to refresh folders: "+p.message)}m.setButtonText("Refresh Folders"),m.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&i.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),i.createEl("p",{text:`Audio transcription: ${k.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
