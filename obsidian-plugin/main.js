...
`;try{let w=await this.app.vault.create(u,y);await this.app.workspace.getLeaf().openFile(w);try{let g=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;g?.revealInFolder&&g.revealInFolder(w)}catch{}new p.Notice(`Created pipeline note: ${c}`),console.log(`[Eudia Pipeline] Created pipeline meeting note: ${u}`)}catch(w){console.error("[Eudia Pipeline] Failed to create pipeline note:",w),new p.Notice(`Could not create pipeline note: ${w.message||"Unknown error"}`)}}},K=class extends p.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null;this.micRibbonIcon=null;this.liveTranscript="";this.liveTranscriptChunkInterval=null;this.isTranscribingChunk=!1;this.sfSyncStatusBarEl=null;this.sfSyncIntervalId=null}async onload(){await this.loadSettings(),this.transcriptionService=new V(this.settings.serverUrl),this.calendarService=new F(this.settings.serverUrl,this.settings.userEmail,this.settings.timezone||"America/New_York"),this.smartTagService=new q,k.setupDisplayMediaHandler().then(t=>{console.log(t?"[Eudia] System audio: loopback handler ready":"[Eudia] System audio: handler not available, will try other strategies on record")}).catch(()=>{}),this.checkForPluginUpdate(),this.registerInterval(window.setInterval(()=>this.checkForPluginUpdate(),2*60*60*1e3)),this.registerView(U,t=>new ae(t,this)),this.registerView(H,t=>new ne(t,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.micRibbonIcon=this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.registerEvent(this.app.vault.on("create",async t=>{if(!(t instanceof p.TFile)||t.extension!=="md")return;let e=this.settings.accountsFolder||"Accounts";if(!t.path.startsWith(e+"/")||!t.basename.startsWith("Untitled"))return;let n=t.path.split("/");if(n.length<3)return;let s=n[1],r=n.slice(0,2).join("/"),i="",o=["Contacts.md","Note 1.md","Intelligence.md"];for(let g of o){let f=this.app.vault.getAbstractFileByPath(`${r}/${g}`);if(f instanceof p.TFile)try{let b=(await this.app.vault.read(f)).match(/account_id:\s*"?([^"\n]+)"?/);if(b){i=b[1].trim();break}}catch{}}let c=this.app.vault.getAbstractFileByPath(r),l=0;if(c&&c.children)for(let g of c.children){let f=g.name?.match(/^Note\s+(\d+)/i);f&&(l=Math.max(l,parseInt(f[1])))}let d=l+1,u=new Date,m=u.toLocaleDateString("en-US",{month:"short",day:"numeric"}),h=u.toISOString().split("T")[0],y=`Note ${d} - ${m}.md`,w=`---
account: "${s}"
account_id: "${i}"
type: meeting_note
sync_to_salesforce: false
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let u of r){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,v=>v.toUpperCase()),h=u.split("@")[0].replace("."," ").toLowerCase(),y=h.split(" ")[0],w=h.split(" ").pop()||"",g=e.filter(v=>{let S=(v.csmName||"").toLowerCase();if(S&&(S.includes(y)||S.includes(w)))return!0;let W=(v.ownerName||"").toLowerCase();return W.includes(y)||W.includes(w)}),f=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
...
|---------|------|-------|--------|
`;for(let v of g){let S=v.name.replace(/[<>:"/\\|?*]/g,"_").trim();f+=`| ${v.name} | ${v.type||""} | ${v.ownerName||""} | [[Accounts/${S}/Contacts\\|View]] |
...
`).filter(w=>w.match(/^- \[[ x]\]/)).slice(0,5);(y.length>0||h!=="Unknown")&&s.push({account:c.name,lastUpdated:h,nextSteps:y})}}s.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let r=new Date().toISOString().split("T")[0],i=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${r}
---
