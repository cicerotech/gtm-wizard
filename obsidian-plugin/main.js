...
- Action items have clear owners`}function be(y){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${y?`
...
`;let i=t.talkTime.repPercent,s=t.talkTime.customerPercent,o=t.talkTime.isHealthyRatio?"\u2705":"\u26A0\uFE0F";n+=`**Talk Time:** Rep ${i}% / Customer ${s}% ${o}
`;let c=Math.round(i/5),d=Math.round(s/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
...
`}return n}static formatSectionsWithAudio(a,e,t,n){let r=this.formatSectionsForNote(a,e,n);return t&&(r+=`
---

## Recording

![[${t}]]
`),r}static formatContextForNote(a){if(!a.success)return"";let e=`## Pre-Call Context
...
`,e}async blobToBase64(a){return new Promise((e,t)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];e(i)},n.onerror=t,n.readAsDataURL(a)})}async transcribeAudio(a,e){try{let t=await this.blobToBase64(a),n=a.type||"audio/webm",r=e?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:e.pipelineContext}:void 0,i=await this.transcribeAndSummarize(t,n,e?.accountName,e?.accountId,r);return{text:i.transcript,confidence:i.success?.95:0,duration:i.duration,sections:i.sections}}catch(t){return console.error("transcribeAudio error:",t),{text:"",confidence:0,duration:0,sections:this.getEmptySections()}}}async processTranscription(a,e){if(!a||a.trim().length===0)return this.getEmptySections();try{let t=await(0,k.requestUrl)({url:`${this.serverUrl}/api/process-sections`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:a,accountName:e?.accountName,context:e})});if(t.json?.success&&t.json?.sections){let n=t.json.sections;return{summary:n.summary||"",painPoints:n.painPoints||n.keyPoints||"",productInterest:n.productInterest||"",meddiccSignals:n.meddiccSignals||"",nextSteps:n.nextSteps||"",actionItems:n.actionItems||"",keyDates:n.keyDates||"",buyingTriggers:n.buyingTriggers||"",dealSignals:n.dealSignals||"",risksObjections:n.risksObjections||n.concerns||"",competitiveIntel:n.competitiveIntel||"",attendees:n.attendees||"",transcript:a}}return console.warn("Server process-sections returned no sections, using fallback"),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:a}}catch(t){return console.error("processTranscription server error:",t),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:a}}}};var G=require("obsidian"),T=class y{constructor(a,e,t="America/New_York"){this.serverUrl=a,this.userEmail=e.toLowerCase(),this.timezone=t}setUserEmail(a){this.userEmail=a.toLowerCase()}setServerUrl(a){this.serverUrl=a}setTimezone(a){this.timezone=a}async getTodaysMeetings(a=!1){if(!this.userEmail)return{success:!1,date:new Date().toISOString().split("T")[0],email:"",meetingCount:0,meetings:[],error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=a?"&forceRefresh=true":"";return(await(0,G.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/today?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch today's meetings:",e),{success:!1,date:new Date().toISOString().split("T")[0],email:this.userEmail,meetingCount:0,meetings:[],error:e.message||"Failed to fetch calendar"}}}async getWeekMeetings(a=!1){if(!this.userEmail)return{success:!1,startDate:"",endDate:"",email:"",totalMeetings:0,byDay:{},error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=a?"&forceRefresh=true":"";return(await(0,G.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/week?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch week's meetings:",e),{success:!1,startDate:"",endDate:"",email:this.userEmail,totalMeetings:0,byDay:{},error:e.message||"Failed to fetch calendar"}}}async getMeetingsInRange(a,e){if(!this.userEmail)return[];try{let t=a.toISOString().split("T")[0],n=e.toISOString().split("T")[0],r=await(0,G.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/range?start=${t}&end=${n}`,method:"GET",headers:{Accept:"application/json"}});return r.json.success?r.json.meetings||[]:[]}catch(t){return console.error("Failed to fetch calendar range:",t),[]}}async getCurrentMeeting(){let a=await this.getTodaysMeetings();if(!a.success||a.meetings.length===0)return{meeting:null,isNow:!1};let e=new Date;for(let t of a.meetings){let n=y.safeParseDate(t.start),r=y.safeParseDate(t.end);if(e>=n&&e<=r)return{meeting:t,isNow:!0};let i=(n.getTime()-e.getTime())/(1e3*60);if(i>0&&i<=15)return{meeting:t,isNow:!1,minutesUntilStart:Math.ceil(i)}}return{meeting:null,isNow:!1}}async getMeetingsForAccount(a){let e=await this.getWeekMeetings();if(!e.success)return[];let t=[];Object.values(e.byDay).forEach(r=>{t.push(...r)});let n=a.toLowerCase();return t.filter(r=>r.accountName?.toLowerCase().includes(n)||r.subject.toLowerCase().includes(n)||r.attendees.some(i=>i.email.toLowerCase().includes(n.split(" ")[0])))}static formatMeetingForNote(a){let e=a.attendees.filter(t=>t.isExternal!==!1).map(t=>t.name||t.email.split("@")[0]).slice(0,5).join(", ");return{title:a.subject,attendees:e,meetingStart:a.start,accountName:a.accountName}}static getDayName(a){let e;a.length===10&&a.includes("-")?e=new Date(a+"T00:00:00"):e=new Date(a);let t=new Date;t.setHours(0,0,0,0);let n=new Date(e);n.setHours(0,0,0,0);let r=Math.round((n.getTime()-t.getTime())/(1e3*60*60*24));return r===0?"Today":r===1?"Tomorrow":e.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}static formatTime(a,e){let t=a;t&&!t.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(t)&&(t=t+"Z");let n=new Date(t);if(isNaN(n.getTime()))return a;let r={hour:"numeric",minute:"2-digit",hour12:!0};return e&&(r.timeZone=e),n.toLocaleTimeString("en-US",r)}static safeParseDate(a){if(!a)return new Date(NaN);let e=a;return!e.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(e)&&(e=e+"Z"),new Date(e)}static getMeetingDuration(a,e){let t=y.safeParseDate(a),n=y.safeParseDate(e);return Math.round((n.getTime()-t.getTime())/(1e3*60))}};var We=["ai-contracting-tech","ai-contracting-services","ai-compliance-tech","ai-compliance-services","ai-ma-tech","ai-ma-services","sigma"],je=["metrics-identified","economic-buyer-identified","decision-criteria-discussed","decision-process-discussed","pain-confirmed","champion-identified","competition-mentioned"],Ae=["progressing","stalled","at-risk","champion-engaged","early-stage"],xe=["discovery","demo","negotiation","qbr","implementation","follow-up"],Ee=`You are a sales intelligence tagger for Eudia, an AI legal technology company. Extract structured tags from meeting analysis.
...
`).replace(/\n/g,"<br>")}onClose(){this.contentEl.empty()}};var te=class extends u.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new F(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return N}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}if(this.plugin.settings.accountsImported){this.steps[2].status="complete";try{let e=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");e&&await this.plugin.app.vault.delete(e)}catch{}}else{console.log("[Eudia Setup] Email set but accounts not imported \u2014 auto-retrying import...");let e=this.plugin.app.workspace.leftSplit,t=e?.collapsed;try{let n=this.plugin.settings.userEmail,r=I(n)?"admin":M(n)?"cs":"bl",i=[],s=[];if(console.log(`[Eudia Setup] Auto-retry for ${n} (group: ${r})`),r==="admin")i=await this.accountOwnershipService.getAllAccountsForAdmin(n);else if(r==="cs"){let o=await this.accountOwnershipService.getCSAccounts(n);i=o.accounts,s=o.prospects,console.log(`[Eudia Setup] Auto-retry CS result: ${i.length} accounts, ${s.length} prospects`)}else{let o=await this.accountOwnershipService.getAccountsWithProspects(n);i=o.accounts,s=o.prospects}if(i.length>0||s.length>0){if(e&&!t&&e.collapse(),r==="admin"?await this.plugin.createAdminAccountFolders(i):(await this.plugin.createTailoredAccountFolders(i,{}),s.length>0&&await this.plugin.createProspectAccountFiles(s)),z(n))try{await this.plugin.createCSManagerDashboard(n,i)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=i.length+s.length,await this.plugin.saveSettings(),this.steps[2].status="complete";try{let c=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");c&&await this.plugin.app.vault.delete(c)}catch{}e&&!t&&e.expand(),new u.Notice(`Imported ${i.length} accounts!`),console.log(`[Eudia Setup] Auto-retry imported ${i.length} accounts for ${n}`);let o=[...i,...s];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(o)}catch{}},500)}else console.warn(`[Eudia Setup] Auto-retry returned 0 accounts for ${n}. Server may still be starting.`),e&&!t&&e.expand()}catch(n){console.error("[Eudia Setup] Auto-retry account import failed:",n),e&&!t&&e.expand()}}}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),n=t.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to unlock your sales superpowers",cls:"eudia-setup-subtitle"});let r=t.createDiv({cls:"eudia-setup-progress-section"}),i=this.getCompletionPercentage(),s=r.createDiv({cls:"eudia-setup-progress-label"});s.createSpan({text:"Setup Progress"}),s.createSpan({text:`${i}%`,cls:"eudia-setup-progress-value"});let c=r.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${i}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),i=r.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"1"),t.status==="complete"&&i.addClass("eudia-step-complete");let s=r.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:t.title}),s.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let d=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});d.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async l=>{l.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let n=await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new u.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");let r=this.plugin.app.workspace.leftSplit,i=r?.collapsed;r&&!i&&r.collapse();try{let s,o=[],c=I(e)?"admin":M(e)?"cs":"bl";if(console.log(`[Eudia] User group detected: ${c} for ${e}`),c==="admin")console.log("[Eudia] Admin user detected - importing all accounts"),s=await this.accountOwnershipService.getAllAccountsForAdmin(e);else if(c==="cs"){console.log("[Eudia] CS user detected - importing CS-relevant accounts"),t&&(t.textContent="Loading Customer Success accounts...");let d=await this.accountOwnershipService.getCSAccounts(e);s=d.accounts,o=d.prospects,console.log(`[Eudia] CS accounts received: ${s.length} active, ${o.length} prospects`)}else{let d=await this.accountOwnershipService.getAccountsWithProspects(e);s=d.accounts,o=d.prospects}if(s.length>0||o.length>0){if(t&&(t.textContent=`Creating ${s.length} account folders...`),c==="admin"?await this.plugin.createAdminAccountFolders(s):(await this.plugin.createTailoredAccountFolders(s,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o)),z(e))try{await this.plugin.createCSManagerDashboard(e,s)}catch(l){console.error("[Eudia] CS Manager dashboard creation failed (non-blocking):",l)}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+o.length,await this.plugin.saveSettings();try{let l=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");l&&await this.plugin.app.vault.delete(l)}catch{}new u.Notice(`Imported ${s.length} active accounts + ${o.length} prospects!`);let d=[...s,...o];setTimeout(async()=>{try{t&&(t.textContent="Loading Salesforce intelligence...",t.className="eudia-setup-validation-message loading"),await this.plugin.enrichAccountFolders(d)}catch(l){console.log("[Eudia] Background enrichment skipped:",l)}},500)}else console.warn(`[Eudia] No accounts returned for ${e} (userGroup: ${c})`),t&&(t.textContent=`No accounts found for ${e}. The server may still be starting \u2014 try again in 30 seconds.`,t.className="eudia-setup-validation-message warning"),new u.Notice("No accounts found. Server may be cold-starting \u2014 please try again shortly.")}catch(s){console.error("[Eudia] Account import failed:",s),t&&(t.textContent="Account import failed. Please try again.",t.className="eudia-setup-validation-message error"),new u.Notice("Account import failed \u2014 please try again.")}finally{r&&!i&&r.expand()}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),i=r.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"2"),t.status==="complete"&&i.addClass("eudia-step-complete");let s=r.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:t.title}),s.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let d=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),l=o.createDiv({cls:"eudia-setup-sf-status"});d.onclick=async()=>{let p=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(p,"_blank"),l.textContent="Complete the login in the popup window...",l.className="eudia-setup-sf-status loading",new u.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(l)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,n=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new u.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=this.plugin.settings.userEmail,n=I(t)?"admin":M(t)?"cs":"bl";console.log(`[Eudia SF Import] Importing for ${t} (group: ${n})`);let r,i=[];if(n==="admin")console.log("[Eudia] Admin user detected - importing all accounts"),e.textContent="Admin detected - importing all accounts...",r=await this.accountOwnershipService.getAllAccountsForAdmin(t);else if(n==="cs"){console.log("[Eudia] CS user detected - importing CS-relevant accounts"),e.textContent="Loading Customer Success accounts...";let p=await this.accountOwnershipService.getCSAccounts(t);r=p.accounts,i=p.prospects,console.log(`[Eudia SF Import] CS result: ${r.length} accounts, ${i.length} prospects`)}else{let p=await this.accountOwnershipService.getAccountsWithProspects(t);r=p.accounts,i=p.prospects}if(r.length===0&&i.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}e.textContent=`Creating ${r.length} account folders...`;let s=this.plugin.app.workspace.leftSplit,o=s?.collapsed;if(s&&!o&&s.collapse(),I(t)?await this.plugin.createAdminAccountFolders(r):(await this.plugin.createTailoredAccountFolders(r,{}),i.length>0&&await this.plugin.createProspectAccountFiles(i)),z(t))try{await this.plugin.createCSManagerDashboard(t,r)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=r.length+i.length,await this.plugin.saveSettings();try{let p=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");p&&await this.plugin.app.vault.delete(p)}catch{}s&&!o&&s.expand(),this.steps[2].status="complete";let c=r.filter(p=>p.isOwned!==!1).length,d=r.filter(p=>p.isOwned===!1).length;n==="admin"&&d>0?e.textContent=`${c} owned + ${d} view-only accounts imported! Enriching...`:e.textContent=`${r.length} active + ${i.length} prospect accounts imported! Enriching...`,e.className="eudia-setup-sf-status success";let l=[...r,...i];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(l);let p=l.filter(g=>g.id&&g.id.length>=15).length;e.textContent=`${r.length+i.length} accounts imported, ${p} enriched with Salesforce data`}catch(p){console.log("[Eudia] Background enrichment skipped:",p),e.textContent=`${r.length+i.length} accounts imported (enrichment will retry on next launch)`}},500)}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error",importLeftSplit&&!importWasCollapsed&&importLeftSplit.expand()}}renderTranscribeStep(e){let t=this.steps[2],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),i=r.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"3"),t.status==="complete"&&i.addClass("eudia-step-complete");let s=r.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:t.title}),s.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),d.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let l=c.createDiv({cls:"eudia-setup-instruction"});l.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),l.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let p=c.createDiv({cls:"eudia-setup-instruction"});p.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),p.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(i=>i.status==="complete")){let i=t.createDiv({cls:"eudia-setup-completion"});i.createEl("h2",{text:"\u{1F389} You're all set!"}),i.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let s=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});s.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(N),await this.plugin.activateCalendarView()}}else{let i=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});i.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(N),new u.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let r=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});r.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},ne=class extends u.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return L}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),n=t.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let r=n.createDiv({cls:"eudia-calendar-actions"}),i=r.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});i.title="Refresh (fetches latest from calendar)",i.onclick=async()=>{i.addClass("spinning"),this._forceRefresh=!0,await this.render(),i.removeClass("spinning")};let s=r.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});s.title="Settings",s.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,r=this.plugin.settings.userEmail;try{(await(0,u.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(r&&t.server==="connected")try{let l=await(0,u.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(r)}`,method:"GET",throw:!1});l.status===200&&l.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(r&&t.server==="connected")try{let l=await(0,u.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(r)}`,method:"GET",throw:!1});l.status===200&&l.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let i=e.createDiv({cls:"eudia-status-indicators"}),s=i.createSpan({cls:`eudia-status-dot ${t.server}`});s.title=t.serverMessage||"Server";let o=i.createSpan({cls:`eudia-status-dot ${t.calendar}`});o.title=t.calendarMessage||"Calendar";let c=i.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(c.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let l=e.createDiv({cls:"eudia-status-warning"});l.innerHTML=`<strong>${r}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),n=t.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let r=new T(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),i=this._forceRefresh||!1;this._forceRefresh=!1;let s=await r.getWeekMeetings(i);if(n.remove(),!s.success){this.renderError(t,s.error||"Failed to load calendar");return}let o=Object.keys(s.byDay||{}).sort();if(o.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,r);for(let c of o){let d=s.byDay[c];!d||d.length===0||this.renderDaySection(t,c,d)}}catch(r){n.remove(),this.renderError(t,r.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let n=await t.getCurrentMeeting();if(n.meeting){let r=e.createDiv({cls:"eudia-now-card"});n.isNow?r.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):r.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let i=r.createDiv({cls:"eudia-now-content"});i.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&i.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let s=r.createEl("button",{cls:"eudia-now-action",text:"Create Note"});s.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(e,t,n){let r=e.createDiv({cls:"eudia-calendar-day"});r.createEl("div",{cls:"eudia-calendar-day-header",text:T.getDayName(t)});for(let i of n){let s=r.createDiv({cls:`eudia-calendar-meeting ${i.isCustomerMeeting?"customer":"internal"}`});s.createEl("div",{cls:"eudia-calendar-time",text:T.formatTime(i.start,this.plugin.settings.timezone)});let o=s.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:i.subject}),i.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:i.accountName});else if(i.attendees&&i.attendees.length>0){let c=i.attendees.slice(0,2).map(d=>d.name||d.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}s.onclick=()=>this.createNoteForMeeting(i),s.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `}renderError(e,t){let n=e.createDiv({cls:"eudia-calendar-error"}),r="",i="Unable to load calendar",s="";t.includes("not authorized")||t.includes("403")?(r="\u{1F512}",i="Calendar Access Required",s="Contact your admin to be added to the authorized users list."):t.includes("network")||t.includes("fetch")?(r="\u{1F4E1}",i="Connection Issue",s="Check your internet connection and try again."):(t.includes("server")||t.includes("500"))&&(r="\u{1F527}",i="Server Unavailable",s="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=t.createDiv({cls:"eudia-setup-input-group"}),r=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});r.addClass("eudia-setup-email");let i=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),s=t.createDiv({cls:"eudia-setup-status"});i.onclick=async()=>{let o=r.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){s.textContent="Please use your @eudia.com email",s.className="eudia-setup-status error";return}i.disabled=!0,i.textContent="Connecting...",s.textContent="";try{if(!(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){s.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,s.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),s.textContent="Connected",s.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let d=c.message||"Connection failed";d.includes("403")?s.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:s.textContent=d,s.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect"}},r.onkeydown=o=>{o.key==="Enter"&&i.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(e){let t=e.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],r=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],i=t.filter(o=>!r.includes(o)&&o.length>1);if(i.length===0)return t[0]||"";if(i.length>1&&n.includes(i[0]))return i[1].charAt(0).toUpperCase()+i[1].slice(1);let s=i[i.length-1];return s.charAt(0).toUpperCase()+s.slice(1)}getExternalDomainsFromAttendees(e){if(!e||e.length===0)return[];let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,r=[];for(let i of e){if(!i.email)continue;let o=i.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let c=o[1];if(c.includes("eudia.com")||t.includes(c)||n.has(c))continue;n.add(c);let d=this.extractCompanyFromDomain(c);d.length>=2&&r.push({domain:c,company:d})}}return r}findBestAccountMatch(e,t,n){let r=this.plugin.settings.accountsFolder||"Accounts",i=this.app.vault.getAbstractFileByPath(r);if(!(i instanceof u.TFolder))return null;let s=[];for(let c of i.children)c instanceof u.TFolder&&s.push(c.name);if(s.length===0)return null;let o=[];for(let{domain:c,company:d}of e){let l=this.findAccountFolder(d),p=l?1:0;o.push({domain:c,company:d,folder:l,score:p})}if(o.sort((c,d)=>d.score-c.score),o.length>0&&o[0].folder){let c=o[0],d=c.folder.split("/").pop()||c.company;return console.log(`[Eudia Calendar] Best domain match: "${c.company}" from ${c.domain} -> ${c.folder}`),{folder:c.folder,accountName:d,source:"domain"}}if(t){let c=this.findAccountFolder(t);if(c){let d=c.split("/").pop()||t;return console.log(`[Eudia Calendar] Server account match: "${t}" -> ${c}`),{folder:c,accountName:d,source:"server"}}}if(n){let c=this.findAccountFolder(n);if(c){let d=c.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${c}`),{folder:c,accountName:d,source:"subject"}}}for(let{company:c}of e){let d=s.find(l=>{let p=l.toLowerCase(),g=c.toLowerCase();return p.includes(g)||g.includes(p)});if(d){let l=`${r}/${d}`;return console.log(`[Eudia Calendar] Partial domain match: "${c}" -> ${l}`),{folder:l,accountName:d,source:"domain-partial"}}}return null}extractAccountFromAttendees(e){let t=this.getExternalDomainsFromAttendees(e);if(t.length===0)return null;let n=t[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let r=(t[1]||t[2]||"").trim();if(r.toLowerCase()!=="eudia")return r}let n=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let i=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(i.toLowerCase()!=="eudia"&&i.length>0)return i}if(!e.toLowerCase().includes("eudia")){let r=e.match(/^([^-–|]+)/);if(r){let i=r[1].trim();if(i.length>2&&i.length<50)return i}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(t);if(!(n instanceof u.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let r=e.toLowerCase().trim(),i=[];for(let p of n.children)p instanceof u.TFolder&&i.push(p.name);console.log(`[Eudia Calendar] Searching for "${r}" in ${i.length} folders`);let s=i.find(p=>p.toLowerCase()===r);if(s)return console.log(`[Eudia Calendar] Exact match found: ${s}`),`${t}/${s}`;let o=i.find(p=>p.toLowerCase().startsWith(r));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${t}/${o}`;let c=i.find(p=>r.startsWith(p.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${t}/${c}`;let d=i.find(p=>{let g=p.toLowerCase();return g.length>=3&&r.includes(g)});if(d)return console.log(`[Eudia Calendar] Search contains folder match: ${d}`),`${t}/${d}`;let l=i.find(p=>{let g=p.toLowerCase();return r.length>=3&&g.includes(r)});return l?(console.log(`[Eudia Calendar] Folder contains search match: ${l}`),`${t}/${l}`):(console.log(`[Eudia Calendar] No folder match found for "${r}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",r=I(n),i=(e.attendees||[]).map(w=>w.email).filter(Boolean),s=se(e.subject,i);if(r&&s.isPipelineMeeting&&s.confidence>=60){await this._createPipelineNote(e,t);return}let o=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),c=`${t} - ${o}.md`,d=null,l=e.accountName||null,p=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(w=>w.email)||[])}`);let g=this.getExternalDomainsFromAttendees(e.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(g)}`);let h=this.extractAccountFromSubject(e.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${h||"none"}"`);let m=this.findBestAccountMatch(g,e.accountName,h||void 0);if(m&&(d=m.folder,l=m.accountName,console.log(`[Eudia Calendar] Best match (${m.source}): "${l}" -> ${d}`)),!d){let w=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(w)instanceof u.TFolder&&(d=w,console.log(`[Eudia Calendar] No match found, using Accounts root: ${d}`))}if(l){let w=this.plugin.settings.cachedAccounts.find(D=>D.name.toLowerCase()===l?.toLowerCase());w&&(p=w.id,l=w.name,console.log(`[Eudia Calendar] Matched to cached account: ${w.name} (${w.id})`))}let f=d?`${d}/${c}`:c,S=this.app.vault.getAbstractFileByPath(f);if(S instanceof u.TFile){await this.app.workspace.getLeaf().openFile(S),new u.Notice(`Opened existing note: ${c}`);return}let v=(e.attendees||[]).map(w=>w.name||w.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${e.subject}"
date: ${t}
attendees: [${v}]
account: "${l||""}"
...
`},{name:"Meeting Notes.md",content:v},{name:"Contacts.md",content:f},{name:"Intelligence.md",content:S},{name:"Next Steps.md",content:C}];for(let x of w){let $=`${p}/${x.name}`;await this.app.vault.create($,x.content)}return console.log(`[Eudia] Created account folder with subnotes${m?" (enriched)":""}: ${l}`),!0}catch(h){return console.error(`[Eudia] Failed to create folder for ${l}:`,h),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c),p=await Promise.allSettled(l.map(g=>o(g)));i+=p.filter(g=>g.status==="fulfilled"&&g.value===!0).length}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),i>0&&new u.Notice(`Created ${i} account folders`),await this.ensureNextStepsFolderExists()}buildContactsContent(e,t,n){let r=t?`
...
*Previous next steps will be archived here.*
`}async fetchEnrichmentData(e){let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=e.filter(s=>s.id&&s.id.length>=15);if(n.length===0)return{};let r={},i=20;console.log(`[Eudia Enrich] Fetching enrichment data for ${n.length} accounts`);for(let s=0;s<n.length;s+=i){let c=n.slice(s,s+i).map(d=>d.id);try{let d=await(0,u.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:c,userEmail:this.settings.userEmail})});d.json?.success&&d.json?.enrichments&&Object.assign(r,d.json.enrichments)}catch(d){console.error(`[Eudia Enrich] Batch fetch failed (batch ${s/i+1}):`,d)}s+i<n.length&&await new Promise(d=>setTimeout(d,100))}return console.log(`[Eudia Enrich] Got enrichment data for ${Object.keys(r).length}/${n.length} accounts`),r}async createProspectAccountFiles(e){if(!e||e.length===0)return 0;let t=this.settings.accountsFolder||"Accounts",n=`${t}/_Prospects`;if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let i=0;for(let s of e){let o=s.name.replace(/[<>:"/\\|?*]/g,"_").trim(),c=`${n}/${o}`;if(this.app.vault.getAbstractFileByPath(c)instanceof u.TFolder)continue;let l=`${t}/${o}`;if(this.app.vault.getAbstractFileByPath(l)instanceof u.TFolder)continue;let g=`${n}/${o}.md`,h=this.app.vault.getAbstractFileByPath(g);if(h instanceof u.TFile)try{await this.app.vault.delete(h)}catch{}try{await this.app.vault.createFolder(c);let m=new Date().toISOString().split("T")[0],f=[{name:"Note 1.md",content:`---
account: "${s.name}"
account_id: "${s.id}"
type: meeting_note
tier: prospect
...
*Previous next steps will be archived here.*
`}];for(let S of f){let v=`${c}/${S.name}`;await this.app.vault.create(v,S.content)}i++}catch(m){console.log(`[Eudia] Failed to create prospect folder for ${s.name}:`,m)}}return i>0&&console.log(`[Eudia] Created ${i} prospect account folders in _Prospects/`),i}async createCSManagerDashboard(e,t){let n="CS Manager",r=new Date().toISOString().split("T")[0],i=ue(e);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s={};for(let p of t){let g=p.ownerName||"Unassigned";s[g]||(s[g]=[]),s[g].push(p)}let o=`---
role: cs_manager
manager: "${e}"
...
`;let d=`${n}/CS Manager Overview.md`,l=this.app.vault.getAbstractFileByPath(d);l instanceof u.TFile?await this.app.vault.modify(l,o):await this.app.vault.create(d,o);for(let p of i){let g=p.split("@")[0].replace("."," ").replace(/\b\w/g,v=>v.toUpperCase()),h=t.filter(v=>{let C=(v.ownerName||"").toLowerCase(),w=p.split("@")[0].replace("."," ").toLowerCase();return C.includes(w.split(" ")[0])||C.includes(w.split(" ").pop()||"")}),m=`---
rep: "${p}"
rep_name: "${g}"
role: cs_rep_summary
...
|---------|------|
`;for(let v of h)m+=`| ${v.name} | ${v.type||""} |
...
`;let f=`${n}/${g}.md`,S=this.app.vault.getAbstractFileByPath(f);S instanceof u.TFile?await this.app.vault.modify(S,m):await this.app.vault.create(f,m)}console.log(`[Eudia] Created CS Manager dashboard for ${e} with ${t.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let r=0,i=0,s=new Date().toISOString().split("T")[0],o=async d=>{let l=d.name.replace(/[<>:"/\\|?*]/g,"_").trim(),p=`${t}/${l}`;if(this.app.vault.getAbstractFileByPath(p)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(p),await this.createExecAccountSubnotes(p,d,s),d.isOwned?r++:i++,console.log(`[Eudia Admin] Created ${d.isOwned?"owned":"view-only"} folder: ${l}`),!0}catch(h){return console.error(`[Eudia Admin] Failed to create folder for ${l}:`,h),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c);await Promise.allSettled(l.map(p=>o(p)))}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),r+i>0&&new u.Notice(`Created ${r} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let r=t.ownerName||"Unknown",i=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let s of i){let o=`${e}/${s.name}`;await this.app.vault.create(o,s.content)}}async createFullAccountSubnotes(e,t,n){let r=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let i of r){let s=`${e}/${i.name}`;await this.app.vault.create(s,i.content)}}async ensurePipelineFolderExists(){let e="Pipeline",t=`${e}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let s=`---
...
### New Opportunities
*None yet*
`;await this.app.vault.create(t,s)}}async ensureNextStepsFolderExists(){let e="Next Steps",t=`${e}/All Next Steps.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let i=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
...
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(m.length>0||h!=="Unknown")&&r.push({account:c.name,lastUpdated:h,nextSteps:m})}}r.sort((c,d)=>d.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`)}catch{}throw n}}buildPipelineNoteContent(e,t,n){let r=new Date,i=String(r.getMonth()+1).padStart(2,"0"),s=String(r.getDate()).padStart(2,"0"),o=String(r.getFullYear()).slice(-2),c=r.toISOString().split("T")[0],d=`${i}.${s}.${o}`,l=m=>m==null?"":Array.isArray(m)?m.map(String).join(`
...
`):typeof f=="object"?JSON.stringify(f):String(f),r=n(e.title)||"Meeting Notes",i=n(e.summary),s=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),l=n(e.actionItems),p=n(e.keyDates),g=n(e.risksObjections),h=n(e.attendees),m=`---
title: "${r}"
...
`;try{await this.app.vault.create(c,d)}catch{}t++,console.log(`[Eudia] Archived: ${i.name}`)}catch(s){console.error(`[Eudia] Failed to archive ${i.name}:`,s)}return t}async syncSpecificNoteToSalesforce(e){let t=await this.app.vault.read(e),n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let r=n.account_id,i=n.account;if(!r&&i){let s=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===i.toLowerCase());s&&(r=s.id)}if(!r){let s=e.path.split("/");if(s.length>=2&&s[0]===this.settings.accountsFolder){let o=s[1]==="_Prospects"&&s.length>=3?s[2]:s[1],c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&(r=c.id,i=c.name)}}if(!r)return{success:!1,error:`Could not determine account for ${e.path}`};try{let s=await(0,u.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:r,accountName:i,noteTitle:e.basename,notePath:e.path,content:t,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return s.json?.success?{success:!0}:{success:!1,error:s.json?.error||"Unknown error",authRequired:s.json?.authRequired}}catch(s){return{success:!1,error:s.message}}}async syncNoteToSalesforce(){let e=this.app.workspace.getActiveFile();if(!e){new u.Notice("No active file to sync");return}if(!this.app.metadataCache.getFileCache(e)?.frontmatter?.sync_to_salesforce){new u.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}new u.Notice("Syncing to Salesforce...");let n=await this.syncSpecificNoteToSalesforce(e);n.success?new u.Notice("Synced to Salesforce"):n.authRequired?new u.Notice("Salesforce authentication required. Please reconnect."):new u.Notice("Failed to sync: "+(n.error||"Unknown error"))}async checkAndAutoEnrich(){let e=this.settings.accountsFolder||"Accounts",t=this.app.vault.getAbstractFileByPath(e);if(!t||!(t instanceof u.TFolder))return;let n=[];for(let r of t.children){if(!(r instanceof u.TFolder)||r.name.startsWith("_"))continue;let i=`${r.path}/Contacts.md`,s=this.app.vault.getAbstractFileByPath(i);if(!(!s||!(s instanceof u.TFile))){if(this.app.metadataCache.getFileCache(s)?.frontmatter?.enriched_at)continue}let o=r.name,c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&c.id&&n.push({id:c.id,name:c.name,owner:"",ownerEmail:""})}if(n.length===0){console.log("[Eudia] Auto-enrich: all account folders already enriched");return}console.log(`[Eudia] Auto-enrich: ${n.length} accounts need enrichment`);try{await this.enrichAccountFolders(n)}catch(r){console.error("[Eudia] Auto-enrich failed:",r)}}async enrichAccountFolders(e){if(!e||e.length===0)return;let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=this.settings.accountsFolder||"Accounts",r=e.filter(l=>l.id&&l.id.length>=15);if(r.length===0)return;let i=r.length,s=0,o=0;console.log(`[Eudia Enrich] Starting enrichment for ${i} accounts`),new u.Notice(`Enriching account data: 0/${i}...`);let c=20;for(let l=0;l<r.length;l+=c){let p=r.slice(l,l+c),g=p.map(m=>m.id);try{let m=await(0,u.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:g,userEmail:this.settings.userEmail})});if(m.json?.success&&m.json?.enrichments){let f=m.json.enrichments;for(let S of p){let v=f[S.id];if(v)try{await this.writeEnrichmentToAccount(S,v,n),s++}catch(C){o++,console.error(`[Eudia Enrich] Write failed for ${S.name}:`,C)}}}}catch(m){o+=p.length,console.error("[Eudia Enrich] Batch fetch failed:",m)}let h=Math.min(l+c,i);new u.Notice(`Enriching account data: ${h}/${i}...`),l+c<r.length&&await new Promise(m=>setTimeout(m,100))}let d=o>0?`Enrichment complete: ${s} enriched, ${o} skipped`:`Enrichment complete: ${s} accounts enriched with Salesforce data`;console.log(`[Eudia Enrich] ${d}`),new u.Notice(d)}async writeEnrichmentToAccount(e,t,n){let r=e.name.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${n}/${r}`,s=this.app.vault.getAbstractFileByPath(i);if(s instanceof u.TFolder||(i=`${n}/_Prospects/${r}`,s=this.app.vault.getAbstractFileByPath(i)),!(s instanceof u.TFolder))return;let o=new Date().toISOString(),c=async(d,l)=>{let p=`${i}/${d}`,g=this.app.vault.getAbstractFileByPath(p);if(!(g instanceof u.TFile))return;let h=await this.app.vault.read(g),m="",f=h;if(h.startsWith("---")){let w=h.indexOf("---",3);w!==-1&&(m=h.substring(0,w+3),f=h.substring(w+3),m.includes("enriched_at:")?m=m.replace(/enriched_at:.*/,`enriched_at: "${o}"`):m=m.substring(0,w)+`enriched_at: "${o}"
---`)}let S=f.match(/^(\s*#[^\n]+)/),C=`${S?S[1]:""}
...
`;await this.app.vault.modify(e,s)}async updateTeamPerformanceNote(e,t){if(!t)return;let n=new Date().toISOString().split("T")[0],r=s=>s>0?`\u2191 ${Math.abs(s).toFixed(1)}%`:s<0?`\u2193 ${Math.abs(s).toFixed(1)}%`:"--",i=`---
type: analytics_dashboard
auto_refresh: true
