var fe=Object.create;var G=Object.defineProperty;var we=Object.getOwnPropertyDescriptor;var ve=Object.getOwnPropertyNames;var Oe=Object.getPrototypeOf,Se=Object.prototype.hasOwnProperty;var Ce=(O,i)=>{for(var t in i)G(O,t,{get:i[t],enumerable:!0})},re=(O,i,t,e)=>{if(i&&typeof i=="object"||typeof i=="function")for(let n of ve(i))!Se.call(O,n)&&n!==t&&G(O,n,{get:()=>i[n],enumerable:!(e=we(i,n))||e.enumerable});return O};var U=(O,i,t)=>(t=O!=null?fe(Oe(O)):{},re(i||!O||!O.__esModule?G(t,"default",{value:O,enumerable:!0}):t,O)),be=O=>re(G({},"__esModule",{value:!0}),O);var Re={};Ce(Re,{default:()=>q});module.exports=be(Re);var p=require("obsidian");var oe=[/blackhole/i,/vb-cable/i,/vb cable/i,/loopback/i,/soundflower/i,/virtual audio/i,/screen ?capture/i],E=class O{constructor(){this.mediaRecorder=null;this.audioChunks=[];this.stream=null;this.secondaryStream=null;this.startTime=0;this.pausedDuration=0;this.pauseStartTime=0;this.durationInterval=null;this.audioContext=null;this.analyser=null;this.levelInterval=null;this.lastExtractedChunkIndex=0;this.mimeTypeCache="audio/webm";this.activeCaptureMode="full_call";this.activeHasVirtualDevice=!1;this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0};this.stateCallback=null;this.levelHistory=[];this.trackingLevels=!1}onStateChange(i){this.stateCallback=i}static isIOSOrSafari(){let i=navigator.userAgent,t=/iPad|iPhone|iPod/.test(i)&&!window.MSStream,e=/^((?!chrome|android).)*safari/i.test(i);return t||e}getSupportedMimeType(){let i=O.isIOSOrSafari(),t=i?["audio/mp4","audio/mp4;codecs=aac","audio/aac","audio/webm;codecs=opus","audio/webm"]:["audio/webm;codecs=opus","audio/webm","audio/mp4","audio/ogg;codecs=opus","audio/ogg"];for(let e of t)if(MediaRecorder.isTypeSupported(e))return console.log(`[AudioRecorder] Using MIME type: ${e} (iOS/Safari: ${i})`),e;return i?"audio/mp4":"audio/webm"}async startRecording(i){if(this.state.isRecording)throw new Error("Already recording");let t=i?.captureMode??"full_call";this.activeCaptureMode=t,this.activeHasVirtualDevice=!1;try{let e=O.isIOSOrSafari(),n;e?n={echoCancellation:!0,noiseSuppression:!0}:t==="full_call"?n={echoCancellation:!1,noiseSuppression:!1,autoGainControl:!0,sampleRate:48e3,channelCount:1}:n={echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0,sampleRate:48e3,channelCount:1},i?.micDeviceId&&(n.deviceId={exact:i.micDeviceId});let r=await navigator.mediaDevices.getUserMedia({audio:n});console.log(`[AudioRecorder] Mic granted | mode=${t} | echoCancellation=${t!=="full_call"}`);let s=r,a=i?.systemAudioDeviceId||(await O.detectVirtualAudioDevice())?.deviceId;if(a&&!e)try{let c=await navigator.mediaDevices.getUserMedia({audio:{deviceId:{exact:a},echoCancellation:!1,noiseSuppression:!1,autoGainControl:!1}});this.audioContext=new AudioContext;let l=this.audioContext.createMediaStreamSource(r),d=this.audioContext.createMediaStreamSource(c),u=this.audioContext.createMediaStreamDestination();l.connect(u),d.connect(u),s=u.stream,this.secondaryStream=c,this.activeHasVirtualDevice=!0,console.log("[AudioRecorder] Virtual device detected \u2014 dual-stream capture active")}catch(c){console.log(`[AudioRecorder] Virtual device open failed (${c.message}), continuing with mic only`)}this.stream=s,this.setupAudioAnalysis();let o=this.getSupportedMimeType();this.mimeTypeCache=o,this.mediaRecorder=new MediaRecorder(this.stream,{mimeType:o,audioBitsPerSecond:128e3}),this.audioChunks=[],this.lastExtractedChunkIndex=0,this.mediaRecorder.ondataavailable=c=>{c.data.size>0&&this.audioChunks.push(c.data)},this.mediaRecorder.start(1e3),this.startTime=Date.now(),this.pausedDuration=0,this.state={isRecording:!0,isPaused:!1,duration:0,audioLevel:0},this.startDurationTracking(),this.startLevelTracking(),this.notifyStateChange()}catch(e){throw this.cleanup(),new Error(`Failed to start recording: ${e.message}`)}}static async detectVirtualAudioDevice(){try{let i=await navigator.mediaDevices.enumerateDevices();for(let t of i)if(t.kind==="audioinput"){for(let e of oe)if(e.test(t.label))return{deviceId:t.deviceId,label:t.label,isVirtual:!0}}}catch(i){console.warn("[AudioRecorder] enumerateDevices failed:",i)}return null}static async getAvailableDevices(){try{return(await navigator.mediaDevices.enumerateDevices()).filter(t=>t.kind==="audioinput").map(t=>{let e=oe.some(n=>n.test(t.label));return{deviceId:t.deviceId,label:t.label||"Unknown Microphone",isVirtual:e}})}catch(i){return console.warn("[AudioRecorder] enumerateDevices failed:",i),[]}}setupAudioAnalysis(){if(this.stream)try{this.audioContext=new AudioContext;let i=this.audioContext.createMediaStreamSource(this.stream);this.analyser=this.audioContext.createAnalyser(),this.analyser.fftSize=256,i.connect(this.analyser)}catch(i){console.warn("Failed to set up audio analysis:",i)}}startDurationTracking(){this.durationInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused){let i=Date.now()-this.startTime-this.pausedDuration;this.state.duration=Math.floor(i/1e3),this.notifyStateChange(),this.state.duration>=5400&&(console.log("[Eudia] Maximum recording duration reached (90 minutes) \u2014 auto-stopping"),this.stop())}},100)}startLevelTracking(){if(!this.analyser)return;let i=new Uint8Array(this.analyser.frequencyBinCount);this.levelInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused&&this.analyser){this.analyser.getByteFrequencyData(i);let t=0;for(let n=0;n<i.length;n++)t+=i[n];let e=t/i.length;this.state.audioLevel=Math.min(100,Math.round(e/255*100*2)),this.notifyStateChange()}},50)}pauseRecording(){!this.state.isRecording||this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="recording"&&(this.mediaRecorder.pause(),this.pauseStartTime=Date.now(),this.state.isPaused=!0,this.notifyStateChange())}resumeRecording(){!this.state.isRecording||!this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="paused"&&(this.mediaRecorder.resume(),this.pausedDuration+=Date.now()-this.pauseStartTime,this.state.isPaused=!1,this.notifyStateChange())}async stopRecording(){return new Promise((i,t)=>{if(!this.mediaRecorder||!this.state.isRecording){t(new Error("Not currently recording"));return}let e=this.mediaRecorder.mimeType,n=this.state.duration,r=this.activeCaptureMode,s=this.activeHasVirtualDevice,a=!1,o=l=>{let d=new Date,u=d.toISOString().split("T")[0],m=d.toTimeString().split(" ")[0].replace(/:/g,"-"),f=e.includes("webm")?"webm":e.includes("mp4")?"m4a":e.includes("ogg")?"ogg":"webm";return{audioBlob:l,duration:n,mimeType:e,filename:`recording-${u}-${m}.${f}`,captureMode:r,hasVirtualDevice:s}},c=setTimeout(()=>{if(!a){a=!0,console.warn("AudioRecorder: onstop timeout, forcing completion");try{let l=new Blob(this.audioChunks,{type:e});this.cleanup(),i(o(l))}catch{this.cleanup(),t(new Error("Failed to process recording after timeout"))}}},1e4);this.mediaRecorder.onstop=()=>{if(!a){a=!0,clearTimeout(c);try{console.log(`[AudioRecorder] Chunks collected: ${this.audioChunks.length}`);let l=new Blob(this.audioChunks,{type:e});console.log(`[AudioRecorder] Blob size: ${l.size} bytes`),this.cleanup(),i(o(l))}catch(l){this.cleanup(),t(l)}}},this.mediaRecorder.onerror=l=>{a||(a=!0,clearTimeout(c),this.cleanup(),t(new Error("Recording error occurred")))},this.mediaRecorder.state==="recording"&&this.mediaRecorder.requestData(),setTimeout(()=>{this.mediaRecorder&&this.mediaRecorder.state!=="inactive"&&this.mediaRecorder.stop()},100)})}cancelRecording(){this.cleanup()}cleanup(){this.durationInterval&&(clearInterval(this.durationInterval),this.durationInterval=null),this.levelInterval&&(clearInterval(this.levelInterval),this.levelInterval=null),this.audioContext&&(this.audioContext.close().catch(()=>{}),this.audioContext=null,this.analyser=null),this.stream&&(this.stream.getTracks().forEach(i=>i.stop()),this.stream=null),this.secondaryStream&&(this.secondaryStream.getTracks().forEach(i=>i.stop()),this.secondaryStream=null),this.mediaRecorder=null,this.audioChunks=[],this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0},this.notifyStateChange()}getState(){return{...this.state}}static isSupported(){if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||!window.MediaRecorder)return!1;let t=["audio/webm","audio/mp4","audio/ogg","audio/webm;codecs=opus"].some(e=>MediaRecorder.isTypeSupported(e));return t||console.warn("[AudioRecorder] No supported audio formats found"),t}static getMobileInstructions(){return this.isIOSOrSafari()?"For best results on iOS, ensure you have granted microphone permissions in Settings > Privacy > Microphone.":null}notifyStateChange(){this.stateCallback&&this.stateCallback({...this.state})}static formatDuration(i){let t=Math.floor(i/60),e=i%60;return`${t.toString().padStart(2,"0")}:${e.toString().padStart(2,"0")}`}static async blobToBase64(i){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let s=n.result.split(",")[1];t(s)},n.onerror=e,n.readAsDataURL(i)})}static async blobToArrayBuffer(i){return i.arrayBuffer()}async start(i){return this.startRecording(i)}async stop(){return this.stopRecording()}pause(){return this.pauseRecording()}resume(){return this.resumeRecording()}cancel(){return this.cancelRecording()}isRecording(){return this.state.isRecording}extractNewChunks(){if(!this.state.isRecording||this.audioChunks.length===0)return null;let i=this.audioChunks.slice(this.lastExtractedChunkIndex);return i.length===0?null:(this.lastExtractedChunkIndex=this.audioChunks.length,new Blob(i,{type:this.mimeTypeCache}))}getAllChunksAsBlob(){return this.audioChunks.length===0?null:new Blob(this.audioChunks,{type:this.mimeTypeCache})}getDuration(){return this.state.duration}getMimeType(){return this.mimeTypeCache}startLevelHistoryTracking(){this.levelHistory=[],this.trackingLevels=!0}recordLevelSample(){this.trackingLevels&&this.levelHistory.push(this.state.audioLevel)}getAudioDiagnostic(){if(this.levelHistory.length===0)return{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:100,warning:"Unable to analyze audio levels - recording may be too short"};let i=this.levelHistory.reduce((s,a)=>s+a,0)/this.levelHistory.length,t=Math.max(...this.levelHistory),e=this.levelHistory.filter(s=>s<5).length,n=Math.round(e/this.levelHistory.length*100),r=null;return t<5?r="SILENT AUDIO: No audio was detected during recording. Check your microphone settings and ensure Obsidian has microphone permission.":i<10&&n>80?r="VERY LOW AUDIO: Audio levels were extremely low. The transcription may not be accurate. Check your microphone or move closer to it.":n>90&&(r="MOSTLY SILENT: Over 90% of the recording had no audio. Make sure you're capturing the meeting audio, not just silence."),{hasAudio:t>=5,averageLevel:Math.round(i),peakLevel:t,silentPercent:n,warning:r}}static async analyzeAudioBlob(i){try{let t=new AudioContext,e=await i.arrayBuffer(),n;try{n=await t.decodeAudioData(e)}catch{return await t.close(),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:"Could not analyze audio format. Proceeding with transcription."}}let r=n.getChannelData(0),s=0,a=0,o=0,c=.01,l=100,d=0;for(let g=0;g<r.length;g+=l){let y=Math.abs(r[g]);s+=y,y>a&&(a=y),y<c&&o++,d++}await t.close();let u=s/d,m=Math.round(o/d*100),f=Math.round(u*100*10),h=Math.round(a*100),v=null;return a<.01?v='SILENT AUDIO DETECTED: The recording appears to contain only silence. This typically causes Whisper to hallucinate random text like "Yes. Yes. Yes." Check your audio input source.':u<.005&&m>95?v="NEAR-SILENT AUDIO: The recording is almost entirely silent. The transcription will likely be inaccurate.":m>90&&(v="MOSTLY SILENT: Over 90% of the recording is silent. Consider checking your audio setup."),{hasAudio:a>=.01,averageLevel:f,peakLevel:h,silentPercent:m,warning:v}}catch(t){return console.error("Audio analysis failed:",t),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:null}}}};var P=require("obsidian");var X=class{constructor(){this.salesforceAccounts=[]}setAccounts(i){this.salesforceAccounts=i}detectAccount(i,t,e){if(i){let n=this.detectFromTitle(i);if(n.confidence>=70)return n}if(e){let n=this.detectFromFilePath(e);if(n.confidence>=70)return n}if(t&&t.length>0){let n=this.detectFromAttendees(t);if(n.confidence>=50)return n}return{account:null,accountId:null,confidence:0,source:"none",evidence:"No account detected from available context"}}detectFromTitle(i){if(!i)return{account:null,accountId:null,confidence:0,source:"title",evidence:"No title"};let t=[{regex:/^([A-Za-z0-9][^-–—]+?)\s*[-–—]\s*(?:[A-Z][a-z]+|[A-Za-z]{2,})/,confidence:85},{regex:/(?:call|meeting|sync|check-in|demo|discovery)\s+(?:with|re:?|@)\s+([^-–—]+?)(?:\s*[-–—]|$)/i,confidence:80},{regex:/^([A-Za-z][^-–—]+?)\s+(?:discovery|demo|review|kickoff|intro|onboarding|sync)\s*(?:call)?$/i,confidence:75},{regex:/^([^:]+?):\s+/i,confidence:70},{regex:/^\[([^\]]+)\]/,confidence:75}],e=["weekly","daily","monthly","internal","team","1:1","one on one","standup","sync","meeting","call","notes","monday","tuesday","wednesday","thursday","friday","untitled","new","test"];for(let n of t){let r=i.match(n.regex);if(r&&r[1]){let s=r[1].trim();if(e.some(o=>s.toLowerCase()===o)||s.length<2)continue;let a=this.fuzzyMatchSalesforce(s);return a?{account:a.name,accountId:a.id,confidence:Math.min(n.confidence+10,100),source:"salesforce_match",evidence:`Matched "${s}" from title to Salesforce account "${a.name}"`}:{account:s,accountId:null,confidence:n.confidence,source:"title",evidence:"Extracted from meeting title pattern"}}}return{account:null,accountId:null,confidence:0,source:"title",evidence:"No pattern matched"}}detectFromFilePath(i){let t=i.match(/Accounts\/([^\/]+)\//i);if(t&&t[1]){let e=t[1].trim(),n=this.fuzzyMatchSalesforce(e);return n?{account:n.name,accountId:n.id,confidence:95,source:"salesforce_match",evidence:`File in account folder "${e}" matched to "${n.name}"`}:{account:e,accountId:null,confidence:85,source:"title",evidence:`File located in Accounts/${e} folder`}}return{account:null,accountId:null,confidence:0,source:"none",evidence:"Not in Accounts folder"}}detectFromAttendees(i){let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com"],e=new Set;for(let a of i){let c=a.toLowerCase().match(/@([a-z0-9.-]+)/);if(c){let l=c[1];!l.includes("eudia.com")&&!t.includes(l)&&e.add(l)}}if(e.size===0)return{account:null,accountId:null,confidence:0,source:"attendee_domain",evidence:"No external domains"};for(let a of e){let o=a.split(".")[0],c=o.charAt(0).toUpperCase()+o.slice(1),l=this.fuzzyMatchSalesforce(c);if(l)return{account:l.name,accountId:l.id,confidence:75,source:"salesforce_match",evidence:`Matched attendee domain ${a} to "${l.name}"`}}let n=Array.from(e)[0],r=n.split(".")[0];return{account:r.charAt(0).toUpperCase()+r.slice(1),accountId:null,confidence:50,source:"attendee_domain",evidence:`Guessed from external attendee domain: ${n}`}}fuzzyMatchSalesforce(i){if(!i||this.salesforceAccounts.length===0)return null;let t=i.toLowerCase().trim();for(let e of this.salesforceAccounts)if(e.name?.toLowerCase()===t)return e;for(let e of this.salesforceAccounts)if(e.name?.toLowerCase().startsWith(t))return e;for(let e of this.salesforceAccounts)if(e.name?.toLowerCase().includes(t))return e;for(let e of this.salesforceAccounts)if(t.includes(e.name?.toLowerCase()))return e;return null}suggestAccounts(i,t=10){if(!i||i.length<2)return this.salesforceAccounts.slice(0,t).map(r=>({...r,score:0}));let e=i.toLowerCase(),n=[];for(let r of this.salesforceAccounts){let s=r.name?.toLowerCase()||"",a=0;s===e?a=100:s.startsWith(e)?a=90:s.includes(e)?a=70:e.includes(s)&&(a=50),a>0&&n.push({...r,score:a})}return n.sort((r,s)=>s.score-r.score).slice(0,t)}},Ge=new X,Ae=["pipeline review","pipeline call","weekly pipeline","forecast call","forecast review","deal review","opportunity review","sales review","pipeline sync","forecast sync","deal sync","pipeline update","forecast meeting"];function ce(O,i){if(O){let t=O.toLowerCase();for(let e of Ae)if(t.includes(e))return{isPipelineMeeting:!0,confidence:95,evidence:`Title contains "${e}"`}}if(i&&i.length>=2){let t=["eudia.com","johnsonhana.com"];if(i.every(n=>{let r=n.toLowerCase().split("@")[1]||"";return t.some(s=>r.includes(s))})&&i.length>=3){if(O){let n=O.toLowerCase();if(["sync","review","update","weekly","team","forecast"].some(a=>n.includes(a)))return{isPipelineMeeting:!0,confidence:70,evidence:`All internal attendees (${i.length}) with team meeting signal`}}return{isPipelineMeeting:!1,confidence:40,evidence:"All internal attendees but no clear pipeline signal"}}}return{isPipelineMeeting:!1,confidence:0,evidence:"No pipeline meeting indicators found"}}function We(O,i){let t="";return(i?.account||i?.opportunities?.length)&&(t=`
...
- Action items have clear owners`}function je(O){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${O?`
...
`;let s=e.talkTime.repPercent,a=e.talkTime.customerPercent,o=e.talkTime.isHealthyRatio?"\u2705":"\u26A0\uFE0F";n+=`**Talk Time:** Rep ${s}% / Customer ${a}% ${o}
`;let c=Math.round(s/5),l=Math.round(a/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
...
`);let r=e?.enabled&&e?.formattedTranscript?e.formattedTranscript:t;if(r){let s=e?.enabled?"Full Transcript (Speaker-Attributed)":"Full Transcript";n+=`---
...
`}return n}static formatSectionsWithAudio(i,t,e,n){let r=this.formatSectionsForNote(i,t,n);return e&&(r+=`
---

## Recording

![[${e}]]
`),r}static formatContextForNote(i){if(!i.success)return"";let t=`## Pre-Call Context
...
`,t}async blobToBase64(i){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let s=n.result.split(",")[1];t(s)},n.onerror=e,n.readAsDataURL(i)})}async transcribeAudio(i,t){try{let e=await this.blobToBase64(i),n=i.type||"audio/webm",r=t?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:t.pipelineContext}:void 0,s=await this.transcribeAndSummarize(e,n,t?.accountName,t?.accountId,r,{captureMode:t?.captureMode,hasVirtualDevice:t?.hasVirtualDevice});return{text:s.transcript,confidence:s.success?.95:0,duration:s.duration,sections:s.sections,diarizedTranscript:s.diarizedTranscript}}catch(e){return console.error("transcribeAudio error:",e),{text:"",confidence:0,duration:0,sections:this.getEmptySections()}}}async processTranscription(i,t){if(!i||i.trim().length===0)return this.getEmptySections();try{let e=await(0,P.requestUrl)({url:`${this.serverUrl}/api/process-sections`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:i,accountName:t?.accountName,context:t})});if(e.json?.success&&e.json?.sections){let n=e.json.sections;return{summary:n.summary||"",painPoints:n.painPoints||n.keyPoints||"",productInterest:n.productInterest||"",meddiccSignals:n.meddiccSignals||"",nextSteps:n.nextSteps||"",actionItems:n.actionItems||"",keyDates:n.keyDates||"",buyingTriggers:n.buyingTriggers||"",dealSignals:n.dealSignals||"",risksObjections:n.risksObjections||n.concerns||"",competitiveIntel:n.competitiveIntel||"",attendees:n.attendees||"",transcript:i}}return console.warn("Server process-sections returned no sections, using fallback"),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:i}}catch(e){return console.error("processTranscription server error:",e),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:i}}}};var V=require("obsidian"),F=class O{constructor(i,t,e="America/New_York"){this.serverUrl=i,this.userEmail=t.toLowerCase(),this.timezone=e}setUserEmail(i){this.userEmail=i.toLowerCase()}setServerUrl(i){this.serverUrl=i}setTimezone(i){this.timezone=i}async getTodaysMeetings(i=!1){if(!this.userEmail)return{success:!1,date:new Date().toISOString().split("T")[0],email:"",meetingCount:0,meetings:[],error:"User email not configured"};try{let t=encodeURIComponent(this.timezone),e=i?"&forceRefresh=true":"";return(await(0,V.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/today?timezone=${t}${e}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(t){return console.error("Failed to fetch today's meetings:",t),{success:!1,date:new Date().toISOString().split("T")[0],email:this.userEmail,meetingCount:0,meetings:[],error:t.message||"Failed to fetch calendar"}}}async getWeekMeetings(i=!1){if(!this.userEmail)return{success:!1,startDate:"",endDate:"",email:"",totalMeetings:0,byDay:{},error:"User email not configured"};try{let t=encodeURIComponent(this.timezone),e=i?"&forceRefresh=true":"";return(await(0,V.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/week?timezone=${t}${e}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(t){return console.error("Failed to fetch week's meetings:",t),{success:!1,startDate:"",endDate:"",email:this.userEmail,totalMeetings:0,byDay:{},error:t.message||"Failed to fetch calendar"}}}async getMeetingsInRange(i,t){if(!this.userEmail)return[];try{let e=i.toISOString().split("T")[0],n=t.toISOString().split("T")[0],r=await(0,V.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/range?start=${e}&end=${n}`,method:"GET",headers:{Accept:"application/json"}});return r.json.success?r.json.meetings||[]:[]}catch(e){return console.error("Failed to fetch calendar range:",e),[]}}async getCurrentMeeting(){let i=await this.getTodaysMeetings();if(!i.success||i.meetings.length===0)return{meeting:null,isNow:!1};let t=new Date;for(let e of i.meetings){let n=O.safeParseDate(e.start),r=O.safeParseDate(e.end);if(t>=n&&t<=r)return{meeting:e,isNow:!0};let s=(n.getTime()-t.getTime())/(1e3*60);if(s>0&&s<=15)return{meeting:e,isNow:!1,minutesUntilStart:Math.ceil(s)}}return{meeting:null,isNow:!1}}async getMeetingsForAccount(i){let t=await this.getWeekMeetings();if(!t.success)return[];let e=[];Object.values(t.byDay).forEach(r=>{e.push(...r)});let n=i.toLowerCase();return e.filter(r=>r.accountName?.toLowerCase().includes(n)||r.subject.toLowerCase().includes(n)||r.attendees.some(s=>s.email.toLowerCase().includes(n.split(" ")[0])))}static formatMeetingForNote(i){let t=i.attendees.filter(e=>e.isExternal!==!1).map(e=>e.name||e.email.split("@")[0]).slice(0,5).join(", ");return{title:i.subject,attendees:t,meetingStart:i.start,accountName:i.accountName}}static getDayName(i){let t;i.length===10&&i.includes("-")?t=new Date(i+"T00:00:00"):t=new Date(i);let e=new Date;e.setHours(0,0,0,0);let n=new Date(t);n.setHours(0,0,0,0);let r=Math.round((n.getTime()-e.getTime())/(1e3*60*60*24));return r===0?"Today":r===1?"Tomorrow":t.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}static formatTime(i,t){let e=i;e&&!e.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(e)&&(e=e+"Z");let n=new Date(e);if(isNaN(n.getTime()))return i;let r={hour:"numeric",minute:"2-digit",hour12:!0};return t&&(r.timeZone=t),n.toLocaleTimeString("en-US",r)}static safeParseDate(i){if(!i)return new Date(NaN);let t=i;return!t.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(t)&&(t=t+"Z"),new Date(t)}static getMeetingDuration(i,t){let e=O.safeParseDate(i),n=O.safeParseDate(t);return Math.round((n.getTime()-e.getTime())/(1e3*60))}};var xe=["ai-contracting-tech","ai-contracting-services","ai-compliance-tech","ai-compliance-services","ai-ma-tech","ai-ma-services","sigma"],Ie=["metrics-identified","economic-buyer-identified","decision-criteria-discussed","decision-process-discussed","pain-confirmed","champion-identified","competition-mentioned"],Ee=["progressing","stalled","at-risk","champion-engaged","early-stage"],ke=["discovery","demo","negotiation","qbr","implementation","follow-up"],Te=`You are a sales intelligence tagger for Eudia, an AI legal technology company. Extract structured tags from meeting analysis.
...
`),e=e.replace(/^#{1,3}\s+.+\n+(?=#{1,3}\s|\s*$)/gm,""),e=e.replace(/^#{2,3}\s+(.+)$/gm,'</p><h3 class="eudia-intel-header">$1</h3><p>'),e=e.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^-\s+\[\s*\]\s+(.+)$/gm,'<li class="eudia-intel-todo">$1</li>'),e=e.replace(/^-\s+\[x\]\s+(.+)$/gm,'<li class="eudia-intel-done">$1</li>'),e=e.replace(/^[•\-]\s+(.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li[^>]*>.*?<\/li>\s*)+/g,'<ul class="eudia-intel-list">$&</ul>'),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=e.replace(/<p>\s*(<ul)/g,"$1"),e=e.replace(/<\/ul>\s*<\/p>/g,"</ul>"),e=e.replace(/<p>\s*(<h3)/g,"$1"),e=e.replace(/<\/h3>\s*<\/p>/g,"</h3>"),e=e.replace(/<\/li>\s*<br>\s*<li/g,"</li><li"),e=e.replace(/<p>\s*<\/p>/g,""),e=e.replace(/<p>\s*<br>\s*<\/p>/g,""),e=e.replace(/(<br>\s*){2,}/g,""),e=e.replace(/<\/h3>\s*<br>/g,"</h3>"),e=e.replace(/<br>\s*<h3/g,"<h3"),e=e.replace(/<br>\s*<ul/g,"<ul"),e=e.replace(/<\/ul>\s*<br>/g,"</ul>"),e=e.replace(/^(<br>)+|(<br>)+$/g,""),e="<p>"+e+"</p>",e=e.replace(/<p><\/p>/g,""),e}onClose(){this.contentEl.empty()}};var ne=class extends p.ItemView{constructor(t,e){super(t);this.emailInput=null;this.pollInterval=null;this.plugin=e,this.accountOwnershipService=new H(e.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return L}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}if(this.plugin.settings.accountsImported){let e=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(a=>a.children!==void 0)||[];e.length>0?(this.steps[2].status="complete",console.log(`[Eudia] Vault reopen: ${e.length} account folders verified`)):(console.warn("[Eudia] accountsImported=true but 0 account folders found \u2014 resetting for re-import"),this.plugin.settings.accountsImported=!1,this.plugin.settings.importedAccountCount=0,await this.plugin.saveSettings());try{let a=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");a&&await this.plugin.app.vault.delete(a)}catch{}let n=this.plugin.settings.userEmail,s=(this.plugin.settings.cachedAccounts||[]).filter(a=>a.id&&String(a.id).startsWith("001"));if(n&&s.length>0){let a=this.plugin.settings.accountsFolder||"Accounts",o=!1;for(let c of s.slice(0,5)){let l=(c.name||"").replace(/[<>:"/\\|?*]/g,"_").trim(),d=`${a}/${l}/Contacts.md`,u=this.plugin.app.vault.getAbstractFileByPath(d);if(u instanceof p.TFile&&!this.plugin.app.metadataCache.getFileCache(u)?.frontmatter?.enriched_at){o=!0;break}}o&&(console.log("[Eudia Setup] Accounts need enrichment \u2014 triggering on vault reopen..."),setTimeout(async()=>{try{let c=s.map(l=>({id:l.id,name:l.name,type:"",isOwned:!1,hadOpportunity:!0,website:null,industry:null}));await this.plugin.enrichAccountFolders(c),console.log(`[Eudia] Vault-reopen enrichment complete: ${c.length} accounts enriched`)}catch(c){console.log("[Eudia] Vault-reopen enrichment failed (will retry next open):",c)}},3e3))}}else{console.log("[Eudia Setup] Email set but accounts not imported \u2014 auto-retrying import...");let t=this.plugin.app.workspace.leftSplit,e=t?.collapsed;try{let n=this.plugin.settings.userEmail,r=D(n)?"admin":R(n)?"cs":"bl",s=[],a=[];if(console.log(`[Eudia Setup] Auto-retry for ${n} (group: ${r})`),r==="cs")s=[...$],console.log(`[Eudia Setup] Auto-retry CS: using ${s.length} static accounts`);else if(r==="admin")s=await this.accountOwnershipService.getAllAccountsForAdmin(n);else{let o=await this.accountOwnershipService.getAccountsWithProspects(n);s=o.accounts,a=o.prospects}if(s.length>0||a.length>0){if(t&&!e&&t.collapse(),r==="admin"?await this.plugin.createAdminAccountFolders(s):(await this.plugin.createTailoredAccountFolders(s,{}),a.length>0&&await this.plugin.createProspectAccountFiles(a)),B(n))try{await this.plugin.createCSManagerDashboard(n,s)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+a.length,await this.plugin.saveSettings(),this.steps[2].status="complete";try{let o=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");o&&await this.plugin.app.vault.delete(o)}catch{}t&&!e&&t.expand(),console.log(`[Eudia Setup] Auto-retry imported ${s.length} accounts for ${n}`),new p.Notice(`Enriching ${s.length} accounts with Salesforce contacts...`);try{let o=r==="cs"?s:[...s,...a];await this.plugin.enrichAccountFolders(o),new p.Notice(`${s.length} accounts loaded and enriched!`),console.log("[Eudia Setup] Auto-retry enrichment complete")}catch(o){console.log("[Eudia Setup] Auto-retry enrichment failed:",o),new p.Notice(`${s.length} accounts imported! Contacts will populate on next open.`)}}else console.warn(`[Eudia Setup] Auto-retry returned 0 accounts for ${n}. Server may still be starting.`),t&&!e&&t.expand()}catch(n){console.error("[Eudia Setup] Auto-retry account import failed:",n),t&&!e&&t.expand()}}}}getCompletionPercentage(){let t=this.steps.filter(e=>e.status==="complete").length;return Math.round(t/this.steps.length*100)}async render(){let t=this.containerEl.children[1];t.empty(),t.addClass("eudia-setup-view"),this.renderHeader(t),this.renderSteps(t),this.renderFooter(t)}renderHeader(t){let e=t.createDiv({cls:"eudia-setup-header"}),n=e.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to transcribe and summarize meetings -- capturing objections, next steps, and pain points to drive better client outcomes and smarter selling.",cls:"eudia-setup-subtitle"});let r=e.createDiv({cls:"eudia-setup-progress-section"}),s=this.getCompletionPercentage(),a=r.createDiv({cls:"eudia-setup-progress-label"});a.createSpan({text:"Setup Progress"}),a.createSpan({text:`${s}%`,cls:"eudia-setup-progress-value"});let c=r.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${s}%`}renderSteps(t){let e=t.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(e),this.renderSalesforceStep(e),this.renderTranscribeStep(e)}renderCalendarStep(t){let e=this.steps[0],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),s=r.createDiv({cls:"eudia-setup-step-number"});s.setText(e.status==="complete"?"":"1"),e.status==="complete"&&s.addClass("eudia-step-complete");let a=r.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:e.title}),a.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(e.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let l=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});l.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async d=>{d.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let t=this.emailInput.value.trim().toLowerCase(),e=this.containerEl.querySelector(".eudia-setup-validation-message");if(!t){e&&(e.textContent="Please enter your email",e.className="eudia-setup-validation-message error");return}if(!t.endsWith("@eudia.com")){e&&(e.textContent="Please use your @eudia.com email address",e.className="eudia-setup-validation-message error");return}e&&(e.textContent="Validating...",e.className="eudia-setup-validation-message loading");try{let n=await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(t)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=t,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new p.Notice("Calendar connected successfully!"),e&&(e.textContent="Importing your accounts...",e.className="eudia-setup-validation-message loading");let r=this.plugin.app.workspace.leftSplit,s=r?.collapsed;r&&!s&&r.collapse();try{let a,o=[],c=D(t)?"admin":R(t)?"cs":"bl";if(console.log(`[Eudia] User group detected: ${c} for ${t}`),c==="cs"){if(console.log(`[Eudia] CS user detected \u2014 loading ${$.length} accounts from static data (instant, no server needed)`),a=[...$],o=[],e&&(e.textContent=`Loading ${a.length} Customer Success accounts...`),await this.plugin.createTailoredAccountFolders(a,{}),B(t))try{await this.plugin.createCSManagerDashboard(t,a)}catch(u){console.error("[Eudia] CS Manager dashboard creation failed (non-blocking):",u)}let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(u=>u.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,console.log(`[Eudia] CS accounts verified: ${d.length} folders created`)):(console.warn(`[Eudia] CS folder creation may have failed \u2014 ${d.length} folders found. Keeping accountsImported=false for retry.`),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let u=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");u&&await this.plugin.app.vault.delete(u)}catch{}console.log(`[Eudia] CS accounts created: ${a.length} folders from static data`),e&&(e.textContent=`Enriching ${a.length} accounts with Salesforce contacts...`),new p.Notice(`Enriching ${a.length} accounts with contacts from Salesforce...`),console.log(`[Eudia] Starting synchronous enrichment for ${a.length} CS accounts...`);try{await this.plugin.enrichAccountFolders(a),console.log("[Eudia] Synchronous enrichment complete"),new p.Notice(`${a.length} accounts loaded with contacts from Salesforce!`),e&&(e.textContent=`${a.length} accounts loaded and enriched with Salesforce contacts!`)}catch(u){console.log("[Eudia] Synchronous enrichment failed, will retry in background:",u),new p.Notice(`${a.length} accounts loaded! Contacts will populate shortly...`);let m=t,f=[5e3,2e4,6e4],h=async v=>{let g=f[v];if(g!==void 0){await new Promise(y=>setTimeout(y,g));try{await this.plugin.enrichAccountFolders(a),console.log(`[Eudia] Background enrichment retry ${v+1} succeeded`)}catch{return h(v+1)}}};h(0)}}else if(c==="admin"){if(console.log("[Eudia] Admin user detected - importing all accounts"),a=await this.accountOwnershipService.getAllAccountsForAdmin(t),a.length>0){e&&(e.textContent=`Creating ${a.length} account folders...`),await this.plugin.createAdminAccountFolders(a);let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(m=>m.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,console.log(`[Eudia] Admin accounts verified: ${d.length} folders created`)):(console.warn("[Eudia] Admin folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let m=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");m&&await this.plugin.app.vault.delete(m)}catch{}new p.Notice(`Imported ${a.length} accounts! Enriching with Salesforce data...`);let u=a.filter(m=>m.id&&m.id.startsWith("001"));if(u.length>0){e&&(e.textContent=`Enriching ${u.length} accounts with Salesforce contacts...`);try{await this.plugin.enrichAccountFolders(u),new p.Notice(`${a.length} accounts loaded and enriched with Salesforce data!`),console.log(`[Eudia] Admin/exec synchronous enrichment complete: ${u.length} accounts`),e&&(e.textContent=`${a.length} accounts loaded and enriched!`)}catch(m){console.log("[Eudia] Admin/exec synchronous enrichment failed, will retry on next open:",m),new p.Notice(`${a.length} accounts imported! Contacts will populate on next vault open.`);let f=[5e3,2e4,6e4],h=async v=>{let g=f[v];if(g!==void 0){await new Promise(y=>setTimeout(y,g));try{await this.plugin.enrichAccountFolders(u),console.log(`[Eudia] Admin/exec background enrichment retry ${v+1} succeeded`)}catch{return h(v+1)}}};h(0)}}}}else{let l=await this.accountOwnershipService.getAccountsWithProspects(t);if(a=l.accounts,o=l.prospects,a.length>0||o.length>0){e&&(e.textContent=`Creating ${a.length} account folders...`),await this.plugin.createTailoredAccountFolders(a,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o);let u=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(f=>f.children!==void 0)||[];u.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length+o.length,console.log(`[Eudia] BL accounts verified: ${u.length} folders created`)):(console.warn("[Eudia] BL folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let f=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");f&&await this.plugin.app.vault.delete(f)}catch{}new p.Notice(`Imported ${a.length} active accounts + ${o.length} prospects!`);let m=[...a,...o];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(m)}catch(f){console.log("[Eudia] Background enrichment skipped:",f)}},500)}else{console.warn(`[Eudia] No accounts returned for ${t} \u2014 auto-retrying...`);let d=!1;for(let u=1;u<=3;u++){e&&(e.textContent=`Server warming up... retrying in 10s (attempt ${u}/3)`,e.className="eudia-setup-validation-message warning"),await new Promise(m=>setTimeout(m,1e4));try{let m=await this.plugin.accountOwnershipService.getAccountsWithProspects(t);if(m.accounts.length>0||m.prospects.length>0){a=m.accounts,o=m.prospects,e&&(e.textContent=`Creating ${a.length} account folders...`),await this.plugin.createTailoredAccountFolders(a,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o),(this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(v=>v.children!==void 0)||[]).length>0&&(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length+o.length),await this.plugin.saveSettings(),new p.Notice(`Imported ${a.length} accounts + ${o.length} prospects!`),d=!0;break}}catch(m){console.warn(`[Eudia] Retry ${u} failed:`,m)}}d||(e&&(e.textContent="Could not load accounts after 3 attempts. Close this window, wait 1 minute, then re-open Obsidian and try again.",e.className="eudia-setup-validation-message error"),new p.Notice("Account import failed after retries. Wait 1 minute and try again."))}}}catch(a){console.error("[Eudia] Account import failed:",a),e&&(e.textContent="Account import failed. Please try again.",e.className="eudia-setup-validation-message error"),new p.Notice("Account import failed \u2014 please try again.")}finally{r&&!s&&r.expand()}await this.render()}else e&&(e.innerHTML=`<strong>${t}</strong> is not authorized for calendar access. Contact your admin.`,e.className="eudia-setup-validation-message error")}catch{e&&(e.textContent="Connection failed. Please try again.",e.className="eudia-setup-validation-message error")}}renderSalesforceStep(t){let e=this.steps[1],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),s=r.createDiv({cls:"eudia-setup-step-number"});s.setText(e.status==="complete"?"":"2"),e.status==="complete"&&s.addClass("eudia-step-complete");let a=r.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:e.title}),a.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(e.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let l=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),d=o.createDiv({cls:"eudia-setup-sf-status"});l.onclick=async()=>{let u=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(u,"_blank"),d.textContent="Complete the login in the popup window...",d.className="eudia-setup-sf-status loading",new p.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(d)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(t){this.pollInterval&&window.clearInterval(this.pollInterval);let e=0,n=60;this.pollInterval=window.setInterval(async()=>{e++;try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new p.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(t),await this.render()):e>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),t.textContent="Connection timed out. Please try again.",t.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(t){t.textContent="Importing your accounts...",t.className="eudia-setup-sf-status loading";try{let e=this.plugin.settings.userEmail,n=D(e)?"admin":R(e)?"cs":"bl";console.log(`[Eudia SF Import] Importing for ${e} (group: ${n})`);let r,s=[];if(n==="cs"){console.log("[Eudia SF Import] CS user SF Connect \u2014 fetching live data from Salesforce..."),t.textContent="Syncing with Salesforce for latest account data...";try{let f=await this.accountOwnershipService.getCSAccounts(e);r=f.accounts,s=f.prospects,console.log(`[Eudia SF Import] CS server sync: ${r.length} accounts (with real SF IDs + CSM data)`)}catch{if(this.plugin.settings.accountsImported){t.textContent="Salesforce connected! Account folders already loaded. Enrichment will retry later.",t.className="eudia-setup-sf-status success",this.steps[1].status="complete";return}r=[...$],console.log(`[Eudia SF Import] CS server unavailable \u2014 using ${r.length} static accounts`)}}else if(n==="admin")console.log("[Eudia] Admin user detected - importing all accounts"),t.textContent="Admin detected - importing all accounts...",r=await this.accountOwnershipService.getAllAccountsForAdmin(e);else{let f=await this.accountOwnershipService.getAccountsWithProspects(e);r=f.accounts,s=f.prospects}if(r.length===0&&s.length===0){t.textContent="No accounts found for your email. Contact your admin.",t.className="eudia-setup-sf-status warning";return}t.textContent=`Creating ${r.length} account folders...`;let a=this.plugin.app.workspace.leftSplit,o=a?.collapsed;if(a&&!o&&a.collapse(),D(e)?await this.plugin.createAdminAccountFolders(r):(await this.plugin.createTailoredAccountFolders(r,{}),s.length>0&&await this.plugin.createProspectAccountFiles(s)),B(e))try{await this.plugin.createCSManagerDashboard(e,r)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=r.length+s.length,await this.plugin.saveSettings();try{let f=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");f&&await this.plugin.app.vault.delete(f)}catch{}a&&!o&&a.expand(),this.steps[2].status="complete";let c=r.filter(f=>f.isOwned!==!1).length,l=r.filter(f=>f.isOwned===!1).length;n==="admin"&&l>0?t.textContent=`${c} owned + ${l} view-only accounts imported! Enriching...`:t.textContent=`${r.length} active + ${s.length} prospect accounts imported! Enriching...`,t.className="eudia-setup-sf-status success";let d=[...r,...s],u=e,m=n;setTimeout(async()=>{try{let f=d.filter(h=>h.id&&h.id.startsWith("001"));if(f.length>0?(t.textContent=`Enriching ${f.length} accounts with Salesforce data...`,await this.plugin.enrichAccountFolders(f),t.textContent=`${r.length} accounts imported, ${f.length} enriched with Salesforce data`):t.textContent=`${r.length} accounts imported (enrichment requires Salesforce IDs)`,m==="cs"&&B(u))try{console.log("[Eudia SF Import] Regenerating CS Manager dashboard with live CSM data..."),await this.plugin.createCSManagerDashboard(u,r),console.log("[Eudia SF Import] CS Manager dashboard updated with CSM assignments")}catch(h){console.error("[Eudia SF Import] Dashboard regeneration failed (non-blocking):",h)}}catch(f){console.log("[Eudia] Background enrichment skipped:",f),t.textContent=`${r.length+s.length} accounts imported (enrichment will retry on next launch)`}},500)}catch{t.textContent="Failed to import accounts. Please try again.",t.className="eudia-setup-sf-status error";let n=this.plugin.app.workspace.leftSplit;if(n?.collapsed===!1)try{n.expand()}catch{}}}renderTranscribeStep(t){let e=this.steps[2],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),s=r.createDiv({cls:"eudia-setup-step-number"});s.setText(e.status==="complete"?"":"3"),e.status==="complete"&&s.addClass("eudia-step-complete");let a=r.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:e.title}),a.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),l=c.createDiv({cls:"eudia-setup-instruction"});l.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),l.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),d.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let u=c.createDiv({cls:"eudia-setup-instruction"});u.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),u.createSpan({text:"AI will summarize and extract key insights automatically"}),e.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(t){let e=t.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(s=>s.status==="complete")){let s=e.createDiv({cls:"eudia-setup-completion"});s.createEl("h2",{text:"\u{1F389} You're all set!"}),s.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let a=e.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});a.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(L),await this.plugin.activateCalendarView()}}else{let s=e.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});s.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(L),new p.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let r=e.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});r.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},ae=class extends p.ItemView{constructor(t,e){super(t);this.refreshInterval=null;this.lastError=null;this.plugin=e}getViewType(){return _}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let t=this.containerEl.children[1];if(t.empty(),t.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(t);return}this.renderHeader(t),await this.renderCalendarContent(t)}renderHeader(t){let e=t.createDiv({cls:"eudia-calendar-header"}),n=e.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let r=n.createDiv({cls:"eudia-calendar-actions"}),s=r.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});s.title="Refresh (fetches latest from calendar)",s.onclick=async()=>{s.addClass("spinning"),this._forceRefresh=!0,await this.render(),s.removeClass("spinning")};let a=r.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});a.title="Settings",a.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=e.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(t){let e={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,r=this.plugin.settings.userEmail;try{(await(0,p.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(e.server="connected",e.serverMessage="Server online"):(e.server="error",e.serverMessage="Server unavailable")}catch{e.server="error",e.serverMessage="Cannot reach server"}if(r&&e.server==="connected")try{let d=await(0,p.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(r)}`,method:"GET",throw:!1});d.status===200&&d.json?.authorized?(e.calendar="connected",e.calendarMessage="Calendar synced"):(e.calendar="not_authorized",e.calendarMessage="Not authorized")}catch{e.calendar="error",e.calendarMessage="Error checking access"}if(r&&e.server==="connected")try{let d=await(0,p.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(r)}`,method:"GET",throw:!1});d.status===200&&d.json?.connected?(e.salesforce="connected",e.salesforceMessage="Salesforce connected"):(e.salesforce="not_configured",e.salesforceMessage="Not connected")}catch{e.salesforce="not_configured"}let s=t.createDiv({cls:"eudia-status-indicators"}),a=s.createSpan({cls:`eudia-status-dot ${e.server}`});a.title=e.serverMessage||"Server";let o=s.createSpan({cls:`eudia-status-dot ${e.calendar}`});o.title=e.calendarMessage||"Calendar";let c=s.createSpan({cls:`eudia-status-dot ${e.salesforce}`});if(c.title=e.salesforceMessage||"Salesforce",t.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),e.calendar==="not_authorized"){let d=t.createDiv({cls:"eudia-status-warning"});d.innerHTML=`<strong>${r}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(t){let e=t.createDiv({cls:"eudia-calendar-content"}),n=e.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let r=new F(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),s=this._forceRefresh||!1;this._forceRefresh=!1;let a=await r.getWeekMeetings(s);if(n.remove(),!a.success){this.renderError(e,a.error||"Failed to load calendar");return}let o=Object.keys(a.byDay||{}).sort();if(o.length===0){this.renderEmptyState(e);return}await this.renderCurrentMeeting(e,r);for(let c of o){let l=a.byDay[c];!l||l.length===0||this.renderDaySection(e,c,l)}}catch(r){n.remove(),this.renderError(e,r.message||"Failed to load calendar")}}async renderCurrentMeeting(t,e){try{let n=await e.getCurrentMeeting();if(n.meeting){let r=t.createDiv({cls:"eudia-now-card"});n.isNow?r.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):r.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let s=r.createDiv({cls:"eudia-now-content"});s.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&s.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let a=r.createEl("button",{cls:"eudia-now-action",text:"Create Note"});a.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(t,e,n){let r=t.createDiv({cls:"eudia-calendar-day"});r.createEl("div",{cls:"eudia-calendar-day-header",text:F.getDayName(e)});for(let s of n){let a=r.createDiv({cls:`eudia-calendar-meeting ${s.isCustomerMeeting?"customer":"internal"}`});a.createEl("div",{cls:"eudia-calendar-time",text:F.formatTime(s.start,this.plugin.settings.timezone)});let o=a.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:s.subject}),s.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:s.accountName});else if(s.attendees&&s.attendees.length>0){let c=s.attendees.slice(0,2).map(l=>l.name||l.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}a.onclick=()=>this.createNoteForMeeting(s),a.title="Click to create meeting note"}}renderEmptyState(t){let e=t.createDiv({cls:"eudia-calendar-empty"});e.innerHTML=`
    ...
    `}renderError(t,e){let n=t.createDiv({cls:"eudia-calendar-error"}),r="",s="Unable to load calendar",a="";e.includes("not authorized")||e.includes("403")?(r="\u{1F512}",s="Calendar Access Required",a="Contact your admin to be added to the authorized users list."):e.includes("network")||e.includes("fetch")?(r="\u{1F4E1}",s="Connection Issue",a="Check your internet connection and try again."):(e.includes("server")||e.includes("500"))&&(r="\u{1F527}",s="Server Unavailable",a="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=e.createDiv({cls:"eudia-setup-input-group"}),r=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});r.addClass("eudia-setup-email");let s=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),a=e.createDiv({cls:"eudia-setup-status"});s.onclick=async()=>{let o=r.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){a.textContent="Please use your @eudia.com email",a.className="eudia-setup-status error";return}s.disabled=!0,s.textContent="Connecting...",a.textContent="";try{if(!(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){a.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,a.className="eudia-setup-status error",s.disabled=!1,s.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),a.textContent="Connected",a.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let l=c.message||"Connection failed";l.includes("403")?a.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:a.textContent=l,a.className="eudia-setup-status error",s.disabled=!1,s.textContent="Connect"}},r.onkeydown=o=>{o.key==="Enter"&&s.click()},e.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(t){let e=t.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],r=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],s=e.filter(o=>!r.includes(o)&&o.length>1);if(s.length===0)return e[0]||"";if(s.length>1&&n.includes(s[0]))return s[1].charAt(0).toUpperCase()+s[1].slice(1);let a=s[s.length-1];return a.charAt(0).toUpperCase()+a.slice(1)}getExternalDomainsFromAttendees(t){if(!t||t.length===0)return[];let e=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,r=[];for(let s of t){if(!s.email)continue;let o=s.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let c=o[1];if(c.includes("eudia.com")||e.includes(c)||n.has(c))continue;n.add(c);let l=this.extractCompanyFromDomain(c);l.length>=2&&r.push({domain:c,company:l})}}return r}findBestAccountMatch(t,e,n){let r=this.plugin.settings.accountsFolder||"Accounts",s=this.app.vault.getAbstractFileByPath(r);if(!(s instanceof p.TFolder))return null;let a=[];for(let c of s.children)c instanceof p.TFolder&&a.push(c.name);if(a.length===0)return null;let o=[];for(let{domain:c,company:l}of t){let d=this.findAccountFolder(l),u=d?1:0;o.push({domain:c,company:l,folder:d,score:u})}if(o.sort((c,l)=>l.score-c.score),o.length>0&&o[0].folder){let c=o[0],l=c.folder.split("/").pop()||c.company;return console.log(`[Eudia Calendar] Best domain match: "${c.company}" from ${c.domain} -> ${c.folder}`),{folder:c.folder,accountName:l,source:"domain"}}if(e){let c=this.findAccountFolder(e);if(c){let l=c.split("/").pop()||e;return console.log(`[Eudia Calendar] Server account match: "${e}" -> ${c}`),{folder:c,accountName:l,source:"server"}}}if(n){let c=this.findAccountFolder(n);if(c){let l=c.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${c}`),{folder:c,accountName:l,source:"subject"}}}for(let{company:c}of t){let l=a.find(d=>{let u=d.toLowerCase(),m=c.toLowerCase();return u.includes(m)||m.includes(u)});if(l){let d=`${r}/${l}`;return console.log(`[Eudia Calendar] Partial domain match: "${c}" -> ${d}`),{folder:d,accountName:l,source:"domain-partial"}}}return null}extractAccountFromAttendees(t){let e=this.getExternalDomainsFromAttendees(t);if(e.length===0)return null;let n=e[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(t){if(!t)return null;let e=t.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(e){let r=(e[1]||e[2]||"").trim();if(r.toLowerCase()!=="eudia")return r}let n=t.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let s=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(s.toLowerCase()!=="eudia"&&s.length>0)return s}if(!t.toLowerCase().includes("eudia")){let r=t.match(/^([^-–|]+)/);if(r){let s=r[1].trim();if(s.length>2&&s.length<50)return s}}return null}findAccountFolder(t){if(!t)return null;let e=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(e);if(!(n instanceof p.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${e}" not found`),null;let r=t.toLowerCase().trim(),s=[];for(let u of n.children)u instanceof p.TFolder&&s.push(u.name);console.log(`[Eudia Calendar] Searching for "${r}" in ${s.length} folders`);let a=s.find(u=>u.toLowerCase()===r);if(a)return console.log(`[Eudia Calendar] Exact match found: ${a}`),`${e}/${a}`;let o=s.find(u=>u.toLowerCase().startsWith(r));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${e}/${o}`;let c=s.find(u=>r.startsWith(u.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${e}/${c}`;let l=s.find(u=>{let m=u.toLowerCase();return m.length>=3&&r.includes(m)});if(l)return console.log(`[Eudia Calendar] Search contains folder match: ${l}`),`${e}/${l}`;let d=s.find(u=>{let m=u.toLowerCase();return r.length>=3&&m.includes(r)});return d?(console.log(`[Eudia Calendar] Folder contains search match: ${d}`),`${e}/${d}`):(console.log(`[Eudia Calendar] No folder match found for "${r}"`),null)}async createNoteForMeeting(t){let e=t.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",r=D(n),s=(t.attendees||[]).map(C=>C.email).filter(Boolean),a=ce(t.subject,s);if(r&&a.isPipelineMeeting&&a.confidence>=60){await this._createPipelineNote(t,e);return}let o=t.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),c=`${e} - ${o}.md`,l=null,d=t.accountName||null,u=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${t.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(t.attendees?.map(C=>C.email)||[])}`);let m=this.getExternalDomainsFromAttendees(t.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(m)}`);let f=this.extractAccountFromSubject(t.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${f||"none"}"`);let h=this.findBestAccountMatch(m,t.accountName,f||void 0);if(h&&(l=h.folder,d=h.accountName,console.log(`[Eudia Calendar] Best match (${h.source}): "${d}" -> ${l}`)),!l){let C=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(C)instanceof p.TFolder&&(l=C,console.log(`[Eudia Calendar] No match found, using Accounts root: ${l}`))}if(d){let C=this.plugin.settings.cachedAccounts.find(w=>w.name.toLowerCase()===d?.toLowerCase());C&&(u=C.id,d=C.name,console.log(`[Eudia Calendar] Matched to cached account: ${C.name} (${C.id})`))}let v=l?`${l}/${c}`:c,g=this.app.vault.getAbstractFileByPath(v);if(g instanceof p.TFile){await this.app.workspace.getLeaf().openFile(g);try{let C=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;C?.revealInFolder&&C.revealInFolder(g)}catch{}new p.Notice(`Opened existing note: ${c}`);return}let y=(t.attendees||[]).map(C=>C.name||C.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),b=`---
title: "${t.subject}"
date: ${e}
attendees: [${y}]
account: "${d||""}"
...
`;try{let v=await this.app.vault.create(u,h);await this.app.workspace.getLeaf().openFile(v);try{let g=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;g?.revealInFolder&&g.revealInFolder(v)}catch{}new p.Notice(`Created pipeline note: ${c}`),console.log(`[Eudia Pipeline] Created pipeline meeting note: ${u}`)}catch(v){console.error("[Eudia Pipeline] Failed to create pipeline note:",v),new p.Notice(`Could not create pipeline note: ${v.message||"Unknown error"}`)}}},q=class extends p.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null;this.micRibbonIcon=null;this.liveTranscript="";this.liveTranscriptChunkInterval=null;this.isTranscribingChunk=!1;this.sfSyncStatusBarEl=null;this.sfSyncIntervalId=null}async onload(){await this.loadSettings(),this.transcriptionService=new z(this.settings.serverUrl),this.calendarService=new F(this.settings.serverUrl,this.settings.userEmail,this.settings.timezone||"America/New_York"),this.smartTagService=new Y,this.checkForPluginUpdate(),this.registerInterval(window.setInterval(()=>this.checkForPluginUpdate(),2*60*60*1e3)),this.registerView(_,t=>new ae(t,this)),this.registerView(L,t=>new ne(t,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.micRibbonIcon=this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.registerEvent(this.app.vault.on("create",async t=>{if(!(t instanceof p.TFile)||t.extension!=="md")return;let e=this.settings.accountsFolder||"Accounts";if(!t.path.startsWith(e+"/")||!t.basename.startsWith("Untitled"))return;let n=t.path.split("/");if(n.length<3)return;let r=n[1],s=n.slice(0,2).join("/"),a="",o=["Contacts.md","Note 1.md","Intelligence.md"];for(let g of o){let y=this.app.vault.getAbstractFileByPath(`${s}/${g}`);if(y instanceof p.TFile)try{let C=(await this.app.vault.read(y)).match(/account_id:\s*"?([^"\n]+)"?/);if(C){a=C[1].trim();break}}catch{}}let c=this.app.vault.getAbstractFileByPath(s),l=0;if(c&&c.children)for(let g of c.children){let y=g.name?.match(/^Note\s+(\d+)/i);y&&(l=Math.max(l,parseInt(y[1])))}let d=l+1,u=new Date,m=u.toLocaleDateString("en-US",{month:"short",day:"numeric"}),f=u.toISOString().split("T")[0],h=`Note ${d} - ${m}.md`,v=`---
account: "${r}"
account_id: "${a}"
type: meeting_note
sync_to_salesforce: false
...
`;try{let g=`${s}/${h}`;await this.app.vault.modify(t,v),await this.app.fileManager.renameFile(t,g),console.log(`[Eudia] Auto-templated: ${g} (account_id: ${a})`)}catch(g){console.warn("[Eudia] Auto-template failed:",g)}})),this.addCommand({id:"transcribe-meeting",name:"Transcribe Meeting",callback:async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}}),this.addCommand({id:"open-calendar",name:"Open Calendar",callback:()=>this.activateCalendarView()}),this.addCommand({id:"sync-accounts",name:"Sync Salesforce Accounts",callback:()=>this.syncAccounts()}),this.addCommand({id:"sync-note",name:"Sync Note to Salesforce",callback:()=>this.syncNoteToSalesforce()}),this.addCommand({id:"new-meeting-note",name:"New Meeting Note",callback:()=>this.createMeetingNote()}),this.addCommand({id:"ask-gtm-brain",name:"Ask gtm-brain",callback:()=>this.openIntelligenceQueryForCurrentNote()}),this.addCommand({id:"enrich-accounts",name:"Enrich Account Folders with Salesforce Data",callback:async()=>{if(!this.settings.userEmail){new p.Notice("Please set up your email first.");return}let t=new H(this.settings.serverUrl),e;R(this.settings.userEmail)?e=await t.getCSAccounts(this.settings.userEmail):e=await t.getAccountsWithProspects(this.settings.userEmail);let n=[...e.accounts,...e.prospects];if(n.length===0){new p.Notice("No accounts found to enrich.");return}await this.enrichAccountFolders(n)}}),this.addCommand({id:"refresh-analytics",name:"Refresh Analytics Dashboard",callback:async()=>{let t=this.app.workspace.getActiveFile();t?await this.refreshAnalyticsDashboard(t):new p.Notice("No active file")}}),this.addCommand({id:"live-query-transcript",name:"Query Current Transcript (Live)",callback:async()=>{if(!this.audioRecorder?.isRecording()){new p.Notice("No active recording. Start recording first to use live query.");return}if(!this.liveTranscript||this.liveTranscript.length<50){new p.Notice("Not enough transcript captured yet. Keep recording for a few more minutes.");return}this.openLiveQueryModal()}}),this.sfSyncStatusBarEl=this.addStatusBarItem(),this.sfSyncStatusBarEl.setText("SF Sync: Idle"),this.sfSyncStatusBarEl.addClass("eudia-sf-sync-status"),this.addSettingTab(new se(this.app,this)),this.registerEditorSuggest(new Q(this.app,this)),this.app.workspace.onLayoutReady(async()=>{if(this.settings.setupCompleted){if(this.settings.syncOnStartup){if(await this.scanLocalAccountFolders(),this.settings.userEmail&&this.settings.syncAccountsOnStartup){let t=new Date().toISOString().split("T")[0];this.settings.lastAccountRefreshDate!==t&&setTimeout(async()=>{try{console.log("[Eudia] Startup account sync - checking for changes...");let n=await this.syncAccountFolders();if(n.success){if(this.settings.lastAccountRefreshDate=t,await this.saveSettings(),n.added>0||n.archived>0){let r=[];n.added>0&&r.push(`${n.added} added`),n.archived>0&&r.push(`${n.archived} archived`),new p.Notice(`Account folders synced: ${r.join(", ")}`)}}else console.log("[Eudia] Sync failed:",n.error)}catch{console.log("[Eudia] Startup sync skipped (server unreachable), will retry tomorrow")}},2e3)}this.settings.showCalendarView&&this.settings.userEmail&&await this.activateCalendarView(),this.settings.userEmail&&this.settings.cachedAccounts.length>0&&setTimeout(async()=>{try{await this.checkAndAutoEnrich()}catch{console.log("[Eudia] Auto-enrich skipped (server unreachable)")}},5e3),this.settings.userEmail&&this.telemetry?setTimeout(async()=>{try{let t=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),e=0;t&&t instanceof p.TFolder&&(e=t.children.filter(s=>s instanceof p.TFolder&&!s.name.startsWith("_")).length);let n={salesforce:this.settings.salesforceConnected?"connected":"not_configured",calendar:this.settings.calendarConfigured?"connected":"not_configured"};await this.telemetry.sendHeartbeat(e,n);let r=await this.telemetry.checkForPushedConfig();if(r.length>0){let s=!1;for(let a of r)a.key&&this.settings.hasOwnProperty(a.key)&&(this.settings[a.key]=a.value,s=!0,console.log(`[Eudia] Applied pushed config: ${a.key} = ${a.value}`));s&&(await this.saveSettings(),new p.Notice("Settings updated by admin"))}await this.checkAndConsumeSyncFlags(),this.startSalesforceSyncScanner()}catch{console.log("[Eudia] Heartbeat/config check skipped"),this.startSalesforceSyncScanner()}},3e3):this.settings.sfAutoSyncEnabled&&this.settings.salesforceConnected&&setTimeout(()=>this.startSalesforceSyncScanner(),5e3)}}else{await new Promise(e=>setTimeout(e,100));let t=document.querySelector(".modal-container .modal");if(t){let e=t.querySelector(".modal-close-button");e&&e.click()}await this.activateSetupView()}this.app.workspace.on("file-open",async t=>{if(t&&(t.path.includes("_Analytics/")||t.path.includes("_Customer Health/")))try{let e=await this.app.vault.read(t);if(e.includes("type: analytics_dashboard")){let r=e.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/)?.[1],s=new Date().toISOString().split("T")[0];r!==s&&(console.log(`[Eudia] Auto-refreshing analytics: ${t.name}`),await this.refreshAnalyticsDashboard(t))}}catch{}})})}async onunload(){this.app.workspace.detachLeavesOfType(_)}async checkForPluginUpdate(){try{let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",e=await(0,p.requestUrl)({url:`${t}/api/plugin/version`,method:"GET",headers:{"Content-Type":"application/json"}});if(!e.json?.success)return;let n=e.json.currentVersion,r=this.manifest?.version||"0.0.0",s=n.split(".").map(Number),a=r.split(".").map(Number),o=!1;for(let c=0;c<3;c++){if((s[c]||0)>(a[c]||0)){o=!0;break}if((s[c]||0)<(a[c]||0))break}o?(console.log(`[Eudia Update] New version available: ${n} (current: ${r})`),await this.performAutoUpdate(t,n,r)):console.log(`[Eudia Update] Plugin is up to date (v${r})`)}catch(t){console.log("[Eudia Update] Could not check for updates:",t.message||t)}}async performAutoUpdate(t,e,n){try{if(this.audioRecorder?.isRecording()){console.log("[Eudia Update] Skipping auto-update \u2014 recording in progress"),new p.Notice(`Eudia update v${e} available.
Finish your recording, then restart Obsidian to update.`,1e4);return}let r=this.manifest.dir;if(!r){console.log("[Eudia Update] Cannot determine plugin directory \u2014 skipping");return}let s=this.app.vault.adapter;console.log("[Eudia Update] Downloading plugin files...");let[a,o,c]=await Promise.all([(0,p.requestUrl)({url:`${t}/api/plugin/main.js`}),(0,p.requestUrl)({url:`${t}/api/plugin/manifest.json`}),(0,p.requestUrl)({url:`${t}/api/plugin/styles.css`})]),l=a.text,d=o.text,u=c.text,m=1024,f=5*1024*1024;for(let[h,v]of[["main.js",l],["manifest.json",d],["styles.css",u]])if(!v||v.length<m||v.length>f){console.log(`[Eudia Update] Downloaded ${h} failed validation (${v?.length??0} bytes) \u2014 aborting`);return}try{let h=await s.read(`${r}/main.js`);await s.write(`${r}/main.js.bak`,h),console.log("[Eudia Update] Backed up current main.js")}catch{console.log("[Eudia Update] Could not back up main.js \u2014 continuing")}await s.write(`${r}/main.js`,l),await s.write(`${r}/manifest.json`,d),await s.write(`${r}/styles.css`,u),console.log(`[Eudia Update] Files written \u2014 v${n} \u2192 v${e}`);try{(0,p.requestUrl)({url:`${t}/api/plugin/telemetry`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:"info",message:`Auto-updated from v${n} to v${e}`,userEmail:this.settings.userEmail||"anonymous",pluginVersion:e,platform:"obsidian",context:{fromVersion:n,toVersion:e}})}).catch(()=>{})}catch{}this.audioRecorder?.isRecording()?new p.Notice(`Eudia v${e} downloaded. Will apply after recording.`,1e4):(new p.Notice(`Eudia updated to v${e}. Applying now...`,3e3),setTimeout(async()=>{try{let h=this.app.plugins,v=this.manifest.id;await h.disablePlugin(v),await h.enablePlugin(v),console.log(`[Eudia Update] Hot-reloaded: v${n} \u2192 v${e}`),new p.Notice(`Eudia v${e} is now active.`,5e3)}catch(h){console.log("[Eudia Update] Hot-reload failed, will apply on next restart:",h),new p.Notice(`Eudia updated to v${e}. Restart Obsidian to apply.`,1e4)}},1500))}catch(r){console.log("[Eudia Update] Auto-update failed:",r.message||r)}}async loadSettings(){this.settings=Object.assign({},Le,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async activateCalendarView(){let t=this.app.workspace,e=t.getLeavesOfType(_);if(e.length>0)t.revealLeaf(e[0]);else{let n=t.getRightLeaf(!1);n&&(await n.setViewState({type:_,active:!0}),t.revealLeaf(n))}}async activateSetupView(){let t=this.app.workspace,e=t.getLeavesOfType(L);if(e.length>0)t.revealLeaf(e[0]);else{let n=t.getLeaf(!0);n&&(await n.setViewState({type:L,active:!0}),t.revealLeaf(n))}}async createTailoredAccountFolders(t,e){let n=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);let s=0,a=new Date().toISOString().split("T")[0],o=async l=>{let d=l.name.replace(/[<>:"/\\|?*]/g,"_").trim(),u=`${n}/${d}`;if(this.app.vault.getAbstractFileByPath(u)instanceof p.TFolder)return console.log(`[Eudia] Account folder already exists: ${d}`),!1;try{await this.app.vault.createFolder(u);let f=e?.[l.id],h=!!f,v=this.buildContactsContent(l,f,a),g=this.buildIntelligenceContent(l,f,a),y=this.buildMeetingNotesContent(l,f),b=this.buildNextStepsContent(l,f,a),C=[{name:"Note 1.md",content:`---
account: "${l.name}"
account_id: "${l.id}"
type: meeting_note
sync_to_salesforce: false
...
`},{name:"Meeting Notes.md",content:y},{name:"Contacts.md",content:v},{name:"Intelligence.md",content:g},{name:"Next Steps.md",content:b}];for(let S of C){let j=`${u}/${S.name}`;await this.app.vault.create(j,S.content)}return console.log(`[Eudia] Created account folder with subnotes${h?" (enriched)":""}: ${d}`),!0}catch(f){return console.error(`[Eudia] Failed to create folder for ${d}:`,f),!1}},c=5;for(let l=0;l<t.length;l+=c){let d=t.slice(l,l+c),u=await Promise.allSettled(d.map(m=>o(m)));s+=u.filter(m=>m.status==="fulfilled"&&m.value===!0).length}s>0?(this.settings.cachedAccounts=t.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),new p.Notice(`Created ${s} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let r=e?`
...
*Log key interactions and relationship developments.*
`}buildIntelligenceContent(t,e,n){let r=e?`
enriched_at: "${new Date().toISOString()}"`:"",s=`---
account: "${t.name}"
account_id: "${t.id}"
type: intelligence
sync_to_salesforce: false${r}
---`;return e?.intelligence?`${s}
...
---`,s=[];return e?.opportunities&&s.push(e.opportunities),e?.recentActivity&&s.push(e.recentActivity),s.length>0?`${r}
...
3. **Next Steps** are auto-extracted after transcription
4. Set \`sync_to_salesforce: true\` to sync to Salesforce
`}buildNextStepsContent(t,e,n){let r=n||new Date().toISOString().split("T")[0],s=e?`
...
*Previous next steps will be archived here.*
`}async fetchEnrichmentData(t){let e=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=t.filter(a=>a.id&&a.id.startsWith("001"));if(n.length===0)return{};let r={},s=20;console.log(`[Eudia Enrich] Fetching enrichment data for ${n.length} accounts`);for(let a=0;a<n.length;a+=s){let c=n.slice(a,a+s).map(l=>l.id);try{let l=await(0,p.requestUrl)({url:`${e}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:c,userEmail:this.settings.userEmail})});l.json?.success&&l.json?.enrichments&&Object.assign(r,l.json.enrichments)}catch(l){console.error(`[Eudia Enrich] Batch fetch failed (batch ${a/s+1}):`,l)}a+s<n.length&&await new Promise(l=>setTimeout(l,100))}return console.log(`[Eudia Enrich] Got enrichment data for ${Object.keys(r).length}/${n.length} accounts`),r}async createProspectAccountFiles(t){if(!t||t.length===0)return 0;let e=this.settings.accountsFolder||"Accounts",n=`${e}/_Prospects`;if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s=0;for(let a of t){let o=a.name.replace(/[<>:"/\\|?*]/g,"_").trim(),c=`${n}/${o}`;if(this.app.vault.getAbstractFileByPath(c)instanceof p.TFolder)continue;let d=`${e}/${o}`;if(this.app.vault.getAbstractFileByPath(d)instanceof p.TFolder)continue;let m=`${n}/${o}.md`,f=this.app.vault.getAbstractFileByPath(m);if(f instanceof p.TFile)try{await this.app.vault.delete(f)}catch{}try{await this.app.vault.createFolder(c);let h=new Date().toISOString().split("T")[0],v=[{name:"Note 1.md",content:`---
account: "${a.name}"
account_id: "${a.id}"
type: meeting_note
tier: prospect
...
*Previous next steps will be archived here.*
`}];for(let g of v){let y=`${c}/${g.name}`;await this.app.vault.create(y,g.content)}s++}catch(h){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,h)}}return s>0&&console.log(`[Eudia] Created ${s} prospect account folders in _Prospects/`),s}async createCSManagerDashboard(t,e){let n="CS Manager",r=new Date().toISOString().split("T")[0],s=he(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let a={};for(let u of e){let m=u.ownerName||"Unassigned";a[m]||(a[m]=[]),a[m].push(u)}let o=`---
role: cs_manager
manager: "${t}"
direct_reports: ${s.length}
...
**Manager:** ${t}
**Direct Reports:** ${s.join(", ")||"None configured"}
...
`,c=Object.keys(a).sort();for(let u of c){let m=a[u];o+=`### ${u} (${m.length} accounts)
`;for(let f of m.slice(0,10))o+=`- **${f.name}** \u2014 ${f.type||"Account"}
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let u of s){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,w=>w.toUpperCase()),f=u.split("@")[0].replace("."," ").toLowerCase(),h=f.split(" ")[0],v=f.split(" ").pop()||"",g=e.filter(w=>{let S=(w.csmName||"").toLowerCase();if(S&&(S.includes(h)||S.includes(v)))return!0;let j=(w.ownerName||"").toLowerCase();return j.includes(h)||j.includes(v)}),y=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
...
`;if(g.length>0){y+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let w of g){let S=w.name.replace(/[<>:"/\\|?*]/g,"_").trim();y+=`| ${w.name} | ${w.type||""} | ${w.ownerName||""} | [[Accounts/${S}/Contacts\\|View]] |
...
`;let b=`${n}/${m}.md`,C=this.app.vault.getAbstractFileByPath(b);C instanceof p.TFile?await this.app.vault.modify(C,y):await this.app.vault.create(b,y)}console.log(`[Eudia] Created CS Manager dashboard for ${t} with ${e.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(t){let e=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),await this.ensurePipelineFolderExists();let r=0,s=0,a=new Date().toISOString().split("T")[0],o=async l=>{let d=l.name.replace(/[<>:"/\\|?*]/g,"_").trim(),u=`${e}/${d}`;if(this.app.vault.getAbstractFileByPath(u)instanceof p.TFolder)return!1;try{return await this.app.vault.createFolder(u),await this.createExecAccountSubnotes(u,l,a),l.isOwned?r++:s++,console.log(`[Eudia Admin] Created ${l.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(f){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,f),!1}},c=5;for(let l=0;l<t.length;l+=c){let d=t.slice(l,l+c);await Promise.allSettled(d.map(u=>o(u)))}this.settings.cachedAccounts=t.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),r+s>0&&new p.Notice(`Created ${r} owned + ${s} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(t,e,n){let r=e.ownerName||"Unknown",s=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let a of s){let o=`${t}/${a.name}`;await this.app.vault.create(o,a.content)}}async createFullAccountSubnotes(t,e,n){let r=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let s of r){let a=`${t}/${s.name}`;await this.app.vault.create(a,s.content)}}async ensurePipelineFolderExists(){let t="Pipeline",e=`${t}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),!this.app.vault.getAbstractFileByPath(e)){let a=`---
...
### New Opportunities
*None yet*
`;await this.app.vault.create(e,a)}}async ensureNextStepsFolderExists(){let t="Next Steps",e=`${t}/All Next Steps.md`;if(this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),!this.app.vault.getAbstractFileByPath(e)){let s=new Date().toISOString().split("T")[0],a=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
...
`;await this.app.vault.create(e,o)}}async updateAccountNextSteps(t,e,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${t}`),console.log(`[Eudia] Content length: ${e?.length||0} chars`);let r=t.replace(/[<>:"/\\|?*]/g,"_").trim(),s=`${this.settings.accountsFolder}/${r}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${s}`);let a=this.app.vault.getAbstractFileByPath(s);if(!a||!(a instanceof p.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${s}`);let y=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${r}`);y&&y instanceof p.TFolder?console.log(`[Eudia] Files in ${r} folder:`,y.children.map(b=>b.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${r}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),l=n.split("/").pop()?.replace(".md","")||"Meeting",d=e;!e.includes("- [ ]")&&!e.includes("- [x]")&&(d=e.split(`
`).filter(y=>y.trim()).map(y=>{let b=y.replace(/^[-•*]\s*/,"").trim();return b?`- [ ] ${b}`:""}).filter(Boolean).join(`
`));let u=await this.app.vault.read(a),m="",f=u.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);f&&f[1]&&(m=f[1].trim());let h=`### ${o} - ${l}
${d||"*None*"}`,v=m?`${h}
...
${v}
`;await this.app.vault.modify(a,g),console.log(`[Eudia] Updated Next Steps for ${t} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(r){console.error(`[Eudia] Failed to update Next Steps for ${t}:`,r)}}async regenerateNextStepsDashboard(){try{let e=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!e||!(e instanceof p.TFile)){await this.ensureNextStepsFolderExists();return}let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!n||!(n instanceof p.TFolder))return;let r=[];for(let c of n.children)if(c instanceof p.TFolder){let l=`${c.path}/Next Steps.md`,d=this.app.vault.getAbstractFileByPath(l);if(d instanceof p.TFile){let u=await this.app.vault.read(d),m=u.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),f=m?m[1]:"Unknown",h=u.split(`
`).filter(v=>v.match(/^- \[[ x]\]/)).slice(0,5);(h.length>0||f!=="Unknown")&&r.push({account:c.name,lastUpdated:f,nextSteps:h})}}r.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let s=new Date().toISOString().split("T")[0],a=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${s}
---
...
---
`;await this.app.vault.modify(e,c+l),this.processTranscriptionAsync(t,e).catch(d=>{console.error("Background transcription failed:",d),new p.Notice(`Transcription failed: ${d.message}`)})}async processTranscriptionAsync(t,e){try{let n={},r=e.path.split("/");console.log(`[Eudia] Processing transcription for: ${e.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(r)}, accountsFolder: ${this.settings.accountsFolder}`);let s=r[0]==="Pipeline Meetings",a=!1;try{let y=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);y&&(a=/meeting_type:\s*pipeline_review/.test(y[1]))}catch{}if(!a&&s&&(a=!0),a){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let g="";try{let y=await(0,p.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});y.json?.success&&y.json?.context&&(g=y.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${g.length} chars)`))}catch(y){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",y)}n={meetingType:"pipeline_review",pipelineContext:g}}else if(r.length>=2&&r[0]===this.settings.accountsFolder){let g=r[1];console.log(`[Eudia] Detected account folder: ${g}`);let y=this.settings.cachedAccounts.find(b=>b.name.toLowerCase()===g.toLowerCase());y?(n={accountName:y.name,accountId:y.id,userEmail:this.settings.userEmail},console.log(`[Eudia] Found cached account: ${y.name} (${y.id})`)):(n={accountName:g,accountId:"",userEmail:this.settings.userEmail},console.log(`[Eudia] Account not in cache, using folder name: ${g}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let g=await this.calendarService.getCurrentMeeting();g.meeting?.attendees&&(o=g.meeting.attendees.map(y=>y.name||y.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let c=await this.transcriptionService.transcribeAudio(t.audioBlob,{...n,speakerHints:o,captureMode:t.captureMode,hasVirtualDevice:t.hasVirtualDevice}),l=g=>g?!!(g.summary?.trim()||g.nextSteps?.trim()):!1,d=c.sections;if(l(d)||c.text?.trim()&&(d=await this.transcriptionService.processTranscription(c.text,n)),!l(d)&&!c.text?.trim()){let y=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(e,y+`

**Transcription failed:** No audio detected.
`),new p.Notice("Transcription failed: No audio detected.");return}let u=await this.app.vault.read(e),m="",f=Math.max(u.indexOf(`---
**Processing your recording`),u.indexOf(`---
**Transcription in progress`));if(f>0){let g=u.indexOf("---"),y=g>=0?u.indexOf("---",g+3):-1;y>0&&y+3<f&&(m=u.substring(y+3,f).trim())}else{let g=u.indexOf("---"),y=g>=0?u.indexOf("---",g+3):-1;if(y>0){let b=u.substring(y+3).trim();b.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(m=b)}}try{let g="_backups";this.app.vault.getAbstractFileByPath(g)||await this.app.vault.createFolder(g);let y=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),b=`${g}/${e.name}_${y}.md`;await this.app.vault.create(b,u),console.log(`[Eudia] Backed up note to ${b}`)}catch(g){console.warn("[Eudia] Backup failed (non-critical):",g.message)}let h;if(a?h=this.buildPipelineNoteContent(d,c,e.path):h=this.buildNoteContent(d,c),m&&m.length>5){let g=h.indexOf("---",h.indexOf("---")+3);if(g>0){let y=h.substring(0,g+3),b=h.substring(g+3);h=y+`
...
---
`+b}}await this.app.vault.modify(e,h);let v=Math.floor(t.duration/60);if(new p.Notice(`Transcription complete (${v} min recording)`),!a){let g=d.nextSteps||d.actionItems;console.log(`[Eudia] Next Steps extraction - accountContext: ${n?.accountName||"undefined"}`),console.log(`[Eudia] Next Steps content found: ${g?"YES ("+g.length+" chars)":"NO"}`),console.log(`[Eudia] sections.nextSteps: ${d.nextSteps?"YES":"NO"}, sections.actionItems: ${d.actionItems?"YES":"NO"}`),g&&n?.accountName?(console.log(`[Eudia] Calling updateAccountNextSteps for ${n.accountName}`),await this.updateAccountNextSteps(n.accountName,g,e.path)):console.log("[Eudia] Skipping Next Steps update - missing content or account context")}this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(n){try{let s=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(e,s+`

**Transcription failed:** ${n.message}
`)}catch{}throw n}}buildPipelineNoteContent(t,e,n){let r=new Date,s=String(r.getMonth()+1).padStart(2,"0"),a=String(r.getDate()).padStart(2,"0"),o=String(r.getFullYear()).slice(-2),c=r.toISOString().split("T")[0],l=`${s}.${a}.${o}`,d=h=>h==null?"":Array.isArray(h)?h.map(String).join(`
`):typeof h=="object"?JSON.stringify(h,null,2):String(h),u=d(t.summary),m=e.transcript||e.text||"",f=`---
...
`;if(u)f+=u;else{let h=[t.painPoints,t.productInterest,t.nextSteps,t.actionItems].filter(Boolean).map(d).join(`

`);h?f+=h:f+="*Pipeline summary could not be generated. See transcript below.*"}return m&&(f+=`
...
`),f}buildNoteContent(t,e){let n=g=>g==null?"":Array.isArray(g)?g.map(y=>typeof y=="object"?y.category?`**${y.category}**: ${y.signal||y.insight||""}`:JSON.stringify(y):String(y)).join(`
`):typeof g=="object"?JSON.stringify(g):String(g),r=n(t.title)||"Meeting Notes",s=n(t.summary),a=n(t.painPoints),o=n(t.productInterest),c=n(t.meddiccSignals),l=n(t.nextSteps),d=n(t.actionItems),u=n(t.keyDates),m=n(t.risksObjections),f=n(t.attendees),h=`---
title: "${r}"
...
`;a&&!a.includes("None explicitly")&&(h+=`## Pain Points
...
`),m&&!m.includes("None raised")&&(h+=`## Risks and Objections
...
`),this.settings.appendTranscript&&(e.diarizedTranscript||e.text)){let g=e.diarizedTranscript||e.text,y=e.diarizedTranscript?"Full Transcript (Speaker-Labeled)":"Full Transcript";h+=`---

## ${y}

${g}
`}return h}openIntelligenceQuery(){new J(this.app,this).open()}openIntelligenceQueryForCurrentNote(){let t=this.app.workspace.getActiveFile(),e;if(t){let n=this.app.metadataCache.getFileCache(t)?.frontmatter;if(n?.account_id&&n?.account)e={id:n.account_id,name:n.account};else if(n?.account){let r=this.settings.cachedAccounts.find(s=>s.name.toLowerCase()===n.account.toLowerCase());r?e={id:r.id,name:r.name}:e={id:"",name:n.account}}else{let r=t.path.split("/");if(r.length>=2&&r[0]===this.settings.accountsFolder){let s=r[1],a=this.settings.cachedAccounts.find(o=>o.name.replace(/[<>:"/\\|?*]/g,"_").trim()===s);a?e={id:a.id,name:a.name}:e={id:"",name:s}}}}new J(this.app,this,e).open()}async syncAccounts(t=!1){t||new p.Notice("Syncing Salesforce accounts...");try{let n=(await(0,p.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!n.success||!n.accounts){t||new p.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=n.accounts.map(r=>({id:r.id,name:r.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),t||new p.Notice(`Synced ${n.accounts.length} accounts for matching`)}catch(e){t||new p.Notice(`Failed to sync accounts: ${e.message}`)}}async scanLocalAccountFolders(){try{let t=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!t||!(t instanceof p.TFolder))return;let e=[];for(let n of t.children)n instanceof p.TFolder&&e.push({id:`local-${n.name.replace(/\s+/g,"-").toLowerCase()}`,name:n.name});this.settings.cachedAccounts=e,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(t){console.error("Failed to scan local account folders:",t)}}async refreshAccountFolders(){if(!this.settings.userEmail)throw new Error("Please configure your email first");let t=new H(this.settings.serverUrl);if((await t.getAccountsForUser(this.settings.userEmail)).length===0)return console.log("[Eudia] No accounts found for user"),0;let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),r=[];if(n&&n instanceof p.TFolder)for(let o of n.children)o instanceof p.TFolder&&r.push(o.name);let s=await t.getNewAccounts(this.settings.userEmail,r);if(s.length===0)return console.log("[Eudia] All account folders exist"),0;console.log(`[Eudia] Creating ${s.length} new account folders`);let a=await this.fetchEnrichmentData(s);return await this.createTailoredAccountFolders(s,a),s.length}async checkAndConsumeSyncFlags(){if(!this.settings.userEmail)return;let t=encodeURIComponent(this.settings.userEmail.toLowerCase().trim()),e=this.settings.serverUrl||"https://gtm-wizard.onrender.com";try{let s=((await(0,p.requestUrl)({url:`${e}/api/admin/users/${t}/sync-flags`,method:"GET"})).json?.flags||[]).filter(o=>!o.consumed_at);if(s.length===0)return;console.log(`[Eudia] Found ${s.length} pending sync flag(s)`);let a=!1;for(let o of s)if(o.flag==="resync_accounts"){a=!0;let c=o.payload||{},l=c.added?.length||0,d=c.removed?.length||0;console.log(`[Eudia] Sync flag: resync_accounts (+${l} / -${d})`)}else o.flag==="update_plugin"?new p.Notice("A plugin update is available. Please download the latest vault."):o.flag==="reset_setup"&&(console.log("[Eudia] Sync flag: reset_setup received"),this.settings.setupCompleted=!1,await this.saveSettings(),new p.Notice("Setup has been reset by admin. Please re-run the setup wizard."));if(a){console.log("[Eudia] Triggering account folder resync from sync flag..."),new p.Notice("Syncing account updates...");let o=await this.syncAccountFolders();o.success?new p.Notice(`Account sync complete: ${o.added} new, ${o.archived} archived`):console.log(`[Eudia] Account resync error: ${o.error}`)}try{await(0,p.requestUrl)({url:`${e}/api/admin/users/${t}/sync-flags/consume`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagIds:s.map(o=>o.id)})}),console.log(`[Eudia] Consumed ${s.length} sync flag(s)`)}catch{console.log("[Eudia] Failed to consume sync flags (will retry next startup)")}}catch{console.log("[Eudia] Sync flag check skipped (endpoint not available)")}}async syncAccountFolders(){if(!this.settings.userEmail)return{success:!1,added:0,archived:0,error:"No email configured"};let t=this.settings.userEmail.toLowerCase().trim();console.log(`[Eudia] Syncing account folders for: ${t}`);try{let e=await fetch(`${this.settings.serverUrl}/api/bl-accounts/${encodeURIComponent(t)}`);if(!e.ok){let A=await e.json().catch(()=>({}));throw new Error(A.error||`Server returned ${e.status}`)}let n=await e.json();if(!n.success||!n.accounts)throw new Error(n.error||"Invalid response from server");let r=n.meta?.userGroup||"bl",s=n.meta?.queryDescription||"accounts",a=n.meta?.region||null;console.log(`[Eudia] User group: ${r}, accounts: ${n.accounts.length} (${s})`),a&&console.log(`[Eudia] Sales Leader region: ${a}`);let o=n.accounts||[],c=n.prospectAccounts||[],l=o.length+c.length;console.log(`[Eudia] Server returned: ${o.length} active + ${c.length} prospects = ${l} total`);let d=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),u=new Map,m=`${this.settings.accountsFolder}/_Prospects`,f=this.app.vault.getAbstractFileByPath(m),h=new Map,v=new Map;if(d&&d instanceof p.TFolder)for(let A of d.children)A instanceof p.TFolder&&!A.name.startsWith("_")&&u.set(A.name.toLowerCase().trim(),A);if(f&&f instanceof p.TFolder)for(let A of f.children)A instanceof p.TFolder?h.set(A.name.toLowerCase().trim(),A):A instanceof p.TFile&&A.extension==="md"&&v.set(A.basename.toLowerCase().trim(),A);let g=new Set(o.map(A=>A.name.toLowerCase().trim())),y=o.filter(A=>{let I=A.name.toLowerCase().trim();return!u.has(I)}),b=c.filter(A=>{let I=A.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();return!h.has(I)&&!v.has(I)&&!u.has(A.name.toLowerCase().trim())}),C=[];for(let A of o){let I=A.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();(h.has(I)||v.has(I))&&!u.has(A.name.toLowerCase().trim())&&C.push(A)}let w=new Set([...o.map(A=>A.name.toLowerCase().trim()),...c.map(A=>A.name.toLowerCase().trim())]),S=[];if(r==="bl")for(let[A,I]of u.entries())w.has(A)||S.push(I);let j=0,x=0,W=0,K=0;if(C.length>0){console.log(`[Eudia] Promoting ${C.length} accounts from prospect to active`);for(let A of C){let I=A.name.replace(/[<>:"/\\|?*]/g,"_").trim(),k=h.get(I.toLowerCase()),T=v.get(I.toLowerCase());try{if(k){let M=`${this.settings.accountsFolder}/${I}`;await this.app.vault.rename(k,M),W++,new p.Notice(`${A.name} promoted to active`)}else if(T){await this.app.vault.delete(T);let M=[{id:A.id,name:A.name,type:A.customerType,isOwned:!0,hadOpportunity:!0}],ye=await this.fetchEnrichmentData(M);await this.createTailoredAccountFolders(M,ye),W++,new p.Notice(`${A.name} promoted to active -- full account folder created`)}}catch(M){console.error(`[Eudia] Failed to promote ${A.name}:`,M)}}}if(y.length>0){console.log(`[Eudia] Creating ${y.length} new active account folders for ${r}`);let A=new Set(C.map(k=>k.name.toLowerCase().trim())),I=y.filter(k=>!A.has(k.name.toLowerCase().trim()));if(I.length>0){let k=I.map(T=>({id:T.id,name:T.name,type:T.customerType,isOwned:r==="bl",ownerName:T.ownerName,hadOpportunity:!0}));if(r==="admin"||r==="exec")await this.createAdminAccountFolders(k);else{let T=await this.fetchEnrichmentData(k);await this.createTailoredAccountFolders(k,T)}j=I.length}this.telemetry&&this.telemetry.reportInfo("Accounts synced - added",{count:j,userGroup:r,region:a||void 0})}return b.length>0&&r==="bl"&&(console.log(`[Eudia] Creating ${b.length} new prospect files`),K=await this.createProspectAccountFiles(b.map(A=>({id:A.id,name:A.name,type:"Prospect",hadOpportunity:!1,website:A.website,industry:A.industry})))),this.settings.archiveRemovedAccounts&&S.length>0&&(console.log(`[Eudia] Archiving ${S.length} removed account folders`),x=await this.archiveAccountFolders(S),this.telemetry&&this.telemetry.reportInfo("Accounts synced - archived",{count:x})),console.log(`[Eudia] Sync complete: ${j} active added, ${K} prospects added, ${W} promoted, ${x} archived (group: ${r})`),{success:!0,added:j+K+W,archived:x,userGroup:r}}catch(e){return console.error("[Eudia] Account sync error:",e),this.telemetry&&this.telemetry.reportError("Account sync failed",{error:e.message}),{success:!1,added:0,archived:0,error:e.message}}}async archiveAccountFolders(t){let e=0,n=`${this.settings.accountsFolder}/_Archived`;this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);for(let s of t)try{let a=`${n}/${s.name}`;if(this.app.vault.getAbstractFileByPath(a)){let d=new Date().toISOString().split("T")[0];await this.app.fileManager.renameFile(s,`${n}/${s.name}_${d}`)}else await this.app.fileManager.renameFile(s,a);let c=`${n}/${s.name}/_archived.md`,l=`---
...
`;try{await this.app.vault.create(c,l)}catch{}e++,console.log(`[Eudia] Archived: ${s.name}`)}catch(a){console.error(`[Eudia] Failed to archive ${s.name}:`,a)}return e}async syncSpecificNoteToSalesforce(t){let e=await this.app.vault.read(t),n=this.app.metadataCache.getFileCache(t)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let r=n.account_id,s=n.account;if(!r&&s){let a=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===s.toLowerCase());a&&(r=a.id)}if(!r){let a=t.path.split("/");if(a.length>=2&&a[0]===this.settings.accountsFolder){let o=a[1]==="_Prospects"&&a.length>=3?a[2]:a[1],c=this.settings.cachedAccounts.find(l=>l.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&(r=c.id,s=c.name)}}if(!r)return{success:!1,error:`Could not determine account for ${t.path}`};try{let a=await(0,p.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:r,accountName:s,noteTitle:t.basename,notePath:t.path,content:e,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return a.json?.success?{success:!0}:{success:!1,error:a.json?.error||"Unknown error",authRequired:a.json?.authRequired}}catch(a){return{success:!1,error:a.message}}}async syncNoteToSalesforce(){let t=this.app.workspace.getActiveFile();if(!t){new p.Notice("No active file to sync");return}if(!this.app.metadataCache.getFileCache(t)?.frontmatter?.sync_to_salesforce){new p.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}new p.Notice("Syncing to Salesforce...");let n=await this.syncSpecificNoteToSalesforce(t);n.success?new p.Notice("Synced to Salesforce"):n.authRequired?new p.Notice("Salesforce authentication required. Please reconnect."):new p.Notice("Failed to sync: "+(n.error||"Unknown error"))}async checkAndAutoEnrich(){let t=this.settings.accountsFolder||"Accounts",e=this.app.vault.getAbstractFileByPath(t);if(!e||!(e instanceof p.TFolder))return;let n=[];for(let r of e.children){if(!(r instanceof p.TFolder)||r.name.startsWith("_"))continue;let s=`${r.path}/Contacts.md`,a=this.app.vault.getAbstractFileByPath(s);if(!(!a||!(a instanceof p.TFile))){if(this.app.metadataCache.getFileCache(a)?.frontmatter?.enriched_at)continue}let o=r.name,c=this.settings.cachedAccounts.find(l=>l.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&c.id&&n.push({id:c.id,name:c.name,owner:"",ownerEmail:""})}if(n.length===0){console.log("[Eudia] Auto-enrich: all account folders already enriched");return}console.log(`[Eudia] Auto-enrich: ${n.length} accounts need enrichment`);try{await this.enrichAccountFolders(n)}catch(r){console.error("[Eudia] Auto-enrich failed:",r)}}async enrichAccountFolders(t){if(!t||t.length===0)return;let e=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=this.settings.accountsFolder||"Accounts",r=t.filter(d=>d.id&&d.id.startsWith("001"));if(r.length===0)return;let s=r.length,a=0,o=0;console.log(`[Eudia Enrich] Starting enrichment for ${s} accounts`),new p.Notice(`Enriching account data: 0/${s}...`);let c=20;for(let d=0;d<r.length;d+=c){let u=r.slice(d,d+c),m=u.map(h=>h.id);try{let h=await(0,p.requestUrl)({url:`${e}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:m,userEmail:this.settings.userEmail})});if(h.json?.success&&h.json?.enrichments){let v=h.json.enrichments;for(let g of u){let y=v[g.id];if(y)try{await this.writeEnrichmentToAccount(g,y,n),a++}catch(b){o++,console.error(`[Eudia Enrich] Write failed for ${g.name}:`,b)}}}}catch(h){o+=u.length,console.error("[Eudia Enrich] Batch fetch failed:",h)}let f=Math.min(d+c,s);new p.Notice(`Enriching account data: ${f}/${s}...`),d+c<r.length&&await new Promise(h=>setTimeout(h,100))}let l=o>0?`Enrichment complete: ${a} enriched, ${o} skipped`:`Enrichment complete: ${a} accounts enriched with Salesforce data`;console.log(`[Eudia Enrich] ${l}`),new p.Notice(l)}async writeEnrichmentToAccount(t,e,n){let r=t.name.replace(/[<>:"/\\|?*]/g,"_").trim(),s=`${n}/${r}`,a=this.app.vault.getAbstractFileByPath(s);if(a instanceof p.TFolder||(s=`${n}/_Prospects/${r}`,a=this.app.vault.getAbstractFileByPath(s)),!(a instanceof p.TFolder))return;let o=new Date().toISOString(),c=async(l,d)=>{let u=`${s}/${l}`,m=this.app.vault.getAbstractFileByPath(u);if(!(m instanceof p.TFile))return;let f=await this.app.vault.read(m),h="",v=f;if(f.startsWith("---")){let C=f.indexOf("---",3);C!==-1&&(h=f.substring(0,C+3),v=f.substring(C+3),h.includes("enriched_at:")?h=h.replace(/enriched_at:.*/,`enriched_at: "${o}"`):h=h.substring(0,C)+`enriched_at: "${o}"
---`)}let g=v.match(/^(\s*#[^\n]+)/),b=`${g?g[1]:""}

${d}
`;await this.app.vault.modify(m,`${h}
${b}`)};if(e.contacts&&await c("Contacts.md",`${e.contacts}

## Relationship Map

*Add org chart, decision makers, champions, and blockers here.*`),e.intelligence&&await c("Intelligence.md",e.intelligence),e.nextSteps&&await c("Next Steps.md",e.nextSteps),e.opportunities||e.recentActivity){let l=`${s}/Meeting Notes.md`,d=this.app.vault.getAbstractFileByPath(l);if(d instanceof p.TFile){let u=await this.app.vault.read(d),m="",f=u;if(u.startsWith("---")){let y=u.indexOf("---",3);y!==-1&&(m=u.substring(0,y+3),f=u.substring(y+3),m.includes("enriched_at:")?m=m.replace(/enriched_at:.*/,`enriched_at: "${o}"`):m=m.substring(0,y)+`enriched_at: "${o}"
---`)}let h=f.match(/^(\s*#[^\n]+)/),g=[h?h[1]:`
...
`;await this.app.vault.modify(t,c)}async updateObjectionNote(t,e){if(!e.objections||e.objections.length===0)return;let n=new Date().toISOString().split("T")[0],r=e.objections.slice(0,10).map(o=>{let c=o.handleRatePercent>=75?"\u2705 Strong":o.handleRatePercent>=50?"\u26A0\uFE0F Moderate":"\u274C Needs Work";return`| ${o.objection?.substring(0,40)||"--"}... | ${o.count||0} | ${o.handleRatePercent||0}% | ${c} |`}).join(`
`),s="";for(let o of e.objections.slice(0,5))if(o.bestResponses&&o.bestResponses.length>0){s+=`
### Objection: "${o.objection?.substring(0,50)}..."

`,s+=`**Frequency:** ${o.count} times  
`,s+=`**Handle Rate:** ${o.handleRatePercent}%

`,s+=`**Best Responses:**
`;for(let c of o.bestResponses.slice(0,2))s+=`1. *"${c.response}"* - ${c.rep||"Team member"}
`;s+=`
`}let a=`---
type: analytics_dashboard
auto_refresh: true
...
## Best Practices
${s||"*No best practices available yet*"}
...
`;await this.app.vault.modify(t,a)}async updateTeamPerformanceNote(t,e){if(!e)return;let n=new Date().toISOString().split("T")[0],r=a=>a>0?`\u2191 ${Math.abs(a).toFixed(1)}%`:a<0?`\u2193 ${Math.abs(a).toFixed(1)}%`:"--",s=`---
type: analytics_dashboard
auto_refresh: true
...
| Avg Score | ${e.avgScore?.toFixed(1)||"--"} | ${r(e.scoreTrend)} |
| Talk Ratio | ${e.avgTalkRatio?Math.round(e.avgTalkRatio*100):"--"}% | ${r(e.talkRatioTrend)} |
| Value Score | ${e.avgValueScore?.toFixed(1)||"--"} | ${r(e.valueScoreTrend)} |
...
`;await this.app.vault.modify(t,s)}},ie=class extends p.Modal{constructor(i,t,e=()=>{}){super(i),this.plugin=t,this.onComplete=e}onOpen(){let{contentEl:i}=this;i.empty(),i.style.cssText="max-width: 540px; padding: 24px;",i.createEl("h2",{text:"Audio Setup for Full-Call Recording"});let t=i.createDiv();t.style.cssText="margin-bottom: 20px; line-height: 1.6; color: var(--text-muted);",t.setText("To capture both sides of a call with the best quality, install a virtual audio device. This creates a software loopback that lets the plugin record system audio (the other person) alongside your microphone.");let e=navigator.platform?.toLowerCase().includes("mac")||navigator.userAgent?.includes("Mac"),n=i.createDiv();if(n.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 12px;",n.createEl("h4",{text:"Step 1: Install Virtual Audio Driver"}),e){let d=n.createDiv();d.style.cssText="line-height: 1.6;",d.innerHTML=`
      ...
      `}else{let d=n.createDiv();d.style.cssText="line-height: 1.6;",d.innerHTML=`
      ...
      `}if(e){let d=i.createDiv();d.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 12px;",d.createEl("h4",{text:"Step 2: Create Multi-Output Device (macOS)"});let u=d.createDiv();u.style.cssText="line-height: 1.6;",u.innerHTML=`
      ...
      `}let r=i.createDiv();r.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px;",r.createEl("h4",{text:e?"Step 3: Verify Setup":"Step 2: Verify Setup"});let s=r.createDiv();s.style.cssText="margin-bottom: 12px; font-size: 13px;";let a=r.createEl("button",{text:"Scan for Virtual Audio Device"});a.style.cssText="padding: 8px 16px; cursor: pointer; border-radius: 6px; margin-right: 8px;",a.onclick=async()=>{s.setText("Scanning...");try{try{(await navigator.mediaDevices.getUserMedia({audio:!0})).getTracks().forEach(m=>m.stop())}catch{}let d=await E.detectVirtualAudioDevice();d?(s.innerHTML=`<span style="color:#22c55e;">&#10003;</span> Found: <strong>${d.label}</strong>. You're all set!`,this.plugin.settings.audioSystemDeviceId=d.deviceId,await this.plugin.saveSettings()):s.innerHTML='<span style="color:#f59e0b;">&#9888;</span> No virtual audio device found. Make sure the driver is installed and Obsidian was restarted.'}catch(d){s.innerHTML=`<span style="color:#ef4444;">Scan failed: ${d.message}</span>`}};let o=i.createDiv();o.style.cssText="display: flex; justify-content: space-between; margin-top: 20px;";let c=o.createEl("button",{text:"Skip \u2014 I'll use Speaker Mode"});c.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px; opacity: 0.7;",c.onclick=async()=>{this.plugin.settings.audioSetupDismissed=!0,await this.plugin.saveSettings(),this.close(),this.onComplete()};let l=o.createEl("button",{text:"Done"});l.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px; font-weight: 600;",l.onclick=()=>{this.close(),this.onComplete()}}onClose(){this.contentEl.empty()}},se=class extends p.PluginSettingTab{constructor(i,t){super(i,t),this.plugin=t}display(){let{containerEl:i}=this;i.empty(),i.createEl("h2",{text:"Eudia Sync & Scribe"}),i.createEl("h3",{text:"Your Profile"});let t=i.createDiv();t.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let e=t.createDiv();e.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let n=e.createSpan(),r=e.createSpan(),s=t.createDiv();s.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",s.setText("Connect with Salesforce to sync notes with your user attribution.");let a=t.createEl("button");a.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,c=async()=>{if(!this.plugin.settings.userEmail)return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",r.setText("Enter email above first"),a.setText("Setup Required"),a.disabled=!0,a.style.opacity="0.5",a.style.cursor="not-allowed",!1;a.disabled=!1,a.style.opacity="1",a.style.cursor="pointer";try{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",r.setText("Checking..."),(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",r.setText("Connected to Salesforce"),a.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",r.setText("Not connected"),a.setText("Connect to Salesforce"),!1)}catch{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",r.setText("Status unavailable"),a.setText("Connect to Salesforce"),!1}};new p.Setting(i).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(w=>w.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async S=>{let j=S.trim().toLowerCase();this.plugin.settings.userEmail=j,await this.plugin.saveSettings(),await c()})),new p.Setting(i).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(w=>{ge.forEach(S=>{w.addOption(S.value,S.label)}),w.setValue(this.plugin.settings.timezone),w.onChange(async S=>{this.plugin.settings.timezone=S,await this.plugin.saveSettings(),this.plugin.calendarService?.setTimezone(S),new p.Notice(`Timezone set to ${ge.find(j=>j.value===S)?.label||S}`)})}),i.createEl("h3",{text:"Salesforce Connection"}),i.appendChild(t);let l=()=>{o&&window.clearInterval(o);let w=0,S=30;o=window.setInterval(async()=>{w++,await c()?(o&&(window.clearInterval(o),o=null),new p.Notice("Salesforce connected successfully!")):w>=S&&o&&(window.clearInterval(o),o=null)},5e3)};a.onclick=async()=>{if(!this.plugin.settings.userEmail){new p.Notice("Please enter your email first");return}let w=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(w,"_blank"),new p.Notice("Complete the Salesforce login in the popup window",5e3),l()},c(),i.createEl("h3",{text:"Server"}),new p.Setting(i).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(w=>w.setValue(this.plugin.settings.serverUrl).onChange(async S=>{this.plugin.settings.serverUrl=S,await this.plugin.saveSettings()}));let d=i.createDiv({cls:"settings-advanced-collapsed"}),u=d.createDiv({cls:"eudia-transcription-status"});u.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",u.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?u.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':u.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{u.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let m=new p.Setting(i).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(w=>w.setValue(!1).onChange(S=>{d.style.display=S?"block":"none"}));d.style.display="none",i.createEl("h3",{text:"Audio Capture"});let f=i.createDiv();f.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 12px; line-height: 1.5;",f.setText("Full Call mode disables echo cancellation so your mic captures both you AND the other person's voice from your speakers. For best quality, install a virtual audio device (BlackHole on Mac, VB-Cable on Windows) \u2014 it will be auto-detected."),new p.Setting(i).setName("Capture Mode").setDesc("Full Call captures both sides via speakers; Mic Only captures your voice only.").addDropdown(w=>{w.addOption("full_call","Full Call (Speaker Mode)"),w.addOption("mic_only","Mic Only"),w.setValue(this.plugin.settings.audioCaptureMode||"full_call"),w.onChange(async S=>{this.plugin.settings.audioCaptureMode=S,await this.plugin.saveSettings()})});let h=i.createDiv();h.style.cssText="padding: 10px 14px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",h.setText("Scanning audio devices...");let v=new p.Setting(i).setName("Microphone").setDesc("Select your physical microphone"),g=new p.Setting(i).setName("System Audio Device").setDesc("Virtual audio device for capturing the other person (optional)"),y=i.createDiv();y.style.cssText="margin-bottom: 16px;";let b=y.createEl("button",{text:"Test Audio (3 seconds)"});b.style.cssText="padding: 8px 16px; cursor: pointer; border-radius: 6px;";let C=y.createDiv();C.style.cssText="font-size: 12px; margin-top: 6px; color: var(--text-muted);",(async()=>{try{try{(await navigator.mediaDevices.getUserMedia({audio:!0})).getTracks().forEach(W=>W.stop())}catch{}let w=await E.getAvailableDevices(),S=w.find(x=>x.isVirtual);S?(h.innerHTML=`<span style="color:#22c55e;">&#10003;</span> Virtual audio device detected: <strong>${S.label}</strong>`,this.plugin.settings.audioSystemDeviceId||(this.plugin.settings.audioSystemDeviceId=S.deviceId,await this.plugin.saveSettings())):h.innerHTML='<span style="color:#f59e0b;">&#9888;</span> No virtual audio device found. Using speaker mode. For best quality, install <a href="https://existential.audio/blackhole/" style="color:var(--text-accent);">BlackHole</a> (Mac) or <a href="https://vb-audio.com/Cable/" style="color:var(--text-accent);">VB-Cable</a> (Windows).';let j=w.filter(x=>!x.isVirtual);v.addDropdown(x=>{x.addOption("","Default Microphone"),j.forEach(W=>x.addOption(W.deviceId,W.label)),x.setValue(this.plugin.settings.audioMicDeviceId||""),x.onChange(async W=>{this.plugin.settings.audioMicDeviceId=W,await this.plugin.saveSettings()})}),g.addDropdown(x=>{x.addOption("","Auto-detect / None"),w.filter(W=>W.isVirtual).forEach(W=>x.addOption(W.deviceId,W.label)),w.filter(W=>!W.isVirtual).forEach(W=>x.addOption(W.deviceId,`(mic) ${W.label}`)),x.setValue(this.plugin.settings.audioSystemDeviceId||""),x.onChange(async W=>{this.plugin.settings.audioSystemDeviceId=W,await this.plugin.saveSettings()})})}catch(w){h.setText("Could not enumerate audio devices."),console.warn("[Eudia Settings] Device enumeration failed:",w)}})(),b.onclick=async()=>{b.disabled=!0,b.setText("Recording..."),C.setText("");try{let w=new E;await w.start({captureMode:this.plugin.settings.audioCaptureMode||"full_call",micDeviceId:this.plugin.settings.audioMicDeviceId||void 0,systemAudioDeviceId:this.plugin.settings.audioSystemDeviceId||void 0}),await new Promise(W=>setTimeout(W,3e3));let S=await w.stop(),j=await E.analyzeAudioBlob(S.audioBlob),x=S.hasVirtualDevice?"Virtual Device + Mic":S.captureMode==="full_call"?"Speaker Mode":"Mic Only";C.innerHTML=`<strong>${x}</strong> | Peak: ${j.peakLevel}% | Avg: ${j.averageLevel}% | Silent: ${j.silentPercent}%`+(j.warning?`<br><span style="color:#ef4444;">${j.warning}</span>`:'<br><span style="color:#22c55e;">Audio detected \u2014 recording should work.</span>')}catch(w){C.innerHTML=`<span style="color:#ef4444;">Test failed: ${w.message}</span>`}finally{b.disabled=!1,b.setText("Test Audio (3 seconds)")}},i.createEl("h3",{text:"Transcription"}),new p.Setting(i).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(w=>w.setValue(this.plugin.settings.saveAudioFiles).onChange(async S=>{this.plugin.settings.saveAudioFiles=S,await this.plugin.saveSettings()})),new p.Setting(i).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(w=>w.setValue(this.plugin.settings.appendTranscript).onChange(async S=>{this.plugin.settings.appendTranscript=S,await this.plugin.saveSettings()})),i.createEl("h3",{text:"Sync"}),new p.Setting(i).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(w=>w.setValue(this.plugin.settings.syncOnStartup).onChange(async S=>{this.plugin.settings.syncOnStartup=S,await this.plugin.saveSettings()})),new p.Setting(i).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(w=>w.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async S=>{this.plugin.settings.autoSyncAfterTranscription=S,await this.plugin.saveSettings()})),new p.Setting(i).setName("Auto-Sync Flagged Notes").setDesc("Periodically push notes with sync_to_salesforce: true to Salesforce").addToggle(w=>w.setValue(this.plugin.settings.sfAutoSyncEnabled).onChange(async S=>{this.plugin.settings.sfAutoSyncEnabled=S,await this.plugin.saveSettings(),S?this.plugin.startSalesforceSyncScanner():this.plugin.updateSfSyncStatusBar("SF Sync: Off")})),new p.Setting(i).setName("Auto-Sync Interval").setDesc("How often to scan for flagged notes (in minutes)").addDropdown(w=>{w.addOption("5","Every 5 minutes"),w.addOption("15","Every 15 minutes"),w.addOption("30","Every 30 minutes"),w.setValue(String(this.plugin.settings.sfAutoSyncIntervalMinutes)),w.onChange(async S=>{this.plugin.settings.sfAutoSyncIntervalMinutes=parseInt(S),await this.plugin.saveSettings(),new p.Notice(`SF auto-sync interval set to ${S} minutes. Restart Obsidian for changes to take effect.`)})}),i.createEl("h3",{text:"Folders"}),new p.Setting(i).setName("Accounts Folder").setDesc("Where account folders are stored").addText(w=>w.setValue(this.plugin.settings.accountsFolder).onChange(async S=>{this.plugin.settings.accountsFolder=S||"Accounts",await this.plugin.saveSettings()})),new p.Setting(i).setName("Recordings Folder").setDesc("Where audio files are saved").addText(w=>w.setValue(this.plugin.settings.recordingsFolder).onChange(async S=>{this.plugin.settings.recordingsFolder=S||"Recordings",await this.plugin.saveSettings()})),i.createEl("h3",{text:"Actions"}),new p.Setting(i).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(w=>w.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new p.Setting(i).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(w=>w.setButtonText("Refresh Folders").onClick(async()=>{w.setButtonText("Checking..."),w.setDisabled(!0);try{let S=await this.plugin.refreshAccountFolders();S>0?new p.Notice(`Created ${S} new account folder${S>1?"s":""}`):new p.Notice("All account folders are up to date")}catch(S){new p.Notice("Failed to refresh folders: "+S.message)}w.setButtonText("Refresh Folders"),w.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&i.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),i.createEl("p",{text:`Audio transcription: ${E.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
