...
`},{name:"Meeting Notes.md",content:O},{name:"Contacts.md",content:g},{name:"Intelligence.md",content:w},{name:"Next Steps.md",content:C}];for(let W of v){let I=`${p}/${W.name}`;await this.app.vault.create(I,W.content)}return console.log(`[Eudia] Created account folder with subnotes${h?" (enriched)":""}: ${d}`),!0}catch(m){return console.error(`[Eudia] Failed to create folder for ${d}:`,m),!1}},c=5;for(let l=0;l<e.length;l+=c){let d=e.slice(l,l+c),p=await Promise.allSettled(d.map(y=>o(y)));i+=p.filter(y=>y.status==="fulfilled"&&y.value===!0).length}i>0?(this.settings.cachedAccounts=e.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),new u.Notice(`Created ${i} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${e.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(e,t,n){let s=t?`
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof u.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let p of i){let y=p.split("@")[0].replace("."," ").replace(/\b\w/g,b=>b.toUpperCase()),m=p.split("@")[0].replace("."," ").toLowerCase(),h=m.split(" ")[0],g=m.split(" ").pop()||"",w=t.filter(b=>{let W=(b.csmName||"").toLowerCase();if(W&&(W.includes(h)||W.includes(g)))return!0;let I=(b.ownerName||"").toLowerCase();return I.includes(h)||I.includes(g)}),O=`---
rep: "${p}"
rep_name: "${y}"
role: cs_rep_summary
account_count: ${w.length}
created: ${s}
---
...
**Email:** ${p}
**CS Accounts:** ${w.length}

---

## Assigned Accounts

`;if(w.length>0){O+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let b of w){let W=b.name.replace(/[<>:"/\\|?*]/g,"_").trim();O+=`| ${b.name} | ${b.type||""} | ${b.ownerName||""} | [[Accounts/${W}/Contacts\\|View]] |
...
${y}`:h,w=`---
account: "${e}"
account_id: "${this.settings.cachedAccounts.find(O=>O.name===e)?.id||""}"
type: next_steps
auto_updated: true
last_updated: ${o}
sync_to_salesforce: false
...
`),m}buildNoteContent(e,t){let n=g=>g==null?"":Array.isArray(g)?g.map(w=>typeof w=="object"?w.category?`**${w.category}**: ${w.signal||w.insight||""}`:JSON.stringify(w):String(w)).join(`
...
*Add org chart, decision makers, champions, and blockers here.*`),t.intelligence&&await c("Intelligence.md",t.intelligence),t.nextSteps&&await c("Next Steps.md",t.nextSteps),t.opportunities||t.recentActivity){let l=`${i}/Meeting Notes.md`,d=this.app.vault.getAbstractFileByPath(l);if(d instanceof u.TFile){let p=await this.app.vault.read(d),y="",m=p;if(p.startsWith("---")){let O=p.indexOf("---",3);O!==-1&&(y=p.substring(0,O+3),m=p.substring(O+3),y.includes("enriched_at:")?y=y.replace(/enriched_at:.*/,`enriched_at: "${o}"`):y=y.substring(0,O)+`enriched_at: "${o}"
---`)}let h=m.match(/^(\s*#[^\n]+)/),w=[h?h[1]:`
