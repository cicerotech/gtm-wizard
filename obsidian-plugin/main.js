...
`;let s=t.talkTime.repPercent,r=t.talkTime.customerPercent,o=t.talkTime.isHealthyRatio?"\u2705":"\u26A0\uFE0F";n+=`**Talk Time:** Rep ${s}% / Customer ${r}% ${o}
`;let c=Math.round(s/5),d=Math.round(r/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
...
`);let i=t?.enabled&&t?.formattedTranscript?t.formattedTranscript:e;if(i){let s=t?.enabled?"Full Transcript (Speaker-Attributed)":"Full Transcript";n+=`---
...
`).replace(/\n/g,"<br>")}onClose(){this.contentEl.empty()}};var Z=class extends u.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new F(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return N}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}this.plugin.settings.accountsImported&&(this.steps[2].status="complete")}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),n=t.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to unlock your sales superpowers",cls:"eudia-setup-subtitle"});let i=t.createDiv({cls:"eudia-setup-progress-section"}),s=this.getCompletionPercentage(),r=i.createDiv({cls:"eudia-setup-progress-label"});r.createSpan({text:"Setup Progress"}),r.createSpan({text:`${s}%`,cls:"eudia-setup-progress-value"});let c=i.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${s}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),s=i.createDiv({cls:"eudia-setup-step-number"});s.setText(t.status==="complete"?"":"1"),t.status==="complete"&&s.addClass("eudia-step-complete");let r=i.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let d=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});d.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async l=>{l.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let n=await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new u.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");try{let i,s=[];if(x(e))console.log("[Eudia] Admin user detected - importing all accounts"),i=await this.accountOwnershipService.getAllAccountsForAdmin(e);else{let r=await this.accountOwnershipService.getAccountsWithProspects(e);i=r.accounts,s=r.prospects}if(i.length>0||s.length>0){t&&(t.textContent=`Creating ${i.length} account folders...`),x(e)?await this.plugin.createAdminAccountFolders(i):(await this.plugin.createTailoredAccountFolders(i,{}),s.length>0&&await this.plugin.createProspectAccountFiles(s)),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=i.length+s.length,await this.plugin.saveSettings(),new u.Notice(`Imported ${i.length} active accounts + ${s.length} prospects!`);let r=[...i,...s];setTimeout(async()=>{try{t&&(t.textContent="Loading Salesforce intelligence...",t.className="eudia-setup-validation-message loading"),await this.plugin.enrichAccountFolders(r)}catch(o){console.log("[Eudia] Background enrichment skipped:",o)}},500)}}catch(i){console.error("[Eudia] Account import failed:",i)}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),s=i.createDiv({cls:"eudia-setup-step-number"});s.setText(t.status==="complete"?"":"2"),t.status==="complete"&&s.addClass("eudia-step-complete");let r=i.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let d=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),l=o.createDiv({cls:"eudia-setup-sf-status"});d.onclick=async()=>{let h=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(h,"_blank"),l.textContent="Complete the login in the popup window...",l.className="eudia-setup-sf-status loading",new u.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(l)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,n=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new u.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=this.plugin.settings.userEmail,n,i=[];if(x(t))console.log("[Eudia] Admin user detected - importing all accounts"),e.textContent="Admin detected - importing all accounts...",n=await this.accountOwnershipService.getAllAccountsForAdmin(t);else{let c=await this.accountOwnershipService.getAccountsWithProspects(t);n=c.accounts,i=c.prospects}if(n.length===0&&i.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}e.textContent=`Creating ${n.length} account folders...`,x(t)?await this.plugin.createAdminAccountFolders(n):(await this.plugin.createTailoredAccountFolders(n,{}),i.length>0&&await this.plugin.createProspectAccountFiles(i)),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=n.length+i.length,await this.plugin.saveSettings(),this.steps[2].status="complete";let s=n.filter(c=>c.isOwned!==!1).length,r=n.filter(c=>c.isOwned===!1).length;x(t)&&r>0?e.textContent=`${s} owned + ${r} view-only accounts imported! Enriching...`:e.textContent=`${n.length} active + ${i.length} prospect accounts imported! Enriching...`,e.className="eudia-setup-sf-status success";let o=[...n,...i];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(o);let c=o.filter(d=>d.id&&d.id.length>=15).length;e.textContent=`${n.length+i.length} accounts imported, ${c} enriched with Salesforce data`}catch(c){console.log("[Eudia] Background enrichment skipped:",c),e.textContent=`${n.length+i.length} accounts imported (enrichment will retry on next launch)`}},500)}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error"}}renderTranscribeStep(e){let t=this.steps[2],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),s=i.createDiv({cls:"eudia-setup-step-number"});s.setText(t.status==="complete"?"":"3"),t.status==="complete"&&s.addClass("eudia-step-complete");let r=i.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),d.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let l=c.createDiv({cls:"eudia-setup-instruction"});l.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),l.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let h=c.createDiv({cls:"eudia-setup-instruction"});h.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),h.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(s=>s.status==="complete")){let s=t.createDiv({cls:"eudia-setup-completion"});s.createEl("h2",{text:"\u{1F389} You're all set!"}),s.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let r=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});r.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(N),await this.plugin.activateCalendarView()}}else{let s=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});s.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(N),new u.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let i=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});i.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},Q=class extends u.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return M}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),n=t.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let i=n.createDiv({cls:"eudia-calendar-actions"}),s=i.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});s.title="Refresh (fetches latest from calendar)",s.onclick=async()=>{s.addClass("spinning"),this._forceRefresh=!0,await this.render(),s.removeClass("spinning")};let r=i.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});r.title="Settings",r.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,i=this.plugin.settings.userEmail;try{(await(0,u.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(i&&t.server==="connected")try{let l=await(0,u.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(i)}`,method:"GET",throw:!1});l.status===200&&l.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(i&&t.server==="connected")try{let l=await(0,u.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(i)}`,method:"GET",throw:!1});l.status===200&&l.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let s=e.createDiv({cls:"eudia-status-indicators"}),r=s.createSpan({cls:`eudia-status-dot ${t.server}`});r.title=t.serverMessage||"Server";let o=s.createSpan({cls:`eudia-status-dot ${t.calendar}`});o.title=t.calendarMessage||"Calendar";let c=s.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(c.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let l=e.createDiv({cls:"eudia-status-warning"});l.innerHTML=`<strong>${i}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),n=t.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let i=new T(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),s=this._forceRefresh||!1;this._forceRefresh=!1;let r=await i.getWeekMeetings(s);if(n.remove(),!r.success){this.renderError(t,r.error||"Failed to load calendar");return}let o=Object.keys(r.byDay||{}).sort();if(o.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,i);for(let c of o){let d=r.byDay[c];!d||d.length===0||this.renderDaySection(t,c,d)}}catch(i){n.remove(),this.renderError(t,i.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let n=await t.getCurrentMeeting();if(n.meeting){let i=e.createDiv({cls:"eudia-now-card"});n.isNow?i.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):i.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let s=i.createDiv({cls:"eudia-now-content"});s.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&s.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let r=i.createEl("button",{cls:"eudia-now-action",text:"Create Note"});r.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(e,t,n){let i=e.createDiv({cls:"eudia-calendar-day"});i.createEl("div",{cls:"eudia-calendar-day-header",text:T.getDayName(t)});for(let s of n){let r=i.createDiv({cls:`eudia-calendar-meeting ${s.isCustomerMeeting?"customer":"internal"}`});r.createEl("div",{cls:"eudia-calendar-time",text:T.formatTime(s.start,this.plugin.settings.timezone)});let o=r.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:s.subject}),s.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:s.accountName});else if(s.attendees&&s.attendees.length>0){let c=s.attendees.slice(0,2).map(d=>d.name||d.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}r.onclick=()=>this.createNoteForMeeting(s),r.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `}renderError(e,t){let n=e.createDiv({cls:"eudia-calendar-error"}),i="",s="Unable to load calendar",r="";t.includes("not authorized")||t.includes("403")?(i="\u{1F512}",s="Calendar Access Required",r="Contact your admin to be added to the authorized users list."):t.includes("network")||t.includes("fetch")?(i="\u{1F4E1}",s="Connection Issue",r="Check your internet connection and try again."):(t.includes("server")||t.includes("500"))&&(i="\u{1F527}",s="Server Unavailable",r="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=t.createDiv({cls:"eudia-setup-input-group"}),i=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});i.addClass("eudia-setup-email");let s=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),r=t.createDiv({cls:"eudia-setup-status"});s.onclick=async()=>{let o=i.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){r.textContent="Please use your @eudia.com email",r.className="eudia-setup-status error";return}s.disabled=!0,s.textContent="Connecting...",r.textContent="";try{if(!(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){r.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,r.className="eudia-setup-status error",s.disabled=!1,s.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),r.textContent="Connected",r.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let d=c.message||"Connection failed";d.includes("403")?r.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:r.textContent=d,r.className="eudia-setup-status error",s.disabled=!1,s.textContent="Connect"}},i.onkeydown=o=>{o.key==="Enter"&&s.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(e){let t=e.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],i=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],s=t.filter(o=>!i.includes(o)&&o.length>1);if(s.length===0)return t[0]||"";if(s.length>1&&n.includes(s[0]))return s[1].charAt(0).toUpperCase()+s[1].slice(1);let r=s[s.length-1];return r.charAt(0).toUpperCase()+r.slice(1)}getExternalDomainsFromAttendees(e){if(!e||e.length===0)return[];let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,i=[];for(let s of e){if(!s.email)continue;let o=s.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let c=o[1];if(c.includes("eudia.com")||t.includes(c)||n.has(c))continue;n.add(c);let d=this.extractCompanyFromDomain(c);d.length>=2&&i.push({domain:c,company:d})}}return i}findBestAccountMatch(e,t,n){let i=this.plugin.settings.accountsFolder||"Accounts",s=this.app.vault.getAbstractFileByPath(i);if(!(s instanceof u.TFolder))return null;let r=[];for(let c of s.children)c instanceof u.TFolder&&r.push(c.name);if(r.length===0)return null;let o=[];for(let{domain:c,company:d}of e){let l=this.findAccountFolder(d),h=l?1:0;o.push({domain:c,company:d,folder:l,score:h})}if(o.sort((c,d)=>d.score-c.score),o.length>0&&o[0].folder){let c=o[0],d=c.folder.split("/").pop()||c.company;return console.log(`[Eudia Calendar] Best domain match: "${c.company}" from ${c.domain} -> ${c.folder}`),{folder:c.folder,accountName:d,source:"domain"}}if(t){let c=this.findAccountFolder(t);if(c){let d=c.split("/").pop()||t;return console.log(`[Eudia Calendar] Server account match: "${t}" -> ${c}`),{folder:c,accountName:d,source:"server"}}}if(n){let c=this.findAccountFolder(n);if(c){let d=c.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${c}`),{folder:c,accountName:d,source:"subject"}}}for(let{company:c}of e){let d=r.find(l=>{let h=l.toLowerCase(),g=c.toLowerCase();return h.includes(g)||g.includes(h)});if(d){let l=`${i}/${d}`;return console.log(`[Eudia Calendar] Partial domain match: "${c}" -> ${l}`),{folder:l,accountName:d,source:"domain-partial"}}}return null}extractAccountFromAttendees(e){let t=this.getExternalDomainsFromAttendees(e);if(t.length===0)return null;let n=t[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let i=(t[1]||t[2]||"").trim();if(i.toLowerCase()!=="eudia")return i}let n=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let s=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(s.toLowerCase()!=="eudia"&&s.length>0)return s}if(!e.toLowerCase().includes("eudia")){let i=e.match(/^([^-–|]+)/);if(i){let s=i[1].trim();if(s.length>2&&s.length<50)return s}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(t);if(!(n instanceof u.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let i=e.toLowerCase().trim(),s=[];for(let h of n.children)h instanceof u.TFolder&&s.push(h.name);console.log(`[Eudia Calendar] Searching for "${i}" in ${s.length} folders`);let r=s.find(h=>h.toLowerCase()===i);if(r)return console.log(`[Eudia Calendar] Exact match found: ${r}`),`${t}/${r}`;let o=s.find(h=>h.toLowerCase().startsWith(i));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${t}/${o}`;let c=s.find(h=>i.startsWith(h.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${t}/${c}`;let d=s.find(h=>{let g=h.toLowerCase();return g.length>=3&&i.includes(g)});if(d)return console.log(`[Eudia Calendar] Search contains folder match: ${d}`),`${t}/${d}`;let l=s.find(h=>{let g=h.toLowerCase();return i.length>=3&&g.includes(i)});return l?(console.log(`[Eudia Calendar] Folder contains search match: ${l}`),`${t}/${l}`):(console.log(`[Eudia Calendar] No folder match found for "${i}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",i=x(n),s=(e.attendees||[]).map(v=>v.email).filter(Boolean),r=ae(e.subject,s);if(i&&r.isPipelineMeeting&&r.confidence>=60){await this._createPipelineNote(e,t);return}let o=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),c=`${t} - ${o}.md`,d=null,l=e.accountName||null,h=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(v=>v.email)||[])}`);let g=this.getExternalDomainsFromAttendees(e.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(g)}`);let m=this.extractAccountFromSubject(e.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${m||"none"}"`);let p=this.findBestAccountMatch(g,e.accountName,m||void 0);if(p&&(d=p.folder,l=p.accountName,console.log(`[Eudia Calendar] Best match (${p.source}): "${l}" -> ${d}`)),!d){let v=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(v)instanceof u.TFolder&&(d=v,console.log(`[Eudia Calendar] No match found, using Accounts root: ${d}`))}if(l){let v=this.plugin.settings.cachedAccounts.find(D=>D.name.toLowerCase()===l?.toLowerCase());v&&(h=v.id,l=v.name,console.log(`[Eudia Calendar] Matched to cached account: ${v.name} (${v.id})`))}let y=d?`${d}/${c}`:c,w=this.app.vault.getAbstractFileByPath(y);if(w instanceof u.TFile){await this.app.workspace.getLeaf().openFile(w),new u.Notice(`Opened existing note: ${c}`);return}let S=(e.attendees||[]).map(v=>v.name||v.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${e.subject}"
date: ${t}
attendees: [${S}]
account: "${l||""}"
...
`;try{let v=await this.app.vault.create(y,C);await this.app.workspace.getLeaf().openFile(v),new u.Notice(`Created: ${y}`)}catch(v){console.error("[Eudia Calendar] Failed to create note:",v),new u.Notice(`Could not create note: ${v.message||"Unknown error"}`)}}async _createPipelineNote(e,t){let n=new Date(t+"T00:00:00"),i=String(n.getMonth()+1).padStart(2,"0"),s=String(n.getDate()).padStart(2,"0"),r=String(n.getFullYear()).slice(-2),o=`${i}.${s}.${r}`,c=`Team Pipeline Meeting - ${o}.md`,d="Pipeline Meetings";this.app.vault.getAbstractFileByPath(d)||await this.app.vault.createFolder(d);let h=`${d}/${c}`,g=this.app.vault.getAbstractFileByPath(h);if(g instanceof u.TFile){await this.app.workspace.getLeaf().openFile(g),new u.Notice(`Opened existing: ${c}`);return}let m=(e.attendees||[]).map(y=>y.name||y.email?.split("@")[0]||"Unknown"),p=`---
...
`},{name:"Meeting Notes.md",content:S},{name:"Contacts.md",content:y},{name:"Intelligence.md",content:w},{name:"Next Steps.md",content:C}];for(let E of v){let P=`${h}/${E.name}`;await this.app.vault.create(P,E.content)}return console.log(`[Eudia] Created account folder with subnotes${p?" (enriched)":""}: ${l}`),!0}catch(m){return console.error(`[Eudia] Failed to create folder for ${l}:`,m),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c),h=await Promise.allSettled(l.map(g=>o(g)));s+=h.filter(g=>g.status==="fulfilled"&&g.value===!0).length}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),s>0&&new u.Notice(`Created ${s} account folders`),await this.ensureNextStepsFolderExists()}buildContactsContent(e,t,n){let i=t?`
...
---`,s=[];return t?.opportunities&&s.push(t.opportunities),t?.recentActivity&&s.push(t.recentActivity),s.length>0?`${i}
...
3. **Next Steps** are auto-extracted after transcription
4. Set \`sync_to_salesforce: true\` to sync to Salesforce
`}buildNextStepsContent(e,t,n){let i=n||new Date().toISOString().split("T")[0],s=t?`
...
*Previous next steps will be archived here.*
`}async fetchEnrichmentData(e){let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=e.filter(r=>r.id&&r.id.length>=15);if(n.length===0)return{};let i={},s=20;console.log(`[Eudia Enrich] Fetching enrichment data for ${n.length} accounts`);for(let r=0;r<n.length;r+=s){let c=n.slice(r,r+s).map(d=>d.id);try{let d=await(0,u.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:c,userEmail:this.settings.userEmail})});d.json?.success&&d.json?.enrichments&&Object.assign(i,d.json.enrichments)}catch(d){console.error(`[Eudia Enrich] Batch fetch failed (batch ${r/s+1}):`,d)}r+s<n.length&&await new Promise(d=>setTimeout(d,100))}return console.log(`[Eudia Enrich] Got enrichment data for ${Object.keys(i).length}/${n.length} accounts`),i}async createProspectAccountFiles(e){if(!e||e.length===0)return 0;let t=this.settings.accountsFolder||"Accounts",n=`${t}/_Prospects`;if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s=0;for(let r of e){let o=r.name.replace(/[<>:"/\\|?*]/g,"_").trim(),c=`${n}/${o}`;if(this.app.vault.getAbstractFileByPath(c)instanceof u.TFolder)continue;let l=`${t}/${o}`;if(this.app.vault.getAbstractFileByPath(l)instanceof u.TFolder)continue;let g=`${n}/${o}.md`,m=this.app.vault.getAbstractFileByPath(g);if(m instanceof u.TFile)try{await this.app.vault.delete(m)}catch{}try{await this.app.vault.createFolder(c);let p=new Date().toISOString().split("T")[0],y=[{name:"Note 1.md",content:`---
account: "${r.name}"
account_id: "${r.id}"
type: meeting_note
tier: prospect
...
*Previous next steps will be archived here.*
`}];for(let w of y){let S=`${c}/${w.name}`;await this.app.vault.create(S,w.content)}s++}catch(p){console.log(`[Eudia] Failed to create prospect folder for ${r.name}:`,p)}}return s>0&&console.log(`[Eudia] Created ${s} prospect account folders in _Prospects/`),s}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let i=0,s=0,r=new Date().toISOString().split("T")[0],o=async d=>{let l=d.name.replace(/[<>:"/\\|?*]/g,"_").trim(),h=`${t}/${l}`;if(this.app.vault.getAbstractFileByPath(h)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(h),await this.createExecAccountSubnotes(h,d,r),d.isOwned?i++:s++,console.log(`[Eudia Admin] Created ${d.isOwned?"owned":"view-only"} folder: ${l}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${l}:`,m),!1}},c=5;for(let d=0;d<e.length;d+=c){let l=e.slice(d,d+c);await Promise.allSettled(l.map(h=>o(h)))}this.settings.cachedAccounts=e.map(d=>({id:d.id,name:d.name})),await this.saveSettings(),i+s>0&&new u.Notice(`Created ${i} owned + ${s} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let i=t.ownerName||"Unknown",s=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let r of s){let o=`${e}/${r.name}`;await this.app.vault.create(o,r.content)}}async createFullAccountSubnotes(e,t,n){let i=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let s of i){let r=`${e}/${s.name}`;await this.app.vault.create(r,s.content)}}async ensurePipelineFolderExists(){let e="Pipeline",t=`${e}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let r=`---
...
`;await this.app.vault.create(t,o)}}async updateAccountNextSteps(e,t,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${e}`),console.log(`[Eudia] Content length: ${t?.length||0} chars`);let i=e.replace(/[<>:"/\\|?*]/g,"_").trim(),s=`${this.settings.accountsFolder}/${i}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${s}`);let r=this.app.vault.getAbstractFileByPath(s);if(!r||!(r instanceof u.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${s}`);let S=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${i}`);S&&S instanceof u.TFolder?console.log(`[Eudia] Files in ${i} folder:`,S.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${i}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),d=n.split("/").pop()?.replace(".md","")||"Meeting",l=t;!t.includes("- [ ]")&&!t.includes("- [x]")&&(l=t.split(`
...
${y}
`;await this.app.vault.modify(r,w),console.log(`[Eudia] Updated Next Steps for ${e} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(i){console.error(`[Eudia] Failed to update Next Steps for ${e}:`,i)}}async regenerateNextStepsDashboard(){try{let t=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!t||!(t instanceof u.TFile)){await this.ensureNextStepsFolderExists();return}let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!n||!(n instanceof u.TFolder))return;let i=[];for(let c of n.children)if(c instanceof u.TFolder){let d=`${c.path}/Next Steps.md`,l=this.app.vault.getAbstractFileByPath(d);if(l instanceof u.TFile){let h=await this.app.vault.read(l),g=h.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),m=g?g[1]:"Unknown",p=h.split(`
`).filter(y=>y.match(/^- \[[ x]\]/)).slice(0,5);(p.length>0||m!=="Unknown")&&i.push({account:c.name,lastUpdated:m,nextSteps:p})}}i.sort((c,d)=>d.lastUpdated.localeCompare(c.lastUpdated));let s=new Date().toISOString().split("T")[0],r=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${s}
---
...
|---------|--------------|--------|
| *None yet* | - | Complete a meeting transcription |
`;else{for(let c of i)o+=`## ${c.account}
...
`):typeof y=="object"?JSON.stringify(y):String(y),i=n(e.title)||"Meeting Notes",s=n(e.summary),r=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),l=n(e.actionItems),h=n(e.keyDates),g=n(e.risksObjections),m=n(e.attendees),p=`---
title: "${i}"
...
${t.text}
`),p}openIntelligenceQuery(){new U(this.app,this).open()}openIntelligenceQueryForCurrentNote(){let e=this.app.workspace.getActiveFile(),t;if(e){let n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(n?.account_id&&n?.account)t={id:n.account_id,name:n.account};else if(n?.account){let i=this.settings.cachedAccounts.find(s=>s.name.toLowerCase()===n.account.toLowerCase());i?t={id:i.id,name:i.name}:t={id:"",name:n.account}}else{let i=e.path.split("/");if(i.length>=2&&i[0]===this.settings.accountsFolder){let s=i[1],r=this.settings.cachedAccounts.find(o=>o.name.replace(/[<>:"/\\|?*]/g,"_").trim()===s);r?t={id:r.id,name:r.name}:t={id:"",name:s}}}}new U(this.app,this,t).open()}async syncAccounts(e=!1){e||new u.Notice("Syncing Salesforce accounts...");try{let n=(await(0,u.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!n.success||!n.accounts){e||new u.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=n.accounts.map(i=>({id:i.id,name:i.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),e||new u.Notice(`Synced ${n.accounts.length} accounts for matching`)}catch(t){e||new u.Notice(`Failed to sync accounts: ${t.message}`)}}async scanLocalAccountFolders(){try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!e||!(e instanceof u.TFolder))return;let t=[];for(let n of e.children)n instanceof u.TFolder&&t.push({id:`local-${n.name.replace(/\s+/g,"-").toLowerCase()}`,name:n.name});this.settings.cachedAccounts=t,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(e){console.error("Failed to scan local account folders:",e)}}async refreshAccountFolders(){if(!this.settings.userEmail)throw new Error("Please configure your email first");let e=new F(this.settings.serverUrl);if((await e.getAccountsForUser(this.settings.userEmail)).length===0)return console.log("[Eudia] No accounts found for user"),0;let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),i=[];if(n&&n instanceof u.TFolder)for(let o of n.children)o instanceof u.TFolder&&i.push(o.name);let s=await e.getNewAccounts(this.settings.userEmail,i);if(s.length===0)return console.log("[Eudia] All account folders exist"),0;console.log(`[Eudia] Creating ${s.length} new account folders`);let r=await this.fetchEnrichmentData(s);return await this.createTailoredAccountFolders(s,r),s.length}async checkAndConsumeSyncFlags(){if(!this.settings.userEmail)return;let e=encodeURIComponent(this.settings.userEmail.toLowerCase().trim()),t=this.settings.serverUrl||"https://gtm-wizard.onrender.com";try{let s=((await(0,u.requestUrl)({url:`${t}/api/admin/users/${e}/sync-flags`,method:"GET"})).json?.flags||[]).filter(o=>!o.consumed_at);if(s.length===0)return;console.log(`[Eudia] Found ${s.length} pending sync flag(s)`);let r=!1;for(let o of s)if(o.flag==="resync_accounts"){r=!0;let c=o.payload||{},d=c.added?.length||0,l=c.removed?.length||0;console.log(`[Eudia] Sync flag: resync_accounts (+${d} / -${l})`)}else o.flag==="update_plugin"?new u.Notice("A plugin update is available. Please download the latest vault."):o.flag==="reset_setup"&&(console.log("[Eudia] Sync flag: reset_setup received"),this.settings.setupCompleted=!1,await this.saveSettings(),new u.Notice("Setup has been reset by admin. Please re-run the setup wizard."));if(r){console.log("[Eudia] Triggering account folder resync from sync flag..."),new u.Notice("Syncing account updates...");let o=await this.syncAccountFolders();o.success?new u.Notice(`Account sync complete: ${o.added} new, ${o.archived} archived`):console.log(`[Eudia] Account resync error: ${o.error}`)}try{await(0,u.requestUrl)({url:`${t}/api/admin/users/${e}/sync-flags/consume`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagIds:s.map(o=>o.id)})}),console.log(`[Eudia] Consumed ${s.length} sync flag(s)`)}catch{console.log("[Eudia] Failed to consume sync flags (will retry next startup)")}}catch{console.log("[Eudia] Sync flag check skipped (endpoint not available)")}}async syncAccountFolders(){if(!this.settings.userEmail)return{success:!1,added:0,archived:0,error:"No email configured"};let e=this.settings.userEmail.toLowerCase().trim();console.log(`[Eudia] Syncing account folders for: ${e}`);try{let t=await fetch(`${this.settings.serverUrl}/api/bl-accounts/${encodeURIComponent(e)}`);if(!t.ok){let O=await t.json().catch(()=>({}));throw new Error(O.error||`Server returned ${t.status}`)}let n=await t.json();if(!n.success||!n.accounts)throw new Error(n.error||"Invalid response from server");let i=n.meta?.userGroup||"bl",s=n.meta?.queryDescription||"accounts",r=n.meta?.region||null;console.log(`[Eudia] User group: ${i}, accounts: ${n.accounts.length} (${s})`),r&&console.log(`[Eudia] Sales Leader region: ${r}`);let o=n.accounts||[],c=n.prospectAccounts||[],d=o.length+c.length;console.log(`[Eudia] Server returned: ${o.length} active + ${c.length} prospects = ${d} total`);let l=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),h=new Map,g=`${this.settings.accountsFolder}/_Prospects`,m=this.app.vault.getAbstractFileByPath(g),p=new Map,y=new Map;if(l&&l instanceof u.TFolder)for(let O of l.children)O instanceof u.TFolder&&!O.name.startsWith("_")&&h.set(O.name.toLowerCase().trim(),O);if(m&&m instanceof u.TFolder)for(let O of m.children)O instanceof u.TFolder?p.set(O.name.toLowerCase().trim(),O):O instanceof u.TFile&&O.extension==="md"&&y.set(O.basename.toLowerCase().trim(),O);let w=new Set(o.map(O=>O.name.toLowerCase().trim())),S=o.filter(O=>{let W=O.name.toLowerCase().trim();return!h.has(W)}),C=c.filter(O=>{let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();return!p.has(W)&&!y.has(W)&&!h.has(O.name.toLowerCase().trim())}),v=[];for(let O of o){let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();(p.has(W)||y.has(W))&&!h.has(O.name.toLowerCase().trim())&&v.push(O)}let D=new Set([...o.map(O=>O.name.toLowerCase().trim()),...c.map(O=>O.name.toLowerCase().trim())]),E=[];if(i==="bl")for(let[O,W]of h.entries())D.has(O)||E.push(W);let P=0,L=0,R=0,Y=0;if(v.length>0){console.log(`[Eudia] Promoting ${v.length} accounts from prospect to active`);for(let O of v){let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim(),j=p.get(W.toLowerCase()),b=y.get(W.toLowerCase());try{if(j){let $=`${this.settings.accountsFolder}/${W}`;await this.app.vault.rename(j,$),R++,new u.Notice(`${O.name} promoted to active`)}else if(b){await this.app.vault.delete(b);let $=[{id:O.id,name:O.name,type:O.customerType,isOwned:!0,hadOpportunity:!0}],de=await this.fetchEnrichmentData($);await this.createTailoredAccountFolders($,de),R++,new u.Notice(`${O.name} promoted to active -- full account folder created`)}}catch($){console.error(`[Eudia] Failed to promote ${O.name}:`,$)}}}if(S.length>0){console.log(`[Eudia] Creating ${S.length} new active account folders for ${i}`);let O=new Set(v.map(j=>j.name.toLowerCase().trim())),W=S.filter(j=>!O.has(j.name.toLowerCase().trim()));if(W.length>0){let j=W.map(b=>({id:b.id,name:b.name,type:b.customerType,isOwned:i==="bl",ownerName:b.ownerName,hadOpportunity:!0}));if(i==="admin"||i==="exec")await this.createAdminAccountFolders(j);else{let b=await this.fetchEnrichmentData(j);await this.createTailoredAccountFolders(j,b)}P=W.length}this.telemetry&&this.telemetry.reportInfo("Accounts synced - added",{count:P,userGroup:i,region:r||void 0})}return C.length>0&&i==="bl"&&(console.log(`[Eudia] Creating ${C.length} new prospect files`),Y=await this.createProspectAccountFiles(C.map(O=>({id:O.id,name:O.name,type:"Prospect",hadOpportunity:!1,website:O.website,industry:O.industry})))),this.settings.archiveRemovedAccounts&&E.length>0&&(console.log(`[Eudia] Archiving ${E.length} removed account folders`),L=await this.archiveAccountFolders(E),this.telemetry&&this.telemetry.reportInfo("Accounts synced - archived",{count:L})),console.log(`[Eudia] Sync complete: ${P} active added, ${Y} prospects added, ${R} promoted, ${L} archived (group: ${i})`),{success:!0,added:P+Y+R,archived:L,userGroup:i}}catch(t){return console.error("[Eudia] Account sync error:",t),this.telemetry&&this.telemetry.reportError("Account sync failed",{error:t.message}),{success:!1,added:0,archived:0,error:t.message}}}async archiveAccountFolders(e){let t=0,n=`${this.settings.accountsFolder}/_Archived`;this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);for(let s of e)try{let r=`${n}/${s.name}`;if(this.app.vault.getAbstractFileByPath(r)){let l=new Date().toISOString().split("T")[0];await this.app.fileManager.renameFile(s,`${n}/${s.name}_${l}`)}else await this.app.fileManager.renameFile(s,r);let c=`${n}/${s.name}/_archived.md`,d=`---
...
`;try{await this.app.vault.create(c,d)}catch{}t++,console.log(`[Eudia] Archived: ${s.name}`)}catch(r){console.error(`[Eudia] Failed to archive ${s.name}:`,r)}return t}async syncSpecificNoteToSalesforce(e){let t=await this.app.vault.read(e),n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let i=n.account_id,s=n.account;if(!i&&s){let r=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===s.toLowerCase());r&&(i=r.id)}if(!i){let r=e.path.split("/");if(r.length>=2&&r[0]===this.settings.accountsFolder){let o=r[1]==="_Prospects"&&r.length>=3?r[2]:r[1],c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&(i=c.id,s=c.name)}}if(!i)return{success:!1,error:`Could not determine account for ${e.path}`};try{let r=await(0,u.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:i,accountName:s,noteTitle:e.basename,notePath:e.path,content:t,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return r.json?.success?{success:!0}:{success:!1,error:r.json?.error||"Unknown error",authRequired:r.json?.authRequired}}catch(r){return{success:!1,error:r.message}}}async syncNoteToSalesforce(){let e=this.app.workspace.getActiveFile();if(!e){new u.Notice("No active file to sync");return}if(!this.app.metadataCache.getFileCache(e)?.frontmatter?.sync_to_salesforce){new u.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}new u.Notice("Syncing to Salesforce...");let n=await this.syncSpecificNoteToSalesforce(e);n.success?new u.Notice("Synced to Salesforce"):n.authRequired?new u.Notice("Salesforce authentication required. Please reconnect."):new u.Notice("Failed to sync: "+(n.error||"Unknown error"))}async checkAndAutoEnrich(){let e=this.settings.accountsFolder||"Accounts",t=this.app.vault.getAbstractFileByPath(e);if(!t||!(t instanceof u.TFolder))return;let n=[];for(let i of t.children){if(!(i instanceof u.TFolder)||i.name.startsWith("_"))continue;let s=`${i.path}/Contacts.md`,r=this.app.vault.getAbstractFileByPath(s);if(!(!r||!(r instanceof u.TFile))){if(this.app.metadataCache.getFileCache(r)?.frontmatter?.enriched_at)continue}let o=i.name,c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&c.id&&n.push({id:c.id,name:c.name,owner:"",ownerEmail:""})}if(n.length===0){console.log("[Eudia] Auto-enrich: all account folders already enriched");return}console.log(`[Eudia] Auto-enrich: ${n.length} accounts need enrichment`);try{await this.enrichAccountFolders(n)}catch(i){console.error("[Eudia] Auto-enrich failed:",i)}}async enrichAccountFolders(e){if(!e||e.length===0)return;let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=this.settings.accountsFolder||"Accounts",i=e.filter(l=>l.id&&l.id.length>=15);if(i.length===0)return;let s=i.length,r=0,o=0;console.log(`[Eudia Enrich] Starting enrichment for ${s} accounts`),new u.Notice(`Enriching account data: 0/${s}...`);let c=20;for(let l=0;l<i.length;l+=c){let h=i.slice(l,l+c),g=h.map(p=>p.id);try{let p=await(0,u.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:g,userEmail:this.settings.userEmail})});if(p.json?.success&&p.json?.enrichments){let y=p.json.enrichments;for(let w of h){let S=y[w.id];if(S)try{await this.writeEnrichmentToAccount(w,S,n),r++}catch(C){o++,console.error(`[Eudia Enrich] Write failed for ${w.name}:`,C)}}}}catch(p){o+=h.length,console.error("[Eudia Enrich] Batch fetch failed:",p)}let m=Math.min(l+c,s);new u.Notice(`Enriching account data: ${m}/${s}...`),l+c<i.length&&await new Promise(p=>setTimeout(p,100))}let d=o>0?`Enrichment complete: ${r} enriched, ${o} skipped`:`Enrichment complete: ${r} accounts enriched with Salesforce data`;console.log(`[Eudia Enrich] ${d}`),new u.Notice(d)}async writeEnrichmentToAccount(e,t,n){let i=e.name.replace(/[<>:"/\\|?*]/g,"_").trim(),s=`${n}/${i}`,r=this.app.vault.getAbstractFileByPath(s);if(r instanceof u.TFolder||(s=`${n}/_Prospects/${i}`,r=this.app.vault.getAbstractFileByPath(s)),!(r instanceof u.TFolder))return;let o=new Date().toISOString(),c=async(d,l)=>{let h=`${s}/${d}`,g=this.app.vault.getAbstractFileByPath(h);if(!(g instanceof u.TFile))return;let m=await this.app.vault.read(g),p="",y=m;if(m.startsWith("---")){let v=m.indexOf("---",3);v!==-1&&(p=m.substring(0,v+3),y=m.substring(v+3),p.includes("enriched_at:")?p=p.replace(/enriched_at:.*/,`enriched_at: "${o}"`):p=p.substring(0,v)+`enriched_at: "${o}"
---`)}let w=y.match(/^(\s*#[^\n]+)/),C=`${w?w[1]:""}
...
${w.join(`
`)}
`)}}}startSalesforceSyncScanner(){if(!this.settings.sfAutoSyncEnabled){console.log("[Eudia SF Sync] Auto-sync is disabled in settings"),this.updateSfSyncStatusBar("SF Sync: Off");return}let e=(this.settings.sfAutoSyncIntervalMinutes||15)*60*1e3;console.log(`[Eudia SF Sync] Starting scanner \u2014 interval: ${this.settings.sfAutoSyncIntervalMinutes}min`),this.updateSfSyncStatusBar("SF Sync: Idle");let t=window.setTimeout(()=>{this.runSalesforceSyncScan()},3e4);this.registerInterval(t),this.sfSyncIntervalId=window.setInterval(()=>{this.runSalesforceSyncScan()},e),this.registerInterval(this.sfSyncIntervalId)}async runSalesforceSyncScan(){if(!(!this.settings.sfAutoSyncEnabled||!this.settings.userEmail)){console.log("[Eudia SF Sync] Running scan...");try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!(e instanceof u.TFolder)){console.log("[Eudia SF Sync] Accounts folder not found");return}let t=[],n=l=>{for(let h of l.children)h instanceof u.TFile&&h.extension==="md"?t.push(h):h instanceof u.TFolder&&n(h)};n(e);let i=[];for(let l of t){let g=this.app.metadataCache.getFileCache(l)?.frontmatter;if(!g?.sync_to_salesforce)continue;let m=g.last_sf_sync?new Date(g.last_sf_sync).getTime():0;l.stat.mtime>m&&i.push(l)}if(i.length===0){console.log("[Eudia SF Sync] No flagged notes need syncing"),this.updateSfSyncStatusBar("SF Sync: Idle");return}console.log(`[Eudia SF Sync] ${i.length} note(s) queued for sync`),this.updateSfSyncStatusBar(`SF Sync: Syncing ${i.length}...`);let s=0,r=0;for(let l of i){let h=await this.syncSpecificNoteToSalesforce(l);if(h.success)s++,await this.updateNoteSyncTimestamp(l);else if(r++,console.log(`[Eudia SF Sync] Failed to sync ${l.path}: ${h.error}`),h.authRequired){new u.Notice("Salesforce authentication expired. Please reconnect to resume auto-sync."),this.updateSfSyncStatusBar("SF Sync: Auth required");return}}let c=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),d=r>0?`SF Sync: ${s} synced, ${r} failed at ${c}`:`SF Sync: ${s} note${s!==1?"s":""} synced at ${c}`;console.log(`[Eudia SF Sync] ${d}`),this.updateSfSyncStatusBar(d),s>0&&new u.Notice(d)}catch(e){console.error("[Eudia SF Sync] Scan error:",e),this.updateSfSyncStatusBar("SF Sync: Error")}}}async updateNoteSyncTimestamp(e){try{let t=await this.app.vault.read(e),n=new Date().toISOString();if(t.startsWith("---")){let i=t.indexOf("---",3);if(i!==-1){let s=t.substring(0,i),r=t.substring(i);if(s.includes("last_sf_sync:")){let o=s.replace(/last_sf_sync:.*/,`last_sf_sync: "${n}"`)+r;await this.app.vault.modify(e,o)}else{let o=s+`last_sf_sync: "${n}"
`+r;await this.app.vault.modify(e,o)}}}}catch(t){console.error(`[Eudia SF Sync] Failed to update sync timestamp for ${e.path}:`,t)}}updateSfSyncStatusBar(e){this.sfSyncStatusBarEl&&this.sfSyncStatusBarEl.setText(e)}async createMeetingNote(){return new Promise(e=>{new X(this.app,this,async n=>{if(!n){e();return}let i=new Date().toISOString().split("T")[0],s=n.name.replace(/[<>:"/\\|?*]/g,"_").trim(),r=`${this.settings.accountsFolder}/${s}`,o=`${i} Meeting.md`,c=`${r}/${o}`;this.app.vault.getAbstractFileByPath(r)||await this.app.vault.createFolder(r);let d=`---
...
`,l=await this.app.vault.create(c,d);await this.app.workspace.getLeaf().openFile(l),new u.Notice(`Created meeting note for ${n.name}`),e()}).open()})}async fetchAndInsertContext(){new u.Notice("Fetching pre-call context...")}async refreshAnalyticsDashboard(e){if(!this.settings.userEmail){console.log("[Eudia] Cannot refresh analytics - no email configured");return}let n=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);if(!n)return;let i=n[1];if(!i.includes("type: analytics_dashboard"))return;let s=i.match(/category:\s*(\w+)/)?.[1]||"team";console.log(`[Eudia] Refreshing analytics dashboard: ${e.name} (${s})`);try{let r=null,o=this.settings.serverUrl,c=encodeURIComponent(this.settings.userEmail);switch(s){case"pain_points":r=(await(0,u.requestUrl)({url:`${o}/api/analytics/pain-points?days=30`,method:"GET"})).json,r.success&&await this.updatePainPointNote(e,r.painPoints);break;case"objections":r=(await(0,u.requestUrl)({url:`${o}/api/analytics/objection-playbook?days=90`,method:"GET"})).json,r.success&&await this.updateObjectionNote(e,r);break;case"coaching":case"team":default:r=(await(0,u.requestUrl)({url:`${o}/api/analytics/team-trends?managerId=${c}`,method:"GET"})).json,r.success&&await this.updateTeamPerformanceNote(e,r.trends);break}r?.success&&new u.Notice(`Analytics refreshed: ${e.name}`)}catch(r){console.error("[Eudia] Analytics refresh error:",r)}}async updatePainPointNote(e,t){if(!t||t.length===0)return;let n=new Date().toISOString().split("T")[0],i=t.slice(0,10).map(d=>`| ${d.painPoint||"--"} | ${d.count||0} | ${d.category||"--"} | ${d.averageSeverity||"medium"} |`).join(`
`),s={};for(let d of t){let l=d.category||"other";s[l]||(s[l]=[]),s[l].push(d)}let r="";for(let[d,l]of Object.entries(s)){r+=`
### ${d.charAt(0).toUpperCase()+d.slice(1)}
`;for(let h of l.slice(0,3))r+=`- ${h.painPoint}
...
`;await this.app.vault.modify(e,c)}async updateObjectionNote(e,t){if(!t.objections||t.objections.length===0)return;let n=new Date().toISOString().split("T")[0],i=t.objections.slice(0,10).map(o=>{let c=o.handleRatePercent>=75?"\u2705 Strong":o.handleRatePercent>=50?"\u26A0\uFE0F Moderate":"\u274C Needs Work";return`| ${o.objection?.substring(0,40)||"--"}... | ${o.count||0} | ${o.handleRatePercent||0}% | ${c} |`}).join(`
`),s="";for(let o of t.objections.slice(0,5))if(o.bestResponses&&o.bestResponses.length>0){s+=`
...
`;await this.app.vault.modify(e,r)}async updateTeamPerformanceNote(e,t){if(!t)return;let n=new Date().toISOString().split("T")[0],i=r=>r>0?`\u2191 ${Math.abs(r).toFixed(1)}%`:r<0?`\u2193 ${Math.abs(r).toFixed(1)}%`:"--",s=`---
type: analytics_dashboard
auto_refresh: true
