...
- Action items have clear owners`}function We(S){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${S?`
...
`).filter(v=>v.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||h!=="Unknown")&&s.push({account:c.name,lastUpdated:h,nextSteps:g})}}s.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let r=new Date().toISOString().split("T")[0],a=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${r}
---
