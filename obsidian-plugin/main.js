...
`;let C=`${n}/${y}.md`,v=this.app.vault.getAbstractFileByPath(C);v instanceof p.TFile?await this.app.vault.modify(v,w):await this.app.vault.create(C,w)}console.log(`[Eudia] Created CS Manager dashboard for ${t} with ${e.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(t){let e=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),await this.ensurePipelineFolderExists();let r=0,i=0,a=new Date().toISOString().split("T")[0],o=async l=>{let d=l.name.replace(/[<>:"/\\|?*]/g,"_").trim(),u=`${e}/${d}`;if(this.app.vault.getAbstractFileByPath(u)instanceof p.TFolder)return!1;try{return await this.app.vault.createFolder(u),await this.createExecAccountSubnotes(u,l,a),l.isOwned?r++:i++,console.log(`[Eudia Admin] Created ${l.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,m),!1}},c=5;for(let l=0;l<t.length;l+=c){let d=t.slice(l,l+c);await Promise.allSettled(d.map(u=>o(u)))}this.settings.cachedAccounts=t.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),r+i>0&&new p.Notice(`Created ${r} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(t,e,n){let r=e.ownerName||"Unknown",i=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
`):typeof g=="object"?JSON.stringify(g):String(g),r=n(t.title)||"Meeting Notes",i=n(t.summary),a=n(t.painPoints),o=n(t.productInterest),c=n(t.meddiccSignals),l=n(t.nextSteps),d=n(t.actionItems),u=n(t.keyDates),y=n(t.risksObjections),m=n(t.attendees),h=`---
title: "${r}"
