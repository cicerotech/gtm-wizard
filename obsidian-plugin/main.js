var ye=Object.create;var G=Object.defineProperty;var ge=Object.getOwnPropertyDescriptor;var fe=Object.getOwnPropertyNames;var we=Object.getPrototypeOf,Oe=Object.prototype.hasOwnProperty;var ve=(O,s)=>{for(var t in s)G(O,t,{get:s[t],enumerable:!0})},se=(O,s,t,e)=>{if(s&&typeof s=="object"||typeof s=="function")for(let n of fe(s))!Oe.call(O,n)&&n!==t&&G(O,n,{get:()=>s[n],enumerable:!(e=ge(s,n))||e.enumerable});return O};var U=(O,s,t)=>(t=O!=null?ye(we(O)):{},se(s||!O||!O.__esModule?G(t,"default",{value:O,enumerable:!0}):t,O)),Se=O=>se(G({},"__esModule",{value:!0}),O);var Le={};ve(Le,{default:()=>V});module.exports=Se(Le);var u=require("obsidian");var T=class O{constructor(){this.mediaRecorder=null;this.audioChunks=[];this.stream=null;this.startTime=0;this.pausedDuration=0;this.pauseStartTime=0;this.durationInterval=null;this.audioContext=null;this.analyser=null;this.levelInterval=null;this.lastExtractedChunkIndex=0;this.mimeTypeCache="audio/webm";this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0};this.stateCallback=null;this.levelHistory=[];this.trackingLevels=!1}onStateChange(s){this.stateCallback=s}static isIOSOrSafari(){let s=navigator.userAgent,t=/iPad|iPhone|iPod/.test(s)&&!window.MSStream,e=/^((?!chrome|android).)*safari/i.test(s);return t||e}getSupportedMimeType(){let s=O.isIOSOrSafari(),t=s?["audio/mp4","audio/mp4;codecs=aac","audio/aac","audio/webm;codecs=opus","audio/webm"]:["audio/webm;codecs=opus","audio/webm","audio/mp4","audio/ogg;codecs=opus","audio/ogg"];for(let e of t)if(MediaRecorder.isTypeSupported(e))return console.log(`[AudioRecorder] Using MIME type: ${e} (iOS/Safari: ${s})`),e;return s?"audio/mp4":"audio/webm"}async startRecording(){if(this.state.isRecording)throw new Error("Already recording");try{let s=O.isIOSOrSafari(),t=s?{echoCancellation:!0,noiseSuppression:!0}:{echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0,sampleRate:48e3,channelCount:1};this.stream=await navigator.mediaDevices.getUserMedia({audio:t}),console.log(`[AudioRecorder] Microphone access granted (iOS/Safari: ${s})`),this.setupAudioAnalysis();let e=this.getSupportedMimeType();this.mimeTypeCache=e,this.mediaRecorder=new MediaRecorder(this.stream,{mimeType:e,audioBitsPerSecond:128e3}),this.audioChunks=[],this.lastExtractedChunkIndex=0,this.mediaRecorder.ondataavailable=n=>{n.data.size>0&&this.audioChunks.push(n.data)},this.mediaRecorder.start(1e3),this.startTime=Date.now(),this.pausedDuration=0,this.state={isRecording:!0,isPaused:!1,duration:0,audioLevel:0},this.startDurationTracking(),this.startLevelTracking(),this.notifyStateChange()}catch(s){throw this.cleanup(),new Error(`Failed to start recording: ${s.message}`)}}setupAudioAnalysis(){if(this.stream)try{this.audioContext=new AudioContext;let s=this.audioContext.createMediaStreamSource(this.stream);this.analyser=this.audioContext.createAnalyser(),this.analyser.fftSize=256,s.connect(this.analyser)}catch(s){console.warn("Failed to set up audio analysis:",s)}}startDurationTracking(){this.durationInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused){let s=Date.now()-this.startTime-this.pausedDuration;this.state.duration=Math.floor(s/1e3),this.notifyStateChange(),this.state.duration>=5400&&(console.log("[Eudia] Maximum recording duration reached (90 minutes) \u2014 auto-stopping"),this.stop())}},100)}startLevelTracking(){if(!this.analyser)return;let s=new Uint8Array(this.analyser.frequencyBinCount);this.levelInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused&&this.analyser){this.analyser.getByteFrequencyData(s);let t=0;for(let n=0;n<s.length;n++)t+=s[n];let e=t/s.length;this.state.audioLevel=Math.min(100,Math.round(e/255*100*2)),this.notifyStateChange()}},50)}pauseRecording(){!this.state.isRecording||this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="recording"&&(this.mediaRecorder.pause(),this.pauseStartTime=Date.now(),this.state.isPaused=!0,this.notifyStateChange())}resumeRecording(){!this.state.isRecording||!this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="paused"&&(this.mediaRecorder.resume(),this.pausedDuration+=Date.now()-this.pauseStartTime,this.state.isPaused=!1,this.notifyStateChange())}async stopRecording(){return new Promise((s,t)=>{if(!this.mediaRecorder||!this.state.isRecording){t(new Error("Not currently recording"));return}let e=this.mediaRecorder.mimeType,n=this.state.duration,r=!1,i=setTimeout(()=>{if(!r){r=!0,console.warn("AudioRecorder: onstop timeout, forcing completion");try{let a=new Blob(this.audioChunks,{type:e}),o=new Date,c=o.toISOString().split("T")[0],l=o.toTimeString().split(" ")[0].replace(/:/g,"-"),d=e.includes("webm")?"webm":e.includes("mp4")?"m4a":e.includes("ogg")?"ogg":"webm",p=`recording-${c}-${l}.${d}`;this.cleanup(),s({audioBlob:a,duration:n,mimeType:e,filename:p})}catch{this.cleanup(),t(new Error("Failed to process recording after timeout"))}}},1e4);this.mediaRecorder.onstop=()=>{if(!r){r=!0,clearTimeout(i);try{console.log(`[AudioRecorder] Chunks collected: ${this.audioChunks.length}`);let a=new Blob(this.audioChunks,{type:e});console.log(`[AudioRecorder] Blob size: ${a.size} bytes`);let o=new Date,c=o.toISOString().split("T")[0],l=o.toTimeString().split(" ")[0].replace(/:/g,"-"),d=e.includes("webm")?"webm":e.includes("mp4")?"m4a":e.includes("ogg")?"ogg":"webm",p=`recording-${c}-${l}.${d}`;this.cleanup(),s({audioBlob:a,duration:n,mimeType:e,filename:p})}catch(a){this.cleanup(),t(a)}}},this.mediaRecorder.onerror=a=>{r||(r=!0,clearTimeout(i),this.cleanup(),t(new Error("Recording error occurred")))},this.mediaRecorder.state==="recording"&&this.mediaRecorder.requestData(),setTimeout(()=>{this.mediaRecorder&&this.mediaRecorder.state!=="inactive"&&this.mediaRecorder.stop()},100)})}cancelRecording(){this.cleanup()}cleanup(){this.durationInterval&&(clearInterval(this.durationInterval),this.durationInterval=null),this.levelInterval&&(clearInterval(this.levelInterval),this.levelInterval=null),this.audioContext&&(this.audioContext.close().catch(()=>{}),this.audioContext=null,this.analyser=null),this.stream&&(this.stream.getTracks().forEach(s=>s.stop()),this.stream=null),this.mediaRecorder=null,this.audioChunks=[],this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0},this.notifyStateChange()}getState(){return{...this.state}}static isSupported(){if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||!window.MediaRecorder)return!1;let t=["audio/webm","audio/mp4","audio/ogg","audio/webm;codecs=opus"].some(e=>MediaRecorder.isTypeSupported(e));return t||console.warn("[AudioRecorder] No supported audio formats found"),t}static getMobileInstructions(){return this.isIOSOrSafari()?"For best results on iOS, ensure you have granted microphone permissions in Settings > Privacy > Microphone.":null}notifyStateChange(){this.stateCallback&&this.stateCallback({...this.state})}static formatDuration(s){let t=Math.floor(s/60),e=s%60;return`${t.toString().padStart(2,"0")}:${e.toString().padStart(2,"0")}`}static async blobToBase64(s){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];t(i)},n.onerror=e,n.readAsDataURL(s)})}static async blobToArrayBuffer(s){return s.arrayBuffer()}async start(){return this.startRecording()}async stop(){return this.stopRecording()}pause(){return this.pauseRecording()}resume(){return this.resumeRecording()}cancel(){return this.cancelRecording()}isRecording(){return this.state.isRecording}extractNewChunks(){if(!this.state.isRecording||this.audioChunks.length===0)return null;let s=this.audioChunks.slice(this.lastExtractedChunkIndex);return s.length===0?null:(this.lastExtractedChunkIndex=this.audioChunks.length,new Blob(s,{type:this.mimeTypeCache}))}getAllChunksAsBlob(){return this.audioChunks.length===0?null:new Blob(this.audioChunks,{type:this.mimeTypeCache})}getDuration(){return this.state.duration}getMimeType(){return this.mimeTypeCache}startLevelHistoryTracking(){this.levelHistory=[],this.trackingLevels=!0}recordLevelSample(){this.trackingLevels&&this.levelHistory.push(this.state.audioLevel)}getAudioDiagnostic(){if(this.levelHistory.length===0)return{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:100,warning:"Unable to analyze audio levels - recording may be too short"};let s=this.levelHistory.reduce((i,a)=>i+a,0)/this.levelHistory.length,t=Math.max(...this.levelHistory),e=this.levelHistory.filter(i=>i<5).length,n=Math.round(e/this.levelHistory.length*100),r=null;return t<5?r="SILENT AUDIO: No audio was detected during recording. Check your microphone settings and ensure Obsidian has microphone permission.":s<10&&n>80?r="VERY LOW AUDIO: Audio levels were extremely low. The transcription may not be accurate. Check your microphone or move closer to it.":n>90&&(r="MOSTLY SILENT: Over 90% of the recording had no audio. Make sure you're capturing the meeting audio, not just silence."),{hasAudio:t>=5,averageLevel:Math.round(s),peakLevel:t,silentPercent:n,warning:r}}static async analyzeAudioBlob(s){try{let t=new AudioContext,e=await s.arrayBuffer(),n;try{n=await t.decodeAudioData(e)}catch{return await t.close(),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:"Could not analyze audio format. Proceeding with transcription."}}let r=n.getChannelData(0),i=0,a=0,o=0,c=.01,l=100,d=0;for(let f=0;f<r.length;f+=l){let w=Math.abs(r[f]);i+=w,w>a&&(a=w),w<c&&o++,d++}await t.close();let p=i/d,y=Math.round(o/d*100),m=Math.round(p*100*10),h=Math.round(a*100),g=null;return a<.01?g='SILENT AUDIO DETECTED: The recording appears to contain only silence. This typically causes Whisper to hallucinate random text like "Yes. Yes. Yes." Check your audio input source.':p<.005&&y>95?g="NEAR-SILENT AUDIO: The recording is almost entirely silent. The transcription will likely be inaccurate.":y>90&&(g="MOSTLY SILENT: Over 90% of the recording is silent. Consider checking your audio setup."),{hasAudio:a>=.01,averageLevel:m,peakLevel:h,silentPercent:y,warning:g}}catch(t){return console.error("Audio analysis failed:",t),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:null}}}};var N=require("obsidian");var X=class{constructor(){this.salesforceAccounts=[]}setAccounts(s){this.salesforceAccounts=s}detectAccount(s,t,e){if(s){let n=this.detectFromTitle(s);if(n.confidence>=70)return n}if(e){let n=this.detectFromFilePath(e);if(n.confidence>=70)return n}if(t&&t.length>0){let n=this.detectFromAttendees(t);if(n.confidence>=50)return n}return{account:null,accountId:null,confidence:0,source:"none",evidence:"No account detected from available context"}}detectFromTitle(s){if(!s)return{account:null,accountId:null,confidence:0,source:"title",evidence:"No title"};let t=[{regex:/^([A-Za-z0-9][^-–—]+?)\s*[-–—]\s*(?:[A-Z][a-z]+|[A-Za-z]{2,})/,confidence:85},{regex:/(?:call|meeting|sync|check-in|demo|discovery)\s+(?:with|re:?|@)\s+([^-–—]+?)(?:\s*[-–—]|$)/i,confidence:80},{regex:/^([A-Za-z][^-–—]+?)\s+(?:discovery|demo|review|kickoff|intro|onboarding|sync)\s*(?:call)?$/i,confidence:75},{regex:/^([^:]+?):\s+/i,confidence:70},{regex:/^\[([^\]]+)\]/,confidence:75}],e=["weekly","daily","monthly","internal","team","1:1","one on one","standup","sync","meeting","call","notes","monday","tuesday","wednesday","thursday","friday","untitled","new","test"];for(let n of t){let r=s.match(n.regex);if(r&&r[1]){let i=r[1].trim();if(e.some(o=>i.toLowerCase()===o)||i.length<2)continue;let a=this.fuzzyMatchSalesforce(i);return a?{account:a.name,accountId:a.id,confidence:Math.min(n.confidence+10,100),source:"salesforce_match",evidence:`Matched "${i}" from title to Salesforce account "${a.name}"`}:{account:i,accountId:null,confidence:n.confidence,source:"title",evidence:"Extracted from meeting title pattern"}}}return{account:null,accountId:null,confidence:0,source:"title",evidence:"No pattern matched"}}detectFromFilePath(s){let t=s.match(/Accounts\/([^\/]+)\//i);if(t&&t[1]){let e=t[1].trim(),n=this.fuzzyMatchSalesforce(e);return n?{account:n.name,accountId:n.id,confidence:95,source:"salesforce_match",evidence:`File in account folder "${e}" matched to "${n.name}"`}:{account:e,accountId:null,confidence:85,source:"title",evidence:`File located in Accounts/${e} folder`}}return{account:null,accountId:null,confidence:0,source:"none",evidence:"Not in Accounts folder"}}detectFromAttendees(s){let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com"],e=new Set;for(let a of s){let c=a.toLowerCase().match(/@([a-z0-9.-]+)/);if(c){let l=c[1];!l.includes("eudia.com")&&!t.includes(l)&&e.add(l)}}if(e.size===0)return{account:null,accountId:null,confidence:0,source:"attendee_domain",evidence:"No external domains"};for(let a of e){let o=a.split(".")[0],c=o.charAt(0).toUpperCase()+o.slice(1),l=this.fuzzyMatchSalesforce(c);if(l)return{account:l.name,accountId:l.id,confidence:75,source:"salesforce_match",evidence:`Matched attendee domain ${a} to "${l.name}"`}}let n=Array.from(e)[0],r=n.split(".")[0];return{account:r.charAt(0).toUpperCase()+r.slice(1),accountId:null,confidence:50,source:"attendee_domain",evidence:`Guessed from external attendee domain: ${n}`}}fuzzyMatchSalesforce(s){if(!s||this.salesforceAccounts.length===0)return null;let t=s.toLowerCase().trim();for(let e of this.salesforceAccounts)if(e.name?.toLowerCase()===t)return e;for(let e of this.salesforceAccounts)if(e.name?.toLowerCase().startsWith(t))return e;for(let e of this.salesforceAccounts)if(e.name?.toLowerCase().includes(t))return e;for(let e of this.salesforceAccounts)if(t.includes(e.name?.toLowerCase()))return e;return null}suggestAccounts(s,t=10){if(!s||s.length<2)return this.salesforceAccounts.slice(0,t).map(r=>({...r,score:0}));let e=s.toLowerCase(),n=[];for(let r of this.salesforceAccounts){let i=r.name?.toLowerCase()||"",a=0;i===e?a=100:i.startsWith(e)?a=90:i.includes(e)?a=70:e.includes(i)&&(a=50),a>0&&n.push({...r,score:a})}return n.sort((r,i)=>i.score-r.score).slice(0,t)}},Be=new X,Ce=["pipeline review","pipeline call","weekly pipeline","forecast call","forecast review","deal review","opportunity review","sales review","pipeline sync","forecast sync","deal sync","pipeline update","forecast meeting"];function re(O,s){if(O){let t=O.toLowerCase();for(let e of Ce)if(t.includes(e))return{isPipelineMeeting:!0,confidence:95,evidence:`Title contains "${e}"`}}if(s&&s.length>=2){let t=["eudia.com","johnsonhana.com"];if(s.every(n=>{let r=n.toLowerCase().split("@")[1]||"";return t.some(i=>r.includes(i))})&&s.length>=3){if(O){let n=O.toLowerCase();if(["sync","review","update","weekly","team","forecast"].some(a=>n.includes(a)))return{isPipelineMeeting:!0,confidence:70,evidence:`All internal attendees (${s.length}) with team meeting signal`}}return{isPipelineMeeting:!1,confidence:40,evidence:"All internal attendees but no clear pipeline signal"}}}return{isPipelineMeeting:!1,confidence:0,evidence:"No pipeline meeting indicators found"}}function be(O,s){let t="";return(s?.account||s?.opportunities?.length)&&(t=`
ACCOUNT CONTEXT (use to inform your analysis):
${s.account?`- Account: ${s.account.name}`:""}
${s.account?.owner?`- Account Owner: ${s.account.owner}`:""}
${s.opportunities?.length?`- Open Opportunities: ${s.opportunities.map(e=>`${e.name} (${e.stage}, $${(e.acv/1e3).toFixed(0)}k)`).join("; ")}`:""}
${s.contacts?.length?`- Known Contacts: ${s.contacts.slice(0,5).map(e=>`${e.name} - ${e.title}`).join("; ")}`:""}
...
Format your response as a brief, actionable answer suitable for quick reference during a call.`}static formatSectionsForNote(s,t,e){let n="";if(s.summary&&(n+=`## TL;DR
...
`,e.coaching){let d=e.coaching;if(d.totalQuestions>0){let p=Math.round(d.openQuestions/d.totalQuestions*100);n+=`**Questions:** ${d.totalQuestions} total (${d.openQuestions} open, ${d.closedQuestions} closed - ${p}% open)
`}if(d.objections&&d.objections.length>0){let p=d.objections.filter(y=>y.handled).length;n+=`**Objections:** ${d.objections.length} raised, ${p} handled
...
`);let r=e?.enabled&&e?.formattedTranscript?e.formattedTranscript:t;if(r){let i=e?.enabled?"Full Transcript (Speaker-Attributed)":"Full Transcript";n+=`---
...
`}return n}static formatSectionsWithAudio(s,t,e,n){let r=this.formatSectionsForNote(s,t,n);return e&&(r+=`
---

## Recording

![[${e}]]
`),r}static formatContextForNote(s){if(!s.success)return"";let t=`## Pre-Call Context

`;if(s.account&&(t+=`**Account:** ${s.account.name}
`,t+=`**Owner:** ${s.account.owner}

`),s.opportunities&&s.opportunities.length>0){t+=`### Open Opportunities

`;for(let e of s.opportunities){let n=e.acv?`$${(e.acv/1e3).toFixed(0)}k`:"TBD";t+=`- **${e.name}** - ${e.stage} - ${n}`,e.targetSignDate&&(t+=` - Target: ${new Date(e.targetSignDate).toLocaleDateString()}`),t+=`
`}t+=`
`}if(s.contacts&&s.contacts.length>0){t+=`### Key Contacts

`;for(let e of s.contacts.slice(0,5))t+=`- **${e.name}**`,e.title&&(t+=` - ${e.title}`),t+=`
`;t+=`
`}if(s.lastMeeting&&(t+=`### Last Meeting

`,t+=`${new Date(s.lastMeeting.date).toLocaleDateString()} - ${s.lastMeeting.subject}

`),s.account?.customerBrain){let e=s.account.customerBrain.substring(0,500);e&&(t+=`### Recent Notes

`,t+=`${e}${s.account.customerBrain.length>500?"...":""}

`)}return t+=`---

`,t}async blobToBase64(s){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];t(i)},n.onerror=e,n.readAsDataURL(s)})}async transcribeAudio(s,t){try{let e=await this.blobToBase64(s),n=s.type||"audio/webm",r=t?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:t.pipelineContext}:void 0,i=await this.transcribeAndSummarize(e,n,t?.accountName,t?.accountId,r);return{text:i.transcript,confidence:i.success?.95:0,duration:i.duration,sections:i.sections}}catch(e){return console.error("transcribeAudio error:",e),{text:"",confidence:0,duration:0,sections:this.getEmptySections()}}}async processTranscription(s,t){if(!s||s.trim().length===0)return this.getEmptySections();try{let e=await(0,N.requestUrl)({url:`${this.serverUrl}/api/process-sections`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:s,accountName:t?.accountName,context:t})});if(e.json?.success&&e.json?.sections){let n=e.json.sections;return{summary:n.summary||"",painPoints:n.painPoints||n.keyPoints||"",productInterest:n.productInterest||"",meddiccSignals:n.meddiccSignals||"",nextSteps:n.nextSteps||"",actionItems:n.actionItems||"",keyDates:n.keyDates||"",buyingTriggers:n.buyingTriggers||"",dealSignals:n.dealSignals||"",risksObjections:n.risksObjections||n.concerns||"",competitiveIntel:n.competitiveIntel||"",attendees:n.attendees||"",transcript:s}}return console.warn("Server process-sections returned no sections, using fallback"),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:s}}catch(e){return console.error("processTranscription server error:",e),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:s}}}};var Y=require("obsidian"),P=class O{constructor(s,t,e="America/New_York"){this.serverUrl=s,this.userEmail=t.toLowerCase(),this.timezone=e}setUserEmail(s){this.userEmail=s.toLowerCase()}setServerUrl(s){this.serverUrl=s}setTimezone(s){this.timezone=s}async getTodaysMeetings(s=!1){if(!this.userEmail)return{success:!1,date:new Date().toISOString().split("T")[0],email:"",meetingCount:0,meetings:[],error:"User email not configured"};try{let t=encodeURIComponent(this.timezone),e=s?"&forceRefresh=true":"";return(await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/today?timezone=${t}${e}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(t){return console.error("Failed to fetch today's meetings:",t),{success:!1,date:new Date().toISOString().split("T")[0],email:this.userEmail,meetingCount:0,meetings:[],error:t.message||"Failed to fetch calendar"}}}async getWeekMeetings(s=!1){if(!this.userEmail)return{success:!1,startDate:"",endDate:"",email:"",totalMeetings:0,byDay:{},error:"User email not configured"};try{let t=encodeURIComponent(this.timezone),e=s?"&forceRefresh=true":"";return(await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/week?timezone=${t}${e}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(t){return console.error("Failed to fetch week's meetings:",t),{success:!1,startDate:"",endDate:"",email:this.userEmail,totalMeetings:0,byDay:{},error:t.message||"Failed to fetch calendar"}}}async getMeetingsInRange(s,t){if(!this.userEmail)return[];try{let e=s.toISOString().split("T")[0],n=t.toISOString().split("T")[0],r=await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/range?start=${e}&end=${n}`,method:"GET",headers:{Accept:"application/json"}});return r.json.success?r.json.meetings||[]:[]}catch(e){return console.error("Failed to fetch calendar range:",e),[]}}async getCurrentMeeting(){let s=await this.getTodaysMeetings();if(!s.success||s.meetings.length===0)return{meeting:null,isNow:!1};let t=new Date;for(let e of s.meetings){let n=O.safeParseDate(e.start),r=O.safeParseDate(e.end);if(t>=n&&t<=r)return{meeting:e,isNow:!0};let i=(n.getTime()-t.getTime())/(1e3*60);if(i>0&&i<=15)return{meeting:e,isNow:!1,minutesUntilStart:Math.ceil(i)}}return{meeting:null,isNow:!1}}async getMeetingsForAccount(s){let t=await this.getWeekMeetings();if(!t.success)return[];let e=[];Object.values(t.byDay).forEach(r=>{e.push(...r)});let n=s.toLowerCase();return e.filter(r=>r.accountName?.toLowerCase().includes(n)||r.subject.toLowerCase().includes(n)||r.attendees.some(i=>i.email.toLowerCase().includes(n.split(" ")[0])))}static formatMeetingForNote(s){let t=s.attendees.filter(e=>e.isExternal!==!1).map(e=>e.name||e.email.split("@")[0]).slice(0,5).join(", ");return{title:s.subject,attendees:t,meetingStart:s.start,accountName:s.accountName}}static getDayName(s){let t;s.length===10&&s.includes("-")?t=new Date(s+"T00:00:00"):t=new Date(s);let e=new Date;e.setHours(0,0,0,0);let n=new Date(t);n.setHours(0,0,0,0);let r=Math.round((n.getTime()-e.getTime())/(1e3*60*60*24));return r===0?"Today":r===1?"Tomorrow":t.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}static formatTime(s,t){let e=s;e&&!e.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(e)&&(e=e+"Z");let n=new Date(e);if(isNaN(n.getTime()))return s;let r={hour:"numeric",minute:"2-digit",hour12:!0};return t&&(r.timeZone=t),n.toLocaleTimeString("en-US",r)}static safeParseDate(s){if(!s)return new Date(NaN);let t=s;return!t.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(t)&&(t=t+"Z"),new Date(t)}static getMeetingDuration(s,t){let e=O.safeParseDate(s),n=O.safeParseDate(t);return Math.round((n.getTime()-e.getTime())/(1e3*60))}};var We=["ai-contracting-tech","ai-contracting-services","ai-compliance-tech","ai-compliance-services","ai-ma-tech","ai-ma-services","sigma"],je=["metrics-identified","economic-buyer-identified","decision-criteria-discussed","decision-process-discussed","pain-confirmed","champion-identified","competition-mentioned"],Ie=["progressing","stalled","at-risk","champion-engaged","early-stage"],xe=["discovery","demo","negotiation","qbr","implementation","follow-up"],Ee=`You are a sales intelligence tagger for Eudia, an AI legal technology company. Extract structured tags from meeting analysis.
...
`),e=e.replace(/^#{1,3}\s+.+\n+(?=#{1,3}\s|\s*$)/gm,""),e=e.replace(/^#{2,3}\s+(.+)$/gm,'</p><h3 class="eudia-intel-header">$1</h3><p>'),e=e.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^-\s+\[\s*\]\s+(.+)$/gm,'<li class="eudia-intel-todo">$1</li>'),e=e.replace(/^-\s+\[x\]\s+(.+)$/gm,'<li class="eudia-intel-done">$1</li>'),e=e.replace(/^[•\-]\s+(.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li[^>]*>.*?<\/li>\s*)+/g,'<ul class="eudia-intel-list">$&</ul>'),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=e.replace(/<p>\s*(<ul)/g,"$1"),e=e.replace(/<\/ul>\s*<\/p>/g,"</ul>"),e=e.replace(/<p>\s*(<h3)/g,"$1"),e=e.replace(/<\/h3>\s*<\/p>/g,"</h3>"),e=e.replace(/<\/li>\s*<br>\s*<li/g,"</li><li"),e=e.replace(/<p>\s*<\/p>/g,""),e=e.replace(/<p>\s*<br>\s*<\/p>/g,""),e=e.replace(/(<br>\s*){2,}/g,""),e=e.replace(/<\/h3>\s*<br>/g,"</h3>"),e=e.replace(/<br>\s*<h3/g,"<h3"),e=e.replace(/<br>\s*<ul/g,"<ul"),e=e.replace(/<\/ul>\s*<br>/g,"</ul>"),e=e.replace(/^(<br>)+|(<br>)+$/g,""),e="<p>"+e+"</p>",e=e.replace(/<p><\/p>/g,""),e}onClose(){this.contentEl.empty()}};var ne=class extends u.ItemView{constructor(t,e){super(t);this.emailInput=null;this.pollInterval=null;this.plugin=e,this.accountOwnershipService=new D(e.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return M}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}if(this.plugin.settings.accountsImported){let e=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(a=>a.children!==void 0)||[];e.length>0?(this.steps[2].status="complete",console.log(`[Eudia] Vault reopen: ${e.length} account folders verified`)):(console.warn("[Eudia] accountsImported=true but 0 account folders found \u2014 resetting for re-import"),this.plugin.settings.accountsImported=!1,this.plugin.settings.importedAccountCount=0,await this.plugin.saveSettings());try{let a=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");a&&await this.plugin.app.vault.delete(a)}catch{}let n=this.plugin.settings.userEmail,i=(this.plugin.settings.cachedAccounts||[]).filter(a=>a.id&&String(a.id).startsWith("001"));if(n&&i.length>0){let a=this.plugin.settings.accountsFolder||"Accounts",o=!1;for(let c of i.slice(0,5)){let l=(c.name||"").replace(/[<>:"/\\|?*]/g,"_").trim(),d=`${a}/${l}/Contacts.md`,p=this.plugin.app.vault.getAbstractFileByPath(d);if(p instanceof u.TFile&&!this.plugin.app.metadataCache.getFileCache(p)?.frontmatter?.enriched_at){o=!0;break}}o&&(console.log("[Eudia Setup] Accounts need enrichment \u2014 triggering on vault reopen..."),setTimeout(async()=>{try{let c=i.map(l=>({id:l.id,name:l.name,type:"",isOwned:!1,hadOpportunity:!0,website:null,industry:null}));await this.plugin.enrichAccountFolders(c),console.log(`[Eudia] Vault-reopen enrichment complete: ${c.length} accounts enriched`)}catch(c){console.log("[Eudia] Vault-reopen enrichment failed (will retry next open):",c)}},3e3))}}else{console.log("[Eudia Setup] Email set but accounts not imported \u2014 auto-retrying import...");let t=this.plugin.app.workspace.leftSplit,e=t?.collapsed;try{let n=this.plugin.settings.userEmail,r=$(n)?"admin":L(n)?"cs":"bl",i=[],a=[];if(console.log(`[Eudia Setup] Auto-retry for ${n} (group: ${r})`),r==="cs")i=[...k],console.log(`[Eudia Setup] Auto-retry CS: using ${i.length} static accounts`);else if(r==="admin")i=await this.accountOwnershipService.getAllAccountsForAdmin(n);else{let o=await this.accountOwnershipService.getAccountsWithProspects(n);i=o.accounts,a=o.prospects}if(i.length>0||a.length>0){if(t&&!e&&t.collapse(),r==="admin"?await this.plugin.createAdminAccountFolders(i):(await this.plugin.createTailoredAccountFolders(i,{}),a.length>0&&await this.plugin.createProspectAccountFiles(a)),H(n))try{await this.plugin.createCSManagerDashboard(n,i)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=i.length+a.length,await this.plugin.saveSettings(),this.steps[2].status="complete";try{let o=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");o&&await this.plugin.app.vault.delete(o)}catch{}t&&!e&&t.expand(),console.log(`[Eudia Setup] Auto-retry imported ${i.length} accounts for ${n}`),new u.Notice(`Enriching ${i.length} accounts with Salesforce contacts...`);try{let o=r==="cs"?i:[...i,...a];await this.plugin.enrichAccountFolders(o),new u.Notice(`${i.length} accounts loaded and enriched!`),console.log("[Eudia Setup] Auto-retry enrichment complete")}catch(o){console.log("[Eudia Setup] Auto-retry enrichment failed:",o),new u.Notice(`${i.length} accounts imported! Contacts will populate on next open.`)}}else console.warn(`[Eudia Setup] Auto-retry returned 0 accounts for ${n}. Server may still be starting.`),t&&!e&&t.expand()}catch(n){console.error("[Eudia Setup] Auto-retry account import failed:",n),t&&!e&&t.expand()}}}}getCompletionPercentage(){let t=this.steps.filter(e=>e.status==="complete").length;return Math.round(t/this.steps.length*100)}async render(){let t=this.containerEl.children[1];t.empty(),t.addClass("eudia-setup-view"),this.renderHeader(t),this.renderSteps(t),this.renderFooter(t)}renderHeader(t){let e=t.createDiv({cls:"eudia-setup-header"}),n=e.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to transcribe and summarize meetings -- capturing objections, next steps, and pain points to drive better client outcomes and smarter selling.",cls:"eudia-setup-subtitle"});let r=e.createDiv({cls:"eudia-setup-progress-section"}),i=this.getCompletionPercentage(),a=r.createDiv({cls:"eudia-setup-progress-label"});a.createSpan({text:"Setup Progress"}),a.createSpan({text:`${i}%`,cls:"eudia-setup-progress-value"});let c=r.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${i}%`}renderSteps(t){let e=t.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(e),this.renderSalesforceStep(e),this.renderTranscribeStep(e)}renderCalendarStep(t){let e=this.steps[0],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),i=r.createDiv({cls:"eudia-setup-step-number"});i.setText(e.status==="complete"?"":"1"),e.status==="complete"&&i.addClass("eudia-step-complete");let a=r.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:e.title}),a.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(e.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let l=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});l.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async d=>{d.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let t=this.emailInput.value.trim().toLowerCase(),e=this.containerEl.querySelector(".eudia-setup-validation-message");if(!t){e&&(e.textContent="Please enter your email",e.className="eudia-setup-validation-message error");return}if(!t.endsWith("@eudia.com")){e&&(e.textContent="Please use your @eudia.com email address",e.className="eudia-setup-validation-message error");return}e&&(e.textContent="Validating...",e.className="eudia-setup-validation-message loading");try{let n=await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(t)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=t,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new u.Notice("Calendar connected successfully!"),e&&(e.textContent="Importing your accounts...",e.className="eudia-setup-validation-message loading");let r=this.plugin.app.workspace.leftSplit,i=r?.collapsed;r&&!i&&r.collapse();try{let a,o=[],c=$(t)?"admin":L(t)?"cs":"bl";if(console.log(`[Eudia] User group detected: ${c} for ${t}`),c==="cs"){if(console.log(`[Eudia] CS user detected \u2014 loading ${k.length} accounts from static data (instant, no server needed)`),a=[...k],o=[],e&&(e.textContent=`Loading ${a.length} Customer Success accounts...`),await this.plugin.createTailoredAccountFolders(a,{}),H(t))try{await this.plugin.createCSManagerDashboard(t,a)}catch(p){console.error("[Eudia] CS Manager dashboard creation failed (non-blocking):",p)}let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(p=>p.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,console.log(`[Eudia] CS accounts verified: ${d.length} folders created`)):(console.warn(`[Eudia] CS folder creation may have failed \u2014 ${d.length} folders found. Keeping accountsImported=false for retry.`),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let p=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");p&&await this.plugin.app.vault.delete(p)}catch{}console.log(`[Eudia] CS accounts created: ${a.length} folders from static data`),e&&(e.textContent=`Enriching ${a.length} accounts with Salesforce contacts...`),new u.Notice(`Enriching ${a.length} accounts with contacts from Salesforce...`),console.log(`[Eudia] Starting synchronous enrichment for ${a.length} CS accounts...`);try{await this.plugin.enrichAccountFolders(a),console.log("[Eudia] Synchronous enrichment complete"),new u.Notice(`${a.length} accounts loaded with contacts from Salesforce!`),e&&(e.textContent=`${a.length} accounts loaded and enriched with Salesforce contacts!`)}catch(p){console.log("[Eudia] Synchronous enrichment failed, will retry in background:",p),new u.Notice(`${a.length} accounts loaded! Contacts will populate shortly...`);let y=t,m=[5e3,2e4,6e4],h=async g=>{let f=m[g];if(f!==void 0){await new Promise(w=>setTimeout(w,f));try{await this.plugin.enrichAccountFolders(a),console.log(`[Eudia] Background enrichment retry ${g+1} succeeded`)}catch{return h(g+1)}}};h(0)}}else if(c==="admin"){if(console.log("[Eudia] Admin user detected - importing all accounts"),a=await this.accountOwnershipService.getAllAccountsForAdmin(t),a.length>0){e&&(e.textContent=`Creating ${a.length} account folders...`),await this.plugin.createAdminAccountFolders(a);let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(y=>y.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length,console.log(`[Eudia] Admin accounts verified: ${d.length} folders created`)):(console.warn("[Eudia] Admin folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let y=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");y&&await this.plugin.app.vault.delete(y)}catch{}new u.Notice(`Imported ${a.length} accounts! Enriching with Salesforce data...`);let p=a.filter(y=>y.id&&y.id.startsWith("001"));if(p.length>0){e&&(e.textContent=`Enriching ${p.length} accounts with Salesforce contacts...`);try{await this.plugin.enrichAccountFolders(p),new u.Notice(`${a.length} accounts loaded and enriched with Salesforce data!`),console.log(`[Eudia] Admin/exec synchronous enrichment complete: ${p.length} accounts`),e&&(e.textContent=`${a.length} accounts loaded and enriched!`)}catch(y){console.log("[Eudia] Admin/exec synchronous enrichment failed, will retry on next open:",y),new u.Notice(`${a.length} accounts imported! Contacts will populate on next vault open.`);let m=[5e3,2e4,6e4],h=async g=>{let f=m[g];if(f!==void 0){await new Promise(w=>setTimeout(w,f));try{await this.plugin.enrichAccountFolders(p),console.log(`[Eudia] Admin/exec background enrichment retry ${g+1} succeeded`)}catch{return h(g+1)}}};h(0)}}}}else{let l=await this.accountOwnershipService.getAccountsWithProspects(t);if(a=l.accounts,o=l.prospects,a.length>0||o.length>0){e&&(e.textContent=`Creating ${a.length} account folders...`),await this.plugin.createTailoredAccountFolders(a,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o);let p=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(m=>m.children!==void 0)||[];p.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length+o.length,console.log(`[Eudia] BL accounts verified: ${p.length} folders created`)):(console.warn("[Eudia] BL folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let m=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");m&&await this.plugin.app.vault.delete(m)}catch{}new u.Notice(`Imported ${a.length} active accounts + ${o.length} prospects!`);let y=[...a,...o];setTimeout(async()=>{try{await this.plugin.enrichAccountFolders(y)}catch(m){console.log("[Eudia] Background enrichment skipped:",m)}},500)}else console.warn(`[Eudia] No accounts returned for ${t} (userGroup: ${c})`),e&&(e.textContent=`No accounts found for ${t}. The server may still be starting \u2014 try again in 30 seconds.`,e.className="eudia-setup-validation-message warning"),new u.Notice("No accounts found. Server may be cold-starting \u2014 please try again shortly.")}}catch(a){console.error("[Eudia] Account import failed:",a),e&&(e.textContent="Account import failed. Please try again.",e.className="eudia-setup-validation-message error"),new u.Notice("Account import failed \u2014 please try again.")}finally{r&&!i&&r.expand()}await this.render()}else e&&(e.innerHTML=`<strong>${t}</strong> is not authorized for calendar access. Contact your admin.`,e.className="eudia-setup-validation-message error")}catch{e&&(e.textContent="Connection failed. Please try again.",e.className="eudia-setup-validation-message error")}}renderSalesforceStep(t){let e=this.steps[1],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),i=r.createDiv({cls:"eudia-setup-step-number"});i.setText(e.status==="complete"?"":"2"),e.status==="complete"&&i.addClass("eudia-step-complete");let a=r.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:e.title}),a.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(e.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let l=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),d=o.createDiv({cls:"eudia-setup-sf-status"});l.onclick=async()=>{let p=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(p,"_blank"),d.textContent="Complete the login in the popup window...",d.className="eudia-setup-sf-status loading",new u.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(d)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(t){this.pollInterval&&window.clearInterval(this.pollInterval);let e=0,n=60;this.pollInterval=window.setInterval(async()=>{e++;try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new u.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(t),await this.render()):e>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),t.textContent="Connection timed out. Please try again.",t.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(t){t.textContent="Importing your accounts...",t.className="eudia-setup-sf-status loading";try{let e=this.plugin.settings.userEmail,n=$(e)?"admin":L(e)?"cs":"bl";console.log(`[Eudia SF Import] Importing for ${e} (group: ${n})`);let r,i=[];if(n==="cs"){console.log("[Eudia SF Import] CS user SF Connect \u2014 fetching live data from Salesforce..."),t.textContent="Syncing with Salesforce for latest account data...";try{let m=await this.accountOwnershipService.getCSAccounts(e);r=m.accounts,i=m.prospects,console.log(`[Eudia SF Import] CS server sync: ${r.length} accounts (with real SF IDs + CSM data)`)}catch{if(this.plugin.settings.accountsImported){t.textContent="Salesforce connected! Account folders already loaded. Enrichment will retry later.",t.className="eudia-setup-sf-status success",this.steps[1].status="complete";return}r=[...k],console.log(`[Eudia SF Import] CS server unavailable \u2014 using ${r.length} static accounts`)}}else if(n==="admin")console.log("[Eudia] Admin user detected - importing all accounts"),t.textContent="Admin detected - importing all accounts...",r=await this.accountOwnershipService.getAllAccountsForAdmin(e);else{let m=await this.accountOwnershipService.getAccountsWithProspects(e);r=m.accounts,i=m.prospects}if(r.length===0&&i.length===0){t.textContent="No accounts found for your email. Contact your admin.",t.className="eudia-setup-sf-status warning";return}t.textContent=`Creating ${r.length} account folders...`;let a=this.plugin.app.workspace.leftSplit,o=a?.collapsed;if(a&&!o&&a.collapse(),$(e)?await this.plugin.createAdminAccountFolders(r):(await this.plugin.createTailoredAccountFolders(r,{}),i.length>0&&await this.plugin.createProspectAccountFiles(i)),H(e))try{await this.plugin.createCSManagerDashboard(e,r)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=r.length+i.length,await this.plugin.saveSettings();try{let m=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");m&&await this.plugin.app.vault.delete(m)}catch{}a&&!o&&a.expand(),this.steps[2].status="complete";let c=r.filter(m=>m.isOwned!==!1).length,l=r.filter(m=>m.isOwned===!1).length;n==="admin"&&l>0?t.textContent=`${c} owned + ${l} view-only accounts imported! Enriching...`:t.textContent=`${r.length} active + ${i.length} prospect accounts imported! Enriching...`,t.className="eudia-setup-sf-status success";let d=[...r,...i],p=e,y=n;setTimeout(async()=>{try{let m=d.filter(h=>h.id&&h.id.startsWith("001"));if(m.length>0?(t.textContent=`Enriching ${m.length} accounts with Salesforce data...`,await this.plugin.enrichAccountFolders(m),t.textContent=`${r.length} accounts imported, ${m.length} enriched with Salesforce data`):t.textContent=`${r.length} accounts imported (enrichment requires Salesforce IDs)`,y==="cs"&&H(p))try{console.log("[Eudia SF Import] Regenerating CS Manager dashboard with live CSM data..."),await this.plugin.createCSManagerDashboard(p,r),console.log("[Eudia SF Import] CS Manager dashboard updated with CSM assignments")}catch(h){console.error("[Eudia SF Import] Dashboard regeneration failed (non-blocking):",h)}}catch(m){console.log("[Eudia] Background enrichment skipped:",m),t.textContent=`${r.length+i.length} accounts imported (enrichment will retry on next launch)`}},500)}catch{t.textContent="Failed to import accounts. Please try again.",t.className="eudia-setup-sf-status error";let n=this.plugin.app.workspace.leftSplit;if(n?.collapsed===!1)try{n.expand()}catch{}}}renderTranscribeStep(t){let e=this.steps[2],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),r=n.createDiv({cls:"eudia-setup-step-header"}),i=r.createDiv({cls:"eudia-setup-step-number"});i.setText(e.status==="complete"?"":"3"),e.status==="complete"&&i.addClass("eudia-step-complete");let a=r.createDiv({cls:"eudia-setup-step-info"});a.createEl("h3",{text:e.title}),a.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),l=c.createDiv({cls:"eudia-setup-instruction"});l.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),l.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),d.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let p=c.createDiv({cls:"eudia-setup-instruction"});p.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),p.createSpan({text:"AI will summarize and extract key insights automatically"}),e.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(t){let e=t.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(i=>i.status==="complete")){let i=e.createDiv({cls:"eudia-setup-completion"});i.createEl("h2",{text:"\u{1F389} You're all set!"}),i.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let a=e.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});a.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(M),await this.plugin.activateCalendarView()}}else{let i=e.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});i.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(M),new u.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let r=e.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});r.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},ae=class extends u.ItemView{constructor(t,e){super(t);this.refreshInterval=null;this.lastError=null;this.plugin=e}getViewType(){return R}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let t=this.containerEl.children[1];if(t.empty(),t.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(t);return}this.renderHeader(t),await this.renderCalendarContent(t)}renderHeader(t){let e=t.createDiv({cls:"eudia-calendar-header"}),n=e.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let r=n.createDiv({cls:"eudia-calendar-actions"}),i=r.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});i.title="Refresh (fetches latest from calendar)",i.onclick=async()=>{i.addClass("spinning"),this._forceRefresh=!0,await this.render(),i.removeClass("spinning")};let a=r.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});a.title="Settings",a.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=e.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(t){let e={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,r=this.plugin.settings.userEmail;try{(await(0,u.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(e.server="connected",e.serverMessage="Server online"):(e.server="error",e.serverMessage="Server unavailable")}catch{e.server="error",e.serverMessage="Cannot reach server"}if(r&&e.server==="connected")try{let d=await(0,u.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(r)}`,method:"GET",throw:!1});d.status===200&&d.json?.authorized?(e.calendar="connected",e.calendarMessage="Calendar synced"):(e.calendar="not_authorized",e.calendarMessage="Not authorized")}catch{e.calendar="error",e.calendarMessage="Error checking access"}if(r&&e.server==="connected")try{let d=await(0,u.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(r)}`,method:"GET",throw:!1});d.status===200&&d.json?.connected?(e.salesforce="connected",e.salesforceMessage="Salesforce connected"):(e.salesforce="not_configured",e.salesforceMessage="Not connected")}catch{e.salesforce="not_configured"}let i=t.createDiv({cls:"eudia-status-indicators"}),a=i.createSpan({cls:`eudia-status-dot ${e.server}`});a.title=e.serverMessage||"Server";let o=i.createSpan({cls:`eudia-status-dot ${e.calendar}`});o.title=e.calendarMessage||"Calendar";let c=i.createSpan({cls:`eudia-status-dot ${e.salesforce}`});if(c.title=e.salesforceMessage||"Salesforce",t.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),e.calendar==="not_authorized"){let d=t.createDiv({cls:"eudia-status-warning"});d.innerHTML=`<strong>${r}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(t){let e=t.createDiv({cls:"eudia-calendar-content"}),n=e.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let r=new P(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),i=this._forceRefresh||!1;this._forceRefresh=!1;let a=await r.getWeekMeetings(i);if(n.remove(),!a.success){this.renderError(e,a.error||"Failed to load calendar");return}let o=Object.keys(a.byDay||{}).sort();if(o.length===0){this.renderEmptyState(e);return}await this.renderCurrentMeeting(e,r);for(let c of o){let l=a.byDay[c];!l||l.length===0||this.renderDaySection(e,c,l)}}catch(r){n.remove(),this.renderError(e,r.message||"Failed to load calendar")}}async renderCurrentMeeting(t,e){try{let n=await e.getCurrentMeeting();if(n.meeting){let r=t.createDiv({cls:"eudia-now-card"});n.isNow?r.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):r.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let i=r.createDiv({cls:"eudia-now-content"});i.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&i.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let a=r.createEl("button",{cls:"eudia-now-action",text:"Create Note"});a.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(t,e,n){let r=t.createDiv({cls:"eudia-calendar-day"});r.createEl("div",{cls:"eudia-calendar-day-header",text:P.getDayName(e)});for(let i of n){let a=r.createDiv({cls:`eudia-calendar-meeting ${i.isCustomerMeeting?"customer":"internal"}`});a.createEl("div",{cls:"eudia-calendar-time",text:P.formatTime(i.start,this.plugin.settings.timezone)});let o=a.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:i.subject}),i.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:i.accountName});else if(i.attendees&&i.attendees.length>0){let c=i.attendees.slice(0,2).map(l=>l.name||l.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}a.onclick=()=>this.createNoteForMeeting(i),a.title="Click to create meeting note"}}renderEmptyState(t){let e=t.createDiv({cls:"eudia-calendar-empty"});e.innerHTML=`
    ...
    `}renderError(t,e){let n=t.createDiv({cls:"eudia-calendar-error"}),r="",i="Unable to load calendar",a="";e.includes("not authorized")||e.includes("403")?(r="\u{1F512}",i="Calendar Access Required",a="Contact your admin to be added to the authorized users list."):e.includes("network")||e.includes("fetch")?(r="\u{1F4E1}",i="Connection Issue",a="Check your internet connection and try again."):(e.includes("server")||e.includes("500"))&&(r="\u{1F527}",i="Server Unavailable",a="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=e.createDiv({cls:"eudia-setup-input-group"}),r=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});r.addClass("eudia-setup-email");let i=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),a=e.createDiv({cls:"eudia-setup-status"});i.onclick=async()=>{let o=r.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){a.textContent="Please use your @eudia.com email",a.className="eudia-setup-status error";return}i.disabled=!0,i.textContent="Connecting...",a.textContent="";try{if(!(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){a.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,a.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),a.textContent="Connected",a.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let l=c.message||"Connection failed";l.includes("403")?a.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:a.textContent=l,a.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect"}},r.onkeydown=o=>{o.key==="Enter"&&i.click()},e.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(t){let e=t.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],r=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],i=e.filter(o=>!r.includes(o)&&o.length>1);if(i.length===0)return e[0]||"";if(i.length>1&&n.includes(i[0]))return i[1].charAt(0).toUpperCase()+i[1].slice(1);let a=i[i.length-1];return a.charAt(0).toUpperCase()+a.slice(1)}getExternalDomainsFromAttendees(t){if(!t||t.length===0)return[];let e=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,r=[];for(let i of t){if(!i.email)continue;let o=i.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let c=o[1];if(c.includes("eudia.com")||e.includes(c)||n.has(c))continue;n.add(c);let l=this.extractCompanyFromDomain(c);l.length>=2&&r.push({domain:c,company:l})}}return r}findBestAccountMatch(t,e,n){let r=this.plugin.settings.accountsFolder||"Accounts",i=this.app.vault.getAbstractFileByPath(r);if(!(i instanceof u.TFolder))return null;let a=[];for(let c of i.children)c instanceof u.TFolder&&a.push(c.name);if(a.length===0)return null;let o=[];for(let{domain:c,company:l}of t){let d=this.findAccountFolder(l),p=d?1:0;o.push({domain:c,company:l,folder:d,score:p})}if(o.sort((c,l)=>l.score-c.score),o.length>0&&o[0].folder){let c=o[0],l=c.folder.split("/").pop()||c.company;return console.log(`[Eudia Calendar] Best domain match: "${c.company}" from ${c.domain} -> ${c.folder}`),{folder:c.folder,accountName:l,source:"domain"}}if(e){let c=this.findAccountFolder(e);if(c){let l=c.split("/").pop()||e;return console.log(`[Eudia Calendar] Server account match: "${e}" -> ${c}`),{folder:c,accountName:l,source:"server"}}}if(n){let c=this.findAccountFolder(n);if(c){let l=c.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${c}`),{folder:c,accountName:l,source:"subject"}}}for(let{company:c}of t){let l=a.find(d=>{let p=d.toLowerCase(),y=c.toLowerCase();return p.includes(y)||y.includes(p)});if(l){let d=`${r}/${l}`;return console.log(`[Eudia Calendar] Partial domain match: "${c}" -> ${d}`),{folder:d,accountName:l,source:"domain-partial"}}}return null}extractAccountFromAttendees(t){let e=this.getExternalDomainsFromAttendees(t);if(e.length===0)return null;let n=e[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(t){if(!t)return null;let e=t.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(e){let r=(e[1]||e[2]||"").trim();if(r.toLowerCase()!=="eudia")return r}let n=t.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let i=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(i.toLowerCase()!=="eudia"&&i.length>0)return i}if(!t.toLowerCase().includes("eudia")){let r=t.match(/^([^-–|]+)/);if(r){let i=r[1].trim();if(i.length>2&&i.length<50)return i}}return null}findAccountFolder(t){if(!t)return null;let e=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(e);if(!(n instanceof u.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${e}" not found`),null;let r=t.toLowerCase().trim(),i=[];for(let p of n.children)p instanceof u.TFolder&&i.push(p.name);console.log(`[Eudia Calendar] Searching for "${r}" in ${i.length} folders`);let a=i.find(p=>p.toLowerCase()===r);if(a)return console.log(`[Eudia Calendar] Exact match found: ${a}`),`${e}/${a}`;let o=i.find(p=>p.toLowerCase().startsWith(r));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${e}/${o}`;let c=i.find(p=>r.startsWith(p.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${e}/${c}`;let l=i.find(p=>{let y=p.toLowerCase();return y.length>=3&&r.includes(y)});if(l)return console.log(`[Eudia Calendar] Search contains folder match: ${l}`),`${e}/${l}`;let d=i.find(p=>{let y=p.toLowerCase();return r.length>=3&&y.includes(r)});return d?(console.log(`[Eudia Calendar] Folder contains search match: ${d}`),`${e}/${d}`):(console.log(`[Eudia Calendar] No folder match found for "${r}"`),null)}async createNoteForMeeting(t){let e=t.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",r=$(n),i=(t.attendees||[]).map(v=>v.email).filter(Boolean),a=re(t.subject,i);if(r&&a.isPipelineMeeting&&a.confidence>=60){await this._createPipelineNote(t,e);return}let o=t.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),c=`${e} - ${o}.md`,l=null,d=t.accountName||null,p=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${t.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(t.attendees?.map(v=>v.email)||[])}`);let y=this.getExternalDomainsFromAttendees(t.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(y)}`);let m=this.extractAccountFromSubject(t.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${m||"none"}"`);let h=this.findBestAccountMatch(y,t.accountName,m||void 0);if(h&&(l=h.folder,d=h.accountName,console.log(`[Eudia Calendar] Best match (${h.source}): "${d}" -> ${l}`)),!l){let v=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(v)instanceof u.TFolder&&(l=v,console.log(`[Eudia Calendar] No match found, using Accounts root: ${l}`))}if(d){let v=this.plugin.settings.cachedAccounts.find(b=>b.name.toLowerCase()===d?.toLowerCase());v&&(p=v.id,d=v.name,console.log(`[Eudia Calendar] Matched to cached account: ${v.name} (${v.id})`))}let g=l?`${l}/${c}`:c,f=this.app.vault.getAbstractFileByPath(g);if(f instanceof u.TFile){await this.app.workspace.getLeaf().openFile(f);try{let v=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;v?.revealInFolder&&v.revealInFolder(f)}catch{}new u.Notice(`Opened existing note: ${c}`);return}let w=(t.attendees||[]).map(v=>v.name||v.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${t.subject}"
date: ${e}
attendees: [${w}]
account: "${d||""}"
...
`;try{let v=await this.app.vault.create(g,C);await this.app.workspace.getLeaf().openFile(v);try{let b=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;b?.revealInFolder&&b.revealInFolder(v)}catch{}new u.Notice(`Created: ${g}`)}catch(v){console.error("[Eudia Calendar] Failed to create note:",v),new u.Notice(`Could not create note: ${v.message||"Unknown error"}`)}}async _createPipelineNote(t,e){let n=new Date(e+"T00:00:00"),r=String(n.getMonth()+1).padStart(2,"0"),i=String(n.getDate()).padStart(2,"0"),a=String(n.getFullYear()).slice(-2),o=`${r}.${i}.${a}`,c=`Team Pipeline Meeting - ${o}.md`,l="Pipeline Meetings";this.app.vault.getAbstractFileByPath(l)||await this.app.vault.createFolder(l);let p=`${l}/${c}`,y=this.app.vault.getAbstractFileByPath(p);if(y instanceof u.TFile){await this.app.workspace.getLeaf().openFile(y);try{let g=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;g?.revealInFolder&&g.revealInFolder(y)}catch{}new u.Notice(`Opened existing: ${c}`);return}let m=(t.attendees||[]).map(g=>g.name||g.email?.split("@")[0]||"Unknown"),h=`---
...
`},{name:"Meeting Notes.md",content:w},{name:"Contacts.md",content:g},{name:"Intelligence.md",content:f},{name:"Next Steps.md",content:C}];for(let W of v){let I=`${p}/${W.name}`;await this.app.vault.create(I,W.content)}return console.log(`[Eudia] Created account folder with subnotes${h?" (enriched)":""}: ${d}`),!0}catch(m){return console.error(`[Eudia] Failed to create folder for ${d}:`,m),!1}},c=5;for(let l=0;l<t.length;l+=c){let d=t.slice(l,l+c),p=await Promise.allSettled(d.map(y=>o(y)));i+=p.filter(y=>y.status==="fulfilled"&&y.value===!0).length}i>0?(this.settings.cachedAccounts=t.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),new u.Notice(`Created ${i} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let r=e?`
...
---`,i=[];return e?.opportunities&&i.push(e.opportunities),e?.recentActivity&&i.push(e.recentActivity),i.length>0?`${r}
...
3. **Next Steps** are auto-extracted after transcription
4. Set \`sync_to_salesforce: true\` to sync to Salesforce
`}buildNextStepsContent(t,e,n){let r=n||new Date().toISOString().split("T")[0],i=e?`
...
*Previous next steps will be archived here.*
`}async fetchEnrichmentData(t){let e=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=t.filter(a=>a.id&&a.id.startsWith("001"));if(n.length===0)return{};let r={},i=20;console.log(`[Eudia Enrich] Fetching enrichment data for ${n.length} accounts`);for(let a=0;a<n.length;a+=i){let c=n.slice(a,a+i).map(l=>l.id);try{let l=await(0,u.requestUrl)({url:`${e}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:c,userEmail:this.settings.userEmail})});l.json?.success&&l.json?.enrichments&&Object.assign(r,l.json.enrichments)}catch(l){console.error(`[Eudia Enrich] Batch fetch failed (batch ${a/i+1}):`,l)}a+i<n.length&&await new Promise(l=>setTimeout(l,100))}return console.log(`[Eudia Enrich] Got enrichment data for ${Object.keys(r).length}/${n.length} accounts`),r}async createProspectAccountFiles(t){if(!t||t.length===0)return 0;let e=this.settings.accountsFolder||"Accounts",n=`${e}/_Prospects`;if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let i=0;for(let a of t){let o=a.name.replace(/[<>:"/\\|?*]/g,"_").trim(),c=`${n}/${o}`;if(this.app.vault.getAbstractFileByPath(c)instanceof u.TFolder)continue;let d=`${e}/${o}`;if(this.app.vault.getAbstractFileByPath(d)instanceof u.TFolder)continue;let y=`${n}/${o}.md`,m=this.app.vault.getAbstractFileByPath(y);if(m instanceof u.TFile)try{await this.app.vault.delete(m)}catch{}try{await this.app.vault.createFolder(c);let h=new Date().toISOString().split("T")[0],g=[{name:"Note 1.md",content:`---
account: "${a.name}"
account_id: "${a.id}"
type: meeting_note
tier: prospect
...
`,c=Object.keys(a).sort();for(let p of c){let y=a[p];o+=`### ${p} (${y.length} accounts)
...
|---------|------|-------|-----|
`;for(let p of e.slice(0,50))o+=`| ${p.name} | ${p.type||""} | ${p.ownerName||""} | ${p.csmName||""} |
`;o+=`
---
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof u.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let p of i){let y=p.split("@")[0].replace("."," ").replace(/\b\w/g,b=>b.toUpperCase()),m=p.split("@")[0].replace("."," ").toLowerCase(),h=m.split(" ")[0],g=m.split(" ").pop()||"",f=e.filter(b=>{let W=(b.csmName||"").toLowerCase();if(W&&(W.includes(h)||W.includes(g)))return!0;let I=(b.ownerName||"").toLowerCase();return I.includes(h)||I.includes(g)}),w=`---
rep: "${p}"
rep_name: "${y}"
role: cs_rep_summary
...
`;let C=`${n}/${y}.md`,v=this.app.vault.getAbstractFileByPath(C);v instanceof u.TFile?await this.app.vault.modify(v,w):await this.app.vault.create(C,w)}console.log(`[Eudia] Created CS Manager dashboard for ${t} with ${e.length} accounts across ${c.length} reps`)}async createAdminAccountFolders(t){let e=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),await this.ensurePipelineFolderExists();let r=0,i=0,a=new Date().toISOString().split("T")[0],o=async l=>{let d=l.name.replace(/[<>:"/\\|?*]/g,"_").trim(),p=`${e}/${d}`;if(this.app.vault.getAbstractFileByPath(p)instanceof u.TFolder)return!1;try{return await this.app.vault.createFolder(p),await this.createExecAccountSubnotes(p,l,a),l.isOwned?r++:i++,console.log(`[Eudia Admin] Created ${l.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,m),!1}},c=5;for(let l=0;l<t.length;l+=c){let d=t.slice(l,l+c);await Promise.allSettled(d.map(p=>o(p)))}this.settings.cachedAccounts=t.map(l=>({id:l.id,name:l.name})),await this.saveSettings(),r+i>0&&new u.Notice(`Created ${r} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(t,e,n){let r=e.ownerName||"Unknown",i=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let a of i){let o=`${t}/${a.name}`;await this.app.vault.create(o,a.content)}}async createFullAccountSubnotes(t,e,n){let r=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let i of r){let a=`${t}/${i.name}`;await this.app.vault.create(a,i.content)}}async ensurePipelineFolderExists(){let t="Pipeline",e=`${t}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),!this.app.vault.getAbstractFileByPath(e)){let a=`---
...
${g}
`;await this.app.vault.modify(a,f),console.log(`[Eudia] Updated Next Steps for ${t} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(r){console.error(`[Eudia] Failed to update Next Steps for ${t}:`,r)}}async regenerateNextStepsDashboard(){try{let e=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!e||!(e instanceof u.TFile)){await this.ensureNextStepsFolderExists();return}let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!n||!(n instanceof u.TFolder))return;let r=[];for(let c of n.children)if(c instanceof u.TFolder){let l=`${c.path}/Next Steps.md`,d=this.app.vault.getAbstractFileByPath(l);if(d instanceof u.TFile){let p=await this.app.vault.read(d),y=p.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),m=y?y[1]:"Unknown",h=p.split(`
`).filter(g=>g.match(/^- \[[ x]\]/)).slice(0,5);(h.length>0||m!=="Unknown")&&r.push({account:c.name,lastUpdated:m,nextSteps:h})}}r.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],a=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`;if(p)m+=p;else{let h=[t.painPoints,t.productInterest,t.nextSteps,t.actionItems].filter(Boolean).map(d).join(`
...
`):typeof g=="object"?JSON.stringify(g):String(g),r=n(t.title)||"Meeting Notes",i=n(t.summary),a=n(t.painPoints),o=n(t.productInterest),c=n(t.meddiccSignals),l=n(t.nextSteps),d=n(t.actionItems),p=n(t.keyDates),y=n(t.risksObjections),m=n(t.attendees),h=`---
title: "${r}"
...
`),p&&!p.includes("No specific dates")&&(h+=`## Key Dates
...
`;try{await this.app.vault.create(c,l)}catch{}e++,console.log(`[Eudia] Archived: ${i.name}`)}catch(a){console.error(`[Eudia] Failed to archive ${i.name}:`,a)}return e}async syncSpecificNoteToSalesforce(t){let e=await this.app.vault.read(t),n=this.app.metadataCache.getFileCache(t)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let r=n.account_id,i=n.account;if(!r&&i){let a=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===i.toLowerCase());a&&(r=a.id)}if(!r){let a=t.path.split("/");if(a.length>=2&&a[0]===this.settings.accountsFolder){let o=a[1]==="_Prospects"&&a.length>=3?a[2]:a[1],c=this.settings.cachedAccounts.find(l=>l.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&(r=c.id,i=c.name)}}if(!r)return{success:!1,error:`Could not determine account for ${t.path}`};try{let a=await(0,u.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:r,accountName:i,noteTitle:t.basename,notePath:t.path,content:e,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return a.json?.success?{success:!0}:{success:!1,error:a.json?.error||"Unknown error",authRequired:a.json?.authRequired}}catch(a){return{success:!1,error:a.message}}}async syncNoteToSalesforce(){let t=this.app.workspace.getActiveFile();if(!t){new u.Notice("No active file to sync");return}if(!this.app.metadataCache.getFileCache(t)?.frontmatter?.sync_to_salesforce){new u.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}new u.Notice("Syncing to Salesforce...");let n=await this.syncSpecificNoteToSalesforce(t);n.success?new u.Notice("Synced to Salesforce"):n.authRequired?new u.Notice("Salesforce authentication required. Please reconnect."):new u.Notice("Failed to sync: "+(n.error||"Unknown error"))}async checkAndAutoEnrich(){let t=this.settings.accountsFolder||"Accounts",e=this.app.vault.getAbstractFileByPath(t);if(!e||!(e instanceof u.TFolder))return;let n=[];for(let r of e.children){if(!(r instanceof u.TFolder)||r.name.startsWith("_"))continue;let i=`${r.path}/Contacts.md`,a=this.app.vault.getAbstractFileByPath(i);if(!(!a||!(a instanceof u.TFile))){if(this.app.metadataCache.getFileCache(a)?.frontmatter?.enriched_at)continue}let o=r.name,c=this.settings.cachedAccounts.find(l=>l.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&c.id&&n.push({id:c.id,name:c.name,owner:"",ownerEmail:""})}if(n.length===0){console.log("[Eudia] Auto-enrich: all account folders already enriched");return}console.log(`[Eudia] Auto-enrich: ${n.length} accounts need enrichment`);try{await this.enrichAccountFolders(n)}catch(r){console.error("[Eudia] Auto-enrich failed:",r)}}async enrichAccountFolders(t){if(!t||t.length===0)return;let e=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=this.settings.accountsFolder||"Accounts",r=t.filter(d=>d.id&&d.id.startsWith("001"));if(r.length===0)return;let i=r.length,a=0,o=0;console.log(`[Eudia Enrich] Starting enrichment for ${i} accounts`),new u.Notice(`Enriching account data: 0/${i}...`);let c=20;for(let d=0;d<r.length;d+=c){let p=r.slice(d,d+c),y=p.map(h=>h.id);try{let h=await(0,u.requestUrl)({url:`${e}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:y,userEmail:this.settings.userEmail})});if(h.json?.success&&h.json?.enrichments){let g=h.json.enrichments;for(let f of p){let w=g[f.id];if(w)try{await this.writeEnrichmentToAccount(f,w,n),a++}catch(C){o++,console.error(`[Eudia Enrich] Write failed for ${f.name}:`,C)}}}}catch(h){o+=p.length,console.error("[Eudia Enrich] Batch fetch failed:",h)}let m=Math.min(d+c,i);new u.Notice(`Enriching account data: ${m}/${i}...`),d+c<r.length&&await new Promise(h=>setTimeout(h,100))}let l=o>0?`Enrichment complete: ${a} enriched, ${o} skipped`:`Enrichment complete: ${a} accounts enriched with Salesforce data`;console.log(`[Eudia Enrich] ${l}`),new u.Notice(l)}async writeEnrichmentToAccount(t,e,n){let r=t.name.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${n}/${r}`,a=this.app.vault.getAbstractFileByPath(i);if(a instanceof u.TFolder||(i=`${n}/_Prospects/${r}`,a=this.app.vault.getAbstractFileByPath(i)),!(a instanceof u.TFolder))return;let o=new Date().toISOString(),c=async(l,d)=>{let p=`${i}/${l}`,y=this.app.vault.getAbstractFileByPath(p);if(!(y instanceof u.TFile))return;let m=await this.app.vault.read(y),h="",g=m;if(m.startsWith("---")){let v=m.indexOf("---",3);v!==-1&&(h=m.substring(0,v+3),g=m.substring(v+3),h.includes("enriched_at:")?h=h.replace(/enriched_at:.*/,`enriched_at: "${o}"`):h=h.substring(0,v)+`enriched_at: "${o}"
---`)}let f=g.match(/^(\s*#[^\n]+)/),C=`${f?f[1]:""}
...
*Add org chart, decision makers, champions, and blockers here.*`),e.intelligence&&await c("Intelligence.md",e.intelligence),e.nextSteps&&await c("Next Steps.md",e.nextSteps),e.opportunities||e.recentActivity){let l=`${i}/Meeting Notes.md`,d=this.app.vault.getAbstractFileByPath(l);if(d instanceof u.TFile){let p=await this.app.vault.read(d),y="",m=p;if(p.startsWith("---")){let w=p.indexOf("---",3);w!==-1&&(y=p.substring(0,w+3),m=p.substring(w+3),y.includes("enriched_at:")?y=y.replace(/enriched_at:.*/,`enriched_at: "${o}"`):y=y.substring(0,w)+`enriched_at: "${o}"
---`)}let h=m.match(/^(\s*#[^\n]+)/),f=[h?h[1]:`
...
${f.join(`
`)}
`)}}}startSalesforceSyncScanner(){if(!this.settings.sfAutoSyncEnabled){console.log("[Eudia SF Sync] Auto-sync is disabled in settings"),this.updateSfSyncStatusBar("SF Sync: Off");return}let t=(this.settings.sfAutoSyncIntervalMinutes||15)*60*1e3;console.log(`[Eudia SF Sync] Starting scanner \u2014 interval: ${this.settings.sfAutoSyncIntervalMinutes}min`),this.updateSfSyncStatusBar("SF Sync: Idle");let e=window.setTimeout(()=>{this.runSalesforceSyncScan()},3e4);this.registerInterval(e),this.sfSyncIntervalId=window.setInterval(()=>{this.runSalesforceSyncScan()},t),this.registerInterval(this.sfSyncIntervalId)}async runSalesforceSyncScan(){if(!(!this.settings.sfAutoSyncEnabled||!this.settings.userEmail)){console.log("[Eudia SF Sync] Running scan...");try{let t=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!(t instanceof u.TFolder)){console.log("[Eudia SF Sync] Accounts folder not found");return}let e=[],n=d=>{for(let p of d.children)p instanceof u.TFile&&p.extension==="md"?e.push(p):p instanceof u.TFolder&&n(p)};n(t);let r=[];for(let d of e){let y=this.app.metadataCache.getFileCache(d)?.frontmatter;if(!y?.sync_to_salesforce)continue;let m=y.last_sf_sync?new Date(y.last_sf_sync).getTime():0;d.stat.mtime>m&&r.push(d)}if(r.length===0){console.log("[Eudia SF Sync] No flagged notes need syncing"),this.updateSfSyncStatusBar("SF Sync: Idle");return}console.log(`[Eudia SF Sync] ${r.length} note(s) queued for sync`),this.updateSfSyncStatusBar(`SF Sync: Syncing ${r.length}...`);let i=0,a=0;for(let d of r){let p=await this.syncSpecificNoteToSalesforce(d);if(p.success)i++,await this.updateNoteSyncTimestamp(d);else if(a++,console.log(`[Eudia SF Sync] Failed to sync ${d.path}: ${p.error}`),p.authRequired){new u.Notice("Salesforce authentication expired. Please reconnect to resume auto-sync."),this.updateSfSyncStatusBar("SF Sync: Auth required");return}}let c=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),l=a>0?`SF Sync: ${i} synced, ${a} failed at ${c}`:`SF Sync: ${i} note${i!==1?"s":""} synced at ${c}`;console.log(`[Eudia SF Sync] ${l}`),this.updateSfSyncStatusBar(l),i>0&&new u.Notice(l)}catch(t){console.error("[Eudia SF Sync] Scan error:",t),this.updateSfSyncStatusBar("SF Sync: Error")}}}async updateNoteSyncTimestamp(t){try{let e=await this.app.vault.read(t),n=new Date().toISOString();if(e.startsWith("---")){let r=e.indexOf("---",3);if(r!==-1){let i=e.substring(0,r),a=e.substring(r);if(i.includes("last_sf_sync:")){let o=i.replace(/last_sf_sync:.*/,`last_sf_sync: "${n}"`)+a;await this.app.vault.modify(t,o)}else{let o=i+`last_sf_sync: "${n}"
`+a;await this.app.vault.modify(t,o)}}}}catch(e){console.error(`[Eudia SF Sync] Failed to update sync timestamp for ${t.path}:`,e)}}updateSfSyncStatusBar(t){this.sfSyncStatusBarEl&&this.sfSyncStatusBarEl.setText(t)}async createMeetingNote(){return new Promise(t=>{new te(this.app,this,async n=>{if(!n){t();return}let r=new Date().toISOString().split("T")[0],i=n.name.replace(/[<>:"/\\|?*]/g,"_").trim(),a=`${this.settings.accountsFolder}/${i}`,o=`${r} Meeting.md`,c=`${a}/${o}`;this.app.vault.getAbstractFileByPath(a)||await this.app.vault.createFolder(a);let l=`---
...
`,d=await this.app.vault.create(c,l);await this.app.workspace.getLeaf().openFile(d),new u.Notice(`Created meeting note for ${n.name}`),t()}).open()})}async fetchAndInsertContext(){new u.Notice("Fetching pre-call context...")}async refreshAnalyticsDashboard(t){if(!this.settings.userEmail){console.log("[Eudia] Cannot refresh analytics - no email configured");return}let n=(await this.app.vault.read(t)).match(/^---\n([\s\S]*?)\n---/);if(!n)return;let r=n[1];if(!r.includes("type: analytics_dashboard"))return;let i=r.match(/category:\s*(\w+)/)?.[1]||"team";console.log(`[Eudia] Refreshing analytics dashboard: ${t.name} (${i})`);try{let a=null,o=this.settings.serverUrl,c=encodeURIComponent(this.settings.userEmail);switch(i){case"pain_points":a=(await(0,u.requestUrl)({url:`${o}/api/analytics/pain-points?days=30`,method:"GET"})).json,a.success&&await this.updatePainPointNote(t,a.painPoints);break;case"objections":a=(await(0,u.requestUrl)({url:`${o}/api/analytics/objection-playbook?days=90`,method:"GET"})).json,a.success&&await this.updateObjectionNote(t,a);break;case"coaching":case"team":default:a=(await(0,u.requestUrl)({url:`${o}/api/analytics/team-trends?managerId=${c}`,method:"GET"})).json,a.success&&await this.updateTeamPerformanceNote(t,a.trends);break}a?.success&&new u.Notice(`Analytics refreshed: ${t.name}`)}catch(a){console.error("[Eudia] Analytics refresh error:",a)}}async updatePainPointNote(t,e){if(!e||e.length===0)return;let n=new Date().toISOString().split("T")[0],r=e.slice(0,10).map(l=>`| ${l.painPoint||"--"} | ${l.count||0} | ${l.category||"--"} | ${l.averageSeverity||"medium"} |`).join(`
`),i={};for(let l of e){let d=l.category||"other";i[d]||(i[d]=[]),i[d].push(l)}let a="";for(let[l,d]of Object.entries(i)){a+=`
### ${l.charAt(0).toUpperCase()+l.slice(1)}
`;for(let p of d.slice(0,3))a+=`- ${p.painPoint}
...
`;await this.app.vault.modify(t,a)}async updateTeamPerformanceNote(t,e){if(!e)return;let n=new Date().toISOString().split("T")[0],r=a=>a>0?`\u2191 ${Math.abs(a).toFixed(1)}%`:a<0?`\u2193 ${Math.abs(a).toFixed(1)}%`:"--",i=`---
type: analytics_dashboard
auto_refresh: true
...
`;await this.app.vault.modify(t,i)}},ie=class extends u.PluginSettingTab{constructor(s,t){super(s,t),this.plugin=t}display(){let{containerEl:s}=this;s.empty(),s.createEl("h2",{text:"Eudia Sync & Scribe"}),s.createEl("h3",{text:"Your Profile"});let t=s.createDiv();t.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let e=t.createDiv();e.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let n=e.createSpan(),r=e.createSpan(),i=t.createDiv();i.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",i.setText("Connect with Salesforce to sync notes with your user attribution.");let a=t.createEl("button");a.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,c=async()=>{if(!this.plugin.settings.userEmail)return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",r.setText("Enter email above first"),a.setText("Setup Required"),a.disabled=!0,a.style.opacity="0.5",a.style.cursor="not-allowed",!1;a.disabled=!1,a.style.opacity="1",a.style.cursor="pointer";try{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",r.setText("Checking..."),(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",r.setText("Connected to Salesforce"),a.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",r.setText("Not connected"),a.setText("Connect to Salesforce"),!1)}catch{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",r.setText("Status unavailable"),a.setText("Connect to Salesforce"),!1}};new u.Setting(s).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(m=>m.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async h=>{let g=h.trim().toLowerCase();this.plugin.settings.userEmail=g,await this.plugin.saveSettings(),await c()})),new u.Setting(s).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(m=>{me.forEach(h=>{m.addOption(h.value,h.label)}),m.setValue(this.plugin.settings.timezone),m.onChange(async h=>{this.plugin.settings.timezone=h,await this.plugin.saveSettings(),this.plugin.calendarService?.setTimezone(h),new u.Notice(`Timezone set to ${me.find(g=>g.value===h)?.label||h}`)})}),s.createEl("h3",{text:"Salesforce Connection"}),s.appendChild(t);let l=()=>{o&&window.clearInterval(o);let m=0,h=30;o=window.setInterval(async()=>{m++,await c()?(o&&(window.clearInterval(o),o=null),new u.Notice("Salesforce connected successfully!")):m>=h&&o&&(window.clearInterval(o),o=null)},5e3)};a.onclick=async()=>{if(!this.plugin.settings.userEmail){new u.Notice("Please enter your email first");return}let m=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(m,"_blank"),new u.Notice("Complete the Salesforce login in the popup window",5e3),l()},c(),s.createEl("h3",{text:"Server"}),new u.Setting(s).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(m=>m.setValue(this.plugin.settings.serverUrl).onChange(async h=>{this.plugin.settings.serverUrl=h,await this.plugin.saveSettings()}));let d=s.createDiv({cls:"settings-advanced-collapsed"}),p=d.createDiv({cls:"eudia-transcription-status"});p.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",p.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,u.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?p.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':p.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{p.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let y=new u.Setting(s).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(m=>m.setValue(!1).onChange(h=>{d.style.display=h?"block":"none"}));d.style.display="none",s.createEl("h3",{text:"Transcription"}),new u.Setting(s).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(m=>m.setValue(this.plugin.settings.saveAudioFiles).onChange(async h=>{this.plugin.settings.saveAudioFiles=h,await this.plugin.saveSettings()})),new u.Setting(s).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(m=>m.setValue(this.plugin.settings.appendTranscript).onChange(async h=>{this.plugin.settings.appendTranscript=h,await this.plugin.saveSettings()})),s.createEl("h3",{text:"Sync"}),new u.Setting(s).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(m=>m.setValue(this.plugin.settings.syncOnStartup).onChange(async h=>{this.plugin.settings.syncOnStartup=h,await this.plugin.saveSettings()})),new u.Setting(s).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(m=>m.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async h=>{this.plugin.settings.autoSyncAfterTranscription=h,await this.plugin.saveSettings()})),new u.Setting(s).setName("Auto-Sync Flagged Notes").setDesc("Periodically push notes with sync_to_salesforce: true to Salesforce").addToggle(m=>m.setValue(this.plugin.settings.sfAutoSyncEnabled).onChange(async h=>{this.plugin.settings.sfAutoSyncEnabled=h,await this.plugin.saveSettings(),h?this.plugin.startSalesforceSyncScanner():this.plugin.updateSfSyncStatusBar("SF Sync: Off")})),new u.Setting(s).setName("Auto-Sync Interval").setDesc("How often to scan for flagged notes (in minutes)").addDropdown(m=>{m.addOption("5","Every 5 minutes"),m.addOption("15","Every 15 minutes"),m.addOption("30","Every 30 minutes"),m.setValue(String(this.plugin.settings.sfAutoSyncIntervalMinutes)),m.onChange(async h=>{this.plugin.settings.sfAutoSyncIntervalMinutes=parseInt(h),await this.plugin.saveSettings(),new u.Notice(`SF auto-sync interval set to ${h} minutes. Restart Obsidian for changes to take effect.`)})}),s.createEl("h3",{text:"Folders"}),new u.Setting(s).setName("Accounts Folder").setDesc("Where account folders are stored").addText(m=>m.setValue(this.plugin.settings.accountsFolder).onChange(async h=>{this.plugin.settings.accountsFolder=h||"Accounts",await this.plugin.saveSettings()})),new u.Setting(s).setName("Recordings Folder").setDesc("Where audio files are saved").addText(m=>m.setValue(this.plugin.settings.recordingsFolder).onChange(async h=>{this.plugin.settings.recordingsFolder=h||"Recordings",await this.plugin.saveSettings()})),s.createEl("h3",{text:"Actions"}),new u.Setting(s).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(m=>m.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new u.Setting(s).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(m=>m.setButtonText("Refresh Folders").onClick(async()=>{m.setButtonText("Checking..."),m.setDisabled(!0);try{let h=await this.plugin.refreshAccountFolders();h>0?new u.Notice(`Created ${h} new account folder${h>1?"s":""}`):new u.Notice("All account folders are up to date")}catch(h){new u.Notice("Failed to refresh folders: "+h.message)}m.setButtonText("Refresh Folders"),m.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&s.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),s.createEl("p",{text:`Audio transcription: ${T.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
