...
- Action items have clear owners`}function Ie(b,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
...
`}function We(b,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${r.account.name}
${r.account.owner?`- Owner: ${r.account.owner}`:""}
...
`}function je(b,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${r.account.name}
${r.account.owner?`- Owner: ${r.account.owner}`:""}
...
6. For strategic takeaways, only include implications that were actually discussed or clearly implied \u2014 do not speculate.`}function Pe(b){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${b?`
...
`;let a=e.talkTime.repPercent,s=e.talkTime.customerPercent,o=e.talkTime.isHealthyRatio?"\u2705":"\u26A0\uFE0F";n+=`**Talk Time:** Rep ${a}% / Customer ${s}% ${o}
`;let l=Math.round(a/5),c=Math.round(s/5);if(n+=`\`${"\u2588".repeat(l)}${"\u2591".repeat(20-l)}\` Rep
...
`);let i=e?.enabled&&e?.formattedTranscript?e.formattedTranscript:t;if(i){let a=e?.enabled?"Full Transcript (Speaker-Attributed)":"Full Transcript";n+=`---
...
`}return n}static formatSectionsWithAudio(r,t,e,n){let i=this.formatSectionsForNote(r,t,n);return e&&(i+=`
---

## Recording

![[${e}]]
`),i}static formatContextForNote(r){if(!r.success)return"";let t=`## Pre-Call Context
...
`,t}async blobToBase64(r){return new Promise((t,e)=>{let n=new FileReader;n.onload=()=>{let a=n.result.split(",")[1];t(a)},n.onerror=e,n.readAsDataURL(r)})}async transcribeAudio(r,t){let e=r.size/1024/1024,n=r.type||"audio/webm";if(e>8)return console.log(`[Eudia] Large recording (${e.toFixed(1)}MB) \u2014 using chunked transcription`),this.transcribeAudioChunked(r,n,t);try{let i=await this.blobToBase64(r),a=t?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:t.pipelineContext}:void 0,s=await this.transcribeAndSummarize(i,n,t?.accountName,t?.accountId,a,{captureMode:t?.captureMode,hasVirtualDevice:t?.hasVirtualDevice},t?.meetingTemplate);return{text:s.transcript,confidence:s.success?.95:0,duration:s.duration,sections:s.sections,diarizedTranscript:s.diarizedTranscript,error:s.error}}catch(i){return console.error("transcribeAudio error:",i),{text:"",confidence:0,duration:0,sections:this.getEmptySections(),error:i.message||"Transcription request failed"}}}static{this.CHUNK_MAX_RETRIES=3}static{this.CHUNK_RETRY_DELAYS=[1e4,3e4,6e4]}async transcribeChunkWithRetry(r,t,e,n){for(let i=0;i<=b.CHUNK_MAX_RETRIES;i++){if(i>0){let a=b.CHUNK_RETRY_DELAYS[i-1]||3e4;console.log(`[Eudia] Chunk ${e+1}/${n} retry ${i}/${b.CHUNK_MAX_RETRIES} in ${a/1e3}s...`),await new Promise(s=>setTimeout(s,a))}try{let s=(0,F.requestUrl)({url:`${this.serverUrl}/api/transcribe-chunk`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({audio:r,mimeType:t})}),o=new Promise((d,u)=>setTimeout(()=>u(new Error(`Chunk request timed out after ${18e4/1e3}s`)),18e4)),l=await Promise.race([s,o]),c=l.json?.text||l.json?.transcript||"";if(l.json?.success&&c)return i>0&&console.log(`[Eudia] Chunk ${e+1}/${n} succeeded on retry ${i}`),{text:c,duration:l.json.duration||0};console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${i+1} returned no text: ${l.json?.error||"unknown"}`)}catch(a){console.warn(`[Eudia] Chunk ${e+1}/${n} attempt ${i+1} failed: ${a.message}`)}}return null}async transcribeAudioChunked(r,t,e){let i=await r.arrayBuffer(),a=i.byteLength,s=Math.ceil(a/4194304),o=3;console.log(`[Eudia] Chunked transcription: ${(a/1024/1024).toFixed(1)}MB \u2192 ${s} chunks (batches of ${o})`);let l=16*1024,c=a/l,d=new Array(s),u=0,h=0;for(let O=0;O<s;O+=o){let S=Math.min(O+o,s);console.log(`[Eudia] Processing batch ${Math.floor(O/o)+1}/${Math.ceil(s/o)} (chunks ${O+1}-${S})`);let C=[];for(let g=O;g<S;g++){let f=g*4194304,x=Math.min(f+4194304,a),E=i.slice(f,x),k=new Blob([E],{type:t});console.log(`[Eudia] Queuing chunk ${g+1}/${s} (${((x-f)/1024/1024).toFixed(1)}MB)`),C.push((async()=>{try{let W=await this.blobToBase64(k),A=await this.transcribeChunkWithRetry(W,t,g,s);return{index:g,result:A}}catch(W){return console.error(`[Eudia] Chunk ${g+1}/${s} unexpected error: ${W.message}`),{index:g,result:null}}})())}let v=await Promise.allSettled(C);for(let g of v)if(g.status==="fulfilled"){let{index:f,result:x}=g.value;if(x)d[f]=x.text,u+=x.duration,console.log(`[Eudia] Chunk ${f+1}/${s} OK: ${x.text.length} chars`);else{h++;let E=f*4194304,k=Math.min(E+4194304,a),W=Math.round(E/a*c),A=Math.round(k/a*c),T=`${Math.floor(W/60)}:${(W%60).toString().padStart(2,"0")}`,j=`${Math.floor(A/60)}:${(A%60).toString().padStart(2,"0")}`;d[f]=`

[~${T} \u2013 ${j} \u2014 audio not transcribed (chunk ${f+1}/${s} failed after ${b.CHUNK_MAX_RETRIES+1} attempts)]

`,console.error(`[Eudia] Chunk ${f+1}/${s} permanently failed \u2014 gap marker inserted`)}}else h++,console.error(`[Eudia] Chunk in batch rejected: ${g.reason}`)}let m=d.filter(O=>O!=null);if(m.filter(O=>!O.includes("\u2014 audio not transcribed")).length===0)return{text:"",confidence:0,duration:0,sections:this.getEmptySections(),error:`All ${s} chunks failed to transcribe after retries. Server may be unavailable.`};h>0&&console.warn(`[Eudia] ${h}/${s} chunks failed after retries \u2014 partial transcript with gap markers`);let w=m.join(`

`);console.log(`[Eudia] Combined transcript: ${w.length} chars from ${s} chunks (${h} gaps)`);try{let O=await this.processTranscription(w,{accountName:e?.accountName,accountId:e?.accountId});return{text:w,confidence:h===0?.9:Math.max(.3,.9-h/s*.6),duration:u,sections:O,...h>0?{error:`${h} of ${s} audio chunks could not be transcribed. Look for [audio not transcribed] markers in the transcript.`}:{}}}catch(O){return console.error("[Eudia] Summarization failed after chunked transcription:",O.message),{text:w,confidence:.5,duration:u,sections:this.getEmptySections(),error:`Transcription succeeded but summarization failed: ${O.message}`}}}async processTranscription(r,t){if(!r||r.trim().length===0)return this.getEmptySections();try{let e=await(0,F.requestUrl)({url:`${this.serverUrl}/api/process-sections`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:r,accountName:t?.accountName,context:t})});if(e.json?.success&&e.json?.sections){let n=e.json.sections;return{summary:n.summary||"",painPoints:n.painPoints||n.keyPoints||"",productInterest:n.productInterest||"",meddiccSignals:n.meddiccSignals||"",nextSteps:n.nextSteps||"",actionItems:n.actionItems||"",keyDates:n.keyDates||"",buyingTriggers:n.buyingTriggers||"",dealSignals:n.dealSignals||"",risksObjections:n.risksObjections||n.concerns||"",competitiveIntel:n.competitiveIntel||"",attendees:n.attendees||"",transcript:r}}return console.warn("Server process-sections returned no sections, using fallback"),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:r}}catch(e){return console.error("processTranscription server error:",e),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:r}}}};var Y=require("obsidian"),M=class b{constructor(r,t,e="America/New_York"){this.serverUrl=r,this.userEmail=t.toLowerCase(),this.timezone=e}setUserEmail(r){this.userEmail=r.toLowerCase()}setServerUrl(r){this.serverUrl=r}setTimezone(r){this.timezone=r}async getTodaysMeetings(r=!1){if(!this.userEmail)return{success:!1,date:new Date().toISOString().split("T")[0],email:"",meetingCount:0,meetings:[],error:"User email not configured"};try{let t=encodeURIComponent(this.timezone),e=r?"&forceRefresh=true":"";return(await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/today?timezone=${t}${e}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(t){return console.error("Failed to fetch today's meetings:",t),{success:!1,date:new Date().toISOString().split("T")[0],email:this.userEmail,meetingCount:0,meetings:[],error:t.message||"Failed to fetch calendar"}}}async getWeekMeetings(r=!1){if(!this.userEmail)return{success:!1,startDate:"",endDate:"",email:"",totalMeetings:0,byDay:{},error:"User email not configured"};try{let t=encodeURIComponent(this.timezone),e=r?"&forceRefresh=true":"";return(await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/week?timezone=${t}${e}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(t){return console.error("Failed to fetch week's meetings:",t),{success:!1,startDate:"",endDate:"",email:this.userEmail,totalMeetings:0,byDay:{},error:t.message||"Failed to fetch calendar"}}}async getMeetingsInRange(r,t){if(!this.userEmail)return[];try{let e=r.toISOString().split("T")[0],n=t.toISOString().split("T")[0],i=await(0,Y.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/range?start=${e}&end=${n}`,method:"GET",headers:{Accept:"application/json"}});return i.json.success?i.json.meetings||[]:[]}catch(e){return console.error("Failed to fetch calendar range:",e),[]}}async getCurrentMeeting(){let r=await this.getTodaysMeetings();if(!r.success||r.meetings.length===0)return{meeting:null,isNow:!1};let t=new Date;for(let e of r.meetings){let n=b.safeParseDate(e.start),i=b.safeParseDate(e.end);if(t>=n&&t<=i)return{meeting:e,isNow:!0};let a=(n.getTime()-t.getTime())/(1e3*60);if(a>0&&a<=15)return{meeting:e,isNow:!1,minutesUntilStart:Math.ceil(a)}}return{meeting:null,isNow:!1}}async getMeetingsForAccount(r){let t=await this.getWeekMeetings();if(!t.success)return[];let e=[];Object.values(t.byDay).forEach(i=>{e.push(...i)});let n=r.toLowerCase();return e.filter(i=>i.accountName?.toLowerCase().includes(n)||i.subject.toLowerCase().includes(n)||i.attendees.some(a=>a.email.toLowerCase().includes(n.split(" ")[0])))}static formatMeetingForNote(r){let t=r.attendees.filter(e=>e.isExternal!==!1).map(e=>e.name||e.email.split("@")[0]).slice(0,5).join(", ");return{title:r.subject,attendees:t,meetingStart:r.start,accountName:r.accountName}}static getDayName(r){let t;r.length===10&&r.includes("-")?t=new Date(r+"T00:00:00"):t=new Date(r);let e=new Date;e.setHours(0,0,0,0);let n=new Date(t);n.setHours(0,0,0,0);let i=Math.round((n.getTime()-e.getTime())/(1e3*60*60*24));return i===0?"Today":i===1?"Tomorrow":t.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}static formatTime(r,t){let e=r;e&&!e.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(e)&&(e=e+"Z");let n=new Date(e);if(isNaN(n.getTime()))return r;let i={hour:"numeric",minute:"2-digit",hour12:!0};return t&&(i.timeZone=t),n.toLocaleTimeString("en-US",i)}static safeParseDate(r){if(!r)return new Date(NaN);let t=r;return!t.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(t)&&(t=t+"Z"),new Date(t)}static getMeetingDuration(r,t){let e=b.safeParseDate(r),n=b.safeParseDate(t);return Math.round((n.getTime()-e.getTime())/(1e3*60))}};var Ne=["ai-contracting-tech","ai-contracting-services","ai-compliance-tech","ai-compliance-services","ai-ma-tech","ai-ma-services","sigma"],Fe=["metrics-identified","economic-buyer-identified","decision-criteria-discussed","decision-process-discussed","pain-confirmed","champion-identified","competition-mentioned"],De=["progressing","stalled","at-risk","champion-engaged","early-stage"],Me=["discovery","demo","negotiation","qbr","implementation","follow-up"],Re=`You are a sales intelligence tagger for Eudia, an AI legal technology company. Extract structured tags from meeting analysis.
...
`),e=e.replace(/^#{1,3}\s+.+\n+(?=#{1,3}\s|\s*$)/gm,""),e=e.replace(/^#{2,3}\s+(.+)$/gm,'</p><h3 class="eudia-intel-header">$1</h3><p>'),e=e.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^-\s+\[\s*\]\s+(.+)$/gm,'<li class="eudia-intel-todo">$1</li>'),e=e.replace(/^-\s+\[x\]\s+(.+)$/gm,'<li class="eudia-intel-done">$1</li>'),e=e.replace(/^[•\-]\s+(.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li[^>]*>.*?<\/li>\s*)+/g,'<ul class="eudia-intel-list">$&</ul>'),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=e.replace(/<p>\s*(<ul)/g,"$1"),e=e.replace(/<\/ul>\s*<\/p>/g,"</ul>"),e=e.replace(/<p>\s*(<h3)/g,"$1"),e=e.replace(/<\/h3>\s*<\/p>/g,"</h3>"),e=e.replace(/<\/li>\s*<br>\s*<li/g,"</li><li"),e=e.replace(/<p>\s*<\/p>/g,""),e=e.replace(/<p>\s*<br>\s*<\/p>/g,""),e=e.replace(/(<br>\s*){2,}/g,""),e=e.replace(/<\/h3>\s*<br>/g,"</h3>"),e=e.replace(/<br>\s*<h3/g,"<h3"),e=e.replace(/<br>\s*<ul/g,"<ul"),e=e.replace(/<\/ul>\s*<br>/g,"</ul>"),e=e.replace(/^(<br>)+|(<br>)+$/g,""),e="<p>"+e+"</p>",e=e.replace(/<p><\/p>/g,""),e}onClose(){this.contentEl.empty()}};var se=class extends p.ItemView{constructor(t,e){super(t);this.emailInput=null;this.pollInterval=null;this.plugin=e,this.accountOwnershipService=new R(e.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return B}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}if(this.plugin.settings.accountsImported){let e=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(s=>s.children!==void 0)||[];e.length>0?(this.steps[2].status="complete",console.log(`[Eudia] Vault reopen: ${e.length} account folders verified`)):(console.warn("[Eudia] accountsImported=true but 0 account folders found \u2014 resetting for re-import"),this.plugin.settings.accountsImported=!1,this.plugin.settings.importedAccountCount=0,await this.plugin.saveSettings());try{let s=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");s&&await this.plugin.app.vault.delete(s)}catch{}let n=this.plugin.settings.userEmail,a=(this.plugin.settings.cachedAccounts||[]).filter(s=>s.id&&String(s.id).startsWith("001"));if(n&&a.length>0){let s=this.plugin.settings.accountsFolder||"Accounts",o=!1;for(let l of a.slice(0,5)){let c=(l.name||"").replace(/[<>:"/\\|?*]/g,"_").trim(),d=`${s}/${c}/Contacts.md`,u=this.plugin.app.vault.getAbstractFileByPath(d);if(u instanceof p.TFile&&!this.plugin.app.metadataCache.getFileCache(u)?.frontmatter?.enriched_at){o=!0;break}}o&&(console.log("[Eudia Setup] Accounts need enrichment \u2014 triggering on vault reopen..."),setTimeout(async()=>{try{let l=a.map(c=>({id:c.id,name:c.name,type:"",isOwned:!1,hadOpportunity:!0,website:null,industry:null}));await this.plugin.enrichAccountFolders(l),console.log(`[Eudia] Vault-reopen enrichment complete: ${l.length} accounts enriched`)}catch(l){console.log("[Eudia] Vault-reopen enrichment failed (will retry next open):",l)}},3e3))}}else{console.log("[Eudia Setup] Email set but accounts not imported \u2014 auto-retrying import...");let t=this.plugin.app.workspace.leftSplit,e=t?.collapsed;try{let n=this.plugin.settings.userEmail,i=this.plugin.settings.userRole||(D(n)?"admin":H(n)?"cs":"sales"),a=[],s=[];if(console.log(`[Eudia Setup] Auto-retry for ${n} (role: ${i})`),i==="other")a=[];else if(i==="exec"||i==="product")a=await this.accountOwnershipService.getExecProductAccounts(n);else if(i==="cs")a=[...N],console.log(`[Eudia Setup] Auto-retry CS: using ${a.length} static accounts`);else if(i==="admin")a=await this.accountOwnershipService.getAllAccountsForAdmin(n);else{let o=await this.accountOwnershipService.getAccountsWithProspects(n);a=o.accounts,s=o.prospects}if(a.length>0||s.length>0){if(t&&!e&&t.collapse(),i==="admin"?await this.plugin.createAdminAccountFolders(a):(await this.plugin.createTailoredAccountFolders(a,{}),s.length>0&&await this.plugin.createProspectAccountFiles(s)),G(n))try{await this.plugin.createCSManagerDashboard(n,a)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=a.length+s.length,await this.plugin.saveSettings(),this.steps[2].status="complete";try{let o=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");o&&await this.plugin.app.vault.delete(o)}catch{}t&&!e&&t.expand(),console.log(`[Eudia Setup] Auto-retry imported ${a.length} accounts for ${n}`),new p.Notice(`Enriching ${a.length} accounts with Salesforce contacts...`);try{let o=i==="cs"?a:[...a,...s];await this.plugin.enrichAccountFolders(o),new p.Notice(`${a.length} accounts loaded and enriched!`),console.log("[Eudia Setup] Auto-retry enrichment complete")}catch(o){console.log("[Eudia Setup] Auto-retry enrichment failed:",o),new p.Notice(`${a.length} accounts imported! Contacts will populate on next open.`)}}else console.warn(`[Eudia Setup] Auto-retry returned 0 accounts for ${n}. Server may still be starting.`),t&&!e&&t.expand()}catch(n){console.error("[Eudia Setup] Auto-retry account import failed:",n),t&&!e&&t.expand()}}}}getCompletionPercentage(){let t=this.steps.filter(e=>e.status==="complete").length;return Math.round(t/this.steps.length*100)}async render(){let t=this.containerEl.children[1];t.empty(),t.addClass("eudia-setup-view"),this.renderHeader(t),this.renderSteps(t),this.renderGettingStarted(t),this.renderFooter(t)}renderGettingStarted(t){let e=t.createDiv({cls:"eudia-getting-started"});e.createEl("h3",{text:"Your Sidebar Tools",cls:"eudia-getting-started-title"});let n=[{icon:"calendar",name:"Calendar",desc:"View your external meetings. Click any event to create a meeting note under the matched account. Adjust your timezone in the Eudia Lite settings."},{icon:"microphone",name:"Transcribe",desc:"Click the mic icon before a meeting to start recording. AI transcribes and extracts key insights, objections, and next steps automatically."},{icon:"message-circle",name:"Ask GTM Brain",desc:"Query Salesforce data in natural language \u2014 pipeline, contacts, deal history, and account intelligence."}];for(let i of n){let a=e.createDiv({cls:"eudia-getting-started-row"}),s=a.createDiv({cls:"eudia-getting-started-icon"});(0,p.setIcon)(s,i.icon);let o=a.createDiv({cls:"eudia-getting-started-text"});o.createEl("strong",{text:i.name}),o.createEl("span",{text:` \u2014 ${i.desc}`})}}renderHeader(t){let e=t.createDiv({cls:"eudia-setup-header"}),n=e.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Notetaker",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to transcribe and summarize meetings -- capturing objections, next steps, and pain points to drive better client outcomes and smarter selling.",cls:"eudia-setup-subtitle"});let i=e.createDiv({cls:"eudia-setup-progress-section"}),a=this.getCompletionPercentage(),s=i.createDiv({cls:"eudia-setup-progress-label"});s.createSpan({text:"Setup Progress"}),s.createSpan({text:`${a}%`,cls:"eudia-setup-progress-value"});let l=i.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});l.style.width=`${a}%`}renderSteps(t){let e=t.createDiv({cls:"eudia-setup-steps-container"});this.renderRoleStep(e),this.renderCalendarStep(e),this.renderSalesforceStep(e),this.renderTranscribeStep(e)}renderRoleStep(t){let e=!!this.plugin.settings.userRole,n=t.createDiv({cls:`eudia-setup-step-card ${e?"complete":"in_progress"}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),a=i.createDiv({cls:"eudia-setup-step-number"});a.setText(""),e?a.addClass("eudia-step-complete"):a.setText("1");let s=i.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:"Select Your Team"}),s.createEl("p",{text:"This determines which accounts and features are loaded"});let o=n.createDiv({cls:"eudia-setup-step-content"});if(e){let l={sales:"Sales",cs:"CS",exec:"Executive",product:"Product",admin:"Ops / Admin",other:"Other"};o.createDiv({cls:"eudia-setup-complete-message",text:l[this.plugin.settings.userRole]||this.plugin.settings.userRole})}else{let l=[{value:"sales",label:"Sales"},{value:"cs",label:"CS"},{value:"exec",label:"Executive"},{value:"product",label:"Product"},{value:"admin",label:"Ops / Admin"},{value:"other",label:"Other"}],c=o.createDiv();c.style.cssText="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;";for(let d of l){let u=c.createEl("button",{text:d.label,cls:"eudia-setup-btn"});u.style.cssText="text-align:center;font-size:12px;padding:6px 10px;",u.onclick=async()=>{this.plugin.settings.userRole=d.value,await this.plugin.saveSettings(),await this.render()}}}}renderCalendarStep(t){let e=this.steps[0],n=!!this.plugin.settings.userRole,i=t.createDiv({cls:`eudia-setup-step-card ${e.status}${n?"":" pending"}`}),a=i.createDiv({cls:"eudia-setup-step-header"}),s=a.createDiv({cls:"eudia-setup-step-number"});s.setText(e.status==="complete"?"":"2"),e.status==="complete"&&s.addClass("eudia-step-complete"),n||s.addClass("pending");let o=a.createDiv({cls:"eudia-setup-step-info"});o.createEl("h3",{text:e.title}),o.createEl("p",{text:e.description});let l=i.createDiv({cls:"eudia-setup-step-content"});if(!n){l.createDiv({cls:"eudia-setup-disabled-message",text:"Select your team first"});return}if(e.status==="complete")l.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=l.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let d=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});d.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async u=>{u.key==="Enter"&&await this.handleCalendarConnect()},l.createDiv({cls:"eudia-setup-validation-message"}),l.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let t=this.emailInput.value.trim().toLowerCase(),e=this.containerEl.querySelector(".eudia-setup-validation-message");if(!t){e&&(e.textContent="Please enter your email",e.className="eudia-setup-validation-message error");return}if(!t.endsWith("@eudia.com")){e&&(e.textContent="Please use your @eudia.com email address",e.className="eudia-setup-validation-message error");return}e&&(e.textContent="Validating...",e.className="eudia-setup-validation-message loading");try{let n=await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(t)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=t,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new p.Notice("Calendar connected successfully!"),e&&(e.textContent="Importing your accounts...",e.className="eudia-setup-validation-message loading");let i=this.plugin.app.workspace.leftSplit,a=i?.collapsed;i&&!a&&i.collapse();try{let s,o=[],l=this.plugin.settings.userRole||(D(t)?"admin":H(t)?"cs":"sales");if(console.log(`[Eudia] User role: ${l} for ${t}`),l==="other")s=[],console.log("[Eudia] Other role \u2014 no account sync"),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=0,await this.plugin.saveSettings();else if(l==="exec"||l==="product"){console.log(`[Eudia] ${l} role \u2014 loading Existing + active pipeline accounts`),s=await this.accountOwnershipService.getExecProductAccounts(t),e&&(e.textContent=`Loading ${s.length} accounts...`),await this.plugin.createTailoredAccountFolders(s,{}),(this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(h=>h.children!==void 0)||[]).length>0&&(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length),await this.plugin.saveSettings();try{let h=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");h&&await this.plugin.app.vault.delete(h)}catch{}new p.Notice(`Imported ${s.length} accounts!`);let u=s.filter(h=>h.id&&h.id.startsWith("001"));if(u.length>0){e&&(e.textContent=`Enriching ${u.length} accounts...`);try{await this.plugin.enrichAccountFolders(u)}catch{}}}else if(l==="cs"){if(console.log(`[Eudia] CS user detected \u2014 loading ${N.length} accounts from static data (instant, no server needed)`),s=[...N],o=[],e&&(e.textContent=`Loading ${s.length} Customer Success accounts...`),await this.plugin.createTailoredAccountFolders(s,{}),G(t))try{await this.plugin.createCSManagerDashboard(t,s)}catch(u){console.error("[Eudia] CS Manager dashboard creation failed (non-blocking):",u)}let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(u=>u.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length,console.log(`[Eudia] CS accounts verified: ${d.length} folders created`)):(console.warn(`[Eudia] CS folder creation may have failed \u2014 ${d.length} folders found. Keeping accountsImported=false for retry.`),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let u=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");u&&await this.plugin.app.vault.delete(u)}catch{}console.log(`[Eudia] CS accounts created: ${s.length} folders from static data`),e&&(e.textContent=`Enriching ${s.length} accounts with Salesforce contacts...`),new p.Notice(`Enriching ${s.length} accounts with contacts from Salesforce...`),console.log(`[Eudia] Starting synchronous enrichment for ${s.length} CS accounts...`);try{await this.plugin.enrichAccountFolders(s),console.log("[Eudia] Synchronous enrichment complete"),new p.Notice(`${s.length} accounts loaded with contacts from Salesforce!`),e&&(e.textContent=`${s.length} accounts loaded and enriched with Salesforce contacts!`)}catch(u){console.log("[Eudia] Synchronous enrichment failed, will retry in background:",u),new p.Notice(`${s.length} accounts loaded! Contacts will populate shortly...`);let h=t,m=[5e3,2e4,6e4],y=async w=>{let O=m[w];if(O!==void 0){await new Promise(S=>setTimeout(S,O));try{await this.plugin.enrichAccountFolders(s),console.log(`[Eudia] Background enrichment retry ${w+1} succeeded`)}catch{return y(w+1)}}};y(0)}}else if(l==="admin"){if(console.log("[Eudia] Admin user detected - importing all accounts"),s=await this.accountOwnershipService.getAllAccountsForAdmin(t),s.length>0){e&&(e.textContent=`Creating ${s.length} account folders...`),await this.plugin.createAdminAccountFolders(s);let d=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(h=>h.children!==void 0)||[];d.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length,console.log(`[Eudia] Admin accounts verified: ${d.length} folders created`)):(console.warn("[Eudia] Admin folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let h=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");h&&await this.plugin.app.vault.delete(h)}catch{}new p.Notice(`Imported ${s.length} accounts! Enriching with Salesforce data...`);let u=s.filter(h=>h.id&&h.id.startsWith("001"));if(u.length>0){e&&(e.textContent=`Enriching ${u.length} accounts with Salesforce contacts...`);try{await this.plugin.enrichAccountFolders(u),new p.Notice(`${s.length} accounts loaded and enriched with Salesforce data!`),console.log(`[Eudia] Admin/exec synchronous enrichment complete: ${u.length} accounts`),e&&(e.textContent=`${s.length} accounts loaded and enriched!`)}catch(h){console.log("[Eudia] Admin/exec synchronous enrichment failed, will retry on next open:",h),new p.Notice(`${s.length} accounts imported! Contacts will populate on next open.`);let m=[5e3,2e4,6e4],y=async w=>{let O=m[w];if(O!==void 0){await new Promise(S=>setTimeout(S,O));try{await this.plugin.enrichAccountFolders(u),console.log(`[Eudia] Admin/exec background enrichment retry ${w+1} succeeded`)}catch{return y(w+1)}}};y(0)}}}}else{let c=await this.accountOwnershipService.getAccountsWithProspects(t);if(s=c.accounts,o=c.prospects,s.length>0||o.length>0){e&&(e.textContent=`Creating ${s.length} account folders...`),await this.plugin.createTailoredAccountFolders(s,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o);let u=this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(m=>m.children!==void 0)||[];u.length>0?(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+o.length,console.log(`[Eudia] BL accounts verified: ${u.length} folders created`)):(console.warn("[Eudia] BL folder creation may have failed \u2014 keeping accountsImported=false for retry"),this.plugin.settings.accountsImported=!1),await this.plugin.saveSettings();try{let m=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");m&&await this.plugin.app.vault.delete(m)}catch{}new p.Notice(`Imported ${s.length} active accounts + ${o.length} prospects! Enriching with Salesforce contacts...`);let h=[...s,...o];try{e&&(e.textContent=`Enriching ${h.length} accounts with Salesforce contacts...`),await this.plugin.enrichAccountFolders(h),new p.Notice(`${h.length} accounts enriched with Salesforce data!`),console.log(`[Eudia] BL enrichment complete: ${h.length} accounts`)}catch(m){console.log("[Eudia] BL enrichment failed, will retry on next launch:",m)}}else{console.warn(`[Eudia] No accounts returned for ${t} \u2014 auto-retrying...`);let d=!1;for(let u=1;u<=3;u++){e&&(e.textContent=`Server warming up... retrying in 10s (attempt ${u}/3)`,e.className="eudia-setup-validation-message warning"),await new Promise(h=>setTimeout(h,1e4));try{let h=await this.plugin.accountOwnershipService.getAccountsWithProspects(t);if(h.accounts.length>0||h.prospects.length>0){s=h.accounts,o=h.prospects,e&&(e.textContent=`Creating ${s.length} account folders...`),await this.plugin.createTailoredAccountFolders(s,{}),o.length>0&&await this.plugin.createProspectAccountFiles(o),(this.plugin.app.vault.getAbstractFileByPath(this.plugin.settings.accountsFolder||"Accounts")?.children?.filter(w=>w.children!==void 0)||[]).length>0&&(this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+o.length),await this.plugin.saveSettings(),new p.Notice(`Imported ${s.length} accounts + ${o.length} prospects! Enriching...`);try{let w=[...s,...o];e&&(e.textContent=`Enriching ${w.length} accounts with Salesforce contacts...`),await this.plugin.enrichAccountFolders(w),new p.Notice(`${w.length} accounts enriched with Salesforce data!`)}catch(w){console.log("[Eudia] Retry enrichment failed, will retry on next launch:",w)}d=!0;break}}catch(h){console.warn(`[Eudia] Retry ${u} failed:`,h)}}d||(e&&(e.textContent="Could not load accounts after 3 attempts. Close this window, wait 1 minute, then re-open Obsidian and try again.",e.className="eudia-setup-validation-message error"),new p.Notice("Account import failed after retries. Wait 1 minute and try again."))}}}catch(s){console.error("[Eudia] Account import failed:",s),e&&(e.textContent="Account import failed. Please try again.",e.className="eudia-setup-validation-message error"),new p.Notice("Account import failed \u2014 please try again.")}finally{i&&!a&&i.expand()}await this.render()}else e&&(e.innerHTML=`<strong>${t}</strong> is not authorized for calendar access. Contact your admin.`,e.className="eudia-setup-validation-message error")}catch{e&&(e.textContent="Connection failed. Please try again.",e.className="eudia-setup-validation-message error")}}renderSalesforceStep(t){let e=this.steps[1],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),a=i.createDiv({cls:"eudia-setup-step-number"});a.setText(e.status==="complete"?"":"2"),e.status==="complete"&&a.addClass("eudia-step-complete");let s=i.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:e.title}),s.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(e.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let c=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),d=o.createDiv({cls:"eudia-setup-sf-status"});c.onclick=async()=>{let u=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(u,"_blank"),d.textContent="Complete the login in the popup window...",d.className="eudia-setup-sf-status loading",new p.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(d)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(t){this.pollInterval&&window.clearInterval(this.pollInterval);let e=0,n=60;this.pollInterval=window.setInterval(async()=>{e++;try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new p.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(t),await this.render()):e>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),t.textContent="Connection timed out. Please try again.",t.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(t){t.textContent="Importing your accounts...",t.className="eudia-setup-sf-status loading";try{let e=this.plugin.settings.userEmail,n=D(e)?"admin":H(e)?"cs":"bl";console.log(`[Eudia SF Import] Importing for ${e} (group: ${n})`);let i,a=[];if(n==="cs"){console.log("[Eudia SF Import] CS user SF Connect \u2014 fetching live data from Salesforce..."),t.textContent="Syncing with Salesforce for latest account data...";try{let u=await this.accountOwnershipService.getCSAccounts(e);i=u.accounts,a=u.prospects,console.log(`[Eudia SF Import] CS server sync: ${i.length} accounts (with real SF IDs + CSM data)`)}catch{if(this.plugin.settings.accountsImported){t.textContent="Salesforce connected! Account folders already loaded. Enrichment will retry later.",t.className="eudia-setup-sf-status success",this.steps[1].status="complete";return}i=[...N],console.log(`[Eudia SF Import] CS server unavailable \u2014 using ${i.length} static accounts`)}}else if(n==="admin")console.log("[Eudia] Admin user detected - importing all accounts"),t.textContent="Admin detected - importing all accounts...",i=await this.accountOwnershipService.getAllAccountsForAdmin(e);else{let u=await this.accountOwnershipService.getAccountsWithProspects(e);i=u.accounts,a=u.prospects}if(i.length===0&&a.length===0){t.textContent="No accounts found for your email. Contact your admin.",t.className="eudia-setup-sf-status warning";return}t.textContent=`Creating ${i.length} account folders...`;let s=this.plugin.app.workspace.leftSplit,o=s?.collapsed;if(s&&!o&&s.collapse(),D(e)?await this.plugin.createAdminAccountFolders(i):(await this.plugin.createTailoredAccountFolders(i,{}),a.length>0&&await this.plugin.createProspectAccountFiles(a)),G(e))try{await this.plugin.createCSManagerDashboard(e,i)}catch{}this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=i.length+a.length,await this.plugin.saveSettings();try{let u=this.plugin.app.vault.getAbstractFileByPath("Accounts/_Setup Required.md");u&&await this.plugin.app.vault.delete(u)}catch{}s&&!o&&s.expand(),this.steps[2].status="complete";let l=i.filter(u=>u.isOwned!==!1).length,c=i.filter(u=>u.isOwned===!1).length;n==="admin"&&c>0?t.textContent=`${l} owned + ${c} view-only accounts imported! Enriching...`:t.textContent=`${i.length} active + ${a.length} prospect accounts imported! Enriching...`,t.className="eudia-setup-sf-status success";let d=[...i,...a];try{let u=d.filter(h=>h.id&&h.id.startsWith("001"));if(u.length>0?(t.textContent=`Enriching ${u.length} accounts with Salesforce contacts...`,await this.plugin.enrichAccountFolders(u),t.textContent=`${i.length} accounts imported, ${u.length} enriched with Salesforce data`):t.textContent=`${i.length} accounts imported (enrichment requires Salesforce IDs)`,n==="cs"&&G(e))try{console.log("[Eudia SF Import] Regenerating CS Manager dashboard with live CSM data..."),await this.plugin.createCSManagerDashboard(e,i),console.log("[Eudia SF Import] CS Manager dashboard updated with CSM assignments")}catch(h){console.error("[Eudia SF Import] Dashboard regeneration failed (non-blocking):",h)}}catch(u){console.log("[Eudia] SF Connect enrichment failed, will retry on next launch:",u),t.textContent=`${i.length+a.length} accounts imported (enrichment will retry on next launch)`}}catch{t.textContent="Failed to import accounts. Please try again.",t.className="eudia-setup-sf-status error";let n=this.plugin.app.workspace.leftSplit;if(n?.collapsed===!1)try{n.expand()}catch{}}}renderTranscribeStep(t){let e=this.steps[2],n=t.createDiv({cls:`eudia-setup-step-card ${e.status}`}),i=n.createDiv({cls:"eudia-setup-step-header"}),a=i.createDiv({cls:"eudia-setup-step-number"});a.setText(e.status==="complete"?"":"3"),e.status==="complete"&&a.addClass("eudia-step-complete");let s=i.createDiv({cls:"eudia-setup-step-info"});s.createEl("h3",{text:e.title}),s.createEl("p",{text:e.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),l=o.createDiv({cls:"eudia-setup-instructions"}),c=l.createDiv({cls:"eudia-setup-instruction"}),d=c.createSpan({cls:"eudia-setup-instruction-icon"});(0,p.setIcon)(d,"microphone"),c.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let u=l.createDiv({cls:"eudia-setup-instruction"}),h=u.createSpan({cls:"eudia-setup-instruction-icon"});(0,p.setIcon)(h,"terminal"),u.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let m=l.createDiv({cls:"eudia-setup-instruction"}),y=m.createSpan({cls:"eudia-setup-instruction-icon"});(0,p.setIcon)(y,"file-text"),m.createSpan({text:"AI will summarize and extract key insights automatically"}),e.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(t){let e=t.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(a=>a.status==="complete")){let a=e.createDiv({cls:"eudia-setup-completion"}),s=a.createEl("h2",{cls:"eudia-setup-completion-title"}),o=s.createSpan({cls:"eudia-setup-completion-icon"});(0,p.setIcon)(o,"check-circle"),s.createSpan({text:" You're all set!"}),a.createEl("p",{text:"Your Eudia Notetaker is ready. Click below to start using Eudia."});let l=e.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});l.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(B),await this.plugin.activateCalendarView()}}else{let a=e.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});a.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(B);let s=this.plugin.app.vault.getAbstractFileByPath("QUICKSTART.md");s instanceof p.TFile&&await this.plugin.app.workspace.getLeaf().openFile(s),new p.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let i=e.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});i.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},re=class extends p.ItemView{constructor(t,e){super(t);this.updateInterval=null;this.chatHistory=[];this.plugin=e}getViewType(){return _}getDisplayText(){return"Live Query"}getIcon(){return"message-circle"}async onOpen(){await this.render(),this.updateInterval=window.setInterval(()=>this.updateStatus(),5e3)}async onClose(){this.updateInterval&&window.clearInterval(this.updateInterval)}updateStatus(){let t=this.containerEl.querySelector(".eudia-lq-status");if(t)if(this.plugin.audioRecorder?.isRecording()){let e=Math.round((this.plugin.liveTranscript?.length||0)/5),n=this.plugin.audioRecorder.getState().duration,i=Math.floor(n/60),a=n%60;t.setText(`Recording ${i}:${a.toString().padStart(2,"0")} \u2014 ${e} words captured`),t.style.color="var(--text-success)"}else t.setText("Not recording. Start a recording to use Live Query."),t.style.color="var(--text-muted)"}async render(){let t=this.containerEl.children[1];t.empty(),t.addClass("eudia-live-query-view"),t.style.cssText="display:flex;flex-direction:column;height:100%;padding:12px;";let e=t.createDiv({cls:"eudia-lq-status"});e.style.cssText="font-size:12px;padding:8px 0;border-bottom:1px solid var(--background-modifier-border);margin-bottom:8px;",this.updateStatus();let n=t.createDiv({cls:"eudia-lq-quick-actions"});n.style.cssText="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px;";let i=[{label:"Summarize so far",query:"Give me a concise summary of everything discussed so far."},{label:"Action items",query:"What action items or next steps have been discussed so far?"},{label:"Key concerns",query:"What concerns, objections, or risks have been raised?"}];for(let c of i){let d=n.createEl("button",{text:c.label});d.style.cssText="font-size:11px;padding:4px 10px;border-radius:12px;border:1px solid var(--background-modifier-border);cursor:pointer;background:var(--background-secondary);",d.onclick=()=>this.submitQuery(c.query,a,o)}let a=t.createDiv({cls:"eudia-lq-chat"});a.style.cssText="flex:1;overflow-y:auto;margin-bottom:12px;display:flex;flex-direction:column;gap:8px;";for(let c of this.chatHistory)this.renderMessage(a,c.role,c.text);if(this.chatHistory.length===0){let c=a.createDiv();c.style.cssText="color:var(--text-muted);font-size:12px;text-align:center;padding:20px 0;",c.setText("Ask a question about the conversation while recording.")}let s=t.createDiv({cls:"eudia-lq-input-area"});s.style.cssText="display:flex;gap:8px;border-top:1px solid var(--background-modifier-border);padding-top:8px;";let o=s.createEl("textarea",{attr:{placeholder:"Ask about the conversation...",rows:"2"}});o.style.cssText="flex:1;resize:none;border-radius:8px;padding:8px;font-size:13px;border:1px solid var(--background-modifier-border);background:var(--background-primary);";let l=s.createEl("button",{text:"Ask"});l.style.cssText="padding:8px 16px;border-radius:8px;cursor:pointer;align-self:flex-end;font-weight:600;",l.addClass("mod-cta"),l.onclick=()=>this.submitQuery(o.value.trim(),a,o),o.addEventListener("keydown",c=>{c.key==="Enter"&&!c.shiftKey&&(c.preventDefault(),l.click())})}renderMessage(t,e,n){let i=t.querySelector(".eudia-lq-chat > div:only-child");i&&i.textContent?.includes("Ask a question")&&i.remove();let a=t.createDiv(),s=e==="user";a.style.cssText=`padding:8px 12px;border-radius:10px;font-size:13px;line-height:1.5;max-width:90%;${s?"align-self:flex-end;background:var(--interactive-accent);color:var(--text-on-accent);":"align-self:flex-start;background:var(--background-secondary);"}`,a.setText(n)}async submitQuery(t,e,n){if(!t)return;if(!this.plugin.audioRecorder?.isRecording()){new p.Notice("Start a recording first to use Live Query.");return}let i=this.plugin.liveTranscript||"";if(i.length<50){new p.Notice("Not enough transcript captured yet. Keep recording for a few more minutes.");return}this.chatHistory.push({role:"user",text:t}),this.renderMessage(e,"user",t),n.value="";let a=e.createDiv();a.style.cssText="align-self:flex-start;padding:8px 12px;border-radius:10px;font-size:13px;background:var(--background-secondary);color:var(--text-muted);",a.setText("Thinking..."),e.scrollTop=e.scrollHeight;try{let s=await this.plugin.transcriptionService.liveQueryTranscript(t,i,this.plugin.getAccountNameFromActiveFile());a.remove();let o=s.success?s.answer:s.error||"Query failed";this.chatHistory.push({role:"assistant",text:o}),this.renderMessage(e,"assistant",o)}catch(s){a.remove();let o=`Error: ${s.message}`;this.chatHistory.push({role:"assistant",text:o}),this.renderMessage(e,"assistant",o)}e.scrollTop=e.scrollHeight}},oe=class b extends p.ItemView{constructor(t,e){super(t);this.refreshInterval=null;this.lastError=null;this.showExternalOnly=!0;this.weeksBack=2;this.plugin=e}getViewType(){return z}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let t=this.containerEl.children[1];if(t.empty(),t.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(t);return}this.renderHeader(t),await this.renderCalendarContent(t)}static{this.INTERNAL_SUBJECT_PATTERNS=[/^block\b/i,/\bblock\s+for\b/i,/\bcommute\b/i,/\bpersonal\b/i,/\blunch\b/i,/\bOOO\b/i,/\bout of office\b/i,/\bfocus time\b/i,/\bno meetings?\b/i,/\bmeeting free\b/i,/\btravel\b/i,/\beye appt\b/i,/\bdoctor\b/i,/\bdentist\b/i,/\bgym\b/i,/\bworkout\b/i]}isExternalMeeting(t){if(t.isCustomerMeeting)return!0;if(!t.attendees||t.attendees.length===0)return!1;let e=this.plugin.settings.userEmail?.split("@")[1]||"eudia.com";if(t.attendees.some(i=>{if(i.isExternal===!0)return!0;if(i.isExternal===!1||!i.email)return!1;let a=i.email.split("@")[1]?.toLowerCase();return a&&a!==e.toLowerCase()}))return!0;for(let i of b.INTERNAL_SUBJECT_PATTERNS)if(i.test(t.subject))return!1;return!1}renderHeader(t){let e=t.createDiv({cls:"eudia-calendar-header"}),n=e.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let i=n.createDiv({cls:"eudia-calendar-actions"}),a=i.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});a.title="Refresh",a.onclick=async()=>{a.addClass("spinning"),this._forceRefresh=!0,await this.render(),a.removeClass("spinning")};let s=i.createEl("button",{cls:"eudia-btn-icon"});(0,p.setIcon)(s,"settings"),s.title="Settings",s.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=e.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o);let l=e.createDiv({cls:"eudia-calendar-filter-row"});l.style.cssText="display:flex;align-items:center;gap:8px;margin-top:6px;padding:4px 0;";let c=l.createEl("button",{text:this.showExternalOnly?"External Only":"All Meetings",cls:"eudia-filter-toggle"});c.style.cssText=`font-size:11px;padding:3px 10px;border-radius:12px;cursor:pointer;border:1px solid var(--background-modifier-border);background:${this.showExternalOnly?"var(--interactive-accent)":"var(--background-secondary)"};color:${this.showExternalOnly?"var(--text-on-accent)":"var(--text-muted)"};`,c.title=this.showExternalOnly?"Showing customer/external meetings only \u2014 click to show all":"Showing all meetings \u2014 click to filter to external only",c.onclick=async()=>{this.showExternalOnly=!this.showExternalOnly,await this.render()}}async renderConnectionStatus(t){let e={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,i=this.plugin.settings.userEmail;try{(await(0,p.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(e.server="connected",e.serverMessage="Server online"):(e.server="error",e.serverMessage="Server unavailable")}catch{e.server="error",e.serverMessage="Cannot reach server"}if(i&&e.server==="connected")try{let d=await(0,p.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(i)}`,method:"GET",throw:!1});d.status===200&&d.json?.authorized?(e.calendar="connected",e.calendarMessage="Calendar synced"):(e.calendar="not_authorized",e.calendarMessage="Not authorized")}catch{e.calendar="error",e.calendarMessage="Error checking access"}if(i&&e.server==="connected")try{let d=await(0,p.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(i)}`,method:"GET",throw:!1});d.status===200&&d.json?.connected?(e.salesforce="connected",e.salesforceMessage="Salesforce connected"):(e.salesforce="not_configured",e.salesforceMessage="Not connected")}catch{e.salesforce="not_configured"}let a=t.createDiv({cls:"eudia-status-indicators"}),s=a.createSpan({cls:`eudia-status-dot ${e.server}`});s.title=e.serverMessage||"Server";let o=a.createSpan({cls:`eudia-status-dot ${e.calendar}`});o.title=e.calendarMessage||"Calendar";let l=a.createSpan({cls:`eudia-status-dot ${e.salesforce}`});if(l.title=e.salesforceMessage||"Salesforce",t.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),e.calendar==="not_authorized"){let d=t.createDiv({cls:"eudia-status-warning"});d.innerHTML=`<strong>${i}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(t){let e=t.createDiv({cls:"eudia-calendar-content"}),n=e.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let i=new M(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),a=this._forceRefresh||!1;this._forceRefresh=!1;let s=await i.getWeekMeetings(a);if(!s.success){n.remove(),this.renderError(e,s.error||"Failed to load calendar");return}let o=new Date,l=new Date(o);l.setDate(l.getDate()-this.weeksBack*7);let c=new Date(o);c.setDate(c.getDate()-1);let d=[];try{d=await i.getMeetingsInRange(l,c)}catch{console.log("[Calendar] Could not fetch past meetings")}n.remove();let u={};for(let S of d){let C=S.start.split("T")[0];u[C]||(u[C]=[]),u[C].push(S)}for(let[S,C]of Object.entries(s.byDay||{})){u[S]||(u[S]=[]);let v=new Set(u[S].map(g=>g.id));for(let g of C)v.has(g.id)||u[S].push(g)}let h={};for(let[S,C]of Object.entries(u)){let v=this.showExternalOnly?C.filter(g=>this.isExternalMeeting(g)):C;v.length>0&&(h[S]=v)}let m=Object.keys(h).sort();if(m.length===0){this.renderEmptyState(e);return}await this.renderCurrentMeeting(e,i);let y=e.createEl("button",{text:"\u2190 Load earlier meetings",cls:"eudia-load-earlier"});y.style.cssText="width:100%;padding:8px;margin-bottom:8px;font-size:12px;cursor:pointer;border-radius:6px;border:1px solid var(--background-modifier-border);background:var(--background-secondary);color:var(--text-muted);",y.onclick=async()=>{this.weeksBack+=2,await this.render()};let w=o.toISOString().split("T")[0],O=null;for(let S of m){let C=h[S];if(!C||C.length===0)continue;let v=this.renderDaySection(e,S,C);S===w&&(O=v)}O&&setTimeout(()=>O.scrollIntoView({block:"start",behavior:"auto"}),100)}catch(i){n.remove(),this.renderError(e,i.message||"Failed to load calendar")}}async renderCurrentMeeting(t,e){try{let n=await e.getCurrentMeeting();if(n.meeting){let i=t.createDiv({cls:"eudia-now-card"});n.isNow?i.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):i.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let a=i.createDiv({cls:"eudia-now-content"});a.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&a.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let s=i.createEl("button",{cls:"eudia-now-action",text:"Create Note"});s.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(t,e,n){let i=t.createDiv({cls:"eudia-calendar-day"}),a=new Date().toISOString().split("T")[0],s=e===a,o=e<a,l=s?"TODAY":M.getDayName(e),c=i.createEl("div",{cls:`eudia-calendar-day-header ${s?"today":""} ${o?"past":""}`,text:l});s?c.style.cssText="font-weight:700;color:var(--interactive-accent);":o&&(c.style.cssText="opacity:0.7;");for(let d of n){let u=i.createDiv({cls:`eudia-calendar-meeting ${d.isCustomerMeeting?"customer":"internal"} ${o?"past":""}`});o&&(u.style.cssText="opacity:0.85;"),u.createEl("div",{cls:"eudia-calendar-time",text:M.formatTime(d.start,this.plugin.settings.timezone)});let h=u.createDiv({cls:"eudia-calendar-details"});if(h.createEl("div",{cls:"eudia-calendar-subject",text:d.subject}),d.accountName)h.createEl("div",{cls:"eudia-calendar-account",text:d.accountName});else if(d.attendees&&d.attendees.length>0){let m=d.attendees.filter(y=>y.isExternal!==!1).slice(0,2).map(y=>y.name||y.email?.split("@")[0]||"Unknown").join(", ");m&&h.createEl("div",{cls:"eudia-calendar-attendees",text:m})}u.onclick=()=>this.createNoteForMeeting(d),u.title="Click to create meeting note"}return i}renderEmptyState(t){let e=t.createDiv({cls:"eudia-calendar-empty"});e.innerHTML=`
    ...
    `}renderError(t,e){let n=t.createDiv({cls:"eudia-calendar-error"}),i="",a="Unable to load calendar",s="";e.includes("not authorized")||e.includes("403")?(i="\u{1F512}",a="Calendar Access Required",s="Contact your admin to be added to the authorized users list."):e.includes("network")||e.includes("fetch")?(i="\u{1F4E1}",a="Connection Issue",s="Check your internet connection and try again."):(e.includes("server")||e.includes("500"))&&(i="\u{1F527}",a="Server Unavailable",s="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=e.createDiv({cls:"eudia-setup-input-group"}),i=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});i.addClass("eudia-setup-email");let a=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),s=e.createDiv({cls:"eudia-setup-status"});a.onclick=async()=>{let o=i.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){s.textContent="Please use your @eudia.com email",s.className="eudia-setup-status error";return}a.disabled=!0,a.textContent="Connecting...",s.textContent="";try{if(!(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){s.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,s.className="eudia-setup-status error",a.disabled=!1,a.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),s.textContent="Connected",s.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(l){let c=l.message||"Connection failed";c.includes("403")?s.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:s.textContent=c,s.className="eudia-setup-status error",a.disabled=!1,a.textContent="Connect"}},i.onkeydown=o=>{o.key==="Enter"&&a.click()},e.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(t){let e=t.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],i=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],a=e.filter(o=>!i.includes(o)&&o.length>1);if(a.length===0)return e[0]||"";if(a.length>1&&n.includes(a[0]))return a[1].charAt(0).toUpperCase()+a[1].slice(1);let s=a[a.length-1];return s.charAt(0).toUpperCase()+s.slice(1)}getExternalDomainsFromAttendees(t){if(!t||t.length===0)return[];let e=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,i=[];for(let a of t){if(!a.email)continue;let o=a.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let l=o[1];if(l.includes("eudia.com")||e.includes(l)||n.has(l))continue;n.add(l);let c=this.extractCompanyFromDomain(l);c.length>=2&&i.push({domain:l,company:c})}}return i}findBestAccountMatch(t,e,n){let i=this.plugin.settings.accountsFolder||"Accounts",a=this.app.vault.getAbstractFileByPath(i);if(!(a instanceof p.TFolder))return null;let s=[];for(let l of a.children)if(l instanceof p.TFolder)if(l.name==="_Prospects")for(let c of l.children)c instanceof p.TFolder&&s.push(c.name);else s.push(l.name);if(s.length===0)return null;let o=[];for(let{domain:l,company:c}of t){let d=this.findAccountFolder(c),u=d?1:0;o.push({domain:l,company:c,folder:d,score:u})}if(o.sort((l,c)=>c.score-l.score),o.length>0&&o[0].folder){let l=o[0],c=l.folder.split("/").pop()||l.company;return console.log(`[Eudia Calendar] Best domain match: "${l.company}" from ${l.domain} -> ${l.folder}`),{folder:l.folder,accountName:c,source:"domain"}}if(e){let l=this.findAccountFolder(e);if(l){let c=l.split("/").pop()||e;return console.log(`[Eudia Calendar] Server account match: "${e}" -> ${l}`),{folder:l,accountName:c,source:"server"}}}if(n){let l=this.findAccountFolder(n);if(l){let c=l.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${l}`),{folder:l,accountName:c,source:"subject"}}}for(let{company:l}of t){let c=s.find(d=>{let u=d.toLowerCase(),h=l.toLowerCase();return u.includes(h)||h.includes(u)});if(c){let d=`${i}/${c}`;return console.log(`[Eudia Calendar] Partial domain match: "${l}" -> ${d}`),{folder:d,accountName:c,source:"domain-partial"}}}return null}extractAccountFromAttendees(t){let e=this.getExternalDomainsFromAttendees(t);if(e.length===0)return null;let n=e[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(t){if(!t)return null;let e=t.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(e){let i=(e[1]||e[2]||"").trim();if(i.toLowerCase()!=="eudia")return i}let n=t.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let a=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(a.toLowerCase()!=="eudia"&&a.length>0)return a}if(!t.toLowerCase().includes("eudia")){let i=t.match(/^([^-–|]+)/);if(i){let a=i[1].trim();if(a.length>2&&a.length<50)return a}}return null}findAccountFolder(t){if(!t)return null;let e=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(e);if(!(n instanceof p.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${e}" not found`),null;let i=t.toLowerCase().trim(),a=[],s=new Set;for(let m of n.children)if(m instanceof p.TFolder)if(m.name==="_Prospects")for(let y of m.children)y instanceof p.TFolder&&(a.push(y.name),s.add(y.name));else a.push(m.name);console.log(`[Eudia Calendar] Searching for "${i}" in ${a.length} folders`);let o=m=>s.has(m)?`${e}/_Prospects/${m}`:`${e}/${m}`,l=a.find(m=>m.toLowerCase()===i);if(l)return console.log(`[Eudia Calendar] Exact match found: ${l}`),o(l);let c=a.find(m=>m.toLowerCase().startsWith(i));if(c)return console.log(`[Eudia Calendar] Folder starts with match: ${c}`),o(c);let d=a.find(m=>i.startsWith(m.toLowerCase()));if(d)return console.log(`[Eudia Calendar] Search starts with folder match: ${d}`),o(d);let u=a.find(m=>{let y=m.toLowerCase();return y.length<3||!i.includes(y)?!1:new RegExp(`\\b${y.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`).test(i)});if(u)return console.log(`[Eudia Calendar] Search contains folder match: ${u}`),o(u);let h=a.find(m=>{let y=m.toLowerCase();return i.length<3||!y.includes(i)?!1:new RegExp(`\\b${i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}\\b`).test(y)});return h?(console.log(`[Eudia Calendar] Folder contains search match: ${h}`),o(h)):(console.log(`[Eudia Calendar] No folder match found for "${i}"`),null)}async createNoteForMeeting(t){let e=t.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",i=D(n),a=(t.attendees||[]).map(v=>v.email).filter(Boolean),s=de(t.subject,a);if(i&&s.isPipelineMeeting&&s.confidence>=60){await this._createPipelineNote(t,e);return}let o=t.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),l=`${e} - ${o}.md`,c=null,d=t.accountName||null,u=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${t.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(t.attendees?.map(v=>v.email)||[])}`);let h=this.getExternalDomainsFromAttendees(t.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(h)}`);let m=this.extractAccountFromSubject(t.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${m||"none"}"`);let y=this.findBestAccountMatch(h,t.accountName,m||void 0);if(y&&(c=y.folder,d=y.accountName,console.log(`[Eudia Calendar] Best match (${y.source}): "${d}" -> ${c}`)),!c){let v=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(v)instanceof p.TFolder&&(c=v,console.log(`[Eudia Calendar] No match found, using Accounts root: ${c}`))}if(d){let v=this.plugin.settings.cachedAccounts.find(g=>g.name.toLowerCase()===d?.toLowerCase());v&&(u=v.id,d=v.name,console.log(`[Eudia Calendar] Matched to cached account: ${v.name} (${v.id})`))}let w=c?`${c}/${l}`:l,O=this.app.vault.getAbstractFileByPath(w);if(O instanceof p.TFile){await this.app.workspace.getLeaf().openFile(O);try{let v=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;v?.revealInFolder&&v.revealInFolder(O)}catch{}new p.Notice(`Opened existing note: ${l}`);return}let S=(t.attendees||[]).map(v=>v.name||v.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${t.subject}"
date: ${e}
attendees: [${S}]
account: "${d||""}"
...
`;try{let v=await this.app.vault.create(w,C);await this.app.workspace.getLeaf().openFile(v);try{let g=this.app.workspace.leftSplit;g?.collapsed&&g.expand();let f=this.app.workspace.getLeavesOfType("file-explorer")[0];f&&this.app.workspace.revealLeaf(f);let x=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;x?.revealInFolder&&(x.revealInFolder(v),setTimeout(()=>{let E=document.querySelector(`.nav-file-title[data-path="${v.path.replace(/"/g,'\\"')}"]`);E&&(E.addClass("is-flashing"),setTimeout(()=>E.removeClass("is-flashing"),3e3))},300))}catch{}new p.Notice(`Note created under ${d||"Accounts"}`)}catch(v){console.error("[Eudia Calendar] Failed to create note:",v),new p.Notice(`Could not create note: ${v.message||"Unknown error"}`)}}async _createPipelineNote(t,e){let n=new Date(e+"T00:00:00"),i=String(n.getMonth()+1).padStart(2,"0"),a=String(n.getDate()).padStart(2,"0"),s=String(n.getFullYear()).slice(-2),o=`${i}.${a}.${s}`,l=`Team Pipeline Meeting - ${o}.md`,c="Pipeline Meetings";this.app.vault.getAbstractFileByPath(c)||await this.app.vault.createFolder(c);let u=`${c}/${l}`,h=this.app.vault.getAbstractFileByPath(u);if(h instanceof p.TFile){await this.app.workspace.getLeaf().openFile(h);try{let w=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;w?.revealInFolder&&w.revealInFolder(h)}catch{}new p.Notice(`Opened existing: ${l}`);return}let m=(t.attendees||[]).map(w=>w.name||w.email?.split("@")[0]||"Unknown"),y=`---
...
`;try{let w=await this.app.vault.create(u,y);await this.app.workspace.getLeaf().openFile(w);try{let O=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;O?.revealInFolder&&O.revealInFolder(w)}catch{}new p.Notice(`Created pipeline note: ${l}`),console.log(`[Eudia Pipeline] Created pipeline meeting note: ${u}`)}catch(w){console.error("[Eudia Pipeline] Failed to create pipeline note:",w),new p.Notice(`Could not create pipeline note: ${w.message||"Unknown error"}`)}}},Q=class b extends p.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null;this.micRibbonIcon=null;this._updateInProgress=!1;this._hotReloadPending=!1;this._migrationInProgress=!1;this._updateStatusEl=null;this.liveTranscript="";this.liveTranscriptChunkInterval=null;this.isTranscribingChunk=!1;this._updateRetryCount=0;this._updateBannerEl=null;this.sfSyncStatusBarEl=null;this.sfSyncIntervalId=null}async onload(){if(await this.loadSettings(),this.transcriptionService=new q(this.settings.serverUrl),this.calendarService=new M(this.settings.serverUrl,this.settings.userEmail,this.settings.timezone||"America/New_York"),this.smartTagService=new J,this.telemetry=new te(this.settings.serverUrl,this.settings.userEmail),!this.settings.deviceId){this.settings.deviceId=crypto.randomUUID();try{this.settings.deviceName=require("os").hostname()}catch{this.settings.deviceName="unknown"}await this.saveSettings(),console.log(`[Eudia] Device registered: ${this.settings.deviceId} (${this.settings.deviceName})`)}this.telemetry.setDeviceIdentity(this.settings.deviceId,this.settings.deviceName),I.setupDisplayMediaHandler().then(n=>{console.log(n?"[Eudia] System audio: loopback handler ready":"[Eudia] System audio: handler not available, will try other strategies on record")}).catch(()=>{}),this.checkForUpdateRollback().catch(n=>console.warn("[Eudia] Rollback check error:",n)),this.ensureLightTheme().catch(()=>{}),this.ensureEditableMode().catch(()=>{});let t=this.settings.lastUpdateTimestamp?Date.now()-new Date(this.settings.lastUpdateTimestamp).getTime():1/0,e=this.manifest?.version||"0.0.0";if(t<3e4&&this.settings.lastUpdateVersion===e&&(this._showUpdateStatus(`\u2713 Eudia v${e} active`),setTimeout(()=>this._hideUpdateStatus(),6e3),console.log(`[Eudia Update] Confirmed: now running v${e}`)),this.settings.pendingReloadVersion&&(this.settings.pendingReloadVersion===e?(this.settings.pendingReloadVersion=null,this.saveSettings(),console.log(`[Eudia Update] Pending reload resolved \u2014 now running v${e}`)):setTimeout(()=>this._showUpdateBanner(this.settings.pendingReloadVersion),3e3)),this.settings.pendingUpdateVersion){let n=this.settings.pendingUpdateVersion;this.settings.pendingUpdateVersion=null,this.saveSettings(),setTimeout(()=>{console.log(`[Eudia Update] Resuming deferred update to v${n}`),this.performAutoUpdate(this.settings.serverUrl||"https://gtm-wizard.onrender.com",n,this.manifest?.version||"0.0.0")},8e3)}setTimeout(()=>this.checkForPluginUpdate(),5e3),setTimeout(()=>this.checkForPluginUpdate(),18e4),this.registerInterval(window.setInterval(()=>this.checkForPluginUpdate(),10*60*1e3)),setTimeout(()=>this.pollVaultOperations(),15e3),this.registerInterval(window.setInterval(()=>this.pollVaultOperations(),60*1e3)),setTimeout(()=>this.healFailedTranscriptions(),3e4),this.registerView(z,n=>new oe(n,this)),this.registerView(B,n=>new se(n,this)),this.registerView(_,n=>new re(n,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.micRibbonIcon=this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.registerEvent(this.app.vault.on("create",async n=>{if(!(n instanceof p.TFile)||n.extension!=="md")return;let i=this.settings.accountsFolder||"Accounts";if(!n.path.startsWith(i+"/")||!n.basename.startsWith("Untitled"))return;let a=n.path.split("/");if(a.length<3)return;let s=a[1],o=a.slice(0,2).join("/"),l="",c=["Contacts.md","Note 1.md","Intelligence.md"];for(let C of c){let v=this.app.vault.getAbstractFileByPath(`${o}/${C}`);if(v instanceof p.TFile)try{let f=(await this.app.vault.read(v)).match(/account_id:\s*"?([^"\n]+)"?/);if(f){l=f[1].trim();break}}catch{}}let d=this.app.vault.getAbstractFileByPath(o),u=0;if(d&&d.children)for(let C of d.children){let v=C.name?.match(/^Note\s+(\d+)/i);v&&(u=Math.max(u,parseInt(v[1])))}let h=u+1,m=new Date,y=m.toLocaleDateString("en-US",{month:"short",day:"numeric"}),w=m.toISOString().split("T")[0],O=`Note ${h} - ${y}.md`,S=`---
account: "${s}"
account_id: "${l}"
type: meeting_note
sync_to_salesforce: false
...
`},{name:"Meeting Notes.md",content:S},{name:"Contacts.md",content:w},{name:"Intelligence.md",content:O},{name:"Next Steps.md",content:C}];for(let f of v){let x=`${u}/${f.name}`;await this.app.vault.create(x,f.content)}return console.log(`[Eudia] Created account folder with subnotes${y?" (enriched)":""}: ${d}`),!0}catch(m){return console.error(`[Eudia] Failed to create folder for ${d}:`,m),!1}},l=5;for(let c=0;c<t.length;c+=l){let d=t.slice(c,c+l),u=await Promise.allSettled(d.map(h=>o(h)));a+=u.filter(h=>h.status==="fulfilled"&&h.value===!0).length}a>0?(this.settings.cachedAccounts=t.map(c=>({id:c.id,name:c.name})),await this.saveSettings(),new p.Notice(`Created ${a} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let i=e?`
...
*Log key interactions and relationship developments.*
`}buildIntelligenceContent(t,e,n){let i=e?`
enriched_at: "${new Date().toISOString()}"`:"",a=`---
account: "${t.name}"
account_id: "${t.id}"
type: intelligence
sync_to_salesforce: false${i}
---`;return e?.intelligence?`${a}
...
---`,a=[];return e?.opportunities&&a.push(e.opportunities),e?.recentActivity&&a.push(e.recentActivity),a.length>0?`${i}
...
3. **Next Steps** are auto-extracted after transcription
4. Set \`sync_to_salesforce: true\` to sync to Salesforce
`}buildNextStepsContent(t,e,n){let i=n||new Date().toISOString().split("T")[0],a=e?`
...
*Previous next steps will be archived here.*
`}];for(let m of h){let y=`${o}/${m.name}`;await this.app.vault.create(y,m.content)}i++}catch(u){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,u)}}if(i>0){console.log(`[Eudia] Created ${i} prospect account folders in Accounts/`);let a=new Set((this.settings.cachedAccounts||[]).map(s=>s.id));for(let s of t)s.id&&!a.has(s.id)&&this.settings.cachedAccounts.push({id:s.id,name:s.name});await this.saveSettings()}return i}async migrateAccountStructure(){if(this._migrationInProgress)return;this._migrationInProgress=!0;let t=this.app.workspace.leftSplit,e=t?.collapsed;t&&!e&&t.collapse(),new p.Notice("Organizing your account folders\u2026",8e3);try{let n=this.settings.accountsFolder||"Accounts",i=`${n}/_Prospects`,a=this.app.vault.getAbstractFileByPath(i);if(a instanceof p.TFolder){let l=[...a.children],c=0;for(let u of l){if(!(u instanceof p.TFolder))continue;let h=`${n}/${u.name}`;if(this.app.vault.getAbstractFileByPath(h)instanceof p.TFolder){let y=[...u.children];for(let w of y){let O=`${h}/${w.name}`;if(!this.app.vault.getAbstractFileByPath(O))try{await this.app.fileManager.renameFile(w,O)}catch{}}try{await this.app.vault.delete(u,!0)}catch{}}else try{await this.app.fileManager.renameFile(u,h),c++}catch(y){console.warn(`[Eudia Migration] Failed to move ${u.name}:`,y)}}let d=this.app.vault.getAbstractFileByPath(i);if(d instanceof p.TFolder&&d.children.length===0)try{await this.app.vault.delete(d,!0)}catch{}c>0&&console.log(`[Eudia Migration] Moved ${c} prospect folders to Accounts/`)}let s=this.settings.userEmail;if(s&&!D(s)&&!H(s))try{let c=await new R(this.settings.serverUrl).getAccountsWithProspects(s),d=new Set([...c.accounts,...c.prospects].map(h=>h.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase())),u=this.app.vault.getAbstractFileByPath(n);if(u instanceof p.TFolder){let h=`${n}/_Other_Accounts`,m=0;for(let y of[...u.children]){if(!(y instanceof p.TFolder)||y.name.startsWith("_")||y.name.startsWith(".")||d.has(y.name.toLowerCase()))continue;if(!this.app.vault.getAbstractFileByPath(h))try{await this.app.vault.createFolder(h)}catch{}let w=`${h}/${y.name}`;if(!this.app.vault.getAbstractFileByPath(w))try{await this.app.fileManager.renameFile(y,w),m++}catch{}}m>0&&console.log(`[Eudia Migration] Archived ${m} non-owned accounts to _Other_Accounts/`)}this.settings.cachedAccounts=[...c.accounts,...c.prospects].map(h=>({id:h.id,name:h.name}))}catch(l){console.warn("[Eudia Migration] Could not fetch ownership \u2014 skipping archive step:",l)}let o=this.app.vault.getAbstractFileByPath(n);if(o instanceof p.TFolder){let l=new Set(o.children.filter(d=>d instanceof p.TFolder&&!d.name.startsWith("_")&&!d.name.startsWith(".")).map(d=>d.name.toLowerCase())),c=this.settings.cachedAccounts.length;this.settings.cachedAccounts=this.settings.cachedAccounts.filter(d=>{let u=d.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();return l.has(u)}),c!==this.settings.cachedAccounts.length&&console.log(`[Eudia Migration] Pruned cachedAccounts: ${c} \u2192 ${this.settings.cachedAccounts.length}`)}this.settings.prospectsMigrated=!0,await this.saveSettings(),console.log("[Eudia Migration] Account structure migration complete")}finally{t&&!e&&t.expand(),this._migrationInProgress=!1}}async createCSManagerDashboard(t,e){let n="CS Manager",i=new Date().toISOString().split("T")[0],a=fe(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s={};for(let u of e){let h=u.ownerName||"Unassigned";s[h]||(s[h]=[]),s[h].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
`;let c=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(c);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(c,o);for(let u of a){let h=u.split("@")[0].replace("."," ").replace(/\b\w/g,g=>g.toUpperCase()),m=u.split("@")[0].replace("."," ").toLowerCase(),y=m.split(" ")[0],w=m.split(" ").pop()||"",O=e.filter(g=>{let f=(g.csmName||"").toLowerCase();if(f&&(f.includes(y)||f.includes(w)))return!0;let x=(g.ownerName||"").toLowerCase();return x.includes(y)||x.includes(w)}),S=`---
rep: "${u}"
rep_name: "${h}"
role: cs_rep_summary
...
`;if(O.length>0){S+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let g of O){let f=g.name.replace(/[<>:"/\\|?*]/g,"_").trim();S+=`| ${g.name} | ${g.type||""} | ${g.ownerName||""} | [[Accounts/${f}/Contacts\\|View]] |
...
`;let C=`${n}/${h}.md`,v=this.app.vault.getAbstractFileByPath(C);v instanceof p.TFile?await this.app.vault.modify(v,S):await this.app.vault.create(C,S)}console.log(`[Eudia] Created CS Manager dashboard for ${t} with ${e.length} accounts across ${l.length} reps`)}async createAdminAccountFolders(t){let e=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),await this.ensurePipelineFolderExists();let i=0,a=0,s=new Date().toISOString().split("T")[0],o=async c=>{let d=c.name.replace(/[<>:"/\\|?*]/g,"_").trim(),u=`${e}/${d}`;if(this.app.vault.getAbstractFileByPath(u)instanceof p.TFolder)return!1;try{return await this.app.vault.createFolder(u),await this.createExecAccountSubnotes(u,c,s),c.isOwned?i++:a++,console.log(`[Eudia Admin] Created ${c.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(m){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,m),!1}},l=5;for(let c=0;c<t.length;c+=l){let d=t.slice(c,c+l);await Promise.allSettled(d.map(u=>o(u)))}this.settings.cachedAccounts=t.map(c=>({id:c.id,name:c.name})),await this.saveSettings(),i+a>0&&new p.Notice(`Created ${i} owned + ${a} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(t,e,n){let i=e.ownerName||"Unknown",a=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let s of a){let o=`${t}/${s.name}`;await this.app.vault.create(o,s.content)}}async createFullAccountSubnotes(t,e,n){let i=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let a of i){let s=`${t}/${a.name}`;await this.app.vault.create(s,a.content)}}async ensurePipelineFolderExists(){let t="Pipeline",e=`${t}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),!this.app.vault.getAbstractFileByPath(e)){let s=`---
...
### New Opportunities
*None yet*
`;await this.app.vault.create(e,s)}}async ensureNextStepsFolderExists(){let t="Next Steps",e=`${t}/All Next Steps.md`;if(this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),!this.app.vault.getAbstractFileByPath(e)){let a=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
...
`;await this.app.vault.create(e,o)}}async updateAccountNextSteps(t,e,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${t}`),console.log(`[Eudia] Content length: ${e?.length||0} chars`);let i=t.replace(/[<>:"/\\|?*]/g,"_").trim(),a=`${this.settings.accountsFolder}/${i}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${a}`);let s=this.app.vault.getAbstractFileByPath(a);if(!s||!(s instanceof p.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${a}`);let S=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${i}`);S&&S instanceof p.TFolder?console.log(`[Eudia] Files in ${i} folder:`,S.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${i}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],l=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),c=n.split("/").pop()?.replace(".md","")||"Meeting",d=e;!e.includes("- [ ]")&&!e.includes("- [x]")&&(d=e.split(`
`).filter(S=>S.trim()).map(S=>{let C=S.replace(/^[-•*]\s*/,"").trim();return C?`- [ ] ${C}`:""}).filter(Boolean).join(`
`));let u=await this.app.vault.read(s),h="",m=u.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);m&&m[1]&&(h=m[1].trim());let y=`### ${o} - ${c}
${d||"*None*"}`,w=h?`${y}
...
${w}
`;await this.app.vault.modify(s,O),console.log(`[Eudia] Updated Next Steps for ${t} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(i){console.error(`[Eudia] Failed to update Next Steps for ${t}:`,i)}}async regenerateNextStepsDashboard(){try{let e=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!e||!(e instanceof p.TFile)){await this.ensureNextStepsFolderExists();return}let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!n||!(n instanceof p.TFolder))return;let i=[];for(let l of n.children)if(l instanceof p.TFolder){let c=`${l.path}/Next Steps.md`,d=this.app.vault.getAbstractFileByPath(c);if(d instanceof p.TFile){let u=await this.app.vault.read(d),h=u.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),m=h?h[1]:"Unknown",y=u.split(`
`).filter(w=>w.match(/^- \[[ x]\]/)).slice(0,5);(y.length>0||m!=="Unknown")&&i.push({account:l.name,lastUpdated:m,nextSteps:y})}}i.sort((l,c)=>c.lastUpdated.localeCompare(l.lastUpdated));let a=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${a}
---
...
`}}await this.app.vault.modify(e,o),console.log("[Eudia] Regenerated All Next Steps dashboard")}catch(t){console.error("[Eudia] Failed to regenerate Next Steps dashboard:",t)}}async startRecording(){if(!I.isSupported()){new p.Notice("Audio transcription is not supported in this environment.");return}let t=await this.showTemplatePicker();if(!t)return;this.settings.meetingTemplate=t;try{(await navigator.mediaDevices.getUserMedia({audio:!0})).getTracks().forEach(s=>s.stop())}catch(a){this.showPermissionGuide(a);return}let e=this.settings.audioCaptureMode||"full_call",n=null;try{let s=(await I.getAvailableDevices()).find(o=>I.isHeadphoneDevice(o.label));s&&(n=s.label,console.log(`[Eudia] Headphones detected (${n}) \u2014 will still attempt system audio capture`),new p.Notice(`${n} detected \u2014 attempting full call capture...`,4e3))}catch{}if(!this.settings.audioSystemDeviceId)try{let a=await I.detectVirtualAudioDevice();a&&(this.settings.audioSystemDeviceId=a.deviceId,await this.saveSettings(),console.log(`[Eudia] Virtual audio device found: ${a.label}`))}catch{}let i=this.app.workspace.getActiveFile();if(i||(await this.createMeetingNote(),i=this.app.workspace.getActiveFile()),!i){new p.Notice("Please open or create a note first");return}this.audioRecorder=new I,this.recordingStatusBar=new ae(()=>this.audioRecorder?.pause(),()=>this.audioRecorder?.resume(),()=>this.stopRecording(),()=>this.cancelRecording());try{this.audioRecorder.onEvent(c=>{switch(c.type){case"deviceChanged":c.activeDeviceLost?new p.Notice("Recording device disconnected. Recording continues on available mic.",8e3):new p.Notice("Audio device changed. Recording continues.",4e3),console.log("[Eudia Telemetry] device_change",c);break;case"headphoneDetected":console.log("[Eudia Telemetry] headphone_detected",c.deviceLabel);break;case"silenceDetected":{let d=this.settings.audioCaptureMode||"full_call",u="Check that your microphone is working.";d==="full_call"&&(u="Ensure your call audio is playing through speakers, not headphones."),new p.Notice(`No audio detected for ${c.durationSeconds}s. ${u}`,1e4),console.log("[Eudia Telemetry] silence_detected",c.durationSeconds);break}case"audioRestored":new p.Notice("Audio signal restored.",3e3),console.log("[Eudia Telemetry] audio_restored");break}});let a=e,s={captureMode:a,micDeviceId:this.settings.audioMicDeviceId||void 0,systemAudioDeviceId:this.settings.audioSystemDeviceId||void 0};if(await this.audioRecorder.start(s),console.log("[Eudia Telemetry] recording_start",{captureMode:a,systemAudio:this.audioRecorder.getSystemAudioMethod()}),this.telemetry.reportRecordingStart({captureMode:a,systemAudioMethod:this.audioRecorder.getSystemAudioMethod(),hasMicPermission:!0}),a==="full_call"&&this.audioRecorder.getState().isRecording){let c=this.audioRecorder.getSystemAudioMethod();if(c==="electron"||c==="display_media"||c==="virtual_device"){let d=c==="virtual_device"?" (Virtual Device)":"";new p.Notice(`Recording \u2014 capturing both sides of the call${d}.`,5e3)}else{let d=n?`System audio capture unavailable with ${n}.
Recording your voice only. Switch to laptop speakers for both sides.`:`Recording (Mic only) \u2014 system audio capture unavailable.
Use laptop speakers, or try Settings > Audio Capture > Test System Audio.`;new p.Notice(d,1e4)}}else a==="mic_only"&&new p.Notice("Recording (Mic Only \u2014 your voice only)",3e3);this.recordingStatusBar.show(),this.micRibbonIcon?.addClass("eudia-ribbon-recording");try{let c=await this.calendarService.getCurrentMeeting();if(c.isNow&&c.meeting?.end){let d=new Date(c.meeting.end),u=new Date,h=d.getTime()-u.getTime();if(h>6e4&&h<54e5){let m=Math.round(h/6e4);new p.Notice(`Recording aligned to meeting \u2014 auto-stops in ${m} min`),setTimeout(async()=>{this.audioRecorder?.isRecording()&&(new p.Notice("Meeting ended \u2014 generating summary."),await this.stopRecording())},h)}}}catch(c){console.log("[Eudia] Could not detect meeting duration for auto-stop:",c)}let o=!1,l=setInterval(()=>{if(this.audioRecorder?.isRecording()){let c=this.audioRecorder.getState();if(this.recordingStatusBar?.updateState(c),c.duration>=2700&&!o){o=!0;let d=new class extends p.Modal{constructor(){super(...arguments);this.result=!0}onOpen(){let{contentEl:m}=this;m.createEl("h2",{text:"Still recording?"}),m.createEl("p",{text:"You have been recording for 45 minutes. Are you still in this meeting?"}),m.createEl("p",{text:"Recording will auto-stop at 90 minutes.",cls:"mod-warning"});let y=m.createDiv({cls:"modal-button-container"});y.createEl("button",{text:"Keep Recording",cls:"mod-cta"}).onclick=()=>{this.close()},y.createEl("button",{text:"Stop Recording"}).onclick=()=>{this.result=!1,this.close()}}onClose(){this.result||u.stopRecording()}}(this.app),u=this;d.open()}c.duration>=5400&&(new p.Notice("Recording stopped \u2014 maximum 90 minutes reached."),this.stopRecording(),clearInterval(l))}else clearInterval(l)},100);this.liveTranscript=""}catch(a){this.micRibbonIcon?.removeClass("eudia-ribbon-recording"),this.recordingStatusBar?.hide(),this.recordingStatusBar=null,this.audioRecorder=null;let s=a.message||"Failed to start recording";console.error("[Eudia Telemetry] recording_start_error",s),s.includes("Permission")||s.includes("NotAllowed")||s.includes("permission")?this.showPermissionGuide(a):new p.Notice(`Recording failed: ${s}`,1e4)}}showPermissionGuide(t){let e=i=>{let a=`x-apple.systempreferences:com.apple.preference.security?${i}`;try{let s=window.require?.("electron");s?.shell?.openExternal?s.shell.openExternal(a):window.open(a)}catch{window.open(a)}};new class extends p.Modal{onOpen(){this.renderInitial()}renderInitial(){let{contentEl:i}=this;i.empty(),i.createEl("h2",{text:"Microphone Access Required"}),i.createEl("p",{text:"Obsidian needs microphone permission to transcribe meetings."});let a=i.createDiv();a.style.cssText="margin:16px 0;padding:12px;background:var(--background-secondary);border-radius:8px;",a.createEl("p",{text:'1. Click "Open Microphone Settings" below'}),a.createEl("p",{text:"2. Find Obsidian in the list and toggle it ON"}),a.createEl("p",{text:'3. Click "Verify Permission" to confirm'});let s=i.createDiv({cls:"modal-button-container"});s.style.cssText="display:flex;flex-wrap:wrap;gap:8px;margin-top:16px;";let o=s.createEl("button",{text:"Open Microphone Settings",cls:"mod-cta"});o.onclick=()=>e("Privacy_Microphone");let l=s.createEl("button",{text:"Screen Recording Settings"});l.style.cssText="font-size:12px;",l.onclick=()=>e("Privacy_ScreenCapture");let c=s.createEl("button",{text:"Verify Permission"});c.onclick=async()=>{try{(await navigator.mediaDevices.getUserMedia({audio:!0})).getTracks().forEach(u=>u.stop()),new p.Notice("Microphone access confirmed!"),this.close()}catch{this.renderFailed()}},s.createEl("button",{text:"Close"}).onclick=()=>this.close()}renderFailed(){let{contentEl:i}=this;i.empty(),i.createEl("h2",{text:"Microphone Permission Not Detected"});let a=i.createDiv();a.style.cssText="margin:16px 0;padding:16px;background:var(--background-secondary);border-radius:8px;line-height:1.8;",a.createEl("p",{text:"Follow these exact steps:"}).style.fontWeight="600",a.createEl("p",{text:"1. Click the Apple menu () \u2192 System Settings"}),a.createEl("p",{text:'2. Click "Privacy & Security" in the left sidebar'}),a.createEl("p",{text:'3. Scroll down and click "Microphone"'}),a.createEl("p",{text:'4. Find "Obsidian" and toggle the switch ON'}),a.createEl("p",{text:"5. Quit and reopen Obsidian (Cmd+Q, then relaunch)"});let s=i.createDiv({cls:"modal-button-container"});s.style.cssText="display:flex;gap:8px;margin-top:16px;",s.createEl("button",{text:"Try Again",cls:"mod-cta"}).onclick=()=>this.renderInitial(),s.createEl("button",{text:"Close"}).onclick=()=>this.close()}onClose(){this.contentEl.empty()}}(this.app).open()}async stopRecording(){if(!this.audioRecorder?.isRecording())return;let t=this.app.workspace.getActiveFile();if(!t){new p.Notice("No active file to save transcription"),this.cancelRecording();return}this.recordingStatusBar?.showProcessing();try{let e=await this.audioRecorder.stop(),n={hasAudio:!0,averageLevel:0,silentPercent:0};try{let i=await I.analyzeAudioBlob(e.audioBlob);if(n=i,!i.hasAudio){let a;e.systemAudioMethod==="electron"||e.systemAudioMethod==="display_media"?a="System audio capture was active but no sound was detected. Check that the call app is playing audio.":e.captureMode==="full_call"?a="Make sure your call audio is playing through laptop speakers (not headphones).":a="Check that your microphone is working and has permission.",new p.Notice(`Recording appears silent. ${a} Open Settings > Audio Capture to test your setup.`,12e3)}}catch(i){console.warn("[Eudia] Pre-transcription audio check failed:",i)}this.telemetry.reportRecordingStop({durationSec:e.duration,blobSizeMB:Math.round(e.audioBlob.size/1024/1024*100)/100,avgAudioLevel:n.averageLevel,silentPercent:n.silentPercent,hasAudio:n.hasAudio,captureMode:e.captureMode,systemAudioMethod:e.systemAudioMethod}),await this.processRecording(e,t)}catch(e){new p.Notice(`Transcription failed: ${e.message}`)}finally{this.micRibbonIcon?.removeClass("eudia-ribbon-recording"),this.stopLiveTranscription(),this.closeLiveQuerySidebar(),this.recordingStatusBar?.hide(),this.recordingStatusBar=null,this.audioRecorder=null}}showTemplatePicker(){return new Promise(t=>{new class extends p.Modal{constructor(){super(...arguments);this.result=null}onOpen(){let{contentEl:i}=this;i.empty(),i.createEl("h3",{text:"Meeting Type"}),i.createEl("p",{text:"Select the template for this recording:",cls:"setting-item-description"});let a=i.createDiv();a.style.cssText="display:flex;flex-direction:column;gap:8px;margin-top:12px;";let s=[{key:"meddic",label:"Sales Discovery (MEDDIC)",desc:"Pain points, decision process, metrics, champions, budget signals"},{key:"demo",label:"Demo / Presentation",desc:"Feature reactions, questions, objections, interest signals"},{key:"cs",label:"Customer Success",desc:"Health signals, feature requests, adoption, renewal/expansion"},{key:"general",label:"General Check-In",desc:"Relationship updates, action items, sentiment"},{key:"internal",label:"Internal Call",desc:"Team sync, pipeline review, strategy discussion"}];for(let o of s){let l=a.createEl("button",{text:o.label});l.style.cssText="padding:10px 16px;text-align:left;cursor:pointer;border-radius:6px;border:1px solid var(--background-modifier-border);";let c=a.createEl("div",{text:o.desc});c.style.cssText="font-size:11px;color:var(--text-muted);margin-top:-4px;margin-bottom:4px;padding-left:4px;",l.onclick=()=>{this.result=o.key,this.close()}}}onClose(){t(this.result)}}(this.app).open()})}async cancelRecording(){this.audioRecorder?.isRecording()&&this.audioRecorder.cancel(),this.micRibbonIcon?.removeClass("eudia-ribbon-recording"),this.stopLiveTranscription(),this.closeLiveQuerySidebar(),this.recordingStatusBar?.hide(),this.recordingStatusBar=null,this.audioRecorder=null,new p.Notice("Transcription cancelled")}startLiveTranscription(){this.stopLiveTranscription();let t=12e4;this.liveTranscriptChunkInterval=setInterval(async()=>{await this.transcribeCurrentChunk()},t),setTimeout(async()=>{this.audioRecorder?.isRecording()&&await this.transcribeCurrentChunk()},3e4),console.log("[Eudia] Live transcription started")}stopLiveTranscription(){this.liveTranscriptChunkInterval&&(clearInterval(this.liveTranscriptChunkInterval),this.liveTranscriptChunkInterval=null),console.log("[Eudia] Live transcription stopped")}async transcribeCurrentChunk(){if(!this.audioRecorder?.isRecording()||this.isTranscribingChunk)return;let t=this.audioRecorder.extractNewChunks();if(!(!t||t.size<5e3)){this.isTranscribingChunk=!0,console.log(`[Eudia] Transcribing chunk: ${t.size} bytes`);try{let e=new FileReader,i=await new Promise((o,l)=>{e.onload=()=>{let d=e.result.split(",")[1];o(d)},e.onerror=l,e.readAsDataURL(t)}),a=this.audioRecorder.getMimeType(),s=await this.transcriptionService.transcribeChunk(i,a);s.success&&s.text&&(this.liveTranscript+=(this.liveTranscript?`

`:"")+s.text,console.log(`[Eudia] Chunk transcribed, total transcript length: ${this.liveTranscript.length}`))}catch(e){console.error("[Eudia] Chunk transcription error:",e)}finally{this.isTranscribingChunk=!1}}}openLiveQueryModal(){let t=new p.Modal(this.app);t.titleEl.setText("Query Live Transcript");let e=t.contentEl;e.addClass("eudia-live-query-modal"),e.createDiv({cls:"eudia-live-query-instructions"}).setText(`Ask a question about what has been discussed so far (${Math.round(this.liveTranscript.length/4)} words captured):`);let i=e.createEl("textarea",{cls:"eudia-live-query-input",attr:{placeholder:'e.g., "What did Tom say about pricing?" or "What were the main concerns raised?"',rows:"3"}}),a=e.createDiv({cls:"eudia-live-query-response"});a.style.display="none";let s=e.createEl("button",{text:"Ask",cls:"eudia-btn-primary"});s.addEventListener("click",async()=>{let o=i.value.trim();if(!o){new p.Notice("Please enter a question");return}s.disabled=!0,s.setText("Searching..."),a.style.display="block",a.setText("Searching transcript..."),a.addClass("eudia-loading");try{let l=await this.transcriptionService.liveQueryTranscript(o,this.liveTranscript,this.getAccountNameFromActiveFile());a.removeClass("eudia-loading"),l.success?a.setText(l.answer):(a.setText(l.error||"Failed to query transcript"),a.addClass("eudia-error"))}catch(l){a.removeClass("eudia-loading"),a.setText(`Error: ${l.message}`),a.addClass("eudia-error")}finally{s.disabled=!1,s.setText("Ask")}}),i.addEventListener("keydown",o=>{o.key==="Enter"&&!o.shiftKey&&(o.preventDefault(),s.click())}),t.open(),i.focus()}getAccountNameFromActiveFile(){let t=this.app.workspace.getActiveFile();if(!t)return;let e=t.path.match(/Accounts\/([^\/]+)\//i);if(e)return e[1]}async processRecording(t,e){let n=t.audioBlob?.size||0;if(console.log(`[Eudia] Audio blob size: ${n} bytes, duration: ${t.duration}s`),n<1e3){new p.Notice("Recording too short or no audio captured. Please try again.");return}try{let g=await I.analyzeAudioBlob(t.audioBlob);console.log(`[Eudia] Audio diagnostic: hasAudio=${g.hasAudio}, peak=${g.peakLevel}, silent=${g.silentPercent}%`),g.warning&&(console.warn(`[Eudia] Audio warning: ${g.warning}`),g.hasAudio?new p.Notice(`Warning: ${g.warning.split(":")[0]}`,5e3):new p.Notice("Warning: Audio appears to be silent. Transcription may not work correctly. Check your microphone settings.",8e3))}catch(g){console.warn("[Eudia] Audio diagnostic failed, continuing anyway:",g)}let i=new Date().toISOString().replace(/[:.]/g,"-").slice(0,19),a=t.audioBlob.type?.includes("mp4")?"mp4":"webm",s=await t.audioBlob.arrayBuffer(),o=n/1024/1024,l=this.settings.recordingsFolder||"Recordings",c="_backups",d=`${l}/recording-${i}.${a}`,u=`${c}/recording-${i}.${a}`,h=!1,m=!1;for(let g of[l,c])if(!this.app.vault.getAbstractFileByPath(g))try{await this.app.vault.createFolder(g)}catch{}for(let g=0;g<3;g++)try{await this.app.vault.createBinary(d,s),h=!0,console.log(`[Eudia] Audio saved: ${d} (${o.toFixed(1)}MB)`);break}catch(f){console.warn(`[Eudia] Primary save attempt ${g+1}/3 failed: ${f.message}`),g<2&&await new Promise(x=>setTimeout(x,5e3))}try{await this.app.vault.createBinary(u,s),m=!0,console.log(`[Eudia] Backup audio saved: ${u}`)}catch(g){console.warn(`[Eudia] Backup save failed: ${g.message}`)}if(h||m){let g=h?d:u;t._savedAudioPath=g,new p.Notice(`Audio saved to ${g}`);try{let f=await this.app.vault.read(e),x=f.indexOf("---",f.indexOf("---")+3);if(x>0){let E=f.substring(0,x);if(!E.includes("recording_path:")){let k=E+`recording_path: "${g}"
`;await this.app.vault.modify(e,k+f.substring(x))}}}catch(f){console.warn("[Eudia] Failed to write recording_path to frontmatter:",f.message)}}else console.error("[Eudia] CRITICAL: All audio save attempts failed \u2014 recording may be lost"),new p.Notice("WARNING: Could not save recording to disk. Audio exists only in memory for this transcription attempt.",15e3),this.telemetry.reportSafetyNetFailure({blobSizeMB:Math.round(o*100)/100,error:"Both primary and backup save failed",retryAttempt:3});let y=t.duration||0,O=Math.max(1,Math.ceil(y/600))*30+30,S=O<60?`~${O} seconds`:`~${Math.ceil(O/60)} minute${Math.ceil(O/60)>1?"s":""}`;new p.Notice(`Processing ${Math.ceil(y/60)} min recording. Should take ${S}.`);let C=await this.app.vault.read(e),v=`
...
---
`;await this.app.vault.modify(e,C+v),this.processTranscriptionAsync(t,e).catch(g=>{console.error("Background transcription failed:",g),new p.Notice(`Transcription failed: ${g.message}`)})}async processTranscriptionAsync(t,e){try{let n={},i=e.path.split("/");console.log(`[Eudia] Processing transcription for: ${e.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(i)}, accountsFolder: ${this.settings.accountsFolder}`);let a=i[0]==="Pipeline Meetings",s=!1;try{let g=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);g&&(s=/meeting_type:\s*pipeline_review/.test(g[1]))}catch{}if(!s&&a&&(s=!0),s){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let v="";try{let g=await(0,p.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});g.json?.success&&g.json?.context&&(v=g.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${v.length} chars)`))}catch(g){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",g)}n={meetingType:"pipeline_review",pipelineContext:v}}else if(i.length>=2&&i[0]===this.settings.accountsFolder){let v=i[1];console.log(`[Eudia] Detected account folder: ${v}`);let g=this.settings.cachedAccounts.find(f=>f.name.toLowerCase()===v.toLowerCase());g?(n={accountName:g.name,accountId:g.id,userEmail:this.settings.userEmail},console.log(`[Eudia] Found cached account: ${g.name} (${g.id})`)):(n={accountName:v,accountId:"",userEmail:this.settings.userEmail},console.log(`[Eudia] Account not in cache, using folder name: ${v}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let v=await this.calendarService.getCurrentMeeting();v.meeting?.attendees&&(o=v.meeting.attendees.map(g=>g.name||g.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let l=Date.now(),c=await this.transcriptionService.transcribeAudio(t.audioBlob,{...n,speakerHints:o,captureMode:t.captureMode,hasVirtualDevice:t.hasVirtualDevice,meetingTemplate:this.settings.meetingTemplate||"meddic"}),d=Math.round(t.audioBlob.size/1024/1024*100)/100,u=d>15;this.telemetry.reportTranscriptionResult({success:!!c.text?.trim(),isChunked:u,totalSizeMB:d,transcriptLength:c.text?.length||0,processingTimeSec:Math.round((Date.now()-l)/1e3),error:c.error});let h=v=>v?!!(v.summary?.trim()||v.nextSteps?.trim()):!1,m=c.sections;if(h(m)||c.text?.trim()&&(m=await this.transcriptionService.processTranscription(c.text,n)),!h(m)&&!c.text?.trim()){let g=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Processing your recording\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,"").replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,""),f=c.error,E=!f||f.includes("audio")||f.includes("microphone")?"No audio detected. Check your microphone settings.":f,k=t._savedAudioPath,W=k?`
Your recording was saved to **${k}** \u2014 you can retry transcription from there.`:"";await this.app.vault.modify(e,g+`

**Transcription failed:** ${E}${W}
`),new p.Notice(`Transcription failed: ${E}`,1e4);return}let y=await this.app.vault.read(e),w="",O=Math.max(y.indexOf(`---
**Processing your recording`),y.indexOf(`---
**Transcription in progress`));if(O>0){let v=y.indexOf("---"),g=v>=0?y.indexOf("---",v+3):-1;g>0&&g+3<O&&(w=y.substring(g+3,O).trim())}else{let v=y.indexOf("---"),g=v>=0?y.indexOf("---",v+3):-1;if(g>0){let f=y.substring(g+3).trim();f.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(w=f)}}try{let v="_backups";this.app.vault.getAbstractFileByPath(v)||await this.app.vault.createFolder(v);let g=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),f=`${v}/${e.name}_${g}.md`;await this.app.vault.create(f,y),console.log(`[Eudia] Backed up note to ${f}`)}catch(v){console.warn("[Eudia] Backup failed (non-critical):",v.message)}let S;if(s?S=this.buildPipelineNoteContent(m,c,e.path):S=this.buildNoteContent(m,c),w&&w.length>5){let v=S.indexOf("---",S.indexOf("---")+3);if(v>0){let g=S.substring(0,v+3),f=S.substring(v+3);S=g+`
...
---
`+f}}await this.app.vault.modify(e,S);let C=Math.floor(t.duration/60);new p.Notice(`Transcription complete (${C} min recording)`);try{if(!s){let v=m.nextSteps||m.actionItems;v&&n?.accountName&&await this.updateAccountNextSteps(n.accountName,v,e.path)}}catch(v){console.warn("[Eudia] Next Steps extraction failed (non-critical):",v.message)}try{this.settings.autoSyncAfterTranscription&&e&&await this._executeSyncToSalesforce(e)}catch(v){console.warn("[Eudia] Auto-sync failed (non-critical):",v.message)}}catch(n){try{let i=await this.app.vault.read(e);if(i.includes("## Summary")||i.includes(`## Next Steps
-`)||i.includes("## Key Discussion Points"))console.warn("[Eudia] Post-transcription step failed but content is intact:",n.message);else{let s=i.replace(/\n\n---\n\*\*Processing your recording\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,"").replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,""),o=t?._savedAudioPath,l=o?`
...
`),new p.Notice(`Transcription failed: ${n.message}`,1e4)}}catch{}}}buildPipelineNoteContent(t,e,n){let i=new Date,a=String(i.getMonth()+1).padStart(2,"0"),s=String(i.getDate()).padStart(2,"0"),o=String(i.getFullYear()).slice(-2),l=i.toISOString().split("T")[0],c=`${a}.${s}.${o}`,d=y=>y==null?"":Array.isArray(y)?y.map(String).join(`
`):typeof y=="object"?JSON.stringify(y,null,2):String(y),u=d(t.summary),h=e.transcript||e.text||"",m=`---
...
`;if(u)m+=u;else{let y=[t.painPoints,t.productInterest,t.nextSteps,t.actionItems].filter(Boolean).map(d).join(`

`);y?m+=y:m+="*Pipeline summary could not be generated. See transcript below.*"}return h&&(m+=`
...
`),m}buildNoteContent(t,e){let n=f=>f==null?"":Array.isArray(f)?f.map(x=>typeof x=="object"?x.category?`**${x.category}**: ${x.signal||x.insight||""}`:JSON.stringify(x):String(x)).join(`
`):typeof f=="object"?JSON.stringify(f):String(f),i=n(t.title)||"Meeting Notes",a=n(t.summary),s=n(t.discussionContext),o=n(t.keyQuotes),l=n(t.painPoints),c=n(t.productInterest),d=n(t.meddiccSignals),u=n(t.nextSteps),h=n(t.actionItems),m=n(t.keyDates),y=n(t.dealSignals),w=n(t.risksObjections),O=n(t.attendees||t.keyStakeholders),S=n(t.emailDraft),C=`---
title: "${i}"
...
`),y&&!y.includes("No significant deal signals")&&!y.includes("Not discussed")&&(C+=`## Deal Signals
...
`);let v=e.text||e.transcript||"",g=e.diarizedTranscript||"";return this.settings.appendTranscript&&(g||v)&&(C+=`---

## ${g?"Full Transcript (Speaker-Labeled)":"Full Transcript"}

${g||v}
`),C}openIntelligenceQuery(){new K(this.app,this).open()}openIntelligenceQueryForCurrentNote(){let t=this.app.workspace.getActiveFile(),e;if(t){let n=this.app.metadataCache.getFileCache(t)?.frontmatter;if(n?.account_id&&n?.account)e={id:n.account_id,name:n.account};else if(n?.account){let i=this.settings.cachedAccounts.find(a=>a.name.toLowerCase()===n.account.toLowerCase());i?e={id:i.id,name:i.name}:e={id:"",name:n.account}}else{let i=t.path.split("/");if(i.length>=2&&i[0]===this.settings.accountsFolder){let a=i[1],s=this.settings.cachedAccounts.find(o=>o.name.replace(/[<>:"/\\|?*]/g,"_").trim()===a);s?e={id:s.id,name:s.name}:e={id:"",name:a}}}}new K(this.app,this,e).open()}async syncAccounts(t=!1){t||new p.Notice("Syncing Salesforce accounts...");try{let n=(await(0,p.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!n.success||!n.accounts){t||new p.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=n.accounts.map(i=>({id:i.id,name:i.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),t||new p.Notice(`Synced ${n.accounts.length} accounts for matching`)}catch(e){t||new p.Notice(`Failed to sync accounts: ${e.message}`)}}async scanLocalAccountFolders(){try{let t=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!t||!(t instanceof p.TFolder))return;let e=[];for(let n of t.children)if(n instanceof p.TFolder)if(n.name==="_Prospects")for(let i of n.children)i instanceof p.TFolder&&e.push({id:`local-${i.name.replace(/\s+/g,"-").toLowerCase()}`,name:i.name});else n.name.startsWith("_")||e.push({id:`local-${n.name.replace(/\s+/g,"-").toLowerCase()}`,name:n.name});this.settings.cachedAccounts=e,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(t){console.error("Failed to scan local account folders:",t)}}async refreshAccountFolders(){if(!this.settings.userEmail)throw new Error("Please configure your email first");let t=new R(this.settings.serverUrl);if((await t.getAccountsForUser(this.settings.userEmail)).length===0)return console.log("[Eudia] No accounts found for user"),0;let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),i=[];if(n&&n instanceof p.TFolder)for(let o of n.children)o instanceof p.TFolder&&i.push(o.name);let a=await t.getNewAccounts(this.settings.userEmail,i);if(a.length===0)return console.log("[Eudia] All account folders exist"),0;console.log(`[Eudia] Creating ${a.length} new account folders`);let s=await this.fetchEnrichmentData(a);return await this.createTailoredAccountFolders(a,s),a.length}async checkAndConsumeSyncFlags(){if(!this.settings.userEmail)return;let t=encodeURIComponent(this.settings.userEmail.toLowerCase().trim()),e=this.settings.serverUrl||"https://gtm-wizard.onrender.com";try{let a=((await(0,p.requestUrl)({url:`${e}/api/admin/users/${t}/sync-flags`,method:"GET"})).json?.flags||[]).filter(o=>!o.consumed_at);if(a.length===0)return;console.log(`[Eudia] Found ${a.length} pending sync flag(s)`);let s=!1;for(let o of a)if(o.flag==="resync_accounts"){s=!0;let l=o.payload||{},c=l.added?.length||0,d=l.removed?.length||0;console.log(`[Eudia] Sync flag: resync_accounts (+${c} / -${d})`)}else o.flag==="update_plugin"?new p.Notice("A plugin update is available. Please visit gtm-wizard.onrender.com/fresh-install"):o.flag==="reset_setup"&&(console.log("[Eudia] Sync flag: reset_setup received"),this.settings.setupCompleted=!1,await this.saveSettings(),new p.Notice("Setup has been reset by admin. Please re-run the setup wizard."));if(s){console.log("[Eudia] Triggering account folder resync from sync flag..."),new p.Notice("Syncing account updates...");let o=await this.syncAccountFolders();o.success?new p.Notice(`Account sync complete: ${o.added} new, ${o.archived} archived`):console.log(`[Eudia] Account resync error: ${o.error}`)}try{await(0,p.requestUrl)({url:`${e}/api/admin/users/${t}/sync-flags/consume`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagIds:a.map(o=>o.id)})}),console.log(`[Eudia] Consumed ${a.length} sync flag(s)`)}catch{console.log("[Eudia] Failed to consume sync flags (will retry next startup)")}}catch{console.log("[Eudia] Sync flag check skipped (endpoint not available)")}}async syncAccountFolders(){if(!this.settings.userEmail)return{success:!1,added:0,archived:0,error:"No email configured"};let t=this.settings.userEmail.toLowerCase().trim();console.log(`[Eudia] Syncing account folders for: ${t}`);try{let e=await fetch(`${this.settings.serverUrl}/api/bl-accounts/${encodeURIComponent(t)}`);if(!e.ok){let A=await e.json().catch(()=>({}));throw new Error(A.error||`Server returned ${e.status}`)}let n=await e.json();if(!n.success||!n.accounts)throw new Error(n.error||"Invalid response from server");let i=n.meta?.userGroup||"bl",a=n.meta?.queryDescription||"accounts",s=n.meta?.region||null;console.log(`[Eudia] User group: ${i}, accounts: ${n.accounts.length} (${a})`),s&&console.log(`[Eudia] Sales Leader region: ${s}`);let o=n.accounts||[],l=n.prospectAccounts||[],c=o.length+l.length;console.log(`[Eudia] Server returned: ${o.length} active + ${l.length} prospects = ${c} total`);let d=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),u=new Map,h=`${this.settings.accountsFolder}/_Prospects`,m=this.app.vault.getAbstractFileByPath(h),y=new Map,w=new Map;if(d&&d instanceof p.TFolder)for(let A of d.children)A instanceof p.TFolder&&!A.name.startsWith("_")&&u.set(A.name.toLowerCase().trim(),A);if(m&&m instanceof p.TFolder)for(let A of m.children)A instanceof p.TFolder?y.set(A.name.toLowerCase().trim(),A):A instanceof p.TFile&&A.extension==="md"&&w.set(A.basename.toLowerCase().trim(),A);let O=new Set(o.map(A=>A.name.toLowerCase().trim())),S=o.filter(A=>{let T=A.name.toLowerCase().trim();return!u.has(T)}),C=l.filter(A=>{let T=A.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();return!y.has(T)&&!w.has(T)&&!u.has(A.name.toLowerCase().trim())}),v=[];for(let A of o){let T=A.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();(y.has(T)||w.has(T))&&!u.has(A.name.toLowerCase().trim())&&v.push(A)}let g=new Set([...o.map(A=>A.name.toLowerCase().trim()),...l.map(A=>A.name.toLowerCase().trim())]),f=[];if(i==="bl")for(let[A,T]of u.entries())g.has(A)||f.push(T);let x=0,E=0,k=0,W=0;if(v.length>0){console.log(`[Eudia] Promoting ${v.length} accounts from prospect to active`);for(let A of v){let T=A.name.replace(/[<>:"/\\|?*]/g,"_").trim(),j=y.get(T.toLowerCase()),$=w.get(T.toLowerCase());try{if(j){let L=`${this.settings.accountsFolder}/${T}`;await this.app.vault.rename(j,L),k++,new p.Notice(`${A.name} promoted to active`)}else if($){await this.app.vault.delete($);let L=[{id:A.id,name:A.name,type:A.customerType,isOwned:!0,hadOpportunity:!0}],we=await this.fetchEnrichmentData(L);await this.createTailoredAccountFolders(L,we),k++,new p.Notice(`${A.name} promoted to active -- full account folder created`)}}catch(L){console.error(`[Eudia] Failed to promote ${A.name}:`,L)}}}if(S.length>0){console.log(`[Eudia] Creating ${S.length} new active account folders for ${i}`);let A=new Set(v.map(j=>j.name.toLowerCase().trim())),T=S.filter(j=>!A.has(j.name.toLowerCase().trim()));if(T.length>0){let j=T.map($=>({id:$.id,name:$.name,type:$.customerType,isOwned:i==="bl",ownerName:$.ownerName,hadOpportunity:!0}));if(i==="admin"||i==="exec")await this.createAdminAccountFolders(j);else{let $=await this.fetchEnrichmentData(j);await this.createTailoredAccountFolders(j,$)}x=T.length}this.telemetry&&this.telemetry.reportInfo("Accounts synced - added",{count:x,userGroup:i,region:s||void 0})}return C.length>0&&i==="bl"&&(console.log(`[Eudia] Creating ${C.length} new prospect files`),W=await this.createProspectAccountFiles(C.map(A=>({id:A.id,name:A.name,type:"Prospect",hadOpportunity:!1,website:A.website,industry:A.industry})))),this.settings.archiveRemovedAccounts&&f.length>0&&(console.log(`[Eudia] Archiving ${f.length} removed account folders`),E=await this.archiveAccountFolders(f),this.telemetry&&this.telemetry.reportInfo("Accounts synced - archived",{count:E})),console.log(`[Eudia] Sync complete: ${x} active added, ${W} prospects added, ${k} promoted, ${E} archived (group: ${i})`),{success:!0,added:x+W+k,archived:E,userGroup:i}}catch(e){return console.error("[Eudia] Account sync error:",e),this.telemetry&&this.telemetry.reportError("Account sync failed",{error:e.message}),{success:!1,added:0,archived:0,error:e.message}}}async archiveAccountFolders(t){let e=0,n=`${this.settings.accountsFolder}/_Archived`;this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);for(let a of t)try{let s=`${n}/${a.name}`;if(this.app.vault.getAbstractFileByPath(s)){let d=new Date().toISOString().split("T")[0];await this.app.fileManager.renameFile(a,`${n}/${a.name}_${d}`)}else await this.app.fileManager.renameFile(a,s);let l=`${n}/${a.name}/_archived.md`,c=`---
...
`;try{await this.app.vault.create(l,c)}catch{}e++,console.log(`[Eudia] Archived: ${a.name}`)}catch(s){console.error(`[Eudia] Failed to archive ${a.name}:`,s)}return e}async syncSpecificNoteToSalesforce(t){let e=await this.app.vault.read(t),n=this.app.metadataCache.getFileCache(t)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let i=n.account_id,a=n.account;if(!i&&a){let s=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===a.toLowerCase());s&&(i=s.id)}if(!i){let s=t.path.split("/");if(s.length>=2&&s[0]===this.settings.accountsFolder){let o=s[1]==="_Prospects"&&s.length>=3?s[2]:s[1],l=this.settings.cachedAccounts.find(c=>c.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);l&&(i=l.id,a=l.name)}}if(!i)return{success:!1,error:`Could not determine account for ${t.path}`};try{let s=await(0,p.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:i,accountName:a,noteTitle:t.basename,notePath:t.path,content:e,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return s.json?.success?{success:!0}:{success:!1,error:s.json?.error||"Unknown error",authRequired:s.json?.authRequired}}catch(s){return{success:!1,error:s.message}}}async copyForSlack(){let t=this.app.workspace.getActiveFile();if(!t){new p.Notice("No note open to copy");return}try{let e=await this.app.vault.read(t),i=e.match(/^account:\s*"?([^"\n]+)"?/m)?.[1]||t.parent?.name||"Meeting",s=e.match(/^last_updated:\s*(\S+)/m)?.[1]||new Date().toISOString().split("T")[0],o="",l=e.match(/## Summary\n([\s\S]*?)(?=\n## |\n---|\Z)/);l&&(o=l[1].trim().split(`
...
\u2022 `));let u="",h=e.match(/"([^"]{20,120})"/);h&&(u=h[1]);let m=`*${i} \u2014 ${s}*
`;o&&(m+=`${o}
`),c&&(m+=`
*Next Steps:*
\u2022 ${c}
...
`;await this.app.vault.modify(t,l)}async updateObjectionNote(t,e){if(!e.objections||e.objections.length===0)return;let n=new Date().toISOString().split("T")[0],i=e.objections.slice(0,10).map(o=>{let l=o.handleRatePercent>=75?"\u2705 Strong":o.handleRatePercent>=50?"\u26A0\uFE0F Moderate":"\u274C Needs Work";return`| ${o.objection?.substring(0,40)||"--"}... | ${o.count||0} | ${o.handleRatePercent||0}% | ${l} |`}).join(`
`),a="";for(let o of e.objections.slice(0,5))if(o.bestResponses&&o.bestResponses.length>0){a+=`
...
`,a+=`**Best Responses:**
`;for(let l of o.bestResponses.slice(0,2))a+=`1. *"${l.response}"* - ${l.rep||"Team member"}
`;a+=`
`}let s=`---
type: analytics_dashboard
auto_refresh: true
...
`;await this.app.vault.modify(t,s)}async updateTeamPerformanceNote(t,e){if(!e)return;let n=new Date().toISOString().split("T")[0],i=s=>s>0?`\u2191 ${Math.abs(s).toFixed(1)}%`:s<0?`\u2193 ${Math.abs(s).toFixed(1)}%`:"--",a=`---
type: analytics_dashboard
auto_refresh: true
...
| Avg Score | ${e.avgScore?.toFixed(1)||"--"} | ${i(e.scoreTrend)} |
| Talk Ratio | ${e.avgTalkRatio?Math.round(e.avgTalkRatio*100):"--"}% | ${i(e.talkRatioTrend)} |
| Value Score | ${e.avgValueScore?.toFixed(1)||"--"} | ${i(e.valueScoreTrend)} |
...
`;await this.app.vault.modify(t,a)}};var ce=class extends p.PluginSettingTab{constructor(r,t){super(r,t),this.plugin=t}display(){let{containerEl:r}=this;r.empty(),r.createEl("h2",{text:"Eudia Sync & Scribe"}),r.createEl("h3",{text:"Your Profile"});let t=r.createDiv();t.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let e=t.createDiv();e.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let n=e.createSpan(),i=e.createSpan(),a=t.createDiv();a.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",a.setText("Connect with Salesforce to sync notes with your user attribution.");let s=t.createEl("button");s.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,l=async()=>{if(!this.plugin.settings.userEmail)return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",i.setText("Enter email above first"),s.setText("Setup Required"),s.disabled=!0,s.style.opacity="0.5",s.style.cursor="not-allowed",!1;s.disabled=!1,s.style.opacity="1",s.style.cursor="pointer";try{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",i.setText("Checking..."),(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",i.setText("Connected to Salesforce"),s.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",i.setText("Not connected"),s.setText("Connect to Salesforce"),!1)}catch{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",i.setText("Status unavailable"),s.setText("Connect to Salesforce"),!1}};new p.Setting(r).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(g=>g.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async f=>{let x=f.trim().toLowerCase();this.plugin.settings.userEmail=x,await this.plugin.saveSettings(),await l()})),new p.Setting(r).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(g=>{ve.forEach(f=>{g.addOption(f.value,f.label)}),g.setValue(this.plugin.settings.timezone),g.onChange(async f=>{this.plugin.settings.timezone=f,await this.plugin.saveSettings(),this.plugin.calendarService?.setTimezone(f),new p.Notice(`Timezone set to ${ve.find(x=>x.value===f)?.label||f}`)})}),r.createEl("h3",{text:"Salesforce Connection"}),r.appendChild(t);let c=()=>{o&&window.clearInterval(o);let g=0,f=30;o=window.setInterval(async()=>{g++,await l()?(o&&(window.clearInterval(o),o=null),new p.Notice("Salesforce connected successfully!")):g>=f&&o&&(window.clearInterval(o),o=null)},5e3)};s.onclick=async()=>{if(!this.plugin.settings.userEmail){new p.Notice("Please enter your email first");return}let g=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(g,"_blank"),new p.Notice("Complete the Salesforce login in the popup window",5e3),c()},l(),r.createEl("h3",{text:"Server"}),new p.Setting(r).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(g=>g.setValue(this.plugin.settings.serverUrl).onChange(async f=>{this.plugin.settings.serverUrl=f,await this.plugin.saveSettings()}));let d=r.createDiv({cls:"settings-advanced-collapsed"}),u=d.createDiv({cls:"eudia-transcription-status"});u.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",u.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,p.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?u.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':u.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{u.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let h=new p.Setting(r).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(g=>g.setValue(!1).onChange(f=>{d.style.display=f?"block":"none"}));d.style.display="none",r.createEl("h3",{text:"Audio Capture"});let m=r.createDiv();m.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 12px; line-height: 1.5;",m.setText(`Full Call mode automatically captures both sides of the call (your mic + the other person's audio). No extra software needed \u2014 the plugin uses native system audio capture. Run "Test System Audio Capture" from the command palette (Cmd+P) to verify your setup.`),new p.Setting(r).setName("Capture Mode").setDesc("Full Call captures both sides; Mic Only captures your voice only.").addDropdown(g=>{g.addOption("full_call","Full Call (Both Sides)"),g.addOption("mic_only","Mic Only"),g.setValue(this.plugin.settings.audioCaptureMode||"full_call"),g.onChange(async f=>{this.plugin.settings.audioCaptureMode=f,await this.plugin.saveSettings()})});let y=r.createDiv();y.style.cssText="padding: 10px 14px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",y.setText("Checking system audio capabilities...");let w=new p.Setting(r).setName("Microphone").setDesc("Select your physical microphone"),O=new p.Setting(r).setName("System Audio Device").setDesc("Override for system audio source (auto-detected \u2014 most users should leave this on Auto)"),S=r.createDiv();S.style.cssText="margin-bottom: 16px;";let C=S.createEl("button",{text:"Test Audio (3 seconds)"});C.style.cssText="padding: 8px 16px; cursor: pointer; border-radius: 6px;";let v=S.createDiv();v.style.cssText="font-size: 12px; margin-top: 6px; color: var(--text-muted);",(async()=>{try{try{(await navigator.mediaDevices.getUserMedia({audio:!0})).getTracks().forEach(k=>k.stop())}catch{}let g=await I.getAvailableDevices(),f=g.find(E=>E.isVirtual);if(f)y.innerHTML=`<span style="color:#22c55e;">&#10003;</span> System audio device detected: <strong>${f.label}</strong>`,this.plugin.settings.audioSystemDeviceId||(this.plugin.settings.audioSystemDeviceId=f.deviceId,await this.plugin.saveSettings());else{let E=await I.probeSystemAudioCapabilities();E.desktopCapturerAvailable||I.isHandlerReady()?y.innerHTML='<span style="color:#22c55e;">&#10003;</span> Native system audio capture available. Both sides of calls will be recorded automatically.':E.getDisplayMediaAvailable?y.innerHTML=`<span style="color:#3b82f6;">&#8505;</span> System audio capture ready. On first recording, macOS may ask for Screen Recording permission \u2014 this is how the plugin captures the other person's audio.`:y.innerHTML=`<span style="color:#f59e0b;">&#9888;</span> System audio not available (Electron ${E.electronVersion||"?"}). Run "Test System Audio Capture" from Cmd+P for details.`}let x=g.filter(E=>!E.isVirtual);w.addDropdown(E=>{E.addOption("","Default Microphone"),x.forEach(k=>E.addOption(k.deviceId,k.label)),E.setValue(this.plugin.settings.audioMicDeviceId||""),E.onChange(async k=>{this.plugin.settings.audioMicDeviceId=k,await this.plugin.saveSettings()})}),O.addDropdown(E=>{E.addOption("","Auto-detect / None"),g.filter(k=>k.isVirtual).forEach(k=>E.addOption(k.deviceId,k.label)),g.filter(k=>!k.isVirtual).forEach(k=>E.addOption(k.deviceId,`(mic) ${k.label}`)),E.setValue(this.plugin.settings.audioSystemDeviceId||""),E.onChange(async k=>{this.plugin.settings.audioSystemDeviceId=k,await this.plugin.saveSettings()})})}catch(g){y.setText("Could not enumerate audio devices."),console.warn("[Eudia Settings] Device enumeration failed:",g)}})(),C.onclick=async()=>{C.disabled=!0,C.setText("Recording..."),v.setText("");try{let g=new I;await g.start({captureMode:this.plugin.settings.audioCaptureMode||"full_call",micDeviceId:this.plugin.settings.audioMicDeviceId||void 0,systemAudioDeviceId:this.plugin.settings.audioSystemDeviceId||void 0}),await new Promise(W=>setTimeout(W,3e3));let f=await g.stop(),x=await I.analyzeAudioBlob(f.audioBlob),E={electron:"System Audio (Electron)",display_media:"System Audio (Screen Share)",virtual_device:"Virtual Device + Mic"},k=f.systemAudioMethod?E[f.systemAudioMethod]||"System Audio":f.captureMode==="full_call"?"Speaker Mode":"Mic Only";v.innerHTML=`<strong>${k}</strong> | Peak: ${x.peakLevel}% | Avg: ${x.averageLevel}% | Silent: ${x.silentPercent}%`+(x.warning?`<br><span style="color:#ef4444;">${x.warning}</span>`:'<br><span style="color:#22c55e;">Audio detected \u2014 recording should work.</span>')}catch(g){v.innerHTML=`<span style="color:#ef4444;">Test failed: ${g.message}</span>`}finally{C.disabled=!1,C.setText("Test Audio (3 seconds)")}},r.createEl("h3",{text:"Transcription"}),new p.Setting(r).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(g=>g.setValue(this.plugin.settings.saveAudioFiles).onChange(async f=>{this.plugin.settings.saveAudioFiles=f,await this.plugin.saveSettings()})),new p.Setting(r).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(g=>g.setValue(this.plugin.settings.appendTranscript).onChange(async f=>{this.plugin.settings.appendTranscript=f,await this.plugin.saveSettings()})),r.createEl("h3",{text:"Sync"}),new p.Setting(r).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(g=>g.setValue(this.plugin.settings.syncOnStartup).onChange(async f=>{this.plugin.settings.syncOnStartup=f,await this.plugin.saveSettings()})),new p.Setting(r).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(g=>g.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async f=>{this.plugin.settings.autoSyncAfterTranscription=f,await this.plugin.saveSettings()})),new p.Setting(r).setName("Auto-Sync Flagged Notes").setDesc("Periodically push notes with sync_to_salesforce: true to Salesforce").addToggle(g=>g.setValue(this.plugin.settings.sfAutoSyncEnabled).onChange(async f=>{this.plugin.settings.sfAutoSyncEnabled=f,await this.plugin.saveSettings(),f?this.plugin.startSalesforceSyncScanner():this.plugin.updateSfSyncStatusBar("SF Sync: Off")})),new p.Setting(r).setName("Auto-Sync Interval").setDesc("How often to scan for flagged notes (in minutes)").addDropdown(g=>{g.addOption("5","Every 5 minutes"),g.addOption("15","Every 15 minutes"),g.addOption("30","Every 30 minutes"),g.setValue(String(this.plugin.settings.sfAutoSyncIntervalMinutes)),g.onChange(async f=>{this.plugin.settings.sfAutoSyncIntervalMinutes=parseInt(f),await this.plugin.saveSettings(),new p.Notice(`SF auto-sync interval set to ${f} minutes. Restart Obsidian for changes to take effect.`)})}),r.createEl("h3",{text:"Folders"}),new p.Setting(r).setName("Accounts Folder").setDesc("Where account folders are stored").addText(g=>g.setValue(this.plugin.settings.accountsFolder).onChange(async f=>{this.plugin.settings.accountsFolder=f||"Accounts",await this.plugin.saveSettings()})),new p.Setting(r).setName("Recordings Folder").setDesc("Where audio files are saved").addText(g=>g.setValue(this.plugin.settings.recordingsFolder).onChange(async f=>{this.plugin.settings.recordingsFolder=f||"Recordings",await this.plugin.saveSettings()})),r.createEl("h3",{text:"Actions"}),new p.Setting(r).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(g=>g.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new p.Setting(r).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(g=>g.setButtonText("Refresh Folders").onClick(async()=>{g.setButtonText("Checking..."),g.setDisabled(!0);try{let f=await this.plugin.refreshAccountFolders();f>0?new p.Notice(`Created ${f} new account folder${f>1?"s":""}`):new p.Notice("All account folders are up to date")}catch(f){new p.Notice("Failed to refresh folders: "+f.message)}g.setButtonText("Refresh Folders"),g.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&r.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),r.createEl("p",{text:`Audio transcription: ${I.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
