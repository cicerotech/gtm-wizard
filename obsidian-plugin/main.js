...
- Action items have clear owners`}function Ie(S,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
...
`}function je(S,r){let t="";return r?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${r.account.name}
${r.account.owner?`- Owner: ${r.account.owner}`:""}
...
6. For strategic takeaways, only include implications that were actually discussed or clearly implied \u2014 do not speculate.`}function Pe(S){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${S?`
...
*Previous next steps will be archived here.*
`}];for(let h of m){let g=`${o}/${h.name}`;await this.app.vault.create(g,h.content)}i++}catch(u){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,u)}}if(i>0){console.log(`[Eudia] Created ${i} prospect account folders in Accounts/`);let a=new Set((this.settings.cachedAccounts||[]).map(s=>s.id));for(let s of t)s.id&&!a.has(s.id)&&this.settings.cachedAccounts.push({id:s.id,name:s.name});await this.saveSettings()}return i}async migrateAccountStructure(){if(this._migrationInProgress)return;this._migrationInProgress=!0;let t=this.app.workspace.leftSplit,e=t?.collapsed;t&&!e&&t.collapse(),new p.Notice("Organizing your account folders\u2026",8e3);try{let n=this.settings.accountsFolder||"Accounts",i=`${n}/_Prospects`,a=this.app.vault.getAbstractFileByPath(i);if(a instanceof p.TFolder){let o=[...a.children],l=0;for(let d of o){if(!(d instanceof p.TFolder))continue;let u=`${n}/${d.name}`;if(this.app.vault.getAbstractFileByPath(u)instanceof p.TFolder){let h=[...d.children];for(let g of h){let v=`${u}/${g.name}`;if(!this.app.vault.getAbstractFileByPath(v))try{await this.app.fileManager.renameFile(g,v)}catch{}}try{await this.app.vault.delete(d,!0)}catch{}}else try{await this.app.fileManager.renameFile(d,u),l++}catch(h){console.warn(`[Eudia Migration] Failed to move ${d.name}:`,h)}}let c=this.app.vault.getAbstractFileByPath(i);if(c instanceof p.TFolder&&c.children.length===0)try{await this.app.vault.delete(c,!0)}catch{}l>0&&console.log(`[Eudia Migration] Moved ${l} prospect folders to Accounts/`)}let s=this.settings.userEmail;if(s&&!F(s)&&!L(s))try{let l=await new R(this.settings.serverUrl).getAccountsWithProspects(s),c=new Set([...l.accounts,...l.prospects].map(u=>u.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase())),d=this.app.vault.getAbstractFileByPath(n);if(d instanceof p.TFolder){let u=`${n}/_Other_Accounts`,m=0;for(let h of[...d.children]){if(!(h instanceof p.TFolder)||h.name.startsWith("_")||h.name.startsWith(".")||c.has(h.name.toLowerCase()))continue;if(!this.app.vault.getAbstractFileByPath(u))try{await this.app.vault.createFolder(u)}catch{}let g=`${u}/${h.name}`;if(!this.app.vault.getAbstractFileByPath(g))try{await this.app.fileManager.renameFile(h,g),m++}catch{}}m>0&&console.log(`[Eudia Migration] Archived ${m} non-owned accounts to _Other_Accounts/`)}this.settings.cachedAccounts=[...l.accounts,...l.prospects].map(u=>({id:u.id,name:u.name}))}catch(o){console.warn("[Eudia Migration] Could not fetch ownership \u2014 skipping archive step:",o)}this.settings.prospectsMigrated=!0,await this.saveSettings(),console.log("[Eudia Migration] Account structure migration complete")}finally{t&&!e&&t.expand(),this._migrationInProgress=!1}}async createCSManagerDashboard(t,e){let n="CS Manager",i=new Date().toISOString().split("T")[0],a=fe(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s={};for(let u of e){let m=u.ownerName||"Unassigned";s[m]||(s[m]=[]),s[m].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
|---------|------|-------|--------|
`;for(let y of b){let w=y.name.replace(/[<>:"/\\|?*]/g,"_").trim();O+=`| ${y.name} | ${y.type||""} | ${y.ownerName||""} | [[Accounts/${w}/Contacts\\|View]] |
...
`).filter(v=>v.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||h!=="Unknown")&&i.push({account:l.name,lastUpdated:h,nextSteps:g})}}i.sort((l,c)=>c.lastUpdated.localeCompare(l.lastUpdated));let a=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${a}
---
...
**Transcription in progress`));if(b>0){let f=g.indexOf("---"),y=f>=0?g.indexOf("---",f+3):-1;y>0&&y+3<b&&(v=g.substring(y+3,b).trim())}else{let f=g.indexOf("---"),y=f>=0?g.indexOf("---",f+3):-1;if(y>0){let w=g.substring(y+3).trim();w.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(v=w)}}try{let f="_backups";this.app.vault.getAbstractFileByPath(f)||await this.app.vault.createFolder(f);let y=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),w=`${f}/${e.name}_${y}.md`;await this.app.vault.create(w,g),console.log(`[Eudia] Backed up note to ${w}`)}catch(f){console.warn("[Eudia] Backup failed (non-critical):",f.message)}let O;if(s?O=this.buildPipelineNoteContent(h,c,e.path):O=this.buildNoteContent(h,c),v&&v.length>5){let f=O.indexOf("---",O.indexOf("---")+3);if(f>0){let y=O.substring(0,f+3),w=O.substring(f+3);O=y+`
...
`),new p.Notice(`Transcription failed: ${n.message}`,1e4)}}catch{}}}buildPipelineNoteContent(t,e,n){let i=new Date,a=String(i.getMonth()+1).padStart(2,"0"),s=String(i.getDate()).padStart(2,"0"),o=String(i.getFullYear()).slice(-2),l=i.toISOString().split("T")[0],c=`${a}.${s}.${o}`,d=g=>g==null?"":Array.isArray(g)?g.map(String).join(`
