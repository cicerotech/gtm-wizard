...
`}function ke(){return`You are a business meeting analyst. You are analyzing an INTERNAL team call \u2014 not a customer-facing meeting.
...
5. If specific accounts, deals, or numbers are mentioned, capture them accurately.`}function We(O){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${O?`
...
`;let l=Math.round(r/5),c=Math.round(i/5);if(n+=`\`${"\u2588".repeat(l)}${"\u2591".repeat(20-l)}\` Rep
...
`;try{let v=await this.app.vault.create(u,g);await this.app.workspace.getLeaf().openFile(v);try{let f=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;f?.revealInFolder&&f.revealInFolder(v)}catch{}new p.Notice(`Created pipeline note: ${l}`),console.log(`[Eudia Pipeline] Created pipeline meeting note: ${u}`)}catch(v){console.error("[Eudia Pipeline] Failed to create pipeline note:",v),new p.Notice(`Could not create pipeline note: ${v.message||"Unknown error"}`)}}},X=class extends p.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null;this.micRibbonIcon=null;this.liveTranscript="";this.liveTranscriptChunkInterval=null;this.isTranscribingChunk=!1;this._updateRetryCount=0;this.sfSyncStatusBarEl=null;this.sfSyncIntervalId=null}async onload(){await this.loadSettings(),this.transcriptionService=new Y(this.settings.serverUrl),this.calendarService=new M(this.settings.serverUrl,this.settings.userEmail,this.settings.timezone||"America/New_York"),this.smartTagService=new J,j.setupDisplayMediaHandler().then(t=>{console.log(t?"[Eudia] System audio: loopback handler ready":"[Eudia] System audio: handler not available, will try other strategies on record")}).catch(()=>{}),setTimeout(()=>this.checkForPluginUpdate(),5e3),this.registerInterval(window.setInterval(()=>this.checkForPluginUpdate(),30*60*1e3)),setTimeout(()=>this.healFailedTranscriptions(),3e4),this.registerView(G,t=>new se(t,this)),this.registerView(L,t=>new ie(t,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.micRibbonIcon=this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.registerEvent(this.app.vault.on("create",async t=>{if(!(t instanceof p.TFile)||t.extension!=="md")return;let e=this.settings.accountsFolder||"Accounts";if(!t.path.startsWith(e+"/")||!t.basename.startsWith("Untitled"))return;let n=t.path.split("/");if(n.length<3)return;let s=n[1],r=n.slice(0,2).join("/"),i="",o=["Contacts.md","Note 1.md","Intelligence.md"];for(let f of o){let y=this.app.vault.getAbstractFileByPath(`${r}/${f}`);if(y instanceof p.TFile)try{let C=(await this.app.vault.read(y)).match(/account_id:\s*"?([^"\n]+)"?/);if(C){i=C[1].trim();break}}catch{}}let l=this.app.vault.getAbstractFileByPath(r),c=0;if(l&&l.children)for(let f of l.children){let y=f.name?.match(/^Note\s+(\d+)/i);y&&(c=Math.max(c,parseInt(y[1])))}let d=c+1,u=new Date,m=u.toLocaleDateString("en-US",{month:"short",day:"numeric"}),h=u.toISOString().split("T")[0],g=`Note ${d} - ${m}.md`,v=`---
account: "${s}"
account_id: "${i}"
type: meeting_note
sync_to_salesforce: false
...
`},{name:"Meeting Notes.md",content:y},{name:"Contacts.md",content:v},{name:"Intelligence.md",content:f},{name:"Next Steps.md",content:S}];for(let b of C){let x=`${u}/${b.name}`;await this.app.vault.create(x,b.content)}return console.log(`[Eudia] Created account folder with subnotes${g?" (enriched)":""}: ${d}`),!0}catch(h){return console.error(`[Eudia] Failed to create folder for ${d}:`,h),!1}},l=5;for(let c=0;c<t.length;c+=l){let d=t.slice(c,c+l),u=await Promise.allSettled(d.map(m=>o(m)));r+=u.filter(m=>m.status==="fulfilled"&&m.value===!0).length}r>0?(this.settings.cachedAccounts=t.map(c=>({id:c.id,name:c.name})),await this.saveSettings(),new p.Notice(`Created ${r} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let s=e?`
...
*Previous next steps will be archived here.*
`}];for(let f of v){let y=`${l}/${f.name}`;await this.app.vault.create(y,f.content)}r++}catch(g){console.log(`[Eudia] Failed to create prospect folder for ${i.name}:`,g)}}return r>0&&console.log(`[Eudia] Created ${r} prospect account folders in _Prospects/`),r}async createCSManagerDashboard(t,e){let n="CS Manager",s=new Date().toISOString().split("T")[0],r=he(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let i={};for(let u of e){let m=u.ownerName||"Unassigned";i[m]||(i[m]=[]),i[m].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
`,l=Object.keys(i).sort();for(let u of l){let m=i[u];o+=`### ${u} (${m.length} accounts)
...
`;let c=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(c);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(c,o);for(let u of r){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,w=>w.toUpperCase()),h=u.split("@")[0].replace("."," ").toLowerCase(),g=h.split(" ")[0],v=h.split(" ").pop()||"",f=e.filter(w=>{let b=(w.csmName||"").toLowerCase();if(b&&(b.includes(g)||b.includes(v)))return!0;let x=(w.ownerName||"").toLowerCase();return x.includes(g)||x.includes(v)}),y=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
...
`;if(f.length>0){y+=`| Account | Type | Owner | Folder |
|---------|------|-------|--------|
`;for(let w of f){let b=w.name.replace(/[<>:"/\\|?*]/g,"_").trim();y+=`| ${w.name} | ${w.type||""} | ${w.ownerName||""} | [[Accounts/${b}/Contacts\\|View]] |
`}}else y+=`*No accounts currently matched to this rep. Accounts will populate after connecting to Salesforce (Step 2).*
`;y+=`
---
...
`;let S=`${n}/${m}.md`,C=this.app.vault.getAbstractFileByPath(S);C instanceof p.TFile?await this.app.vault.modify(C,y):await this.app.vault.create(S,y)}console.log(`[Eudia] Created CS Manager dashboard for ${t} with ${e.length} accounts across ${l.length} reps`)}async createAdminAccountFolders(t){let e=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),await this.ensurePipelineFolderExists();let s=0,r=0,i=new Date().toISOString().split("T")[0],o=async c=>{let d=c.name.replace(/[<>:"/\\|?*]/g,"_").trim(),u=`${e}/${d}`;if(this.app.vault.getAbstractFileByPath(u)instanceof p.TFolder)return!1;try{return await this.app.vault.createFolder(u),await this.createExecAccountSubnotes(u,c,i),c.isOwned?s++:r++,console.log(`[Eudia Admin] Created ${c.isOwned?"owned":"view-only"} folder: ${d}`),!0}catch(h){return console.error(`[Eudia Admin] Failed to create folder for ${d}:`,h),!1}},l=5;for(let c=0;c<t.length;c+=l){let d=t.slice(c,c+l);await Promise.allSettled(d.map(u=>o(u)))}this.settings.cachedAccounts=t.map(c=>({id:c.id,name:c.name})),await this.saveSettings(),s+r>0&&new p.Notice(`Created ${s} owned + ${r} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(t,e,n){let s=e.ownerName||"Unknown",r=[{name:"Note 1.md",content:`---
account: "${e.name}"
account_id: "${e.id}"
type: meeting_note
sync_to_salesforce: false
...
${m}`:g,f=`---
account: "${t}"
account_id: "${this.settings.cachedAccounts.find(y=>y.name===t)?.id||""}"
type: next_steps
auto_updated: true
last_updated: ${o}
sync_to_salesforce: false
...
---
`;await this.app.vault.modify(e,l+c),this.processTranscriptionAsync(t,e).catch(d=>{console.error("Background transcription failed:",d),new p.Notice(`Transcription failed: ${d.message}`)})}async processTranscriptionAsync(t,e){try{let n={},s=e.path.split("/");console.log(`[Eudia] Processing transcription for: ${e.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(s)}, accountsFolder: ${this.settings.accountsFolder}`);let r=s[0]==="Pipeline Meetings",i=!1;try{let y=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);y&&(i=/meeting_type:\s*pipeline_review/.test(y[1]))}catch{}if(!i&&r&&(i=!0),i){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let f="";try{let y=await(0,p.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});y.json?.success&&y.json?.context&&(f=y.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${f.length} chars)`))}catch(y){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",y)}n={meetingType:"pipeline_review",pipelineContext:f}}else if(s.length>=2&&s[0]===this.settings.accountsFolder){let f=s[1];console.log(`[Eudia] Detected account folder: ${f}`);let y=this.settings.cachedAccounts.find(S=>S.name.toLowerCase()===f.toLowerCase());y?(n={accountName:y.name,accountId:y.id,userEmail:this.settings.userEmail},console.log(`[Eudia] Found cached account: ${y.name} (${y.id})`)):(n={accountName:f,accountId:"",userEmail:this.settings.userEmail},console.log(`[Eudia] Account not in cache, using folder name: ${f}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let f=await this.calendarService.getCurrentMeeting();f.meeting?.attendees&&(o=f.meeting.attendees.map(y=>y.name||y.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let l=await this.transcriptionService.transcribeAudio(t.audioBlob,{...n,speakerHints:o,captureMode:t.captureMode,hasVirtualDevice:t.hasVirtualDevice,meetingTemplate:this.settings.meetingTemplate||"meddic"}),c=f=>f?!!(f.summary?.trim()||f.nextSteps?.trim()):!1,d=l.sections;if(c(d)||l.text?.trim()&&(d=await this.transcriptionService.processTranscription(l.text,n)),!c(d)&&!l.text?.trim()){let y=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Processing your recording\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,"").replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,""),S=l.error,w=!S||S.includes("audio")||S.includes("microphone")?"No audio detected. Check your microphone settings.":S,b=t._savedAudioPath,x=b?`
Your recording was saved to **${b}** \u2014 you can retry transcription from there.`:"";await this.app.vault.modify(e,y+`
...
**Transcription in progress`));if(h>0){let f=u.indexOf("---"),y=f>=0?u.indexOf("---",f+3):-1;y>0&&y+3<h&&(m=u.substring(y+3,h).trim())}else{let f=u.indexOf("---"),y=f>=0?u.indexOf("---",f+3):-1;if(y>0){let S=u.substring(y+3).trim();S.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(m=S)}}try{let f="_backups";this.app.vault.getAbstractFileByPath(f)||await this.app.vault.createFolder(f);let y=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),S=`${f}/${e.name}_${y}.md`;await this.app.vault.create(S,u),console.log(`[Eudia] Backed up note to ${S}`)}catch(f){console.warn("[Eudia] Backup failed (non-critical):",f.message)}let g;if(i?g=this.buildPipelineNoteContent(d,l,e.path):g=this.buildNoteContent(d,l),m&&m.length>5){let f=g.indexOf("---",g.indexOf("---")+3);if(f>0){let y=g.substring(0,f+3),S=g.substring(f+3);g=y+`
...
`)}catch{}throw n}}buildPipelineNoteContent(t,e,n){let s=new Date,r=String(s.getMonth()+1).padStart(2,"0"),i=String(s.getDate()).padStart(2,"0"),o=String(s.getFullYear()).slice(-2),l=s.toISOString().split("T")[0],c=`${r}.${i}.${o}`,d=g=>g==null?"":Array.isArray(g)?g.map(String).join(`
`):typeof g=="object"?JSON.stringify(g,null,2):String(g),u=d(t.summary),m=e.transcript||e.text||"",h=`---
...
`;if(u)h+=u;else{let g=[t.painPoints,t.productInterest,t.nextSteps,t.actionItems].filter(Boolean).map(d).join(`

`);g?h+=g:h+="*Pipeline summary could not be generated. See transcript below.*"}return m&&(h+=`
...
`),l&&!l.includes("None explicitly")&&!l.includes("Not discussed")&&(S+=`## Pain Points

${l}

`),c&&!c.includes("None identified")&&!c.includes("No specific products")&&(S+=`## Product Interest
...
`),g&&!g.includes("No significant deal signals")&&!g.includes("Not discussed")&&(S+=`## Deal Signals
