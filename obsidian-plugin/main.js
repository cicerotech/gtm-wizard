...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof u.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let p of i){let y=p.split("@")[0].replace("."," ").replace(/\b\w/g,A=>A.toUpperCase()),m=p.split("@")[0].replace("."," ").toLowerCase(),h=m.split(" ")[0],f=m.split(" ").pop()||"",S=t.filter(A=>{let W=(A.csmName||"").toLowerCase();if(W&&(W.includes(h)||W.includes(f)))return!0;let x=(A.ownerName||"").toLowerCase();return x.includes(h)||x.includes(f)}),O=`---
rep: "${p}"
rep_name: "${y}"
role: cs_rep_summary
account_count: ${S.length}
created: ${r}
---
...
**Email:** ${p}
**CS Accounts:** ${S.length}

---

## Assigned Accounts

`;if(S.length>0){O+=`| Account | Type | Owner |
|---------|------|-------|
`;for(let A of S)O+=`| ${A.name} | ${A.type||""} | ${A.ownerName||""} |
...
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(h.length>0||m!=="Unknown")&&r.push({account:c.name,lastUpdated:m,nextSteps:h})}}r.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],a=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`):typeof f=="object"?JSON.stringify(f):String(f),r=n(e.title)||"Meeting Notes",i=n(e.summary),a=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),l=n(e.nextSteps),d=n(e.actionItems),p=n(e.keyDates),y=n(e.risksObjections),m=n(e.attendees),h=`---
title: "${r}"
