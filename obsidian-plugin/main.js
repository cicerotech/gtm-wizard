...
`},{name:"Meeting Notes.md",content:O},{name:"Contacts.md",content:f},{name:"Intelligence.md",content:b},{name:"Next Steps.md",content:C}];for(let w of v){let x=`${u}/${w.name}`;await this.app.vault.create(x,w.content)}return console.log(`[Eudia] Created account folder with subnotes${g?" (enriched)":""}: ${d}`),!0}catch(m){return console.error(`[Eudia] Failed to create folder for ${d}:`,m),!1}},l=5;for(let c=0;c<t.length;c+=l){let d=t.slice(c,c+l),u=await Promise.allSettled(d.map(h=>o(h)));a+=u.filter(h=>h.status==="fulfilled"&&h.value===!0).length}a>0?(this.settings.cachedAccounts=t.map(c=>({id:c.id,name:c.name})),await this.saveSettings(),new p.Notice(`Created ${a} account folders`)):console.warn(`[Eudia] createTailoredAccountFolders: 0 folders created out of ${t.length} accounts \u2014 not updating cachedAccounts`),await this.ensureNextStepsFolderExists()}buildContactsContent(t,e,n){let i=e?`
...
`;let c=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(c);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(c,o);for(let u of a){let h=u.split("@")[0].replace("."," ").replace(/\b\w/g,y=>y.toUpperCase()),m=u.split("@")[0].replace("."," ").toLowerCase(),g=m.split(" ")[0],f=m.split(" ").pop()||"",b=e.filter(y=>{let w=(y.csmName||"").toLowerCase();if(w&&(w.includes(g)||w.includes(f)))return!0;let x=(y.ownerName||"").toLowerCase();return x.includes(g)||x.includes(f)}),O=`---
rep: "${u}"
rep_name: "${h}"
role: cs_rep_summary
...
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||m!=="Unknown")&&i.push({account:l.name,lastUpdated:m,nextSteps:g})}}i.sort((l,c)=>c.lastUpdated.localeCompare(l.lastUpdated));let a=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${a}
---
...
`),new p.Notice(`Transcription failed: ${E}`,1e4);return}let g=await this.app.vault.read(e),f="",b=Math.max(g.indexOf(`---
**Processing your recording`),g.indexOf(`---
**Transcription in progress`));if(b>0){let v=g.indexOf("---"),y=v>=0?g.indexOf("---",v+3):-1;y>0&&y+3<b&&(f=g.substring(y+3,b).trim())}else{let v=g.indexOf("---"),y=v>=0?g.indexOf("---",v+3):-1;if(y>0){let w=g.substring(y+3).trim();w.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(f=w)}}try{let v="_backups";this.app.vault.getAbstractFileByPath(v)||await this.app.vault.createFolder(v);let y=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),w=`${v}/${e.name}_${y}.md`;await this.app.vault.create(w,g),console.log(`[Eudia] Backed up note to ${w}`)}catch(v){console.warn("[Eudia] Backup failed (non-critical):",v.message)}let O;if(s?O=this.buildPipelineNoteContent(m,c,e.path):O=this.buildNoteContent(m,c),f&&f.length>5){let v=O.indexOf("---",O.indexOf("---")+3);if(v>0){let y=O.substring(0,v+3),w=O.substring(v+3);O=y+`
...
`),f&&!f.includes("None raised")&&!f.includes("No objections")&&!f.includes("Not discussed")&&(C+=`## Risks and Objections
...
`).filter(f=>f.startsWith("-")||f.startsWith("\u2022")).slice(0,3).map(f=>f.replace(/^[-•]\s*/,"").trim()).join(`
`));let c="",d=e.match(/## Next Steps\n([\s\S]*?)(?=\n## |\n---|\Z)/);d&&(c=d[1].trim().split(`
`).filter(f=>f.startsWith("-")||f.startsWith("\u2022")).slice(0,3).map(f=>f.replace(/^[-•\s[\]x]*/,"").trim()).join(`
