...
`;try{let u=await this.app.vault.create(c,m);await this.app.workspace.getLeaf().openFile(u),new l.Notice(`Created: ${c}`)}catch(u){console.error("[Eudia Calendar] Failed to create note:",u),new l.Notice(`Could not create note: ${u.message||"Unknown error"}`)}}},$=class extends l.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null}async onload(){await this.loadSettings(),this.transcriptionService=new x(this.settings.serverUrl,this.settings.openaiApiKey),this.calendarService=new S(this.settings.serverUrl,this.settings.userEmail),this.smartTagService=new k,this.registerView(b,e=>new R(e,this)),this.registerView(I,e=>new L(e,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addCommand({id:"transcribe-meeting",name:"Transcribe Meeting",callback:async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}}),this.addCommand({id:"open-calendar",name:"Open Calendar",callback:()=>this.activateCalendarView()}),this.addCommand({id:"sync-accounts",name:"Sync Salesforce Accounts",callback:()=>this.syncAccounts()}),this.addCommand({id:"sync-note",name:"Sync Note to Salesforce",callback:()=>this.syncNoteToSalesforce()}),this.addCommand({id:"new-meeting-note",name:"New Meeting Note",callback:()=>this.createMeetingNote()}),this.addSettingTab(new F(this.app,this)),this.registerEditorSuggest(new M(this.app,this)),this.app.workspace.onLayoutReady(async()=>{if(this.settings.setupCompleted)this.settings.syncOnStartup&&(await this.scanLocalAccountFolders(),this.settings.showCalendarView&&this.settings.userEmail&&await this.activateCalendarView());else{await new Promise(t=>setTimeout(t,100));let e=document.querySelector(".modal-container .modal");if(e){let t=e.querySelector(".modal-close-button");t&&t.click()}await this.activateSetupView()}})}async onunload(){this.app.workspace.detachLeavesOfType(b)}async loadSettings(){this.settings=Object.assign({},Z,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async activateCalendarView(){let e=this.app.workspace,t=e.getLeavesOfType(b);if(t.length>0)e.revealLeaf(t[0]);else{let i=e.getRightLeaf(!1);i&&(await i.setViewState({type:b,active:!0}),e.revealLeaf(i))}}async activateSetupView(){let e=this.app.workspace,t=e.getLeavesOfType(I);if(t.length>0)e.revealLeaf(t[0]);else{let i=e.getLeaf(!0);i&&(await i.setViewState({type:I,active:!0}),e.revealLeaf(i))}}async createTailoredAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t);let a=0;for(let s of e){let r=s.name.replace(/[<>:"/\\|?*]/g,"_").trim(),o=`${t}/${r}`;if(this.app.vault.getAbstractFileByPath(o)instanceof l.TFolder){console.log(`[Eudia] Account folder already exists: ${r}`);continue}try{await this.app.vault.createFolder(o);let p=new Date().toISOString().split("T")[0],d=[{name:"Note 1.md",content:`---
account: "${s.name}"
account_id: "${s.id}"
type: meeting_note
sync_to_salesforce: false
...
`).filter(C=>C.trim()).map(C=>{let _=C.replace(/^[-•*]\s*/,"").trim();return _?`- [ ] ${_}`:""}).filter(Boolean).join(`
...
${u}`:h,v=`---
account: "${e}"
account_id: "${this.settings.cachedAccounts.find(C=>C.name===e)?.id||""}"
type: next_steps
auto_updated: true
last_updated: ${o}
sync_to_salesforce: false
