...
- Action items have clear owners`}function We(S,i){let t="";return i?.account&&(t=`
ACCOUNT CONTEXT:
...
`}function je(S,i){let t="";return i?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${i.account.name}
${i.account.owner?`- Owner: ${i.account.owner}`:""}
...
`}function xe(S){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${S?`
...
`;let c=Math.round(r/5),l=Math.round(a/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
...
`;try{let w=await this.app.vault.create(u,g);await this.app.workspace.getLeaf().openFile(w);try{let y=this.app.internalPlugins?.getPluginById?.("file-explorer")?.instance;y?.revealInFolder&&y.revealInFolder(w)}catch{}new p.Notice(`Created pipeline note: ${c}`),console.log(`[Eudia Pipeline] Created pipeline meeting note: ${u}`)}catch(w){console.error("[Eudia Pipeline] Failed to create pipeline note:",w),new p.Notice(`Could not create pipeline note: ${w.message||"Unknown error"}`)}}},J=class extends p.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null;this.micRibbonIcon=null;this.liveTranscript="";this.liveTranscriptChunkInterval=null;this.isTranscribingChunk=!1;this.sfSyncStatusBarEl=null;this.sfSyncIntervalId=null}async onload(){await this.loadSettings(),this.transcriptionService=new z(this.settings.serverUrl),this.calendarService=new F(this.settings.serverUrl,this.settings.userEmail,this.settings.timezone||"America/New_York"),this.smartTagService=new Y,this.checkForPluginUpdate(),this.registerInterval(window.setInterval(()=>this.checkForPluginUpdate(),2*60*60*1e3)),this.registerView(_,t=>new ae(t,this)),this.registerView(H,t=>new ne(t,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.micRibbonIcon=this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.registerEvent(this.app.vault.on("create",async t=>{if(!(t instanceof p.TFile)||t.extension!=="md")return;let e=this.settings.accountsFolder||"Accounts";if(!t.path.startsWith(e+"/")||!t.basename.startsWith("Untitled"))return;let n=t.path.split("/");if(n.length<3)return;let s=n[1],r=n.slice(0,2).join("/"),a="",o=["Contacts.md","Note 1.md","Intelligence.md"];for(let y of o){let f=this.app.vault.getAbstractFileByPath(`${r}/${y}`);if(f instanceof p.TFile)try{let b=(await this.app.vault.read(f)).match(/account_id:\s*"?([^"\n]+)"?/);if(b){a=b[1].trim();break}}catch{}}let c=this.app.vault.getAbstractFileByPath(r),l=0;if(c&&c.children)for(let y of c.children){let f=y.name?.match(/^Note\s+(\d+)/i);f&&(l=Math.max(l,parseInt(f[1])))}let d=l+1,u=new Date,m=u.toLocaleDateString("en-US",{month:"short",day:"numeric"}),h=u.toISOString().split("T")[0],g=`Note ${d} - ${m}.md`,w=`---
account: "${s}"
account_id: "${a}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let y of w){let f=`${c}/${y.name}`;await this.app.vault.create(f,y.content)}r++}catch(g){console.log(`[Eudia] Failed to create prospect folder for ${a.name}:`,g)}}return r>0&&console.log(`[Eudia] Created ${r} prospect account folders in _Prospects/`),r}async createCSManagerDashboard(t,e){let n="CS Manager",s=new Date().toISOString().split("T")[0],r=me(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let a={};for(let u of e){let m=u.ownerName||"Unassigned";a[m]||(a[m]=[]),a[m].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
`;let l=`${n}/CS Manager Overview.md`,d=this.app.vault.getAbstractFileByPath(l);d instanceof p.TFile?await this.app.vault.modify(d,o):await this.app.vault.create(l,o);for(let u of r){let m=u.split("@")[0].replace("."," ").replace(/\b\w/g,v=>v.toUpperCase()),h=u.split("@")[0].replace("."," ").toLowerCase(),g=h.split(" ")[0],w=h.split(" ").pop()||"",y=e.filter(v=>{let O=(v.csmName||"").toLowerCase();if(O&&(O.includes(g)||O.includes(w)))return!0;let W=(v.ownerName||"").toLowerCase();return W.includes(g)||W.includes(w)}),f=`---
rep: "${u}"
rep_name: "${m}"
role: cs_rep_summary
...
|---------|------|-------|--------|
`;for(let v of y){let O=v.name.replace(/[<>:"/\\|?*]/g,"_").trim();f+=`| ${v.name} | ${v.type||""} | ${v.ownerName||""} | [[Accounts/${O}/Contacts\\|View]] |
...
`).filter(w=>w.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||h!=="Unknown")&&s.push({account:c.name,lastUpdated:h,nextSteps:g})}}s.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let r=new Date().toISOString().split("T")[0],a=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${r}
---
...
\u2022 `));let u="",m=e.match(/"([^"]{20,120})"/);m&&(u=m[1]);let h=`*${s} \u2014 ${a}*
`;o&&(h+=`${o}
`),l&&(h+=`
*Next Steps:*
\u2022 ${l}
