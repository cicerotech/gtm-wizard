...
- Action items have clear owners`}function oe(h){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${h?`
...
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(p.length>0||m!=="Unknown")&&a.push({account:c.name,lastUpdated:m,nextSteps:p})}}a.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],r=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
`),m}buildNoteContent(e,t){let n=f=>f==null?"":Array.isArray(f)?f.map(v=>typeof v=="object"?v.category?`**${v.category}**: ${v.signal||v.insight||""}`:JSON.stringify(v):String(v)).join(`
`):typeof f=="object"?JSON.stringify(f):String(f),a=n(e.title)||"Meeting Notes",i=n(e.summary),r=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),l=n(e.nextSteps),u=n(e.actionItems),g=n(e.keyDates),y=n(e.risksObjections),m=n(e.attendees),p=`---
title: "${a}"
