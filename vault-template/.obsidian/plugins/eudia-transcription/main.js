...
- Action items have clear owners`}function We(O,i){let t="";return i?.account&&(t=`
ACCOUNT CONTEXT:
...
`}function Te(O,i){let t="";return i?.account&&(t=`
ACCOUNT CONTEXT:
- Account: ${i.account.name}
${i.account.owner?`- Owner: ${i.account.owner}`:""}
...
*Previous next steps will be archived here.*
`}];for(let b of v){let w=`${c}/${b.name}`;await this.app.vault.create(w,b.content)}r++}catch(g){console.log(`[Eudia] Failed to create prospect folder for ${s.name}:`,g)}}if(r>0){console.log(`[Eudia] Created ${r} prospect account folders in _Prospects/`);let s=new Set((this.settings.cachedAccounts||[]).map(o=>o.id));for(let o of t)o.id&&!s.has(o.id)&&this.settings.cachedAccounts.push({id:o.id,name:o.name});await this.saveSettings()}return r}async createCSManagerDashboard(t,e){let n="CS Manager",a=new Date().toISOString().split("T")[0],r=ye(t);if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let s={};for(let u of e){let h=u.ownerName||"Unassigned";s[h]||(s[h]=[]),s[h].push(u)}let o=`---
role: cs_manager
manager: "${t}"
...
`,c=Object.keys(s).sort();for(let u of c){let h=s[u];o+=`### ${u} (${h.length} accounts)
...
`).filter(v=>v.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||y!=="Unknown")&&a.push({account:c.name,lastUpdated:y,nextSteps:g})}}a.sort((c,l)=>l.lastUpdated.localeCompare(c.lastUpdated));let r=new Date().toISOString().split("T")[0],s=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${r}
---
...
---
`;await this.app.vault.modify(e,C+f),this.processTranscriptionAsync(t,e).catch(m=>{console.error("Background transcription failed:",m),new p.Notice(`Transcription failed: ${m.message}`)})}async processTranscriptionAsync(t,e){try{let n={},a=e.path.split("/");console.log(`[Eudia] Processing transcription for: ${e.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(a)}, accountsFolder: ${this.settings.accountsFolder}`);let r=a[0]==="Pipeline Meetings",s=!1;try{let m=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);m&&(s=/meeting_type:\s*pipeline_review/.test(m[1]))}catch{}if(!s&&r&&(s=!0),s){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let f="";try{let m=await(0,p.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});m.json?.success&&m.json?.context&&(f=m.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${f.length} chars)`))}catch(m){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",m)}n={meetingType:"pipeline_review",pipelineContext:f}}else if(a.length>=2&&a[0]===this.settings.accountsFolder){let f=a[1];console.log(`[Eudia] Detected account folder: ${f}`);let m=this.settings.cachedAccounts.find(S=>S.name.toLowerCase()===f.toLowerCase());m?(n={accountName:m.name,accountId:m.id,userEmail:this.settings.userEmail},console.log(`[Eudia] Found cached account: ${m.name} (${m.id})`)):(n={accountName:f,accountId:"",userEmail:this.settings.userEmail},console.log(`[Eudia] Account not in cache, using folder name: ${f}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let f=await this.calendarService.getCurrentMeeting();f.meeting?.attendees&&(o=f.meeting.attendees.map(m=>m.name||m.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let c=Date.now(),l=await this.transcriptionService.transcribeAudio(t.audioBlob,{...n,speakerHints:o,captureMode:t.captureMode,hasVirtualDevice:t.hasVirtualDevice,meetingTemplate:this.settings.meetingTemplate||"meddic"}),d=Math.round(t.audioBlob.size/1024/1024*100)/100,u=d>15;this.telemetry.reportTranscriptionResult({success:!!l.text?.trim(),isChunked:u,totalSizeMB:d,transcriptLength:l.text?.length||0,processingTimeSec:Math.round((Date.now()-c)/1e3),error:l.error});let h=f=>f?!!(f.summary?.trim()||f.nextSteps?.trim()):!1,y=l.sections;if(h(y)||l.text?.trim()&&(y=await this.transcriptionService.processTranscription(l.text,n)),!h(y)&&!l.text?.trim()){let m=(await this.app.vault.read(e)).replace(/\n\n---\n\*\*Processing your recording\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,"").replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away[^*]*\*\n---\n/g,""),S=l.error,E=!S||S.includes("audio")||S.includes("microphone")?"No audio detected. Check your microphone settings.":S,k=t._savedAudioPath,j=k?`
...
**Transcription in progress`));if(b>0){let f=g.indexOf("---"),m=f>=0?g.indexOf("---",f+3):-1;m>0&&m+3<b&&(v=g.substring(m+3,b).trim())}else{let f=g.indexOf("---"),m=f>=0?g.indexOf("---",f+3):-1;if(m>0){let S=g.substring(m+3).trim();S.replace(/^#.*$/gm,"").replace(/Date:\s*\nAttendees:\s*/g,"").replace(/Add meeting notes here\.\.\./g,"").replace(/---/g,"").trim().length>10&&(v=S)}}try{let f="_backups";this.app.vault.getAbstractFileByPath(f)||await this.app.vault.createFolder(f);let m=new Date().toISOString().replace(/[:.]/g,"-").substring(0,19),S=`${f}/${e.name}_${m}.md`;await this.app.vault.create(S,g),console.log(`[Eudia] Backed up note to ${S}`)}catch(f){console.warn("[Eudia] Backup failed (non-critical):",f.message)}let w;if(s?w=this.buildPipelineNoteContent(y,l,e.path):w=this.buildNoteContent(y,l),v&&v.length>5){let f=w.indexOf("---",w.indexOf("---")+3);if(f>0){let m=w.substring(0,f+3),S=w.substring(f+3);w=m+`
...
\u2022 `));let u="",h=e.match(/"([^"]{20,120})"/);h&&(u=h[1]);let y=`*${a} \u2014 ${s}*
`;o&&(y+=`${o}
`),l&&(y+=`
*Next Steps:*
\u2022 ${l}
