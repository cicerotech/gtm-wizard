var de=Object.create;var R=Object.defineProperty;var le=Object.getOwnPropertyDescriptor;var ue=Object.getOwnPropertyNames;var pe=Object.getPrototypeOf,me=Object.prototype.hasOwnProperty;var he=(g,a)=>{for(var e in a)R(g,e,{get:a[e],enumerable:!0})},te=(g,a,e,t)=>{if(a&&typeof a=="object"||typeof a=="function")for(let n of ue(a))!me.call(g,n)&&n!==e&&R(g,n,{get:()=>a[n],enumerable:!(t=le(a,n))||t.enumerable});return g};var ne=(g,a,e)=>(e=g!=null?de(pe(g)):{},te(a||!g||!g.__esModule?R(e,"default",{value:g,enumerable:!0}):e,g)),ye=g=>te(R({},"__esModule",{value:!0}),g);var Ie={};he(Ie,{default:()=>U});module.exports=ye(Ie);var l=require("obsidian");var E=class g{constructor(){this.mediaRecorder=null;this.audioChunks=[];this.stream=null;this.startTime=0;this.pausedDuration=0;this.pauseStartTime=0;this.durationInterval=null;this.audioContext=null;this.analyser=null;this.levelInterval=null;this.lastExtractedChunkIndex=0;this.mimeTypeCache="audio/webm";this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0};this.stateCallback=null;this.levelHistory=[];this.trackingLevels=!1}onStateChange(a){this.stateCallback=a}static isIOSOrSafari(){let a=navigator.userAgent,e=/iPad|iPhone|iPod/.test(a)&&!window.MSStream,t=/^((?!chrome|android).)*safari/i.test(a);return e||t}getSupportedMimeType(){let a=g.isIOSOrSafari(),e=a?["audio/mp4","audio/mp4;codecs=aac","audio/aac","audio/webm;codecs=opus","audio/webm"]:["audio/webm;codecs=opus","audio/webm","audio/mp4","audio/ogg;codecs=opus","audio/ogg"];for(let t of e)if(MediaRecorder.isTypeSupported(t))return console.log(`[AudioRecorder] Using MIME type: ${t} (iOS/Safari: ${a})`),t;return a?"audio/mp4":"audio/webm"}async startRecording(){if(this.state.isRecording)throw new Error("Already recording");try{let a=g.isIOSOrSafari(),e=a?{echoCancellation:!0,noiseSuppression:!0}:{echoCancellation:!0,noiseSuppression:!0,autoGainControl:!0,sampleRate:48e3,channelCount:1};this.stream=await navigator.mediaDevices.getUserMedia({audio:e}),console.log(`[AudioRecorder] Microphone access granted (iOS/Safari: ${a})`),this.setupAudioAnalysis();let t=this.getSupportedMimeType();this.mimeTypeCache=t,this.mediaRecorder=new MediaRecorder(this.stream,{mimeType:t,audioBitsPerSecond:96e3}),this.audioChunks=[],this.lastExtractedChunkIndex=0,this.mediaRecorder.ondataavailable=n=>{n.data.size>0&&this.audioChunks.push(n.data)},this.mediaRecorder.start(1e3),this.startTime=Date.now(),this.pausedDuration=0,this.state={isRecording:!0,isPaused:!1,duration:0,audioLevel:0},this.startDurationTracking(),this.startLevelTracking(),this.notifyStateChange()}catch(a){throw this.cleanup(),new Error(`Failed to start recording: ${a.message}`)}}setupAudioAnalysis(){if(this.stream)try{this.audioContext=new AudioContext;let a=this.audioContext.createMediaStreamSource(this.stream);this.analyser=this.audioContext.createAnalyser(),this.analyser.fftSize=256,a.connect(this.analyser)}catch(a){console.warn("Failed to set up audio analysis:",a)}}startDurationTracking(){this.durationInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused){let a=Date.now()-this.startTime-this.pausedDuration;this.state.duration=Math.floor(a/1e3),this.notifyStateChange()}},100)}startLevelTracking(){if(!this.analyser)return;let a=new Uint8Array(this.analyser.frequencyBinCount);this.levelInterval=setInterval(()=>{if(this.state.isRecording&&!this.state.isPaused&&this.analyser){this.analyser.getByteFrequencyData(a);let e=0;for(let n=0;n<a.length;n++)e+=a[n];let t=e/a.length;this.state.audioLevel=Math.min(100,Math.round(t/255*100*2)),this.notifyStateChange()}},50)}pauseRecording(){!this.state.isRecording||this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="recording"&&(this.mediaRecorder.pause(),this.pauseStartTime=Date.now(),this.state.isPaused=!0,this.notifyStateChange())}resumeRecording(){!this.state.isRecording||!this.state.isPaused||this.mediaRecorder&&this.mediaRecorder.state==="paused"&&(this.mediaRecorder.resume(),this.pausedDuration+=Date.now()-this.pauseStartTime,this.state.isPaused=!1,this.notifyStateChange())}async stopRecording(){return new Promise((a,e)=>{if(!this.mediaRecorder||!this.state.isRecording){e(new Error("Not currently recording"));return}let t=this.mediaRecorder.mimeType,n=this.state.duration,s=!1,i=setTimeout(()=>{if(!s){s=!0,console.warn("AudioRecorder: onstop timeout, forcing completion");try{let r=new Blob(this.audioChunks,{type:t}),o=new Date,c=o.toISOString().split("T")[0],d=o.toTimeString().split(" ")[0].replace(/:/g,"-"),u=t.includes("webm")?"webm":t.includes("mp4")?"m4a":t.includes("ogg")?"ogg":"webm",h=`recording-${c}-${d}.${u}`;this.cleanup(),a({audioBlob:r,duration:n,mimeType:t,filename:h})}catch{this.cleanup(),e(new Error("Failed to process recording after timeout"))}}},1e4);this.mediaRecorder.onstop=()=>{if(!s){s=!0,clearTimeout(i);try{console.log(`[AudioRecorder] Chunks collected: ${this.audioChunks.length}`);let r=new Blob(this.audioChunks,{type:t});console.log(`[AudioRecorder] Blob size: ${r.size} bytes`);let o=new Date,c=o.toISOString().split("T")[0],d=o.toTimeString().split(" ")[0].replace(/:/g,"-"),u=t.includes("webm")?"webm":t.includes("mp4")?"m4a":t.includes("ogg")?"ogg":"webm",h=`recording-${c}-${d}.${u}`;this.cleanup(),a({audioBlob:r,duration:n,mimeType:t,filename:h})}catch(r){this.cleanup(),e(r)}}},this.mediaRecorder.onerror=r=>{s||(s=!0,clearTimeout(i),this.cleanup(),e(new Error("Recording error occurred")))},this.mediaRecorder.state==="recording"&&this.mediaRecorder.requestData(),setTimeout(()=>{this.mediaRecorder&&this.mediaRecorder.state!=="inactive"&&this.mediaRecorder.stop()},100)})}cancelRecording(){this.cleanup()}cleanup(){this.durationInterval&&(clearInterval(this.durationInterval),this.durationInterval=null),this.levelInterval&&(clearInterval(this.levelInterval),this.levelInterval=null),this.audioContext&&(this.audioContext.close().catch(()=>{}),this.audioContext=null,this.analyser=null),this.stream&&(this.stream.getTracks().forEach(a=>a.stop()),this.stream=null),this.mediaRecorder=null,this.audioChunks=[],this.state={isRecording:!1,isPaused:!1,duration:0,audioLevel:0},this.notifyStateChange()}getState(){return{...this.state}}static isSupported(){if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||!window.MediaRecorder)return!1;let e=["audio/webm","audio/mp4","audio/ogg","audio/webm;codecs=opus"].some(t=>MediaRecorder.isTypeSupported(t));return e||console.warn("[AudioRecorder] No supported audio formats found"),e}static getMobileInstructions(){return this.isIOSOrSafari()?"For best results on iOS, ensure you have granted microphone permissions in Settings > Privacy > Microphone.":null}notifyStateChange(){this.stateCallback&&this.stateCallback({...this.state})}static formatDuration(a){let e=Math.floor(a/60),t=a%60;return`${e.toString().padStart(2,"0")}:${t.toString().padStart(2,"0")}`}static async blobToBase64(a){return new Promise((e,t)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];e(i)},n.onerror=t,n.readAsDataURL(a)})}static async blobToArrayBuffer(a){return a.arrayBuffer()}async start(){return this.startRecording()}async stop(){return this.stopRecording()}pause(){return this.pauseRecording()}resume(){return this.resumeRecording()}cancel(){return this.cancelRecording()}isRecording(){return this.state.isRecording}extractNewChunks(){if(!this.state.isRecording||this.audioChunks.length===0)return null;let a=this.audioChunks.slice(this.lastExtractedChunkIndex);return a.length===0?null:(this.lastExtractedChunkIndex=this.audioChunks.length,new Blob(a,{type:this.mimeTypeCache}))}getAllChunksAsBlob(){return this.audioChunks.length===0?null:new Blob(this.audioChunks,{type:this.mimeTypeCache})}getDuration(){return this.state.duration}getMimeType(){return this.mimeTypeCache}startLevelHistoryTracking(){this.levelHistory=[],this.trackingLevels=!0}recordLevelSample(){this.trackingLevels&&this.levelHistory.push(this.state.audioLevel)}getAudioDiagnostic(){if(this.levelHistory.length===0)return{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:100,warning:"Unable to analyze audio levels - recording may be too short"};let a=this.levelHistory.reduce((i,r)=>i+r,0)/this.levelHistory.length,e=Math.max(...this.levelHistory),t=this.levelHistory.filter(i=>i<5).length,n=Math.round(t/this.levelHistory.length*100),s=null;return e<5?s="SILENT AUDIO: No audio was detected during recording. Check your microphone settings and ensure Obsidian has microphone permission.":a<10&&n>80?s="VERY LOW AUDIO: Audio levels were extremely low. The transcription may not be accurate. Check your microphone or move closer to it.":n>90&&(s="MOSTLY SILENT: Over 90% of the recording had no audio. Make sure you're capturing the meeting audio, not just silence."),{hasAudio:e>=5,averageLevel:Math.round(a),peakLevel:e,silentPercent:n,warning:s}}static async analyzeAudioBlob(a){try{let e=new AudioContext,t=await a.arrayBuffer(),n;try{n=await e.decodeAudioData(t)}catch{return await e.close(),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:"Could not analyze audio format. Proceeding with transcription."}}let s=n.getChannelData(0),i=0,r=0,o=0,c=.01,d=100,u=0;for(let w=0;w<s.length;w+=d){let S=Math.abs(s[w]);i+=S,S>r&&(r=S),S<c&&o++,u++}await e.close();let h=i/u,y=Math.round(o/u*100),m=Math.round(h*100*10),p=Math.round(r*100),f=null;return r<.01?f='SILENT AUDIO DETECTED: The recording appears to contain only silence. This typically causes Whisper to hallucinate random text like "Yes. Yes. Yes." Check your audio input source.':h<.005&&y>95?f="NEAR-SILENT AUDIO: The recording is almost entirely silent. The transcription will likely be inaccurate.":y>90&&(f="MOSTLY SILENT: Over 90% of the recording is silent. Consider checking your audio setup."),{hasAudio:r>=.01,averageLevel:m,peakLevel:p,silentPercent:y,warning:f}}catch(e){return console.error("Audio analysis failed:",e),{hasAudio:!0,averageLevel:0,peakLevel:0,silentPercent:0,warning:null}}}};var k=require("obsidian");var V=class{constructor(){this.salesforceAccounts=[]}setAccounts(a){this.salesforceAccounts=a}detectAccount(a,e,t){if(a){let n=this.detectFromTitle(a);if(n.confidence>=70)return n}if(t){let n=this.detectFromFilePath(t);if(n.confidence>=70)return n}if(e&&e.length>0){let n=this.detectFromAttendees(e);if(n.confidence>=50)return n}return{account:null,accountId:null,confidence:0,source:"none",evidence:"No account detected from available context"}}detectFromTitle(a){if(!a)return{account:null,accountId:null,confidence:0,source:"title",evidence:"No title"};let e=[{regex:/^([A-Za-z0-9][^-–—]+?)\s*[-–—]\s*(?:[A-Z][a-z]+|[A-Za-z]{2,})/,confidence:85},{regex:/(?:call|meeting|sync|check-in|demo|discovery)\s+(?:with|re:?|@)\s+([^-–—]+?)(?:\s*[-–—]|$)/i,confidence:80},{regex:/^([A-Za-z][^-–—]+?)\s+(?:discovery|demo|review|kickoff|intro|onboarding|sync)\s*(?:call)?$/i,confidence:75},{regex:/^([^:]+?):\s+/i,confidence:70},{regex:/^\[([^\]]+)\]/,confidence:75}],t=["weekly","daily","monthly","internal","team","1:1","one on one","standup","sync","meeting","call","notes","monday","tuesday","wednesday","thursday","friday","untitled","new","test"];for(let n of e){let s=a.match(n.regex);if(s&&s[1]){let i=s[1].trim();if(t.some(o=>i.toLowerCase()===o)||i.length<2)continue;let r=this.fuzzyMatchSalesforce(i);return r?{account:r.name,accountId:r.id,confidence:Math.min(n.confidence+10,100),source:"salesforce_match",evidence:`Matched "${i}" from title to Salesforce account "${r.name}"`}:{account:i,accountId:null,confidence:n.confidence,source:"title",evidence:"Extracted from meeting title pattern"}}}return{account:null,accountId:null,confidence:0,source:"title",evidence:"No pattern matched"}}detectFromFilePath(a){let e=a.match(/Accounts\/([^\/]+)\//i);if(e&&e[1]){let t=e[1].trim(),n=this.fuzzyMatchSalesforce(t);return n?{account:n.name,accountId:n.id,confidence:95,source:"salesforce_match",evidence:`File in account folder "${t}" matched to "${n.name}"`}:{account:t,accountId:null,confidence:85,source:"title",evidence:`File located in Accounts/${t} folder`}}return{account:null,accountId:null,confidence:0,source:"none",evidence:"Not in Accounts folder"}}detectFromAttendees(a){let e=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com"],t=new Set;for(let r of a){let c=r.toLowerCase().match(/@([a-z0-9.-]+)/);if(c){let d=c[1];!d.includes("eudia.com")&&!e.includes(d)&&t.add(d)}}if(t.size===0)return{account:null,accountId:null,confidence:0,source:"attendee_domain",evidence:"No external domains"};for(let r of t){let o=r.split(".")[0],c=o.charAt(0).toUpperCase()+o.slice(1),d=this.fuzzyMatchSalesforce(c);if(d)return{account:d.name,accountId:d.id,confidence:75,source:"salesforce_match",evidence:`Matched attendee domain ${r} to "${d.name}"`}}let n=Array.from(t)[0],s=n.split(".")[0];return{account:s.charAt(0).toUpperCase()+s.slice(1),accountId:null,confidence:50,source:"attendee_domain",evidence:`Guessed from external attendee domain: ${n}`}}fuzzyMatchSalesforce(a){if(!a||this.salesforceAccounts.length===0)return null;let e=a.toLowerCase().trim();for(let t of this.salesforceAccounts)if(t.name?.toLowerCase()===e)return t;for(let t of this.salesforceAccounts)if(t.name?.toLowerCase().startsWith(e))return t;for(let t of this.salesforceAccounts)if(t.name?.toLowerCase().includes(e))return t;for(let t of this.salesforceAccounts)if(e.includes(t.name?.toLowerCase()))return t;return null}suggestAccounts(a,e=10){if(!a||a.length<2)return this.salesforceAccounts.slice(0,e).map(s=>({...s,score:0}));let t=a.toLowerCase(),n=[];for(let s of this.salesforceAccounts){let i=s.name?.toLowerCase()||"",r=0;i===t?r=100:i.startsWith(t)?r=90:i.includes(t)?r=70:t.includes(i)&&(r=50),r>0&&n.push({...s,score:r})}return n.sort((s,i)=>i.score-s.score).slice(0,e)}},Fe=new V,ge=["pipeline review","pipeline call","weekly pipeline","forecast call","forecast review","deal review","opportunity review","sales review","pipeline sync","forecast sync","deal sync","pipeline update","forecast meeting"];function ae(g,a){if(g){let e=g.toLowerCase();for(let t of ge)if(e.includes(t))return{isPipelineMeeting:!0,confidence:95,evidence:`Title contains "${t}"`}}if(a&&a.length>=2){let e=["eudia.com","johnsonhana.com"];if(a.every(n=>{let s=n.toLowerCase().split("@")[1]||"";return e.some(i=>s.includes(i))})&&a.length>=3){if(g){let n=g.toLowerCase();if(["sync","review","update","weekly","team","forecast"].some(r=>n.includes(r)))return{isPipelineMeeting:!0,confidence:70,evidence:`All internal attendees (${a.length}) with team meeting signal`}}return{isPipelineMeeting:!1,confidence:40,evidence:"All internal attendees but no clear pipeline signal"}}}return{isPipelineMeeting:!1,confidence:0,evidence:"No pipeline meeting indicators found"}}function fe(g,a){let e="";return(a?.account||a?.opportunities?.length)&&(e=`
...
- Action items have clear owners`}function Oe(g){return`You are a sales operations analyst producing the weekly pipeline review summary for Eudia, an AI-powered legal technology company. You are processing the transcript of an internal team pipeline review meeting.
${g?`
...
`;let c=Math.round(i/5),d=Math.round(r/5);if(n+=`\`${"\u2588".repeat(c)}${"\u2591".repeat(20-c)}\` Rep
`,n+=`\`${"\u2588".repeat(d)}${"\u2591".repeat(20-d)}\` Customer

`,t.coaching){let u=t.coaching;if(u.totalQuestions>0){let h=Math.round(u.openQuestions/u.totalQuestions*100);n+=`**Questions:** ${u.totalQuestions} total (${u.openQuestions} open, ${u.closedQuestions} closed - ${h}% open)
`}if(u.objections&&u.objections.length>0){let h=u.objections.filter(y=>y.handled).length;n+=`**Objections:** ${u.objections.length} raised, ${h} handled
...
`,e}async blobToBase64(a){return new Promise((e,t)=>{let n=new FileReader;n.onload=()=>{let i=n.result.split(",")[1];e(i)},n.onerror=t,n.readAsDataURL(a)})}async transcribeAudio(a,e){try{let t=await this.blobToBase64(a),n=a.type||"audio/webm",s=e?.meetingType==="pipeline_review"?{success:!0,meetingType:"pipeline_review",pipelineContext:e.pipelineContext}:void 0,i=await this.transcribeAndSummarize(t,n,e?.accountName,e?.accountId,s);return{text:i.transcript,confidence:i.success?.95:0,duration:i.duration,sections:i.sections}}catch(t){return console.error("transcribeAudio error:",t),{text:"",confidence:0,duration:0,sections:this.getEmptySections()}}}async processTranscription(a,e){if(!a||a.trim().length===0)return this.getEmptySections();try{let t=await(0,k.requestUrl)({url:`${this.serverUrl}/api/process-sections`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:a,accountName:e?.accountName,context:e})});if(t.json?.success&&t.json?.sections){let n=t.json.sections;return{summary:n.summary||"",painPoints:n.painPoints||n.keyPoints||"",productInterest:n.productInterest||"",meddiccSignals:n.meddiccSignals||"",nextSteps:n.nextSteps||"",actionItems:n.actionItems||"",keyDates:n.keyDates||"",buyingTriggers:n.buyingTriggers||"",dealSignals:n.dealSignals||"",risksObjections:n.risksObjections||n.concerns||"",competitiveIntel:n.competitiveIntel||"",attendees:n.attendees||"",transcript:a}}return console.warn("Server process-sections returned no sections, using fallback"),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:a}}catch(t){return console.error("processTranscription server error:",t),{summary:"Meeting transcript captured. Review for key details.",painPoints:"",productInterest:"",meddiccSignals:"",nextSteps:"",actionItems:"",keyDates:"",buyingTriggers:"",dealSignals:"",risksObjections:"",competitiveIntel:"",attendees:"",transcript:a}}}};var B=require("obsidian"),T=class g{constructor(a,e,t="America/New_York"){this.serverUrl=a,this.userEmail=e.toLowerCase(),this.timezone=t}setUserEmail(a){this.userEmail=a.toLowerCase()}setServerUrl(a){this.serverUrl=a}setTimezone(a){this.timezone=a}async getTodaysMeetings(a=!1){if(!this.userEmail)return{success:!1,date:new Date().toISOString().split("T")[0],email:"",meetingCount:0,meetings:[],error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=a?"&forceRefresh=true":"";return(await(0,B.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/today?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch today's meetings:",e),{success:!1,date:new Date().toISOString().split("T")[0],email:this.userEmail,meetingCount:0,meetings:[],error:e.message||"Failed to fetch calendar"}}}async getWeekMeetings(a=!1){if(!this.userEmail)return{success:!1,startDate:"",endDate:"",email:"",totalMeetings:0,byDay:{},error:"User email not configured"};try{let e=encodeURIComponent(this.timezone),t=a?"&forceRefresh=true":"";return(await(0,B.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/week?timezone=${e}${t}`,method:"GET",headers:{Accept:"application/json"}})).json}catch(e){return console.error("Failed to fetch week's meetings:",e),{success:!1,startDate:"",endDate:"",email:this.userEmail,totalMeetings:0,byDay:{},error:e.message||"Failed to fetch calendar"}}}async getMeetingsInRange(a,e){if(!this.userEmail)return[];try{let t=a.toISOString().split("T")[0],n=e.toISOString().split("T")[0],s=await(0,B.requestUrl)({url:`${this.serverUrl}/api/calendar/${this.userEmail}/range?start=${t}&end=${n}`,method:"GET",headers:{Accept:"application/json"}});return s.json.success?s.json.meetings||[]:[]}catch(t){return console.error("Failed to fetch calendar range:",t),[]}}async getCurrentMeeting(){let a=await this.getTodaysMeetings();if(!a.success||a.meetings.length===0)return{meeting:null,isNow:!1};let e=new Date;for(let t of a.meetings){let n=g.safeParseDate(t.start),s=g.safeParseDate(t.end);if(e>=n&&e<=s)return{meeting:t,isNow:!0};let i=(n.getTime()-e.getTime())/(1e3*60);if(i>0&&i<=15)return{meeting:t,isNow:!1,minutesUntilStart:Math.ceil(i)}}return{meeting:null,isNow:!1}}async getMeetingsForAccount(a){let e=await this.getWeekMeetings();if(!e.success)return[];let t=[];Object.values(e.byDay).forEach(s=>{t.push(...s)});let n=a.toLowerCase();return t.filter(s=>s.accountName?.toLowerCase().includes(n)||s.subject.toLowerCase().includes(n)||s.attendees.some(i=>i.email.toLowerCase().includes(n.split(" ")[0])))}static formatMeetingForNote(a){let e=a.attendees.filter(t=>t.isExternal!==!1).map(t=>t.name||t.email.split("@")[0]).slice(0,5).join(", ");return{title:a.subject,attendees:e,meetingStart:a.start,accountName:a.accountName}}static getDayName(a){let e;a.length===10&&a.includes("-")?e=new Date(a+"T00:00:00"):e=new Date(a);let t=new Date;t.setHours(0,0,0,0);let n=new Date(e);n.setHours(0,0,0,0);let s=Math.round((n.getTime()-t.getTime())/(1e3*60*60*24));return s===0?"Today":s===1?"Tomorrow":e.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric"})}static formatTime(a,e){let t=a;t&&!t.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(t)&&(t=t+"Z");let n=new Date(t);if(isNaN(n.getTime()))return a;let s={hour:"numeric",minute:"2-digit",hour12:!0};return e&&(s.timeZone=e),n.toLocaleTimeString("en-US",s)}static safeParseDate(a){if(!a)return new Date(NaN);let e=a;return!e.endsWith("Z")&&!/[+-]\d{2}:\d{2}$/.test(e)&&(e=e+"Z"),new Date(e)}static getMeetingDuration(a,e){let t=g.safeParseDate(a),n=g.safeParseDate(e);return Math.round((n.getTime()-t.getTime())/(1e3*60))}};var ve=["ai-contracting-tech","ai-contracting-services","ai-compliance-tech","ai-compliance-services","ai-ma-tech","ai-ma-services","sigma"],we=["metrics-identified","economic-buyer-identified","decision-criteria-discussed","decision-process-discussed","pain-confirmed","champion-identified","competition-mentioned"],Se=["progressing","stalled","at-risk","champion-engaged","early-stage"],Ce=["discovery","demo","negotiation","qbr","implementation","follow-up"],We=`You are a sales intelligence tagger for Eudia, an AI legal technology company. Extract structured tags from meeting analysis.
...
`).replace(/\n/g,"<br>")}onClose(){this.contentEl.empty()}};var Z=class extends l.ItemView{constructor(e,t){super(e);this.emailInput=null;this.pollInterval=null;this.plugin=t,this.accountOwnershipService=new I(t.settings.serverUrl),this.steps=[{id:"calendar",title:"Connect Your Calendar",description:"View your meetings and create notes with one click",status:"pending"},{id:"salesforce",title:"Connect to Salesforce",description:"Sync notes and access your accounts",status:"pending"},{id:"transcribe",title:"Ready to Transcribe",description:"Record and summarize meetings automatically",status:"pending"}]}getViewType(){return P}getDisplayText(){return"Setup"}getIcon(){return"settings"}async onOpen(){await this.checkExistingStatus(),await this.render()}async onClose(){this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null)}async checkExistingStatus(){if(this.plugin.settings.userEmail){this.steps[0].status="complete";try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0&&(this.steps[1].status="complete",this.plugin.settings.salesforceConnected=!0)}catch{}this.plugin.settings.accountsImported&&(this.steps[2].status="complete")}}getCompletionPercentage(){let e=this.steps.filter(t=>t.status==="complete").length;return Math.round(e/this.steps.length*100)}async render(){let e=this.containerEl.children[1];e.empty(),e.addClass("eudia-setup-view"),this.renderHeader(e),this.renderSteps(e),this.renderFooter(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-setup-header"}),n=t.createDiv({cls:"eudia-setup-title-section"});n.createEl("h1",{text:"Welcome to Eudia Sales Vault",cls:"eudia-setup-main-title"}),n.createEl("p",{text:"Complete these steps to unlock your sales superpowers",cls:"eudia-setup-subtitle"});let s=t.createDiv({cls:"eudia-setup-progress-section"}),i=this.getCompletionPercentage(),r=s.createDiv({cls:"eudia-setup-progress-label"});r.createSpan({text:"Setup Progress"}),r.createSpan({text:`${i}%`,cls:"eudia-setup-progress-value"});let c=s.createDiv({cls:"eudia-setup-progress-bar"}).createDiv({cls:"eudia-setup-progress-fill"});c.style.width=`${i}%`}renderSteps(e){let t=e.createDiv({cls:"eudia-setup-steps-container"});this.renderCalendarStep(t),this.renderSalesforceStep(t),this.renderTranscribeStep(t)}renderCalendarStep(e){let t=this.steps[0],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),s=n.createDiv({cls:"eudia-setup-step-header"}),i=s.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"1"),t.status==="complete"&&i.addClass("eudia-step-complete");let r=s.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:`Connected as ${this.plugin.settings.userEmail}`});else{let c=o.createDiv({cls:"eudia-setup-input-group"});this.emailInput=c.createEl("input",{type:"email",placeholder:"yourname@eudia.com",cls:"eudia-setup-input"}),this.plugin.settings.userEmail&&(this.emailInput.value=this.plugin.settings.userEmail);let d=c.createEl("button",{text:"Connect",cls:"eudia-setup-btn primary"});d.onclick=async()=>{await this.handleCalendarConnect()},this.emailInput.onkeydown=async u=>{u.key==="Enter"&&await this.handleCalendarConnect()},o.createDiv({cls:"eudia-setup-validation-message"}),o.createEl("p",{cls:"eudia-setup-help-text",text:"Your calendar syncs automatically via Microsoft 365. We use your email to identify your meetings."})}}async handleCalendarConnect(){if(!this.emailInput)return;let e=this.emailInput.value.trim().toLowerCase(),t=this.containerEl.querySelector(".eudia-setup-validation-message");if(!e){t&&(t.textContent="Please enter your email",t.className="eudia-setup-validation-message error");return}if(!e.endsWith("@eudia.com")){t&&(t.textContent="Please use your @eudia.com email address",t.className="eudia-setup-validation-message error");return}t&&(t.textContent="Validating...",t.className="eudia-setup-validation-message loading");try{let n=await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${encodeURIComponent(e)}`,method:"GET",throw:!1});if(n.status===200&&n.json?.authorized){this.plugin.settings.userEmail=e,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),this.steps[0].status="complete",new l.Notice("Calendar connected successfully!"),t&&(t.textContent="Importing your accounts...",t.className="eudia-setup-validation-message loading");try{let s,i=[];if(A(e))console.log("[Eudia] Admin user detected - importing all accounts"),s=await this.accountOwnershipService.getAllAccountsForAdmin(e);else{let r=await this.accountOwnershipService.getAccountsWithProspects(e);s=r.accounts,i=r.prospects}if(s.length>0||i.length>0){A(e)?await this.plugin.createAdminAccountFolders(s):(await this.plugin.createTailoredAccountFolders(s),i.length>0&&await this.plugin.createProspectAccountFiles(i)),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=s.length+i.length,await this.plugin.saveSettings(),new l.Notice(`Imported ${s.length} active accounts + ${i.length} prospects!`);let r=[...s,...i];setTimeout(()=>this.plugin.enrichAccountFolders(r),2e3)}}catch(s){console.error("[Eudia] Account import failed:",s)}await this.render()}else t&&(t.innerHTML=`<strong>${e}</strong> is not authorized for calendar access. Contact your admin.`,t.className="eudia-setup-validation-message error")}catch{t&&(t.textContent="Connection failed. Please try again.",t.className="eudia-setup-validation-message error")}}renderSalesforceStep(e){let t=this.steps[1],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),s=n.createDiv({cls:"eudia-setup-step-header"}),i=s.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"2"),t.status==="complete"&&i.addClass("eudia-step-complete");let r=s.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"});if(!this.plugin.settings.userEmail){o.createDiv({cls:"eudia-setup-disabled-message",text:"Complete the calendar step first"});return}if(t.status==="complete")o.createDiv({cls:"eudia-setup-complete-message",text:"Salesforce connected successfully"}),this.plugin.settings.accountsImported&&o.createDiv({cls:"eudia-setup-account-status",text:`${this.plugin.settings.importedAccountCount} accounts imported`});else{let d=o.createDiv({cls:"eudia-setup-button-group"}).createEl("button",{text:"Connect to Salesforce",cls:"eudia-setup-btn primary"}),u=o.createDiv({cls:"eudia-setup-sf-status"});d.onclick=async()=>{let h=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(h,"_blank"),u.textContent="Complete the login in the popup window...",u.className="eudia-setup-sf-status loading",new l.Notice("Complete the Salesforce login in the popup window",5e3),this.startSalesforcePolling(u)},o.createEl("p",{cls:"eudia-setup-help-text",text:"This links your Obsidian notes to your Salesforce account for automatic sync."})}}startSalesforcePolling(e){this.pollInterval&&window.clearInterval(this.pollInterval);let t=0,n=60;this.pollInterval=window.setInterval(async()=>{t++;try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),this.steps[1].status="complete",new l.Notice("Salesforce connected successfully!"),await this.importTailoredAccounts(e),await this.render()):t>=n&&(this.pollInterval&&(window.clearInterval(this.pollInterval),this.pollInterval=null),e.textContent="Connection timed out. Please try again.",e.className="eudia-setup-sf-status error")}catch{}},5e3)}async importTailoredAccounts(e){e.textContent="Importing your accounts...",e.className="eudia-setup-sf-status loading";try{let t=this.plugin.settings.userEmail,n,s=[];if(A(t))console.log("[Eudia] Admin user detected - importing all accounts"),e.textContent="Admin detected - importing all accounts...",n=await this.accountOwnershipService.getAllAccountsForAdmin(t);else{let c=await this.accountOwnershipService.getAccountsWithProspects(t);n=c.accounts,s=c.prospects}if(n.length===0&&s.length===0){e.textContent="No accounts found for your email. Contact your admin.",e.className="eudia-setup-sf-status warning";return}A(t)?await this.plugin.createAdminAccountFolders(n):(await this.plugin.createTailoredAccountFolders(n),s.length>0&&await this.plugin.createProspectAccountFiles(s)),this.plugin.settings.accountsImported=!0,this.plugin.settings.importedAccountCount=n.length+s.length,await this.plugin.saveSettings(),this.steps[2].status="complete";let i=n.filter(c=>c.isOwned!==!1).length,r=n.filter(c=>c.isOwned===!1).length;A(t)&&r>0?e.textContent=`${i} owned + ${r} view-only accounts imported!`:e.textContent=`${n.length} active + ${s.length} prospect accounts imported!`,e.className="eudia-setup-sf-status success";let o=[...n,...s];setTimeout(()=>this.plugin.enrichAccountFolders(o),2e3)}catch{e.textContent="Failed to import accounts. Please try again.",e.className="eudia-setup-sf-status error"}}renderTranscribeStep(e){let t=this.steps[2],n=e.createDiv({cls:`eudia-setup-step-card ${t.status}`}),s=n.createDiv({cls:"eudia-setup-step-header"}),i=s.createDiv({cls:"eudia-setup-step-number"});i.setText(t.status==="complete"?"":"3"),t.status==="complete"&&i.addClass("eudia-step-complete");let r=s.createDiv({cls:"eudia-setup-step-info"});r.createEl("h3",{text:t.title}),r.createEl("p",{text:t.description});let o=n.createDiv({cls:"eudia-setup-step-content"}),c=o.createDiv({cls:"eudia-setup-instructions"}),d=c.createDiv({cls:"eudia-setup-instruction"});d.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F399}"}),d.createSpan({text:"Click the microphone icon in the left sidebar during a call"});let u=c.createDiv({cls:"eudia-setup-instruction"});u.createSpan({cls:"eudia-setup-instruction-icon",text:"\u2328"}),u.createSpan({text:'Or press Cmd/Ctrl+P and search for "Transcribe Meeting"'});let h=c.createDiv({cls:"eudia-setup-instruction"});h.createSpan({cls:"eudia-setup-instruction-icon",text:"\u{1F4DD}"}),h.createSpan({text:"AI will summarize and extract key insights automatically"}),t.status!=="complete"&&o.createEl("p",{cls:"eudia-setup-help-text muted",text:"This step completes automatically after connecting to Salesforce and importing accounts."})}renderFooter(e){let t=e.createDiv({cls:"eudia-setup-footer"});if(this.steps.every(i=>i.status==="complete")){let i=t.createDiv({cls:"eudia-setup-completion"});i.createEl("h2",{text:"\u{1F389} You're all set!"}),i.createEl("p",{text:"Your sales vault is ready. Click below to start using Eudia."});let r=t.createEl("button",{text:"Open Calendar \u2192",cls:"eudia-setup-btn primary large"});r.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(P),await this.plugin.activateCalendarView()}}else{let i=t.createEl("button",{text:"Skip Setup (I'll do this later)",cls:"eudia-setup-btn secondary"});i.onclick=async()=>{this.plugin.settings.setupCompleted=!0,await this.plugin.saveSettings(),this.plugin.app.workspace.detachLeavesOfType(P),new l.Notice("You can complete setup anytime from Settings \u2192 Eudia Sync")}}let s=t.createEl("a",{text:"Advanced Settings",cls:"eudia-setup-settings-link"});s.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")}}},Q=class extends l.ItemView{constructor(e,t){super(e);this.refreshInterval=null;this.lastError=null;this.plugin=t}getViewType(){return $}getDisplayText(){return"Calendar"}getIcon(){return"calendar"}async onOpen(){await this.render(),this.refreshInterval=window.setInterval(()=>this.render(),5*60*1e3)}async onClose(){this.refreshInterval&&window.clearInterval(this.refreshInterval)}async render(){let e=this.containerEl.children[1];if(e.empty(),e.addClass("eudia-calendar-view"),!this.plugin.settings.userEmail){this.renderSetupPanel(e);return}this.renderHeader(e),await this.renderCalendarContent(e)}renderHeader(e){let t=e.createDiv({cls:"eudia-calendar-header"}),n=t.createDiv({cls:"eudia-calendar-title-row"});n.createEl("h4",{text:"Your Meetings"});let s=n.createDiv({cls:"eudia-calendar-actions"}),i=s.createEl("button",{cls:"eudia-btn-icon",text:"\u21BB"});i.title="Refresh (fetches latest from calendar)",i.onclick=async()=>{i.addClass("spinning"),this._forceRefresh=!0,await this.render(),i.removeClass("spinning")};let r=s.createEl("button",{cls:"eudia-btn-icon",text:"\u2699"});r.title="Settings",r.onclick=()=>{this.app.setting.open(),this.app.setting.openTabById("eudia-sync")};let o=t.createDiv({cls:"eudia-status-bar"});this.renderConnectionStatus(o)}async renderConnectionStatus(e){let t={server:"connecting",calendar:"not_configured",salesforce:"not_configured"},n=this.plugin.settings.serverUrl,s=this.plugin.settings.userEmail;try{(await(0,l.requestUrl)({url:`${n}/api/health`,method:"GET",throw:!1})).status===200?(t.server="connected",t.serverMessage="Server online"):(t.server="error",t.serverMessage="Server unavailable")}catch{t.server="error",t.serverMessage="Cannot reach server"}if(s&&t.server==="connected")try{let u=await(0,l.requestUrl)({url:`${n}/api/calendar/validate/${encodeURIComponent(s)}`,method:"GET",throw:!1});u.status===200&&u.json?.authorized?(t.calendar="connected",t.calendarMessage="Calendar synced"):(t.calendar="not_authorized",t.calendarMessage="Not authorized")}catch{t.calendar="error",t.calendarMessage="Error checking access"}if(s&&t.server==="connected")try{let u=await(0,l.requestUrl)({url:`${n}/api/sf/auth/status?email=${encodeURIComponent(s)}`,method:"GET",throw:!1});u.status===200&&u.json?.connected?(t.salesforce="connected",t.salesforceMessage="Salesforce connected"):(t.salesforce="not_configured",t.salesforceMessage="Not connected")}catch{t.salesforce="not_configured"}let i=e.createDiv({cls:"eudia-status-indicators"}),r=i.createSpan({cls:`eudia-status-dot ${t.server}`});r.title=t.serverMessage||"Server";let o=i.createSpan({cls:`eudia-status-dot ${t.calendar}`});o.title=t.calendarMessage||"Calendar";let c=i.createSpan({cls:`eudia-status-dot ${t.salesforce}`});if(c.title=t.salesforceMessage||"Salesforce",e.createDiv({cls:"eudia-status-labels"}).createSpan({cls:"eudia-status-label",text:this.plugin.settings.userEmail}),t.calendar==="not_authorized"){let u=e.createDiv({cls:"eudia-status-warning"});u.innerHTML=`<strong>${s}</strong> is not authorized for calendar access. Contact your admin.`}}async renderCalendarContent(e){let t=e.createDiv({cls:"eudia-calendar-content"}),n=t.createDiv({cls:"eudia-calendar-loading"});n.innerHTML='<div class="eudia-spinner"></div><span>Loading meetings...</span>';try{let s=new T(this.plugin.settings.serverUrl,this.plugin.settings.userEmail,this.plugin.settings.timezone||"America/New_York"),i=this._forceRefresh||!1;this._forceRefresh=!1;let r=await s.getWeekMeetings(i);if(n.remove(),!r.success){this.renderError(t,r.error||"Failed to load calendar");return}let o=Object.keys(r.byDay||{}).sort();if(o.length===0){this.renderEmptyState(t);return}await this.renderCurrentMeeting(t,s);for(let c of o){let d=r.byDay[c];!d||d.length===0||this.renderDaySection(t,c,d)}}catch(s){n.remove(),this.renderError(t,s.message||"Failed to load calendar")}}async renderCurrentMeeting(e,t){try{let n=await t.getCurrentMeeting();if(n.meeting){let s=e.createDiv({cls:"eudia-now-card"});n.isNow?s.createDiv({cls:"eudia-now-badge",text:"\u25CF NOW"}):s.createDiv({cls:"eudia-now-badge soon",text:`In ${n.minutesUntilStart}m`});let i=s.createDiv({cls:"eudia-now-content"});i.createEl("div",{cls:"eudia-now-subject",text:n.meeting.subject}),n.meeting.accountName&&i.createEl("div",{cls:"eudia-now-account",text:n.meeting.accountName});let r=s.createEl("button",{cls:"eudia-now-action",text:"Create Note"});r.onclick=()=>this.createNoteForMeeting(n.meeting)}}catch{}}renderDaySection(e,t,n){let s=e.createDiv({cls:"eudia-calendar-day"});s.createEl("div",{cls:"eudia-calendar-day-header",text:T.getDayName(t)});for(let i of n){let r=s.createDiv({cls:`eudia-calendar-meeting ${i.isCustomerMeeting?"customer":"internal"}`});r.createEl("div",{cls:"eudia-calendar-time",text:T.formatTime(i.start,this.plugin.settings.timezone)});let o=r.createDiv({cls:"eudia-calendar-details"});if(o.createEl("div",{cls:"eudia-calendar-subject",text:i.subject}),i.accountName)o.createEl("div",{cls:"eudia-calendar-account",text:i.accountName});else if(i.attendees&&i.attendees.length>0){let c=i.attendees.slice(0,2).map(d=>d.name||d.email?.split("@")[0]||"Unknown").join(", ");o.createEl("div",{cls:"eudia-calendar-attendees",text:c})}r.onclick=()=>this.createNoteForMeeting(i),r.title="Click to create meeting note"}}renderEmptyState(e){let t=e.createDiv({cls:"eudia-calendar-empty"});t.innerHTML=`
    ...
    `}renderError(e,t){let n=e.createDiv({cls:"eudia-calendar-error"}),s="",i="Unable to load calendar",r="";t.includes("not authorized")||t.includes("403")?(s="\u{1F512}",i="Calendar Access Required",r="Contact your admin to be added to the authorized users list."):t.includes("network")||t.includes("fetch")?(s="\u{1F4E1}",i="Connection Issue",r="Check your internet connection and try again."):(t.includes("server")||t.includes("500"))&&(s="\u{1F527}",i="Server Unavailable",r="The server may be waking up. Try again in 30 seconds."),n.innerHTML=`
    ...
    `;let n=t.createDiv({cls:"eudia-setup-input-group"}),s=n.createEl("input",{type:"email",placeholder:"yourname@eudia.com"});s.addClass("eudia-setup-email");let i=n.createEl("button",{cls:"eudia-setup-connect",text:"Connect"}),r=t.createDiv({cls:"eudia-setup-status"});i.onclick=async()=>{let o=s.value.trim().toLowerCase();if(!o||!o.endsWith("@eudia.com")){r.textContent="Please use your @eudia.com email",r.className="eudia-setup-status error";return}i.disabled=!0,i.textContent="Connecting...",r.textContent="";try{if(!(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/calendar/validate/${o}`,method:"GET"})).json?.authorized){r.innerHTML=`<strong>${o}</strong> is not authorized. Contact your admin to be added.`,r.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect";return}this.plugin.settings.userEmail=o,this.plugin.settings.calendarConfigured=!0,await this.plugin.saveSettings(),r.textContent="Connected",r.className="eudia-setup-status success",this.plugin.scanLocalAccountFolders().catch(()=>{}),setTimeout(()=>this.render(),500)}catch(c){let d=c.message||"Connection failed";d.includes("403")?r.innerHTML=`<strong>${o}</strong> is not authorized for calendar access.`:r.textContent=d,r.className="eudia-setup-status error",i.disabled=!1,i.textContent="Connect"}},s.onkeydown=o=>{o.key==="Enter"&&i.click()},t.createEl("p",{cls:"eudia-setup-help",text:"Your calendar syncs automatically via Microsoft 365."})}extractCompanyFromDomain(e){let t=e.toLowerCase().split("."),n=["mail","email","app","portal","crm","www","smtp","sales","support","login","sso","auth","api","my"],s=["com","org","net","io","co","ai","gov","edu","uk","us","de","fr","jp","au","ca"],i=t.filter(o=>!s.includes(o)&&o.length>1);if(i.length===0)return t[0]||"";if(i.length>1&&n.includes(i[0]))return i[1].charAt(0).toUpperCase()+i[1].slice(1);let r=i[i.length-1];return r.charAt(0).toUpperCase()+r.slice(1)}getExternalDomainsFromAttendees(e){if(!e||e.length===0)return[];let t=["gmail.com","outlook.com","hotmail.com","yahoo.com","icloud.com","live.com","msn.com","aol.com","protonmail.com","googlemail.com","mail.com","zoho.com","ymail.com"],n=new Set,s=[];for(let i of e){if(!i.email)continue;let o=i.email.toLowerCase().match(/@([a-z0-9.-]+)/);if(o){let c=o[1];if(c.includes("eudia.com")||t.includes(c)||n.has(c))continue;n.add(c);let d=this.extractCompanyFromDomain(c);d.length>=2&&s.push({domain:c,company:d})}}return s}findBestAccountMatch(e,t,n){let s=this.plugin.settings.accountsFolder||"Accounts",i=this.app.vault.getAbstractFileByPath(s);if(!(i instanceof l.TFolder))return null;let r=[];for(let c of i.children)c instanceof l.TFolder&&r.push(c.name);if(r.length===0)return null;let o=[];for(let{domain:c,company:d}of e){let u=this.findAccountFolder(d),h=u?1:0;o.push({domain:c,company:d,folder:u,score:h})}if(o.sort((c,d)=>d.score-c.score),o.length>0&&o[0].folder){let c=o[0],d=c.folder.split("/").pop()||c.company;return console.log(`[Eudia Calendar] Best domain match: "${c.company}" from ${c.domain} -> ${c.folder}`),{folder:c.folder,accountName:d,source:"domain"}}if(t){let c=this.findAccountFolder(t);if(c){let d=c.split("/").pop()||t;return console.log(`[Eudia Calendar] Server account match: "${t}" -> ${c}`),{folder:c,accountName:d,source:"server"}}}if(n){let c=this.findAccountFolder(n);if(c){let d=c.split("/").pop()||n;return console.log(`[Eudia Calendar] Subject match: "${n}" -> ${c}`),{folder:c,accountName:d,source:"subject"}}}for(let{company:c}of e){let d=r.find(u=>{let h=u.toLowerCase(),y=c.toLowerCase();return h.includes(y)||y.includes(h)});if(d){let u=`${s}/${d}`;return console.log(`[Eudia Calendar] Partial domain match: "${c}" -> ${u}`),{folder:u,accountName:d,source:"domain-partial"}}}return null}extractAccountFromAttendees(e){let t=this.getExternalDomainsFromAttendees(e);if(t.length===0)return null;let n=t[0];return console.log(`[Eudia Calendar] Extracted company "${n.company}" from attendee domain ${n.domain}`),n.company}extractAccountFromSubject(e){if(!e)return null;let t=e.match(/^([^\/]+)\s*\/\s*Eudia|Eudia\s*\/\s*([^\/\-|]+)/i);if(t){let s=(t[1]||t[2]||"").trim();if(s.toLowerCase()!=="eudia")return s}let n=e.match(/^Eudia\s*[-–]\s*([^|]+)|^([^-–]+)\s*[-–]\s*Eudia/i);if(n){let i=(n[1]||n[2]||"").trim().replace(/\s+(Connect|Weekly|Call|Meeting|Intro|Demo|Check\s*in|Sync).*$/i,"").trim();if(i.toLowerCase()!=="eudia"&&i.length>0)return i}if(!e.toLowerCase().includes("eudia")){let s=e.match(/^([^-–|]+)/);if(s){let i=s[1].trim();if(i.length>2&&i.length<50)return i}}return null}findAccountFolder(e){if(!e)return null;let t=this.plugin.settings.accountsFolder||"Accounts",n=this.app.vault.getAbstractFileByPath(t);if(!(n instanceof l.TFolder))return console.log(`[Eudia Calendar] Accounts folder "${t}" not found`),null;let s=e.toLowerCase().trim(),i=[];for(let h of n.children)h instanceof l.TFolder&&i.push(h.name);console.log(`[Eudia Calendar] Searching for "${s}" in ${i.length} folders`);let r=i.find(h=>h.toLowerCase()===s);if(r)return console.log(`[Eudia Calendar] Exact match found: ${r}`),`${t}/${r}`;let o=i.find(h=>h.toLowerCase().startsWith(s));if(o)return console.log(`[Eudia Calendar] Folder starts with match: ${o}`),`${t}/${o}`;let c=i.find(h=>s.startsWith(h.toLowerCase()));if(c)return console.log(`[Eudia Calendar] Search starts with folder match: ${c}`),`${t}/${c}`;let d=i.find(h=>{let y=h.toLowerCase();return y.length>=3&&s.includes(y)});if(d)return console.log(`[Eudia Calendar] Search contains folder match: ${d}`),`${t}/${d}`;let u=i.find(h=>{let y=h.toLowerCase();return s.length>=3&&y.includes(s)});return u?(console.log(`[Eudia Calendar] Folder contains search match: ${u}`),`${t}/${u}`):(console.log(`[Eudia Calendar] No folder match found for "${s}"`),null)}async createNoteForMeeting(e){let t=e.start.split("T")[0],n=this.plugin.settings.eudiaEmail||"",s=A(n),i=(e.attendees||[]).map(v=>v.email).filter(Boolean),r=ae(e.subject,i);if(s&&r.isPipelineMeeting&&r.confidence>=60){await this._createPipelineNote(e,t);return}let o=e.subject.replace(/[<>:"/\\|?*]/g,"_").substring(0,50),c=`${t} - ${o}.md`,d=null,u=e.accountName||null,h=null;console.log(`[Eudia Calendar] === Creating note for meeting: "${e.subject}" ===`),console.log(`[Eudia Calendar] Attendees: ${JSON.stringify(e.attendees?.map(v=>v.email)||[])}`);let y=this.getExternalDomainsFromAttendees(e.attendees||[]);console.log(`[Eudia Calendar] External domains found: ${JSON.stringify(y)}`);let m=this.extractAccountFromSubject(e.subject);console.log(`[Eudia Calendar] Subject-extracted name: "${m||"none"}"`);let p=this.findBestAccountMatch(y,e.accountName,m||void 0);if(p&&(d=p.folder,u=p.accountName,console.log(`[Eudia Calendar] Best match (${p.source}): "${u}" -> ${d}`)),!d){let v=this.plugin.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(v)instanceof l.TFolder&&(d=v,console.log(`[Eudia Calendar] No match found, using Accounts root: ${d}`))}if(u){let v=this.plugin.settings.cachedAccounts.find(F=>F.name.toLowerCase()===u?.toLowerCase());v&&(h=v.id,u=v.name,console.log(`[Eudia Calendar] Matched to cached account: ${v.name} (${v.id})`))}let f=d?`${d}/${c}`:c,w=this.app.vault.getAbstractFileByPath(f);if(w instanceof l.TFile){await this.app.workspace.getLeaf().openFile(w),new l.Notice(`Opened existing note: ${c}`);return}let S=(e.attendees||[]).map(v=>v.name||v.email?.split("@")[0]||"Unknown").slice(0,5).join(", "),C=`---
title: "${e.subject}"
date: ${t}
attendees: [${S}]
account: "${u||""}"
...
`;try{let v=await this.app.vault.create(f,C);await this.app.workspace.getLeaf().openFile(v),new l.Notice(`Created: ${f}`)}catch(v){console.error("[Eudia Calendar] Failed to create note:",v),new l.Notice(`Could not create note: ${v.message||"Unknown error"}`)}}async _createPipelineNote(e,t){let n=new Date(t+"T00:00:00"),s=String(n.getMonth()+1).padStart(2,"0"),i=String(n.getDate()).padStart(2,"0"),r=String(n.getFullYear()).slice(-2),o=`${s}.${i}.${r}`,c=`Team Pipeline Meeting - ${o}.md`,d="Pipeline Meetings";this.app.vault.getAbstractFileByPath(d)||await this.app.vault.createFolder(d);let h=`${d}/${c}`,y=this.app.vault.getAbstractFileByPath(h);if(y instanceof l.TFile){await this.app.workspace.getLeaf().openFile(y),new l.Notice(`Opened existing: ${c}`);return}let m=(e.attendees||[]).map(f=>f.name||f.email?.split("@")[0]||"Unknown"),p=`---
...
`;try{let f=await this.app.vault.create(h,p);await this.app.workspace.getLeaf().openFile(f),new l.Notice(`Created pipeline note: ${c}`),console.log(`[Eudia Pipeline] Created pipeline meeting note: ${h}`)}catch(f){console.error("[Eudia Pipeline] Failed to create pipeline note:",f),new l.Notice(`Could not create pipeline note: ${f.message||"Unknown error"}`)}}},U=class extends l.Plugin{constructor(){super(...arguments);this.audioRecorder=null;this.recordingStatusBar=null;this.micRibbonIcon=null;this.liveTranscript="";this.liveTranscriptChunkInterval=null;this.isTranscribingChunk=!1;this.sfSyncStatusBarEl=null;this.sfSyncIntervalId=null}async onload(){await this.loadSettings(),this.transcriptionService=new H(this.settings.serverUrl),this.calendarService=new T(this.settings.serverUrl,this.settings.userEmail,this.settings.timezone||"America/New_York"),this.smartTagService=new _,this.checkForPluginUpdate(),this.registerView($,e=>new Q(e,this)),this.registerView(P,e=>new Z(e,this)),this.addRibbonIcon("calendar","Open Calendar",()=>this.activateCalendarView()),this.micRibbonIcon=this.addRibbonIcon("microphone","Transcribe Meeting",async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}),this.addRibbonIcon("message-circle","Ask GTM Brain",()=>{this.openIntelligenceQueryForCurrentNote()}),this.addCommand({id:"transcribe-meeting",name:"Transcribe Meeting",callback:async()=>{this.audioRecorder?.isRecording()?await this.stopRecording():await this.startRecording()}}),this.addCommand({id:"open-calendar",name:"Open Calendar",callback:()=>this.activateCalendarView()}),this.addCommand({id:"sync-accounts",name:"Sync Salesforce Accounts",callback:()=>this.syncAccounts()}),this.addCommand({id:"sync-note",name:"Sync Note to Salesforce",callback:()=>this.syncNoteToSalesforce()}),this.addCommand({id:"new-meeting-note",name:"New Meeting Note",callback:()=>this.createMeetingNote()}),this.addCommand({id:"ask-gtm-brain",name:"Ask gtm-brain",callback:()=>this.openIntelligenceQueryForCurrentNote()}),this.addCommand({id:"enrich-accounts",name:"Enrich Account Folders with Salesforce Data",callback:async()=>{if(!this.settings.userEmail){new l.Notice("Please set up your email first.");return}let t=await new I(this.settings.serverUrl).getAccountsWithProspects(this.settings.userEmail),n=[...t.accounts,...t.prospects];if(n.length===0){new l.Notice("No accounts found to enrich.");return}await this.enrichAccountFolders(n)}}),this.addCommand({id:"refresh-analytics",name:"Refresh Analytics Dashboard",callback:async()=>{let e=this.app.workspace.getActiveFile();e?await this.refreshAnalyticsDashboard(e):new l.Notice("No active file")}}),this.addCommand({id:"live-query-transcript",name:"Query Current Transcript (Live)",callback:async()=>{if(!this.audioRecorder?.isRecording()){new l.Notice("No active recording. Start recording first to use live query.");return}if(!this.liveTranscript||this.liveTranscript.length<50){new l.Notice("Not enough transcript captured yet. Keep recording for a few more minutes.");return}this.openLiveQueryModal()}}),this.sfSyncStatusBarEl=this.addStatusBarItem(),this.sfSyncStatusBarEl.setText("SF Sync: Idle"),this.sfSyncStatusBarEl.addClass("eudia-sf-sync-status"),this.addSettingTab(new ee(this.app,this)),this.registerEditorSuggest(new K(this.app,this)),this.app.workspace.onLayoutReady(async()=>{if(this.settings.setupCompleted){if(this.settings.syncOnStartup){if(await this.scanLocalAccountFolders(),this.settings.userEmail&&this.settings.syncAccountsOnStartup){let e=new Date().toISOString().split("T")[0];this.settings.lastAccountRefreshDate!==e&&setTimeout(async()=>{try{console.log("[Eudia] Startup account sync - checking for changes...");let n=await this.syncAccountFolders();if(n.success){if(this.settings.lastAccountRefreshDate=e,await this.saveSettings(),n.added>0||n.archived>0){let s=[];n.added>0&&s.push(`${n.added} added`),n.archived>0&&s.push(`${n.archived} archived`),new l.Notice(`Account folders synced: ${s.join(", ")}`)}}else console.log("[Eudia] Sync failed:",n.error)}catch{console.log("[Eudia] Startup sync skipped (server unreachable), will retry tomorrow")}},2e3)}this.settings.showCalendarView&&this.settings.userEmail&&await this.activateCalendarView(),this.settings.userEmail&&this.telemetry?setTimeout(async()=>{try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),t=0;e&&e instanceof l.TFolder&&(t=e.children.filter(i=>i instanceof l.TFolder&&!i.name.startsWith("_")).length);let n={salesforce:this.settings.salesforceConnected?"connected":"not_configured",calendar:this.settings.calendarConfigured?"connected":"not_configured"};await this.telemetry.sendHeartbeat(t,n);let s=await this.telemetry.checkForPushedConfig();if(s.length>0){let i=!1;for(let r of s)r.key&&this.settings.hasOwnProperty(r.key)&&(this.settings[r.key]=r.value,i=!0,console.log(`[Eudia] Applied pushed config: ${r.key} = ${r.value}`));i&&(await this.saveSettings(),new l.Notice("Settings updated by admin"))}await this.checkAndConsumeSyncFlags(),this.startSalesforceSyncScanner()}catch{console.log("[Eudia] Heartbeat/config check skipped"),this.startSalesforceSyncScanner()}},3e3):this.settings.sfAutoSyncEnabled&&this.settings.salesforceConnected&&setTimeout(()=>this.startSalesforceSyncScanner(),5e3)}}else{await new Promise(t=>setTimeout(t,100));let e=document.querySelector(".modal-container .modal");if(e){let t=e.querySelector(".modal-close-button");t&&t.click()}await this.activateSetupView()}this.app.workspace.on("file-open",async e=>{if(e&&(e.path.includes("_Analytics/")||e.path.includes("_Customer Health/")))try{let t=await this.app.vault.read(e);if(t.includes("type: analytics_dashboard")){let s=t.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/)?.[1],i=new Date().toISOString().split("T")[0];s!==i&&(console.log(`[Eudia] Auto-refreshing analytics: ${e.name}`),await this.refreshAnalyticsDashboard(e))}}catch{}})})}async onunload(){this.app.workspace.detachLeavesOfType($)}async checkForPluginUpdate(){try{let e=this.settings.serverUrl||"https://gtm-wizard.onrender.com",t=await(0,l.requestUrl)({url:`${e}/api/plugin/version`,method:"GET",headers:{"Content-Type":"application/json"}});if(!t.json?.success)return;let n=t.json.currentVersion,s=this.manifest?.version||"0.0.0",i=n.split(".").map(Number),r=s.split(".").map(Number),o=!1;for(let c=0;c<3;c++){if((i[c]||0)>(r[c]||0)){o=!0;break}if((i[c]||0)<(r[c]||0))break}o?(console.log(`[Eudia Update] New version available: ${n} (current: ${s})`),await this.performAutoUpdate(e,n,s)):console.log(`[Eudia Update] Plugin is up to date (v${s})`)}catch(e){console.log("[Eudia Update] Could not check for updates:",e.message||e)}}async performAutoUpdate(e,t,n){try{if(this.audioRecorder?.isRecording()){console.log("[Eudia Update] Skipping auto-update \u2014 recording in progress"),new l.Notice(`Eudia update v${t} available.
Finish your recording, then restart Obsidian to update.`,1e4);return}let s=this.manifest.dir;if(!s){console.log("[Eudia Update] Cannot determine plugin directory \u2014 skipping");return}let i=this.app.vault.adapter;console.log("[Eudia Update] Downloading plugin files...");let[r,o,c]=await Promise.all([(0,l.requestUrl)({url:`${e}/api/plugin/main.js`}),(0,l.requestUrl)({url:`${e}/api/plugin/manifest.json`}),(0,l.requestUrl)({url:`${e}/api/plugin/styles.css`})]),d=r.text,u=o.text,h=c.text,y=1024,m=5*1024*1024;for(let[C,v]of[["main.js",d],["manifest.json",u],["styles.css",h]])if(!v||v.length<y||v.length>m){console.log(`[Eudia Update] Downloaded ${C} failed validation (${v?.length??0} bytes) \u2014 aborting`);return}try{let C=await i.read(`${s}/main.js`);await i.write(`${s}/main.js.bak`,C),console.log("[Eudia Update] Backed up current main.js")}catch{console.log("[Eudia Update] Could not back up main.js \u2014 continuing")}await i.write(`${s}/main.js`,d),await i.write(`${s}/manifest.json`,u),await i.write(`${s}/styles.css`,h),console.log(`[Eudia Update] Files written \u2014 v${n} \u2192 v${t}`);try{(0,l.requestUrl)({url:`${e}/api/plugin/telemetry`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:"info",message:`Auto-updated from v${n} to v${t}`,userEmail:this.settings.userEmail||"anonymous",pluginVersion:t,platform:"obsidian",context:{fromVersion:n,toVersion:t}})}).catch(()=>{})}catch{}let p=document.createDocumentFragment(),f=document.createElement("div");f.style.cssText="display:flex;flex-direction:column;gap:8px;";let w=document.createElement("span");w.textContent=`Eudia updated to v${t}. Reload to apply.`,f.appendChild(w);let S=document.createElement("button");S.textContent="Reload now",S.style.cssText="padding:4px 12px;border-radius:4px;border:1px solid var(--interactive-accent);background:var(--interactive-accent);color:var(--text-on-accent);cursor:pointer;font-size:12px;align-self:flex-start;",S.addEventListener("click",()=>{this.app.commands.executeCommandById("app:reload")}),f.appendChild(S),p.appendChild(f),new l.Notice(p,0)}catch(s){console.log("[Eudia Update] Auto-update failed:",s.message||s)}}async loadSettings(){this.settings=Object.assign({},Te,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}async activateCalendarView(){let e=this.app.workspace,t=e.getLeavesOfType($);if(t.length>0)e.revealLeaf(t[0]);else{let n=e.getRightLeaf(!1);n&&(await n.setViewState({type:$,active:!0}),e.revealLeaf(n))}}async activateSetupView(){let e=this.app.workspace,t=e.getLeavesOfType(P);if(t.length>0)e.revealLeaf(t[0]);else{let n=e.getLeaf(!0);n&&(await n.setViewState({type:P,active:!0}),e.revealLeaf(n))}}async createTailoredAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t);let s=0;for(let i of e){let r=i.name.replace(/[<>:"/\\|?*]/g,"_").trim(),o=`${t}/${r}`;if(this.app.vault.getAbstractFileByPath(o)instanceof l.TFolder){console.log(`[Eudia] Account folder already exists: ${r}`);continue}try{await this.app.vault.createFolder(o);let d=new Date().toISOString().split("T")[0],u=[{name:"Note 1.md",content:`---
account: "${i.name}"
account_id: "${i.id}"
type: meeting_note
sync_to_salesforce: false
...
`}];for(let h of u){let y=`${o}/${h.name}`;await this.app.vault.create(y,h.content)}s++,console.log(`[Eudia] Created account folder with subnotes: ${r}`)}catch(d){console.error(`[Eudia] Failed to create folder for ${r}:`,d)}}this.settings.cachedAccounts=e.map(i=>({id:i.id,name:i.name})),await this.saveSettings(),s>0&&new l.Notice(`Created ${s} account folders`),await this.ensureNextStepsFolderExists()}async createProspectAccountFiles(e){if(!e||e.length===0)return 0;let t=this.settings.accountsFolder||"Accounts",n=`${t}/_Prospects`;if(!this.app.vault.getAbstractFileByPath(n))try{await this.app.vault.createFolder(n)}catch{}let i=0;for(let r of e){let o=r.name.replace(/[<>:"/\\|?*]/g,"_").trim(),c=`${n}/${o}`;if(this.app.vault.getAbstractFileByPath(c)instanceof l.TFolder)continue;let u=`${t}/${o}`;if(this.app.vault.getAbstractFileByPath(u)instanceof l.TFolder)continue;let y=`${n}/${o}.md`,m=this.app.vault.getAbstractFileByPath(y);if(m instanceof l.TFile)try{await this.app.vault.delete(m)}catch{}try{await this.app.vault.createFolder(c);let p=new Date().toISOString().split("T")[0],f=[{name:"Note 1.md",content:`---
account: "${r.name}"
account_id: "${r.id}"
type: meeting_note
tier: prospect
...
*Previous next steps will be archived here.*
`}];for(let w of f){let S=`${c}/${w.name}`;await this.app.vault.create(S,w.content)}i++}catch(p){console.log(`[Eudia] Failed to create prospect folder for ${r.name}:`,p)}}return i>0&&console.log(`[Eudia] Created ${i} prospect account folders in _Prospects/`),i}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let s=0,i=0,r=new Date().toISOString().split("T")[0];for(let o of e){let c=o.name.replace(/[<>:"/\\|?*]/g,"_").trim(),d=`${t}/${c}`;if(!(this.app.vault.getAbstractFileByPath(d)instanceof l.TFolder))try{await this.app.vault.createFolder(d),await this.createExecAccountSubnotes(d,o,r),o.isOwned?s++:i++,console.log(`[Eudia Admin] Created ${o.isOwned?"owned":"view-only"} folder: ${c}`)}catch(h){console.error(`[Eudia Admin] Failed to create folder for ${c}:`,h)}}this.settings.cachedAccounts=e.map(o=>({id:o.id,name:o.name})),await this.saveSettings(),s+i>0&&new l.Notice(`Created ${s} owned + ${i} view-only account folders`),await this.ensureNextStepsFolderExists()}async createExecAccountSubnotes(e,t,n){let s=t.ownerName||"Unknown",i=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let r of i){let o=`${e}/${r.name}`;await this.app.vault.create(o,r.content)}}async createFullAccountSubnotes(e,t,n){let s=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let i of s){let r=`${e}/${i.name}`;await this.app.vault.create(r,i.content)}}async ensurePipelineFolderExists(){let e="Pipeline",t=`${e}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let r=`---
...
`;await this.app.vault.create(t,o)}}async updateAccountNextSteps(e,t,n){try{console.log(`[Eudia] updateAccountNextSteps called for: ${e}`),console.log(`[Eudia] Content length: ${t?.length||0} chars`);let s=e.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${this.settings.accountsFolder}/${s}/Next Steps.md`;console.log(`[Eudia] Looking for Next Steps file at: ${i}`);let r=this.app.vault.getAbstractFileByPath(i);if(!r||!(r instanceof l.TFile)){console.log(`[Eudia] \u274C Next Steps file NOT FOUND at: ${i}`);let S=this.app.vault.getAbstractFileByPath(`${this.settings.accountsFolder}/${s}`);S&&S instanceof l.TFolder?console.log(`[Eudia] Files in ${s} folder:`,S.children.map(C=>C.name)):console.log(`[Eudia] Account folder also not found: ${this.settings.accountsFolder}/${s}`);return}console.log("[Eudia] Found Next Steps file, updating...");let o=new Date().toISOString().split("T")[0],c=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),d=n.split("/").pop()?.replace(".md","")||"Meeting",u=t;!t.includes("- [ ]")&&!t.includes("- [x]")&&(u=t.split(`
`).filter(S=>S.trim()).map(S=>{let C=S.replace(/^[-•*]\s*/,"").trim();return C?`- [ ] ${C}`:""}).filter(Boolean).join(`
`));let h=await this.app.vault.read(r),y="",m=h.match(/## History\n\n\*Previous next steps are archived below\.\*\n\n([\s\S]*?)$/);m&&m[1]&&(y=m[1].trim());let p=`### ${o} - ${d}
${u||"*None*"}`,f=y?`${p}
...
${f}
`;await this.app.vault.modify(r,w),console.log(`[Eudia] Updated Next Steps for ${e} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(s){console.error(`[Eudia] Failed to update Next Steps for ${e}:`,s)}}async regenerateNextStepsDashboard(){try{let t=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!t||!(t instanceof l.TFile)){await this.ensureNextStepsFolderExists();return}let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!n||!(n instanceof l.TFolder))return;let s=[];for(let c of n.children)if(c instanceof l.TFolder){let d=`${c.path}/Next Steps.md`,u=this.app.vault.getAbstractFileByPath(d);if(u instanceof l.TFile){let h=await this.app.vault.read(u),y=h.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),m=y?y[1]:"Unknown",p=h.split(`
`).filter(f=>f.match(/^- \[[ x]\]/)).slice(0,5);(p.length>0||m!=="Unknown")&&s.push({account:c.name,lastUpdated:m,nextSteps:p})}}s.sort((c,d)=>d.lastUpdated.localeCompare(c.lastUpdated));let i=new Date().toISOString().split("T")[0],r=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${i}
---
...
---
`;await this.app.vault.modify(t,r+o),this.processTranscriptionAsync(e,t).catch(c=>{console.error("Background transcription failed:",c),new l.Notice(`Transcription failed: ${c.message}`)})}async processTranscriptionAsync(e,t){try{let n={},s=t.path.split("/");console.log(`[Eudia] Processing transcription for: ${t.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(s)}, accountsFolder: ${this.settings.accountsFolder}`);let i=s[0]==="Pipeline Meetings",r=!1;try{let p=(await this.app.vault.read(t)).match(/^---\n([\s\S]*?)\n---/);p&&(r=/meeting_type:\s*pipeline_review/.test(p[1]))}catch{}if(!r&&i&&(r=!0),r){console.log("[Eudia Pipeline] Detected pipeline review meeting, using pipeline prompt");let m="";try{let p=await(0,l.requestUrl)({url:`${this.settings.serverUrl||"https://gtm-brain.onrender.com"}/api/pipeline-context`,method:"GET",headers:{"Content-Type":"application/json"}});p.json?.success&&p.json?.context&&(m=p.json.context,console.log(`[Eudia Pipeline] Loaded Salesforce pipeline context (${m.length} chars)`))}catch(p){console.warn("[Eudia Pipeline] Could not fetch pipeline context:",p)}n={meetingType:"pipeline_review",pipelineContext:m}}else if(s.length>=2&&s[0]===this.settings.accountsFolder){let m=s[1];console.log(`[Eudia] Detected account folder: ${m}`);let p=this.settings.cachedAccounts.find(f=>f.name.toLowerCase()===m.toLowerCase());p?(n={accountName:p.name,accountId:p.id},console.log(`[Eudia] Found cached account: ${p.name} (${p.id})`)):(n={accountName:m,accountId:""},console.log(`[Eudia] Account not in cache, using folder name: ${m}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let o=[];try{let m=await this.calendarService.getCurrentMeeting();m.meeting?.attendees&&(o=m.meeting.attendees.map(p=>p.name||p.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let c=await this.transcriptionService.transcribeAudio(e.audioBlob,{...n,speakerHints:o}),d=m=>m?!!(m.summary?.trim()||m.nextSteps?.trim()):!1,u=c.sections;if(d(u)||c.text?.trim()&&(u=await this.transcriptionService.processTranscription(c.text,n)),!d(u)&&!c.text?.trim()){let p=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,p+`

**Transcription failed:** No audio detected.
`),new l.Notice("Transcription failed: No audio detected.");return}let h;r?h=this.buildPipelineNoteContent(u,c,t.path):h=this.buildNoteContent(u,c),await this.app.vault.modify(t,h);let y=Math.floor(e.duration/60);if(new l.Notice(`Transcription complete (${y} min recording)`),!r){let m=u.nextSteps||u.actionItems;console.log(`[Eudia] Next Steps extraction - accountContext: ${n?.accountName||"undefined"}`),console.log(`[Eudia] Next Steps content found: ${m?"YES ("+m.length+" chars)":"NO"}`),console.log(`[Eudia] sections.nextSteps: ${u.nextSteps?"YES":"NO"}, sections.actionItems: ${u.actionItems?"YES":"NO"}`),m&&n?.accountName?(console.log(`[Eudia] Calling updateAccountNextSteps for ${n.accountName}`),await this.updateAccountNextSteps(n.accountName,m,t.path)):console.log("[Eudia] Skipping Next Steps update - missing content or account context")}this.settings.autoSyncAfterTranscription&&await this.syncNoteToSalesforce()}catch(n){try{let i=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,i+`

**Transcription failed:** ${n.message}
`)}catch{}throw n}}buildPipelineNoteContent(e,t,n){let s=new Date,i=String(s.getMonth()+1).padStart(2,"0"),r=String(s.getDate()).padStart(2,"0"),o=String(s.getFullYear()).slice(-2),c=s.toISOString().split("T")[0],d=`${i}.${r}.${o}`,u=p=>p==null?"":Array.isArray(p)?p.map(String).join(`
`):typeof p=="object"?JSON.stringify(p,null,2):String(p),h=u(e.summary),y=t.transcript||t.text||"",m=`---
...
`;if(h)m+=h;else{let p=[e.painPoints,e.productInterest,e.nextSteps,e.actionItems].filter(Boolean).map(u).join(`

`);p?m+=p:m+="*Pipeline summary could not be generated. See transcript below.*"}return y&&(m+=`
...
`),m}buildNoteContent(e,t){let n=f=>f==null?"":Array.isArray(f)?f.map(w=>typeof w=="object"?w.category?`**${w.category}**: ${w.signal||w.insight||""}`:JSON.stringify(w):String(w)).join(`
`):typeof f=="object"?JSON.stringify(f):String(f),s=n(e.title)||"Meeting Notes",i=n(e.summary),r=n(e.painPoints),o=n(e.productInterest),c=n(e.meddiccSignals),d=n(e.nextSteps),u=n(e.actionItems),h=n(e.keyDates),y=n(e.risksObjections),m=n(e.attendees),p=`---
title: "${s}"
...
`;return r&&!r.includes("None explicitly")&&(p+=`## Pain Points
...
`),y&&!y.includes("None raised")&&(p+=`## Risks and Objections
...
${t.text}
`),p}openIntelligenceQuery(){new G(this.app,this).open()}openIntelligenceQueryForCurrentNote(){let e=this.app.workspace.getActiveFile(),t;if(e){let n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(n?.account_id&&n?.account)t={id:n.account_id,name:n.account};else if(n?.account){let s=this.settings.cachedAccounts.find(i=>i.name.toLowerCase()===n.account.toLowerCase());s?t={id:s.id,name:s.name}:t={id:"",name:n.account}}else{let s=e.path.split("/");if(s.length>=2&&s[0]===this.settings.accountsFolder){let i=s[1],r=this.settings.cachedAccounts.find(o=>o.name.replace(/[<>:"/\\|?*]/g,"_").trim()===i);r?t={id:r.id,name:r.name}:t={id:"",name:i}}}}new G(this.app,this,t).open()}async syncAccounts(e=!1){e||new l.Notice("Syncing Salesforce accounts...");try{let n=(await(0,l.requestUrl)({url:`${this.settings.serverUrl}/api/accounts/obsidian`,method:"GET",headers:{Accept:"application/json"}})).json;if(!n.success||!n.accounts){e||new l.Notice("Failed to fetch accounts from server");return}this.settings.cachedAccounts=n.accounts.map(s=>({id:s.id,name:s.name})),this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings(),e||new l.Notice(`Synced ${n.accounts.length} accounts for matching`)}catch(t){e||new l.Notice(`Failed to sync accounts: ${t.message}`)}}async scanLocalAccountFolders(){try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!e||!(e instanceof l.TFolder))return;let t=[];for(let n of e.children)n instanceof l.TFolder&&t.push({id:`local-${n.name.replace(/\s+/g,"-").toLowerCase()}`,name:n.name});this.settings.cachedAccounts=t,this.settings.lastSyncTime=new Date().toISOString(),await this.saveSettings()}catch(e){console.error("Failed to scan local account folders:",e)}}async refreshAccountFolders(){if(!this.settings.userEmail)throw new Error("Please configure your email first");let e=new I(this.settings.serverUrl);if((await e.getAccountsForUser(this.settings.userEmail)).length===0)return console.log("[Eudia] No accounts found for user"),0;let n=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),s=[];if(n&&n instanceof l.TFolder)for(let r of n.children)r instanceof l.TFolder&&s.push(r.name);let i=await e.getNewAccounts(this.settings.userEmail,s);return i.length===0?(console.log("[Eudia] All account folders exist"),0):(console.log(`[Eudia] Creating ${i.length} new account folders`),await this.createTailoredAccountFolders(i),i.length)}async checkAndConsumeSyncFlags(){if(!this.settings.userEmail)return;let e=encodeURIComponent(this.settings.userEmail.toLowerCase().trim()),t=this.settings.serverUrl||"https://gtm-wizard.onrender.com";try{let i=((await(0,l.requestUrl)({url:`${t}/api/admin/users/${e}/sync-flags`,method:"GET"})).json?.flags||[]).filter(o=>!o.consumed_at);if(i.length===0)return;console.log(`[Eudia] Found ${i.length} pending sync flag(s)`);let r=!1;for(let o of i)if(o.flag==="resync_accounts"){r=!0;let c=o.payload||{},d=c.added?.length||0,u=c.removed?.length||0;console.log(`[Eudia] Sync flag: resync_accounts (+${d} / -${u})`)}else o.flag==="update_plugin"?new l.Notice("A plugin update is available. Please download the latest vault."):o.flag==="reset_setup"&&(console.log("[Eudia] Sync flag: reset_setup received"),this.settings.setupCompleted=!1,await this.saveSettings(),new l.Notice("Setup has been reset by admin. Please re-run the setup wizard."));if(r){console.log("[Eudia] Triggering account folder resync from sync flag..."),new l.Notice("Syncing account updates...");let o=await this.syncAccountFolders();o.success?new l.Notice(`Account sync complete: ${o.added} new, ${o.archived} archived`):console.log(`[Eudia] Account resync error: ${o.error}`)}try{await(0,l.requestUrl)({url:`${t}/api/admin/users/${e}/sync-flags/consume`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({flagIds:i.map(o=>o.id)})}),console.log(`[Eudia] Consumed ${i.length} sync flag(s)`)}catch{console.log("[Eudia] Failed to consume sync flags (will retry next startup)")}}catch{console.log("[Eudia] Sync flag check skipped (endpoint not available)")}}async syncAccountFolders(){if(!this.settings.userEmail)return{success:!1,added:0,archived:0,error:"No email configured"};let e=this.settings.userEmail.toLowerCase().trim();console.log(`[Eudia] Syncing account folders for: ${e}`);try{let t=await fetch(`${this.settings.serverUrl}/api/bl-accounts/${encodeURIComponent(e)}`);if(!t.ok){let O=await t.json().catch(()=>({}));throw new Error(O.error||`Server returned ${t.status}`)}let n=await t.json();if(!n.success||!n.accounts)throw new Error(n.error||"Invalid response from server");let s=n.meta?.userGroup||"bl",i=n.meta?.queryDescription||"accounts",r=n.meta?.region||null;console.log(`[Eudia] User group: ${s}, accounts: ${n.accounts.length} (${i})`),r&&console.log(`[Eudia] Sales Leader region: ${r}`);let o=n.accounts||[],c=n.prospectAccounts||[],d=o.length+c.length;console.log(`[Eudia] Server returned: ${o.length} active + ${c.length} prospects = ${d} total`);let u=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder),h=new Map,y=`${this.settings.accountsFolder}/_Prospects`,m=this.app.vault.getAbstractFileByPath(y),p=new Map,f=new Map;if(u&&u instanceof l.TFolder)for(let O of u.children)O instanceof l.TFolder&&!O.name.startsWith("_")&&h.set(O.name.toLowerCase().trim(),O);if(m&&m instanceof l.TFolder)for(let O of m.children)O instanceof l.TFolder?p.set(O.name.toLowerCase().trim(),O):O instanceof l.TFile&&O.extension==="md"&&f.set(O.basename.toLowerCase().trim(),O);let w=new Set(o.map(O=>O.name.toLowerCase().trim())),S=o.filter(O=>{let W=O.name.toLowerCase().trim();return!h.has(W)}),C=c.filter(O=>{let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();return!p.has(W)&&!f.has(W)&&!h.has(O.name.toLowerCase().trim())}),v=[];for(let O of o){let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim().toLowerCase();(p.has(W)||f.has(W))&&!h.has(O.name.toLowerCase().trim())&&v.push(O)}let F=new Set([...o.map(O=>O.name.toLowerCase().trim()),...c.map(O=>O.name.toLowerCase().trim())]),N=[];if(s==="bl")for(let[O,W]of h.entries())F.has(O)||N.push(W);let D=0,M=0,L=0,z=0;if(v.length>0){console.log(`[Eudia] Promoting ${v.length} accounts from prospect to active`);for(let O of v){let W=O.name.replace(/[<>:"/\\|?*]/g,"_").trim(),j=p.get(W.toLowerCase()),x=f.get(W.toLowerCase());try{if(j){let Y=`${this.settings.accountsFolder}/${W}`;await this.app.vault.rename(j,Y),L++,new l.Notice(`${O.name} promoted to active`)}else x&&(await this.app.vault.delete(x),await this.createTailoredAccountFolders([{id:O.id,name:O.name,type:O.customerType,isOwned:!0,hadOpportunity:!0}]),L++,new l.Notice(`${O.name} promoted to active -- full account folder created`))}catch(Y){console.error(`[Eudia] Failed to promote ${O.name}:`,Y)}}}if(S.length>0){console.log(`[Eudia] Creating ${S.length} new active account folders for ${s}`);let O=new Set(v.map(j=>j.name.toLowerCase().trim())),W=S.filter(j=>!O.has(j.name.toLowerCase().trim()));if(W.length>0){let j=W.map(x=>({id:x.id,name:x.name,type:x.customerType,isOwned:s==="bl",ownerName:x.ownerName,hadOpportunity:!0}));s==="admin"||s==="exec"?await this.createAdminAccountFolders(j):await this.createTailoredAccountFolders(j),D=W.length}this.telemetry&&this.telemetry.reportInfo("Accounts synced - added",{count:D,userGroup:s,region:r||void 0})}return C.length>0&&s==="bl"&&(console.log(`[Eudia] Creating ${C.length} new prospect files`),z=await this.createProspectAccountFiles(C.map(O=>({id:O.id,name:O.name,type:"Prospect",hadOpportunity:!1,website:O.website,industry:O.industry})))),this.settings.archiveRemovedAccounts&&N.length>0&&(console.log(`[Eudia] Archiving ${N.length} removed account folders`),M=await this.archiveAccountFolders(N),this.telemetry&&this.telemetry.reportInfo("Accounts synced - archived",{count:M})),console.log(`[Eudia] Sync complete: ${D} active added, ${z} prospects added, ${L} promoted, ${M} archived (group: ${s})`),{success:!0,added:D+z+L,archived:M,userGroup:s}}catch(t){return console.error("[Eudia] Account sync error:",t),this.telemetry&&this.telemetry.reportError("Account sync failed",{error:t.message}),{success:!1,added:0,archived:0,error:t.message}}}async archiveAccountFolders(e){let t=0,n=`${this.settings.accountsFolder}/_Archived`;this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n);for(let i of e)try{let r=`${n}/${i.name}`;if(this.app.vault.getAbstractFileByPath(r)){let u=new Date().toISOString().split("T")[0];await this.app.fileManager.renameFile(i,`${n}/${i.name}_${u}`)}else await this.app.fileManager.renameFile(i,r);let c=`${n}/${i.name}/_archived.md`,d=`---
...
`;try{await this.app.vault.create(c,d)}catch{}t++,console.log(`[Eudia] Archived: ${i.name}`)}catch(r){console.error(`[Eudia] Failed to archive ${i.name}:`,r)}return t}async syncSpecificNoteToSalesforce(e){let t=await this.app.vault.read(e),n=this.app.metadataCache.getFileCache(e)?.frontmatter;if(!n?.sync_to_salesforce)return{success:!1,error:"sync_to_salesforce not enabled"};let s=n.account_id,i=n.account;if(!s&&i){let r=this.settings.cachedAccounts.find(o=>o.name.toLowerCase()===i.toLowerCase());r&&(s=r.id)}if(!s){let r=e.path.split("/");if(r.length>=2&&r[0]===this.settings.accountsFolder){let o=r[1]==="_Prospects"&&r.length>=3?r[2]:r[1],c=this.settings.cachedAccounts.find(d=>d.name.replace(/[<>:"/\\|?*]/g,"_").trim()===o);c&&(s=c.id,i=c.name)}}if(!s)return{success:!1,error:`Could not determine account for ${e.path}`};try{let r=await(0,l.requestUrl)({url:`${this.settings.serverUrl}/api/notes/sync`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountId:s,accountName:i,noteTitle:e.basename,notePath:e.path,content:t,frontmatter:n,syncedAt:new Date().toISOString(),userEmail:this.settings.userEmail})});return r.json?.success?{success:!0}:{success:!1,error:r.json?.error||"Unknown error",authRequired:r.json?.authRequired}}catch(r){return{success:!1,error:r.message}}}async syncNoteToSalesforce(){let e=this.app.workspace.getActiveFile();if(!e){new l.Notice("No active file to sync");return}if(!this.app.metadataCache.getFileCache(e)?.frontmatter?.sync_to_salesforce){new l.Notice("Set sync_to_salesforce: true in frontmatter to enable sync");return}new l.Notice("Syncing to Salesforce...");let n=await this.syncSpecificNoteToSalesforce(e);n.success?new l.Notice("Synced to Salesforce"):n.authRequired?new l.Notice("Salesforce authentication required. Please reconnect."):new l.Notice("Failed to sync: "+(n.error||"Unknown error"))}async enrichAccountFolders(e){if(!e||e.length===0)return;let t=this.settings.serverUrl||"https://gtm-wizard.onrender.com",n=this.settings.accountsFolder||"Accounts",s=e.filter(u=>u.id&&u.id.length>=15);if(s.length===0)return;let i=s.length,r=0,o=0;console.log(`[Eudia Enrich] Starting enrichment for ${i} accounts`),new l.Notice(`Enriching account data: 0/${i}...`);let c=20;for(let u=0;u<s.length;u+=c){let h=s.slice(u,u+c),y=h.map(p=>p.id);try{let p=await(0,l.requestUrl)({url:`${t}/api/accounts/enrich-batch`,method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accountIds:y,userEmail:this.settings.userEmail})});if(p.json?.success&&p.json?.enrichments){let f=p.json.enrichments;for(let w of h){let S=f[w.id];if(S)try{await this.writeEnrichmentToAccount(w,S,n),r++}catch(C){o++,console.error(`[Eudia Enrich] Write failed for ${w.name}:`,C)}}}}catch(p){o+=h.length,console.error("[Eudia Enrich] Batch fetch failed:",p)}let m=Math.min(u+c,i);new l.Notice(`Enriching account data: ${m}/${i}...`),u+c<s.length&&await new Promise(p=>setTimeout(p,500))}let d=o>0?`Account enrichment complete: ${r} enriched, ${o} skipped`:`Account enrichment complete: ${r} accounts enriched with Salesforce data`;console.log(`[Eudia Enrich] ${d}`),new l.Notice(d)}async writeEnrichmentToAccount(e,t,n){let s=e.name.replace(/[<>:"/\\|?*]/g,"_").trim(),i=`${n}/${s}`,r=this.app.vault.getAbstractFileByPath(i);if(r instanceof l.TFolder||(i=`${n}/_Prospects/${s}`,r=this.app.vault.getAbstractFileByPath(i)),!(r instanceof l.TFolder))return;let o=new Date().toISOString(),c=async(d,u)=>{let h=`${i}/${d}`,y=this.app.vault.getAbstractFileByPath(h);if(!(y instanceof l.TFile))return;let m=await this.app.vault.read(y),p="",f=m;if(m.startsWith("---")){let v=m.indexOf("---",3);v!==-1&&(p=m.substring(0,v+3),f=m.substring(v+3),p.includes("enriched_at:")?p=p.replace(/enriched_at:.*/,`enriched_at: "${o}"`):p=p.substring(0,v)+`enriched_at: "${o}"
---`)}let w=f.match(/^(\s*#[^\n]+)/),C=`${w?w[1]:""}

${u}
`;await this.app.vault.modify(y,`${p}
${C}`)};if(t.contacts&&await c("Contacts.md",`${t.contacts}

## Relationship Map

*Add org chart, decision makers, champions, and blockers here.*`),t.intelligence&&await c("Intelligence.md",t.intelligence),t.nextSteps&&await c("Next Steps.md",t.nextSteps),t.opportunities||t.recentActivity){let d=`${i}/Meeting Notes.md`,u=this.app.vault.getAbstractFileByPath(d);if(u instanceof l.TFile){let h=await this.app.vault.read(u),y="",m=h;if(h.startsWith("---")){let S=h.indexOf("---",3);S!==-1&&(y=h.substring(0,S+3),m=h.substring(S+3),y.includes("enriched_at:")?y=y.replace(/enriched_at:.*/,`enriched_at: "${o}"`):y=y.substring(0,S)+`enriched_at: "${o}"
---`)}let p=m.match(/^(\s*#[^\n]+)/),w=[p?p[1]:`
# ${e.name} - Meeting Notes`,""];t.opportunities&&w.push(t.opportunities,""),t.recentActivity&&w.push(t.recentActivity,""),w.push("## Quick Start","","1. Open **Note 1** for your next meeting","2. Click the **microphone** to record and transcribe","3. **Next Steps** are auto-extracted after transcription","4. Set `sync_to_salesforce: true` to sync to Salesforce"),await this.app.vault.modify(u,`${y}
${w.join(`
`)}
`)}}}startSalesforceSyncScanner(){if(!this.settings.sfAutoSyncEnabled){console.log("[Eudia SF Sync] Auto-sync is disabled in settings"),this.updateSfSyncStatusBar("SF Sync: Off");return}let e=(this.settings.sfAutoSyncIntervalMinutes||15)*60*1e3;console.log(`[Eudia SF Sync] Starting scanner \u2014 interval: ${this.settings.sfAutoSyncIntervalMinutes}min`),this.updateSfSyncStatusBar("SF Sync: Idle");let t=window.setTimeout(()=>{this.runSalesforceSyncScan()},3e4);this.registerInterval(t),this.sfSyncIntervalId=window.setInterval(()=>{this.runSalesforceSyncScan()},e),this.registerInterval(this.sfSyncIntervalId)}async runSalesforceSyncScan(){if(!(!this.settings.sfAutoSyncEnabled||!this.settings.userEmail)){console.log("[Eudia SF Sync] Running scan...");try{let e=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!(e instanceof l.TFolder)){console.log("[Eudia SF Sync] Accounts folder not found");return}let t=[],n=u=>{for(let h of u.children)h instanceof l.TFile&&h.extension==="md"?t.push(h):h instanceof l.TFolder&&n(h)};n(e);let s=[];for(let u of t){let y=this.app.metadataCache.getFileCache(u)?.frontmatter;if(!y?.sync_to_salesforce)continue;let m=y.last_sf_sync?new Date(y.last_sf_sync).getTime():0;u.stat.mtime>m&&s.push(u)}if(s.length===0){console.log("[Eudia SF Sync] No flagged notes need syncing"),this.updateSfSyncStatusBar("SF Sync: Idle");return}console.log(`[Eudia SF Sync] ${s.length} note(s) queued for sync`),this.updateSfSyncStatusBar(`SF Sync: Syncing ${s.length}...`);let i=0,r=0;for(let u of s){let h=await this.syncSpecificNoteToSalesforce(u);if(h.success)i++,await this.updateNoteSyncTimestamp(u);else if(r++,console.log(`[Eudia SF Sync] Failed to sync ${u.path}: ${h.error}`),h.authRequired){new l.Notice("Salesforce authentication expired. Please reconnect to resume auto-sync."),this.updateSfSyncStatusBar("SF Sync: Auth required");return}}let c=new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),d=r>0?`SF Sync: ${i} synced, ${r} failed at ${c}`:`SF Sync: ${i} note${i!==1?"s":""} synced at ${c}`;console.log(`[Eudia SF Sync] ${d}`),this.updateSfSyncStatusBar(d),i>0&&new l.Notice(d)}catch(e){console.error("[Eudia SF Sync] Scan error:",e),this.updateSfSyncStatusBar("SF Sync: Error")}}}async updateNoteSyncTimestamp(e){try{let t=await this.app.vault.read(e),n=new Date().toISOString();if(t.startsWith("---")){let s=t.indexOf("---",3);if(s!==-1){let i=t.substring(0,s),r=t.substring(s);if(i.includes("last_sf_sync:")){let o=i.replace(/last_sf_sync:.*/,`last_sf_sync: "${n}"`)+r;await this.app.vault.modify(e,o)}else{let o=i+`last_sf_sync: "${n}"
`+r;await this.app.vault.modify(e,o)}}}}catch(t){console.error(`[Eudia SF Sync] Failed to update sync timestamp for ${e.path}:`,t)}}updateSfSyncStatusBar(e){this.sfSyncStatusBarEl&&this.sfSyncStatusBarEl.setText(e)}async createMeetingNote(){return new Promise(e=>{new X(this.app,this,async n=>{if(!n){e();return}let s=new Date().toISOString().split("T")[0],i=n.name.replace(/[<>:"/\\|?*]/g,"_").trim(),r=`${this.settings.accountsFolder}/${i}`,o=`${s} Meeting.md`,c=`${r}/${o}`;this.app.vault.getAbstractFileByPath(r)||await this.app.vault.createFolder(r);let d=`---
...
`,u=await this.app.vault.create(c,d);await this.app.workspace.getLeaf().openFile(u),new l.Notice(`Created meeting note for ${n.name}`),e()}).open()})}async fetchAndInsertContext(){new l.Notice("Fetching pre-call context...")}async refreshAnalyticsDashboard(e){if(!this.settings.userEmail){console.log("[Eudia] Cannot refresh analytics - no email configured");return}let n=(await this.app.vault.read(e)).match(/^---\n([\s\S]*?)\n---/);if(!n)return;let s=n[1];if(!s.includes("type: analytics_dashboard"))return;let i=s.match(/category:\s*(\w+)/)?.[1]||"team";console.log(`[Eudia] Refreshing analytics dashboard: ${e.name} (${i})`);try{let r=null,o=this.settings.serverUrl,c=encodeURIComponent(this.settings.userEmail);switch(i){case"pain_points":r=(await(0,l.requestUrl)({url:`${o}/api/analytics/pain-points?days=30`,method:"GET"})).json,r.success&&await this.updatePainPointNote(e,r.painPoints);break;case"objections":r=(await(0,l.requestUrl)({url:`${o}/api/analytics/objection-playbook?days=90`,method:"GET"})).json,r.success&&await this.updateObjectionNote(e,r);break;case"coaching":case"team":default:r=(await(0,l.requestUrl)({url:`${o}/api/analytics/team-trends?managerId=${c}`,method:"GET"})).json,r.success&&await this.updateTeamPerformanceNote(e,r.trends);break}r?.success&&new l.Notice(`Analytics refreshed: ${e.name}`)}catch(r){console.error("[Eudia] Analytics refresh error:",r)}}async updatePainPointNote(e,t){if(!t||t.length===0)return;let n=new Date().toISOString().split("T")[0],s=t.slice(0,10).map(d=>`| ${d.painPoint||"--"} | ${d.count||0} | ${d.category||"--"} | ${d.averageSeverity||"medium"} |`).join(`
`),i={};for(let d of t){let u=d.category||"other";i[u]||(i[u]=[]),i[u].push(d)}let r="";for(let[d,u]of Object.entries(i)){r+=`
### ${d.charAt(0).toUpperCase()+d.slice(1)}
`;for(let h of u.slice(0,3))r+=`- ${h.painPoint}
...
`;await this.app.vault.modify(e,r)}async updateTeamPerformanceNote(e,t){if(!t)return;let n=new Date().toISOString().split("T")[0],s=r=>r>0?`\u2191 ${Math.abs(r).toFixed(1)}%`:r<0?`\u2193 ${Math.abs(r).toFixed(1)}%`:"--",i=`---
type: analytics_dashboard
auto_refresh: true
...
`;await this.app.vault.modify(e,i)}},ee=class extends l.PluginSettingTab{constructor(a,e){super(a,e),this.plugin=e}display(){let{containerEl:a}=this;a.empty(),a.createEl("h2",{text:"Eudia Sync & Scribe"}),a.createEl("h3",{text:"Your Profile"});let e=a.createDiv();e.style.cssText="padding: 16px; background: var(--background-secondary); border-radius: 8px; margin-bottom: 16px; margin-top: 16px;";let t=e.createDiv();t.style.cssText="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;";let n=t.createSpan(),s=t.createSpan(),i=e.createDiv();i.style.cssText="font-size: 12px; color: var(--text-muted); margin-bottom: 16px;",i.setText("Connect with Salesforce to sync notes with your user attribution.");let r=e.createEl("button");r.style.cssText="padding: 10px 20px; cursor: pointer; border-radius: 6px;";let o=null,c=async()=>{if(!this.plugin.settings.userEmail)return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted);",s.setText("Enter email above first"),r.setText("Setup Required"),r.disabled=!0,r.style.opacity="0.5",r.style.cursor="not-allowed",!1;r.disabled=!1,r.style.opacity="1",r.style.cursor="pointer";try{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); animation: pulse 1s infinite;",s.setText("Checking..."),(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/sf/auth/status?email=${encodeURIComponent(this.plugin.settings.userEmail)}`,method:"GET",throw:!1})).json?.authenticated===!0?(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #22c55e;",s.setText("Connected to Salesforce"),r.setText("Reconnect"),this.plugin.settings.salesforceConnected=!0,await this.plugin.saveSettings(),!0):(n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #f59e0b;",s.setText("Not connected"),r.setText("Connect to Salesforce"),!1)}catch{return n.style.cssText="width: 8px; height: 8px; border-radius: 50%; background: #ef4444;",s.setText("Status unavailable"),r.setText("Connect to Salesforce"),!1}};new l.Setting(a).setName("Eudia Email").setDesc("Your @eudia.com email address for calendar and Salesforce sync").addText(m=>m.setPlaceholder("yourname@eudia.com").setValue(this.plugin.settings.userEmail).onChange(async p=>{let f=p.trim().toLowerCase();this.plugin.settings.userEmail=f,await this.plugin.saveSettings(),await c()})),new l.Setting(a).setName("Timezone").setDesc("Your local timezone for calendar event display").addDropdown(m=>{ce.forEach(p=>{m.addOption(p.value,p.label)}),m.setValue(this.plugin.settings.timezone),m.onChange(async p=>{this.plugin.settings.timezone=p,await this.plugin.saveSettings(),this.plugin.calendarService?.setTimezone(p),new l.Notice(`Timezone set to ${ce.find(f=>f.value===p)?.label||p}`)})}),a.createEl("h3",{text:"Salesforce Connection"}),a.appendChild(e);let d=()=>{o&&window.clearInterval(o);let m=0,p=30;o=window.setInterval(async()=>{m++,await c()?(o&&(window.clearInterval(o),o=null),new l.Notice("Salesforce connected successfully!")):m>=p&&o&&(window.clearInterval(o),o=null)},5e3)};r.onclick=async()=>{if(!this.plugin.settings.userEmail){new l.Notice("Please enter your email first");return}let m=`${this.plugin.settings.serverUrl}/api/sf/auth/start?email=${encodeURIComponent(this.plugin.settings.userEmail)}`;window.open(m,"_blank"),new l.Notice("Complete the Salesforce login in the popup window",5e3),d()},c(),a.createEl("h3",{text:"Server"}),new l.Setting(a).setName("GTM Brain Server").setDesc("Server URL for calendar, accounts, and sync").addText(m=>m.setValue(this.plugin.settings.serverUrl).onChange(async p=>{this.plugin.settings.serverUrl=p,await this.plugin.saveSettings()}));let u=a.createDiv({cls:"settings-advanced-collapsed"}),h=u.createDiv({cls:"eudia-transcription-status"});h.style.cssText="padding: 12px; background: var(--background-secondary); border-radius: 6px; margin-bottom: 12px; font-size: 13px;",h.innerHTML='<span style="color: var(--text-muted);">Checking server transcription status...</span>',(async()=>{try{(await(0,l.requestUrl)({url:`${this.plugin.settings.serverUrl}/api/plugin/config`,method:"GET"})).json?.capabilities?.serverTranscription?h.innerHTML='<span class="eudia-check-icon"></span> Server transcription is available. No local API key needed.':h.innerHTML='<span class="eudia-warn-icon"></span> Server transcription unavailable. Add a local API key below.'}catch{h.innerHTML='<span style="color: #f59e0b;">\u26A0</span> Could not check server status. Local API key recommended as backup.'}})();let y=new l.Setting(a).setName("Advanced Options").setDesc("Show fallback API key (usually not needed)").addToggle(m=>m.setValue(!1).onChange(p=>{u.style.display=p?"block":"none"}));u.style.display="none",a.createEl("h3",{text:"Transcription"}),new l.Setting(a).setName("Save Audio Files").setDesc("Keep original audio recordings").addToggle(m=>m.setValue(this.plugin.settings.saveAudioFiles).onChange(async p=>{this.plugin.settings.saveAudioFiles=p,await this.plugin.saveSettings()})),new l.Setting(a).setName("Append Full Transcript").setDesc("Include complete transcript in notes").addToggle(m=>m.setValue(this.plugin.settings.appendTranscript).onChange(async p=>{this.plugin.settings.appendTranscript=p,await this.plugin.saveSettings()})),a.createEl("h3",{text:"Sync"}),new l.Setting(a).setName("Sync on Startup").setDesc("Automatically sync accounts when Obsidian opens").addToggle(m=>m.setValue(this.plugin.settings.syncOnStartup).onChange(async p=>{this.plugin.settings.syncOnStartup=p,await this.plugin.saveSettings()})),new l.Setting(a).setName("Auto-Sync After Transcription").setDesc("Push notes to Salesforce after transcription").addToggle(m=>m.setValue(this.plugin.settings.autoSyncAfterTranscription).onChange(async p=>{this.plugin.settings.autoSyncAfterTranscription=p,await this.plugin.saveSettings()})),new l.Setting(a).setName("Auto-Sync Flagged Notes").setDesc("Periodically push notes with sync_to_salesforce: true to Salesforce").addToggle(m=>m.setValue(this.plugin.settings.sfAutoSyncEnabled).onChange(async p=>{this.plugin.settings.sfAutoSyncEnabled=p,await this.plugin.saveSettings(),p?this.plugin.startSalesforceSyncScanner():this.plugin.updateSfSyncStatusBar("SF Sync: Off")})),new l.Setting(a).setName("Auto-Sync Interval").setDesc("How often to scan for flagged notes (in minutes)").addDropdown(m=>{m.addOption("5","Every 5 minutes"),m.addOption("15","Every 15 minutes"),m.addOption("30","Every 30 minutes"),m.setValue(String(this.plugin.settings.sfAutoSyncIntervalMinutes)),m.onChange(async p=>{this.plugin.settings.sfAutoSyncIntervalMinutes=parseInt(p),await this.plugin.saveSettings(),new l.Notice(`SF auto-sync interval set to ${p} minutes. Restart Obsidian for changes to take effect.`)})}),a.createEl("h3",{text:"Folders"}),new l.Setting(a).setName("Accounts Folder").setDesc("Where account folders are stored").addText(m=>m.setValue(this.plugin.settings.accountsFolder).onChange(async p=>{this.plugin.settings.accountsFolder=p||"Accounts",await this.plugin.saveSettings()})),new l.Setting(a).setName("Recordings Folder").setDesc("Where audio files are saved").addText(m=>m.setValue(this.plugin.settings.recordingsFolder).onChange(async p=>{this.plugin.settings.recordingsFolder=p||"Recordings",await this.plugin.saveSettings()})),a.createEl("h3",{text:"Actions"}),new l.Setting(a).setName("Sync Accounts Now").setDesc(`${this.plugin.settings.cachedAccounts.length} accounts available for matching`).addButton(m=>m.setButtonText("Sync").setCta().onClick(async()=>{await this.plugin.syncAccounts(),this.display()})),new l.Setting(a).setName("Refresh Account Folders").setDesc("Check for new account assignments and create folders for them").addButton(m=>m.setButtonText("Refresh Folders").onClick(async()=>{m.setButtonText("Checking..."),m.setDisabled(!0);try{let p=await this.plugin.refreshAccountFolders();p>0?new l.Notice(`Created ${p} new account folder${p>1?"s":""}`):new l.Notice("All account folders are up to date")}catch(p){new l.Notice("Failed to refresh folders: "+p.message)}m.setButtonText("Refresh Folders"),m.setDisabled(!1),this.display()})),this.plugin.settings.lastSyncTime&&a.createEl("p",{text:`Last synced: ${new Date(this.plugin.settings.lastSyncTime).toLocaleString()}`,cls:"setting-item-description"}),a.createEl("p",{text:`Audio transcription: ${E.isSupported()?"Supported":"Not supported"}`,cls:"setting-item-description"})}};
