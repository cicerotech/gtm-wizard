...
describe('Contact Enrichment Comprehensive Tests', () => {
  ...
  describe('Stress Tests (Fixtures)', () => {
    test('should handle batch of name+company lookups', async () => {
      if (!testFixtures?.testCases || testFixtures.testCases.length === 0) {
        console.log('  Skipping batch test - no test fixtures loaded');
        testResults.skipped++;
        return;
      }
      ...
      if (nameCompanyTests.length === 0) {
        console.log('  Skipping batch test - no name_company test cases');
        testResults.skipped++;
        return;
      }
