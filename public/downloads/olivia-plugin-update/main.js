...
`}];for(let h of m){let d=`${o}/${h.name}`;await this.app.vault.create(d,h.content)}a++,console.log(`[Eudia] Created account folder with subnotes: ${r}`)}catch(u){console.error(`[Eudia] Failed to create folder for ${r}:`,u)}}this.settings.cachedAccounts=e.map(s=>({id:s.id,name:s.name})),await this.saveSettings(),a>0&&new l.Notice(`Created ${a} account folders`),await this.ensureNextStepsFolderExists()}async createAdminAccountFolders(e){let t=this.settings.accountsFolder||"Accounts";this.app.vault.getAbstractFileByPath(t)||await this.app.vault.createFolder(t),await this.ensurePipelineFolderExists();let a=0,s=0,r=new Date().toISOString().split("T")[0];for(let o of e){let c=o.name.replace(/[<>:"/\\|?*]/g,"_").trim(),u=`${t}/${c}`;if(!(this.app.vault.getAbstractFileByPath(u)instanceof l.TFolder))try{await this.app.vault.createFolder(u),o.isOwned?(await this.createFullAccountSubnotes(u,o,r),a++):(await this.createViewOnlyAccountNote(u,o,r),s++),console.log(`[Eudia Admin] Created ${o.isOwned?"owned":"view-only"} folder: ${c}`)}catch(h){console.error(`[Eudia Admin] Failed to create folder for ${c}:`,h)}}this.settings.cachedAccounts=e.map(o=>({id:o.id,name:o.name})),await this.saveSettings(),a+s>0&&new l.Notice(`Created ${a} owned + ${s} view-only account folders`),await this.ensureNextStepsFolderExists()}async createViewOnlyAccountNote(e,t,i){let a=t.ownerName||"Unknown",s=`---
account: "${t.name}"
account_id: "${t.id}"
type: view_only
owner: "${a}"
...
`,r=`${e}/_Account Info.md`;await this.app.vault.create(r,s)}async createFullAccountSubnotes(e,t,i){let a=[{name:"Note 1.md",content:`---
account: "${t.name}"
account_id: "${t.id}"
type: meeting_note
sync_to_salesforce: false
...
*Previous next steps will be archived here.*
`}];for(let s of a){let r=`${e}/${s.name}`;await this.app.vault.create(r,s.content)}}async ensurePipelineFolderExists(){let e="Pipeline",t=`${e}/Pipeline Review Notes.md`;if(this.app.vault.getAbstractFileByPath(e)||await this.app.vault.createFolder(e),!this.app.vault.getAbstractFileByPath(t)){let r=`---
...
${y}
`;await this.app.vault.modify(r,v),console.log(`[Eudia] Updated Next Steps for ${e} (history preserved)`),await this.regenerateNextStepsDashboard()}catch(a){console.error(`[Eudia] Failed to update Next Steps for ${e}:`,a)}}async regenerateNextStepsDashboard(){try{let t=this.app.vault.getAbstractFileByPath("Next Steps/All Next Steps.md");if(!t||!(t instanceof l.TFile)){await this.ensureNextStepsFolderExists();return}let i=this.app.vault.getAbstractFileByPath(this.settings.accountsFolder);if(!i||!(i instanceof l.TFolder))return;let a=[];for(let c of i.children)if(c instanceof l.TFolder){let u=`${c.path}/Next Steps.md`,m=this.app.vault.getAbstractFileByPath(u);if(m instanceof l.TFile){let h=await this.app.vault.read(m),d=h.match(/last_updated:\s*(\d{4}-\d{2}-\d{2})/),p=d?d[1]:"Unknown",g=h.split(`
`).filter(y=>y.match(/^- \[[ x]\]/)).slice(0,5);(g.length>0||p!=="Unknown")&&a.push({account:c.name,lastUpdated:p,nextSteps:g})}}a.sort((c,u)=>u.lastUpdated.localeCompare(c.lastUpdated));let s=new Date().toISOString().split("T")[0],r=new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}),o=`---
type: next_steps_dashboard
auto_updated: true
last_updated: ${s}
---
...
---
`;await this.app.vault.modify(t,r+o),this.processTranscriptionAsync(e,t).catch(c=>{console.error("Background transcription failed:",c),new l.Notice(`Transcription failed: ${c.message}`)})}async processTranscriptionAsync(e,t){try{let i,a=t.path.split("/");if(console.log(`[Eudia] Processing transcription for: ${t.path}`),console.log(`[Eudia] Path parts: ${JSON.stringify(a)}, accountsFolder: ${this.settings.accountsFolder}`),a.length>=2&&a[0]===this.settings.accountsFolder){let d=a[1];console.log(`[Eudia] Detected account folder: ${d}`);let p=this.settings.cachedAccounts.find(g=>g.name.toLowerCase()===d.toLowerCase());p?(i={accountName:p.name,accountId:p.id},console.log(`[Eudia] Found cached account: ${p.name} (${p.id})`)):(i={accountName:d,accountId:""},console.log(`[Eudia] Account not in cache, using folder name: ${d}`))}else console.log("[Eudia] File not in Accounts folder, skipping account context");let s=[];try{let d=await this.calendarService.getCurrentMeeting();d.meeting?.attendees&&(s=d.meeting.attendees.map(p=>p.name||p.email.split("@")[0]).filter(Boolean).slice(0,10))}catch{}let r=await this.transcriptionService.transcribeAudio(e.audioBlob,i?{...i,speakerHints:s}:{speakerHints:s}),o=d=>d?!!(d.summary?.trim()||d.nextSteps?.trim()):!1,c=r.sections;if(o(c)||r.text?.trim()&&(c=await this.transcriptionService.processTranscription(r.text,i)),!o(c)&&!r.text?.trim()){let p=(await this.app.vault.read(t)).replace(/\n\n---\n\*\*Transcription in progress\.\.\.\*\*[\s\S]*?\*You can navigate away\. Check back shortly\.\*\n---\n/g,"");await this.app.vault.modify(t,p+`
...
`):typeof y=="object"?JSON.stringify(y):String(y),a=i(e.title)||"Meeting Notes",s=i(e.summary),r=i(e.painPoints),o=i(e.productInterest),c=i(e.meddiccSignals),u=i(e.nextSteps),m=i(e.actionItems),h=i(e.keyDates),d=i(e.risksObjections),p=i(e.attendees),g=`---
title: "${a}"
